...
inline bool Options::checkProfilePath(Glib::ustring& path)
{
    ...
    if (!p.empty()
        && Glib::file_test(path + App::PARAM_FILE_EXTENSION, Glib::FILE_TEST_EXISTS))
    {
        return true;
    }
...
void Options::updatePaths()
{
    ...
    if (Glib::path_is_absolute(profilePath)) {
        ...
        if (!checkDirPath(profilePath, "")) {
            ...
            if (!checkDirPath(profilePath, ""))
            {  // had problems with mkdir_with_parents return value on OS X, just check
               // dir again
                Glib::ustring msg = Glib::ustring::compose(
                    "Creation of the user's processing profile directory \"%1\" failed!\n",
                    profilePath);
                throw Error(msg);
            }
        }
                ...
                "Error: the user's processing profile path doesn't point to a directory or doesn't exist!\n"))
        {
            ...
            if (checkDirPath(
                    tmpPath,
                    "Error: the global's processing profile path doesn't point to a directory or doesn't exist!\n"))
            {
        ...
        if (checkDirPath(
                tmpPath,
                "Error: the user's processing profile path doesn't point to a directory!\n"))
        {
    ...
    if (lastRetinexDir.empty() || !Glib::file_test(lastRetinexDir, Glib::FILE_TEST_EXISTS)
        || !Glib::file_test(lastLabCurvesDir, Glib::FILE_TEST_IS_DIR))
    {
    ...
    if (lastlocalCurvesDirghs.empty()
        || !Glib::file_test(lastlocalCurvesDirghs, Glib::FILE_TEST_EXISTS)
        || !Glib::file_test(lastlocalCurvesDirghs, Glib::FILE_TEST_IS_DIR))
    {
        lastlocalCurvesDirghs = preferredPath;
    }

    if (lastPFCurvesDir.empty()
        || !Glib::file_test(lastPFCurvesDir, Glib::FILE_TEST_EXISTS)
        || !Glib::file_test(lastPFCurvesDir, Glib::FILE_TEST_IS_DIR))
    {
    ...
    if (lastIcmCurvesDir.empty()
        || !Glib::file_test(lastIcmCurvesDir, Glib::FILE_TEST_EXISTS)
        || !Glib::file_test(lastIcmCurvesDir, Glib::FILE_TEST_IS_DIR))
    {
        lastIcmCurvesDir = preferredPath;
    }

    if (lastProfilingReferenceDir.empty()
        || !Glib::file_test(lastProfilingReferenceDir, Glib::FILE_TEST_EXISTS)
        || !Glib::file_test(lastProfilingReferenceDir, Glib::FILE_TEST_IS_DIR))
    {
 ...
 *can be provided for backward compatibility. In this case, this parameter will be updated
 *with the new format.
 *@return Send back the absolute path of the given filename or "Neutral" if "Neutral" has
 *been set to profName. Implementer will have to test for this particular value. If the
 *absolute path is invalid (e.g. the file doesn't exist), it will return an empty string.
 */
Glib::ustring Options::findProfilePath(Glib::ustring& profName) const
{
    ...
    if (p == "${U}") {
        ...
        Glib::ustring fullPath =
            Glib::build_filename(p, profName.substr(5) + App::PARAM_FILE_EXTENSION);
        ...
        p = getUserProfilePath();
        Glib::ustring fullPath =
            Glib::build_filename(p, profName + App::PARAM_FILE_EXTENSION);
...
void Options::setDefaults()
{
    ...
    saveParamsFile = true;  // was false, but saving the procparams files next to the file
                            // make more sense when reorganizing file tree than in a cache
    ...
    // There is no reasonable default for curves. We can still suppose that they will take
    // place in a subdirectory of the user's own ProcParams presets, i.e. in a
...
void Options::readFromFile(Glib::ustring fname)
{
    ...
    try {
        if (keyFile.load_from_file(fname)) {
                    ...
                    || keyFile.has_key("External Editor", "IconsSerialized"))
                {
                    ...
                    const auto& icons_serialized =
                        !keyFile.has_key("External Editor", "IconsSerialized")
                            ? std::vector<Glib::ustring>()
                            : static_cast<std::vector<Glib::ustring>>(
                                  keyFile.get_string_list("External Editor",
                                                          "IconsSerialized"));
                    ...
                    auto executable =
                        Glib::build_filename(gimpDir, "bin", "gimp-win-remote");
                    if (Glib::file_test(executable, Glib::FILE_TEST_IS_EXECUTABLE)) {
                        ...
                        for (auto ver = 12; ver >= 0; --ver) {
                            executable = Glib::build_filename(
                                gimpDir, "bin",
                                Glib::ustring::compose(Glib::ustring("gimp-2.%1.exe"),
                                                       ver));
                            if (Glib::file_test(executable,
                    ...
                    if (Glib::find_program_in_path("gimp").compare("")) {
                        ...
                        externalEditors.emplace_back("GIMP", "gimp", true,
                                                     gimp_icon_serialized);
                    } else if (Glib::find_program_in_path("gimp-remote").compare("")) {
                        if (editorToSendTo == 1) {
                            externalEditorIndex = externalEditors.size();
                        }
                        externalEditors.emplace_back("GIMP", "gimp-remote", true,
                                                     gimp_icon_serialized);
                    }
            ...
            if (keyFile.has_group("Profiles")) {
                ...
                if (keyFile.has_key("Profiles", "CustomProfileBuilderKeys")) {
                    CPBKeys = (CPBKeyType)keyFile.get_integer("Profiles",
                                                              "CustomProfileBuilderKeys");
                }
            }
            ...
            if (keyFile.has_group("File Browser")) {
                ...
                if (keyFile.has_key("File Browser", "ParseExtensionsEnabled")) {
                    auto l = keyFile.get_integer_list("File Browser",
                ...
                if (keyFile.has_key("File Browser", "menuGroupProfileOperations")) {
                    menuGroupProfileOperations =
                        keyFile.get_boolean("File Browser", "menuGroupProfileOperations");
                }
                ...
                if (keyFile.has_key("File Browser", "BrowseRecursiveFollowLinks")) {
                    browseRecursiveFollowLinks =
                        keyFile.get_boolean("File Browser", "BrowseRecursiveFollowLinks");
                }
            ...
            if (keyFile.has_group("Performance")) {
                ...
                if (keyFile.has_key("Performance", "ChunkSizeAMAZE")) {
                    chunkSizeAMAZE =
                        std::min(16, std::max(1, keyFile.get_integer("Performance",
                                                                     "ChunkSizeAMAZE")));
                }
                ...
                if (keyFile.has_key("Performance", "ChunkSizeRCD")) {
                    chunkSizeRCD = std::min(
                        16,
                        std::max(1, keyFile.get_integer("Performance", "ChunkSizeRCD")));
                }

                if (keyFile.has_key("Performance", "ChunkSizeRGB")) {
                    chunkSizeRGB = std::min(
                        16,
                        std::max(1, keyFile.get_integer("Performance", "ChunkSizeRGB")));
                }
                ...
                if (keyFile.has_key("Performance", "ThumbnailInspectorMode")) {
                    rtSettings.thumbnail_inspector_mode =
                        static_cast<rtengine::Settings::ThumbnailInspectorMode>(
                            keyFile.get_integer("Performance", "ThumbnailInspectorMode"));
                }
            }
            ...
            if (keyFile.has_group("Crop Settings")) {
                ...
                if (keyFile.has_key("Crop Settings", "GuidesMode")) {
                    cropGuides = CropGuidesMode(std::max(
                        int(CROP_GUIDE_NONE),
                ...
                */
                /*
                                if (keyFile.has_key ("Color Management", "greySc")) {
                                    rtSettings.viewinggreySc = keyFile.get_integer ("Color
                   Management", "greySc");
                                }
                */
                ...
                if (keyFile.has_key("Color Management", "Previewselection"))
                {  // Intensity of preview selection deltaE
                    rtSettings.previewselection =
                        keyFile.get_integer("Color Management", "Previewselection");
                }

                if (keyFile.has_key("Color Management", "Cbdlsensi"))
                {  // sensibility to crash for cbdl
                    rtSettings.cbdlsensi =
                        keyFile.get_double("Color Management", "Cbdlsensi");
                }

                if (keyFile.has_key("Color Management", "Besecorlog"))
                {  // sensi base log for Q
                    rtSettings.basecorlog =
            ...
            if (keyFile.has_group("ICC Profile Creator")) {
                ...
                if (keyFile.has_key("ICC Profile Creator", "AppendParamsToDesc")) {
                    ICCPC_appendParamsToDesc =
                        keyFile.get_boolean("ICC Profile Creator", "AppendParamsToDesc");
                }
            }
            ...
            if (keyFile.has_group("Batch Processing")) {
                if (keyFile.has_key("Batch Processing", "AdjusterBehavior")) {
                    baBehav =
                        keyFile.get_integer_list("Batch Processing", "AdjusterBehavior");
            ...
            if (keyFile.has_group("Fast Export")) {
                ...
                // fastexport_bypass_lumaDenoise = keyFile.get_boolean ("Fast Export",
                // "fastexport_bypass_lumaDenoise" ); if (keyFile.has_key ("Fast Export",
                // "fastexport_bypass_colorDenoise" )) fastexport_bypass_colorDenoise =
                // keyFile.get_boolean ("Fast Export", "fastexport_bypass_colorDenoise" );
                if (keyFile.has_key("Fast Export", "fastexport_bypass_defringe")) {
                    fastexport_bypass_defringe =
                        keyFile.get_boolean("Fast Export", "fastexport_bypass_defringe");
                }
                ...
                if (keyFile.has_key("Fast Export", "fastexport_raw_bayer_method")) {
                    fastexport_raw_bayer_method =
                        keyFile.get_string("Fast Export", "fastexport_raw_bayer_method");
                }
                ...
                if (keyFile.has_key("Fast Export",
                                    "fastexport_bypass_raw_lmmse_iterations"))
                {
                ...
                if (keyFile.has_key("Fast Export", "fastexport_raw_xtrans_method")) {
                    fastexport_raw_xtrans_method =
                        keyFile.get_string("Fast Export", "fastexport_raw_xtrans_method");
                }
                ...
                if (keyFile.has_key("Fast Export", "fastexport_resize_appliesTo")) {
                    fastexport_resize_appliesTo =
                        keyFile.get_string("Fast Export", "fastexport_resize_appliesTo");
                }
                ...
                if (keyFile.has_key("Fast Export", "fastexport_resize_dataspec")) {
                    fastexport_resize_dataspec =
                        keyFile.get_integer("Fast Export", "fastexport_resize_dataspec");
                }
                ...
                if (keyFile.has_key("Fast Export", "fastexport_resize_longedge")) {
                    fastexport_resize_longedge =
                        keyFile.get_integer("Fast Export", "fastexport_resize_longedge");
                }

                if (keyFile.has_key("Fast Export", "fastexport_resize_shortedge")) {
                    fastexport_resize_shortedge =
                        keyFile.get_integer("Fast Export", "fastexport_resize_shortedge");
                }
            ...
            if (keyFile.has_group("Dialogs")) {
                ...
                safeDirGet(keyFile, "Dialogs", "LastCameraProfilesDir",
                           lastCameraProfilesDir);
                safeDirGet(keyFile, "Dialogs", "LastLensProfilesDir",
                           lastLensProfilesDir);
                ...
                safeDirGet(keyFile, "Dialogs", "LastDenoiseCurvesDir",
                           lastDenoiseCurvesDir);
                safeDirGet(keyFile, "Dialogs", "LastWaveletCurvesDir",
                           lastWaveletCurvesDir);
                safeDirGet(keyFile, "Dialogs", "LastlocalCurvesDir", lastlocalCurvesDir);
                safeDirGet(keyFile, "Dialogs", "LastlocalCurvesDirghs",
                           lastlocalCurvesDirghs);
                ...
                safeDirGet(keyFile, "Dialogs", "LastVibranceCurvesDir",
                           lastVibranceCurvesDir);
                safeDirGet(keyFile, "Dialogs", "LastProfilingReferenceDir",
                           lastProfilingReferenceDir);
                safeDirGet(keyFile, "Dialogs", "LastLensProfileDir", lastLensProfileDir);
                safeDirGet(keyFile, "Dialogs", "LastICCProfCreatorDir",
                           lastICCProfCreatorDir);
        ...
        Glib::ustring msg = Glib::ustring::compose(
            "Options::readFromFile / Error code %1 while reading values from \"%2\":\n%3",
...
bool Options::safeDirGet(const Glib::KeyFile& keyFile,
...
void Options::saveToFile(Glib::ustring fname)
{
    ...
    try {
        ...
        keyFile.set_string("General", "CameraProfilesPath",
                           rtSettings.cameraProfilesPath);
        ...
        keyFile.set_boolean("External Editor", "BypassOutputProfile",
                            editor_bypass_output_profile);

        {
            ...
            keyFile.set_boolean_list("External Editor", "NativeCommands",
                                     native_commands);
            keyFile.set_string_list("External Editor", "IconsSerialized",
                                    icons_serialized);
        ...
        keyFile.set_boolean("File Browser", "FilmStripOverlayedFileNames",
                            filmStripOverlayedFileNames);
        keyFile.set_boolean("File Browser", "ShowFileNames", showFileNames);
        keyFile.set_boolean("File Browser", "FilmStripShowFileNames",
                            filmStripShowFileNames);
        keyFile.set_boolean("File Browser", "InternalThumbIfUntouched",
                            internalThumbIfUntouched);
        ...
        keyFile.set_boolean("File Browser", "menuGroupFileOperations",
                            menuGroupFileOperations);
        keyFile.set_boolean("File Browser", "menuGroupProfileOperations",
                            menuGroupProfileOperations);
        ...
        keyFile.set_integer("File Browser", "MaxRecentFolders", maxRecentFolders);
        {
            ...
            for (unsigned int i = 0; i < std::min(recentFolders.size(), maxRecentFolders);
                 i++)
            {
        ...
        keyFile.set_integer("File Browser", "BrowseRecursiveMaxDirs",
                            browseRecursiveMaxDirs);
        keyFile.set_boolean("File Browser", "BrowseRecursiveFollowLinks",
                            browseRecursiveFollowLinks);
        keyFile.set_integer("Clipping Indication", "HighlightThreshold",
                            highlightThreshold);
        keyFile.set_integer("Clipping Indication", "ShadowThreshold", shadowThreshold);
        keyFile.set_boolean("Clipping Indication", "BlinkClipped", blinkClipped);

        keyFile.set_integer("Performance", "RgbDenoiseThreadLimit",
                            rgbDenoiseThreadLimit);
        ...
        keyFile.set_boolean("Output", "TiffUncompressedBatch",
                            saveFormatBatch.tiffUncompressed);
        ...
        keyFile.set_boolean("GUI", "FileBrowserToolbarSingleRow",
                            FileBrowserToolbarSingleRow);
        ...
        keyFile.set_string("Color Management", "PrinterProfile",
                           rtSettings.printerProfile);
        keyFile.set_integer("Color Management", "PrinterIntent",
        ...
        keyFile.set_string("Color Management", "MonitorProfile",
                           rtSettings.monitorProfile);
        keyFile.set_boolean("Color Management", "AutoMonitorProfile",
                            rtSettings.autoMonitorProfile);
        ...
        keyFile.set_integer("Color Management", "WhiteBalanceSpotSize",
                            whiteBalanceSpotSize);
        ...
        keyFile.set_integer("Color Management", "DenoiseLabgamma",
                            rtSettings.denoiselabgamma);
        ...
        keyFile.set_double("Color Management", "Itcwb_deltaspec",
                           rtSettings.itcwb_deltaspec);
        keyFile.set_double("Color Management", "Itcwb_powponder",
                           rtSettings.itcwb_powponder);
        ...
        keyFile.set_integer("Color Management", "Previewselection",
                            rtSettings.previewselection);
        ...
        keyFile.set_string("ICC Profile Creator", "PimariesPreset",
                           ICCPC_primariesPreset);
        ...
        keyFile.set_boolean("ICC Profile Creator", "AppendParamsToDesc",
                            ICCPC_appendParamsToDesc);
        ...
        // "fastexport_bypass_colorDenoise" , fastexport_bypass_colorDenoise);
        keyFile.set_boolean("Fast Export", "fastexport_bypass_defringe",
                            fastexport_bypass_defringe);
        ...
        keyFile.set_boolean("Fast Export", "fastexport_bypass_wavelet",
                            fastexport_bypass_wavelet);
        keyFile.set_string("Fast Export", "fastexport_raw_bayer_method",
                           fastexport_raw_bayer_method);
        ...
        keyFile.set_boolean("Fast Export", "fastexport_bypass_raw_bayer_dcb_enhance",
                            fastexport_bypass_raw_bayer_dcb_enhance);
        keyFile.set_boolean("Fast Export", "fastexport_bypass_raw_bayer_lmmse_iterations",
                            fastexport_bypass_raw_bayer_lmmse_iterations);
        ...
        keyFile.set_boolean("Fast Export", "fastexport_bypass_raw_ca",
                            fastexport_bypass_raw_ca);
        keyFile.set_boolean("Fast Export", "fastexport_bypass_raw_df",
                            fastexport_bypass_raw_df);
        keyFile.set_boolean("Fast Export", "fastexport_bypass_raw_ff",
                            fastexport_bypass_raw_ff);
        keyFile.set_string("Fast Export", "fastexport_icm_input",
                           fastexport_icm_input_profile);
        keyFile.set_string("Fast Export", "fastexport_icm_working",
                           fastexport_icm_working_profile);
        keyFile.set_string("Fast Export", "fastexport_icm_output",
                           fastexport_icm_output_profile);
        keyFile.set_integer("Fast Export", "fastexport_icm_output_intent",
                            fastexport_icm_outputIntent);
        keyFile.set_boolean("Fast Export", "fastexport_icm_output_bpc",
                            fastexport_icm_outputBPC);
        keyFile.set_boolean("Fast Export", "fastexport_resize_enabled",
                            fastexport_resize_enabled);
        keyFile.set_double("Fast Export", "fastexport_resize_scale",
                           fastexport_resize_scale);
        keyFile.set_string("Fast Export", "fastexport_resize_appliesTo",
                           fastexport_resize_appliesTo);
        keyFile.set_string("Fast Export", "fastexport_resize_method",
                           fastexport_resize_method);
        keyFile.set_integer("Fast Export", "fastexport_resize_dataspec",
                            fastexport_resize_dataspec);
        keyFile.set_integer("Fast Export", "fastexport_resize_width",
                            fastexport_resize_width);
        keyFile.set_integer("Fast Export", "fastexport_resize_height",
                            fastexport_resize_height);
        keyFile.set_integer("Fast Export", "fastexport_resize_longedge",
                            fastexport_resize_longedge);
        keyFile.set_integer("Fast Export", "fastexport_resize_shortedge",
                            fastexport_resize_shortedge);
        ...
        keyFile.set_string("Dialogs", "LastProfilingReferenceDir",
                           lastProfilingReferenceDir);
        ...
        keyFile.set_boolean("Dialogs", "GimpPluginShowInfoDialog",
                            gimpPluginShowInfoDialog);
...
void Options::load(bool lightweight)
{
    ...
    if (path != nullptr) {
        ...
        if (SHGetSpecialFolderPathW(NULL, pathW, CSIDL_LOCAL_APPDATA, false)) {
            ...
            rtdir = Glib::build_filename(Glib::ustring(pathA),
                                         Glib::ustring(CACHEFOLDERNAME));
        }
        ...
        rtdir = Glib::build_filename(Glib::ustring(g_get_home_dir()),
                                     ...
                                     Glib::ustring(CACHEFOLDERNAME), "/config/");
#else
        rtdir = Glib::build_filename(Glib::ustring(g_get_user_config_dir()),
                                     Glib::ustring(CACHEFOLDERNAME));
#endif
#endif
    }
    ...
    if (path != nullptr) {
        ...
        if (!Glib::path_is_absolute(cacheBaseDir)) {
            Glib::ustring msg =
                Glib::ustring::compose("Cache base dir %1 is not absolute", cacheBaseDir);
            throw Error(msg);
        }
    }
    ...
    else if (options.multiUser)
    {
        ...
        cacheBaseDir = Glib::build_filename(Glib::ustring(g_get_home_dir()),
                                            "/Library/Application Support/",
                                            Glib::ustring(CACHEFOLDERNAME), "/cache/");
#else
        cacheBaseDir = Glib::build_filename(Glib::ustring(g_get_user_cache_dir()),
                                            Glib::ustring(CACHEFOLDERNAME));
#endif
#endif
    }
    ...
    if (options.defProfImg.empty()) {
        ...
        if (!options.findProfilePath(options.defProfImg).empty()) {
            if (options.rtSettings.verbose) {
                std::cout << "Default profile for non-raw images \"" << options.defProfImg
                          << "\" found" << std::endl;
            }
        } else {
    ...
    // We handle languages using a hierarchy of translations.  The top of the hierarchy is
    // default.  This includes a default translation for all items (most likely using
    ...
    // translation "HISTORY_MSG_34;Avoid Colour Clipping" where English would translate it
    // as "HISTORY_MSG_34;Avoid Color Clipping" (note the difference in the spelling of
    // 'colour').
    //
    // It is important that when naming the translation files, that you stick to the
    // format <Language> or <Language> (<LC>).  We depend on that to figure out which are
    // the parent translations.  Furthermore, there must be a file <Language> for each
    // locale <Language> (<LC>) -- you cannot have 'French (CA)' unless there is a file
    // 'French'.

    Glib::ustring defaultTranslation =
        Glib::build_filename(App::get().argv0(), "languages", "default");
    ...
    if (!options.language.empty()) {
        ...
        if (langPortions.size() >= 1) {
            languageTranslation =
                Glib::build_filename(App::get().argv0(), "languages", langPortions.at(0));
        }

        if (langPortions.size() >= 2) {
            localeTranslation =
                Glib::build_filename(App::get().argv0(), "languages", options.language);
        }
    }

    langMgr.load(options.language,
                 { localeTranslation, languageTranslation, defaultTranslation });
...
ExternalEditor::ExternalEditor(const Glib::ustring& name,
