...
void setExpandAlignProperties(Gtk::Widget* widget,
                              bool hExpand,
                              bool vExpand,
                              enum Gtk::Align hAlign,
                              enum Gtk::Align vAlign);
Gtk::Border getPadding(const Glib::RefPtr<Gtk::StyleContext> style);

/**
 * @class IdleRegister
 *
 * @brief A helper class for registering functions to be called asynchronously when there
 * are no higher priority events pending. Purpose of the IdleRegister is to make sure
 * in-flight idle functions queued by `IdleRegister::add()` are unregistered and not
 * called after destruction.
 *
 * Uses gdk_threads_add_idle_full
 *
 * Notes:
 * It's best to call `IdleRegister::destroy()` in the destructor of the class owning the
 * `IdleRegister` instance. Otherwise make sure, it is the last member which will be
 * deleted first.
 */
...
public:
    ...
    /**
     * Registers a function to be called from the GTK event main loop later when there are
     * no higher priority events pending. If the registered function returns false, it is
     * automatically cleared from the list of event sources and will not be called again.
     */
...
// by g_main_context_invoke(), g_idle_add() and related functions, but this will require
// more extensive changes. We silence those warnings until then so that we notice the
// others.
...
{
public:
    GThreadLock() { gdk_threads_enter(); }
    ~GThreadLock() { gdk_threads_leave(); }
};
...
{
public:
    GThreadUnLock() { gdk_threads_leave(); }
    ~GThreadUnLock() { gdk_threads_enter(); }
};
...
{
public:
    explicit ConnectionBlocker(Gtk::Widget* associatedWidget,
                               sigc::connection& connection)
        : connection(associatedWidget ? &connection : nullptr), wasBlocked(false)
    {
        if (this->connection) {
...
/**
 * @brief Glue box to control visibility of the MyExpender's content ; also handle the
 * frame around it
 */
 ...
 *
 * Custom made expander for responsive widgets in the header. It also handle a
 * "enabled/disabled" property that display a different arrow depending on this boolean
 * value.
 *
 * Warning: once you've instantiated this class with a text label or a widget label, you
 * won't be able to revert to the other solution.
 */
...
private:
    ...
    const Glib::ustring
        inconsistentImage;  /// "inconsistent" image, displayed when useEnabled is true ;
                            /// in this case, nothing will tell that an expander is
                            /// opened/closed
    const Glib::ustring enabledImage;  ///      "enabled" image, displayed when useEnabled
                                       ///      is true ; in this case, nothing will tell
                                       ///      that an expander is opened/closed
    const Glib::ustring
        disabledImage;  ///     "disabled" image, displayed when useEnabled is true ; in
                        ///     this case, nothing will tell that an expander is
                        ///     opened/closed
    const Glib::ustring
        openedImage;  ///       "opened" image, displayed when useEnabled is false
    const Glib::ustring
        closedImage;    ///       "closed" image, displayed when useEnabled is false
    ...
    Gtk::Box* headerHBox;
    bool flushEvent;  /// Flag to control the weird event mechanism of Gtk (please prove
                      /// me wrong!)
    ...
    /// Triggered on enabled/disabled change -> will emit a toggle event to the connected
    /// objects
...
protected:
    ...
    Gtk::Widget*
        headerWidget;  /// Widget to display in the header, next to the arrow image ; can
                       /// be NULL if the "string" version of the ctor has been used
    RTImage*
        statusImage;    /// Image to display the opened/closed status (if useEnabled is
                        /// false) of the enabled/disabled status (if useEnabled is true)
    Gtk::Label* label;  /// Text to display in the header, next to the arrow image ; can
                        /// be NULL if the "widget" version of the ctor has been used
    bool useEnabled;    /// Set whether to handle an enabled/disabled feature and display
                        /// the appropriate images

public:
    /** @brief Create a custom expander with a simple header made of a label.
     * @param useEnabled Set whether to handle an enabled/disabled toggle button and
     * display the appropriate image
     * @param titleLabel A string to display in the header. Warning: you won't be able to
     * switch to a widget label.
     */
     ...
     * @param useEnabled Set whether to handle an enabled/disabled toggle button and
     * display the appropriate image
     * @param titleWidget A widget to display in the header. Warning: you won't be able to
     * switch to a string label.
     */
    ...
    /// Set a new label string. If it has been instantiated with a Gtk::Widget, this
    ...
    bool getEnabled();
    /// If not inconsistent, set the enabled button to true or false and emit the message
    /// if the state is different If inconsistent, set the internal value to true or
    /// false, but do not update the image and do not emit the message
    ...
    /// Get the header widget. It'll send back the Gtk::Label* if it has been instantiated
    /// with a simple text
    ...
    /// Warning: do not manually Show/Hide the widget, because this parameter is handled
    /// by the click on the Expander's title
...
/**
 * @brief subclass of Gtk::ScrolledWindow in order to handle the large toolbars (wider
...
/**
 * @brief A helper method to connect the current folder property of a file chooser to an
 * arbitrary variable.
 */
...
void bindCurrentFolder(FileChooser& chooser, Glib::ustring& variable)
{
    chooser.signal_selection_changed().connect([&]() {
        ...
        if (!current_folder.empty()) variable = current_folder;
    });

    if (!variable.empty()) chooser.set_current_folder(variable);
}

typedef enum RTUpdatePolicy { RTUP_STATIC, RTUP_DYNAMIC } eUpdatePolicy;
...
typedef enum RTNav { NAV_NONE, NAV_NEXT, NAV_PREVIOUS } eRTNav;

/**
 * @brief Handle the switch between text and image to be displayed in the HBox (to be used
 * in a button/toolpanel)
 */
class TextOrIcon final : public Gtk::Box
{

public:
    TextOrIcon(const Glib::ustring& icon_name,
               const Glib::ustring& labelTx,
 ...
 */
class MyRadioImageMenuItem final :
    public Gtk::RadioMenuItem,
    public MyImageMenuItemInterface
{
    class Impl;
    std::unique_ptr<Impl> pimpl;

public:
    MyRadioImageMenuItem(const Glib::ustring& label,
                         RTImage* image,
...
/**
 * @brief Handle back buffers as automatically as possible, and suitable to be used with
 * Glib::RefPtr
 */
...
public:
    ...
    bool setDrawRectangle(Cairo::Format format,
                          ...
                          int newH,
                          bool updateBackBufferSize = true);
    // set the destination drawing location, do not modify other parameters like size and
    // offset. Use setDrawRectangle to set all parameters at the same time
    ...
    // you have to check if the surface is created thanks to surfaceCreated before
...
inline void
setActiveTextOrIndex(Gtk::ComboBoxText& comboBox, const Glib::ustring& text, int index)
{
    bool valueSet = false;
    if (!text.empty()) {
