      ...
      panel_listener(nullptr)
{
    ...
    setExpandAlignProperties(histogramRGBAreaHori.get(), true, false, Gtk::ALIGN_FILL,
                             Gtk::ALIGN_END);

    histogramRGBAreaVert.reset(new HistogramRGBAreaVert());
    setExpandAlignProperties(histogramRGBAreaVert.get(), false, true, Gtk::ALIGN_END,
                             Gtk::ALIGN_FILL);
    ...
    Gtk::Image* histImage =
        Gtk::manage(new RTImage("histogram-type-histogram-small", Gtk::ICON_SIZE_BUTTON));
    ...
    Gtk::Image* waveImage =
        Gtk::manage(new RTImage("histogram-type-waveform-small", Gtk::ICON_SIZE_BUTTON));
    ...
    setExpandAlignProperties(showGreen, false, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_CENTER);
    setExpandAlignProperties(showBlue, false, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_CENTER);
    setExpandAlignProperties(showValue, false, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_CENTER);
    setExpandAlignProperties(showChro, false, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_CENTER);
    setExpandAlignProperties(showMode, false, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_CENTER);
    setExpandAlignProperties(scopeOptions, false, false, Gtk::ALIGN_START,
                             Gtk::ALIGN_CENTER);
    ...
    setExpandAlignProperties(scopeHistRawBtn, true, false, Gtk::ALIGN_FILL,
                             Gtk::ALIGN_FILL);
    setExpandAlignProperties(scopeParadeBtn, true, false, Gtk::ALIGN_FILL,
                             Gtk::ALIGN_FILL);
    setExpandAlignProperties(scopeWaveBtn, true, false, Gtk::ALIGN_FILL, Gtk::ALIGN_FILL);
    setExpandAlignProperties(scopeVectHcBtn, true, false, Gtk::ALIGN_FILL,
                             Gtk::ALIGN_FILL);
    setExpandAlignProperties(scopeVectHsBtn, true, false, Gtk::ALIGN_FILL,
                             Gtk::ALIGN_FILL);
    setExpandAlignProperties(persistentButtons, false, true, Gtk::ALIGN_START,
                             Gtk::ALIGN_FILL);
    setExpandAlignProperties(optionButtons, false, true, Gtk::ALIGN_START,
                             Gtk::ALIGN_FILL);

    showRed->signal_toggled().connect(sigc::mem_fun(*this, &HistogramPanel::red_toggled),
                                      showRed);
    ...
    showBAR->signal_toggled().connect(sigc::mem_fun(*this, &HistogramPanel::bar_toggled),
                                      showBAR);
    ...
    scopeParadeBtn->signal_toggled().connect(
        sigc::bind(sigc::mem_fun(*this, &HistogramPanel::type_selected), scopeParadeBtn));
    scopeWaveBtn->signal_toggled().connect(
        sigc::bind(sigc::mem_fun(*this, &HistogramPanel::type_selected), scopeWaveBtn));
    scopeVectHcBtn->signal_toggled().connect(
        sigc::bind(sigc::mem_fun(*this, &HistogramPanel::type_selected), scopeVectHcBtn));
    scopeVectHsBtn->signal_toggled().connect(
        sigc::bind(sigc::mem_fun(*this, &HistogramPanel::type_selected), scopeVectHsBtn));

    brightnessWidget = Gtk::manage(new Gtk::Scale(Gtk::ORIENTATION_VERTICAL));
    brightnessWidget->set_inverted();
    brightnessWidget->set_range(log(HistogramArea::MIN_BRIGHT),
                                log(HistogramArea::MAX_BRIGHT));
    ...
    setExpandAlignProperties(brightnessWidget, true, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_START);
...
{
    histogramRGBAreaHori->set_visible(histogramRGBArea == histogramRGBAreaHori.get()
                                      && showBAR->get_active());
    histogramRGBAreaVert->set_visible(histogramRGBArea == histogramRGBAreaVert.get()
                                      && showBAR->get_active());
}
...
void HistogramPanel::reorder(Gtk::PositionType align)
{
    if (align == Gtk::POS_LEFT) {
        ...
        gfxGrid->remove(*histogramRGBAreaVert);
        gfxGrid->attach_next_to(*histogramRGBAreaVert, *histogramArea, Gtk::POS_LEFT, 1,
                                1);
...
HistogramRGBArea::HistogramRGBArea()
    :  // Saved pointer parameters
      ...
      scaleMode(App::get().options().histogramDrawMode),
      showBar(App::get().options().histogramBar)
{
...
void HistogramRGBArea::getPreferredThickness(int& min_thickness,
                                             int& natural_thickness) const
{
    int minimumLength = 0;
    int naturalLength = 0;
...
void HistogramRGBArea::getPreferredThicknessForLength(int length,
                                                      int& min_thickness,
                                                      int& natural_thickness) const
{
...
// unused?
void HistogramRGBArea::getPreferredLengthForThickness(int thickness,
                                                      int& min_length,
...
void HistogramRGBArea::updateDrawingArea(const ::Cairo::RefPtr<Cairo::Context>& cc)
{
    ...
    // Note: updateDrawingArea is called by the on_draw function so its call does not need
    // to be protected GThreadLock lock; // All GUI access from idle_add callbacks or
    // separate thread HAVE to be protected
...
bool HistogramRGBArea::updatePointer(
    const int new_r,
    const int new_g,
    const int new_b,
    const rtengine::procparams::ColorManagementParams* cmp)
{
    ...
    if (new_r == -1 || new_g == -1 || new_b == -1) {
        ...
        ImProcFunctions::rgb2lab(
            ...
            static_cast<std::uint8_t>(b), lab_L, lab_a, lab_b,
            cmp != nullptr ? *cmp : App::get().fallbackColorCmp(), true);
        pointerValid = true;
    }
                                   ...
                                   const int winh)
{
    ...
    if (App::get().options().histogramDrawMode < 2) {
        ...
        pos = padding
              + HistogramScaling::log(max_value, value) * (winw - padding * 2.0)
                    / max_value;
    }
    cc->move_to(pos, 0.0);
...
void HistogramRGBAreaHori::get_preferred_height_vfunc(int& minimum_height,
                                                      int& natural_height) const
{
    getPreferredThickness(minimum_height, natural_height);
}

void HistogramRGBAreaHori::get_preferred_width_vfunc(int& minimum_width,
                                                     int& natural_width) const
{
    getPreferredLength(minimum_width, natural_width);
}

void HistogramRGBAreaHori::get_preferred_height_for_width_vfunc(int width,
                                                                int& minimum_height,
                                                                int& natural_height) const
{
...
void HistogramRGBAreaHori::get_preferred_width_for_height_vfunc(int height,
                                                                int& minimum_width,
                                                                int& natural_width) const
{
                                   ...
                                   const int winh)
{
    double pos;
    const auto& options = App::get().options();
    if (options.histogramDrawMode < 2 || options.histogramScopeType == ScopeType::PARADE
        || options.histogramScopeType == ScopeType::WAVEFORM)
    {
        ...
        pos =
            padding
            + HistogramScaling::log(max_value, value) * (winh - padding * 2.0) / max_value
            + 0.5;
    }
...
void HistogramRGBAreaVert::get_preferred_height_vfunc(int& minimum_height,
                                                      int& natural_height) const
{
...
void HistogramRGBAreaVert::get_preferred_height_for_width_vfunc(int width,
                                                                int& minimum_height,
                                                                int& natural_height) const
{
...
void HistogramRGBAreaVert::get_preferred_width_for_height_vfunc(int height,
                                                                int& minimum_width,
                                                                int& natural_width) const
{
...
HistogramArea::HistogramArea(DrawModeListener* fml)
    :  // Histogram parameters
      ...
      bwave(0, 0),
      lwave(0, 0),
      parade_buffer_r_dirty(true),
      parade_buffer_g_dirty(true),
      parade_buffer_b_dirty(true),
      ...
      scopeType(App::get().options().histogramScopeType),
      drawMode(App::get().options().histogramDrawMode),
...
void HistogramArea::get_preferred_width_vfunc(int& minimum_width,
                                              int& natural_width) const
{
...
void HistogramArea::get_preferred_height_for_width_vfunc(int width,
                                                         int& minimum_height,
                                                         int& natural_height) const
{
    minimum_height = 0;
...
void HistogramArea::get_preferred_width_for_height_vfunc(int height,
                                                         int& minimum_width,
                                                         int& natural_width) const
{
    get_preferred_width_vfunc(minimum_width, natural_width);
...
void HistogramArea::update(const LUTu& histRed,
        ...
        if (histRed) {
            ...
            case ScopeType::WAVEFORM:
            {
                ...
                parade_buffer_r_dirty = parade_buffer_g_dirty = parade_buffer_b_dirty =
                    wave_buffer_dirty = wave_buffer_luma_dirty = true;
                break;
            }
            case ScopeType::VECTORSCOPE_HS:
...
void HistogramArea::updateDrawingArea(const ::Cairo::RefPtr<Cairo::Context>& cr)
{
               ...
               || scopeType == Options::ScopeType::VECTORSCOPE_HS)
    {
        ...
        for (int i = 1; i < nrOfHGridPartitions; i++) {
            cr->move_to(padding,
                        winh
                            - HistogramScaling::log(winh, i * static_cast<double>(winh)
                                                              / nrOfHGridPartitions)
                            + 0.5);
            cr->line_to(winw - padding,
                        winh
            ...
            || scopeType == Options::ScopeType::HISTOGRAM_RAW))
    {
        ...
        // make double copies of LUT, one for faster access, another one to scale down the
        // raw histos
...
bool HistogramArea::updatePointer(const int r,
                                  const int g,
                                  const int b,
                                  const rtengine::procparams::ColorManagementParams* cmp)
{
    ...
    if (r == -1 || g == -1 || b == -1) {
        ...
        ImProcFunctions::rgb2lab(
            static_cast<std::uint8_t>(r), static_cast<std::uint8_t>(g),
            static_cast<std::uint8_t>(b), L, pointer_a, pointer_b,
            cmp != nullptr ? *cmp : App::get().fallbackColorCmp(), true);
        L /= 327.68f;
        pointer_a /= 327.68f;
        pointer_b /= 327.68f;
...
void HistogramArea::drawParade(const Cairo::RefPtr<Cairo::Context>& cr,
                               const int w,
                               const int h)
{
    ...
    for (unsigned i = 0; i < buffers.size(); i++) {
        ...
        surface = Cairo::ImageSurface::create(buffers[i], Cairo::FORMAT_ARGB32,
...
void HistogramArea::drawVectorscope(const Cairo::RefPtr<Cairo::Context>& cr,
                                    const int w,
                                    const int h)
{
...
void HistogramArea::drawWaveform(const Cairo::RefPtr<Cairo::Context>& cr,
                                 const int w,
                                 const int h)
{
    // Arbitrary scale factor divided by current scale.
    ...
    if (needLuma) {
        surface =
            Cairo::ImageSurface::create(wave_buffer_luma.data(), Cairo::FORMAT_ARGB32,
                                        wave_width, wave_height, cairo_stride);
        cr->set_source(surface, 0, 0);
        cr->set_operator(Cairo::OPERATOR_OVER);
        cr->paint();
