...
/*
 * Load the General, DateTime, ExifInfo, File info and ExtraRawInfo sections of the image
 * data file
 */
int CacheImageData::load(const Glib::ustring& fname)
{
    ...
    try {
        ...
        if (rtengine::settings->verbose) {
            printf(
                "CacheImageData::load / Error code %d while reading values from \"%s\":\n%s\n",
                err.code(), fname.c_str(), err.what().c_str());
        }
    } catch (...) {
        if (rtengine::settings->verbose) {
            printf(
                "CacheImageData::load / Unknown exception while trying to load \"%s\"!\n",
                fname.c_str());
        }
    }
...
/*
 * Save the General, DateTime, ExifInfo, File info and ExtraRawInfo sections of the image
 * data file
 */
int CacheImageData::save(const Glib::ustring& fname)
{
    ...
    try {
        ...
        if (rtengine::settings->verbose) {
            printf(
                "CacheImageData::save / Error code %d while reading values from \"%s\":\n%s\n",
                err.code(), fname.c_str(), err.what().c_str());
        }
    } catch (...) {
        if (rtengine::settings->verbose) {
            printf(
                "CacheImageData::save / Unknown exception while trying to save \"%s\"!\n",
                fname.c_str());
        }
    }
    ...
    if (!f) {
        if (rtengine::settings->verbose) {
            printf(
                "CacheImageData::save / Error: unable to open file \"%s\" with write access!\n",
                fname.c_str());
        }
