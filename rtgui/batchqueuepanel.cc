...
BatchQueuePanel::BatchQueuePanel(FileCatalog* aFileCatalog) : parent(nullptr)
{
    ...
    destinationPreviewScrolledWindow->set_policy(Gtk::POLICY_AUTOMATIC,
...
// it is expected to have a non null forceOrientation value on Preferences update only. In
// this case, qsize is ignored and computed automatically
void BatchQueuePanel::updateTab(int qsize, int forceOrientation)
{
    ...
    if ((forceOrientation == 0 && App::get().options().mainNBVertical)
        || (forceOrientation == 2))
    {
        Gtk::Label* l;
...
void BatchQueuePanel::populateTemplateHelpBuffer(Glib::RefPtr<Gtk::TextBuffer> buffer)
{
...
#ifdef _WIN32
    pos =
        buffer->insert_markup(pos, M("QUEUE_LOCATION_TEMPLATE_HELP_PATHS_INTRO_WINDOWS"));
    pos = buffer->insert(pos, "\n");
#endif
    pos = buffer->insert(pos, "\n");
    ...
    // Since this code only ever runs once (the first time the help text is presented), no
    // attempt is made to be efficient. Use a brute-force method to discover the number of
    // elements in exampleFilePath.
    ...
    // Function inserts examples for a particular specifier, with every valid N value for
    // the number of elements in the path.
    const auto insertPathExamples = [&buffer, &pos, pathElementCount,
                                     exampleFilePath](char letter, int offset1, int mult1,
    ...
    insertPathExamples('d', pathElementCount, -1, -1,
                       -1);  //   <b>%d4</b> = <b>%d-1</b> = <i>home</i>
    insertPathExamples(
        'p', 1, 1, -pathElementCount,
    ...
    const Glib::ustring dateTimeFormatExamples[] = { "%Y-%m-%d", "%Y%m%d_%H%M%S",
                                                     "%y/%b/%-d/" };
...
void BatchQueuePanel::addBatchQueueJobs(const std::vector<BatchQueueEntry*>& entries,
                                        bool head)
{
