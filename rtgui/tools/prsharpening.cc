...
PrSharpening::PrSharpening()
    : FoldableToolPanel(this, TOOL_NAME, M("TP_PRSHARPENING_LABEL"), false, true)
{
    ...
    dradius =
        Gtk::manage(new Adjuster(M("TP_SHARPENING_EDRADIUS"), 0.4, 2.5, 0.01, 0.45));
    ...
    threshold = Gtk::manage(new ThresholdAdjuster(M("TP_SHARPENING_THRESHOLD"), 0., 2000.,
                                                  20., 80., 2000., 1200., 0, false));
    ...
    eonlyConn = edgesonly->signal_toggled().connect(
        sigc::mem_fun(*this, &PrSharpening::edgesonly_toggled));
    hcConn = halocontrol->signal_toggled().connect(
        sigc::mem_fun(*this, &PrSharpening::halocontrol_toggled));
...
void PrSharpening::read(const ProcParams* pp, const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        etolerance->setEditedState(pedited->prsharpening.edges_tolerance ? Edited
                                                                         : UnEdited);
        hcamount->setEditedState(pedited->prsharpening.halocontrol_amount ? Edited
                                                                          : UnEdited);
...
void PrSharpening::setDefaults(const ProcParams* defParams, const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        contrast->setDefaultEditedState(pedited->prsharpening.contrast ? Edited
                                                                       : UnEdited);
        ...
        threshold->setDefaultEditedState(pedited->prsharpening.threshold ? Edited
                                                                         : UnEdited);
        eradius->setDefaultEditedState(pedited->prsharpening.edges_radius ? Edited
                                                                          : UnEdited);
        etolerance->setDefaultEditedState(
            pedited->prsharpening.edges_tolerance ? Edited : UnEdited);
        hcamount->setDefaultEditedState(
            pedited->prsharpening.halocontrol_amount ? Edited : UnEdited);
        damount->setDefaultEditedState(pedited->prsharpening.deconvamount ? Edited
                                                                          : UnEdited);
        dradius->setDefaultEditedState(pedited->prsharpening.deconvradius ? Edited
                                                                          : UnEdited);
        diter->setDefaultEditedState(pedited->prsharpening.deconviter ? Edited
                                                                      : UnEdited);
        ddamping->setDefaultEditedState(pedited->prsharpening.deconvdamping ? Edited
                                                                            : UnEdited);
    } else {
...
void PrSharpening::adjusterChanged(Adjuster* a, double newval)
{
    if (listener && (multiImage || getEnabled())) {
        ...
        if (a == radius || a == dradius) {
            ...
            costr = Glib::ustring::format(std::setw(2), std::fixed, std::setprecision(1),
                                          a->getValue());
        } else {
...
void PrSharpening::adjusterChanged(ThresholdAdjuster* a,
...
void PrSharpening::adjusterChanged2(ThresholdAdjuster* a,
                                    int newBottomL,
                                    int newTopL,
                                    int newBottomR,
                                    int newTopR)
...
void PrSharpening::setAdjusterBehavior(bool contrastadd,
                                       bool radiusadd,
                                       bool amountadd,
                                       bool dampingadd,
                                       bool iteradd,
