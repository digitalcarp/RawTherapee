...
void Locallab::read(const rtengine::procparams::ProcParams* pp,
                    const ParamsEdited* pedited)
{
    ...
    // Specific case: if there is no spot, GUI isn't anymore editable (i.e. Locallab tool
    // cannot be managed)
    if (pp->locallab.spots.size() > 0) {
...
void Locallab::write(rtengine::procparams::ProcParams* pp, ParamsEdited* pedited)
{
    ...
    case (ControlSpotPanel::SpotCreation):
    {  // Spot creation event
        ...
        if (provider && !batchMode) {
            ...
            if (imW && imH) {  // Image loaded
                ...
                newSpot->centerX = rtengine::LIM(
                    int(int((double)prX - (double)imW / 2.) * 2000. / (double)imW), -1000,
                    1000);
                newSpot->centerY = rtengine::LIM(
                    int(int((double)prY - (double)imH / 2.) * 2000. / (double)imH), -1000,
                    1000);
    ...
    case (ControlSpotPanel::SpotDeletion):  // Spot deletion event
        ...
        for (int i = 0; i < (int)pp->locallab.spots.size(); i++) {
            if (i == spotIndex) {
                ...
                if (pp->locallab.spots.size() > 0) {
                    for (int j = i - 1; j >= 0; j--) {
                        if (expsettings->setSelectedSpot(j))
                        {  // True if an existing spot has been selected on
                           // controlspotpanel
    ...
    case (ControlSpotPanel::SpotDuplication):
    {  // Spot duplication event
        newSpot = nullptr;
        spotIndex = expsettings->getSelectedSpot();
        ...
        // newSpot->loc.at(1), (double) newSpot->loc.at(2), (double) newSpot->loc.at(3));
        if (newSpot->avoidgamutMethod == "NONE") {
            r.avoidgamutMethod = 0;
        } else if (newSpot->avoidgamutMethod == "LAB") {
            r.avoidgamutMethod = 1;
        ...
        if (provider && !batchMode) {
            ...
            if (imW && imH) {  // Image loaded
                ...
                // Ellipse/rectangle size computation
                /*
                newSpot->loc.at(0) = rtengine::LIM(int(((double)prW / 2. - 5.) * 2000. /
                (double)imW), 2, newSpot->loc.at(0)); newSpot->loc.at(1) =
                rtengine::LIM(int(((double)prW / 2. - 5.) * 2000. / (double)imW), 2,
                newSpot->loc.at(1)); newSpot->loc.at(2) = rtengine::LIM(int(((double)prH
                / 2. - 5.) * 2000. / (double)imH), 2, newSpot->loc.at(2));
                newSpot->loc.at(3) = rtengine::LIM(int(((double)prH / 2. - 5.) * 2000. /
                (double)imH), 2, newSpot->loc.at(3));
                */
            }
        }
    ...
    case (ControlSpotPanel::SpotAllVisibilityChanged):
    {  // Event when updating visibility of all spots
 ...
 */
void Locallab::setDefaults(const rtengine::procparams::ProcParams* defParams,
                           const ParamsEdited* pedited)
{
...
void Locallab::madChanged(const std::vector<locallabDenoiseMAD>& madlc, int selspot)
{
    ...
    if (selspot < (int)madllc.size()) {
        ...
        expblur.updatemadlc(m0, m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12, m13,
                            m14, m15, m16, m17, m18, m19, m20, madloc);
    }
}
...
void Locallab::denChanged(const std::vector<locallabDenoiseLC>& denlc, int selspot)
{
    ...
    if (selspot < (int)denoiselc.size()) {
        ...
        expblur.updatedenlc(highres, nres, highres46, nres46, Lhighres, Lnres, Lhighres46,
                            Lnres46);
    }
}
...
void Locallab::maiChanged(const std::vector<locallabsetLC>& setlc, int selspot)
{
    ...
    if (selspot < (int)set_lc.size()) {
        ...
        expsettings->updateguiset(spottype, iscolor, issh, isvib, isexpos, issoft, isblur,
                                  istom, isret, issharp, iscont, iscbdl, islog, ismas,
                                  isci);
    }
}

void Locallab::ghsbw2Changed(const std::vector<locallabshGHSbw2>& shghsbw2,
                             int selspot)  // update sliders black and white point if auto
{
    sh_ghsbw2 = shghsbw2;
...
void Locallab::ghsbwChanged(
    const std::vector<locallabshGHSbw>& shghsbw,
    int selspot)  // info black and white point and Symmetry point auto
{
    sh_ghsbw = shghsbw;
    int bw[2] = { 0, 1 };
    ...
    if (selspot < (int)sh_ghsbw.size()) {
        ...
        expshadhigh.updateghsbw(bw[0], bw[1], bwvalue[0], bwvalue[1], symev, bwcol[0],
                                bwcol[1], bwcol[2], bwcol[3], ghsau);
        if (autoradsp) {
...
void Locallab::cieChanged(const std::vector<locallabcieLC>& cielc, int selspot)
{
    ...
    if (selspot < (int)cie_lc.size()) {
        ...
        expcie.updateiPrimloc(r1, r2, g1, g2, b1, b2, w1, w2, m1, m2, me1, me2, pri, slg,
                              lkg);
    }
}

void Locallab::refChanged2(float* huerefp,
                           float* chromarefp,
                           float* lumarefp,
                           float* fabrefp,
                           int selspot)
...
Locallab::llMaskVisibility Locallab::getMaskVisibility() const
{
    ...
    for (auto tool : locallabTools) {
        tool->getMaskView(colorMask, colorMaskinv, expMask, expMaskinv, shMask, shMaskinv,
                          vibMask, softMask, blMask, tmMask, retiMask, sharMask, lcMask,
                          cbMask, logMask, maskMask, cieMask);
    }

    // Indicate to spot control panel if one mask preview is active
    const bool isMaskActive = (colorMask == 0) || (colorMaskinv == 0) || (expMask == 0)
                              || (expMaskinv == 0) || (shMask == 0) || (shMaskinv == 0)
                              || (vibMask == 0) || (softMask == 0) || (blMask == 0)
                              || (tmMask == 0) || (retiMask == 0) || (sharMask == 0)
                              || (lcMask == 0) || (cbMask == 0) || (logMask == 0)
                              || (maskMask == 0) || (cieMask == 0);
    expsettings->setMaskPrevActive(isMaskActive);

    return { prevDeltaE, colorMask, colorMaskinv, expMask, expMaskinv, shMask,
             shMaskinv,  vibMask,   softMask,     blMask,  tmMask,     retiMask,
             sharMask,   lcMask,    cbMask,       logMask, maskMask,   cieMask };
}
