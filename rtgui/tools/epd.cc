                                         ...
                                         const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        reweightingIterates->setEditedState(pedited->epd.reweightingIterates ? Edited
                                                                             : UnEdited);
                                                ...
                                                const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        edgeStopping->setDefaultEditedState(pedited->epd.edgeStopping ? Edited
                                                                      : UnEdited);
...
void EdgePreservingDecompositionUI::adjusterChanged(Adjuster* a, double newval)
{
    if (listener && getEnabled()) {
        if (a == strength) {
            listener->panelChanged(EvEPDStrength,
            ...
            listener->panelChanged(EvEPDEdgeStopping,
                                   Glib::ustring::format(std::setw(2), std::fixed,
                                                         std::setprecision(2),
                                                         a->getValue()));
        } else if (a == scale) {
            ...
            listener->panelChanged(EvEPDReweightingIterates,
...
void EdgePreservingDecompositionUI::setAdjusterBehavior(bool stAdd,
                                                        bool gAdd,
                                                        bool esAdd,
                                                        bool scAdd,
                                                        bool rAdd)
