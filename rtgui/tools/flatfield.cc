...
FlatField::FlatField() : FoldableToolPanel(this, TOOL_NAME, M("TP_FLATFIELD_LABEL"))
{
    ...
    flatFieldFile = Gtk::manage(
        new MyFileChooserButton(M("TP_FLATFIELD_LABEL"), Gtk::FILE_CHOOSER_ACTION_OPEN));
    ...
    flatFieldAutoSelect =
        Gtk::manage(new Gtk::CheckButton((M("TP_FLATFIELD_AUTOSELECT"))));
    flatFieldFromMetaData =
        Gtk::manage(new CheckBox((M("TP_FLATFIELD_FROMMETADATA")), multiImage));
    ...
    flatFieldBlurRadius =
        Gtk::manage(new Adjuster(M("TP_FLATFIELD_BLURRADIUS"), 0, 200, 2, 32));
    flatFieldBlurRadius->setAdjusterListener(this);

    flatFieldBlurRadius->setDelay(
    ...
    flatFieldClipControl =
        Gtk::manage(new Adjuster(M("TP_FLATFIELD_CLIPCONTROL"), 0., 100., 1., 0.));
    flatFieldClipControl->setAdjusterListener(this);
    flatFieldClipControl->addAutoButton("");

    flatFieldClipControl->setDelay(
        std::max(options.adjusterMinDelay, options.adjusterMaxDelay));
    ...
    flatFieldFileconn = flatFieldFile->signal_file_set().connect(
        sigc::mem_fun(*this, &FlatField::flatFieldFileChanged));  //, true);
    flatFieldFileReset->signal_clicked().connect(
        sigc::mem_fun(*this, &FlatField::flatFieldFile_Reset), true);
    flatFieldAutoSelectconn = flatFieldAutoSelect->signal_toggled().connect(
        sigc::mem_fun(*this, &FlatField::flatFieldAutoSelectChanged), true);
    flatFieldBlurTypeconn = flatFieldBlurType->signal_changed().connect(
        sigc::mem_fun(*this, &FlatField::flatFieldBlurTypeChanged));
                     ...
                     const ParamsEdited* pedited)
{
    ...
    // flatFieldBlurType
    for (size_t i = 0; i < procparams::RAWParams::getFlatFieldBlurTypeStrings().size();
         ++i)
    {
        if (pp->raw.ff_BlurType
            == procparams::RAWParams::getFlatFieldBlurTypeStrings()[i])
        {
    ...
    if (multiImage
        || pp->raw.ff_BlurType
               == procparams::RAWParams::getFlatFieldBlurTypeString(
                   procparams::RAWParams::FlatFieldBlurType::AREA))
    {
        flatFieldClipControl->show();
    } else {
    ...
    if (pedited) {
        ...
        flatFieldBlurRadius->setEditedState(pedited->raw.ff_BlurRadius ? Edited
                                                                       : UnEdited);
        flatFieldClipControl->setEditedState(pedited->raw.ff_clipControl ? Edited
                                                                         : UnEdited);
        flatFieldClipControl->setAutoInconsistent(multiImage
                                                  && !pedited->raw.ff_AutoClipControl);
    ...
    if (pp->raw.ff_AutoSelect && ffp && !batchMode) {
        ...
        if (img) {
            ffInfo->set_text(Glib::ustring::compose(
                "%1: f/%2", Glib::path_get_basename(img->get_filename()),
                img->get_aperture()));  // !!! need to add focallength in mm and format
                                        // aperture to ##.#
        } else {
    ...
    if (ffp && !batchMode) {
        ...
        if (!fname.empty()) {
            ...
            if (idx != std::string::npos) {
                ...
                // exclude non-raw
                israw = filetype.uppercase() != "JPG" && filetype.uppercase() != "JPEG"
                        && filetype.uppercase() != "PNG" && filetype.uppercase() != "TIF"
                        && filetype.uppercase() != "TIFF";

                if (israw) {
                    b_filter_asCurrent =
                        true;  // prevent re-adding this filter on every pp3 file read
                    Glib::RefPtr<Gtk::FileFilter> filter_asCurrent =
                        Gtk::FileFilter::create();
                    filter_asCurrent->add_pattern("*." + filetype);
                    filter_asCurrent->set_name(M("FILECHOOSER_FILTER_SAME") + " ("
                                               + filetype + ")");
...
void FlatField::write(rtengine::procparams::ProcParams* pp, ParamsEdited* pedited)
{
    ...
    {
        pp->raw.ff_BlurType =
            procparams::RAWParams::getFlatFieldBlurTypeStrings()[currentRow];
    }
    ...
    if (pedited) {
        ...
        pedited->raw.ff_BlurType =
            flatFieldBlurType->get_active_text() != M("GENERAL_UNCHANGED");
    }
}
...
void FlatField::adjusterAutoToggled(Adjuster* a, bool newval)
{
    ...
    if (listener) {
        if (a == flatFieldClipControl) {
            if (flatFieldClipControl->getAutoInconsistent()) {
                listener->panelChanged(EvFlatFieldAutoClipControl,
                                       M("GENERAL_UNCHANGED"));
                            ...
                            const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        flatFieldBlurRadius->setDefaultEditedState(pedited->raw.ff_BlurRadius ? Edited
                                                                              : UnEdited);
        flatFieldClipControl->setDefaultEditedState(
            pedited->raw.ff_clipControl ? Edited : UnEdited);
    } else {
...
void FlatField::flatFieldFileChanged()
{
    ...
    if (listener) {
        listener->panelChanged(EvFlatFieldFile,
                               Glib::path_get_basename(flatFieldFile->get_filename()));
    }
}
...
void FlatField::flatFieldFile_Reset()
{
    ...
    // correctly!
    //          Because szeva doesn't exist since he was committed to happy hunting ground
    //          in Issue 316 we can use him now for this hack
...
void FlatField::flatFieldBlurTypeChanged()
{
    ...
    const RAWParams::FlatFieldBlurType blur_type =
        RAWParams::FlatFieldBlurType(curSelection);
...
void FlatField::checkBoxToggled(CheckBox* c, CheckValue newval)
{
    if (listener && c == flatFieldFromMetaData) {
        listener->panelChanged(EvFlatFieldFromMetaData,
                               flatFieldFromMetaData->getLastActive()
                                   ? M("GENERAL_ENABLED")
                                   : M("GENERAL_DISABLED"));
    }
}
...
void FlatField::flatFieldAutoSelectChanged()
{
    ...
    if (listener) {
        listener->panelChanged(EvFlatFieldAutoSelect, flatFieldAutoSelect->get_active()
                                                          ? M("GENERAL_ENABLED")
                                                          : M("GENERAL_DISABLED"));
    }
}
