    ...
    : FoldableToolPanel(this, TOOL_NAME, M("TP_DIRPYREQUALIZER_LABEL"), true, true)
{
    ...
    */
    hueskin = Gtk::manage(new ThresholdAdjuster(
        M("TP_DIRPYREQUALIZER_HUESKIN"), -40., 210., -5., 25., 170., 120., 0,
        false));  // default (b_l 0, t_l 30, b_r 170, t_r 120);
...
void DirPyrEqualizer::read(const ProcParams* pp, const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        for (int i = 0; i < 6; i++) {
            multiplier[i]->setEditedState(pedited->dirpyrequalizer.mult[i] ? Edited
                                                                           : UnEdited);
        }

        threshold->setEditedState(pedited->dirpyrequalizer.threshold ? Edited : UnEdited);
        skinprotect->setEditedState(pedited->dirpyrequalizer.skinprotect ? Edited
                                                                         : UnEdited);
                                  ...
                                  const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        threshold->setDefaultEditedState(pedited->dirpyrequalizer.threshold ? Edited
                                                                            : UnEdited);
        skinprotect->setDefaultEditedState(
            pedited->dirpyrequalizer.skinprotect ? Edited : UnEdited);
        hueskin->setDefaultEditedState(pedited->dirpyrequalizer.hueskin ? Edited
                                                                        : UnEdited);
    } else {
        for (int i = 0; i < 6; i++) {
...
void DirPyrEqualizer::adjusterChanged(ThresholdAdjuster* a,
                                      double newBottom,
                                      double newTop)
{
}
...
void DirPyrEqualizer::adjusterChanged2(ThresholdAdjuster* a,
                                       int newBottomL,
                                       int newTopL,
                                       int newBottomR,
                                       int newTopR)
