...
WhiteBalance::WhiteBalance()
    : FoldableToolPanel(this, TOOL_NAME, M("TP_WBALANCE_LABEL"), true, true),
      wbp(nullptr),
      wblistener(nullptr)
{
    ...
    Gtk::CellRendererText* const renderer_label =
    ...
    for (unsigned int i = 0; i < WBParams::getWbEntries().size(); i++) {
        ...
        if (currType == WBEntry::Type::FLUORESCENT || currType == WBEntry::Type::LAMP
            || currType == WBEntry::Type::WATER || currType == WBEntry::Type::FLASH
            || currType == WBEntry::Type::LED || currType == WBEntry::Type::AUTO)
        {
    ...
    spotbutton->set_image(
        *Gtk::manage(new RTImage("color-picker-small", Gtk::ICON_SIZE_BUTTON)));
    ...
    Gtk::Image* itempL =
        Gtk::manage(new RTImage("circle-blue-small", Gtk::ICON_SIZE_BUTTON));
    Gtk::Image* itempR =
        Gtk::manage(new RTImage("circle-yellow-small", Gtk::ICON_SIZE_BUTTON));
    Gtk::Image* igreenL =
        Gtk::manage(new RTImage("circle-magenta-small", Gtk::ICON_SIZE_BUTTON));
    Gtk::Image* igreenR =
        Gtk::manage(new RTImage("circle-green-small", Gtk::ICON_SIZE_BUTTON));
    Gtk::Image* iblueredL =
        Gtk::manage(new RTImage("circle-blue-small", Gtk::ICON_SIZE_BUTTON));
    Gtk::Image* iblueredR =
        Gtk::manage(new RTImage("circle-red-small", Gtk::ICON_SIZE_BUTTON));
    Gtk::Image* itempbiasL =
        Gtk::manage(new RTImage("circle-blue-small", Gtk::ICON_SIZE_BUTTON));
    Gtk::Image* itempbiasR =
        Gtk::manage(new RTImage("circle-yellow-small", Gtk::ICON_SIZE_BUTTON));
    ...
    temp = Gtk::manage(new Adjuster(M("TP_WBALANCE_TEMPERATURE"), MINTEMP, MAXTEMP, 5,
                                    CENTERTEMP, itempL, itempR, &wbSlider2Temp,
                                    &wbTemp2Slider));
    green = Gtk::manage(new Adjuster(M("TP_WBALANCE_GREEN"), MINGREEN, MAXGREEN, 0.001,
                                     1.0, igreenL, igreenR));
    equal = Gtk::manage(new Adjuster(M("TP_WBALANCE_EQBLUERED"), MINEQUAL, MAXEQUAL,
                                     0.001, 1.0, iblueredL, iblueredR));
    tempBias = Gtk::manage(new Adjuster(M("TP_WBALANCE_TEMPBIAS"), -1.1, 1.1, 0.005, 0.0,
                                        itempbiasL, itempbiasR));
    ...
    itcwb_green =
        Gtk::manage(new Adjuster(M("TP_WBALANCE_ITCWGREEN"), -0.35, 0.35, 0.005, 0.));
    ...
    itcwb_primconn = itcwb_prim->signal_changed().connect(
        sigc::mem_fun(*this, &WhiteBalance::itcwb_prim_changed));
    itcwb_prim->set_tooltip_markup(M("TP_WBALANCE_ITCWPRIM_TOOLTIP"));

    compatVersionAdjuster.reset(
        new Adjuster("", 0., procparams::WBParams::CURRENT_COMPAT_VERSION, 1.,
                     procparams::WBParams::CURRENT_COMPAT_VERSION));
    ...
    methconn =
        method->signal_changed().connect(sigc::mem_fun(*this, &WhiteBalance::optChanged));
    itcwb_algconn = itcwb_alg->signal_toggled().connect(
        sigc::mem_fun(*this, &WhiteBalance::itcwb_alg_toggled));
...
void WhiteBalance::adjusterChanged(Adjuster* a, double newval)
{
    ...
    const std::pair<bool, const WBEntry&> ppMethod =
        findWBEntry(row[methodColumns.colLabel], WBLT_GUI);
    const std::pair<bool, const WBEntry&> wbCustom = findWBEntry("Custom", WBLT_PP);

    if (!ppMethod.first
        || (ppMethod.second.ppLabel != wbCustom.second.ppLabel
            && !((a == equal || a == tempBias || a == itcwb_green)
    ...
    if (listener && getEnabled()) {
        if (a == temp) {
            ...
            listener->panelChanged(
                EvWBGreen, Glib::ustring::format(std::setw(4), std::fixed,
                                                 std::setprecision(3), a->getValue()));
            itcwbFrame->set_sensitive(false);
        } else if (a == equal) {
            listener->panelChanged(
                EvWBequal, Glib::ustring::format(std::setw(4), std::fixed,
                                                 std::setprecision(3), a->getValue()));
        } else if (a == tempBias) {
            listener->panelChanged(
                EvWBtempBias, Glib::ustring::format(std::setw(4), std::fixed,
                                                    std::setprecision(2), a->getValue()));
        } else if (a == itcwb_green) {
            listener->panelChanged(EvWBitcwgreen,
...
void WhiteBalance::checkBoxToggled(CheckBox* c, CheckValue newval)
{
    ...
    if (c == observer10) {
        ...
        if (row[methodColumns.colLabel] != M("GENERAL_UNCHANGED")
            && currMethod.type == WBEntry::Type::CAMERA && wbp)
        {
            double ctemp, cgreen;
        ...
        listener->panelChanged(
            EvWBObserver10, c->getValue() == CheckValue::on    ? M("GENERAL_ENABLED")
                            : c->getValue() == CheckValue::off ? M("GENERAL_DISABLED")
                                                               : M("GENERAL_UNCHANGED"));
    }
}
    ...
    if (opt != row[methodColumns.colId]) {
        ...
        if (row[methodColumns.colLabel] == M("GENERAL_UNCHANGED")) {
            ...
            default:
                ...
                temp->setValue(temp->getAddMode() ? 0.0
                                                  : (double)(currMethod.temperature));
                ...
                && (!batchMode || currMethod.type != WBEntry::Type::AUTO))
            {
                ...
                compatVersionAdjuster->setValue(
                    procparams::WBParams::CURRENT_COMPAT_VERSION);
                if (batchMode) {
...
void WhiteBalance::read(const ProcParams* pp, const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        compatVersionAdjuster->setEditedState(pedited->wb.compat_version ? Edited
                                                                         : UnEdited);
    }
    ...
    if (pedited && !pedited->wb.method) {
        ...
        const WBEntry& wbValues = [this, pp]() -> const WBEntry& {
            const std::pair<bool, const WBEntry&> res =
                findWBEntry(pp->wb.method, WBLT_PP);
            return !res.first ? findWBEntry("Camera", WBLT_PP).second : res.second;
        }();
...
void WhiteBalance::write(ProcParams* pp, ParamsEdited* pedited)
{
    ...
    pp->wb.observer = observer10->getValue() == CheckValue::on
                          ? rtengine::StandardObserver::TEN_DEGREES
                      : observer10->getValue() == CheckValue::off
                          ? rtengine::StandardObserver::TWO_DEGREES
                          : pp->wb.observer;
...
void WhiteBalance::setWB(int vtemp, double vgreen)
{
    ...
    cache_customWB(vtemp, vgreen);  // sequence in which this call is made is important;
                                    // must be before "method->set_active (2);"
    cache_customEqual(equal->getValue());
    temp->setEditedState(Edited);
    green->setEditedState(Edited);
    methconn.block(false);
...
void WhiteBalance::setAdjusterBehavior(bool tempadd,
                                       bool greenadd,
                                       bool equaladd,
                                       bool tempbiasadd)
{
...
unsigned int WhiteBalance::findWBEntryId(const Glib::ustring& label,
                                         enum WB_LabelType lblType)
{
    for (unsigned int i = 0; i < WBParams::getWbEntries().size(); i++) {
        if (label
            == (lblType == WBLT_GUI ? WBParams::getWbEntries()[i].GUILabel
                                    : WBParams::getWbEntries()[i].ppLabel))
        {
            return i;
        }
    }
...
std::pair<bool, const WBEntry&> WhiteBalance::findWBEntry(const Glib::ustring& label,
                                                          enum WB_LabelType lblType)
{
    for (unsigned int i = 0; i < WBParams::getWbEntries().size(); ++i) {
        if (label
...
int WhiteBalance::_setActiveMethod(Glib::ustring& label,
                                   Gtk::TreeModel::Children& children)
{
    int found = -1;

    for (Gtk::TreeModel::Children::iterator iter = children.begin();
         iter != children.end() && found == -1; ++iter)
    {
...
void WhiteBalance::WBChanged(int met,
                             double temperature,
                             double greenVal,
                             double rw,
                             double gw,
                             ...
                             AWBMode aWBMode)
{
    idle_register.add([this, met, temperature, greenVal, rw, gw, bw, temp0, delta, bia,
                       dread, studgood, minchrom, kmin, histmin, histmax,
                       aWBMode]() -> bool {
        disableListener();
        ...
        int bia2 = bia;
        mulLabel->set_text(Glib::ustring::compose(
            M("TP_WBALANCE_MULLABEL"),
        ...
        if (bia == 3) {
            bia2 = bia - 1;
            StudLabel->set_text(Glib::ustring::compose(
                M("TP_WBALANCE_STUDLABEL"),
            ...
            StudLabel->set_text(Glib::ustring::compose(
                M("TP_WBALANCE_STUDLABEL1"),
                Glib::ustring::format(std::fixed, std::setprecision(4), stud),
                Glib::ustring::format(std::fixed, std::setprecision(0), bia),
                Glib::ustring::format(std::fixed, std::setprecision(0), temp0)));
        } else {
            StudLabel->set_text(Glib::ustring::compose(
                M("TP_WBALANCE_STUDLABEL0"),
                Glib::ustring::format(std::fixed, std::setprecision(4), stud),
                Glib::ustring::format(std::fixed, std::setprecision(0), bia),
                Glib::ustring::format(std::fixed, std::setprecision(0), temp0)));
        }
        PatchLabel->set_text(Glib::ustring::compose(
            M("TP_WBALANCE_PATCHLABEL"),
        ...
        PatchlevelLabel->set_text(Glib::ustring::compose(
            M("TP_WBALANCE_PATCHLEVELLABEL"),
