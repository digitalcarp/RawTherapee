...
void ChMixer::setDefaults(const ProcParams* defParams, const ParamsEdited* pedited)
{
    ...
    if (pedited)
        for (int i = 0; i < 3; i++) {
            ...
            green[i]->setDefaultEditedState(pedited->chmixer.green[i] ? Edited
                                                                      : UnEdited);
...
void ChMixer::adjusterChanged(Adjuster* a, double newval)
{
    ...
    if (listener && getEnabled()) {
        Glib::ustring descr = Glib::ustring::compose(
            "R=%1,%2,%3\nG=%4,%5,%6\nB=%7,%8,%9", red[0]->getValue(), red[1]->getValue(),
            red[2]->getValue(), green[0]->getValue(), green[1]->getValue(),
            green[2]->getValue(), blue[0]->getValue(), blue[1]->getValue(),
            blue[2]->getValue());
