                        ...
                        App::get().options().prevdemo != PD_Sidecar)
{
    ...
    //  hotdeadPixel->show();
    hpixelconn = hotPixel->signal_toggled().connect(
        sigc::mem_fun(*this, &PreProcess::hotPixelChanged), true);
    dpixelconn = deadPixel->signal_toggled().connect(
...
void PreProcess::hotPixelChanged()
{
    ...
    if (listener) {
        listener->panelChanged(EvPreProcessHotPixel, hotPixel->get_active()
                                                         ? M("GENERAL_ENABLED")
                                                         : M("GENERAL_DISABLED"));
    }
}
...
void PreProcess::deadPixelChanged()
{
    ...
    if (listener) {
        listener->panelChanged(EvPreProcessDeadPixel, deadPixel->get_active()
                                                          ? M("GENERAL_ENABLED")
                                                          : M("GENERAL_DISABLED"));
    }
}
