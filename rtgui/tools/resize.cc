...
Resize::Resize()
    : FoldableToolPanel(this, TOOL_NAME, M("TP_RESIZE_LABEL"), false, true),
      maxw(100000),
      maxh(100000)
{
    ...
    Gtk::Label* label =
        Gtk::manage(new Gtk::Label(M("TP_RESIZE_APPLIESTO"), Gtk::ALIGN_START));
    ...
    hbox->pack_start(*h);
    lebox->set_spacing(3);
    lebox->pack_start(*Gtk::manage(new Gtk::Label(M("TP_RESIZE_LE"))), Gtk::PACK_SHRINK,
                      0);
    lebox->pack_start(*le);
    sebox->set_spacing(3);
    sebox->pack_start(*Gtk::manage(new Gtk::Label(M("TP_RESIZE_SE"))), Gtk::PACK_SHRINK,
                      0);
    sebox->pack_start(*se);
    ...
    allowUpscaling->signal_toggled().connect(
        sigc::mem_fun(*this, &Resize::allowUpscalingChanged));
    ...
    wconn = w->signal_value_changed().connect(
        sigc::mem_fun(*this, &Resize::entryWChanged), true);
    hconn = h->signal_value_changed().connect(
        sigc::mem_fun(*this, &Resize::entryHChanged), true);
    leconn = le->signal_value_changed().connect(
        sigc::mem_fun(*this, &Resize::entryLEChanged), true);
    seconn = se->signal_value_changed().connect(
        sigc::mem_fun(*this, &Resize::entrySEChanged), true);
    aconn = appliesTo->signal_changed().connect(
        sigc::mem_fun(*this, &Resize::appliesToChanged));
...
void Resize::read(const ProcParams* pp, const ParamsEdited* pedited)
{
    ...
    setDimensions();  // Sets Width/Height in the GUI according to value of Specify after
                      // loading a .pp3 profile (same behavior as if changed manually)
    updateGUI();
    ...
    setDimensions();  // fixes the issue that values in GUI are not recomputed when
                      // loading profile
...
void Resize::adjusterChanged(Adjuster* a, double newval)
{
    if (!batchMode) {
        wconn.block(true);
        hconn.block(true);
        h->set_value((croph && appliesTo->get_active_row_number() == 0 ? croph : maxh)
                     * a->getValue());
        w->set_value((cropw && appliesTo->get_active_row_number() == 0 ? cropw : maxw)
                     * a->getValue());
        wconn.block(false);
        hconn.block(false);
    }
    ...
    if (listener && (getEnabled() || batchMode)) {
        listener->panelChanged(EvResizeScale,
...
void Resize::methodChanged()
{
    ...
    // Post-resize Sharpening assumes the image is in Lab space, and currently Lanczos is
    // the only method which uses that space, and Lanczos is on row 0.
    if (method->get_active_row_number() == 0) {
...
void Resize::setDimensions()
{
    idle_register.add([this]() -> bool {
        ...
        {
            // Scale mode
            w->set_value(static_cast<double>(
                static_cast<int>(static_cast<double>(refw) * scale->getValue() + 0.5)));
            h->set_value(static_cast<double>(
                static_cast<int>(static_cast<double>(refh) * scale->getValue() + 0.5)));
            break;
        }
        ...
        case 1:
        {
            ...
            h->set_value(static_cast<double>(
                static_cast<int>(static_cast<double>(refh) * tmp_scale + 0.5)));
            break;
        }
        ...
        case 2:
        {
            ...
            scale->setValue(tmp_scale);
            w->set_value(static_cast<double>(
                static_cast<int>(static_cast<double>(refw) * tmp_scale + 0.5)));
            break;
        }
        ...
        case 3:
        {
            ...
            const double tmp_scale =
                w->get_value() / h->get_value()
                        < static_cast<double>(refw) / static_cast<double>(refh)
        ...
        case 4:
        {
            ...
            if (refw > refh) {
                ...
                scale->setValue(tmp_scale);
                se->set_value(static_cast<double>(
                    static_cast<int>(static_cast<double>(refh) * tmp_scale + 0.5)));
            } else {
        ...
        case 5:
        {
            ...
            if (refw > refh) {
                ...
                scale->setValue(tmp_scale);
                le->set_value(static_cast<double>(
                    static_cast<int>(static_cast<double>(refw) * tmp_scale + 0.5)));
            } else {
...
void Resize::entryWChanged()
{
    ...
    if (!batchMode) {
        if (spec->get_active_row_number() == 3) {
            ...
            scale->setValue(w->get_value()
                            / (cropw && appliesTo->get_active_row_number() == 0
                                   ? (double)cropw
                                   : (double)maxw));
    ...
    if (listener) {
        if (spec->get_active_row_number() == 3) {
            ...
            if (getEnabled() || batchMode) {
                listener->panelChanged(EvResizeWidth,
                                       Glib::ustring::format(w->get_value_as_int()));
            }
        }
    }
}
...
void Resize::entryHChanged()
{
    ...
    if (!batchMode && listener) {
        if (spec->get_active_row_number() == 3) {
            ...
            scale->setValue(h->get_value()
                            / (croph && appliesTo->get_active_row_number() == 0
                                   ? (double)croph
                                   : (double)maxh));
    ...
    if (listener) {
        if (spec->get_active_row_number() == 3) {
            ...
            if (getEnabled() || batchMode) {
                listener->panelChanged(EvResizeHeight,
                                       Glib::ustring::format(h->get_value_as_int()));
            }
        }
    }
}
...
void Resize::entryLEChanged()
{
    ...
    if (listener) {
        if (getEnabled() || batchMode) {
            listener->panelChanged(EvResizeLongedge,
                                   Glib::ustring::format(le->get_value_as_int()));
        }
    }
}
...
void Resize::entrySEChanged()
{
    ...
    if (listener) {
        if (getEnabled() || batchMode) {
            listener->panelChanged(EvResizeShortedge,
                                   Glib::ustring::format(se->get_value_as_int()));
        }
    }
}
...
void Resize::notifyBBox()
{
    if (listener && (getEnabled() || batchMode)) {
        listener->panelChanged(
            EvResizeBoundingBox,
            Glib::ustring::compose("(%1x%2)", (int)w->get_value(), (int)h->get_value()));
    }
}
