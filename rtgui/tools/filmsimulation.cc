    ...
    if (!pp->filmSimulation.clutFilename.empty()) {
        m_clutComboBox->setSelectedClut(
            !Glib::path_is_absolute(pp->filmSimulation.clutFilename)
                ? Glib::ustring(Glib::build_filename(options.clutsDir,
...
void FilmSimulation::write(rtengine::procparams::ProcParams* pp, ParamsEdited* pedited)
{
    ...
    if (clutFName != "NULL")
    {  // We do not want to set "NULL" in clutFilename, even if "unedited"
        pp->filmSimulation.clutFilename =
            stripPrefixDir(clutFName, App::get().options().clutsDir);
    }
...
Gtk::TreeIter ClutComboBox::findRowByClutFilename(Gtk::TreeModel::Children childs,
