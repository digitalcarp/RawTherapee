 ...
 * move the cursor over one of them, a circle is displayed to show the radius of the
 * brush, as well as a second circle representing the source data (S point). The user can
 * then use the left mouse button over the icon to drag the T point. The left mouse button
 * can be used over the S circle or the right mouse button can be used over the T point to
 * move the S point.
 *
 * Using the left mouse button over the circle of the T point will let the user adjust its
 * radius.
 *
 * Using the left mouse button over the feather circle will let the user adjust its radius
 * by setting a coefficient (0.0 = same radius than the inner circle ; 1.0 = 2 times the
 * inner radius).
 *
 * To create a new point, just move over a free area, and press the left mouse button
 * while holding the CTRL key. This will create a new S and T pair of points. The CTRL key
 * can be released, but keep the left mouse button pressed and move away to position the S
 * point.
 *
