    ...
    : FoldableToolPanel(this, TOOL_NAME, M("TP_BWMIX_LABEL"), false, true)
{
    ...
    luminanceCurve =
        static_cast<FlatCurveEditor*>(luminanceCEG->addCurve(CT_Flat, M("TP_BWMIX_VAL")));
    ...
    mixerRed =
        Gtk::manage(new Adjuster(/*M("TP_BWMIX_RED")*/ "", -100, 200, 1, 33, imgIcon[0]));
...
bool BlackWhite::BWComputed_()
{
    ...
    if (listener && nextcount <= 1) {
        // activated only 1 time, but perhaps in some cases if we want that all is update
        // pp3, auto, sliders : nextcount <= 2 but it cost time and result is identical
...
void BlackWhite::read(const ProcParams* pp, const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        mixerMagenta->setEditedState(pedited->blackwhite.mixerMagenta ? Edited
                                                                      : UnEdited);
...
{
    if (listener) {
        listener->panelChanged(EvBWBeforeCurveMode,
                               escapeHtmlChars(beforeCurveMode->get_active_text()));
    }
...
*/
void BlackWhite::colorForValue(double valX,
                               double valY,
...
void BlackWhite::settingChanged()
{
    ...
    // Checking "listener" to avoid "autoch" getting toggled off because it has to change
    // the sliders when toggling on
    if (listener) {
...
void BlackWhite::setDefaults(const ProcParams* defParams, const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        mixerGreen->setDefaultEditedState(pedited->blackwhite.mixerGreen ? Edited
                                                                         : UnEdited);
        mixerBlue->setDefaultEditedState(pedited->blackwhite.mixerBlue ? Edited
                                                                       : UnEdited);
        gammaRed->setDefaultEditedState(pedited->blackwhite.gammaRed ? Edited : UnEdited);
        gammaGreen->setDefaultEditedState(pedited->blackwhite.gammaGreen ? Edited
                                                                         : UnEdited);
        gammaBlue->setDefaultEditedState(pedited->blackwhite.gammaBlue ? Edited
                                                                       : UnEdited);
        mixerOrange->setDefaultEditedState(pedited->blackwhite.mixerOrange ? Edited
                                                                           : UnEdited);
        mixerYellow->setDefaultEditedState(pedited->blackwhite.mixerYellow ? Edited
                                                                           : UnEdited);
        mixerCyan->setDefaultEditedState(pedited->blackwhite.mixerCyan ? Edited
                                                                       : UnEdited);
        mixerMagenta->setDefaultEditedState(pedited->blackwhite.mixerMagenta ? Edited
                                                                             : UnEdited);
        mixerPurple->setDefaultEditedState(pedited->blackwhite.mixerPurple ? Edited
                                                                           : UnEdited);
    } else {
...
void BlackWhite::adjusterChanged(Adjuster* a, double newval)
{
    ...
    // the sliders when toggling on
    if (listener
        && (a == mixerRed || a == mixerGreen || a == mixerBlue || a == mixerOrange
            || a == mixerYellow || a == mixerMagenta || a == mixerPurple
            || a == mixerCyan))
    {
        if (multiImage && autoch->get_inconsistent()) {
            autoch->set_inconsistent(false);
...
void BlackWhite::updateRGBLabel()
{
    if (!batchMode) {
        ...
        Color::computeBWMixerConstants(
            sSetting, getFilterString(), getalgoString(), filcor, r, g, b,
            mixerOrange->getValue(), mixerYellow->getValue(), mixerCyan->getValue(),
            mixerPurple->getValue(), mixerMagenta->getValue(), autoch->get_active(),
            enabledcc->get_active(), kcorrec, mixR, mixG, mixB);
        ...
        RGBLabels->set_text(Glib::ustring::compose(
            M("TP_BWMIX_RGBLABEL"),
        ...
        if (sSetting != "RGB-Abs" && sSetting != "RGB-Rel" && sSetting != "ROYGCBPM-Abs"
            && sSetting != "ROYGCBPM-Rel")
        {
