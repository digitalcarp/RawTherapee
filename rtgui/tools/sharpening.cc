...
void Sharpening::read(const ProcParams* pp, const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        etolerance->setEditedState(pedited->sharpening.edges_tolerance ? Edited
                                                                       : UnEdited);
        hcamount->setEditedState(pedited->sharpening.halocontrol_amount ? Edited
                                                                        : UnEdited);
...
void Sharpening::setDefaults(const ProcParams* defParams, const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        threshold->setDefaultEditedState(pedited->sharpening.threshold ? Edited
                                                                       : UnEdited);
        eradius->setDefaultEditedState(pedited->sharpening.edges_radius ? Edited
                                                                        : UnEdited);
        etolerance->setDefaultEditedState(pedited->sharpening.edges_tolerance ? Edited
                                                                              : UnEdited);
        hcamount->setDefaultEditedState(
            pedited->sharpening.halocontrol_amount ? Edited : UnEdited);
        damount->setDefaultEditedState(pedited->sharpening.deconvamount ? Edited
                                                                        : UnEdited);
        dradius->setDefaultEditedState(pedited->sharpening.deconvradius ? Edited
                                                                        : UnEdited);
        diter->setDefaultEditedState(pedited->sharpening.deconviter ? Edited : UnEdited);
        ddamping->setDefaultEditedState(pedited->sharpening.deconvdamping ? Edited
                                                                          : UnEdited);
    } else {
...
void Sharpening::adjusterChanged(ThresholdAdjuster* a,
...
void Sharpening::adjusterChanged2(ThresholdAdjuster* a,
                                  int newBottomL,
                                  int newTopL,
                                  int newBottomR,
                                  int newTopR)
...
void Sharpening::setAdjusterBehavior(bool contrastadd,
                                     bool radiusadd,
                                     bool amountadd,
                                     bool dampingadd,
                                     bool iteradd,
