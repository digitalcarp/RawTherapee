                     ...
                     const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        caAutoiterations->setEditedState(pedited->raw.caautoiterations ? Edited
                                                                       : UnEdited);
...
void RAWCACorr::checkBoxToggled(CheckBox* c, CheckValue newval)
{
    if (c == caAutocorrect) {
        ...
        if (listener) {
            listener->panelChanged((caAutocorrect->getLastActive()
                                    || caRed->getValue() != 0 || caBlue->getValue() != 0)
                                       ? EvPreProcessCAColourshift
                                       : EvPreProcessCAColourshiftHistory,
                                   caAvoidcolourshift->getLastActive()
                                       ? M("GENERAL_ENABLED")
                                       : M("GENERAL_DISABLED"));
        }
    }
                            ...
                            const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        caAutoiterations->setDefaultEditedState(pedited->raw.caautoiterations ? Edited
                                                                              : UnEdited);
