...
SharpenMicro::SharpenMicro()
    : FoldableToolPanel(this, TOOL_NAME, M("TP_SHARPENMICRO_LABEL"), true, true)
{

    auto m = ProcEventMapper::getInstance();
    EvSharpenMicroContrast =
        m->newEvent(SHARPENING, "HISTORY_MSG_MICROCONTRAST_CONTRAST");
    ...
    matrixconn = matrix->signal_toggled().connect(
        sigc::mem_fun(*this, &SharpenMicro::matrix_toggled));
}
...
void SharpenMicro::setDefaults(const ProcParams* defParams, const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        contrast->setDefaultEditedState(pedited->sharpenMicro.contrast ? Edited
                                                                       : UnEdited);
        uniformity->setDefaultEditedState(pedited->sharpenMicro.uniformity ? Edited
                                                                           : UnEdited);
    } else {
...
void SharpenMicro::setAdjusterBehavior(bool amountadd,
                                       bool contrastadd,
                                       bool uniformityadd)
{
