    ...
    : FoldableToolPanel(this, TOOL_NAME, M("TP_COMPRESSGAMUT_LABEL"), false, true)
{
    ...
    setExpandAlignProperties(acLabelrgb, true, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_START);
    acLabelrgb->set_tooltip_markup(M("TP_COMPRESSGAMUT_MACLABELRGB_TOOLTIP"));
    acLabelrgb->show();

    acLabelcmy = Gtk::manage(new Gtk::Label("---"));
    setExpandAlignProperties(acLabelcmy, true, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_START);
    acLabelcmy->set_tooltip_markup(M("TP_COMPRESSGAMUT_MACLABELRGB_TOOLTIP"));
    acLabelcmy->show();

    // Percentage of the core gamut to protect Limits
    // Values calculated to protect all the colors of the ColorChecker Classic 24 as given
    // by ISO 17321-1 and Ohta (1997)

    th_c = Gtk::manage(new Adjuster(M("TP_COMPRESSGAMUT_CYANTH"), 0., 1., 0.001,
                                    0.815));  // 0.25 sRGB 0.999 to avoid: 1 - th = 0
    th_m = Gtk::manage(new Adjuster(M("TP_COMPRESSGAMUT_MAGENTATH"), 0., 1., 0.001,
                                    0.803));  // 0.925 sRGB
    th_y = Gtk::manage(new Adjuster(M("TP_COMPRESSGAMUT_YELLOWTH"), 0., 1., 0.001,
                                    0.880));  // 0.934 sRGB
    ...
    d_c = Gtk::manage(new Adjuster(M("TP_COMPRESSGAMUT_CYANLIM"), 1.001, 10., 0.001,
                                   1.147));  // 1.05 sRGB
    d_m = Gtk::manage(new Adjuster(M("TP_COMPRESSGAMUT_MAGENTALIM"), 1.001, 10., 0.001,
                                   1.264));  // 1.08 sRGB
    d_y = Gtk::manage(new Adjuster(M("TP_COMPRESSGAMUT_YELLOWLIM"), 1.001, 10., 0.001,
                                   1.312));  // 1.10 sRGB
                                      ...
                                      bool auto_dy)
{
    ...
    idle_register.add(
...

        {
            ...
            disableListener();
            acLabel->set_text(Glib::ustring::compose(
                M("TP_COMPRESSGAMUT_MACLABEL"),

                Glib::ustring::format(std::fixed, std::setprecision(2),
                                      acmax))  // achromatic maximum value
            );
            acLabelrgb->set_text(Glib::ustring::compose(
                M("TP_COMPRESSGAMUT_MACLABELRGB"),
            ...
            acLabelcmy->set_text(Glib::ustring::compose(
                M("TP_COMPRESSGAMUT_MACLABELCMY"),
                Glib::ustring::format(std::fixed, std::setprecision(1),
                                      (acmax1 + acmax2)
                                          * 0.43),  // estimated value for Cyan - about 2
                                                    // * (acmax1 + acmax2) * Sin (PI/3)
                Glib::ustring::format(std::fixed, std::setprecision(1),
                                      (acmax0 + acmax2)
                                          * 0.43),  // estimated value for Magenta
                Glib::ustring::format(std::fixed, std::setprecision(1),
                                      (acmax0 + acmax1)
                                          * 0.43))  // estimated value for Yellow
            );
            if (auto_dc) {
...
void Compressgamut::updategamutGUI()
{
    ...
    /* Calculating the 'Threshold' values ​​is anything but straightforward. Of
    course, one might say, "Just match them with the colors in ColorChecker24." But there
    are many unknown parameters:
    * What is the actual illuminant and its temperature and green settings?
    * The image colors may have been compressed using "Maximum limits," but there's no
    guarantee they're within the new, reduced gamut.
    * Differences in white points, such as the atypical white point of DCI-P3, complicate
    matters.

    * Furthermore, what about the necessary color adaptation, both at this stage of
    compression and at the stage implied by the output temperature (see CIECAM)?

    Therefore, these parameters correspond roughly to the ratio of the distances between
    the white point of the Working Profile,
    * The white point of the "Target compression gamut" corrected by chromatic adaptation
    and the Yellow, Magenta, and Cyan values ​​of the primary color triangle.
    * These calculated values ​​are for 'normal' cases where the illuminants are not
    exhaustive, and the colors to be recovered are within reasonable limits.
    * In other cases, the 3 Threshold sliders need to be adjusted (often lowered). Pay
