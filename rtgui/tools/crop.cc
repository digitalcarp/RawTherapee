      ...
      lastFixRatio(true)
{
    ...
    setExpandAlignProperties(settingsgrid, true, false, Gtk::ALIGN_FILL,
                             Gtk::ALIGN_CENTER);
    ...
    setExpandAlignProperties(orientation, true, false, Gtk::ALIGN_FILL,
                             Gtk::ALIGN_CENTER);

    customRatioLabel = Gtk::manage(new Gtk::Label(""));
    customRatioLabel->hide();
    setExpandAlignProperties(customRatioLabel, false, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_CENTER);
    ...
    sizecm =
        Gtk::manage(new Gtk::Label(M("GENERAL_NA") + " cm x " + M("GENERAL_NA") + " cm"));
    setExpandAlignProperties(sizecm, false, false, Gtk::ALIGN_CENTER, Gtk::ALIGN_CENTER);

    sizein =
        Gtk::manage(new Gtk::Label(M("GENERAL_NA") + " in x " + M("GENERAL_NA") + " in"));
    ...
    yconn = y->signal_value_changed().connect(
        sigc::mem_fun(*this, &Crop::positionChanged), true);
    wconn = w->signal_value_changed().connect(sigc::mem_fun(*this, &Crop::widthChanged),
                                              true);
    hconn = h->signal_value_changed().connect(sigc::mem_fun(*this, &Crop::heightChanged),
                                              true);
    fconn =
    ...
    oconn =
        orientation->signal_changed().connect(sigc::mem_fun(*this, &Crop::ratioChanged));
...
void Crop::read(const ProcParams* pp, const ParamsEdited* pedited)
{
    ...
    const bool flip_orientation =
        pp->crop.fixratio && crop_ratios->getValue(ratio->get_active_row_number()) > 0
...
void Crop::write(ProcParams* pp, ParamsEdited* pedited)
{
    ...
    // instead of 3:2, but in GUI 'landscape' is always long side horizontal regardless of
    // the ratio is written short or long side first.
    const bool flip_orientation =
        fixr->get_active() && crop_ratios->getValue(ratio->get_active_row_number()) > 0
        && crop_ratios->getValue(ratio->get_active_row_number()) < 1.0;
    ...
    if (listener && getEnabled()) {
        if (nw == 1 && nh == 1) {
            ...
            listener->panelChanged(
                EvCrop, Glib::ustring::compose("%1=%2, %3=%4\n%5=%6, %7=%8",
                                               M("TP_CROP_X"), nx, M("TP_CROP_Y"), ny,
                                               M("TP_CROP_W"), nw, M("TP_CROP_H"), nh));
        }
    }
}
...
void Crop::refreshSize()
{
    ...
    if (!batchMode) {
        ...
        ostrcm << nh / ppi->get_value() * 2.54 << " cm x " << nw / ppi->get_value() * 2.54
               << " cm";
        ;
...
void Crop::updateCurrentRatio()
{
    ...
    crop_ratios->updateCurrentRatio(static_cast<double>(w->get_value())
                                    / static_cast<double>(h->get_value()));
}
