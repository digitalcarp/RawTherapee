    ...
    public rtengine::AutoWBListener
{
    ...
    enum WB_LabelType { WBLT_GUI, WBLT_PP };
...
protected:
    ...
    Glib::ustring
        wbIcons[rtengine::toUnderlying(rtengine::procparams::WBEntry::Type::CUSTOM) + 1];
    ...
    double nextTemp;
    double nextGreen;
    WBProvider* wbp;  // pointer to a ToolPanelCoordinator object, or its subclass
                      // BatchToolPanelCoordinator
    ...
    void cache_customWB(int temp,
                        double green);     // cache custom WB setting to allow its recall
    ...
    unsigned int findWBEntryId(const Glib::ustring& label,
                               enum WB_LabelType lblType = WBLT_GUI);
    std::pair<bool, const rtengine::procparams::WBEntry&>
    findWBEntry(const Glib::ustring& label, enum WB_LabelType lblType = WBLT_GUI);
...
public:
    ...
    void WBChanged(int met,
                   ...
                   float histmin,
                   float histmax,
                   AWBMode aWBMode) override;
    void itcwb_alg_toggled();
    void itcwb_prim_changed();
    void
    setAdjusterBehavior(bool tempadd, bool greenadd, bool equaladd, bool tempbiasadd);
