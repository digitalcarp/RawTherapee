    ...
    : FoldableToolPanel(this, TOOL_NAME, M("TP_VIBRANCE_LABEL"), false, true)
{
    ...
    psThreshold = Gtk::manage(
        new ThresholdAdjuster(M("TP_VIBRANCE_PSTHRESHOLD"), -100., 100., 0.,
                              M("TP_VIBRANCE_PSTHRESHOLD_WEIGTHING"), 0, 0., 100., 75.,
    ...
    curveEditorGG = new CurveEditorGroup(options.lastVibranceCurvesDir,
...
void Vibrance::adjusterChanged(ThresholdAdjuster* a,
...
{
    if (listener && getEnabled()) {
        listener->panelChanged(EvVibrancePastSatThreshold,
...
void Vibrance::adjusterChanged2(ThresholdAdjuster* a,
                                int newBottomL,
                                int newTopL,
                                int newBottomR,
                                int newTopR)
...
void Vibrance::setDefaults(const ProcParams* defParams, const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        psThreshold->setDefaultEditedState(pedited->vibrance.psthreshold ? Edited
                                                                         : UnEdited);
