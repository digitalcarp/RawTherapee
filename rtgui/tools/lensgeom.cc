...
LensGeometry::LensGeometry()
    : FoldableToolPanel(this, TOOL_NAME, M("TP_LENSGEOM_LABEL")),
      rlistener(nullptr),
      lastFill(false)
{
    auto m = ProcEventMapper::getInstance();
    EvTransScale = m->newEvent(TRANSFORM, "HISTORY_MSG_TRANS_SCALE");
    EvTransScaleHorizontally =
        m->newEvent(TRANSFORM, "HISTORY_MSG_TRANS_SCALE_HORIZONTALLY");
    ...
    scale_horizontally =
        Gtk::manage(new Adjuster(M("TP_LENSGEOM_SCALE_HORIZONTALLY"), 0.1, 10, 0.01, 1));
    scale_horizontally->setAdjusterListener(this);
    scale_horizontally->setLogScale(300, 0.1);
    pack_start(*scale_horizontally);

    scale_vertically =
        Gtk::manage(new Adjuster(M("TP_LENSGEOM_SCALE_VERTICALLY"), 0.1, 10, 0.01, 1));
    ...
    fillConn =
        fill->signal_toggled().connect(sigc::mem_fun(*this, &LensGeometry::fillPressed));
...
void LensGeometry::read(const ProcParams* pp, const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        scale_horizontally->setEditedState(
            pedited->commonTrans.scale_horizontally ? Edited : UnEdited);
        scale_vertically->setEditedState(
            pedited->commonTrans.scale_vertically ? Edited : UnEdited);
    }
