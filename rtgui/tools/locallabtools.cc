...
void update_ghs_curve(double b,
                      double d,
                      double lp,
                      double sp,
                      double hp,
                      ...
                      LabGrid& lab_grid)
{
    const rtengine::ImProcFunctions::GHTStrType str_type =
        inverse ? rtengine::ImProcFunctions::GHTStrType::INVERSE
                : rtengine::ImProcFunctions::GHTStrType::NORMAL;
    const rtengine::ght_compute_params c =
        rtengine::ImProcFunctions::GHT_setup(b, d, lp, sp, hp, str_type);
    ...
    const LabGridArea::FunctionParams::ResolutionFunction resolution_function =
        [](int width) -> int { return std::min(width, 400); };
    lab_grid.setFunctionParams(
        LabGridArea::FunctionParams(0.0, 1.0, 0.0, 1.0, function, resolution_function));
}
...
/* ==== LocallabTool ==== */
LocallabTool::LocallabTool(Gtk::Box* content,
      ...
      complexity(Gtk::manage(new MyComboBoxText()))
{
    ...
    titleLabel->set_markup(Glib::ustring("<b>") + escapeHtmlChars(UILabel)
                           + Glib::ustring("</b>"));
    titleLabel->set_alignment(Gtk::ALIGN_START, Gtk::ALIGN_CENTER);
    titleBox->pack_start(*titleLabel, Gtk::PACK_EXPAND_WIDGET, 0);

    Gtk::EventBox* const removeEvBox = Gtk::manage(
        new Gtk::EventBox());  // Glue to manage mouse clicking event on remove image
    removeEvBox->set_can_focus(false);
    removeEvBox->set_above_child(false);  // To have priority over expander title bar when
                                          // mouse clicking on remove image
    removeEvBox->signal_button_release_event().connect(
        sigc::mem_fun(this, &LocallabTool::on_remove_change));
    RTImage* const removeImage =
    ...
    if (needMode) {
        ...
        complexityConn = complexity->signal_changed().connect(
            sigc::mem_fun(*this, &LocallabTool::complexityModeChanged));
    }
    ...
    if (need100Percent) {
        RTImage* const titleImage =
            Gtk::manage(new RTImage("one-to-one-small", Gtk::ICON_SIZE_BUTTON));
    ...
    exp->signal_button_release_event().connect_notify(
        sigc::mem_fun(this, &LocallabTool::foldThemAll));
    enaExpConn = exp->signal_enabled_toggled().connect(
        sigc::mem_fun(*this, &LocallabTool::enabledChanged));
...
void LocallabTool::refChanged(const double huer,
                              const double lumar,
                              ...
                              const float fab)
{
    ...
    // (double) corfap * (chromar / 195.f));
    const double normChromar =
        LIM01(corfap * (chromar / 195.f));  // 195 a little more than 128 * 1.414 = 181
...
void LocallabTool::colorForValue(double valX,
                                 double valY,
...
bool LocallabTool::on_remove_change(GdkEventButton* event)
{
    ...
    return true;  // No event propagation further (to avoid closing expander when mouse
                  // clicking on remove image)
}
...
{
    if (complexity->get_active_row_number() == Simple)
    {  // New selected mode is Simple one
        ...
        if (listener && isLocActivated) {
            listener->panelChanged(EvlocallabcomplexityWithRefresh,
                                   M("TP_LOCALLAB_MODE_SIMPLE") + " ("
                                       + escapeHtmlChars(getSpotName()) + ")");
        }
    } else if (complexity->get_active_row_number() == Normal)
    {  // New selected mode is Normal one
        // Convert tool widget parameters
        convertParamToNormal();
        // Update GUI based on new mode
        updateGUIToMode(Normal);
        ...
        if (listener && isLocActivated) {
            listener->panelChanged(EvlocallabcomplexityWithRefresh,
                                   M("TP_LOCALLAB_MODE_NORMAL") + " ("
                                       + escapeHtmlChars(getSpotName()) + ")");
        }
    } else if (complexity->get_active_row_number() == Expert)
    {  // New selected mode is Expert one
        // Update GUI based on new mode
        updateGUIToMode(Expert);

        if (listener && isLocActivated) {
            listener->panelChanged(EvlocallabcomplexityWithRefresh,
                                   M("TP_LOCALLAB_MODE_EXPERT") + " ("
                                       + escapeHtmlChars(getSpotName()) + ")");
        }
    }
}
...
LocallabColor::LocallabColor()
    : LocallabTool(this, M("TP_LOCALLAB_COLOR_TOOLNAME"), M("TP_LOCALLAB_COFR"), false),
      ...
      labgrid(Gtk::manage(new LabGrid(EvLocallabLabGridValue,
                                      M("TP_LOCALLAB_LABGRID_VALUES"),
                                      true,
                                      false))),
      ...
      strcolab(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_GRADSTRCHRO"), -6., 6., 0.05, 0.))),
      ...
      llCurveEditorG(new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
                                          M("TP_LOCALLAB_LUM"))),
      llshape(static_cast<DiagonalCurveEditor*>(
          llCurveEditorG->addCurve(CT_Diagonal, "L(L)"))),
      ccshape(static_cast<DiagonalCurveEditor*>(
          llCurveEditorG->addCurve(CT_Diagonal, "C(C)"))),
      clCurveEditorG(new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
                                          M("TP_LOCALLAB_CH"))),
      clshape(static_cast<DiagonalCurveEditor*>(
          clCurveEditorG->addCurve(CT_Diagonal, "C(L)"))),
      lcshape(static_cast<DiagonalCurveEditor*>(
          clCurveEditorG->addCurve(CT_Diagonal, "L(C)"))),
      HCurveEditorG(new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
                                         M("TP_LOCALLAB_HLH"))),
      LHshape(static_cast<FlatCurveEditor*>(
          HCurveEditorG->addCurve(CT_Flat, "L(h)", nullptr, false, true))),
      H3CurveEditorG(new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
                                          M("TP_LOCALLAB_HLH"))),
      CHshape(static_cast<FlatCurveEditor*>(
          H3CurveEditorG->addCurve(CT_Flat, "C(h)", nullptr, false, true))),
      H2CurveEditorG(new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
                                          M("TP_LOCALLAB_HLH"))),
      HHshape(static_cast<FlatCurveEditor*>(
          H2CurveEditorG->addCurve(CT_Flat, "h(h)", nullptr, false, true))),
      rgbCurveEditorG(new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
                                           M("TP_LOCALLAB_RGB"))),
      toneMethod(Gtk::manage(new MyComboBoxText())),
      rgbshape(static_cast<DiagonalCurveEditor*>(
          rgbCurveEditorG->addCurve(CT_Diagonal, "", toneMethod))),
      ...
      labgridmerg(Gtk::manage(new LabGrid(EvLocallabLabGridmergValue,
                                          M("TP_LOCALLAB_LABGRID_VALUES"),
                                          false))),
      merlucol(Gtk::manage(new Adjuster(M("TP_LOCALLAB_MERLUCOL"),
                                        0.0,
                                        100.0,
                                        0.5,
                                        32.,
      ...
      //    CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
      //    M("TP_LOCALLAB_MASKCOL"))),
      maskCurveEditorG(
          new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir, "", 1)),
      CCmaskshape(static_cast<FlatCurveEditor*>(
          maskCurveEditorG->addCurve(CT_Flat, "C", nullptr, false, false))),
      LLmaskshape(static_cast<FlatCurveEditor*>(
          maskCurveEditorG->addCurve(CT_Flat, "L", nullptr, false, false))),
      HHmaskshape(static_cast<FlatCurveEditor*>(
          maskCurveEditorG->addCurve(CT_Flat, "LC(h)", nullptr, false, true))),
      ...
      //   maskHCurveEditorG(new
      //   CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
      //   M("TP_LOCALLAB_MASKH"))),
      maskHCurveEditorG(
          new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir, "")),
      HHhmaskshape(static_cast<FlatCurveEditor*>(
          maskHCurveEditorG->addCurve(CT_Flat, "h(h)", nullptr, false, true))),
      mask2CurveEditorG(new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
                                             M("TP_LOCALLAB_MASK2"))),
      Lmaskshape(static_cast<DiagonalCurveEditor*>(
          mask2CurveEditorG->addCurve(CT_Diagonal, "L(L)"))),
      mask2CurveEditorGwav(
          new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
                               M("TP_LOCALLAB_WAVMASK"))),
      LLmaskcolshapewav(static_cast<FlatCurveEditor*>(
          mask2CurveEditorGwav->addCurve(CT_Flat, "L(L)", nullptr, false, false))),
      csThresholdcol(Gtk::manage(new ThresholdAdjuster(M("TP_LOCALLAB_CSTHRESHOLDBLUR"),
                                                       0,
                                                       9,
                                                       0,
                                                       0,
                                                       ...
                                                       false)))
{
    ...
    curvactivConn = curvactiv->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabColor::curvactivChanged));
    ...
    gridMethodConn = gridMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabColor::gridMethodChanged));
    ...
    inversConn = invers->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabColor::inversChanged));
    ...
    previewcolConn = previewcol->signal_clicked().connect(
        sigc::mem_fun(*this, &LocallabColor::previewcolChanged));

    qualitycurveMethod->append(M("TP_LOCALLAB_CURVNONE"));
    qualitycurveMethod->append(M("TP_LOCALLAB_CURVCURR"));
    qualitycurveMethod->set_active(0);
    qualitycurveMethodConn = qualitycurveMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabColor::qualitycurveMethodChanged));
    ...
    toneMethodConn = toneMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabColor::toneMethodChanged));
    ...
    specialConn = special->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabColor::specialChanged));
    ...
    merMethodConn = merMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabColor::merMethodChanged));
    ...
    mergecolMethodConn = mergecolMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabColor::mergecolMethodChanged));
    ...
    showmaskcolMethodConn = showmaskcolMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabColor::showmaskcolMethodChanged));
    ...
    showmaskcolMethodConninv = showmaskcolMethodinv->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabColor::showmaskcolMethodChangedinv));

    enaColorMaskConn = enaColorMask->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabColor::enaColorMaskChanged));

    maskCurveEditorG->setCurveListener(this);

    CCmaskshape->setIdentityValue(0.);
    CCmaskshape->setResetCurve(FlatCurveType(defSpot.CCmaskcurve.at(0)),
                               defSpot.CCmaskcurve);
    CCmaskshape->setBottomBarColorProvider(this, 1);

    LLmaskshape->setIdentityValue(0.);
    LLmaskshape->setResetCurve(FlatCurveType(defSpot.LLmaskcurve.at(0)),
                               defSpot.LLmaskcurve);
    LLmaskshape->setBottomBarBgGradient({ { 0., 0., 0., 0. }, { 1., 1., 1., 1. } });

    HHmaskshape->setIdentityValue(0.);
    HHmaskshape->setResetCurve(FlatCurveType(defSpot.HHmaskcurve.at(0)),
                               defSpot.HHmaskcurve);
    ...
    toolcolConn = toolcol->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabColor::toolcolChanged));

    blurFrame->set_label_align(0.025, 0.5);

    fftColorMaskConn = fftColorMask->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabColor::fftColorMaskChanged));
    ...
    HHhmaskshape->setResetCurve(FlatCurveType(defSpot.HHhmaskcurve.at(0)),
                                defSpot.HHhmaskcurve);
    ...
    Lmaskshape->setResetCurve(DiagonalCurveType(defSpot.Lmaskcurve.at(0)),
                              defSpot.Lmaskcurve);
    ...
    LLmaskcolshapewav->setResetCurve(FlatCurveType(defSpot.LLmaskcolcurvewav.at(0)),
                                     defSpot.LLmaskcolcurvewav);
    ...
    curvBox->pack_start(*llCurveEditorG, Gtk::PACK_SHRINK,
                        4);  // Padding is mandatory to correct behavior of curve editor
    curvBox->pack_start(*clCurveEditorG, Gtk::PACK_SHRINK,
                        4);  // Padding is mandatory to correct behavior of curve editor
    curvBox->pack_start(*HCurveEditorG, Gtk::PACK_SHRINK,
                        4);  // Padding is mandatory to correct behavior of curve editor
    curvBox->pack_start(*H3CurveEditorG, Gtk::PACK_SHRINK,
                        4);  // Padding is mandatory to correct behavior of curve editor
    curvBox->pack_start(*H2CurveEditorG, Gtk::PACK_SHRINK,
                        4);  // Padding is mandatory to correct behavior of curve editor
    curvBox->pack_start(*rgbCurveEditorG, Gtk::PACK_SHRINK,
                        4);  // Padding is mandatory to correct behavior of curve editor
    ...
    maskcolBox->pack_start(
        *maskCurveEditorG, Gtk::PACK_SHRINK,
        4);  // Padding is mandatory to correct behavior of curve editor
    ...
    //    Gtk::Frame* const toolcolFrame = Gtk::manage(new
    //    Gtk::Frame(M("TP_LOCALLAB_TOOLMASK")));
    ...
    toolcolBox->pack_start(
        *maskHCurveEditorG, Gtk::PACK_SHRINK,
        4);  // Padding is mandatory to correct behavior of curve editor
    toolcolBox->pack_start(
        *mask2CurveEditorG, Gtk::PACK_SHRINK,
        4);  // Padding is mandatory to correct behavior of curve editor
    ToolParamBlock* const toolcolBox2 = Gtk::manage(new ToolParamBlock());
    toolcolBox2->pack_start(
        *mask2CurveEditorGwav, Gtk::PACK_SHRINK,
        4);  // Padding is mandatory to correct behavior of curve editor
...
{
    {
        idle_register.add([this, scope]() -> bool {
            GThreadLock lock;  // All GUI access from idle_add callbacks or separate
                               // thread HAVE to be protected
...
{
    return ((showmaskcolMethod->get_active_row_number() != 0)
            || (showmaskcolMethodinv->get_active_row_number() != 0));
}
...
void LocallabColor::read(const rtengine::procparams::ProcParams* pp,
                         const ParamsEdited* pedited)
{
    ...
    if (index < (int)pp->locallab.spots.size()) {
        ...
        labgrid->setParams(spot.labgridALow / LocallabParams::LABGRIDL_CORR_MAX,
                           ...
                           spot.labgridBHigh / LocallabParams::LABGRIDL_CORR_MAX, 0, 0, 0,
                           0, 0, 0, false);
        // printf("labgridlow=%f \n", spot.labgridALow);
...
void LocallabColor::write(rtengine::procparams::ProcParams* pp, ParamsEdited* pedited)
{
    ...
    if (index < (int)pp->locallab.spots.size()) {
        ...
        double zerox = 0.;
        double zeroy = 0.;
        labgrid->getParams(spot.labgridALow, spot.labgridBLow, spot.labgridAHigh,
        ...
        double zerox1 = 0.;
        double zeroy1 = 0.;
        labgridmerg->getParams(spot.labgridALowmerg, spot.labgridBLowmerg,
                               spot.labgridAHighmerg, spot.labgridBHighmerg, zerox1,
                               zeroy1, zerox1, zeroy1, zerox1, zeroy1);
...
void LocallabColor::setDefaults(const rtengine::procparams::ProcParams* defParams,
                                const ParamsEdited* pedited)
{
    ...
    if (index < (int)defParams->locallab.spots.size()) {
        ...
        labgrid->setDefault(defSpot.labgridALow / LocallabParams::LABGRIDL_CORR_MAX,
                            ...
                            defSpot.labgridBHigh / LocallabParams::LABGRIDL_CORR_MAX, 0,
                            0, 0, 0, 0, 0);
        ...
        labgridmerg->setDefault(
            defSpot.labgridALowmerg / LocallabParams::LABGRIDL_CORR_MAX,
            defSpot.labgridBLowmerg / LocallabParams::LABGRIDL_CORR_MAX,
            defSpot.labgridAHighmerg / LocallabParams::LABGRIDL_CORR_MAX,
            defSpot.labgridBHighmerg / LocallabParams::LABGRIDL_CORR_MAX, 0, 0, 0, 0, 0,
            0);
...
void LocallabColor::adjusterChanged2(ThresholdAdjuster* a,
                                     int newBottomL,
                                     int newTopL,
                                     int newBottomR,
                                     int newTopR)
        ...
        if (a == csThresholdcol) {
            if (listener) {
                listener->panelChanged(EvlocallabcsThresholdcol,
                                       csThresholdcol->getHistoryString() + " ("
                                           + escapeHtmlChars(getSpotName()) + ")");
            }
        }
    }
...
void LocallabColor::enabledChanged()
{
    if (isLocActivated) {
        if (listener) {
            if (exp->getEnabled()) {
                listener->panelChanged(EvLocenacolor, M("GENERAL_ENABLED") + " ("
                                                          + escapeHtmlChars(getSpotName())
                                                          + ")");
            } else {
                listener->panelChanged(EvLocenacolor, M("GENERAL_DISABLED") + " ("
                                                          + escapeHtmlChars(getSpotName())
                                                          + ")");
            }
        }
...
void LocallabColor::updateMaskBackground(const double normChromar,
    ...
    if (isLocActivated && exp->getEnabled()) {
        if (listener) {
            listener->panelChanged(EvlocallabqualitycurveMethod,
                                   qualitycurveMethod->get_active_text() + " ("
                                       + escapeHtmlChars(getSpotName()) + ")");
        }
    }
}
    ...
    if (isLocActivated && exp->getEnabled()) {
        if (listener) {
            listener->panelChanged(EvLocallabmergecolMethod,
                                   mergecolMethod->get_active_text() + " ("
                                       + escapeHtmlChars(getSpotName()) + ")");
        }
    }
}
...
{
    // Note: When a merMethod is selected, invers button is set insensitive to avoid the
...
LocallabExposure::LocallabExposure()
    : LocallabTool(this, M("TP_LOCALLAB_EXP_TOOLNAME"), M("TP_LOCALLAB_EXPOSE"), false),
      ...
      laplacexp(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_LAPLACEXP"), 0.0, 100.0, 0.1, 0.))),
      reparexp(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_LOGREPART"), 1.0, 100.0, 1., 100.0))),
      ...
      fatdetail(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_FATDETAIL"), -100., 300., 1., 0.))),
      ...
      fatanchor(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_FATANCHOR"),
                                   0.1,
                                   100.0,
                                   0.01,
                                   50.,
      ...
      expcomp(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_EXPCOMP"), MINEXP, MAXEXP, 0.01, 0.))),
      ...
      curveEditorG(new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
                                        M("TP_LOCALLAB_CURVEEDITOR_TONES_LABEL"))),
      shapeexpos(
          static_cast<DiagonalCurveEditor*>(curveEditorG->addCurve(CT_Diagonal, ""))),
      ...
      //   maskexpCurveEditorG(new
      //   CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
      //   M("TP_LOCALLAB_MASK"))),
      maskexpCurveEditorG(
          new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir, "", 1)),
      CCmaskexpshape(static_cast<FlatCurveEditor*>(
          maskexpCurveEditorG->addCurve(CT_Flat, "C", nullptr, false, false))),
      LLmaskexpshape(static_cast<FlatCurveEditor*>(
          maskexpCurveEditorG->addCurve(CT_Flat, "L", nullptr, false, false))),
      HHmaskexpshape(static_cast<FlatCurveEditor*>(
          maskexpCurveEditorG->addCurve(CT_Flat, "LC(h)", nullptr, false, true))),
      blendmaskexp(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_BLENDMASKCOL"), -100, 100, 1, 0))),
      radmaskexp(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_RADMASKCOL"), 0.0, 100.0, 0.1, 0.))),
      ...
      angmaskexp(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_GRADANG"), -180., 180., 0.1, 0.))),
      mask2expCurveEditorG(
          new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
                               M("TP_LOCALLAB_MASK2"))),
      Lmaskexpshape(static_cast<DiagonalCurveEditor*>(
          mask2expCurveEditorG->addCurve(CT_Diagonal, "L(L)"))),
      Evlocallabtmosatur(
          ProcEventMapper::getInstance()->newEvent(AUTOEXP,
                                                   "HISTORY_MSG_LOCAL_TMO_SATUR"))

{
    ...
    expMethodConn = expMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabExposure::expMethodChanged));
    ...
    exnoiseMethodConn = exnoiseMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabExposure::exnoiseMethodChanged));
    ...
    previewexeConn = previewexe->signal_clicked().connect(
        sigc::mem_fun(*this, &LocallabExposure::previewexeChanged));

    setExpandAlignProperties(exprecove, true, false, Gtk::ALIGN_FILL, Gtk::ALIGN_START);
    normConn = norm->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabExposure::normChanged));
    fatsaturConn = fatsatur->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabExposure::fatsaturChanged));

    inversexConn = inversex->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabExposure::inversexChanged));
    ...
    showmaskexpMethodConn = showmaskexpMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabExposure::showmaskexpMethodChanged));
    ...
    showmaskexpMethodConninv = showmaskexpMethodinv->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabExposure::showmaskexpMethodChangedinv));

    enaExpMaskConn = enaExpMask->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabExposure::enaExpMaskChanged));

    enaExpMaskaftConn = enaExpMaskaft->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabExposure::enaExpMaskaftChanged));

    maskexpCurveEditorG->setCurveListener(this);

    CCmaskexpshape->setIdentityValue(0.);
    CCmaskexpshape->setResetCurve(FlatCurveType(defSpot.CCmaskexpcurve.at(0)),
                                  defSpot.CCmaskexpcurve);
    CCmaskexpshape->setBottomBarColorProvider(this, 1);

    LLmaskexpshape->setIdentityValue(0.);
    LLmaskexpshape->setResetCurve(FlatCurveType(defSpot.LLmaskexpcurve.at(0)),
                                  defSpot.LLmaskexpcurve);
    LLmaskexpshape->setBottomBarBgGradient({ { 0., 0., 0., 0. }, { 1., 1., 1., 1 } });

    HHmaskexpshape->setIdentityValue(0.);
    HHmaskexpshape->setResetCurve(FlatCurveType(defSpot.HHmaskexpcurve.at(0)),
                                  defSpot.HHmaskexpcurve);
    ...
    Lmaskexpshape->setResetCurve(DiagonalCurveType(defSpot.Lmaskexpcurve.at(0)),
                                 defSpot.Lmaskexpcurve);
    ...
    //    Gtk::Label* const labelexpmethod = Gtk::manage(new
    //    Gtk::Label(M("TP_LOCALLAB_NOISEMETH") + ":"));
    ...
    toolBox->pack_start(*curveEditorG, Gtk::PACK_SHRINK,
                        4);  // Padding is mandatory to correct behavior of curve editor
    ...
    maskexpBox->pack_start(
        *maskexpCurveEditorG, Gtk::PACK_SHRINK,
        4);  // Padding is mandatory to correct behavior of curve editor
    ...
    maskexpBox->pack_start(
        *mask2expCurveEditorG, Gtk::PACK_SHRINK,
        4);  // Padding is mandatory to correct behavior of curve editor
...
{
    return ((showmaskexpMethod->get_active_row_number() != 0)
            || (showmaskexpMethodinv->get_active_row_number() != 0));
}
...
void LocallabExposure::read(const rtengine::procparams::ProcParams* pp,
                            const ParamsEdited* pedited)
{
    // Disable all listeners
    disableListener();
...
void LocallabExposure::setDefaults(const rtengine::procparams::ProcParams* defParams,
                                   const ParamsEdited* pedited)
{
    const int index = defParams->locallab.selspot;
...
void LocallabExposure::updateMaskBackground(const double normChromar,
                                            const double normLumar,
                                            const double normHuer,
                                            const double normHuerjz)
{
...
LocallabShadow::LocallabShadow()
    : LocallabTool(this, M("TP_LOCALLAB_SH_TOOLNAME"), M("TP_LOCALLAB_SHADHIGH"), false),
      ...
      sensihs(Gtk::manage(
          new Adjuster(M("TP_LOCALLAB_SENSI"),
                       0,
                       100,
                       1,
                       30))),  // reused - unused here, but used for normalize_mean_dt
      ...
      labgridghs(Gtk::manage(new LabGrid(EvlocallabGridciexy,
                                         M("TP_LOCALLAB_GHS_GHSDIAG"),
                                         true,
                                         false,
                                         true,
                                         false))),
      ...
      ghs_slope(Gtk::manage(
          new Adjuster(M("TP_LOCALLAB_GHS_SLOPE"), 1.0, 100.0, 0.01, 9.03296))),
      ghs_chro(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_GHS_CHRO"), -30., 100.0, 0.001, 0.))),
      ghs_B(Gtk::manage(new Adjuster(M("TP_LOCALLAB_GHS_B"), -5.0, 15.0, 0.001, 0.0))),
      ghs_SP(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_GHS_SP"), 0.0, 1.0, 0.00001, 0.015))),
      ...
      ghs_MID(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_GHS_MID"), -100.0, 100.0, 0.1, 0.0))),
      ...
      ghs_BLP(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_GHS_BLP"), -0.2, 1.0, 0.0001, 0.0))),
      ghs_HLP(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_GHS_HLP"), 0.2002, 25.0, 0.0001, 1.))),
      ...
      featherSH(Gtk::manage(
          new Adjuster(M("TP_LOCALLAB_FEATVALUE"), 10., 100., 0.1, 25.))),  // 10.
      ...
      //    maskSHCurveEditorG(new
      //    CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
      //    M("TP_LOCALLAB_MASK"))),
      maskSHCurveEditorG(
          new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir, "", 1)),
      CCmaskSHshape(static_cast<FlatCurveEditor*>(
          maskSHCurveEditorG->addCurve(CT_Flat, "C", nullptr, false, false))),
      LLmaskSHshape(static_cast<FlatCurveEditor*>(
          maskSHCurveEditorG->addCurve(CT_Flat, "L", nullptr, false, false))),
      HHmaskSHshape(static_cast<FlatCurveEditor*>(
          maskSHCurveEditorG->addCurve(CT_Flat, "LC(h)", nullptr, false, true))),
      blendmaskSH(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_BLENDMASKCOL"), -100, 100, 1, 0))),
      radmaskSH(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_RADMASKCOL"), 0.0, 100.0, 0.1, 0.))),
      ...
      mask2SHCurveEditorG(
          new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
                               M("TP_LOCALLAB_MASK2"))),
      LmaskSHshape(static_cast<DiagonalCurveEditor*>(
          mask2SHCurveEditorG->addCurve(CT_Diagonal, "L(L)"))),
      fatSHFrame(Gtk::manage(new Gtk::Frame(M("TP_LOCALLAB_FATSHFRA")))),
      fatamountSH(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_FATAMOUNT"), 1., 100., 1., 1.))),
      fatanchorSH(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_FATANCHOR"),
                                   1.,
                                   100.,
                                                   ...
                                                   "HISTORY_MSG_LOCALLAB_TE_PIVOT"))
{
    ...
    shMethodConn = shMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabShadow::shMethodChanged));
    ...
    ghsMethodConn = ghsMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabShadow::ghsMethodChanged));
    ...
    setExpandAlignProperties(ghssymLabel, true, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_START);
    ...
    setExpandAlignProperties(ghsbpwpLabels, true, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_START);
    ghsbpwpvalueLabels->set_line_wrap();
    ghsbpwpvalueLabels->set_justify(Gtk::Justification::JUSTIFY_CENTER);
    setExpandAlignProperties(ghsbpwpvalueLabels, true, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_START);
    ghscolorLabels->set_line_wrap();
    ghscolorLabels->set_justify(Gtk::Justification::JUSTIFY_CENTER);
    setExpandAlignProperties(ghscolorLabels, true, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_START);
    ghsDRLabels->set_line_wrap();
    ghsDRLabels->set_justify(Gtk::Justification::JUSTIFY_CENTER);
    setExpandAlignProperties(ghsDRLabels, true, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_START);
    ...
    ghs_autobwConn = ghs_autobw->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabShadow::ghs_autobwChanged));
    ghs_smoothConn = ghs_smooth->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabShadow::ghs_smoothChanged));
    ghs_invConn = ghs_inv->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabShadow::ghs_invChanged));
    ghs_agxConn = ghs_agx->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabShadow::ghs_agxChanged));
    ...
    inversshConn = inverssh->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabShadow::inversshChanged));
    ...
    previewshConn = previewsh->signal_clicked().connect(
        sigc::mem_fun(*this, &LocallabShadow::previewshChanged));
    ...
    showmaskSHMethodConn = showmaskSHMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabShadow::showmaskSHMethodChanged));
    ...
    showmaskSHMethodConninv = showmaskSHMethodinv->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabShadow::showmaskSHMethodChangedinv));

    enaSHMaskConn = enaSHMask->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabShadow::enaSHMaskChanged));

    maskSHCurveEditorG->setCurveListener(this);

    CCmaskSHshape->setIdentityValue(0.);
    CCmaskSHshape->setResetCurve(FlatCurveType(defSpot.CCmaskSHcurve.at(0)),
                                 defSpot.CCmaskSHcurve);
    CCmaskSHshape->setBottomBarColorProvider(this, 1);

    LLmaskSHshape->setIdentityValue(0.);
    LLmaskSHshape->setResetCurve(FlatCurveType(defSpot.LLmaskSHcurve.at(0)),
                                 defSpot.LLmaskSHcurve);
    LLmaskSHshape->setBottomBarBgGradient({ { 0., 0., 0., 0. }, { 1., 1., 1., 1. } });

    HHmaskSHshape->setIdentityValue(0.);
    HHmaskSHshape->setResetCurve(FlatCurveType(defSpot.HHmaskSHcurve.at(0)),
                                 defSpot.HHmaskSHcurve);
    ...
    LmaskSHshape->setResetCurve(DiagonalCurveType(defSpot.LmaskSHcurve.at(0)),
                                defSpot.LmaskSHcurve);
    ...
    maskSHBox->pack_start(*maskSHCurveEditorG, Gtk::PACK_SHRINK,
                          4);  // Padding is mandatory to correct behavior of curve editor
    ...
    maskSHBox->pack_start(*mask2SHCurveEditorG, Gtk::PACK_SHRINK,
                          4);  // Padding is mandatory to correct behavior of curve editor
...
{
    return ((showmaskSHMethod->get_active_row_number() != 0)
            || (showmaskSHMethodinv->get_active_row_number() != 0));
}
...
void LocallabShadow::getMaskView(int& colorMask,
                                 int& colorMaskinv,
                                 int& expMask,
                                 int& expMaskinv,
                                 int& shMask,
...
void LocallabShadow::read(const rtengine::procparams::ProcParams* pp,
                          const ParamsEdited* pedited)
{
    // Disable all listeners
    disableListener();
    ...
    ghsMethodChanged();
    update_ghs_curve(ghs_B->getValue(), ghs_D->getValue(), ghs_LP->getValue(),
                     ghs_SP->getValue(), ghs_HP->getValue(), ghs_inv->get_active(),
                     *labgridghs);
    // Enable all listeners
    enableListener();
...
void LocallabShadow::setDefaults(const rtengine::procparams::ProcParams* defParams,
                                 const ParamsEdited* pedited)
{
    const int index = defParams->locallab.selspot;
...
void LocallabShadow::adjusterChanged(Adjuster* a, double newval)
{
    ...
    if (isLocActivated && exp->getEnabled()) {
        if (a == multipliersh[0] || a == multipliersh[1] || a == multipliersh[2]
            || a == multipliersh[3] || a == multipliersh[4] || a == multipliersh[5])
        {
            if (listener) {
                listener->panelChanged(
                    EvlocallabEqualizersh,
                    Glib::ustring::compose(
                        "%1, %2, %3, %4, %5, %6",
                        Glib::ustring::format(std::fixed, std::setprecision(2),
                                              multipliersh[0]->getIntValue()),
                        Glib::ustring::format(std::fixed, std::setprecision(2),
                                              multipliersh[1]->getIntValue()),
                        Glib::ustring::format(std::fixed, std::setprecision(2),
                                              multipliersh[2]->getIntValue()),
                        Glib::ustring::format(std::fixed, std::setprecision(2),
                                              multipliersh[3]->getIntValue()),
                        Glib::ustring::format(std::fixed, std::setprecision(2),
                                              multipliersh[4]->getIntValue()),
                        Glib::ustring::format(std::fixed, std::setprecision(2),
                                              multipliersh[5]->getIntValue()))
                        + " (" + escapeHtmlChars(getSpotName()) + ")");
            }
        }
        ...
        if (a == ghs_D) {
            if (ghs_D->getValue() > 0.002 || ghs_D->getValue() == 0.f)
            {  // hide sliders WP and HP si D too big
                ghs_BLP->set_sensitive(false);
                ghs_HLP->set_sensitive(false);
                ghs_inv->set_sensitive(false);
                ghs_autobw->set_sensitive(false);
        ...
        if (a == fatanchorSH) {
            if (listener) {
                listener->panelChanged(EvlocallabfatanchorSH,
                                       fatanchorSH->getTextValue() + " ("
                                           + escapeHtmlChars(getSpotName()) + ")");
            }
        }

        if (a == ghs_D || a == ghs_B || a == ghs_SP || a == ghs_LP || a == ghs_HP) {
            update_ghs_curve(ghs_B->getValue(), ghs_D->getValue(), ghs_LP->getValue(),
                             ghs_SP->getValue(), ghs_HP->getValue(),
                             ghs_inv->get_active(), *labgridghs);
        }
    }
}

void LocallabShadow::updateghsbw2(double ghsb,
                                  double ghsw,
                                  bool ghsaut)  // auto GHS black point and white point
{
    idle_register.add([this, ghsb, ghsw, ghsaut]() -> bool {
        GThreadLock lock;  // All GUI access from idle_add callbacks or separate thread
                           // HAVE to be protected
        if (ghsaut) {
            nbwb++;
...
void LocallabShadow::updateghsbw(
    int bp,
    int wp,
    double minbp,
    double maxwp,
    ...
    double maxB,
    double drghs,
    bool ghsau)  // update informations for Black point and White point
{
    idle_register.add(
        [this, bp, wp, minbp, maxwp, symev, maxR, maxG, maxB, drghs, ghsau]() -> bool {
            ...
            if (ghsau) {
                ...
                ghsDRLabels->set_text(Glib::ustring::compose(
                    M("TP_LOCALLAB_GHSDR"),
...
void LocallabShadow::updateGUIToMode(const modeType new_type)
{
    ...
    case Normal:
        ...
        {  // Keep widget hidden when shMethod is equal to 0
            gamFrame->show();
        }
        ...
        if (ghsMethod->get_active_row_number() == 2
            && shMethod->get_active_row_number() == 2)
        {
            Lab_Frame->show();
        }
        ghs_slope->hide();
...
void LocallabShadow::updateMaskBackground(const double normChromar,
                                          const double normLumar,
                                          const double normHuer,
                                          const double normHuerjz)
{
...
void LocallabShadow::updateShadowGUIsym()
{
    ...
    const double tempSP = rtengine::LIM(
        ghs_SP->getValue(), 0.0001,
        0.9999);  // avoid 0 and 1 no real sens for symmetry must be enough for most cases
    double tempHP = ghs_HP->getValue();  // high values Protect highlight
    double secur = 0.001;  // keep range security to avoid crash and wrong GUI - no or
                           // small incidence on usage
...
LocallabVibrance::LocallabVibrance()
    : LocallabTool(this, M("TP_LOCALLAB_VIB_TOOLNAME"), M("TP_LOCALLAB_VIBRANCE"), false),
      ...
      warm(Gtk::manage(new Adjuster(M("TP_LOCALLAB_WARM"),
                                    -100.,
                                    100.,
                                    1.,
                                    0.,
      ...
      psThreshold(
          Gtk::manage(new ThresholdAdjuster(M("TP_VIBRANCE_PSTHRESHOLD"),
                                            -100.,
                                            100.,
                                            0.,
                                            M("TP_VIBRANCE_PSTHRESHOLD_WEIGTHING"),
                                            0,
                                            0.,
                                            100.,
                                            75.,
                                            M("TP_VIBRANCE_PSTHRESHOLD_SATTHRESH"),
                                            0,
                                            this,
                                            false))),
      ...
      curveEditorGG(new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
                                         M("TP_VIBRANCE_CURVEEDITOR_SKINTONES_LABEL"))),
      ...
      strvibab(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_GRADSTRCHRO"), -4., 4., 0.05, 0.))),
      ...
      //   maskvibCurveEditorG(new
      //   CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
      //   M("TP_LOCALLAB_MASK"))),
      maskvibCurveEditorG(
          new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir, "", 1)),
      CCmaskvibshape(static_cast<FlatCurveEditor*>(
          maskvibCurveEditorG->addCurve(CT_Flat, "C", nullptr, false, false))),
      LLmaskvibshape(static_cast<FlatCurveEditor*>(
          maskvibCurveEditorG->addCurve(CT_Flat, "L", nullptr, false, false))),
      HHmaskvibshape(static_cast<FlatCurveEditor*>(
          maskvibCurveEditorG->addCurve(CT_Flat, "LC(h)", nullptr, false, true))),
      blendmaskvib(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_BLENDMASKCOL"), -100, 100, 1, 0))),
      radmaskvib(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_RADMASKCOL"), 0.0, 100.0, 0.1, 0.))),
      lapmaskvib(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_LAPMASKCOL"), 0.0, 100.0, 0.1, 0.))),
      chromaskvib(Gtk::manage(
          new Adjuster(M("TP_LOCALLAB_CHROMASKCOL"), -100.0, 100.0, 0.1, 0.))),
      gammaskvib(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_GAMMASKCOL"), 0.25, 4.0, 0.01, 1.))),
      slomaskvib(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_SLOMASKCOL"), 0.0, 15.0, 0.1, 0.))),
      mask2vibCurveEditorG(
          new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
                               M("TP_LOCALLAB_MASK2"))),
      Lmaskvibshape(static_cast<DiagonalCurveEditor*>(
          mask2vibCurveEditorG->addCurve(CT_Diagonal, "L(L)")))
{
    auto m = ProcEventMapper::getInstance();
    ...
    pskinsConn = protectSkins->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabVibrance::protectskins_toggled));

    ashiftConn = avoidColorShift->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabVibrance::avoidcolorshift_toggled));

    pastsattogConn = pastSatTog->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabVibrance::pastsattog_toggled));
    ...
    previewvibConn = previewvib->signal_clicked().connect(
        sigc::mem_fun(*this, &LocallabVibrance::previewvibChanged));
    ...
    showmaskvibMethodConn = showmaskvibMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabVibrance::showmaskvibMethodChanged));

    enavibMaskConn = enavibMask->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabVibrance::enavibMaskChanged));

    maskvibCurveEditorG->setCurveListener(this);

    CCmaskvibshape->setIdentityValue(0.);
    CCmaskvibshape->setResetCurve(FlatCurveType(defSpot.CCmaskvibcurve.at(0)),
                                  defSpot.CCmaskvibcurve);
    CCmaskvibshape->setBottomBarColorProvider(this, 1);

    LLmaskvibshape->setIdentityValue(0.);
    LLmaskvibshape->setResetCurve(FlatCurveType(defSpot.LLmaskvibcurve.at(0)),
                                  defSpot.LLmaskvibcurve);
    LLmaskvibshape->setBottomBarBgGradient({ { 0., 0., 0., 0. }, { 1., 1., 1., 1. } });

    HHmaskvibshape->setIdentityValue(0.);
    HHmaskvibshape->setResetCurve(FlatCurveType(defSpot.HHmaskvibcurve.at(0)),
                                  defSpot.HHmaskvibcurve);
    ...
    Lmaskvibshape->setResetCurve(DiagonalCurveType(defSpot.Lmaskvibcurve.at(0)),
                                 defSpot.Lmaskvibcurve);
    ...
    pack_start(*sensiv, Gtk::PACK_SHRINK,
               0);  // reused - nused here, but used for normalize_mean_dt
    ...
    Gtk::Separator* const separatorvib =
        Gtk::manage(new Gtk::Separator(Gtk::ORIENTATION_HORIZONTAL));
    ...
    //    pack_start(*sensiv, Gtk::PACK_SHRINK, 0);//reused - nused here, but used for
    //    normalize_mean_dt
    pack_start(*curveEditorGG, Gtk::PACK_SHRINK,
               4);  // Padding is mandatory to correct behavior of curve editor
    ...
    maskvibBox->pack_start(
        *maskvibCurveEditorG, Gtk::PACK_SHRINK,
        4);  // Padding is mandatory to correct behavior of curve editor
    ...
    maskvibBox->pack_start(
        *mask2vibCurveEditorG, Gtk::PACK_SHRINK,
        4);  // Padding is mandatory to correct behavior of curve editor
...
void LocallabVibrance::getMaskView(int& colorMask,
                                   int& colorMaskinv,
                                   int& expMask,
                                   int& expMaskinv,
                                   int& shMask,
...
void LocallabVibrance::read(const rtengine::procparams::ProcParams* pp,
                            const ParamsEdited* pedited)
{
    // Disable all listeners
    disableListener();
...
void LocallabVibrance::setDefaults(const rtengine::procparams::ProcParams* defParams,
                                   const ParamsEdited* pedited)
{
    const int index = defParams->locallab.selspot;
...
std::vector<double> LocallabVibrance::getCurvePoints(ThresholdSelector* tAdjuster) const
{
    ...
    tAdjuster->getPositions<double>(transitionWeighting,
                                    threshold);  // ( range -100;+100, range 0;+100 )
    ...
    // Y value of the chroma mean point, calculated to get a straight line between p2 and
    // s0
    ...
    // point at the location of the Top cursor, at the end of the first linear transition
...
void LocallabVibrance::updateMaskBackground(const double normChromar,
                                            const double normLumar,
                                            const double normHuer,
                                            const double normHuerjz)
{
      ...
      sensisf(Gtk::manage(new Adjuster(M("TP_LOCALLAB_SENSI"), 1, 100, 1, 30)))
{
    ...
    softMethodConn = softMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabSoft::softMethodChanged));
    ...
    showmasksoftMethodConn = showmasksoftMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabSoft::showmasksoftMethodChanged));
    ...
    Gtk::Label* const labelsoftmethod =
        Gtk::manage(new Gtk::Label(M("TP_LOCALLAB_SHOWDCT") + ":"));
...
void LocallabSoft::read(const rtengine::procparams::ProcParams* pp,
                        const ParamsEdited* pedited)
{
    // Disable all listeners
    disableListener();
...
void LocallabSoft::setDefaults(const rtengine::procparams::ProcParams* defParams,
                               const ParamsEdited* pedited)
{
    const int index = defParams->locallab.selspot;
...
void LocallabSoft::enabledChanged()
{
    if (isLocActivated) {
        if (listener) {
            if (exp->getEnabled()) {
                listener->panelChanged(EvLocenasoft, M("GENERAL_ENABLED") + " ("
                                                         + escapeHtmlChars(getSpotName())
                                                         + ")");
            } else {
                listener->panelChanged(EvLocenasoft, M("GENERAL_DISABLED") + " ("
                                                         + escapeHtmlChars(getSpotName())
                                                         + ")");
            }
        }
...
void LocallabSoft::updateGUIToMode(const modeType new_type)
{
    ...
    case Expert:
        ...
        if (softMethod->get_active_row_number() != 0)
        {  // Keep widget hidden when softMethod is equal to 0
...
LocallabBlur::LocallabBlur()
    : LocallabTool(this, M("TP_LOCALLAB_BLUR_TOOLNAME"), M("TP_LOCALLAB_BLUFR"), true),
      ...
      radius(Gtk::manage(new Adjuster(M("TP_LOCALLAB_RADIUS"),
                                      MINRAD,
                                      MAXRAD,
                                      0.1,
                                      1.5,
      ...
      recothres(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_MASKRECOTHRES"), 1., 2., 0.001, 1.))),
      lowthres(
      ...
      denocontrast(
          Gtk::manage(new Adjuster(M("TP_SHARPENING_CONTRAST"), 0.5, 100., 0.5, 10.))),
      ...
      LocalcurveEditorwavden(
          new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
                               M("TP_LOCALLAB_WAVDEN"))),
      wavshapeden(static_cast<FlatCurveEditor*>(
          LocalcurveEditorwavden->addCurve(CT_Flat, "", nullptr, false, false))),
      ...
      madls([]() -> std::array<Adjuster*, 21>

            {
                ...
                for (unsigned int i = 0; i < res.size(); ++i) {
                    ...
                    res[i] = Gtk::manage(
                        new Adjuster(std::move(ss), 1.0, 2000000.0, 1., 100.0));
                }
      ...
      lnoiselow(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_MASKLNOISELOW"), 0.7, 2., 0.01, 1.))),
      levelthr(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_MASKLCTHR2"), 20., 99., 0.5, 85.))),
      levelthrlow(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_MASKLCTHRLOW2"), 1., 80., 0.5, 12.))),
      noiselumf0(Gtk::manage(
          new Adjuster(M("TP_LOCALLAB_NOISELUMFINEZERO"), MINCHRO, MAXCHRO, 0.01, 0.))),
      noiselumf(Gtk::manage(
          new Adjuster(M("TP_LOCALLAB_NOISELUMFINE"), MINCHRO, MAXCHRO, 0.01, 0.))),
      noiselumf2(Gtk::manage(
          new Adjuster(M("TP_LOCALLAB_NOISELUMFINETWO"), MINCHRO, MAXCHRO, 0.01, 0.))),
      noiselumc(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_NOISELUMCOARSE"),
                                   MINCHRO,
                                   MAXCHROCC,
                                   0.01,
                                   0.))),  // unused here, but used for normalize_mean_dt
      noiselumdetail(Gtk::manage(
          new Adjuster(M("TP_LOCALLAB_NOISELUMDETAIL"), 0., 100., 0.01, 50.))),
      noiselequal(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_NOISELEQUAL"),
                                   -2,
                                   10,
                                   1,
                                   7,
      ...
      LocalcurveEditorwavhue(
          new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
                               M("TP_WAVELET_DENOISEHUE"))),
      wavhue(static_cast<FlatCurveEditor*>(
          LocalcurveEditorwavhue->addCurve(CT_Flat, "", nullptr, false, true))),
      LocalcurveEditorwavhuecont(
          new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
                               M("TP_LOCALLAB_DENOIHUECONT"))),
      wavhuecont(static_cast<FlatCurveEditor*>(
          LocalcurveEditorwavhuecont->addCurve(CT_Flat, "", nullptr, false, true))),
      noisechrof(Gtk::manage(
          new Adjuster(M("TP_LOCALLAB_NOISECHROFINE"), MINCHRO, MAXCHRO, 0.01, 0.))),
      noisechroc(Gtk::manage(
          new Adjuster(M("TP_LOCALLAB_NOISECHROCOARSE"), MINCHRO, MAXCHROCC, 0.01, 0.))),
      noisechrodetail(Gtk::manage(
          new Adjuster(M("TP_LOCALLAB_NOISECHRODETAIL"), 0., 100., 0.01, 50.))),
      ...
      adjblur(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_ADJ"),
                                   -100.,
                                   100.,
                                   1.,
                                   0.,
      ...
      lowthresd(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_MASKLCTHRLOW2"), 1., 80., 0.5, 12.))),
      midthresd(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_MASKLCTHRMID"), 0., 100., 0.5, 0.))),
      midthresdch(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_MASKLCTHRMIDCH"), 0., 100., 0.5, 0.))),
      higthresd(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_MASKLCTHR2"), 20., 99., 0.5, 85.))),
      ...
      //    maskblCurveEditorG(new
      //    CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
      //    M("TP_LOCALLAB_MASK"))),
      maskblCurveEditorG(
          new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir, "", 1)),
      CCmaskblshape(static_cast<FlatCurveEditor*>(
          maskblCurveEditorG->addCurve(CT_Flat, "C", nullptr, false, false))),
      LLmaskblshape(static_cast<FlatCurveEditor*>(
          maskblCurveEditorG->addCurve(CT_Flat, "L", nullptr, false, false))),
      HHmaskblshape(static_cast<FlatCurveEditor*>(
          maskblCurveEditorG->addCurve(CT_Flat, "LC(h)", nullptr, false, true))),
      strumaskbl(
          Gtk::manage(new Adjuster(M("TP_LOCALLAB_STRUMASKCOL"), 0., 200., 0.1, 0.))),
      ...
      mask2blCurveEditorG(
          new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
                               M("TP_LOCALLAB_MASK"))),
      Lmaskblshape(static_cast<DiagonalCurveEditor*>(
          mask2blCurveEditorG->addCurve(CT_Diagonal, "L(L)"))),
      mask2blCurveEditorGwav(
          new CurveEditorGroup(App::get().mut_options().lastlocalCurvesDir,
                               M("TP_LOCALLAB_WAVMASK"))),
      LLmaskblshapewav(static_cast<FlatCurveEditor*>(
          mask2blCurveEditorGwav->addCurve(CT_Flat, "L(L)", nullptr, false, false))),
      quaHBox(Gtk::manage(new Gtk::Box())),
      csThresholdblur(Gtk::manage(new ThresholdAdjuster(M("TP_LOCALLAB_CSTHRESHOLDBLUR"),
                                                        0,
                                                        9,
                                                        0,
                                                        0,
                                                        ...
                                                        false)))
{
    ...
    blMethodConn = blMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabBlur::blMethodChanged));

    fftwblConn = fftwbl->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabBlur::fftwblChanged));
    usemaskConn = usemask->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabBlur::usemaskChanged));
    invblConn = invbl->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabBlur::invblChanged));
    invmaskdConn = invmaskd->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabBlur::invmaskdChanged));
    invmaskConn = invmask->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabBlur::invmaskChanged));
    contrshowConn = contrshow->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabBlur::contrshowChanged));
    enacontrastConn = enacontrast->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabBlur::enacontrastChanged));
    lockmadlConn = lockmadl->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabBlur::lockmadlChanged));
    madllockConn = madllock->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabBlur::madllockChanged));
    ...
    medMethodConn = medMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabBlur::medMethodChanged));
    ...
    blurMethodConn = blurMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabBlur::blurMethodChanged));
    ...
    chroMethodConn = chroMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabBlur::chroMethodChanged));

    activlumConn = activlum->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabBlur::activlumChanged));
    ...
    quamethodconn = quamethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabBlur::quamethodChanged));
    ...
    setExpandAlignProperties(expdenoisenl, true, false, Gtk::ALIGN_FILL,
                             Gtk::ALIGN_START);
    setExpandAlignProperties(expdenoiselum, true, false, Gtk::ALIGN_FILL,
                             Gtk::ALIGN_START);
    setExpandAlignProperties(expdenoisech, true, false, Gtk::ALIGN_FILL,
                             Gtk::ALIGN_START);

    LocalcurveEditorwavden->setCurveListener(this);

    wavshapeden->setIdentityValue(0.);
    wavshapeden->setResetCurve(FlatCurveType(defSpot.locwavcurveden.at(0)),
                               defSpot.locwavcurveden);
    ...
    setExpandAlignProperties(lum46Labels, true, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_START);

    setExpandAlignProperties(chroLabels, true, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_START);
    setExpandAlignProperties(chro46Labels, true, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_START);
    ...
    wavhue->setResetCurve(FlatCurveType(defSpot.locwavcurvehue.at(0)),
                          defSpot.locwavcurvehue);
    ...
    //    wavguid->setResetCurve(FlatCurveType(defSpot.locwavcurveguid.at(0)),
    //    defSpot.locwavcurveguid);

    LocalcurveEditorwavhue->curveListComplete();

    LocalcurveEditorwavhuecont->setCurveListener(this);
    wavhuecont->setIdentityValue(0.);
    wavhuecont->setResetCurve(FlatCurveType(defSpot.locwavcurvehuecont.at(0)),
                              defSpot.locwavcurvehuecont);
    ...
    setExpandAlignProperties(resetImg, false, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_CENTER);
    neutral->set_image(*resetImg);
    neutral->set_tooltip_text(M("TP_RETINEX_NEUTRAL_TOOLTIP"));
    neutralconn = neutral->signal_pressed().connect(
        sigc::mem_fun(*this, &LocallabBlur::neutral_pressed));
    neutral->show();
    ...
    showmaskblMethodConn = showmaskblMethod->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabBlur::showmaskblMethodChanged));
    ...
    showmaskblMethodtypConn = showmaskblMethodtyp->signal_changed().connect(
        sigc::mem_fun(*this, &LocallabBlur::showmaskblMethodtypChanged));

    enablMaskConn = enablMask->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabBlur::enablMaskChanged));

    maskblCurveEditorG->setCurveListener(this);

    CCmaskblshape->setIdentityValue(0.);
    CCmaskblshape->setResetCurve(FlatCurveType(defSpot.CCmaskblcurve.at(0)),
                                 defSpot.CCmaskblcurve);
    CCmaskblshape->setBottomBarColorProvider(this, 1);

    LLmaskblshape->setIdentityValue(0.);
    LLmaskblshape->setResetCurve(FlatCurveType(defSpot.LLmaskblcurve.at(0)),
                                 defSpot.LLmaskblcurve);
    LLmaskblshape->setBottomBarBgGradient({ { 0., 0., 0., 0. }, { 1., 1., 1., 1. } });

    HHmaskblshape->setIdentityValue(0.);
    HHmaskblshape->setResetCurve(FlatCurveType(defSpot.HHmaskblcurve.at(0)),
                                 defSpot.HHmaskblcurve);
    ...
    toolblConn = toolbl->signal_toggled().connect(
        sigc::mem_fun(*this, &LocallabBlur::toolblChanged));
    ...
    Lmaskblshape->setResetCurve(DiagonalCurveType(defSpot.Lmaskblcurve.at(0)),
                                defSpot.Lmaskblcurve);
    ...
    LLmaskblshapewav->setResetCurve(FlatCurveType(defSpot.LLmaskblcurvewav.at(0)),
                                    defSpot.LLmaskblcurvewav);
    ...
    wchBox->pack_start(*LocalcurveEditorwavden, Gtk::PACK_SHRINK,
                       4);  // Padding is mandatory to correct behavior of curve editor
    ...
    toolblBox->pack_start(*mask2blCurveEditorG, Gtk::PACK_SHRINK,
                          4);  // Padding is mandatory to correct behavior of curve editor
    ToolParamBlock* const toolblBox2 = Gtk::manage(new ToolParamBlock());
    toolblBox2->pack_start(
        *mask2blCurveEditorGwav, Gtk::PACK_SHRINK,
        4);  // Padding is mandatory to correct behavior of curve editor
...
{
    idle_register.add([this, autodenoContrast]() -> bool {
        disableListener();
...
void LocallabBlur::updatemadlc(const double m0,
                               const double m1,
                               const double m2,
                               const double m3,
                               const double m4,
                               ...
                               const bool madloc)
{
    idle_register.add([this, m0, m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12, m13,
                       m14, m15, m16, m17, m18, m19, m20, madloc]() -> bool {
        GThreadLock lock;  // All GUI access from idle_add callbacks or separate thread
                           // HAVE to be protected
...
void LocallabBlur::updatedenlc(const double highres,
                               const double nres,
...
{
    idle_register.add([this, highres, nres, highres46, nres46, Lhighres, Lnres,
                       Lhighres46, Lnres46]() -> bool {
        GThreadLock lock;  // All GUI access from idle_add callbacks or separate thread
                           // HAVE to be protected
...
void LocallabBlur::read(const rtengine::procparams::ProcParams* pp,
                        const ParamsEdited* pedited)
{
    // Disable all listeners
    disableListener();
...
void LocallabBlur::setDefaults(const rtengine::procparams::ProcParams* defParams,
                               const ParamsEdited* pedited)
{
    const int index = defParams->locallab.selspot;
...
void LocallabBlur::adjusterChanged(Adjuster* a, double newval)
{
    if (isLocActivated && exp->getEnabled()) {
        ...
        if (a == madls[0] || a == madls[1] || a == madls[2] || a == madls[3]
            || a == madls[4] || a == madls[5] || a == madls[6])
        {
        ...
        if (a == madls[7] || a == madls[8] || a == madls[9] || a == madls[10]
            || a == madls[11] || a == madls[12] || a == madls[13])
        {
        ...
        if (a == madls[14] || a == madls[15] || a == madls[16] || a == madls[17]
            || a == madls[18] || a == madls[19] || a == madls[20])
        {
...
void LocallabBlur::adjusterChanged2(ThresholdAdjuster* a,
                                    int newBottomL,
                                    int newTopL,
                                    int newBottomR,
                                    int newTopR)
    ...
    if (isLocActivated && exp->getEnabled()) {
        if (listener) {
            listener->panelChanged(EvlocallabcsThresholdblur,
                                   csThresholdblur->getHistoryString() + " ("
                                       + escapeHtmlChars(getSpotName()) + ")");
        }
    }
}
...
void LocallabBlur::enabledChanged()
{
    if (isLocActivated) {
        if (listener) {
            if (exp->getEnabled()) {
                listener->panelChanged(EvLocenablur, M("GENERAL_ENABLED") + " ("
                                                         + escapeHtmlChars(getSpotName())
                                                         + ")");
            } else {
                listener->panelChanged(EvLocenablur, M("GENERAL_DISABLED") + " ("
                                                         + escapeHtmlChars(getSpotName())
                                                         + ")");
            }
        }
...
void LocallabBlur::updateGUIToMode(const modeType new_type)
{
    ...
    case Expert:
        ...
        // Show widgets hidden in Normal and Simple mode
        /*
                    if (blMethod->get_active_row_number() == 0) { // Keep widget hidden
           when blMethod is > 0 fftwbl->show(); expdenoise2->hide();
                    }
...
void LocallabBlur::updateMaskBackground(const double normChromar,
                                        const double normLumar,
                                        const double normHuer,
                                        const double normHuerjz)
{
...
void LocallabBlur::showmaskblMethodtypChanged()
{
    ...
    if (listener) {
        listener->panelChanged(EvlocallabshowmasktypMethod,
                               showmaskblMethodtyp->get_active_text() + " ("
                                   + escapeHtmlChars(getSpotName()) + ")");
    }
}
