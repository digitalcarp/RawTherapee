    ...
    : FoldableToolPanel(this, TOOL_NAME, M("TP_COLORTONING_LABEL"), false, true)
{
    ...
    colorCurveEditorG =
        new CurveEditorGroup(options.lastColorToningCurvesDir, M("TP_COLORTONING_COLOR"));
    ...
    opacityCurveEditorG = new CurveEditorGroup(options.lastColorToningCurvesDir,
                                               M("TP_COLORTONING_OPACITY"));
    ...
    opacityShape->setResetCurve(FlatCurveType(default_params.opacityCurve.at(0)),
                                default_params.opacityCurve);
    ...
    clCurveEditorG = new CurveEditorGroup(options.lastColorToningCurvesDir,
                                          M("TP_COLORTONING_CHROMAC"));
    ...
    clshape->setResetCurve(DiagonalCurveType(default_params.clcurve.at(0)),
                           default_params.clcurve);
    ...
    cl2CurveEditorG = new CurveEditorGroup(options.lastColorToningCurvesDir,
                                           M("TP_COLORTONING_CHROMAC"));
    ...
    cl2shape->setResetCurve(DiagonalCurveType(default_params.cl2curve.at(0)),
                            default_params.cl2curve);
    ...
    ("undo-small")); neutralCurves->set_image(*resetImgc); neutralCurves->set_tooltip_text
    (M("TP_COLORTONING_NEUTRALCUR_TIP")); neutralcurvesconn =
    neutralCurves->signal_pressed().connect( sigc::mem_fun(*this,
    ...
    */
    ...
    hlColSat = Gtk::manage(new ThresholdAdjuster(
        M("TP_COLORTONING_HIGHLIGHT"), 0., 100., 60., M("TP_COLORTONING_STRENGTH"), 1.,
        0., 360., 80., M("TP_COLORTONING_HUE"), 1., nullptr, false));
    ...
    shadowsColSat = Gtk::manage(new ThresholdAdjuster(
        M("TP_COLORTONING_SHADOWS"), 0., 100., 80., M("TP_COLORTONING_STRENGTH"), 1., 0.,
        360., 208., M("TP_COLORTONING_HUE"), 1., nullptr, false));
    ...
    p1VBox->pack_start(*saturatedOpacity, Gtk::PACK_SHRINK,
                       2);  // I have moved after Chanmixer
    p1Frame->add(*p1VBox);
    ...
    labRegionChromaticityMask = static_cast<FlatCurveEditor*>(labRegionEditorG->addCurve(
        CT_Flat, M("TP_COLORTONING_LABREGION_CHROMATICITYMASK"), nullptr, false, false));
...
void ColorToning::read(const ProcParams* pp, const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        shadowsColSat->setEditedState(pedited->colorToning.shadowsColSat ? Edited
                                                                         : UnEdited);
    ...
    labgrid->setParams(pp->colorToning.labgridALow / ColorToningParams::LABGRID_CORR_MAX,
...
void ColorToning::write(ProcParams* pp, ParamsEdited* pedited)
{
    ...
    labgrid->getParams(pp->colorToning.labgridALow, pp->colorToning.labgridBLow,
                       pp->colorToning.labgridAHigh, pp->colorToning.labgridBHigh, zerox,
                       zeroy, zerox, zeroy, zerox, zeroy);
...
void ColorToning::setDefaults(const ProcParams* defParams, const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        greenlow->setDefaultEditedState(pedited->colorToning.greenlow ? Edited
                                                                      : UnEdited);
        ...
        greenmed->setDefaultEditedState(pedited->colorToning.greenmed ? Edited
                                                                      : UnEdited);
        ...
        greenhigh->setDefaultEditedState(pedited->colorToning.greenhigh ? Edited
                                                                        : UnEdited);
        bluehigh->setDefaultEditedState(pedited->colorToning.bluehigh ? Edited
                                                                      : UnEdited);
        ...
        hlColSat->setDefaultEditedState(pedited->colorToning.hlColSat ? Edited
                                                                      : UnEdited);
        shadowsColSat->setDefaultEditedState(
            pedited->colorToning.shadowsColSat ? Edited : UnEdited);
        strength->setDefaultEditedState(pedited->colorToning.strength ? Edited
                                                                      : UnEdited);
        labgrid->setEdited(
            (pedited->colorToning.labgridALow || pedited->colorToning.labgridBLow
             || pedited->colorToning.labgridAHigh || pedited->colorToning.labgridBHigh)
...
void ColorToning::setAdjusterBehavior(bool splitAdd,
...
void ColorToning::adjusterChanged(ThresholdAdjuster* a,
...
void ColorToning::adjusterChanged2(ThresholdAdjuster* a,
                                   int newBottomL,
                                   int newTopL,
                                   int newBottomR,
                                   int newTopR)
    ...
    if (!batchMode) {
        ...
        if (method->get_active_row_number() == 0) {  // Lab
            ...
            lumamode->show();
                   ...
                   || method->get_active_row_number() == 6)
        {  // Lab Grid or Lab Regions
            colorSep->hide();
            colorCurveEditorG->hide();
            twocolor->hide();
            opacityCurveEditorG->hide();
...
void ColorToning::colorForValue(double valX,
                                double valY,
    ...
    for (size_t i = 0; i < labRegionData.size(); ++i) {
        ...
        labRegionList->set_text(j, 1,
                                Glib::ustring::compose("a=%1 b=%2 S=%3\ns=%4 o=%5 p=%6",
                                                       round_ab(r.a), round_ab(r.b),
                                                       r.saturation, r.slope, r.offset,
                                                       r.power));
        const char* ch = "";
        switch (r.channel) {
        ...
        labRegionList->set_text(
            ...
            Glib::ustring::compose(
                "%1%2%3%4%5", hasMask(dflt.hueMask, r.hueMask) ? "H" : "",
...
float ColorToning::blendPipetteValues(CurveEditor* ce,
                                      float chan1,
                                      float chan2,
                                      float chan3)
{
