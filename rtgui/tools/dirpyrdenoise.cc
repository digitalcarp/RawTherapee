...
DirPyrDenoise::DirPyrDenoise()
    : FoldableToolPanel(this, TOOL_NAME, M("TP_DIRPYRDENOISE_LABEL"), true, true),
      lastmedian(false)
{
    ...
    Gtk::Label* labmL =
        Gtk::manage(new Gtk::Label(M("TP_DIRPYRDENOISE_LUMINANCE_CONTROL") + ":"));
    ...
    Lmethodconn = Lmethod->signal_changed().connect(
        sigc::mem_fun(*this, &DirPyrDenoise::LmethodChanged));

    luma = Gtk::manage(
        new Adjuster(M("TP_DIRPYRDENOISE_LUMINANCE_SMOOTHING"), 0, 100, 0.01, 0));
    Ldetail = Gtk::manage(
        new Adjuster(M("TP_DIRPYRDENOISE_LUMINANCE_DETAIL"), 0, 100, 0.01, 0));
    auto& options = App::get().mut_options();
    NoiscurveEditorG = new CurveEditorGroup(options.lastDenoiseCurvesDir,
                                            M("TP_DIRPYRDENOISE_LUMINANCE_CURVE"));
    ...
    lshape = static_cast<FlatCurveEditor*>(
        NoiscurveEditorG->addCurve(CT_Flat, "", nullptr, false, false));
    ...
    Gtk::Label* labmC =
        Gtk::manage(new Gtk::Label(M("TP_DIRPYRDENOISE_CHROMINANCE_METHOD") + ":"));
    ...
    Cmethodconn = Cmethod->signal_changed().connect(
        sigc::mem_fun(*this, &DirPyrDenoise::CmethodChanged));
    ...
    C2methodconn = C2method->signal_changed().connect(
        sigc::mem_fun(*this, &DirPyrDenoise::C2methodChanged));

    NoiseLabels = Gtk::manage(new Gtk::Label("---", Gtk::ALIGN_CENTER));
    NoiseLabels->set_tooltip_text(
        M("TP_DIRPYRDENOISE_CHROMINANCE_PREVIEWRESIDUAL_INFO_TOOLTIP"));

    TileLabels = Gtk::manage(new Gtk::Label("---", Gtk::ALIGN_CENTER));
    PrevLabels = Gtk::manage(new Gtk::Label("---", Gtk::ALIGN_CENTER));

    chroma = Gtk::manage(
        new Adjuster(M("TP_DIRPYRDENOISE_CHROMINANCE_MASTER"), 0, 100, 0.01, 15));
    redchro = Gtk::manage(
        new Adjuster(M("TP_DIRPYRDENOISE_CHROMINANCE_REDGREEN"), -100, 100, 0.1, 0));
    bluechro = Gtk::manage(
        new Adjuster(M("TP_DIRPYRDENOISE_CHROMINANCE_BLUEYELLOW"), -100, 100, 0.1, 0));
    ...
    dmethodconn = dmethod->signal_changed().connect(
        sigc::mem_fun(*this, &DirPyrDenoise::dmethodChanged));
    ...
    CCcurveEditorG = new CurveEditorGroup(options.lastDenoiseCurvesDir,
                                          M("TP_DIRPYRDENOISE_CHROMINANCE_CURVE"));
    ...
    ccshape = static_cast<FlatCurveEditor*>(
        CCcurveEditorG->addCurve(CT_Flat, "", nullptr, false, false));
    ...
    median = Gtk::manage(
        new Gtk::CheckButton(M("TP_DIRPYRDENOISE_MEDIAN_METHOD_LABEL") + ":"));
    ...
    methodmedconn = methodmed->signal_changed().connect(
        sigc::mem_fun(*this, &DirPyrDenoise::methodmedChanged));
    ...
    rgbmethodconn = rgbmethod->signal_changed().connect(
        sigc::mem_fun(*this, &DirPyrDenoise::rgbmethodChanged));
    ...
    medmethodconn = medmethod->signal_changed().connect(
        sigc::mem_fun(*this, &DirPyrDenoise::medmethodChanged));

    ctboxm = Gtk::manage(new Gtk::Box());
    Gtk::Label* labmm =
        Gtk::manage(new Gtk::Label(M("TP_DIRPYRDENOISE_MEDIAN_METHOD") + ":"));
    ctboxm->pack_start(*labmm, Gtk::PACK_SHRINK, 1);

    ctbox = Gtk::manage(new Gtk::Box());
    Gtk::Label* labm =
        Gtk::manage(new Gtk::Label(M("TP_DIRPYRDENOISE_MEDIAN_TYPE") + ":"));
    ...
    hb11->pack_start(*Gtk::manage(new Gtk::Label(M("TP_DIRPYRDENOISE_MAIN_MODE") + ": ")),
                     Gtk::PACK_SHRINK, 1);
    ...
    smethodconn = smethod->signal_changed().connect(
        sigc::mem_fun(*this, &DirPyrDenoise::smethodChanged));

    autoGain =
        Gtk::manage(new CheckBox(M("TP_DIRPYRDENOISE_MAIN_AUTO_GAIN"), multiImage));
    ...
    gamma =
        Gtk::manage(new Adjuster(M("TP_DIRPYRDENOISE_MAIN_GAMMA"), 1.0, 3.0, 0.01, 1.7));
    ...
    passes =
        Gtk::manage(new Adjuster(M("TP_DIRPYRDENOISE_MEDIAN_PASSES"), 1.0, 3.0, 1., 1.));
    ...
    medianConn = median->signal_toggled().connect(
        sigc::mem_fun(*this, &DirPyrDenoise::medianChanged));
    ctboxrgb->hide();
}
...
void DirPyrDenoise::noiseTilePrev(int tileX,
                                  int tileY,
                                  int prevX,
                                  int prevY,
                                  int sizeT,
...
void DirPyrDenoise::updateTileLabel()
{
    if (!batchMode) {
        ...
        nX = nexttileX;
        nY = nexttileY;
        TileLabels->set_text(Glib::ustring::compose(
            M("TP_DIRPYRDENOISE_CHROMINANCE_PREVIEW_TILEINFO"),
...
void DirPyrDenoise::updatePrevLabel()
{
    if (!batchMode) {
        ...
        pX = nextprevX;
        pY = nextprevY;
        PrevLabels->set_text(Glib::ustring::compose(
            M("TP_DIRPYRDENOISE_CHROMINANCE_PREVIEW_INFO"),
    ...
    if (!batchMode) {
        ...
        if (nois == 0.f && high == 0.f) {
            NoiseLabels->set_text(
                M("TP_DIRPYRDENOISE_CHROMINANCE_PREVIEW_NOISEINFO_EMPTY"));
        } else {
            NoiseLabels->set_text(Glib::ustring::compose(
                M("TP_DIRPYRDENOISE_CHROMINANCE_PREVIEW_NOISEINFO"),
...
void DirPyrDenoise::setDefaults(const ProcParams* defParams, const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        Ldetail->setDefaultEditedState(pedited->dirpyrDenoise.Ldetail ? Edited
                                                                      : UnEdited);
        chroma->setDefaultEditedState(pedited->dirpyrDenoise.chroma ? Edited : UnEdited);
        redchro->setDefaultEditedState(pedited->dirpyrDenoise.redchro ? Edited
                                                                      : UnEdited);
        bluechro->setDefaultEditedState(pedited->dirpyrDenoise.bluechro ? Edited
                                                                        : UnEdited);
...
void DirPyrDenoise::setAdjusterBehavior(bool lumaadd,
                                        bool lumdetadd,
                                        bool chromaadd,
                                        bool chromaredadd,
                                        bool chromablueadd,
...
void DirPyrDenoise::colorForValue(double valX,
                                  double valY,
