...
DarkFrame::DarkFrame()
    : FoldableToolPanel(this, TOOL_NAME, M("TP_DARKFRAME_LABEL")),
      dfChanged(false),
      lastDFauto(false),
      dfp(nullptr),
      israw(true)
{
    ...
    hbdf->set_spacing(4);
    darkFrameFile = Gtk::manage(
        new MyFileChooserButton(M("TP_DARKFRAME_LABEL"), Gtk::FILE_CHOOSER_ACTION_OPEN));
    ...
    btnReset->signal_clicked().connect(sigc::mem_fun(*this, &DarkFrame::darkFrameReset),
                                       true);
    ...
    if (pp->raw.df_autoselect && dfp && !multiImage) {
        ...
        if (img) {
            dfInfo->set_text(Glib::ustring::compose(
                "%1: %2ISO %3s", Glib::path_get_basename(img->get_filename()),
                img->get_ISOspeed(), img->get_shutter()));
        } else {
...
void DarkFrame::dfAutoChanged()
{
    ...
    if (listener) {
        listener->panelChanged(EvPreProcessAutoDF, dfAuto->get_active()
                                                       ? M("GENERAL_ENABLED")
                                                       : M("GENERAL_DISABLED"));
    }
}
...
void DarkFrame::darkFrameChanged()
{
    ...
    if (listener) {
        listener->panelChanged(EvPreProcessDFFile,
                               Glib::path_get_basename(darkFrameFile->get_filename()));
    }
}
