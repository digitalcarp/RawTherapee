...
Distortion::Distortion() : FoldableToolPanel(this, TOOL_NAME, M("TP_DISTORTION_LABEL"))
{
    ...
    focal_length =
        Gtk::manage(new Adjuster(M("TP_DISTORTION_FOCAL_LENGTH"), 0.5, 25, 0.01,
...
void Distortion::read(const ProcParams* pp, const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        focal_length->setEditedState(pedited->distortion.focal_length
                                             != DistortionParams::DEFAULT_FOCAL_LENGTH
                                         ? Edited
                                         : UnEdited);
    }
...
{
    if (listener) {
        listener->panelChanged(EvDistortionDefish, defish->get_active()
                                                       ? M("GENERAL_ENABLED")
                                                       : M("GENERAL_DISABLED"));
        focal_length->setEnabled(defish->get_active());
    }
