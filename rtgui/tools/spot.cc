      ...
      editedCheckBox(nullptr)
{
    ...
    spotSize = Gtk::manage(new Adjuster(M("TP_SPOT_DEFAULT_SIZE"), SpotParams::minRadius,
                                        SpotParams::maxRadius, 1, 25));
...
Spot::~Spot()
{
    ...
    if (EditSubscriber::visibleGeometry.size()) {
        for (size_t i = 0;
             i < EditSubscriber::visibleGeometry.size() - VISIBLE_OBJECT_COUNT; ++i)
        {  // static visible geometry at the end of the list
...
void Spot::setBatchMode(bool batchMode)
{
    ...
    if (batchMode) {
        ...
        if (!editedCheckBox) {
            ...
            countLabel = nullptr;
            editedCheckBox = Gtk::manage(
                new Gtk::CheckButton(Glib::ustring::compose(M("TP_SPOT_COUNTLABEL"), 0)));
...
void Spot::enabledChanged()
{
    if (listener) {
        if (get_inconsistent()) {
            listener->panelChanged(edit->get_active() ? EvSpotEnabledOPA : EvSpotEnabled,
                                   M("GENERAL_UNCHANGED"));
        } else if (getEnabled()) {
            listener->panelChanged(edit->get_active() ? EvSpotEnabledOPA : EvSpotEnabled,
                                   M("GENERAL_ENABLED"));
...
void Spot::createGeometry()
{
    ...
    for (; j < EditSubscriber::visibleGeometry.size() - VISIBLE_OBJECT_COUNT; ++i, ++j) {
        EditSubscriber::mouseOverGeometry.at(i) = EditSubscriber::visibleGeometry.at(j) =
            new OPIcon(normalImg, activeImg, prelightImg, nullptr, nullptr,
                       Geometry::DP_CENTERCENTER);
...
void Spot::updateGeometry()
{
    ...
    if (dataProvider) {
        ...
        if (activeSpot > -1) {
            ...
            targetFeatherCircle.radius =
                float(spots.at(activeSpot).radius) * (1.f + spots.at(activeSpot).feather);
            targetFeatherCircle.radiusInImageSpace = true;
...
CursorShape Spot::getCursor(int objectID, int xPos, int yPos) const
{
    ...
    if (editProvider && activeSpot > -1) {
        ...
        if (objectID >= MO_TARGET_CIRCLE && objectID <= MO_SOURCE_FEATHER_CIRCLE) {
            Coord delta(
                Coord(xPos, yPos)
                - ((objectID == MO_SOURCE_CIRCLE || objectID == MO_SOURCE_FEATHER_CIRCLE)
...
bool Spot::drag1(int modifierKey)
{
    ...
    if (loGeom == &sourceMODisc) {
        ...
        // printf("sourceCircle / deltaPrevImage = %d / %d\n", editProvider->deltaImage.x,
        // editProvider->deltaImage.y);
        ...
        spots.at(activeSpot).radius =
            LIM<int>(int(currPolar.radius), SpotParams::minRadius, SpotParams::maxRadius);
...
void Spot::tweakParams(procparams::ProcParams& pparams)
{
    ...
    // pparams.toneCurve.hrenabled = false;  // not sure for this one, it could be useful
    // for ExpComp w/o performance penalty
