      ...
      oldMethod(-1)
{
    ...
    imageNumberBox->pack_start(*Gtk::manage(new Gtk::Label(M("TP_RAW_IMAGENUM") + ": ")),
                               Gtk::PACK_SHRINK, 4);
    ...
    pixelShiftDemosaicMethod->connect(pixelShiftDemosaicMethod->signal_changed().connect(
                        ...
                        const ParamsEdited* pedited)
{
    ...
    for (size_t i = 0; i < procparams::RAWParams::BayerSensor::getMethodStrings().size();
         ++i)
    {
    ...
    for (size_t i = 0;
         i < procparams::RAWParams::BayerSensor::getPSDemosaicMethodStrings().size(); ++i)
    {
    ...
    if (pedited) {
        ...
        dcbIterations->setEditedState(pedited->raw.bayersensor.dcbIterations ? Edited
                                                                             : UnEdited);
    ...
    if (!batchMode) {
        ...
        lmmseOptions->set_visible(pp->raw.bayersensor.method
                                  == procparams::RAWParams::BayerSensor::getMethodString(
...
void BayerProcess::write(rtengine::procparams::ProcParams* pp, ParamsEdited* pedited)
{
    ...
    if (currentRow >= 0 && method->get_active_text() != M("GENERAL_UNCHANGED")) {
        pp->raw.bayersensor.method = procparams::RAWParams::BayerSensor::getMethodString(
...
void BayerProcess::setAdjusterBehavior(bool falsecoloradd,
                                       bool iteradd,
                               ...
                               const ParamsEdited* pedited)
{
    ...
    if (pedited) {
        ...
        pixelShiftSigma->setDefaultEditedState(
            pedited->raw.bayersensor.pixelShiftSigma ? Edited : UnEdited);
        border->setDefaultEditedState(pedited->raw.bayersensor.border ? Edited
                                                                      : UnEdited);
        ccSteps->setDefaultEditedState(pedited->raw.bayersensor.ccSteps ? Edited
                                                                        : UnEdited);
    } else {
...
void BayerProcess::checkBoxToggled(CheckBox* c, CheckValue newval)
{
    if (c == dcbEnhance) {
        ...
        if (listener) {
            listener->panelChanged(EvPixelshiftShowMotion,
                                   pixelShiftShowMotion->getValueAsStr());
        }
        ...
        if (listener) {
            listener->panelChanged(EvPixelShiftHoleFill,
                                   pixelShiftHoleFill->getValueAsStr());
        }
        ...
        if (listener) {
            listener->panelChanged(EvPixelshiftAverage,
                                   pixelShiftAverage->getValueAsStr());
        }
        ...
        if (listener) {
            listener->panelChanged(EvPixelShiftEqualBright,
                                   pixelShiftEqualBright->getValueAsStr());
        }
        ...
        if (listener) {
            listener->panelChanged(EvPixelShiftNonGreenCross,
                                   pixelShiftNonGreenCross->getValueAsStr());
        }
    }
}
...
void BayerProcess::pixelShiftMotionMethodChanged()
{
    ...
    if (listener && pixelShiftMotionMethod->get_active_row_number() >= 0) {
        listener->panelChanged(EvPixelShiftMotionMethod,
                               pixelShiftMotionMethod->get_active_text());
    }
}
