...
void ThumbBrowserEntryBase::updateBackBuffer()
{
    ...
    const hidpi::ScaledDeviceSize expectedDeviceSize =
        expected.scaleToDevice(activeDeviceScale);

    if (backBuffer
        && (backBuffer->getWidth() != expectedDeviceSize.width
            || backBuffer->getHeight() != expectedDeviceSize.height))
    {
    ...
    if (!backBuffer) {
        backBuffer = Glib::RefPtr<BackBuffer>(
            new BackBuffer(expectedDeviceSize.width, expectedDeviceSize.height));
    }
    ...
    if ((activeDeviceScale == pendingDeviceScale) && !preview.empty()) {
        ...
        const int hh = expected.height
                       - (upperMargin + bsHeight + borderWidth + infoh + lowerMargin);
        prevPos.y = upperMargin + bsHeight + borderWidth
                    + std::max((hh - previewSize.height) / 2, 0);
    ...
    {
        cc->begin_new_path();
        cc->rectangle(istartx, istarty, previewSize.width,
...
void ThumbBrowserEntryBase::resize(int h)
{
    ...
    if (previewSize.height < 24) {
        ...
        height = previewSize.height + (upperMargin + 2 * borderWidth + lowerMargin) + bsh
                 + infoh;
    }
    ...
    {  // if new thumbnail height or new orientation
        preview.clear();
                                      ...
                                      const Gdk::RGBA& fg)
{
    ...
    if (selected || framed) {
        ...
        cc->arc(expected.width - 1 - radius, expected.height - 1 - radius, radius, 0,
                rtengine::RT_PI / 2);
        cc->arc(radius, expected.height - 1 - radius, radius, rtengine::RT_PI / 2,
                rtengine::RT_PI);
    ...
    if (framed) {
        ...
        cc->arc(-2 + 0.5 + expected.width - 1 - radius, +2 + 0.5 + radius, radius,
                -rtengine::RT_PI / 2, 0);
        cc->arc(-2 + 0.5 + expected.width - 1 - radius,
                -2 + 0.5 + expected.height - 1 - radius, radius, 0, rtengine::RT_PI / 2);
        cc->arc(+2 + 0.5 + radius, -2 + expected.height - 1 - radius, radius,
...
void ThumbBrowserEntryBase::draw(Cairo::RefPtr<Cairo::Context> cc)
{
    ...
    //    ofsX+startx, expected.width, ofsY+starty +
    //    upperMargin+bsHeight+borderWidth+previewSize.height+borderWidth+textGap+tpos,
    //    fnlabh);
...
bool ThumbBrowserEntryBase::inside(int x, int y) const
{
    ...
    return x > ofsX + startx && x < ofsX + startx + expected.width && y > ofsY + starty
           && y < ofsY + starty + expected.height;
}
...
rtengine::Coord2D ThumbBrowserEntryBase::getPosInImgSpace(int x, int y) const
{
    ...
    if (!preview.empty()) {
        ...
        if (x >= prevPos.x && x <= prevPos.x + previewSize.width && y >= prevPos.y
            && y <= prevPos.y + previewSize.height)
        {
...
bool ThumbBrowserEntryBase::insideWindow(int x, int y, int w, int h) const
{
    ...
    return !(ofsX + startx > x + w || ofsX + startx + expected.width < x
             || ofsY + starty > y + h || ofsY + starty + expected.height < y);
}
...
std::tuple<Glib::ustring, bool> ThumbBrowserEntryBase::getToolTip(int x, int y) const
{
    ...
    // be truncated. If "Show Exif info" is disabled, also show Exif info in the tooltip.
