...
void ExtProgStore::init()
{
    ...
    searchProgram("Photomatix Pro", "PhotomatixPro%1\\PhotomatixPro.exe", "", 9, true,
                  true);
    searchProgram("Paint.NET", "Paint.NET\\PaintDotNet.exe", "", 0, false, true);
    searchProgram("MS Image Composition Editor",
                  "Microsoft Research\\Image Composite Editor\\ICE.exe", "", 0, false,
                  true);
    searchProgram("PTGui", "PTGui\\PTGui.exe", "", 0, false, true);
    searchProgram("GeoSetter", "GeoSetter\\GeoSetter.exe", "", 0, true, true);
    searchProgram("FastStone Image Viewer", "FastStone Image Viewer\\FSViewer.exe", "", 0,
                  true, true);
    searchProgram("FastPictureViewer", "FastPictureViewer\\FastPictureViewer.exe", "", 0,
                  true, true);
                       ...
                       "Kolor\\Autopano Giga 3.%1\\AutopanoGiga.exe", 15, true, true))
    {
        ...
        {
            if (!searchProgram(
                    "Autopano Giga 2", "Kolor\\Autopano Giga 2.%1\\AutopanoGiga_x64.exe",
...
bool ExtProgStore::openInGimp(const Glib::ustring& fileName)
{
    ...
    auto success = ShellExecute(NULL, "open", executable.c_str(), fileName.c_str(), NULL,
                                SW_SHOWNORMAL);
...
bool ExtProgStore::openInExternalEditor(const Glib::ustring& fileName,
                                        const EditorInfo& editorInfo)
{
    ...
    if (rtengine::settings->verbose) {
        std::cout << "Unable to launch external editor with Gio. Trying custom launcher."
                  << std::endl;
    }
