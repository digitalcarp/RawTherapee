...
void SaveAsDialog::okPressed()
{
    ...
    // Checking if the filename field is empty. The user have to click Cancel if he don't
    // want to specify a filename
    if (fname.empty()) {
        Gtk::MessageDialog(*this,
                           Glib::ustring("<b>") + M("MAIN_MSG_EMPTYFILENAME") + "</b>",
                   ...
                   && !rtengine::hasPngExtension(fname)))
    {
        ...
        Gtk::MessageDialog msgd(
            *this,
            Glib::ustring("<b>") + M("GENERAL_WARNING") + ": " + M("SAVEDLG_WARNFILENAME")
...
void SaveAsDialog::formatChanged(const Glib::ustring& format)
{
    const auto sanitize_suffix =
        [this, format](const std::function<bool(const Glib::ustring&)>& has_suffix) {
            ...
            if (!has_suffix(name)) {
                fchooser->set_current_name(removeExtension(Glib::path_get_basename(name))
                                           + '.' + format);
            }
        };
