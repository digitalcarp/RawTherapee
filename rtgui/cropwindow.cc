...
CropWindow::CropWindow (ImageArea* parent, bool isLowUpdatePriority_, bool isDetailWindow)
    : ObjectMOBuffer(parent), state(SNormal), press_x(0), press_y(0), action_x(0), action_y(0), pickedObject(-1), pickModifierKey(0), rot_deg(0), onResizeArea(false), deleted(false),
      ...
      upperBorderWidth(1), sepWidth(2), cropPos(30, 30), windowSize(0, 0), imgAreaPos(0, 0), imgAreaSize(0, 0),
...
void CropWindow::setPosition (int x, int y)
{
    ...
    if (decorated) {
        buttonSet.arrangeButtons (cropPos.x + sideBorderWidth, cropPos.y + upperBorderWidth, windowSize.width - 2 * sideBorderWidth, titleHeight);
    }
}
...
bool CropWindow::isInside (int x, int y)
{
    ...
    return x >= cropPos.x && x < cropPos.x + windowSize.width && y >= cropPos.y && y < cropPos.y + windowSize.height;
}
...
void CropWindow::pointerMoved (int bstate, int x, int y)
{
    ...
    if (pmlistener) {
        ...
        if (!onArea (CropImage, x, y) || !cropHandler.cropPixbuftrue) {
            ...
            int vx = x - cropPos.x - imgPos.x;
            int vy = y - cropPos.y - imgPos.y;
...
bool CropWindow::onArea (CursorArea a, int x, int y)
{
    ...
    case CropToolBar:
        return x > cropPos.x && y > cropPos.y && x < cropPos.x + windowSize.width - 1 && y < cropPos.y + imgAreaPos.y;

    case CropImage:
        return x >= cropPos.x + imgPos.x + imgAreaPos.x && y >= cropPos.y + imgPos.y + imgAreaPos.y && x < cropPos.x + imgPos.x + imgAreaPos.x + imgSize.width && y < cropPos.y + imgPos.y + imgAreaPos.y + imgSize.height;
    ...
    case CropBorder:
        return
            (x >= cropPos.x + imgAreaPos.x && y >= cropPos.y + imgAreaPos.y && x < cropPos.x + imgAreaPos.x + imgAreaSize.width && y < cropPos.y + imgAreaPos.y + imgAreaSize.height) &&
            !(x >= cropPos.x + imgPos.x && y >= cropPos.y + imgPos.y && x < cropPos.x + imgPos.x + imgSize.width && y < cropPos.y + imgPos.y + imgSize.height);
    ...
    case CropTopRight:
        ...
        return cropHandler.cropParams->enabled &&
               ...
               x1 <= cropHandler.cropParams->x + cropHandler.cropParams->w - 1 + CROPRESIZEBORDER &&
               x < cropPos.x + imgPos.x + imgSize.width;
    ...
    case CropBottomLeft:
        ...
        return cropHandler.cropParams->enabled &&
               ...
               y1 <= cropHandler.cropParams->y + cropHandler.cropParams->h - 1 + CROPRESIZEBORDER &&
               y < cropPos.y + imgPos.y + imgSize.height &&
               x1 >= cropHandler.cropParams->x - CROPRESIZEBORDER &&
               x1 <= cropHandler.cropParams->x + CROPRESIZEBORDER &&
               x >= cropPos.x + imgPos.x;
    ...
    case CropResize:
        return decorated && x >= cropPos.x + windowSize.width - 16 && y >= cropPos.y + windowSize.height - 16 && x < cropPos.x + windowSize.width && y < cropPos.y + windowSize.height;
...
void CropWindow::expose (Cairo::RefPtr<Cairo::Context> cr)
{
    ...
    if (backColor == 0) {
        hidpi::LogicalCoord offset = cropPos + imgAreaPos;
    ...
    if (state == SCropImgMove || state == SCropWinResize) {
        ...
        Glib::RefPtr<Gdk::Pixbuf> rough = iarea->getPreviewHandler()->getRoughImage(
            handlerPos, desiredSize, zoomSteps[cropZoom].zoom);
...
void CropWindow::zoomIn (bool toCursor, int cursorX, int cursorY)
{
    ...
    if (toCursor) {
        ...
        if (zoomSteps[cropZoom].zoom <= cropHandler.getFitZoom()) {
            ...
            screenCoordToImage(cropPos.x + imgPos.x + imgSize.width / 2, cropPos.y + imgPos.y + imgSize.height / 2, x, y);
...
void CropWindow::screenCoordToCropBuffer (int phyx, int phyy, int& cropx, int& cropy)
{
    ...
    rtengine::Crop* crop = static_cast<rtengine::Crop*>(cropHandler.getCrop());
    cropx = phyx - cropPos.x - imgPos.x - imgAreaPos.x;
    cropy = phyy - cropPos.y - imgPos.y - imgAreaPos.y;
...
{
    ImageCoord cropPos = cropHandler.getPosition();
    imgx = cropPos.x + (phyx - cropPos.x - imgPos.x - imgAreaPos.x) / zoomSteps[cropZoom].zoom;
    imgy = cropPos.y + (phyy - cropPos.y - imgPos.y - imgAreaPos.y) / zoomSteps[cropZoom].zoom;
}

void CropWindow::screenCoordToCropCanvas (int phyx, int phyy, int& prevx, int& prevy)
{

    prevx = phyx - cropPos.x - imgAreaPos.x;
    prevy = phyy - cropPos.y - imgAreaPos.y;
}

void CropWindow::imageCoordToScreen (int imgx, int imgy, int& phyx, int& phyy)
{
    ImageCoord cropPos = cropHandler.getPosition();
    phyx = (imgx - cropPos.x) * zoomSteps[cropZoom].zoom + cropPos.x + imgPos.x + imgAreaPos.x;
    phyy = (imgy - cropPos.y) * zoomSteps[cropZoom].zoom + cropPos.y + imgPos.y + imgAreaPos.y;
}
...
void CropWindow::imageCoordToCropBuffer (int imgx, int imgy, int& phyx, int& phyy)
{
    ImageCoord cropPos = cropHandler.getPosition();
    rtengine::Crop* crop = static_cast<rtengine::Crop*>(cropHandler.getCrop());
    phyx = (imgx - cropPos.x) * zoomSteps[cropZoom].zoom + /*cropPos.x + imgPos.x +*/ crop->getLeftBorder();
    phyy = (imgy - cropPos.y) * zoomSteps[cropZoom].zoom + /*cropPos.y + imgPos.y +*/ crop->getUpperBorder();
}
...
void CropWindow::drawScaledSpotRectangle (Cairo::RefPtr<Cairo::Context> cr, int rectSize)
{
    ...
    cr->set_line_width (1.0);
    cr->rectangle (cropPos.x + imgPos.x + imgAreaPos.x - 0.5, cropPos.y + imgPos.y + imgAreaPos.y - 0.5, imgAreaSize.width, imgAreaSize.height);
    cr->clip ();
