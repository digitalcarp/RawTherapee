...
//                 defprofname = Glib::path_get_basename (defprof.c_str());
...
bool hasUserOnlyPermission(const Glib::ustring &dirname)
{
    ...
    return (mode & 0777) == 0700 && owner == Glib::get_user_name().c_str();
#elif defined(_WIN32)
    const Glib::RefPtr<Gio::File> file = Gio::File::create_for_path(dirname);
...
void setUserOnlyPermission(const Glib::RefPtr<Gio::File> file, bool execute)
{
    ...
    try {
        file->set_attribute_uint32("unix::mode", mode, Gio::FileQueryInfoFlags::NONE);
    } catch (Gio::Error &) {
    }
#elif defined(_WIN32)
...
Glib::ustring getTmpDirectory()
{
    ...
    auto is_usable_dir = [](const Glib::ustring &dir_path) {
        return !Glib::file_test(dir_path, Glib::FileTest::EXISTS) || (Glib::file_test(dir_path, Glib::FileTest::IS_DIR) && hasUserOnlyPermission(dir_path));
    };
...
//     {
//         Gtk::Image *softProofImage = Gtk::manage (new RtImage ("gamut-softproof"));
//         softProof.add (*softProofImage);
//         softProof.set_has_frame(false);
//         softProof.set_tooltip_markup (M ("SOFTPROOF_TOOLTIP"));
//
//         softProof.set_active (false);
//         softProof.show ();
//
//         Gtk::Image *spGamutCheckImage = Gtk::manage (new RtImage ("gamut-warning"));
//         spGamutCheck.add (*spGamutCheckImage);
//         spGamutCheck.set_has_frame(false);
...
//     tpc->setProgressListener(this);
    ...
    ppframe->set_label_align(0.025);
    ppframe->set_name ("ProfilePanel");
    ppframe->set_child (*profilep);
...
//     pack2(leftsubpaned, *history, true, false);
    ...
    pack2(leftbox, *leftsubpaned, true, true);
    ...
    Gtk::Image* infoimg = Gtk::manage (new RtImage ("info"));
    info->set_child (*infoimg);
    info->set_has_frame(false);
    info->set_tooltip_markup (M ("MAIN_TOOLTIP_QINFO"));

    beforeAfter = Gtk::manage (new Gtk::ToggleButton ());
    Gtk::Image* beforeAfterIcon = Gtk::manage (new RtImage ("beforeafter"));
    beforeAfter->set_child (*beforeAfterIcon);
    beforeAfter->set_has_frame(false);
    beforeAfter->set_tooltip_markup (M ("MAIN_TOOLTIP_TOGGLE"));

    iBeforeLockON = new RtImage ("padlock-locked-small");
    iBeforeLockOFF = new RtImage ("padlock-unlocked-small");
    ...
    iHistoryShow = new RtImage ("panel-to-right");
    iHistoryHide = new RtImage ("panel-to-left");

    hidehp->set_has_frame(false);
    hidehp->set_active (options.showHistory);
    hidehp->set_tooltip_markup (M ("MAIN_TOOLTIP_HIDEHP"));

    if (options.showHistory) {
        hidehp->set_child (*iHistoryHide);
    } else {
        hidehp->set_child (*iHistoryShow);
    }
    ...
    if (!simpleEditor && filePanel) {
        ...
        iTopPanel_1_Show = new RtImage ("panel-to-bottom");
        iTopPanel_1_Hide = new RtImage ("panel-to-top");
        tbTopPanel_1->set_has_frame(false);
        tbTopPanel_1->set_active (true);
        tbTopPanel_1->set_tooltip_markup (M ("MAIN_TOOLTIP_SHOWHIDETP1"));
        tbTopPanel_1->set_child (*iTopPanel_1_Hide);
    }
    ...
    Gtk::Image* histProfImg = Gtk::manage (new RtImage ("gamut-hist"));
    toggleHistogramProfile->set_child (*histProfImg);
    toggleHistogramProfile->set_has_frame(false);
...
//     tpc->getToolBar()->setLockablePickerToolListener (iareapanel->imageArea);
    ...
    pack_start(toolBarPanel, *hidehp, Pack::SHRINK, 1);
    pack_start(toolBarPanel, *vseph, Pack::SHRINK, 2);
...
//     pack_start (toolBarPanel, *iareapanel->imageArea->indClippedPanel, Pack::SHRINK, 0);
    ...
    pack_start (toolBarPanel, *toggleHistogramProfile, Pack::SHRINK, 1);
    pack_start (toolBarPanel, *vsepcl, Pack::SHRINK, 2);
//     pack_start (toolBarPanel, *tpc->coarse, Pack::SHRINK, 2);

    if (tbTopPanel_1) {
        Gtk::Separator* vsep = Gtk::manage (new Gtk::Separator(Gtk::Orientation::VERTICAL));
        pack_start (toolBarPanel, *vsep, Pack::SHRINK, 2);
        pack_start (toolBarPanel, *tbTopPanel_1, Pack::SHRINK, 1);
    }
...
//     pack_start(afterBox, *iareapanel);
    ...
    pack_start(beforeAfterBox, *afterBox);

    MyScrolledToolbar *stb1 = Gtk::manage(new MyScrolledToolbar());
    stb1->set_name("EditorToolbarTop");
    stb1->set_child(*toolBarPanel);
    pack_start(editbox, *stb1, Pack::SHRINK, 2);
    pack_start(editbox, *beforeAfterBox);
...
//     pack_start(vsubboxright, *tpc->toolPanelNotebook);
    ...
    Gtk::Image *saveButtonImage =  Gtk::manage (new RtImage ("save"));
    saveimgas = Gtk::manage (new Gtk::Button ());
    saveimgas->set_has_frame(false);
    saveimgas->set_child (*saveButtonImage);
    ...
    Gtk::Image *queueButtonImage = Gtk::manage (new RtImage ("gears"));
    queueimg = Gtk::manage (new Gtk::Button ());
    queueimg->set_has_frame(false);
    queueimg->set_child (*queueButtonImage);
...
//     setExpandAlignProperties(send_to_external->buttonGroup, false, false, Gtk::Align::CENTER, Gtk::Align::FILL);
    ...
    iShowHideSidePanels = new RtImage ("crossed-arrows-out");
    iShowHideSidePanels_exit = new RtImage ("crossed-arrows-in");
    tbShowHideSidePanels->set_has_frame(false);
    tbShowHideSidePanels->set_active (false);
    tbShowHideSidePanels->set_tooltip_markup (M ("MAIN_BUTTON_SHOWHIDESIDEPANELS_TOOLTIP"));
    tbShowHideSidePanels->set_child (*iShowHideSidePanels);
    ...
    if (!simpleEditor && !options.tabbedUI) {
        // Navigation buttons
        Gtk::Image *navPrevImage = Gtk::manage (new RtImage ("arrow2-left"));
        navPrev = Gtk::manage (new Gtk::Button ());
        navPrev->set_child (*navPrevImage);
        navPrev->set_has_frame(false);
        ...
        Gtk::Image *navNextImage = Gtk::manage (new RtImage ("arrow2-right"));
        navNext = Gtk::manage (new Gtk::Button ());
        navNext->set_child (*navNextImage);
        navNext->set_has_frame(false);
        ...
        Gtk::Image *navSyncImage = Gtk::manage (new RtImage ("arrow-updown"));
        navSync = Gtk::manage (new Gtk::Button ());
        navSync->set_child (*navSyncImage);
        navSync->set_has_frame(false);
...
//     iops->attach_next_to (*iareapanel->imageArea->zoomPanel, Gtk::PositionType::RIGHT, 1, 1);
    ...
    stb2->set_child(*iops);
    pack_start(editbox, *stb2, Pack::SHRINK, 0);
    ...
    if (options.showHistory) {
        pack1(hpanedl, *leftbox, false, false);
    ...
    pack2(viewpaned, *editbox, true, true);

    pack2(hpanedl, *viewpaned, true, true);
...
//     hpanedr->signal_button_release_event().connect_notify ( sigc::mem_fun (*this, &EditorPanel::rightPaneButtonReleased) );
    ...
    /*
        // save as dialog
        if (Glib::file_test (options.lastSaveAsPath, Glib::FileTest::IS_DIR))
...
//     tpc->readOptions ();
    ...
    saveimgas->signal_clicked().connect ( sigc::mem_fun (*this, &EditorPanel::saveAsPressed) );
    queueimg->signal_clicked().connect ( sigc::mem_fun (*this, &EditorPanel::queueImgPressed) );
//     send_to_external->signal_changed().connect(sigc::mem_fun(*this, &EditorPanel::sendToExternalChanged));
//     send_to_external->signal_clicked().connect(sigc::mem_fun(*this, &EditorPanel::sendToExternalPressed));
    toggleHistogramProfile->signal_toggled().connect( sigc::mem_fun (*this, &EditorPanel::histogramProfile_toggled) );

    if (navPrev) {
        navPrev->signal_clicked().connect ( sigc::mem_fun (*this, &EditorPanel::openPreviousEditorImage) );
    }

    if (navNext) {
        navNext->signal_clicked().connect ( sigc::mem_fun (*this, &EditorPanel::openNextEditorImage) );
    }

    if (navSync) {
        navSync->signal_clicked().connect ( sigc::mem_fun (*this, &EditorPanel::syncFileBrowser) );
    }
...
void EditorPanel::open (Thumbnail* tmb, rtengine::InitialImage* isrc)
{
    ...
    lastSaveAsFileName = removeExtension (Glib::path_get_basename (fname.c_str()));
...
{
    if (openThm) {
        return Glib::path_get_basename (openThm->getFileName ().c_str());
    } else {
        return "";
    }
}
...
void EditorPanel::info_toggled ()
{
    ...
    if (idata && idata->hasExif()) {
        ...
        infoString = Glib::ustring::compose ("%1\n<span size=\"small\">%2</span><span>%3</span>",
                                              infoString,
                                              escapeHtmlChars (Glib::path_get_dirname (openThm->getFileName().c_str())) + G_DIR_SEPARATOR_S,
                                              escapeHtmlChars (Glib::path_get_basename (openThm->getFileName().c_str()))  );
...
void EditorPanel::tbRightPanel_1_toggled ()
{
    /*
        ...
        if (tbRightPanel_1->get_active()){
            pack2(hpanedr, *vboxright, false, true);
            tbRightPanel_1->set_child (*iRightPanel_1_Hide);
        }
        else {
...
void EditorPanel::tbTopPanel_1_toggled ()
{
    ...
    if (catalogPane) { // catalogPane does not exist in multitab mode
        ...
        if (tbTopPanel_1->get_active()) {
            ...
            catalogPane->hide();
            tbTopPanel_1->set_child (*iTopPanel_1_Show);
...
//     }
//
//     if (!cached_exported_filename.empty() && cached_exported_image == ipc->getInitialImage() && pparams == cached_exported_pparams && Glib::file_test(cached_exported_filename, Glib::FileTest::IS_REGULAR)) {
...
bool EditorPanel::idle_sendToGimp ( ProgressConnector<rtengine::IImagefloat*> *pc, Glib::ustring fname)
{
    ...
    if (img) {
        // get file name base
        Glib::ustring shortname = removeExtension(Glib::path_get_basename(fname.c_str()));
        Glib::ustring dirname;
        switch (options.editor_out_dir) {
        case Options::EDITOR_OUT_DIR_CURRENT:
            dirname = Glib::path_get_dirname(fname.c_str());
            break;
...
bool EditorPanel::idle_sentToGimp (ProgressConnector<int> *pc, rtengine::IImagefloat* img, Glib::ustring filename)
{
    ...
    if ((!img && Glib::file_test(filename, Glib::FileTest::IS_REGULAR)) || (img && !errore)) {
        saveimgas->set_sensitive (true);
//         send_to_external->set_sensitive(send_to_external->getEntryCount());
        parent->setProgressStr ("");
        parent->setProgress (0.);
        ...
        if (!success) {
            auto msgd = Gtk::make_managed<RtMessageDialog>(
                "", RtMessageDialog::Type::ERROR, RtMessageDialog::ButtonSet::OK);
            msgd->set_title(M("MAIN_BUTTON_SENDTOEDITOR"));
            msgd->setHeader(M("MAIN_MSG_CANNOTSTARTEDITOR"));
            msgd->setDetailedText(M("MAIN_MSG_CANNOTSTARTEDITOR_SECONDARY"));
            msgd->show(parent);
        }
    }
...
void EditorPanel::toggleSidePanels()
{
    ...
    if (!bAllSidePanelsVisible) {
        ...
        tbShowHideSidePanels->set_child (*iShowHideSidePanels_exit);
    }
}
