...
ProfilePanel::ProfilePanel()
    : storedPProfile(nullptr),
      modeOn("profile-filled"),
      modeOff("profile-partial"),
      profileFillImage(
          Gtk::manage(new RTImage(App::get().options().filledProfile ? modeOn : modeOff,
...
{
    return profiles->getCurrentLabel().collate_key()
           == Glib::ustring("(" + M("PROFILEPANEL_PCUSTOM") + ")").collate_key();
}

bool ProfilePanel::isLastSavedSelected()
{
    return profiles->getCurrentLabel().collate_key()
           == Glib::ustring("(" + M("PROFILEPANEL_PLASTSAVED") + ")").collate_key();
}
 ...
 * If you want want to update the ProfileStore list itself (rescan the dir tree), use its
 * "parseProfiles" method instead
 */
...
void ProfilePanel::save_clicked(GdkEventButton* event)
{
    ...
    Gtk::FileChooserDialog dialog(getToplevelWindow(this), M("PROFILEPANEL_SAVEDLGLABEL"),
                                  Gtk::FILE_CHOOSER_ACTION_SAVE);
    bindCurrentFolder(dialog, options.loadSaveProfilePath);
    dialog.set_current_name(lastFilename);

    // Add the user's default (or global if multiuser=false) profile path to the Shortcut
    // list
    try {
...
void ProfilePanel::copy_clicked(GdkEventButton* event)
{
    ...
    if (toSave) {
        if (event->state & Gdk::CONTROL_MASK) {
            ...
            PartialProfile ppTemp(true);
            partialProfileDlg->applyPaste(ppTemp.pparams, ppTemp.pedited, toSave->pparams,
                                          toSave->pedited);
...
void ProfilePanel::load_clicked(GdkEventButton* event)
{
    ...
    Gtk::FileChooserDialog dialog(getToplevelWindow(this), M("PROFILEPANEL_LOADDLGLABEL"),
    ...
    if (result == Gtk::RESPONSE_OK) {
        ...
        if (!err) {
            ...
            // params Missing tool params in pe shall be also set to true to avoid a "spot
            // merge" issue
            ...
            // partialPaste.pedited )
            {
                ...
                partialProfileDlg->applyPaste(
                    custom->pparams, !fillMode->get_active() ? custom->pedited : nullptr,
                    &pp, &pe);
            } else {
                // custom.pparams = loadedFile.pparams filtered by ( loadedFile.pedited )
...
void ProfilePanel::paste_clicked(GdkEventButton* event)
{
    ...
    if (clipboard.hasPEdited()) {
        ...
        // partialPaste.pedited )
        {
            ...
            partialProfileDlg->applyPaste(
                custom->pparams, !fillMode->get_active() ? custom->pedited : nullptr, &pp,
                &pe);
        } else {
            // custom.pparams = clipboard.pparams filtered by ( clipboard.pedited )
            pe.combine(*custom->pparams, pp, true);
 ...
 *
 * @param profileFullPath   full path of the profile; must start by the virtual root (${G}
 * or ${U}, and without suffix
