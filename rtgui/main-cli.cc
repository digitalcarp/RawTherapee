...
int main(int argc, char** argv)
{
    ...
    if (Glib::file_test(Glib::build_filename(options.rtdir, "cache"),
    ...
    // printing RT's version in all case, particularly useful for the 'verbose' mode, but
    // also for the batch processing
...
int processLineParams(int argc, char** argv)
{
    ...
    for (int iArg = 1; iArg < argc; iArg++) {
        ...
        if (currParam.at(0) == '-' && currParam.size() > 1) {
            ...
            case 'p':  // processing parameters for all inputs; all set procparams are
                       // required, so
                ...
                if (iArg + 1 < argc) {
                    ...
                    if (fname.at(0) == '-') {
                        std::cerr << "Error: filename missing next to the -p switch."
                                  << std::endl;
            ...
            case 'j':
                if (currParam.length() > 2 && currParam.at(2) == 's') {
                    if (currParam.length() == 3) {
                        std::cerr << "Error: the -js switch requires a mandatory value!"
                                  << std::endl;
                        deleteProcParams(processingParams);
                        return -3;
                    }
                    ...
                    if (subsampling < 1 || subsampling > 3) {
                        std::cerr
                            << "Error: the value accompanying the -js switch has to be in the [1-3] range!"
                            << std::endl;
                        deleteProcParams(processingParams);
                        return -3;
                    }
                    ...
                    if (currParam.size() < 3) {
                        ...
                        if (compression < 0 || compression > 100) {
                            std::cerr
                                << "Error: the value accompanying the -j switch has to be in the [0-100] range!"
                                << std::endl;
                            deleteProcParams(processingParams);
                            return -3;
                        }
            ...
            case 'b':
                ...
                if (currParam.length() >= 3 && currParam.at(2) == '8') {  // -b8
                    bits = 8;
                           ...
                           && currParam.at(2) == '1' && currParam.at(3) == '6')
                {  // -b16, -b16f
                    bits = 16;
                    if (currParam.length() == 5 && currParam.at(4) == 'f') {
                        isFloat = true;
                           ...
                           && currParam.at(2) == '3' && currParam.at(3) == '2')
                {  // -b32 == -b32f
                    bits = 32;
                    isFloat = true;
                }
                ...
                if (bits != 8 && bits != 16 && bits != 32) {
                    std::cerr
                        << "Error: specify output bit depth per channel as -b8 for 8-bit integer, -b16 for 16-bit integer, -b16f for 16-bit float or -b32 for 32-bit float."
                        << std::endl;
                    deleteProcParams(processingParams);
                    return -3;
                }
            ...
            case 'c':  // MUST be last option
                while (iArg + 1 < argc) {
                    ...
                    if (Glib::file_test(argument, Glib::FILE_TEST_IS_REGULAR)) {
                        ...
                        bool notRetained =
                            !allExtensions && !options.has_retained_extention(argument);

                        if (notAll || notRetained) {
                            if (notAll) {
                                std::cout
                                    << "\"" << argument
                                    << "\"  is not one of the parsed extensions. Image skipped."
                                    << std::endl;
                            } else if (notRetained) {
                                std::cout
                                    << "\"" << argument
                    ...
                    if (Glib::file_test(argument, Glib::FILE_TEST_IS_DIR)) {
                        ...
                        try {
                            ...
                            auto enumerator =
                                dir->enumerate_children("standard::name,standard::type");

                            while (auto file = enumerator->next_file()) {

                                const auto fileName =
                                    Glib::build_filename(argument, file->get_name());
                                bool isDir =
                                ...
                                if (sideProcParams && skipIfNoSidecar) {
                                                         ...
                                                         Glib::FILE_TEST_EXISTS))
                                    {
                                        std::cout
                                            << "\"" << fileName
                                            << "\"  has no side-car file. Image skipped."
                                            << std::endl;
                                        continue;
                                    }
                                }
            ...
            default:
            {
                ...
                std::cout << "  Documentation: http://rawpedia.rawtherapee.com/"
                          << std::endl;
                std::cout << "  Forum: https://discuss.pixls.us/c/software/rawtherapee"
                          << std::endl;
                std::cout
                ...
                std::cout << "  [Square brackets] mean the parameter is optional."
                          << std::endl;
                std::cout << "  The pipe symbol | indicates a choice of one or the other."
                          << std::endl;
                std::cout
                ...
                std::cout
                    ...
                    << " -c <dir>|<files>   Convert files in batch with default parameters."
                    << std::endl;
                std::cout
                    << "  " << Glib::path_get_basename(argv[0])
                    << " <other options> -c <dir>|<files>   Convert files in batch with your own settings."
                    << std::endl;
                std::cout << std::endl;
                std::cout << "Options:" << std::endl;
                std::cout
                    << "  " << Glib::path_get_basename(argv[0])
                    << "[-o <output>|-O <output>] [-q] [-a] [-s|-S] [-p <one.pp3> [-p <two.pp3> ...] ] [-d] [ -j[1-100] -js<1-3> | -t[z] -b<8|16|16f|32> | -n -b<8|16> ] [-Y] [-f] -c <input>"
                    << std::endl;
                std::cout << std::endl;
                std::cout
                ...
                std::cout
                    ...
                    << std::endl;
                std::cout << "  -O <file>|<dir>  Set output file or folder and copy "
                ...
                std::cout
                    ...
                    << std::endl;
                std::cout
                    << "                   e.g. for photo.raw there should be a photo.raw."
                ...
                std::cout
                    ...
                    << std::endl;
                std::cout << "                   16f = 16-bit float.   Applies to TIFF."
                          << std::endl;
                std::cout << "                   32  = 32-bit float.   Applies to TIFF."
                          << std::endl;
                std::cout << "  -t[z]            Specify output to be TIFF." << std::endl;
                std::cout
                    << "                   Uncompressed by default, or deflate compression with 'z'."
                    << std::endl;
                std::cout << "  -n               Specify output to be compressed PNG."
                          << std::endl;
                std::cout
                ...
                std::cout
                    << "Your " << pparamsExt
                    << " files can be incomplete, RawTherapee will build the final values as follows:"
                    << std::endl;
                std::cout
...
#if ECLIPSE_ARGS
            App::get().setArgv1(
                App::get().argv1().substr(1, App::get().argv1().length() - 2));
#endif
    ...
    for (size_t iFile = 0; iFile < inputFiles.size(); iFile++) {
        ...
        if (!overwriteFiles && Glib::file_test(outputFile, Glib::FILE_TEST_EXISTS)) {
            std::cerr
                << outputFile
                << " already exists: use -Y option to overwrite. This image has been skipped."
                << std::endl;
            continue;
        }
        ...
        do {
            if (sideProcParams && i == sideCarFilePos) {
                ...
                Glib::ustring sideProcessingParams =
                    inputFile + App::PARAM_FILE_EXTENSION;

                // the "load" method don't reset the procparams values anymore, so values
                // found in the procparam file override the one of currentParams
        ...
        if (errorCode) {
            ...
            if (copyParamsFile) {
                Glib::ustring outputProcessingParams =
                    outputFile + App::PARAM_FILE_EXTENSION;
