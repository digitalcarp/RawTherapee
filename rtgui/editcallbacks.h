...
public:
private:
    EditUniqueID ID;  /// this will be used in improcfun to locate the data that has to be
                      /// stored in the buffer; it must be unique in RT
...
protected:
    ...
    enum class Action {
        ...
        PICKING  /// set action to this value in the buttonPressed event whenever the user
                 /// is picking something through a single click. In this case, the pickX
                 /// events will be called INSTEAD of buttonXReleased !
    };
...
public:
    ...
    key if the subscriber is of type ET_PIPETTE Once the key is pressed, RT will enter in
    drag1 mode on subsequent mouse movements
    @param modifierKey Gtk's event modifier key (GDK_CONTROL_MASK | GDK_SHIFT_MASK | ...)
    @return true if the preview has to be redrawn, false otherwise */
 ...
 *
 * It is admitted that only one Subscriber can ask data at a time. If the Subscriber is of
 * type ET_PIPETTE, it will have to trigger the usual event so that the image will be
...
private:
    ...
    //    int object;            /// ET_OBJECTS mode: Object detected under the cursor, 0
    //    otherwise; ET_PIPETTE mode: 1 if above the image, 0 otherwise
...
public:
    ...
    rtengine::Coord deltaPrevImage;  /// Delta relative to the previous mouse location, in
                                     /// the full image space
