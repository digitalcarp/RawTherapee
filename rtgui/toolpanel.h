...
public:
    ...
    /// @brief Ask to refresh the preview not triggered by a parameter change (e.g. 'On
    /// Preview' editing).
    ...
    virtual void panelChanged(const rtengine::ProcEvent& event,
                              const Glib::ustring& descr) = 0;
    /// @brief Set the TweakOperator to the StagedImageProcessor, to let some tool enter
    /// into special modes
...
/// @brief This class control the space around the group of tools inside a tab, as well as
/// the space separating each tool. */
...
protected:
    ...
    bool multiImage;  // True if more than one image are being edited at the same time
                      // (also imply that batchMode=true), false otherwise
    bool need100Percent;
...
public:
    ...
    virtual ToolParamBlock* getSubToolsContainer() const { return nullptr; }
     ...
     *
     * @return Return the previous state of the broadcast (true: enabled ; false:
     * disabled)
     */
...
public:
    ...
    MyExpander* getExpander() final { return exp; }

    ToolParamBlock* getSubToolsContainer() const final { return subToolsContainer; }
    ...
    void setParent(Gtk::Box* parent) final { parentContainer = parent; }
    Gtk::Box* getParent() final { return parentContainer; }
    ...
    void setGrayedOut(bool doGrayOut);  // Set whether the tool should be disabled,
                                        // collapsed and grayed-out.
