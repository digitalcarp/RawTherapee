...
CoarsePanel::CoarsePanel () : ToolPanel (), oldhflip(false), oldvflip(false)
{
    ...
    Gtk::Image* rotateli = Gtk::manage (new RtImage ("rotate-left-90"));
    rotate_left = Gtk::manage (new Gtk::Button ());
    rotate_left->set_child (*rotateli);
    rotate_left->set_has_frame(false);
    pack_start (this, *rotate_left);

    Gtk::Image* rotateri = Gtk::manage (new RtImage ("rotate-right-90"));
    rotate_right = Gtk::manage (new Gtk::Button ());
    rotate_right->set_child (*rotateri);
    rotate_right->set_has_frame(false);
    pack_start (this, *rotate_right);

    Gtk::Image* fliphi = Gtk::manage (new RtImage ("flip-horizontal"));
    hflip = Gtk::manage (new Gtk::ToggleButton ());
    hflip->set_child (*fliphi);
    hflip->set_has_frame(false);
    pack_start (this, *hflip);

    Gtk::Image* flipvi = Gtk::manage (new RtImage ("flip-vertical"));
    vflip = Gtk::manage (new Gtk::ToggleButton ());
    vflip->set_child (*flipvi);
    vflip->set_has_frame(false);
    pack_start (this, *vflip);
    ...
    rotate_left->signal_clicked().connect( sigc::mem_fun(*this, &CoarsePanel::rotateLeft) );
    rotate_right->signal_clicked().connect( sigc::mem_fun(*this, &CoarsePanel::rotateRight) );
