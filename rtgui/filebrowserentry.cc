...
std::shared_ptr<RTSurface>
    FileBrowserEntry::editedIcon(std::shared_ptr<RTSurface>(nullptr));
std::shared_ptr<RTSurface>
    FileBrowserEntry::recentlySavedIcon(std::shared_ptr<RTSurface>(nullptr));
std::shared_ptr<RTSurface>
    FileBrowserEntry::enqueuedIcon(std::shared_ptr<RTSurface>(nullptr));
...
void FileBrowserEntry::refreshQuickThumbnailImage()
{
    ...
    bool upgrade_to_processed =
        (!App::get().options().internalThumbIfUntouched || thumbnail->isPParamsValid());
...
void FileBrowserEntry::calcThumbnailSize()
{
    if (thumbnail) {
        ...
        if (ow != previewSize.width || oh != previewSize.height
            || preview.size() != expected_size)
        {
            preview.clear();
...
{
    // somewhere in pipeline customBackBufferUpdate is called when scale == 1.0, which is
    // nonsense for a thumb
    ...
    auto drawScaled = [&](const rtengine::procparams::CropParams& crop) {
        ...
        drawCrop(c, prevPos.x, prevPos.y, previewSize.width, previewSize.height,
                 previewSize.width, previewSize.height, 0, 0, zoom, crop, true, false);
    };
        ...
        || state == SCropMove)
    {
        ...
        if (cparams.enabled && !thumbnail->isQuick())
        {  // Quick thumb have arbitrary sizes, so don't apply the crop
            drawScaled(cparams);
        }
    }
}
...
void FileBrowserEntry::procParamsChanged(Thumbnail* thm,
                                         int whoChangedIt,
                                         bool upgradeHint)
{
    ...
    *this->cropParams = update.crop;
    ...
    if (update.size == previewSize && update.device_scale == pendingDeviceScale) {
        ...
        if (update.img->getData()) {
            std::copy(update.img->getData(), update.img->getData() + preview.size(),
                      preview.begin());
        } else {
...
bool FileBrowserEntry::motionNotify(int x, int y)
{
    ...
    if (inspector && inspector->isActive()
        && (!parent->isInTabMode() || App::get().options().inspectorWindow))
    {
            ...
            if (onArea(CropTopLeft, ix, iy)) {
                ...
                if (tm == TMStraighten) {
                    ...
                    iatlistener->spotWBselected(
                        ...
                        (iy - prevPos.y) / scale * activeDeviceScale, thumbnail);
                    b = true;
                    ...
                    if (cropgl) {
                        ...
                        press_x = cropParams->x =
                            (ix - prevPos.x) / scale * activeDeviceScale;
                        press_y = cropParams->y =
                            (iy - prevPos.y) / scale * activeDeviceScale;
                        cropParams->w = cropParams->h = 1;
                        cropgl->cropInit(cropParams->x, cropParams->y, cropParams->w,
                                         cropParams->h);
                        b = true;
                    }
                }
...
void FileBrowserEntry::drawStraightenGuide(Cairo::RefPtr<Cairo::Context> cr)
{
...

    {
        ...
        if (x2 < prevPos.x + ofsX + startx) {
            ...
            y2 = y1
                 - (double)(y1 - y2)
                       * (x1 - (previewSize.width + prevPos.x + ofsX + startx - 1))
                       / (x1 - x2);
        ...
        if (y2 < prevPos.y + ofsY + starty) {
            ...
            x2 = x1
                 - (double)(x1 - x2)
                       * (y1 - (previewSize.height + prevPos.y + ofsY + starty - 1))
                       / (y1 - y2);
