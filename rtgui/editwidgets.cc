...
void Geometry::setMOChannelColor(Cairo::RefPtr<Cairo::Context>& cr,
...
void Circle::drawOuterGeometry(Cairo::RefPtr<Cairo::Context>& cr,
                               ObjectMOBuffer* objectBuffer,
                               EditCoordSystem& coordSystem)
{
    double lineWidth = getOuterLineWidth();
    if ((flags & F_VISIBLE) && state != INSENSITIVE && lineWidth > 0.
        && innerLineWidth > 0.)
    {
        RGBColor color;
        ...
        cr->set_source_rgba(color.getR(), color.getG(), color.getB(),
                            OUTERGEOM_OPACITY * rtengine::min(innerLineWidth / 2.f, 1.f));
...
void Circle::drawInnerGeometry(Cairo::RefPtr<Cairo::Context>& cr,
                               ObjectMOBuffer* objectBuffer,
                               EditCoordSystem& coordSystem)
{
    if (flags & F_VISIBLE) {
...
void Circle::drawToMOChannel(Cairo::RefPtr<Cairo::Context>& cr,
                             unsigned short id,
                             ObjectMOBuffer* objectBuffer,
                             EditCoordSystem& coordSystem)
{
...
void Line::drawOuterGeometry(Cairo::RefPtr<Cairo::Context>& cr,
                             ObjectMOBuffer* objectBuffer,
                             EditCoordSystem& coordSystem)
{
    double lineWidth = getOuterLineWidth();
...
void Line::drawInnerGeometry(Cairo::RefPtr<Cairo::Context>& cr,
                             ObjectMOBuffer* objectBuffer,
                             EditCoordSystem& coordSystem)
{
...
void Polyline::drawOuterGeometry(Cairo::RefPtr<Cairo::Context>& cr,
                                 ObjectMOBuffer* objectBuffer,
                                 EditCoordSystem& coordSystem)
{
    double lineWidth = getOuterLineWidth();
        ...
        && lineWidth > 0.)
    {
        ...
        for (unsigned int i = 0; i < points.size(); ++i) {
            ...
            if (datum == IMAGE) {
                coordSystem.imageCoordToScreen(points.at(i).x, points.at(i).y, currPos.x,
                                               currPos.y);
            } else if (datum == CLICKED_POINT) {
...
void Polyline::drawInnerGeometry(Cairo::RefPtr<Cairo::Context>& cr,
                                 ObjectMOBuffer* objectBuffer,
                                 EditCoordSystem& coordSystem)
{
    if ((flags & F_VISIBLE) && points.size() > 1) {
        ...
        if (filled && state != INSENSITIVE) {
            ...
            for (unsigned int i = 0; i < points.size(); ++i) {
                ...
                if (datum == IMAGE) {
                    coordSystem.imageCoordToScreen(points.at(i).x, points.at(i).y,
                                                   currPos.x, currPos.y);
                } else if (datum == CLICKED_POINT) {
                    currPos += objectBuffer->getDataProvider()->posScreen;
                } else if (datum == CURSOR) {
...
void Polyline::drawToMOChannel(Cairo::RefPtr<Cairo::Context>& cr,
                               unsigned short id,
                               ObjectMOBuffer* objectBuffer,
                               EditCoordSystem& coordSystem)
{
    if ((flags & F_HOVERABLE) && points.size() > 1) {
        ...
        for (unsigned int i = 0; i < points.size(); ++i) {
            ...
            if (datum == IMAGE) {
                coordSystem.imageCoordToCropCanvas(points.at(i).x, points.at(i).y,
                                                   currPos.x, currPos.y);
            } else if (datum == CLICKED_POINT) {
                currPos += objectBuffer->getDataProvider()->posScreen;
            } else if (datum == CURSOR) {
...
void EditRectangle::drawOuterGeometry(Cairo::RefPtr<Cairo::Context>& cr,
                                      ObjectMOBuffer* objectBuffer,
                                      EditCoordSystem& coordSystem)
{
    double lineWidth = getOuterLineWidth();
...
void EditRectangle::drawInnerGeometry(Cairo::RefPtr<Cairo::Context>& cr,
                                      ObjectMOBuffer* objectBuffer,
                                      EditCoordSystem& coordSystem)
{
    if (flags & F_VISIBLE) {
...
void EditRectangle::drawToMOChannel(Cairo::RefPtr<Cairo::Context>& cr,
                                    unsigned short id,
                                    ObjectMOBuffer* objectBuffer,
                                    EditCoordSystem& coordSystem)
{
...
void Ellipse::drawOuterGeometry(Cairo::RefPtr<Cairo::Context>& cr,
                                ObjectMOBuffer* objectBuffer,
                                EditCoordSystem& coordSystem)
{
    if ((flags & F_VISIBLE) && state != INSENSITIVE) {
        ...
        rtengine::Coord center_ = center;
        double radYT_ = radiusInImageSpace ? coordSystem.scaleValueToCanvas(double(radYT))
                                           : double(radYT);
        double radY_ = radiusInImageSpace ? coordSystem.scaleValueToCanvas(double(radY))
                                          : double(radY);
        double radXL_ = radiusInImageSpace ? coordSystem.scaleValueToCanvas(double(radXL))
                                           : double(radXL);
        double radX_ = radiusInImageSpace ? coordSystem.scaleValueToCanvas(double(radX))
                                          : double(radX);
        ...
        if (radYT_ > 0 && radY_ > 0 && radXL_ > 0 && radX_ > 0) {
            // To have an ellipse with radius of (radX, radX), a circle of radius 1. shall
            // be twisted with a scale of radX for x-axis, radY for y-axis Center of
            // coordinates (x, y) in previous coordinates system becomes (X, Y) = (radX *
            // x, radY * y) in new one To go back to previous location, center shall be
            // translated to tx = -X * (1 - 1 / radX) in x-axis (x = tx + X) and ty = -Y *
            // (1 - 1 / radY) in y-axis (y = ty + Y)
            cr->save();
            ...
            cr->arc(center_.x, center_.y, 1.0, 3. * rtengine::RT_PI_2,
                    2. * rtengine::RT_PI);
...
void Ellipse::drawInnerGeometry(Cairo::RefPtr<Cairo::Context>& cr,
                                ObjectMOBuffer* objectBuffer,
                                EditCoordSystem& coordSystem)
{
    if (flags & F_VISIBLE) {
        ...
        if (filled && state != INSENSITIVE) {
            if (radYT_ > 0 && radY_ > 0 && radXL_ > 0 && radX_ > 0) {
                // To have an ellipse with radius of (radX, radX), a circle of radius 1.
                // shall be twisted with a scale of radX for x-axis, radY for y-axis
                // Center of coordinates (x, y) in previous coordinates system becomes (X,
                // Y) = (radX * x, radY * y) in new one To go back to previous location,
                // center shall be translated to tx = -X * (1 - 1 / radX) in x-axis (x =
                // tx + X) and ty = -Y * (1 - 1 / radY) in y-axis (y = ty + Y)
                cr->save();
                ...
                // Drawing bottom-left part
                cr->scale(radXL_, radY_);
                cr->translate(-center_.x * (1 - 1 / radXL_),
                              -center_.y * (1 - 1 / radY_));
                cr->arc(center_.x, center_.y, 1.0, rtengine::RT_PI_2, rtengine::RT_PI);
                cr->scale(radXL_, radY_);
                ...
                cr->scale(radXL_, radYT_);
                cr->translate(-center_.x * (1 - 1 / radXL_),
                              -center_.y * (1 - 1 / radYT_));
                cr->arc(center_.x, center_.y, 1.0, rtengine::RT_PI,
                        3. * rtengine::RT_PI_2);
                ...
                // Drawing top-right part
                cr->scale(radX_, radYT_);
                cr->translate(-center_.x * (1 - 1 / radX_),
                              -center_.y * (1 - 1 / radYT_));
                cr->arc(center_.x, center_.y, 1.0, 3. * rtengine::RT_PI_2,
                        2. * rtengine::RT_PI);
...
void Ellipse::drawToMOChannel(Cairo::RefPtr<Cairo::Context>& cr,
                              unsigned short id,
                              ObjectMOBuffer* objectBuffer,
                              EditCoordSystem& coordSystem)
{
...
OPIcon::OPIcon(Glib::ustring normalImage,
    ...
    : drivenPoint(drivenPoint)
{
    if (!normalImage.empty()) {
        normalImg =
            std::shared_ptr<RTSurface>(new RTSurface(normalImage, Gtk::ICON_SIZE_MENU));
    }

    if (!prelightImage.empty()) {
        prelightImg =
            std::shared_ptr<RTSurface>(new RTSurface(prelightImage, Gtk::ICON_SIZE_MENU));
    }

    if (!activeImage.empty()) {
        activeImg =
            std::shared_ptr<RTSurface>(new RTSurface(activeImage, Gtk::ICON_SIZE_MENU));
    }

    if (!draggedImage.empty()) {
        draggedImg =
            std::shared_ptr<RTSurface>(new RTSurface(draggedImage, Gtk::ICON_SIZE_MENU));
    }

    if (!insensitiveImage.empty()) {
        insensitiveImg = std::shared_ptr<RTSurface>(
            new RTSurface(insensitiveImage, Gtk::ICON_SIZE_MENU));
    }
}
...
void OPIcon::drawMOImage(std::shared_ptr<RTSurface>& img,
                         Cairo::RefPtr<Cairo::Context>& cr,
                         unsigned short id,
                         ObjectMOBuffer* objectBuffer,
                         EditCoordSystem& coordSystem)
...
void OPIcon::drawToMOChannel(Cairo::RefPtr<Cairo::Context>& cr,
                             unsigned short id,
                             ObjectMOBuffer* objectBuffer,
                             EditCoordSystem& coordSystem)
{
