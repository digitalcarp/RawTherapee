...
void MyReaderLock::acquire()
{
    ...
    if (mutex.writerCount == 0) {
        // There's no writer operating, we can increment the writer count which will lock
        // writers.
        ++mutex.writerCount;
...
void MyWriterLock::release()
{
    ...
    // Decrement the writer number first...
    if (--mutex.writerCount == 0) {
        // ...and if the writer count is zero again, we wake up all of the waiting writer
        // or reader.
        mutex.cond.notify_all();
    }
