 ...
 * process it so it can display it. Its position on the preview is handled not set by this
 * class but by the CropHandlerListener (i.e. CropWindow) with which it works closely.
 */
...
public:
    ...
    void colorPick(const rtengine::Coord& pickerPos,
                   float& r,
                   float& g,
                   float& b,
                   float& rpreview,
    ...
    void setDetailedCrop(rtengine::IImage8* im,
    ...
    Glib::RefPtr<Gdk::Pixbuf>
        cropPixbuf;  // image displayed on monitor, using the monitor profile (i.e. lab to
                     // monitor profile)
    Glib::RefPtr<Gdk::Pixbuf>
        cropPixbuftrue;  // internal image in output color space for analysis (i.e. lab to
                         // either Working profile or Output profile, depending on
...
private:
    ...
    // size of the crop's canvas on the screen ; might be bigger than the displayed image,
    // but not smaller
