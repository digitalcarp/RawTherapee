...
void History::historySelectionChanged()
{
    ...
    if (iter) {
        ...
        if (row && tpc) {
            ...
            tpc->profileChange(&pp, EvHistoryBrowsed, row[historyColumns.text],
                               &paramsEdited);
        }
...
void History::bookmarkSelectionChanged()
{
    ...
    if (iter) {
        ...
        if (row && tpc) {
            ...
            tpc->profileChange(&pp, EvBookmarkSelected, row[bookmarkColumns.text],
                               &paramsEdited);
        }
    }
}

void History::procParamsChanged(const ProcParams* params,
                                const ProcEvent& ev,
                                const Glib::ustring& descr,
                                const ParamsEdited* paramsEdited)
{
    // to prevent recursion, we filter out the events triggered by the history and events
    ...
    if (size == 0 || !row || row[historyColumns.chev] != ev || ev == EvProfileChanged
        || ev == EvLocallabSpotCreated || ev == EvLocallabSpotDeleted)
    {  // Special cases: If Locallab spot is created , deleted or duplicated several times
       // in a row, a new history row is used
...
bool History::on_query_tooltip(int x,
                               int y,
