 ...
 *         - Non-absolute size (i.e. "pt"): The default resolution is 72 DPI (i.e. pt per
 * inch). To convert the size to "px", use the following formula: "size in px" = "size in
 * pt" * ("device resolution" / 72) Notes:
 ...
 *     - Windows: A default DPI of 96 is considered. Current DPI parameter is provided by
 * the OS. Scale is calculated by (int)("current DPI" / 96). If Scale is greater than 1,
 * DPI is forced to 96.
 *     - MacOS: Scale is calculated from OS parameters (= "Retina screen width" / "Virtual
 * width"). DPI is forced to 72.
 *     - Linux: DPI is calculated from OS parameter (= 96 * "text-scaling-factor"). Note:
 * "text-scaling-factor" is different from "device factor".
 */
class RTScalable
{
private:
...
protected:
    ...
    loadSurfaceFromIcon(const Glib::ustring& icon_name,
                        const Gtk::IconSize iconSize = Gtk::ICON_SIZE_SMALL_TOOLBAR);
    static Cairo::RefPtr<Cairo::ImageSurface>
    loadSurfaceFromPNG(const Glib::ustring& fname, const bool is_path = false);
    static Cairo::RefPtr<Cairo::ImageSurface>
    loadSurfaceFromSVG(const Glib::ustring& fname,
                       const int width = -1,
                       const int height = -1,
