...
void CursorManager::init(Glib::RefPtr<Gdk::Window> mainWindow)
{
    ...
    auto createCursor = [this](const Glib::ustring& name,
        ...
        auto cursor = Gdk::Cursor::create(
            this->display, cursor_surf.get(),
...
void CursorManager::setCursor(Glib::RefPtr<Gdk::Window> window, CursorShape shape)
{
    ...
    case CursorShape::CSArrow:
        window
            ->set_cursor();  // set_cursor without any arguments to select system default
        break;
...
void CursorManager::setCursorOfMainWindow(Glib::RefPtr<Gdk::Window> window,
                                          CursorShape shape)
{
    if (window->get_display() == mainWindowCursorManager.display) {
