...
void CacheManager::init()
{
    ...
    for (const auto& cacheDir : cacheDirs) {
        error |= g_mkdir_with_parents(Glib::build_filename(baseDir, cacheDir).c_str(),
                                      cacheDirMode);
    }
                               ...
                               const std::string& newfilename)
{
    ...
    auto error = g_rename(
        getCacheFileName("profiles", oldfilename, App::PARAM_FILE_EXTENSION, oldmd5)
            .c_str(),
        getCacheFileName("profiles", newfilename, App::PARAM_FILE_EXTENSION, newmd5)
            .c_str());
...
void CacheManager::deleteFiles(const Glib::ustring& fname,
                               ...
                               bool purgeProfile) const
{
    ...
    if (purgeProfile) {
        error |= g_remove(
            getCacheFileName("profiles", fname, App::PARAM_FILE_EXTENSION, md5).c_str());
    }
...
std::string CacheManager::getMD5(const Glib::ustring& fname)
{
    ...
    if (GetFileAttributesExW(wfname.get(), GetFileExInfoStandard, &fileAttr)) {
        ...
        const auto identifier = Glib::ustring::compose(
            "%1-%2-%3-%4", fileAttr.nFileSizeLow, fileAttr.ftCreationTime.dwHighDateTime,
