...
bool CPBDump(const Glib::ustring& commFName,
             ...
             bool flagMode)
{
    ...
    const std::unique_ptr<FILE, int (*)(FILE*)> f(g_fopen(commFName.c_str(), "wt"),
                                                  &std::fclose);
 ...
 * metadata. May be null.
 */
void getRankAndColorFromMetadata(const CacheImageData& cfs,
                                 ...
                                 bool* hasColorPtr)
{
    ...
    const std::unique_ptr<const rtengine::FramesMetaData> md(
 ...
 */
bool getRankFromMetadata(const CacheImageData& cfs, const Glib::ustring& fname, int& rank)
{
    bool has_rank = false;
    int color;
 ...
 */
bool getRankFromXmpOrMetadata(const Options& options,
 ...
 * @returns If the color is in the XMP.
 */
bool getColorFromXmpOrNone(const Options& options,
                           const Exiv2::XmpData& xmp,
                           const Glib::ustring& fname,
                           int& color)
{
      ...
      initial_(false)
{
    ...
    // should be safe to use the unprotected version of loadThumbnail, since we are in the
    // constructor
    _loadThumbnail();
    generateExifDateTimeStrings();

    if (cfs.rankOld >= 0) {
        // rank and inTrash were found in cache (old style), move them over to pparams or
        // xmp sidecar

        // try to load the last saved parameters from the cache or from the paramfile file
        createProcParamsForUpdate(
            false,
            false);  // this can execute customprofilebuilder to generate param file
...
Thumbnail::Thumbnail(CacheManager* cm,
                     const Glib::ustring& fname,
...
void Thumbnail::_generateThumbnailImage()
{
    ...
    if (!tpp) {
        ...
        if (initial_ && options.internalThumbIfUntouched) {
            quick = true;
            tpp =
                rtengine::Thumbnail::loadQuickFromRaw(fname, sensorType, tw, th, 1, TRUE);
        }

        if (!tpp) {
            quick = false;
            tpp = rtengine::Thumbnail::loadFromRaw(
                fname, sensorType, tw, th, 1, pparams->wb.equal, pparams->wb.observer,
                TRUE, &(pparams->raw));
        }
        ...
        if (tpp) {
            cfs.format = FT_Raw;
            cfs.thumbImgType =
                quick ? CacheImageData::QUICK_THUMBNAIL : CacheImageData::FULL_THUMBNAIL;
            infoFromImage(fname);
...
const ProcParams& Thumbnail::getProcParamsU()
{
    if (pparamsValid) {
        ...
        *pparams =
            *(ProfileStore::getInstance()->getDefaultProcParams(getType() == FT_Raw));
        ...
        if (pparams->wb.method == "Camera") {
            ...
            double ct;
            getAutoWB(ct, pparams->wb.green, pparams->wb.equal, pparams->wb.observer,
                      pparams->wb.tempBias);
            pparams->wb.temperature = ct;
        }
    }
 ...
 *
 *  The loaded profile may be partial, but it return a complete ProcParams (i.e. without
 * ParamsEdited)
 *
 ...
 *  @param flaggingMode True if the ProcParams will be created because the file browser is
 * being flagging an image (rank, to trash, color labels). This parameter is passed to the
 * CPB.
 *
 *  @return Return a pointer to a ProcPamas structure to be updated if returnParams is
 * true and if everything went fine, NULL otherwise.
 */
rtengine::procparams::ProcParams*
Thumbnail::createProcParamsForUpdate(bool returnParams, bool force, bool flaggingMode)
{
    // try to load the last saved parameters from the cache or from the paramfile file
    ...
    const bool run_cpb = !options.CPBPath.empty() && !defaultPparamsPath.empty() && cfs
                         && cfs->exifValid && create;
    ...
    if (!run_cpb) {
        if (defProf == DEFPROFILE_DYNAMIC && create && cfs && cfs->exifValid) {
            const auto pp_deleter = [](PartialProfile* pp) {
                pp->deleteInstance();
                delete pp;
            };
            const std::unique_ptr<const rtengine::FramesMetaData> imageMetaData(
                rtengine::FramesMetaData::fromFile(fname));
            const std::unique_ptr<PartialProfile, decltype(pp_deleter)> pp(
                imageMetaData ? ProfileStore::getInstance()->loadDynamicProfile(
                                    imageMetaData.get(), fname)
                              : nullptr,
                pp_deleter);
        ...
        Glib::ustring tmpFileName(Glib::build_filename(
            options.cacheBaseDir, Glib::ustring::compose("CPB_temp_%1.txt", index++)));

        CPBDump(tmpFileName, fname, outFName,
                defaultPparamsPath == DEFPROFILE_INTERNAL
                    ? DEFPROFILE_INTERNAL
                    : Glib::build_filename(defaultPparamsPath,
                                           Glib::path_get_basename(defProf)
                                               + App::PARAM_FILE_EXTENSION),
                cfs, flaggingMode);

        // For the filename etc. do NOT use streams, since they are not UTF8 safe
        Glib::ustring cmdLine =
            options.CPBPath + Glib::ustring(" \"") + tmpFileName + Glib::ustring("\"");
        ...
        // Now they SHOULD be there (and potentially "partial"), so try to load them and
        // store it as a full procparam
        if (success) {
            loadProcParams();
        }
...
void Thumbnail::loadProcParams()
{
    ...
    if (App::get().options().paramsLoadLocation == PLL_Input) {
        ...
        if (!pparamsValid) {
            pparamsValid =
                !pparams->load(getCacheFileName("profiles", App::PARAM_FILE_EXTENSION));
        }
    } else {
...
void Thumbnail::clearProcParams(int whoClearedIt)
{
...

    {
        ...
        if (pparamsValid) {
            ...
            if (cfs.format == FT_Raw && App::get().options().internalThumbIfUntouched
                && cfs.thumbImgType != CacheImageData::QUICK_THUMBNAIL)
            {
                // regenerate thumbnail, ie load the quick thumb again. For the rare
                // formats not supporting quick thumbs this will be a bit slow as a new
                // full thumbnail will be generated unnecessarily, but currently there is
                // no way to pre-check if the format supports quick thumbs.
                initial_ = true;
                _generateThumbnailImage();
                initial_ = false;
            }
...
void Thumbnail::setProcParams(const ProcParams& pp,
                              ParamsEdited* pe,
                              int whoChangedIt,
                              bool updateCacheNow,
                              bool resetToDefault)
{
    ...
    const bool needsReprocessing =
        resetToDefault || blackLevelChanged || pparams->raw.expos != pp.raw.expos
        || pparams->toneCurve != pp.toneCurve || pparams->locallab != pp.locallab
        || pparams->labCurve != pp.labCurve || pparams->localContrast != pp.localContrast
        || pparams->rgbCurves != pp.rgbCurves || pparams->colorToning != pp.colorToning
        || pparams->vibrance != pp.vibrance || pparams->wb != pp.wb
        || pparams->colorappearance != pp.colorappearance || pparams->epd != pp.epd
        || pparams->fattal != pp.fattal || pparams->sh != pp.sh
        || pparams->toneEqualizer != pp.toneEqualizer || pparams->crop != pp.crop
        || pparams->coarse != pp.coarse || pparams->commonTrans != pp.commonTrans
        || pparams->rotate != pp.rotate || pparams->distortion != pp.distortion
        || pparams->lensProf != pp.lensProf || pparams->perspective != pp.perspective
        || pparams->gradient != pp.gradient || pparams->pcvignette != pp.pcvignette
        || pparams->cacorrection != pp.cacorrection
        || pparams->vignetting != pp.vignetting || pparams->chmixer != pp.chmixer
        || pparams->blackwhite != pp.blackwhite || pparams->icm != pp.icm
        || pparams->hsvequalizer != pp.hsvequalizer
        || pparams->filmSimulation != pp.filmSimulation
        || pparams->softlight != pp.softlight || pparams->dehaze != pp.dehaze
        || pparams->filmNegative != pp.filmNegative || whoChangedIt == FILEBROWSER
...
void Thumbnail::getThumbnailSize(int& w,
                                 int& h,
                             ...
                             int& h)
{
    ...
    // WARNING: When downscaled, the ratio have loosed a lot of precision, so we can't get
    // back the exact initial dimensions
                             ...
                             double& scale)
{
    ...
    if (cfs.thumbImgType == CacheImageData::QUICK_THUMBNAIL) {
        ...
        // image = tpp->processImage (pparams, h, rtengine::TI_Bilinear, cfs.getCamera(),
        // cfs.focalLen, cfs.focalLen35mm, cfs.focusDist, cfs.shutter, cfs.fnumber,
        // cfs.iso, cfs.expcomp, scale );
        image =
            tpp->processImage(pparams, static_cast<rtengine::eSensorType>(cfs.sensortype),
                              h, rtengine::TI_Bilinear, &cfs, scale);
    }
                             ...
                             bool forceUpgrade)
{
    ...
    // rtengine::IImage8* image = tpp->processImage (pparams, h, rtengine::TI_Bilinear,
    // cfs.getCamera(), cfs.focalLen, cfs.focalLen35mm, cfs.focusDist, cfs.shutter,
    // cfs.fnumber, cfs.iso, cfs.expcomp,  scale );
    rtengine::IImage8* image =
        tpp->processImage(pparams, static_cast<rtengine::eSensorType>(cfs.sensortype), h,
...
void Thumbnail::generateExifDateTimeStrings()
{
    ...
    if (cfs.timeValid) {
        ...
        dateTime = Glib::DateTime::create_local(cfs.year, cfs.month, cfs.day, cfs.hour,
                                                cfs.min, cfs.sec);
    }
    ...
    exifString = Glib::ustring::compose(
        ...
        Glib::ustring(rtengine::FramesData::shutterToString(cfs.shutter)), M("QINFO_ISO"),
        cfs.iso,
        Glib::ustring::format(std::setw(3), std::fixed, std::setprecision(2),
                              cfs.focalLen));

    if (options.fbShowExpComp && cfs.expcomp != "0.00" && !cfs.expcomp.empty())
    {  // don't show exposure compensation if it is 0.00EV;old cache files do not have
       // ExpComp, so value will not be displayed.
        exifString = Glib::ustring::compose(
            "%1 %2EV", exifString,
            cfs.expcomp);  // append exposure compensation to exifString
    }
}
...
{
    std::unique_ptr<rtengine::FramesMetaData> idata(
        rtengine::FramesMetaData::fromFile(fname));
...
/*
 * Read all thumbnail's data from the cache; build and save them if doesn't exist - NON
 ...
 *  - updatePParams==true (default)        : write the procparams file (sidecar or cache,
 * depending on the options)
 *  - updateCacheImageData==true (default) : write the CacheImageData values in the cache
 * folder, i.e. some General, DateTime, ExifInfo, File info and ExtraRawInfo,
 */
void Thumbnail::updateCache(bool updatePParams, bool updateCacheImageData)
{
    ...
    if (updatePParams && pparamsValid) {
        const auto& options = App::get().options();
        pparams->save(options.saveParamsFile ? fname + App::PARAM_FILE_EXTENSION : "",
...
bool Thumbnail::openDefaultViewer(int destination)
{
    ...
    if (destination == 1) {
        openFName =
            Glib::ustring::compose("%1.%2", BatchQueue::calcAutoFileNameBase(fname),
                                   options.saveFormatBatch.format);

        if (Glib::file_test(openFName, Glib::FILE_TEST_EXISTS)) {
            wchar_t* wfilename =
                (wchar_t*)g_utf8_to_utf16(openFName.c_str(), -1, NULL, NULL, NULL);
        ...
        if (Glib::file_test(openFName, Glib::FILE_TEST_EXISTS)) {
            ...
            wchar_t* org = (wchar_t*)g_utf8_to_utf16(
                Glib::ustring::compose("/select,\"%1\"", openFName).c_str(), -1, NULL,
                NULL, NULL);
            ...
            delete[] par;
            g_free(org);
        } else if (Glib::file_test(Glib::path_get_dirname(openFName),
                                   Glib::FILE_TEST_EXISTS))
        {
            // Out file does not exist, but directory
            wchar_t* wfilename = (wchar_t*)g_utf8_to_utf16(
                Glib::path_get_dirname(openFName).c_str(), -1, NULL, NULL, NULL);
...
void Thumbnail::getCamWB(double& temp,
                         double& green,
                         rtengine::StandardObserver observer) const
{
    if (tpp) {
...
void Thumbnail::loadProperties()
{
    ...
    getRankAndColorFromMetadata(cfs, fname, properties.rank.value, properties.color.value,
                                nullptr, nullptr);
...
void Thumbnail::updateProcParamsProperties(bool forceUpdate)
{
    ...
    if ((properties.trashed.edited || forceUpdate)
        && properties.trashed != pparams->inTrash)
    {
        pparams->inTrash = properties.trashed;
        pparamsValid = true;
    }

    const rtengine::MemoizingSupplier<Exiv2::XmpData> getXmpSidecar(
        [this]() { return rtengine::Exiv2Metadata::getXmpSidecar(fname); });

    const auto& options = App::get().options();

    // save procparams rank and color also when options.thumbnailRankColorMode ==
    // Options::ThumbnailPropertyMode::XMP so they'll be kept in sync Rank can be -1 to
    // prioritize the rank in the metadata. If the metadata rank doesn't exist, it is
    // interpreted as 0.
    if ((properties.rank.edited || forceUpdate)
        && rtengine::LIM(properties.rank.value, 0, 5)
               != rtengine::LIM(pparams->rank, 0, 5))
    {
        ...
        if (!forceUpdate) {
            ...
            int initial_rank = 0;
            bool has_initial_rank = getRankFromXmpOrMetadata(options, getXmpSidecar(),
                                                             cfs, fname, initial_rank);
            pparamsValid |= !(has_initial_rank && properties.rank == initial_rank);
        }
    }

    if ((properties.color.edited || forceUpdate)
        && properties.color != pparams->colorlabel)
    {
        ...
        if (!forceUpdate) {
            ...
            int initial_color = 0;
            bool has_initial_color =
                getColorFromXmpOrNone(options, getXmpSidecar(), fname, initial_color);
...
void Thumbnail::saveXMPSidecarProperties()
{
    ...
    if (App::get().options().thumbnailRankColorMode
    ...
    try {
        ...
        if (properties.color.edited) {
            xmp["Xmp.xmp.Label"] =
                rtengine::FramesData::xmp_color2label(properties.color);
        }
