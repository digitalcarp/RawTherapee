...
{
    static const std::pair<Glib::ustring, Glib::ustring>
    key(const Glib::ustring& major, const Glib::ustring& minor = Glib::ustring())
    {
...
std::string TranslationMetadata::get(const std::string& key,
...
void MultiLangMgr::load(const Glib::ustring& language,
...
const TranslationMetadata* MultiLangMgr::getMetadata(const Glib::ustring& fname) const
{
    ...
    const auto read_key_value = [&raw_metadata](const std::string& meta_line) {
        ...
        std::string key =
            meta_line.substr(definition_start + 1, separator_pos - definition_start - 1);
...
Glib::ustring MultiLangMgr::getOSUserLanguage()
{
    ...
    if (countryCodeStr != NULL) {
        ...
        CFStringGetCString(countryCodeStr, countryCodeBuffer, countryCodeStrLength,
                           strEncoding);
