...
void ThresholdSelector::updateDrawingArea(const ::Cairo::RefPtr<Cairo::Context>& cr)
{
    ...
    if (bgCurveProvider) {
        ...
        std::vector<double> pts =
            bgCurveProvider->getCurvePoints(this);  // the values sent by the provider are
        ...
        if (!separatedSliders) {
            ...
            {  // we use minValTop since if this block is executed, it means that we are
               // in a simple Threshold where both bottom and top range are the same
...
void ThresholdSelector::findBoundaries(double& min, double& max)
{
    ...
    case (TS_BOTTOMRIGHT):
        if (initalEq1) {
            ...
            max = secondaryMovedCursor == TS_UNDEFINED
                      ? positions[TS_TOPRIGHT]
                      : maxValTop - (positions[TS_TOPRIGHT] - positions[TS_BOTTOMRIGHT]);
        } else {
            min = secondaryMovedCursor == TS_UNDEFINED
                      ? positions[TS_TOPRIGHT]
                      : positions[TS_TOPLEFT]
    ...
    case (TS_TOPRIGHT):
        if (initalEq1) {
            ...
            max = secondaryMovedCursor == TS_UNDEFINED
                      ? positions[TS_BOTTOMRIGHT]
                      : maxValTop - (positions[TS_BOTTOMRIGHT] - positions[TS_TOPRIGHT]);
        }
