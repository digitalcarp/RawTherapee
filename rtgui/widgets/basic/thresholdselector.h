...
public:
    ...
    /*
     * The curve provider has to send back a list of point (at least 2 points) in the [0.0
     * ; 1.0] range for both X and Y axis; X and Y values are streamlined ( X1, Y1, X2,
     * Y2, X3, Y3, ...)
     */
 ...
 * having his own range, min/max/default values and precision. This let developers create
 * their own threshold curve, that they will have to provide through the
...
protected:
    ...
    unsigned int precisionBottom;  // Decimal number if this object has to handle "double"
                                   // values, for the Bottom slider
