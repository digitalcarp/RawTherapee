...
public:
    ...
    void setColoredBarSize(const int newX,
                           const int newY,
                           const int newW,
                           const int newH);  // Note: updateColoredBar shall be called
                                             // after to update the bar

    void updateColoredBar(const Cairo::RefPtr<Cairo::Context>& cr);

    bool canGetColors();

    // Method for convenience; if no Gradient provided, the ColoredBar will ask colors on
    // a per pixel basis
