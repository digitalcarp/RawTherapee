        ...
        if (edited_point == -1) {  // curve.type!=FCT_Parametric) {
            if (event->button.button == 1) {
                ...
                case (FCT_Area_Insertion):
                    new_type = CSMove2D;  // Shown when adding a new node in a blank area,
                                          // both click and drag.
                ...
                case (FCT_Area_V):
                    new_type =
                        CSMove2D;  // Shown when vertical line clicked, not dragged.
                    editedHandle = FCT_EditedHandle_CPointUD;
                    ugpX = curve.x.at(lit_point);
                    ugpY = curve.y.at(lit_point);
                    break;
    ...
    case GDK_BUTTON_RELEASE:
        if (edited_point == -1) {  // curve.type!=FCT_Parametric) {
            if (buttonPressed && event->button.button == 1) {
                ...
                // Removing any deleted point if we were previously modifying the point
                // position
                if (editedHandle
        ...
        if (curve.type == FCT_Linear || curve.type == FCT_MinMaxCPoints) {
            ...
            case (FCT_EditedHandle_None):
            {
                ...
                if ((lit_point != -1 && previous_lit_point != lit_point)
                    && (area & (FCT_Area_V | FCT_Area_Point)) && edited_point == -1)
                {
                    ...
                    if (curve.x.at(lit_point) < 3. * minDistanceX) {
                        // lit_point too near of the left border -> both handles are
                        // displayed on the right of the vertical line
            ...
            case (FCT_EditedHandle_LeftTan):
            {
                ...
                if (snapTo) {
                    // since this handle can only move in one direction, we can reuse the
                    // snapCoordinateX mechanism
