 ...
 *     curveLabel:    Name of the curve that will be inserted in the toggle button, before
 * the image. If empty, no text will prepend the image relatedWidget: pointer to a widget
 * (or NULL) that will be inserted next to the curve's toggle button. if a smart pointer
 * created by Gtk::manage is passed in, the widget will be deleted by the destructor,
 ...
 * This method should handle all curve number correctly, i.e. eventually display the curve
 * type buttons in a grid (or table)
 */
void CurveEditorGroup::curveListComplete()
{
    newLine();

    // We check the length of the label ; if it contains only one char (':'), we set it to
    // the right default string
...
void CurveEditorGroup::typeSelectionChanged(CurveEditor* ce, int n)
{
    ...
    if (!ce->curveType->get_active()) {
        ...
        if (n == ce->subGroup->valLinear || n == ce->subGroup->valUnchanged) {
            ...
            // button to 'Linear', we have to to call the curve listener manually, because
            // 'curveChanged' uses displayedCurve...
            if (cl) {
                if (cl->isMulti()) {
                    cl->curveChanged(ce);
                } else {
...
void CurveEditorGroup::updateGUI(CurveEditor* ce)
{
    ...
    // we update the curve type button to the corresponding curve type, only if it is not
    // currently set to 'Unchanged'
    if (ce->curveType->getSelected() < ce->subGroup->valUnchanged) {
        ce->curveType->setSelected(ce->selected);
    }
