    ...
    */
    ...
    void processNextJob()
    {
        ...
        try {
            ...
            if (tmb) {
                DEBUG("Preview Ready\n");
                j.listener_->previewReady(j.dir_id_,
                                          new FileBrowserEntry(tmb, j.dir_entry_));
        ...
        /* Issue 2406
                {
                    // the purpose of the output_ vector is to deliver the previewReady()
           calls in the same
...
void PreviewLoader::add(int dir_id,
                        const Glib::ustring& dir_entry,
