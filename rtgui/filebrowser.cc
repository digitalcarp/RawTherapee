      ...
      exportPanel(nullptr)
{
    ...
    p++;
    pmenu->attach(
        *Gtk::manage(developfast = new Gtk::MenuItem(M("FILEBROWSER_POPUPPROCESSFAST"))),
        0, 1, p, p + 1);
    p++;
    ...
    const std::array<std::string, Options::SORT_METHOD_COUNT> cnameSortMethods = {
        ...
        M("SORT_BY_RANK"), M("SORT_BY_LABEL"),
    };

    pmenu->attach(
        *Gtk::manage(menuSort = new Gtk::MenuItem(M("FILEBROWSER_POPUPSORTBY"))), 0, 1, p,
        p + 1);
    p++;
    ...
    if (options.menuGroupRank) {
        pmenu->attach(
            *Gtk::manage(menuRank = new Gtk::MenuItem(M("FILEBROWSER_POPUPRANK"))), 0, 1,
            p, p + 1);
        p++;
        Gtk::Menu* submenuRank = Gtk::manage(new Gtk::Menu());
        submenuRank->attach(
            *Gtk::manage(rank[0] = new Gtk::MenuItem(M("FILEBROWSER_POPUPUNRANK"))), 0, 1,
            p, p + 1);
        p++;
    ...
    std::array<std::string, 6> clabelInactiveIcons = {
        "circle-empty-darkgray-small", "circle-empty-red-small",
    ...
    if (options.menuGroupLabel) {
        pmenu->attach(
            *Gtk::manage(menuLabel = new Gtk::MenuItem(M("FILEBROWSER_POPUPCOLORLABEL"))),
            0, 1, p, p + 1);
        p++;
        ...
        if (options.menuGroupExtProg) {
                 ...
                 it++, itemNo++)
            {
                submenuExtProg->attach(
                    *Gtk::manage(amiExtProg[itemNo] = new Gtk::MenuItem((*it).first)), 0,
                    1, p, p + 1);
                p++;
            }
    ...
    if (options.menuGroupFileOperations) {
        ...
        submenuFileOperations->attach(
            *Gtk::manage(trash = new Gtk::MenuItem(M("FILEBROWSER_POPUPTRASH"))), 0, 1, p,
            p + 1);
        p++;
        submenuFileOperations->attach(
            *Gtk::manage(untrash = new Gtk::MenuItem(M("FILEBROWSER_POPUPUNTRASH"))), 0,
            1, p, p + 1);
        p++;
        submenuFileOperations->attach(*Gtk::manage(new Gtk::SeparatorMenuItem()), 0, 1, p,
                                      p + 1);
        p++;
        submenuFileOperations->attach(
            *Gtk::manage(rename = new Gtk::MenuItem(M("FILEBROWSER_POPUPRENAME"))), 0, 1,
            p, p + 1);
        p++;
        submenuFileOperations->attach(
            *Gtk::manage(remove = new Gtk::MenuItem(M("FILEBROWSER_POPUPREMOVE"))), 0, 1,
            p, p + 1);
        p++;
        submenuFileOperations->attach(
        ...
        p++;
        submenuFileOperations->attach(
            *Gtk::manage(copyTo = new Gtk::MenuItem(M("FILEBROWSER_POPUPCOPYTO"))), 0, 1,
            p, p + 1);
        p++;
        submenuFileOperations->attach(
            *Gtk::manage(moveTo = new Gtk::MenuItem(M("FILEBROWSER_POPUPMOVETO"))), 0, 1,
            p, p + 1);
        p++;
    ...
    if (options.menuGroupProfileOperations) {
        ...
        p++;
        submenuProfileOperations->attach(
            *Gtk::manage(pasteprof = new Gtk::MenuItem(M("FILEBROWSER_PASTEPROFILE"))), 0,
            1, p, p + 1);
        p++;
        submenuProfileOperations->attach(
        ...
        p++;
        submenuProfileOperations->attach(
            *Gtk::manage(applyprof = new Gtk::MenuItem(M("FILEBROWSER_APPLYPROFILE"))), 0,
            1, p, p + 1);
        p++;
        submenuProfileOperations->attach(
        ...
        p++;
        submenuProfileOperations->attach(
            *Gtk::manage(clearprof = new Gtk::MenuItem(M("FILEBROWSER_CLEARPROFILE"))), 0,
            1, p, p + 1);
        p++;
    ...
    pmenu->attach(*Gtk::manage(new Gtk::SeparatorMenuItem()), 0, 1, p, p + 1);
    p++;
    pmenu->attach(*Gtk::manage(menuDF = new Gtk::MenuItem(M("FILEBROWSER_DARKFRAME"))), 0,
                  1, p, p + 1);
    p++;
    pmenu->attach(*Gtk::manage(menuFF = new Gtk::MenuItem(M("FILEBROWSER_FLATFIELD"))), 0,
                  1, p, p + 1);
    p++;

    pmenu->attach(*Gtk::manage(new Gtk::SeparatorMenuItem()), 0, 1, p, p + 1);
    p++;
    pmenu->attach(*Gtk::manage(cachemenu = new Gtk::MenuItem(M("FILEBROWSER_CACHE"))), 0,
                  1, p, p + 1);
    ...
    trash->add_accelerator("activate", pmenu->get_accel_group(), GDK_KEY_Delete,
                           (Gdk::ModifierType)0, Gtk::ACCEL_VISIBLE);
    untrash->add_accelerator("activate", pmenu->get_accel_group(), GDK_KEY_Delete,
...
#ifdef _WIN32
    if (miOpenDefaultViewer) {
        miOpenDefaultViewer->signal_activate().connect(sigc::bind(
            sigc::mem_fun(*this, &FileBrowser::menuItemActivated), miOpenDefaultViewer));
    }
#endif
    ...
    partpasteprof->signal_activate().connect(
        sigc::bind(sigc::mem_fun(*this, &FileBrowser::menuItemActivated), partpasteprof));
    applyprof->signal_activate().connect(
        sigc::bind(sigc::mem_fun(*this, &FileBrowser::menuItemActivated), applyprof));
    applypartprof->signal_activate().connect(
        sigc::bind(sigc::mem_fun(*this, &FileBrowser::menuItemActivated), applypartprof));
...
void FileBrowser::rightClicked()
{
    ...
    p++;
    submenuDF->attach(
        *Gtk::manage(autoDF = new Gtk::MenuItem(M("FILEBROWSER_AUTODARKFRAME"))), 0, 1, p,
        p + 1);
    p++;
    submenuDF->attach(
        *Gtk::manage(thisIsDF = new Gtk::MenuItem(M("FILEBROWSER_MOVETODARKFDIR"))), 0, 1,
        p, p + 1);
    ...
    p++;
    submenuFF->attach(
        *Gtk::manage(autoFF = new Gtk::MenuItem(M("FILEBROWSER_AUTOFLATFIELD"))), 0, 1, p,
        p + 1);
    p++;
    submenuFF->attach(
    ...
    else if (m == autoDF)
    {
        ...
        if (!mselected.empty()) {
            ...
            if (fc.run() == Gtk::RESPONSE_APPLY) {
                ...
                for (size_t i = 0; i < mselected.size(); i++) {
                    ...
                    mselected[i]->thumbnail->setProcParams(lpp, nullptr, FILEBROWSER,
                                                           false);
                }
        ...
        if (!options.rtSettings.darkFramesPath.empty()) {
                    ...
                    ->query_exists())
            {
                ...
                Gtk::MessageDialog msgd(msg_, true, Gtk::MESSAGE_ERROR, Gtk::BUTTONS_OK,
                                        true);
...
void FileBrowser::pasteProfile()
{
    ...
    if (clipboard.hasProcParams()) {
        ...
        for (unsigned int i = 0; i < mselected.size(); i++) {
            ...
            rtengine::procparams::PartialProfile pastedPartProf(cbPartProf.pparams,
                                                                cbPartProf.pedited, true);

            // applying the PartialProfile to the thumb's ProcParams
            mselected[i]->thumbnail->setProcParams(*pastedPartProf.pparams,
                                                   pastedPartProf.pedited, FILEBROWSER);
    ...
    if (clipboard.hasProcParams()) {
        ...
        if (i == Gtk::RESPONSE_OK) {
            ...
            for (auto entry : mselected) {
                ...
                partialPasteDlg.applyPaste(pastedPartProf.pparams, pastedPartProf.pedited,
                                           cbPartProf.pparams, cbPartProf.pedited);

                // applying the temporary PartialProfile to the thumb's ProcParams
                entry->thumbnail->setProcParams(*pastedPartProf.pparams,
                                                pastedPartProf.pedited, FILEBROWSER);
                pastedPartProf.deleteInstance();
            }
...
bool FileBrowser::keyPressed(GdkEventKey* event)
{
               ...
               && shift)
    {
        ...
        return true;
    } else if (event->keyval == GDK_KEY_F3 && !(ctrl || shift || alt))
    {  // open Previous image from FileBrowser perspective
        FileBrowser::openPrevImage();
        return true;
    } else if (event->keyval == GDK_KEY_F4 && !(ctrl || shift || alt))
    {  // open Next image from FileBrowser perspective
...
void FileBrowser::enableTabMode(bool enable)
{
    ...
    if (options.inspectorWindow) {
        if (enable) {
            inspect->remove_accelerator(pmenu->get_accel_group(), GDK_KEY_f,
                                        (Gdk::ModifierType)0);
        } else {
            inspect->add_accelerator("activate", pmenu->get_accel_group(), GDK_KEY_f,
                                     (Gdk::ModifierType)0, Gtk::ACCEL_VISIBLE);
    ...
    if (partProfile->pparams && !selected.empty()) {
        ...
        for (size_t i = 0; i < selected.size(); i++) {
            (static_cast<FileBrowserEntry*>(selected[i]))
                ->thumbnail->setProcParams(*partProfile->pparams, partProfile->pedited,
                                           FILEBROWSER);
        }
        ...
        if (partialPasteDlg.run() == Gtk::RESPONSE_OK) {
            ...
            for (size_t i = 0; i < selected.size(); i++) {
                ...
                *dstProfile.pparams = (static_cast<FileBrowserEntry*>(selected[i]))
                ...
                (static_cast<FileBrowserEntry*>(selected[i]))
                    ->thumbnail->setProcParams(*dstProfile.pparams, dstProfile.pedited,
                                               FILEBROWSER);
...
void FileBrowser::applyFilter(const BrowserFilter& filter)
{
    ...
    numFiltered = 0;
    {
        ...
        if (selected.empty()
            || (anchor
                && std::find(selected.begin(), selected.end(), anchor) == selected.end()))
        {
            anchor = nullptr;
        }
    }
    ...
    ThumbBrowserEntryBase* entryb) const  // true -> entry complies filter
{
    ...
    if (!filter.vFilterStrings.empty()) {
        ...
        if (filter.matchEqual) {
            ...
            if (iFilenameMatch > 0)
            {  // match is found for at least one of vFilterStrings in FileName
                return false;
            }
        }
    }
    ...
    for (size_t i = 0; i < tbe.size(); i++) {
        ...
        if (tbe[i]->getThumbButtonSet()) {
            ...
            tbe[i]->thumbnail->updateCache();  // needed to save the colorlabel to disk in
                                               // the procparam file(s) and the cache
                                               // image data file
        }
    }
...
void FileBrowser::fromTrashRequested(std::vector<FileBrowserEntry*> tbe)
{
    ...
    for (size_t i = 0; i < tbe.size(); i++) {
        // if thumbnail was marked inTrash=true then param file must be there, no need to
        // run customprofilebuilder
...
void FileBrowser::rankingRequested(std::vector<FileBrowserEntry*> tbe, int rank)
{
    ...
    for (size_t i = 0; i < tbe.size(); i++) {
        ...
        tbe[i]->thumbnail->updateCache();  // needed to save the colorlabel to disk in the
                                           // procparam file(s) and the cache image data
                                           // file
...
void FileBrowser::openNextImage()
{
    ...
    if (!fd.empty() && selected.size() > 0 && !App::get().options().tabbedUI) {
        for (size_t i = 0; i < fd.size() - 1; i++) {
            if (selected[0]->thumbnail->getFileName() == fd[i]->filename)
            {  // located 1-st image in current selection
                if (i < fd.size() && tbl) {
...
void FileBrowser::openNextPreviousEditorImage(const Glib::ustring& fname,
                                              eRTNav nextPrevious)
{
...
void FileBrowser::openRequested(std::vector<FileBrowserEntry*> mselected)
{
    ...
    size_t openStart = App::get().options().tabbedUI
                           ? 0
