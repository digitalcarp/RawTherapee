...
void BatchToolPanelCoordinator::initSession()
{
    ...
    /*    if (!selected.empty()) {
       ...
       "batch processing", &pparamsEdited);
        }
    */

    if (!selected.empty()) {

        // The first selected image (in the thumbnail list, not the click list) is used to
        // populate the EditorPanel and set the default values
        pparams = selected[0]->getProcParams();
        ...
        if (selected.size() == 1) {
            ...
            toneCurve->setAdjusterBehavior(false, false, false, false, false, false,
                                           false, false);
            ...
            colorappearance->setAdjusterBehavior(false, false, false, false, false, false,
            ...
            perspective->setAdjusterBehavior(false, false, false, false, false, false,
                                             false);
            ...
            wavelet->setAdjusterBehavior(false, false, false, false, false, false, false,
            ...
            toneCurve->setAdjusterBehavior(options.baBehav[ADDSET_TC_EXPCOMP],
                                           ...
                                           options.baBehav[ADDSET_TC_SHCOMP],
            ...
            whitebalance->setAdjusterBehavior(
                options.baBehav[ADDSET_WB_TEMPERATURE], options.baBehav[ADDSET_WB_GREEN],
                options.baBehav[ADDSET_WB_EQUAL], options.baBehav[ADDSET_WB_TEMPBIAS]);
            ...
            colorappearance->setAdjusterBehavior(
                options.baBehav[ADDSET_CAT_DEGREE],
                ...
                options.baBehav[ADDSET_CAT_CHROMA], options.baBehav[ADDSET_CAT_CONTRAST],
            ...
            retinex->setAdjusterBehavior(
                options.baBehav[ADDSET_RETI_STR], options.baBehav[ADDSET_RETI_NEIGH],
                options.baBehav[ADDSET_RETI_LIMD], options.baBehav[ADDSET_RETI_OFFS],
                options.baBehav[ADDSET_RETI_VART], options.baBehav[ADDSET_RETI_GAM],
                options.baBehav[ADDSET_RETI_SLO]);

            chmixer->setAdjusterBehavior(options.baBehav[ADDSET_CHMIXER]);
            blackwhite->setAdjusterBehavior(options.baBehav[ADDSET_BLACKWHITE_HUES],
                                            options.baBehav[ADDSET_BLACKWHITE_GAMMA]);
            shadowshighlights->setAdjusterBehavior(options.baBehav[ADDSET_SH_HIGHLIGHTS],
            ...
            dirpyrequalizer->setAdjusterBehavior(
                options.baBehav[ADDSET_DIRPYREQ],
                options.baBehav[ADDSET_DIRPYREQ_THRESHOLD],
                options.baBehav[ADDSET_DIRPYREQ_SKINPROTECT]);
            wavelet->setAdjusterBehavior(
                options.baBehav[ADDSET_WA], options.baBehav[ADDSET_WA_THRESHOLD],
                ...
                options.baBehav[ADDSET_WA_RADIUS], options.baBehav[ADDSET_WA_SKYPROTECT],
            ...
            dirpyrdenoise->setAdjusterBehavior(
                options.baBehav[ADDSET_DIRPYRDN_LUMA],
            ...
            if (options.baBehav[ADDSET_SHARP_DAMPING]) {
                pparams.sharpening.deconvdamping = pparams.prsharpening.deconvdamping = 0;
            }
                ...
                pparams.colorToning.redlow = pparams.colorToning.greenlow =
                    pparams.colorToning.bluelow = pparams.colorToning.redmed =
                        pparams.colorToning.greenmed = pparams.colorToning.bluemed =
                            pparams.colorToning.redhigh = pparams.colorToning.greenhigh =
        ...
        for (size_t i = 0; i < toolPanels.size(); i++) {
            toolPanels[i]->setDefaults(&pparams, &pparamsEdited);
            toolPanels[i]->read(&pparams, &pparamsEdited);
            // TODO: autoOpenCurve has been disabled because initSession is called on each
            // parameter change from the editor panel, if the thumbnail remains selected
            // in the DirectoryBrowser (i.e. always, unless the user think about
            // deselecting it)
    ...
    // ParamsEdited are set to false for initialization and is updated each time panel is
    // changed (mandatory for Locallab)
    pparamsEdited.set(false);
}

void BatchToolPanelCoordinator::panelChanged(const rtengine::ProcEvent& event,
                                             const Glib::ustring& descr)
{
    ...
    // panel coordinator, otherwise we adjust the initial parameters on a per-image basis.
    if (selected.size() == 1) {
        ...
        if (event == rtengine::EvCTHFlip) {
            ...
            crop->rotateCrop(pparams.coarse.rotate, pparams.coarse.hflip,
                             pparams.coarse.vflip);
            crop->write(&pparams, &pparamsEdited);
    ...
    if (event == rtengine::EvAutoExp || event == rtengine::EvClip)
        for (size_t i = 0; i < selected.size(); i++) {
            ...
            // (depending on the raw histogram), so we're forcing its paramseditd value to
            // false (i.e. mixed state)
    ...
    for (size_t i = 0; i < selected.size(); i++) {
        newParams = initialPP[i];
        // If only one file is selected, slider's addMode has been set to false, and hence
        // the behave like in SET mode like in an editor ; that's why we force the
...
void BatchToolPanelCoordinator::getAutoWB(double& temp,
                                          double& green,
                                          double equal,
...
void BatchToolPanelCoordinator::getCamWB(double& temp,
                                         double& green,
                                         rtengine::StandardObserver observer)
{
...
void BatchToolPanelCoordinator::procParamsChanged(Thumbnail* thm,
                                                  int whoChangedIt,
                                                  bool upgradeHint)
{
