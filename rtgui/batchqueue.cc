...
{
// Look for N or -N in templateText at position ix, meaning "index from end" and "index
// from start". For N, return Nth index from the end, and for -N return the Nth index from
// the start. N is a digit 1 through 9. The returned value is not range-checked, so it may
// be >=numPathElements. or negative. The caller performs any required range-checking.
int decodePathIndex(unsigned int& ix,
                    const Glib::ustring& templateText,
                    size_t numPathElements)
{
    int pathIndex =
        static_cast<int>(numPathElements);  // a value that means input was invalid
    bool fromStart = false;
    if (ix < templateText.size()) {
        if (templateText[ix] == '-') {
            fromStart = true;  // minus sign means N is from the start rather than the end
                               // of the path
            ix++;
        }
    }
...
void appendFormattedTime(Glib::ustring& path,
                         unsigned int& ix,
                         const Glib::ustring& templateText,
...
BatchQueue::BatchQueue(FileCatalog* aFileCatalog)
    : processing(nullptr), fileCatalog(aFileCatalog), sequence(0), listener(nullptr)
{
    ...
    pmenu.attach(
        *Gtk::manage(open = new Gtk::MenuItem(M("FILEBROWSER_POPUPOPENINEDITOR"))), 0, 1,
        p, p + 1);
    p++;
    pmenu.attach(
    ...
    pmenu.attach(*Gtk::manage(head = new MyImageMenuItem(M("FILEBROWSER_POPUPMOVEHEAD"),
                                                         "goto-start-small")),
                 0, 1, p, p + 1);
    p++;

    pmenu.attach(*Gtk::manage(tail = new MyImageMenuItem(M("FILEBROWSER_POPUPMOVEEND"),
                                                         "goto-end-small")),
                 0, 1, p, p + 1);
    p++;
    ...
    pmenu.attach(*Gtk::manage(cancel = new MyImageMenuItem(
                                  M("FILEBROWSER_POPUPCANCELJOB"), "cancel-small")),
                 0, 1, p, p + 1);
    ...
    open->add_accelerator("activate", pmaccelgroup, GDK_KEY_e, Gdk::CONTROL_MASK,
                          Gtk::ACCEL_VISIBLE);
    selall->add_accelerator("activate", pmaccelgroup, GDK_KEY_a, Gdk::CONTROL_MASK,
                            Gtk::ACCEL_VISIBLE);
    head->add_accelerator("activate", pmaccelgroup, GDK_KEY_Home, (Gdk::ModifierType)0,
                          Gtk::ACCEL_VISIBLE);
    tail->add_accelerator("activate", pmaccelgroup, GDK_KEY_End, (Gdk::ModifierType)0,
                          Gtk::ACCEL_VISIBLE);
    ...
    cancel->signal_activate().connect(
        std::bind(&BatchQueue::cancelItems, this, std::ref(selected)));
...
// Reduce the max size of a thumb, since thumb is processed synchronously on adding to
// queue leading to very long waiting when adding more images
...
{
    {
        ...
        for (const auto entry : entries) {
            ...
            // BatchQueueButtonSet have to be added before resizing to take them into
            // account
            ...
            if (!entry->params->save(tempFile)) entry->savedParamsFile = tempFile;
            ...
            if (entry->thumbnail) entry->thumbnail->imageEnqueued();
        }
    }
...
{
    const auto fileName =
        Glib::build_filename(App::get().options().rtdir, "batch", "queue.csv");
...

    {
        ...
        file
            << "input image full path|param file full path|output image full path|file format|jpeg quality|jpeg subsampling|"
        ...
        for (const auto fdEntry : fd) {
            ...
            // Warning: for code's simplicity in loadBatchQueue, each field must end by
            // the '|' character, safer than ';' or ',' since it can't be used in paths
#ifdef _WIN32
            // on windows it crashes if we don't use c_str() and filename etc. contain
            // special (e.g. chinese) characters, see issue 3387
            file << entry->filename.c_str() << '|' << entry->savedParamsFile.c_str()
            ...
            file << entry->filename << '|' << entry->savedParamsFile << '|'
                 ...
                 << (saveFormat.tiffFloat ? 1 : 0) << '|' << saveFormat.tiffUncompressed
                 << '|' << saveFormat.saveParams << '|' << entry->forceFormatOpts << '|'
                 << entry->fast_pipeline << '|' << saveFormat.bigTiff << '|' << std::endl;
        }
    }
...
bool BatchQueue::loadBatchQueue()
{
    ...
    if (file.is_open()) {
        ...
        while (std::getline(file, row)) {
            ...
            if (source.empty() || paramsFile.empty()) continue;
            ...
            if (pparams.load(paramsFile)) continue;

            auto thumb = CacheManager::getInstance()->getEntry(source);

            if (!thumb) continue;

            auto job = rtengine::ProcessingJob::create(source, thumb->getType() == FT_Raw,
                                                       pparams, fast);
...
void BatchQueue::cancelItems(const std::vector<ThumbBrowserEntryBase*>& items)
{
...

    {
        ...
        for (const auto item : items) {
            ...
            if (pos == fd.end()) continue;

            fd.erase(pos);

            rtengine::ProcessingJob::destroy(entry->job);

            if (entry->thumbnail) entry->thumbnail->imageRemovedFromQueue();
...
{
    {
        ...
        for (auto item = items.rbegin(); item != items.rend(); ++item) {
            ...
            if (pos == fd.end() || pos == fd.begin()) continue;

            fd.erase(pos);

            // find the first item that is not under processing
            const auto newPos = std::find_if(fd.begin(), fd.end(),
                                             [](const ThumbBrowserEntryBase* fdEntry) {
                                                 return !fdEntry->processing;
                                             });
...
void BatchQueue::updateDestinationPathPreview()
{
    ...
    if (!selected.empty()) {
        ...
        int sequence =
            0;  // Sequence during subsequent queue processing can't be determined here
        const auto& options = App::get().options();
        Glib::ustring baseDestination = calcAutoFileNameBase(
            entry.filename, sequence, options.saveFormatBatch.format);
        Glib::ustring destination = Glib::ustring::compose(
            "%1.%2", baseDestination, options.saveFormatBatch.format);
...
void BatchQueue::error(const Glib::ustring& descr)
{
    ...
    if (listener) {
        ...
        idle_register.add([bql, descr]() -> bool {
...
rtengine::ProcessingJob* BatchQueue::imageReady(rtengine::IImagefloat* img)
{
    ...
    if (processing->outFileName.empty()) {  // auto file name
        ...
        // The output filename's extension is forced to the current or selected output
        // format, despite what the user have set in the filename's field of the "Save as"
        // dialog box
    ...
    if (img && !fname.empty()) {
        ...
        if (saveFormat.format == "tif") {
            err = img->saveAsTIFF(fname, saveFormat.tiffBits, saveFormat.tiffFloat,
                                  saveFormat.tiffUncompressed, saveFormat.bigTiff);
    ...
    if (remove_button_set) {
        // ButtonSet have Cairo::Surface which might be rendered while we're trying to
        // delete them
        GThreadLock lock;
    ...
    if (saveBatchQueue()) {
        ...
        // Delete all files in directory batch when finished, just to be sure to remove
        // zombies
        auto isEmpty = false;
...
// format is the output file format, e.g. "jpg", "tif", "png", etc.  It is used to put
// format in file path using "%e"
                                               ...
                                               const Glib::ustring& format)
{
    ...
    for (int k = extpos - 1; k >= 0 && origFileName[k] != '/' && origFileName[k] != '\\';
         k--)
    {
        ...
        while (ix < options.savePathTemplate.size()) {
            if (options.savePathTemplate[ix] == '%') {
                ...
                if (options.savePathTemplate[ix] == 'P') {
                    ...
                    if (n < 0) {
                        n = 0;  // if too many elements specified, return all available
                                // elements
                    }
                    if (n < static_cast<int>(da.size())) {
                        ...
                        for (unsigned int i = static_cast<unsigned int>(n); i < da.size();
                             i++)
                        {
                            path += da[i] + PATH_SEPARATOR;
                        }
                    }
                    // If the next template character is a separator, skip it, because
                    ...
                    // insert path elements from the start of the path up to the given
                    // index
                    ix++;
                    int n = decodePathIndex(ix, options.savePathTemplate, da.size());
                    if (n >= 0) {
                    ...
                    for (unsigned int i = 0; static_cast<int>(i) <= n && i < da.size();
                         i++)
                    {
                        path += da[i] + PATH_SEPARATOR;
                    }
                ...
                {  // file format from input args
                    ...
                    if (pparams.load(origFileName + App::PARAM_FILE_EXTENSION) == 0) {
                        ...
                        rank = '0';  // if param file not loaded (e.g. does not exist),
                                     // default to rank=0
                    }
                    ...
                    // Insert formatted date/time value. Character after 't' defines time
                    // source
                    if (++ix < options.savePathTemplate.size()) {
                        ...
                        case 'F':  // time when file was last saved
                        {
                            Glib::RefPtr<Gio::File> file =
                                Gio::File::create_for_path(origFileName);
                            if (file) {
                                Glib::RefPtr<Gio::FileInfo> info =
                                    file->query_info(G_FILE_ATTRIBUTE_TIME_MODIFIED);
                                if (info) {
                        ...
                        case 'P':  // time when picture was taken
                        {
                            const auto timestamp =
                                FramesData(origFileName).getDateTimeAsTS();
                        ...
                        if (dateTime) {
                            appendFormattedTime(path, ix, options.savePathTemplate,
                                                dateTime);
                        }
                    }
                }
                                               ...
                                               const Glib::ustring& format)
{
    ...
    // if that's not possible (e.g. locked by viewer, R/O), we revert to the standard
    // naming scheme
    ...
    for (int tries = 0; tries < 100; tries++) {
        if (tries == 0) {
            fname = Glib::ustring::compose(
                "%1.%2", Glib::build_filename(dstdir, dstfname), format);
        } else {
            fname = Glib::ustring::compose(
                "%1-%2.%3", Glib::build_filename(dstdir, dstfname), tries, format);
        }
...
{
    const std::vector<ThumbBrowserEntryBase*> bqe = { static_cast<BatchQueueEntry*>(
        actionData) };
