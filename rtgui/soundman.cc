...
{
#ifdef _WIN32
    // TODO: On Windows Vista/7 RT should register with the OS sound system, so it can
    // enjoy application specific volume, safed, process independent etc. from the start.
    // Function call is IAudioClient::Initialize
    // Unfortunately MinGW does not support this yet. If audioclient.h is available, add
    // an Init called once on program start.
    //
    // This mitigation plays an empty file on start, so RT is immediately available in the
    // Windows mixer at least
    playSoundAsync(Glib::ustring("sounds\\Empty.wav"));
#endif
}

// Plays a sound in async mode to not block the main thread
// param is either file name or name of the system event on Windows (e.g. "SystemAsterisk"
// or "SystemDefault").
