    ...
    , selected (-1) // -1 means that the button is invalid
{
    ...
    setExpandAlignProperties(imageContainer, true, false, Gtk::Align::FILL, Gtk::Align::CENTER);
    button->set_relief (Gtk::RELIEF_NORMAL);
    button->add(*imageContainer);

    if (!label.empty()) {
        Gtk::Label* buttonLabel = Gtk::manage ( new Gtk::Label(label + " ") );
        setExpandAlignProperties(buttonLabel, false, false, Gtk::Align::START, Gtk::Align::CENTER);
    ...
    setExpandAlignProperties(buttonGroup, true, false, Gtk::Align::FILL, Gtk::Align::CENTER);
