                    ...
                    const rtengine::procparams::CropParams& cparams)
{
    ...
    if (cparams.guide != rtengine::procparams::CropParams::Guide::RULE_OF_DIAGONALS
        ...
        && cparams.guide != rtengine::procparams::CropParams::Guide::GOLDEN_TRIANGLE_2)
    {
        ...
        case rtengine::procparams::CropParams::Guide::EPASSPORT:
        {
             ...
             * measurements within which the eyes and chin-crown distance must lie. I
             * averaged those measurements to produce these guides. The first horizontal
             ...
             * (ie the top of the head, not the top of the hair) is between 29mm and
             * 34mm."
             */
    ...
    } else if (cparams.guide == rtengine::procparams::CropParams::Guide::GOLDEN_TRIANGLE_1
               || cparams.guide
...
bool confirmOverwrite(Gtk::Window& parent, const std::string& filename)
{
    ...
    if (Glib::file_test(filename, Glib::FILE_TEST_EXISTS)) {
        Glib::ustring msg_ =
            Glib::ustring("<b>\"") + escapeHtmlChars(Glib::path_get_basename(filename))
              ...
              bool fullImageVisible)
{
    ...
    cr->rectangle(imx, round(imy + c1y) + 0.5, round(c1x) + 0.5,
                  round(c2y - c1y + 1) + 0.5);
    cr->rectangle(round(imx + c2x) + 0.5, round(imy + c1y) + 0.5, round(imw - c2x) + 0.5,
                  round(c2y - c1y + 1) + 0.5);
    cr->fill();
      ...
      useEnabled(useEnabled)
{
    ...
    titleEvBox->set_above_child(false);  // this is the key! By making it below the child,
...
MyScrolledToolbar::MyScrolledToolbar()
{
    ...
    // Works fine with Gtk 3.22, but a custom made get_preferred_height had to be created
    // as a workaround taken from the official Gtk3.22 source code
...
bool MyScrolledToolbar::on_scroll_event(GdkEventScroll* event)
{
    ...
    if (adjust && scroll) {
        ...
        //        printf("MyScrolledToolbar::on_scroll_event / delta_x=%.5f, delta_y=%.5f,
...
bool MyComboBoxText::on_scroll_event(GdkEventScroll* event)
{
    ...
    //    direction=%d, type=%d, send_event=%d\n",
    //            event->delta_x, event->delta_y, (int)event->direction, (int)event->type,
    //            event->send_event);
...
MySpinButton::MySpinButton()
{
    ...
    set_update_policy(
        Gtk::SpinButtonUpdatePolicy::UPDATE_IF_VALID);  // Avoid updating text if input is
                                                        // not a numeric
}
...
TextOrIcon::TextOrIcon(const Glib::ustring& icon_name,
...
MyRadioImageMenuItem::MyRadioImageMenuItem(const Glib::ustring& label,
                                           RTImage* image,
...
void MyProgressBar::get_preferred_width_for_height_vfunc(int height,
                                                         int& minimum_width,
                                                         int& natural_width) const
{
                                  ...
                                  bool updateBackBufferSize)
{
    ...
    // WARNING: we're assuming that the surface type won't change during all the execution
    // time of RT. I guess it may be wrong when the user change the gfx card display
    // settings!?
