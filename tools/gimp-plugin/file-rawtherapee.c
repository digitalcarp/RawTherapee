...
static gint32 load_image(const gchar* filename, GimpRunMode run_mode, GError** error);

static gint32
load_thumbnail_image(const gchar* filename, gint thumb_size, GError** error);
...
static void init(void)
{
    static const GimpParamDef load_args[] = {
        { GIMP_PDB_INT32, "run-mode",
          "The run mode { RUN-INTERACTIVE (0), RUN-NONINTERACTIVE (1) }" },
        { GIMP_PDB_STRING, "filename", "The name of the file to load." },
        { GIMP_PDB_STRING, "raw-filename", "The name entered" },
    };

    static const GimpParamDef load_return_vals[] = { { GIMP_PDB_IMAGE, "image",
                                                       "Output image" } };
    ...
    static const GimpParamDef thumb_return_vals[] = { { GIMP_PDB_IMAGE, "image",
                                                        "Thumbnail image" } };
    ...
    if (g_spawn_sync(NULL, argv, NULL, G_SPAWN_STDERR_TO_DEV_NULL | G_SPAWN_SEARCH_PATH,
                     NULL, NULL, &rawtherapee_stdout, NULL, NULL, NULL))
    {
        int rtmajor = 0, rtminor = 0;

        if (sscanf(rawtherapee_stdout, "RawTherapee, version %d.%d", &rtmajor, &rtminor)
                == 2
            && rtmajor >= 5 && rtminor >= 2)
        {
    ...
    if (!have_rawtherapee) return;

    gimp_install_procedure(
        LOAD_THUMB_PROC, "Load thumbnail from a raw image via rawtherapee",
        "This plug-in loads a thumbnail from a raw image by calling rawtherapee-cli.",
        "Alberto Griggio", "Alberto Griggio", "2017", NULL, NULL, GIMP_PLUGIN,
        G_N_ELEMENTS(thumb_args), G_N_ELEMENTS(thumb_return_vals), thumb_args,
        thumb_return_vals);

    for (i = 0; i < G_N_ELEMENTS(file_formats); i++) {
        const FileFormat* format = &file_formats[i];

        gimp_install_procedure(format->load_proc, format->load_blurb, format->load_help,
                               "Alberto Griggio", "Alberto Griggio", "2017",
                               format->file_type, NULL, GIMP_PLUGIN,
                               G_N_ELEMENTS(load_args), G_N_ELEMENTS(load_return_vals),
                               load_args, load_return_vals);

        gimp_register_file_handler_mime(format->load_proc, format->mime_type);
        gimp_register_file_handler_raw(format->load_proc);
        gimp_register_magic_load_handler(format->load_proc, format->extensions, "",
                                         format->magic);
    ...
    *return_vals = values;
    ...
    for (i = 0; i < G_N_ELEMENTS(file_formats); i++) {
        ...
        if (format->load_proc && !strcmp(name, format->load_proc)) {
            ...
            image_ID = load_thumbnail_image(param[0].data.d_string, param[1].data.d_int32,
                                            &error);
    ...
    if (i == G_N_ELEMENTS(file_formats)) status = GIMP_PDB_CALLING_ERROR;
...
static gint32 load_image(const gchar* filename, GimpRunMode run_mode, GError** error)
{
    ...
    gchar* argv[] = { "rawtherapee", "-gimp", (gchar*)filename, filename_out, NULL };

    gimp_progress_init_printf(_("Opening '%s'"), gimp_filename_to_utf8(filename));

    if (g_spawn_sync(NULL, argv, NULL,
                     /*G_SPAWN_STDOUT_TO_DEV_NULL |*/
                     G_SPAWN_STDERR_TO_DEV_NULL | G_SPAWN_SEARCH_PATH, NULL, NULL,
                     &rawtherapee_stdout, NULL, NULL, error))
    {
        image_ID = gimp_file_load(run_mode, filename_out, filename_out);
        if (image_ID != -1) gimp_image_set_filename(image_ID, filename);
    }
...
static gint32 load_thumbnail_image(const gchar* filename, gint thumb_size, GError** error)
{
    gint32 image_ID = -1;
                        ...
                        &rawtherapee_stdout, NULL, NULL, error))
    {
        ...
        image_ID = gimp_file_load(GIMP_RUN_NONINTERACTIVE, filename_out, filename_out);
        if (image_ID != -1) {
