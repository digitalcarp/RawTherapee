...
void Imagefloat::setScanline(int row,
                             ...
                             unsigned int numSamples)
{
    ...
    // The DNG decoder convert to 32 bits float data even if the file contains 16 or 24
    // bits data. DNG_HalfToFloat and DNG_FP24ToFloat from dcraw.cc can be used to
    // manually convert from 16 and 24 bits to 32 bits float respectively
    switch (sampleFormat) {
    case (IIOSF_FLOAT16):
    {
        int ix = 0;
    ...
    case (IIOSF_LOGLUV32):
    {
        ...
        for (int i = 0; i < width; i++) {
            ...
            Color::xyz2srgb(xyzvalues[0], xyzvalues[1], xyzvalues[2], rgbvalues[0],
...
void Imagefloat::getStdImage(const ColorTemp& ctemp,
                             int tran,
                             Imagefloat* image,
...
// convert values's range to [0;1] ; this method assumes that the input values's range is
// [0;65535]
...
// convert values's range to [0;65535 ; this method assumes that the input values's range
// is [0;1]
...
void Imagefloat::ExecCMSTransform(cmsHTRANSFORM hTransform)
{
    ...
    // this new feature, memory allocation have to be modified too to build temporary
    // buffers that allow multi processor execution
#ifdef _OPENMP
#pragma omp parallel
#endif
    {
