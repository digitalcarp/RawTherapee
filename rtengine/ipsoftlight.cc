...
inline float sl(float blend, float x)
{
    if (!rtengine::OOG(x)) {
        ...
        return rtengine::intp(
            blend,
            rtengine::Color::igamma_srgb(v * v * (3.f - 2.f * v) * rtengine::MAXVALF), x);
    }
...
inline vfloat sl(vfloat blend, vfloat x)
{
    ...
    return vself(vmaskf_gt(x, F2V(rtengine::MAXVALF)), x,
                 vself(vmaskf_lt(x, ZEROV), x,
                       vintpf(blend,
                              rtengine::Color::igammatab_srgb[v * v * (F2V(3.f) - (v + v))
                                                              * rtengine::MAXVALF],
                              x)));
}
#endif
...
void ImProcFunctions::softLight(
    LabImage* lab,
    const rtengine::procparams::SoftLightParams& softLightParams)
{
...
#pragma omp for schedule(dynamic, 16)
        ...
        for (int i = 0; i < lab->H; ++i) {
            ...
            for (; j < lab->W - 3; j += 4) {
                vfloat Xv, Yv, Zv;
                vfloat Rv, Gv, Bv;
                Color::Lab2XYZ(LVFU(lab->L[i][j]), LVFU(lab->a[i][j]), LVFU(lab->b[i][j]),
                               Xv, Yv, Zv);
                ...
                for (int k = 0; k < 4; ++k) {
                    Color::XYZ2Lab(Xv[k], Yv[k], Zv[k], lab->L[i][j + k],
                                   lab->a[i][j + k], lab->b[i][j + k]);
                }
            }
