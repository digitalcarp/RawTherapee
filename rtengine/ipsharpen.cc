...
void sharpenHaloCtrl(float** luminance,
                     float** blurmap,
                     float** base,
                     float** blend,
                     int W,
    ...
    for (int i = 2; i < H - 2; i++) {
        ...
        for (int j = 2; j < W - 2; j++) {
            ...
            float delta = sharpenParam.threshold.multiply<float, float, float>(
                rtengine::min(
                    fabsf(diff),
                    upperBound),  // X axis value = absolute value of the difference
                sharpFac
                    * diff  // Y axis max value = sharpening.amount * signed difference
            );
...
void ImProcFunctions::deconvsharpening(float** luminance,
                                       float** tmp,
...
#pragma omp for
            ...
            for (int i = 0; i < H; ++i) {
                for (int j = 0; j < W; ++j) {
                    blur[i][j] =
                        intp(blend[i][j], luminance[i][j], std::max(blur[i][j], 0.0f));
                }
            }
        }
    }
...
#pragma omp for
        ...
        for (int i = 0; i < H; ++i) {
            for (int j = 0; j < W; ++j) {
                luminance[i][j] =
                    intp(blend[i][j] * amount, max(tmpI[i][j], 0.0f), luminance[i][j]);
            }
        }
...
#pragma omp for
            ...
            for (int i = 0; i < H; ++i) {
                for (int j = 0; j < W; ++j) {
                    luminance[i][j] =
                        intp(blend[i][j], luminance[i][j], max(blur[i][j], 0.0f));
                }
            }
        }
    }  // end parallel
...
void ImProcFunctions::deconvsharpeningloc(float** luminance,
                                          float** tmp,
                                          int W,
                                          int H,
                                          float** loctemp,
...
#pragma omp parallel for
    ...
    // calculate contrast based blend factors to reduce sharpening in regions with low
    // contrast
...
#pragma omp parallel
#endif
    {
        for (int k = 0; k < itera; k++) {
            if (!needdamp) {
                // apply gaussian blur and divide luminance by result of gaussian blur
                //    gaussianBlur (tmpI, tmp, W, H, sigma, nullptr, GAUSS_DIV,
                //    luminance);
                gaussianBlur(tmpI, tmp, W, H, sigma, false, GAUSS_DIV, luminance);
            } else {
                // apply gaussian blur + damping
...
#pragma omp for
            ...
            for (int i = 0; i < H; ++i) {
                for (int j = 0; j < W; ++j) {
                    loctemp[i][j] =
                        intp(blend[i][j], loctemp[i][j], max(blur[i][j], 0.0f));
                }
            }
        }
...
void deconvsharpeningrgbloc(float** luminance,
                            float** blend,
                            char** impulse,
                            int W,
                            int H,
...
bool checkForStop(float** tmpIThr, float** iterCheck, int fullTileSize, int border)
{
    for (int ii = border; ii < fullTileSize - border; ++ii) {
        ...
        for (int jj = border; jj < fullTileSize - border; jj += 4) {
            if (UNLIKELY(_mm_movemask_ps((vfloat)vmaskf_lt(
                    LVFU(tmpIThr[ii][jj]), LVFU(iterCheck[ii - border][jj - border])))))
            {
                return true;
            }
        }
...
void ImProcFunctions::CaptureDeconvSharpening_SE(float** luminance,
                                                 ...
                                                 double endVal)
{
    ...
    const float cornerDistance =
        sqrt(rtengine::SQR(bfw * nocoboot) + rtengine::SQR(bfh * nocoboot));
        ...
        for (int i = border; i < bfh - border; i += tileSize) {
            for (int j = border; j < bfw - border; j += tileSize) {
                ...
                if (endOfRow || endOfCol) {
                    ...
                    float maxVal = 0.f;
                    if (checkIterStop) {
                        for (int k = 0, ii = endOfCol ? bfh - fullTileSize + border : i;
                             k < tileSize; ++k, ++ii)
                        {
                            for (int l = 0,
                                     jj = endOfRow ? bfw - fullTileSize + border : j;
                                 l < tileSize; ++l, ++jj)
                            {
                                iterCheck[k][l] =
                                    oldLuminance[ii][jj] * blend[ii][jj] * 0.5f;
                    ...
                    if (maxVal < minBlend) {
                        // no pixel of the tile has a blend factor >= minBlend => skip the
                        // tile
                        continue;
                    }
                    for (int k = 0, ii = endOfCol ? bfh - fullTileSize : i - border;
                         k < fullTileSize; ++k, ++ii)
                    {
                        for (int l = 0, jj = endOfRow ? bfw - fullTileSize : j - border;
                             l < fullTileSize; ++l, ++jj)
                        {
                    ...
                    if (checkIterStop) {
                        for (int ii = 0; ii < tileSize; ++ii) {
                            for (int jj = 0; jj < tileSize; ++jj) {
                                iterCheck[ii][jj] = oldLuminance[i + ii][j + jj]
                                                    * blend[i + ii][j + jj] * 0.5f;
                    ...
                    for (int ii = i; ii < i + fullTileSize; ++ii) {
                        for (int jj = j; jj < j + fullTileSize; ++jj) {
                            tmpIThr[ii - i][jj - j] =
                                oldLuminance[ii - border][jj - border];
                            lumThr[ii - i][jj - j] =
                                oldLuminance[ii - border][jj - border];
                        }
                    }
                ...
                if (is3x3) {
                    for (int k = 0; k < iterations; ++k) {
                        // apply 3x3 gaussian blur and divide luminance by result of
                        // gaussian blur
                        rtengine::gauss3x3div2(tmpIThr, tmpThr, lumThr, fullTileSize,
                                               kernel3);
                        rtengine::gauss3x3mult2(tmpThr, tmpIThr, fullTileSize, kernel3);
                        if (checkIterStop && k < iterations - 1
                            && checkForStop(tmpIThr, iterCheck, fullTileSize, border))
                        {
                            break;
                        }
                    }
                    ...
                    for (int k = 0; k < iterations; ++k) {
                        // apply 5x5 gaussian blur and divide luminance by result of
                        // gaussian blur
                        rtengine::gauss5x5div2(tmpIThr, tmpThr, lumThr, fullTileSize,
                                               kernel5);
                    ...
                    for (int k = 0; k < iterations; ++k) {
                        // apply 5x5 gaussian blur and divide luminance by result of
                        // gaussian blur
                        rtengine::gauss7x7div2(tmpIThr, tmpThr, lumThr, fullTileSize,
                                               kernel7);
                    ...
                    for (int k = 0; k < iterations; ++k) {
                        // apply 5x5 gaussian blur and divide luminance by result of
                        // gaussian blur
                        rtengine::gauss9x9div2(tmpIThr, tmpThr, lumThr, fullTileSize,
                                               kernel9);
                    ...
                    if (sigmaCornerOffset != 0.f) {
                        const float distance =
                            sqrt(rtengine::SQR(i + tileSize / 2 - bfh / 2)
                                 + rtengine::SQR(j + tileSize / 2 - bfw / 2));
                        const float sigmaTile =
                            static_cast<float>(sigma) + distanceFactor * distance;
                        if (sigmaTile >= 0.4f) {
                            if (sigmaTile > 1.5f) {  // have to use 13x13 kernel
                                float lkernel13[13][13];
                                rtengine::compute13x13kernel2(
                                    static_cast<float>(sigma) + distanceFactor * distance,
                                    lkernel13);
                                for (int k = 0; k < iterations; ++k) {
                                    // apply 13x13 gaussian blur and divide luminance by
                                    // result of gaussian blur
                                    rtengine::gauss13x13div2(tmpIThr, tmpThr, lumThr,
                                                             fullTileSize, lkernel13);
                                    rtengine::gauss13x13mult2(tmpThr, tmpIThr,
                                                              fullTileSize, lkernel13);
                                    if (checkIterStop && k < iterations - 1
                                        && checkForStop(tmpIThr, iterCheck, fullTileSize,
                                                        border))
                                    {
                                        break;
                                    }
                                ...
                                float lkernel9[9][9];
                                rtengine::compute9x9kernel2(
                                    static_cast<float>(sigma) + distanceFactor * distance,
                                    lkernel9);
                                for (int k = 0; k < iterations; ++k) {
                                    // apply 9x9 gaussian blur and divide luminance by
                                    // result of gaussian blur
                                    rtengine::gauss9x9div2(tmpIThr, tmpThr, lumThr,
                                                           fullTileSize, lkernel9);
                                    rtengine::gauss9x9mult2(tmpThr, tmpIThr, fullTileSize,
                                                            lkernel9);
                                    if (checkIterStop && k < iterations - 1
                                        && checkForStop(tmpIThr, iterCheck, fullTileSize,
                                                        border))
                                ...
                                float lkernel7[7][7];
                                rtengine::compute7x7kernel2(
                                    static_cast<float>(sigma) + distanceFactor * distance,
                                    lkernel7);
                                for (int k = 0; k < iterations; ++k) {
                                    // apply 7x7 gaussian blur and divide luminance by
                                    // result of gaussian blur
                                    rtengine::gauss7x7div2(tmpIThr, tmpThr, lumThr,
                                                           fullTileSize, lkernel7);
                                    rtengine::gauss7x7mult2(tmpThr, tmpIThr, fullTileSize,
                                                            lkernel7);
                                    if (checkIterStop && k < iterations - 1
                                        && checkForStop(tmpIThr, iterCheck, fullTileSize,
                                                        border))
                                ...
                                float lkernel5[5][5];
                                rtengine::compute5x5kernel2(
                                    static_cast<float>(sigma) + distanceFactor * distance,
                                    lkernel5);
                                for (int k = 0; k < iterations; ++k) {
                                    // apply 7x7 gaussian blur and divide luminance by
                                    // result of gaussian blur
                                    rtengine::gauss5x5div2(tmpIThr, tmpThr, lumThr,
                                                           fullTileSize, lkernel5);
                                    rtengine::gauss5x5mult2(tmpThr, tmpIThr, fullTileSize,
                                                            lkernel5);
                                    if (checkIterStop && k < iterations - 1
                                        && checkForStop(tmpIThr, iterCheck, fullTileSize,
                                                        border))
                        ...
                        for (int k = 0; k < iterations; ++k) {
                            // apply 13x13 gaussian blur and divide luminance by result of
                            // gaussian blur
                            rtengine::gauss13x13div2(tmpIThr, tmpThr, lumThr,
                                                     fullTileSize, kernel13);
                            rtengine::gauss13x13mult2(tmpThr, tmpIThr, fullTileSize,
                                                      kernel13);
                            if (checkIterStop && k < iterations - 1
                                && checkForStop(tmpIThr, iterCheck, fullTileSize, border))
                            {
                ...
                if (endOfRow || endOfCol) {
                    // special handling for small tiles at end of row or column
                    for (int k = border, ii = endOfCol ? bfh - fullTileSize : i - border;
                         k < fullTileSize - border; ++k)
                    {
                        for (int l = border,
                                 jj = endOfRow ? bfw - fullTileSize : j - border;
                             l < fullTileSize - border; ++l)
                        {
                            luminance[ii + k][jj + l] =
                                rtengine::intp(blend[ii + k][jj + l], tmpIThr[k][l],
                                               luminance[ii + k][jj + l]);
                        }
                    }
                } else {
                    for (int ii = border; ii < fullTileSize - border; ++ii) {
                        for (int jj = border; jj < fullTileSize - border; ++jj) {
                            luminance[i + ii - border][j + jj - border] = rtengine::intp(
                                blend[i + ii - border][j + jj - border], tmpIThr[ii][jj],
                                luminance[i + ii - border][j + jj - border]);
                        }
                    }
                }
            }
...
float igammalog2(float x,
                 float p,
                 float s,
                 float g2,
                 float g4)  // same function as in iplocallab.cc
{
    return x <= g2 ? x / s : pow_F((x + g4) / (1.f + g4), p);  // continuous
}

float gammalog2(float x,
                float p,
                float s,
                float g3,
                float g4)  // same function as in iplocallab.cc
...
void ImProcFunctions::doCapture_Sharpening_SE(Imagefloat* rgb,
                                              int bfw,
                                              int bfh,
                                              struct localpass& locp,
                                              int sk,
...
#pragma omp parallel for schedule(dynamic, 16) if (multiThread)
    ...
    // calculate gamma as it was with Lab calculation but in RGB linear mode to have the
    // same 'behavior' as other gamma in Selective Editing This is actually not of major
    // importance, the main thing is to use the inverse function afterwards
    ...
    double ts1 = 9.03296;  // always the same 'slope' in the extreme shadows - slope Lab ,
                           // I can choose also 5 or 13!
    rtengine::Color::calcGamma(pwr1, ts1,
                               g_a);  // call to calcGamma with selected gamma and slope

    if (gamma1 != 1.f) {  // calculate new values with gamma for R, G, B of course with
                          // 65535 instead of 32768
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic, 16) if (multiThread)
#endif
        for (int i = 0; i < bfh; ++i) {
            for (int j = 0; j < bfw; ++j) {
                redgam[i][j] =
                    65535.f
                    * igammalog2(redgam[i][j] / 65535.f, gamma1, ts1, g_a[2], g_a[4]);
                greengam[i][j] =
                    65535.f
                    * igammalog2(greengam[i][j] / 65535.f, gamma1, ts1, g_a[2], g_a[4]);
                bluegam[i][j] =
                    65535.f
                    * igammalog2(bluegam[i][j] / 65535.f, gamma1, ts1, g_a[2], g_a[4]);
            }
        }
    }
...
#pragma omp parallel for schedule(dynamic, 16) if (multiThread)
        ...
        for (int i = 0; i < bfh; ++i) {
            Color::RGB2L(redVals[i], greenVals[i], blueVals[i], Y[i], wip,
                         bfw);  // mask values with gamma
        }
        float reducautocontrast = 1.f;  // to take noise into account

        buildBlendMask2(Y, clipMask, bfw, bfh, contrastsh, 1.f, autoshar, 2.f / s_scale,
                        1.f, reducautocontrast);  // build with gamma if need
        ...
        for (int i = 0; i < bfh; ++i) {
            for (int j = 0; j < bfw; ++j) {
                rgb->r(i, j) = rgb->g(i, j) = rgb->b(i, j) =
                    clipMask[i][j] * 65536.f;  // same values R G B to black and white
                                               // image, just for mask
            }
        }
        if (settings->verbose) {
...
#pragma omp parallel for schedule(dynamic, 16)
        ...
        buildBlendMask2(L, clipMask2, bfw, bfh, contrast, 1.f, autoshar, 2.f / s_scale,
                        1.f, reducautocontrast);
        ...
        CaptureDeconvSharpening_SE(YNew, YOld, clipMask2, bfw, bfh, locp, capradiu,
                                   deconvCo, deconvLat, itcheck, 0.2,
                                   0.9);  // 0.2 and 0.9 not used but in case.
...
#pragma omp parallel for schedule(dynamic, 16) if (multiThread)
            ...
            for (int i = 0; i < bfh; ++i) {
                for (int j = 0; j < bfw; ++j) {
                    redgam[i][j] =
                        65535.f
                        * gammalog2(redgam[i][j] / 65535.f, gamma1, ts1, g_a[3], g_a[4]);
                    greengam[i][j] = 65535.f
                                     * gammalog2(greengam[i][j] / 65535.f, gamma1, ts1,
                                                 g_a[3], g_a[4]);
                    bluegam[i][j] =
                        65535.f
                        * gammalog2(bluegam[i][j] / 65535.f, gamma1, ts1, g_a[3], g_a[4]);
                }
            }
        }
...
void ImProcFunctions::sharpening(LabImage* lab,
                                 const procparams::SharpeningParams& sharpenParam,
                                 bool showMask)
{
...
#pragma omp for
            ...
            for (int i = 0; i < H; ++i) {
                for (int j = 0; j < W; ++j) {
                    blur[i][j] =
                        intp(blend[i][j], lab->L[i][j], std::max(blur[i][j], 0.0f));
                }
            }
        }
    }
...
#pragma omp parallel
#endif
    {
        ...
        if (!sharpenParam.edgesonly) {
            ...
            bilateral<float, float>(lab->L, (float**)b3, b2, W, H,
        ...
        for (int i = 0; i < H; i++)
            for (int j = 0; j < W; j++) {
                ...
                float delta = sharpenParam.threshold.multiply<float, float, float>(
                    min(fabsf(diff),
                        upperBound),  // X axis value = absolute value of the difference,
                                      // truncated to the max value of this field
...
//! MicroContrast is a sharpening method developed by Manuel Llorens and documented here:
//! http://www.rawness.es/sharpening/?lang=en <BR>The purpose is maximize clarity of the
//! image without creating halo's. <BR>Addition from JD : pyramid  + pondered contrast
//! with matrix 5x5 <BR>2017 Ingo Weyrich : reduced processing time \param luminance :
//! Luminance channel of image
void ImProcFunctions::MLmicrocontrast(float** luminance, int W, int H)
{
    if (!params->sharpenMicro.enabled || params->sharpenMicro.contrast == 100
        || params->sharpenMicro.amount < 1.0)
    {
        return;
    }
    BENCHFUN
    ...
    const float amount = (k == 1 ? 2.7 : 1.) * params->sharpenMicro.amount
                         / 1500.0;  // amount 2000.0 quasi no artifacts ==> 1500 =
                                    // maximum, after artifacts, 25/9 if 3x3
    ...
    const float L98[11] = { 0.001f, 0.0015f, 0.002f, 0.004f, 0.006f, 0.008f,
                            0.01f,  0.03f,   0.05f,  0.1f,   0.1f };
    const float L95[11] = { 0.0012f, 0.002f, 0.005f, 0.01f, 0.02f, 0.05f,
                            0.1f,    0.12f,  0.15f,  0.2f,  0.25f };
    const float L92[11] = { 0.01f, 0.015f, 0.02f, 0.06f, 0.10f, 0.13f,
                            0.17f, 0.25f,  0.3f,  0.32f, 0.35f };
    const float L90[11] = { 0.015f, 0.02f, 0.04f, 0.08f, 0.12f, 0.15f,
                            0.2f,   0.3f,  0.4f,  0.5f,  0.6f };
    const float L87[11] = { 0.025f, 0.03f, 0.05f, 0.1f,  0.15f, 0.25f,
                            0.3f,   0.4f,  0.5f,  0.63f, 0.75f };
    const float L83[11] = { 0.055f, 0.08f, 0.1f, 0.15f, 0.2f, 0.3f,
                            0.4f,   0.5f,  0.6f, 0.75f, 0.85f };
    const float L80[11] = { 0.15f, 0.2f, 0.25f, 0.3f, 0.35f, 0.4f,
                            0.5f,  0.6f, 0.7f,  0.8f, 0.9f };
    const float L75[11] = { 0.22f, 0.25f, 0.3f,  0.4f, 0.5f, 0.6f,
                            0.7f,  0.8f,  0.85f, 0.9f, 0.95f };
    const float L70[11] = { 0.35f, 0.4f, 0.5f, 0.6f, 0.7f, 0.8f,
                            0.97f, 1.0f, 1.0f, 1.0f, 1.0f };
    const float L63[11] = { 0.55f, 0.6f, 0.7f, 0.8f, 0.85f, 0.9f,
                            1.0f,  1.0f, 1.0f, 1.0f, 1.0f };
    const float L58[11] = { 0.75f, 0.77f, 0.8f, 0.9f, 1.0f, 1.0f,
                            1.0f,  1.0f,  1.0f, 1.0f, 1.0f };
    // default 5
    // modulation contrast
    const float Cont0[11] = { 0.05f, 0.1f, 0.2f, 0.25f, 0.3f, 0.4f,
                              0.5f,  0.6f, 0.7f, 0.8f,  0.9f };
    const float Cont1[11] = { 0.1f, 0.2f, 0.3f, 0.4f,  0.5f, 0.6f,
                              0.7f, 0.8f, 0.9f, 0.95f, 1.0f };
    const float Cont2[11] = { 0.2f,  0.40f, 0.6f, 0.7f,  0.8f, 0.85f,
                              0.90f, 0.95f, 1.0f, 1.05f, 1.10f };
    const float Cont3[11] = { 0.5f, 0.6f, 0.7f,  0.8f,  0.85f, 0.9f,
                              1.0f, 1.0f, 1.05f, 1.10f, 1.20f };
    const float Cont4[11] = { 0.8f,  0.85f,  0.9f, 0.95f, 1.0f, 1.05f,
                              1.10f, 1.150f, 1.2f, 1.25f, 1.40f };
    const float Cont5[11] = { 1.0f,  1.1f,  1.2f, 1.25f, 1.3f, 1.4f,
                              1.45f, 1.50f, 1.6f, 1.65f, 1.80f };
...
#pragma omp for schedule(dynamic, 16)
        ...
        for (int j = 0; j < height; j++)
            for (int i = 0, offset = j * width + i; i < width; i++, offset++) {
                LM[offset] =
                    luminance[j][i] / 327.68f;  // adjust to [0;100] and to RT variables
            }
...
#pragma omp for schedule(dynamic, 16)
        ...
        for (int j = k; j < height - k; j++)
            for (int i = k, offset = j * width + i; i < width - k; i++, offset++) {
                ...
                float contrast;
                if (k == 1) {
                    contrast = sqrtf(SQR(LM[offset + 1] - LM[offset - 1])
                                     + SQR(LM[offset + width] - LM[offset - width]))
                               * 0.125f;  // for 3x3
                } else                    /* if (k==2) */
                    contrast =
                        sqrtf(SQR(LM[offset + 1] - LM[offset - 1])
                              + SQR(LM[offset + width] - LM[offset - width])
                              + SQR(LM[offset + 2] - LM[offset - 2])
                              + SQR(LM[offset + 2 * width] - LM[offset - 2 * width]))
                        * 0.0625f;  // for 5x5
                ...
                float temp1 = sqrt2 * amount
                              * (LM[offset - width - 1] + LM[offset - width + 1]
                                 + LM[offset + width - 1] + LM[offset + width + 1]);
                temp1 += amount
                         * (LM[offset - width] + LM[offset - 1] + LM[offset + 1]
                            + LM[offset + width]);
                ...
                // add JD continue 5x5
                if (k == 2) {
                    float temp2 = -(LM[offset + 2 * width] + LM[offset - 2 * width]
                                    + LM[offset - 2] + LM[offset + 2]);

                    temp2 -= sqrt1d25
                             * (LM[offset + 2 * width - 1] + LM[offset + 2 * width + 1]
                                + LM[offset + width + 2] + LM[offset + width - 2]
                                + LM[offset - 2 * width - 1] + LM[offset - 2 * width + 1]
                                + LM[offset - width + 2] + LM[offset - width - 2]);

                    temp2 -=
                        sqrt2
                        * (LM[offset + 2 * width - 2] + LM[offset + 2 * width + 2]
                           + LM[offset - 2 * width - 2] + LM[offset - 2 * width + 2]);
                    temp2 += 18.601126159f
                             * v;  // 18.601126159 = 4 + 4 * sqrt(2) + 8 * sqrt(1.25)
                    temp2 *= 2.f * amount;
                    temp += temp2;
                }
                ...
                for (int row = j - k; row <= j + k; ++row) {
                    for (int offset2 = row * width + i - k;
                         offset2 <= row * width + i + k; ++offset2)
                    {
            ...
            breakout:
                ...
                if (LM[offset] > 95.0f || LM[offset] < 5.0f) {
                    contrast *= Cont0[unif];  //+ JD : luminance  pyramid to adjust
                                              //contrast by evaluation of LM[offset]
                ...
                // JD: modulation of microcontrast in function of original Luminance and
                // modulation of luminance
                if (tempL > LM[offset]) {
                    ...
                    luminance[j][i] =
                        intp(blend[j][i], luminance[j][i] * (temp * temp2 + 1.f),
                             luminance[j][i]);
                } else {
                    ...
                    if (temp4 > 1.0f) {
                        ...
                        luminance[j][i] =
                            intp(blend[j][i], luminance[j][i] / (temp * temp4 + 1.f),
                                 luminance[j][i]);
                    }
                }
            }
...
{
    if ((!params->sharpening.enabled) || params->sharpening.amount < 1 || ncie->W < 8
        || ncie->H < 8)
    {
        return;
    }
...
#pragma omp parallel for
    ...
    if (params->sharpening.method == "rld") {
        deconvsharpening(ncie->sh_p, b2, blend, ncie->W, ncie->H, params->sharpening,
                         scale);
        return;
    }
...
#pragma omp parallel
#endif
    {
        ...
        if (!params->sharpening.edgesonly) {
            ...
            bilateral<float, float>(ncie->sh_p, (float**)b3, b2, W, H,
                                    params->sharpening.edges_radius / scale,
                                    params->sharpening.edges_tolerance, multiThread);
        ...
        for (int i = 0; i < H; i++)
            for (int j = 0; j < W; j++) {
                ...
                if (ncie->J_p[i][j] > 8.0f && ncie->J_p[i][j] < 92.0f) {
                    ncie->sh_p[i][j] =
                        intp(blend[i][j], ncie->sh_p[i][j] + delta, ncie->sh_p[i][j]);
                }
            }
    } else {
