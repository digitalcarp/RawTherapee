...
public:
    ...
    typedef enum InterpolationDirection {
        ID_UP,   /// Interpolate color by increasing the hue value, crossing the upper
                 /// limit
        ID_DOWN  /// Interpolate color by decreasing the hue value, crossing the lower
                 /// limit
    ...
    // expressed as a single numerical value. The overall gamma is approximately 2.2,
    // consisting of a linear (gamma 1.0) section near black, and a non-linear section
    // elsewhere involving a 2.4 exponent and a gamma (slope of log output versus log
    // input) changing from 1.0 through about 2.3.
    ...
    constexpr static double u0 =
        4.0 * static_cast<double>(D50x)
        / (static_cast<double>(D50x) + 15.0 + 3.0 * static_cast<double>(D50z));
    constexpr static double v0 =
        9.0 / (static_cast<double>(D50x) + 15.0 + 3.0 * static_cast<double>(D50z));
     ...
     * The range of the r, g and b channel has no importance ([0 ; 1] or [0 ; 65535]...) ;
     * r,g,b can be negatives or > max, but must be in "sRGB"
     * @param r red channel
     * @param g green channel
     * @param b blue channel
     * @return luminance value
     */
    // xyz_sRGBD65 : conversion matrix from XYZ to sRGB for D65 illuminant: we use
    // diagonal values
    static float rgbLuminance(float r, float g, float b)
    {
        // WArning: The sum of xyz_sRGBd65[1][] is > 1.0 (i.e. 1.0000001), so we use our
        // own adapted values) 0.2126729,  0.7151521,  0.0721750
    ...
    {
        return static_cast<double>(r) * workingspace[1][0]
               + static_cast<double>(g) * workingspace[1][1]
               + static_cast<double>(b) * workingspace[1][2];
    }
     ...
     * @param workingSpace true: compute the Lab value using the Working color space ;
     * false: use the Output color space
     */
    // do not use this function in a loop. It really eats processing time caused by
    // Glib::ustring comparisons
    ...
    static inline void
    rgb2hslfloat(float r, float g, float b, float& h, float& s, float& l)
    {
    ...
    static inline void
    hsl2rgbfloat(float h, float s, float l, float& r, float& g, float& b)
    {
    ...
    static inline float
    rgb2s(float r, float g, float b)  // fast version if only saturation is needed
    {
     ...
     * Color space : undefined - use adhoc matrix: rgb_xyz[3][3] (iccmatrice.h) in
     * function of working space
     * @param x X coordinate [0 ; 1] or [0 ; 65535]
     * @param y Y coordinate [0 ; 1] or [0 ; 65535]
     * @param z Z coordinate [0 ; 1] or [0 ; 65535]
     * @param r red channel [same range than xyz channel] (return value)
    ...
    static inline void xyz2rgb(float x,
                               float y,
                               float z,
                               float& r,
                               float& g,
...
#ifdef __SSE2__
    static inline void xyz2rgb(vfloat x,
                               vfloat y,
                               vfloat z,
                               vfloat& r,
                               vfloat& g,
                               vfloat& b,
                               const vfloat rgb_xyz[3][3])
    {
        r = ((rgb_xyz[0][0] * x + rgb_xyz[0][1] * y + rgb_xyz[0][2] * z));
        g = ((rgb_xyz[1][0] * x + rgb_xyz[1][1] * y + rgb_xyz[1][2] * z));
        b = ((rgb_xyz[2][0] * x + rgb_xyz[2][1] * y + rgb_xyz[2][2] * z));
    ...
    /**
     * @brief Convert rgb in xyz
     * Color space : undefined - use adhoc matrix : xyz_rgb[3][3] (iccmatrice.h) in
     * function of working space
    ...
    static void rgbxyz(float r,
                       ...
                       const float xyz_rgb[3][3]);
#ifdef __SSE2__
    static void rgbxyz(vfloat r,
                       vfloat g,
                       vfloat b,
                       vfloat& x,
                       vfloat& y,
                       vfloat& z,
                       const vfloat xyz_rgb[3][3]);
#endif
...
#ifdef __SSE2__
    static inline void
    Lab2XYZ(vfloat L, vfloat a, vfloat b, vfloat& x, vfloat& y, vfloat& z)
    {
     ...
     * @param L L channel [0 ; 32768] ; L can be negative rarely or superior 32768 (return
     * value)
    ...
    static void RGB2Lab(float* X,
                        ...
                        const float wp[3][3],
                        int width);
    static void Lab2RGBLimit(float* L,
                             float* a,
                             float* b,
                             float* R,
                             float* G,
                             ...
                             float bfactor,
                             int width);
    static void RGB2L(const float* R,
                      const float* G,
                      const float* B,
                      float* L,
                      const float wp[3][3],
    ...
    /**
     * @brief Convert the 'a' and 'b' channels of the L*a*b color space to 'c' and 'h'
     * channels of the Lch color space (channel 'L' is identical [0 ; 32768])
     * @param a 'a' channel [-42000 ; +42000] ; can be more than 42000
     * @param b 'b' channel [-42000 ; +42000] ; can be more than 42000
     * @param c 'c' channel return value, in [0 ; 42000] ; can be more than 42000 (return
     * value)
    ...
    /**
     * @brief Convert 'c' and 'h' channels of the Lch color space to the 'a' and 'b'
     * channels of the L*a*b color space (channel 'L' is identical [0 ; 32768])
    ...
    /**
     * @brief Convert the 'u' and 'v' channels of the Luv color space to 'c' and 'h'
     * channels of the Lch color space ('L' channel is identical)
    ...
    /**
     * @brief Convert 'c' and 'h' channels of the Lch color space to the 'u' and 'v'
     * channels of the Luv color space ('L' channel is identical)
    ...
    /**
     * @brief Calculate the effective direction (up or down) to linearly interpolating 2
     * colors so that it follows the shortest or longest path
     ...
     */
    static inline eInterpolationDirection
    getHueInterpolationDirection(double h1, double h2, eInterpolationPath path)
    {
        if (path == IP_SHORTEST) {
            if (h2 > h1) {
                if (h2 - h1 <= 0.5) {
     ...
     * @param dir Tells which direction the interpolation have to follow. You can get the
     * value with getHueInterpolationDirection
     * @return The interpolated hue
     */
    static inline double
    interpolateHueHSV(double h1, double h2, double balance, eInterpolationDirection dir)
    {
        if (h1 == h2) {
            return h1;
        }
    ...
    /**
     * @brief Interpolate 2 colors from their respective red/green/blue channels, with a
     * balance factor
     ...
     * @param channels bitfield of channel to interpolate
     * (CHANNEL_LIGHTNESS|CHANNEL_CHROMATICITY|CHANNEL_HUE)
     ...
     */
    static void interpolateRGBColor(float balance,
                                    float r1,
                                    float g1,
                                    float b1,
                                    float r2,
    ...
    * @param bo blue channel of output color [0 ; 65535] (return value)
    */
    static void interpolateRGBColor(float realL,
                                    float iplow,
                                    float iphigh,
                                    int algm,
                                    float balance,
    ...
    /**
     * @brief Interpolate a hue value as the angle of a polar coordinate with hue in the
     * [0;1] range Chose the shorter path from hue 1 to hue 2.
     * @param h1 First hue [0; 1]
     * @param h2 Second hue  [0; 1]
    ...
    /**
     * @brief Interpolate a hue value as the angle of a polar coordinate with hue in the
     * [-PI ; +PI] range Chose the shorter path from hue 1 to hue 2.
    ...
    static inline T interpolatePolarHue_01(T h1, T h2, U balance)
    {
        ...
        if (d < T(-rtengine::RT_PI) || d < T(0) || d > T(rtengine::RT_PI))
        {  // there was an inversion here !! d > T(rtengine::RT_PI)
            h1 += T(2 * rtengine::RT_PI);
            h = h1 + f * (h2 - h1);
     ...
     *        gamma1 used in ip2Lab2rgb [0 ; 20], can be superior to 20, but it's quite
     * unusual(return value) gamma2 used in ip2Lab2rgb [0 ; 1], usually near 0.03(return
     * value) gamma3 used in ip2Lab2rgb [0 ; 1], usually near 0.003(return value) gamma4
     * used in ip2Lab2rgb [0 ; 1], usually near 0.03(return value) gamma5 used in
     * ip2Lab2rgb [0 ; 1], usually near 0.5 (return value)
     */
    static void calcGamma(double pwr, double ts, GammaValues& gamma);

    /**
     * @brief Used by Black and White to correct gamma for each channel red, green and
     * blue channel
     ...
     */
    static void trcGammaBW(float& r,
                           float& g,
                           float& b,
                           float gammabwr,
                           float gammabwg,
                           float gammabwb);
#ifdef __SSE2__
    static void trcGammaBWRow(float* r,
                              float* g,
                              float* b,
                              int width,
                              float gammabwr,
     ...
     * @param kcorec in absolute mode, value to correct the mixer [1 ; 3], usually near 1
     * (return value)
     ...
     */
    static void computeBWMixerConstants(const Glib::ustring& setting,
                                        const Glib::ustring& filter,
                                        const Glib::ustring& algo,
                                        float& filcor,
                                        float& mixerRed,
                                        float& mixerGreen,
                                        float& mixerBlue,
    ...
    static inline double gamma2(double x)  //  g3                  1+g4
    {
        ...
        // return x <= 0.0031308 ? x * 12.92310 : 1.055 * exp(log(x) / sRGBGammaCurve) -
        // 0.055;//standard discontinuous very small differences between the 2
        return x <= 0.003040
                   ? x * 12.92310
                   : 1.055 * exp(log(x) / sRGBGammaCurve) - 0.055;  // continuous
        //  return x <= 0.003041 ? x * 12.92310 : 1.055011 * exp(log(x) / sRGBGammaCurve)
        //  - 0.055011;//continuous
    }

    /**
     * @brief Inverse sRGB gamma
     * See also calcGamma above with the following values: pwr=2.3999  ts=12.92310
     * mode=0.003041  imax=0.055
     * @param x red, green or blue channel's value [0 ; 1]
    ...
    static inline double igamma2(double x)  // g2
    {
        ...
        // * 2.39990);//calculate with calcgamma return x <= 0.04045 ? x / 12.92310 :
        // exp(log((x + 0.055) / 1.055) * sRGBGammaCurve);//standard discontinuous
        // very small differences between the 4
        return x <= 0.039286
                   ? x / 12.92310
                   : exp(log((x + 0.055) / 1.055) * sRGBGammaCurve);  // continuous
        //  return x <= 0.039293 ? x / 12.92310 : exp(log((x + 0.055011) / 1.055011) *
    ...
    {
        return x <= 0.154449 ? x / 5.0
                             : exp(log((x + 0.478793) / 1.478793) * 4.1);  // 4 5
    }
    ...
    // gamma for Retinex
    static inline double
    gammareti(double x, double gamma, double start, double slope, double mul, double add)
    {
        return (x <= start ? x * slope : exp(log(x) / gamma) * mul - add);
    }
    static inline double
    igammareti(double x, double gamma, double start, double slope, double mul, double add)
    {
    ...
    // X range 0..1
    static inline double
    gamma(double x, double gamma, double start, double slope, double mul, double add)
    {
        return (x <= start ? x * slope : exp(log(x) / gamma) * mul - add);
    }
    ...
    static void gammaf2lut(LUTf& gammacurve,
                           float gamma,
                           float start,
                           float slope,
                           float divisor,
    ...
    static inline double
    igamma(double x, double gamma, double start, double slope, double mul, double add)
    {
        return (x <= start * slope ? x / slope : exp(log((x + add) / mul) * gamma));
    }
     ...
     * @return the gamma modified's value [0 ; 1]
     */
    static inline double gamman(double x, double gamma)  // standard gamma without
                                                         // slope...
    {
     ...
     * @return the inverse gamma modified's value [0 ; 1]
     */
    static inline double igamman(double x,
                                 double gamma)  // standard inverse gamma without slope...
    {
        return exp(log(x) * gamma);
    }
    ...
    template <class T>
    static void
    rgb2oklab(float R, float G, float B, float& L, float& a, float& b, const T ws[3][3])
    {
        float X, Y, Z;
    ...
    template <class T>
    static void
    oklab2rgb(float L, float a, float b, float& R, float& G, float& B, const T iws[3][3])
    {
        float X, Y, Z;
     ...
     *
     * @param lumaMuns true => luminance correction (for delta L > 10) and chroma
     * correction ; false => only chroma
     ...
     * @param correctionHueChroma hue correction depending on chromaticity (saturation),
     * in radians [0 ; 0.45] (return value)
     * @param correctlum hue correction depending on luminance (brightness, contrast,...),
     * in radians [0 ; 0.45] (return value)
     * @param munsDbgInfo (Debug target only) object to collect information
     */

    static void AllMunsellLch(bool lumaMuns,
                              float Lprov1,
                              float Loldd,
                              float HH,
                              float Chprov1,
                              ...
                              float& correctlum);
    static void AllMunsellLch(float Lprov1,
                              float HH,
                              float Chprov1,
                              float CC,
                              float& correctionHueChroma);
    ...
    /**
     * @brief Correct chromaticity and luminance so that the color stays in the working
     * profile's gamut
     *
     ...
     * @param Lprov1 : input luminance value, sent back corrected [0 ; 100]  (input &
     * output value)
     * @param Chprov1: input chroma value, sent back corrected [0 ; 180 (can be superior)]
     * (input & output value)
     * @param R red value of the corrected color [0 ; 65535 but can be negative or
     * superior to 65535] (return value)
     * @param G green value of the corrected color [0 ; 65535 but can be negative or
     * superior to 65535] (return value)
     * @param B blue value of the corrected color [0 ; 65535 but can be negative or
     * superior to 65535] (return value)
     ...
     *                  The nearest it is from 1.0, the more precise it will be, and the
     * longer too as more iteration will be necessary
     ...
     */
    static void gamutLchonly(float HH,
                             float& Lprov1,
                             float& Chprov1,
                             float& R,
                             float& G,
    ...
    static void gamutLchonly(float HH,
                             ...
                             float lowerCoef,
                             float higherCoef);
    static void gamutLchonly(float2 sincosval,
                             float& Lprov1,
                             float& Chprov1,
                             const float wip[3][3],
                             bool isHLEnabled,
     ...
     *
     * This function is the overall Munsell's corrections, but only on global statement.
     * It may be better to use local statement with AllMunsellLch. They are named
     * accordingly :  gamutLchonly and AllMunsellLch It can be used before and after
     * treatment (saturation, gamma, luminance, ...)
     *
     ...
     */
    static void LabGamutMunsell(float* labL,
                                float* laba,
                                float* labb,
                                int N,
                                bool corMunsell,
     ...
     * Skin colors: mixed from NX2 skin color palette, Von Luschan, and photos of white,
     * black, yellow people... There are some little exceptions, but it should cover 99%
     * case. Pay attention to white balance, and do not change hue and saturation,
     * upstream of the modification Used by vibrance
     ...
     */
    static void
    SkinSat(float lum, float hue, float chrom, float& satreduc);  // jacques Skin color
     ...
     * @param zone  [1 ; 4]  1=PB correction + sky  2=red yellow correction 3=Green yellow
     * correction  4=Red purple correction
     * @param correctL true=enable the Luminance correction
     */
    static void MunsellLch(float lum,
                           float hue,
                           float chrom,
                           float memChprov,
                           float& correction,
                           int zone,
                           float& lbe,
                           bool& correctL);  // jacques:  Munsell correction
    ...
    static void scalered(float rstprotection,
                         ...
                         float& scale,
                         float& scaleext);
    static void transitred(float HH,
                           float Chprov1,
                           float dred,
                           float factorskin,
                           float protect_red,
                           ...
                           float factorsat,
                           float& factor);
    static void skinredfloat(float J,
                             float h,
                             float sres,
                             float Sp,
                             float dred,
    ...
    static inline void
    pregamutlab(float lum,
                float hue,
                float& chr)  // big approximation to limit gamut (Prophoto) before good
                             // gamut procedure for locallab chroma, to avoid crash
    {
        if (lum >= 95.0f) {
            if (hue > 1.5f && hue < 2.f) {
                chr = 120.f;
    ...
    static inline void SkinSatCbdl(float lum,
                                   float hue,
                                   float chrom,
                                   float skinprot,
                                   float& scale,
                                   ...
                                   float t_r)
    {
        ...
        static const float C9 = 8.f, C8 = 15.f, C7 = 12.f, C4 = 7.f, C3 = 5.f, C2 = 5.f,
                           C1 = 5.f;
        static const float H9 = 0.05f, H8 = 0.25f, H7 = 0.1f, H4 = 0.02f, H3 = 0.02f,
                           H2 = 0.1f, H1 = 0.1f, H10 = -0.2f, H11 = -0.2f;

        // "real" skin color : take into account a slight usage of contrast and saturation
        // in RT if option "skin" = 1, uses implicit factor 1.0 wide area skin color,
        // useful if not accurate colorimetry or if the user has changed hue and
        // saturation, uses explicit factor 0.6 wide area for transition, uses explicit
        // factor 0.4

        if (lum >= 85.0f) {
            if ((hue > (t_l + 0.53f - H9) && hue < (t_r + H9))
                && (chrom > 8.0f && chrom < (14.0f + C9)))
            {
            ...
            } else if ((hue > t_l + 0.4f && hue < t_r - 0.3f)
                       && (chrom > 7.0f && chrom < (26.0f + C9)))
            {
                scale = (100.f - skinprot * 0.6f) / 100.1f;
            } else if ((hue > b_l + 0.05f && hue < t_r)
                       && (chrom > 7.0f && chrom < (35.0f + C9)))
            {
                scale = (100.f - skinprot * 0.4f) / 100.1f;
            }
        } else if (lum >= 70.0f) {
            if ((hue > t_l + 0.15f && hue < (t_r - 0.2f + H8))
                && (chrom > 8.0f && chrom < (35.0f + C8)))
            {
                scale = (100.f - skinprot) / 100.1f;
            } else if ((hue > (b_l + 0.07f + H11) && hue < t_r - 0.2f)
                       && (chrom > 7.0f && chrom < (48.0f + C9)))
            {
                scale = (100.f - skinprot * 0.6f) / 100.1f;
            } else if ((hue > (b_l + 0.07f + H11) && hue < t_r)
                       && (chrom > 7.0f && chrom < (55.0f + C9)))
            {
                scale = (100.f - skinprot * 0.4f) / 100.1f;
            }
        } else if (lum >= 52.0f) {
            if ((hue > t_l && hue < (t_r + H7))
                && (chrom > 11.0f && chrom < (35.0f + C7)))
            {
                scale = (100.f - skinprot) / 100.1f;
            } else if ((hue > (b_l + 0.07f + H11) && hue < t_r - 0.2f)
                       && (chrom > 7.0f && chrom < (48.0f + C9)))
            ...
            if ((hue > t_l && hue < (t_r + H4))
                && (chrom > 13.0f && chrom < (37.0f + C4)))
            {
                scale = (100.f - skinprot) / 100.1f;
            } else if ((hue > (b_l + 0.07f + H11) && hue < t_r - 0.2f)
                       && (chrom > 7.0f && chrom < (48.0f + C9)))
            ...
            if ((hue > (t_l - 0.25f + H10) && hue < (t_r - 0.3f + H2))
                && (chrom > 8.0f && chrom < (23.0f + C2)))
            {
                scale = (100.f - skinprot) / 100.1f;
            } else if ((hue > (b_l + 0.07f + H11) && hue < t_r - 0.2f)
                       && (chrom > 7.0f && chrom < (35.0f + C1)))
            {
                scale = (100.f - skinprot * 0.6f) / 100.1f;
            } else if ((hue > (b_l + 0.07f + H11) && hue < t_r - 0.1f)
                       && (chrom > 7.0f && chrom < (45.0f + C1)))
            {
                scale = (100.f - skinprot * 0.4f) / 100.1f;
            }
        } else if ((hue > (t_l - 0.2f + H10) && hue < (t_r - 0.3f + H1))
                   && (chrom > 8.0f && chrom < (23.0f + C1)))
        {
            scale = (100.f - skinprot) / 100.1f;
        } else if ((hue > (b_l + 0.07f + H11) && hue < t_r - 0.2f)
                   && (chrom > 7.0f && chrom < (35.0f + C1)))
    ...
    static inline void SkinSatCbdl2(float lum,
                                    float hue,
                                    float chrom,
                                    float skinprot,
                                    float& scale,
                                    ...
                                    int basc)
    {
        ...
        if ((b_l > -0.3f && b_r < 2.f) || basc == 0) {  // range maxi skin
            if (lum >= 85.0f) {
                    ...
                    && (chrom > 8.0f && chrom < (14.0f + C9)))
                {
                    ...
                    if ((hue > t_l + 0.4f && hue < t_r)
                        && (chrom > 7.0f && chrom < (15.0f)))
                    {
                        scale = (100.f - skinprot * 0.6f) / 100.1f;
                    } else if ((hue > b_l && hue < t_r)
                               && (chrom > 7.0f && chrom < (18.0f)))
                    {
                        scale = (100.f - skinprot * 0.4f) / 100.1f;
                    }
                } else if ((hue > t_l + 0.4f && hue < t_r - 0.3f)
                ...
                if ((hue > t_l && hue < (t_r + H3))
                    && (chrom > 7.0f && chrom < (35.0f + C3)))
                {
                    scale = (100.f - skinprot) / 100.1f;
                } else if ((hue > (b_l + 0.07f + H11) && hue < t_r - 0.2f)
                           && (chrom > 7.0f && chrom < (48.0f + C9)))
        ...
        {  // not hue skin  linear transition or mod chroma curve
            if (hue >= t_l && hue <= t_r) {
                scale = (100.f - skinprot) / 100.1f;
    ...
    static inline void SkinSatCbdlCam(float lum,
                                      float hue,
                                      float chrom,
                                      float skinprot,
                                      float& scale,
     ...
     */
    static void
    primaries_to_xyz(double p[6], double Wx, double Wz, double* pxyz, int cat);
    ...
    static void
    mult3(std::array<float, 3>& in, const Matrix& ma, std::array<float, 3>& out);
    // conversion RGB data with Agx matrix and inverse
    static void agx_trans(const std::array<float, 3>& rgb_in,
                          const Matrix& to_agx,
                          float& R,
                          float& G,
                          float& B);
    ...
    static void
    aces_reference_gamut_compression(const std::array<float, 3>& rgb_in,
                                     const std::array<float, 3>& threshold,
                                     const std::array<float, 3>& distance_limit,
                                     const Matrix& to_out,
                                     const Matrix& from_out,
                                     float pwr,
                                     bool rolloff,
                                     float& R,
                                     float& G,
    ...
    {
        // hr=translate Hue Lab value  (-Pi +Pi) in approximative hr (hsv values) (0 1)
        // [red 1/6 yellow 1/6 green 1/6 cyan 1/6 blue 1/6 magenta 1/6 ]
    ...
    static inline double huejz_to_huehsv2(float HH)
    {
        ...
        //  with multi linear correspondences (I expect another time with Jz there is no
        //  error !!)
        double hr = 0.0;
        // always put h between 0 and 1
        ...
        if (HH >= 0.2f && HH < 0.75f) {
            ...
            hr = 0.23442623 * double(HH)
                 - 0.18368853;  // hr 0.25  0.393    yellow green green
        } else if (HH >= 2.46f && HH < 3.14159f) {
            hr = 0.177526 * double(HH)
                 - 0.043714;  // hr 0.393  0.51315    green  ==> 0.42 Lab
        } else if (HH >= -3.14159f && HH < -2.89f) {
            hr = 0.3009078 * double(HH)
                 + 1.459329;  // hr 0.51315  0.5897    green cyan ==> -2.30 Lab
        } else if (HH >= -2.89f && HH < -2.7f) {
            hr = 0.204542 * double(HH) + 1.1808264;  // hr 0.5897  0.628563    cyan
        } else if (HH >= -2.7f && HH < -2.17f) {
            hr = 0.121547 * double(HH)
                 + 0.956399;  // hr 0.628563  0.692642    blue blue-sky
        } else if (HH >= -2.17f && HH < -0.9f) {
            hr = 0.044882 * double(HH)
                 + 0.789901;  // hr 0.692642  0.749563    blue blue-sky
        } else if (HH >= -0.9f && HH < -0.1f) {
            hr = 0.2125 * double(HH) + 0.940813;  // hr 0.749563 0.919563    purple
                                                  // magenta
    ...
    static inline void
    RGB2Y(const float* R, const float* G, const float* B, float* Y1, float* Y2, int W)
    {
        int i = 0;
#ifdef __SSE2__
