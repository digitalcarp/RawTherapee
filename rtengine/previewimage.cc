...
PreviewImage::PreviewImage(const Glib::ustring& fname,
                           ...
                           const PreviewImageMode mode)
{
    ...
    if (mode == PIM_EmbeddedPreviewOnly || mode == PIM_EmbeddedOrRaw) {
        ...
        if (ext.lowercase() == "jpg" || ext.lowercase() == "jpeg") {
            ...
            tpp = rtengine::Thumbnail::loadFromImage(fname, width, height, 1, 1.,
                                                     ColorTemp::DEFAULT_OBSERVER, true);
            ...
            tpp = rtengine::Thumbnail::loadQuickFromRaw(fname, sensorType, width, height,
                                                        1, true, true);
...
#pragma omp parallel for
    ...
    if ((mode == PIM_EmbeddedOrRaw && !tpp) || mode == PIM_ForceRaw) {
        ...
        if (!error) {
            ...
            params.raw.bayersensor.method = RAWParams::BayerSensor::getMethodString(
                RAWParams::BayerSensor::Method::FAST);
            params.raw.deadPixelFilter = false;
            params.raw.ca_autocorrect = false;
            params.raw.xtranssensor.method = RAWParams::XTransSensor::getMethodString(
                RAWParams::XTransSensor::Method::FAST);
            float reddeha = 0.f;
            float greendeha = 0.f;
            float bluedeha = 0.f;
            rawImage.preprocess(params.raw, params.lensProf, params.coarse, reddeha,
                                greendeha, bluedeha, true);
