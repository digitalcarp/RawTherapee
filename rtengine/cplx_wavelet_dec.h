      ...
      memoryAllocationFailed(false)
{
    ...
    if (wavfilt_len == 6) {
        ...
        for (int n = 0; n < 2; n++) {
            for (int i = 0; i < wavfilt_len; i++) {
                ...
                wavfilt_synth[wavfilt_len * (n) + i] =
                    Daub4_anal22[n][wavfilt_len - 1 - i];
                // n=0 lopass, n=1 hipass
            }
        }
    ...
    wavelet_decomp[lvltot] = new wavelet_level<internal_type>(
        src, buffer[bufferindex ^ 1], lvltot /*level*/, subsamp, m_w, m_h, wavfilt_anal,
        wavfilt_anal, wavfilt_len, wavfilt_offset, skipcrop, numThreads);
    ...
    while (lvltot < maxlvl - 1) {
        ...
        bufferindex ^= 1;
        wavelet_decomp[lvltot] = new wavelet_level<internal_type>(
            buffer[bufferindex], buffer[bufferindex ^ 1] /*lopass*/, lvltot /*level*/,
            subsamp, wavelet_decomp[lvltot - 1]->width(),
            wavelet_decomp[lvltot - 1]->height(), wavfilt_anal, wavfilt_anal, wavfilt_len,
            wavfilt_offset, skipcrop, numThreads);
...
template <typename E> void wavelet_decomposition::reconstruct(E* dst, const float blend)
{
    ...
    {  // bigBlockOfMemoryUsed means that wavcoeffs[2] points to a block of memory big
    ...
    wavelet_decomp[0]->reconstruct_level(tmpLo, tmpHi, coeff0, dst, wavfilt_synth,
                                         wavfilt_synth, wavfilt_len, wavfilt_offset,
                                         blend);
