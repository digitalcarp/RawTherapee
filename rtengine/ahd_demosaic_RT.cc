...
void RawImageSource::ahd_demosaic()
{
    ...
    if (plistener) {
        plistener->setProgressStr(
            Glib::ustring::compose(M("TP_RAW_DMETHOD_PROGRESSBAR"), M("TP_RAW_AHD")));
    ...
    for (int i = 0; i < 3; i++) {
        for (unsigned int j = 0; j < 3; j++) {
            xyz_cam[i][j] = 0;
            for (int k = 0; k < 3; k++) {
                xyz_cam[i][j] += xyz_rgb[i][k]
                                 * static_cast<float>(imatrices.rgb_cam[k][j])
                                 / d65_white[i];
            }
        }
    }
        ...
        for (int top = 2; top < height - 5; top += TS - 6) {
            for (int left = 2; left < width - 5; left += TS - 6) {
                ...
                for (int row = top; row < top + TS && row < height - 2; row++) {
                    for (int col = left + (fc(cfa, row, left) & 1);
                         col < std::min(left + TS, width - 2); col += 2)
                    {
                        auto pix = &rawData[row][col];
                        float val0 =
                            0.25f * ((pix[-1] + pix[0] + pix[1]) * 2 - pix[-2] - pix[2]);
                        rgb[0][row - top][col - left][1] = median(val0, pix[-1], pix[1]);
                        float val1 = 0.25f
                                     * ((pix[-width] + pix[0] + pix[width]) * 2
                                        - pix[-2 * width] - pix[2 * width]);
                        rgb[1][row - top][col - left][1] =
                            median(val1, pix[-width], pix[width]);
                    }
                }
                ...
                for (int d = 0; d < 2; d++)
                    for (int row = top + 1; row < top + TS - 1 && row < height - 3; row++)
                    {
                        ...
                        for (int col = left + 1; col < std::min(left + TS - 1, width - 3);
                             col++)
                        {
                            auto pix = &rawData[row][col];
                            ...
                            if (fc(cfa, row, col) == 1) {
                                rix[0][2 - cng] = CLIP(
                                    pix[0]
                                    + (0.5f
                                       * (pix[-1] + pix[1] - rix[-1][1] - rix[1][1])));
                                rix[0][cng] = CLIP(pix[0]
                                                   + (0.5f
                                                      * (pix[-width] + pix[width]
                                                         - rix[-TS][1] - rix[TS][1])));
                                rix[0][1] = pix[0];
                ...
                for (int row = top + 2; row < top + TS - 2 && row < height - 4; row++) {
                    ...
                    for (int col = left + 2, tc = 2;
                         col < left + TS - 2 && col < width - 4; col++, tc++)
                    {
                        for (int d = 0; d < 2; d++) {
                        ...
                        for (int d = 0; d < 2; d++) {
                            ...
                            for (int i = 0; i < 4; i++) {
                                homo[d][tr][tc] +=
                                    (ldiff[d][i] <= leps) * (abdiff[d][i] <= abeps);
                            }
                        }
                    }
                }
                ...
                for (int row = top + 3; row < top + TS - 3 && row < height - 5; row++) {
                    ...
                    for (int col = left + 3, tc = 3;
                         col < std::min(left + TS - 3, width - 5); col++, tc++)
                    {
                        ...
                        if (hm0 != hm1) {
                            ...
                            red[row][col] =
                                0.5f * (rgb[0][tr][tc][0] + rgb[1][tr][tc][0]);
                            green[row][col] =
                                0.5f * (rgb[0][tr][tc][1] + rgb[1][tr][tc][1]);
                            blue[row][col] =
                                0.5f * (rgb[0][tr][tc][2] + rgb[1][tr][tc][2]);
                        }
                    }
                }
