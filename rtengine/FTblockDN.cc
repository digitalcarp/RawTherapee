...
void do_median_denoise(float** src,
                       float** dst,
                       float upperBound,
                       int width,
                       int height,
                       ImProcFunctions::Median medianType,
                       int iterations,
                       int numThreads,
                       float** buffer)
{
        ...
        for (int i = border; i < height - border; ++i) {
            ...
            case Median::TYPE_3X3_SOFT:
            {
                for (; j < width - border; ++j) {
                    if (!useUpperBound || medianIn[i][j] <= upperBound) {
                        medianOut[i][j] =
                            median(medianIn[i - 1][j], medianIn[i][j - 1], medianIn[i][j],
                                   medianIn[i][j + 1], medianIn[i + 1][j]);
                    } else {
                        medianOut[i][j] = medianIn[i][j];
                    }
                }
                ...
                for (; j < width - border; ++j) {
                    if (!useUpperBound || medianIn[i][j] <= upperBound) {
                        medianOut[i][j] = median(
                            ...
                            medianIn[i - 1][j + 1], medianIn[i][j - 1], medianIn[i][j],
                            medianIn[i][j + 1], medianIn[i + 1][j - 1],
                    ...
                    if (!useUpperBound || medianIn[i][j] <= upperBound) {
                        medianOut[i][j] =
                            median(medianIn[i - 2][j], medianIn[i - 1][j - 1],
                                   medianIn[i - 1][j], medianIn[i - 1][j + 1],
                                   medianIn[i][j - 2], medianIn[i][j - 1], medianIn[i][j],
                                   medianIn[i][j + 1], medianIn[i][j + 2],
                                   medianIn[i + 1][j - 1], medianIn[i + 1][j],
                                   medianIn[i + 1][j + 1], medianIn[i + 2][j]);
                    } else {
                        medianOut[i][j] = medianIn[i][j];
                    }
                }
            ...
            case Median::TYPE_5X5_STRONG:
            {
                ...
                for (; !useUpperBound && j < width - border - 3; j += 4) {
                    STVFU(
                        medianOut[i][j],
                        median(LVFU(medianIn[i - 2][j - 2]), LVFU(medianIn[i - 2][j - 1]),
                               LVFU(medianIn[i - 2][j]), LVFU(medianIn[i - 2][j + 1]),
                               LVFU(medianIn[i - 2][j + 2]), LVFU(medianIn[i - 1][j - 2]),
                               LVFU(medianIn[i - 1][j - 1]), LVFU(medianIn[i - 1][j]),
                               LVFU(medianIn[i - 1][j + 1]), LVFU(medianIn[i - 1][j + 2]),
                               LVFU(medianIn[i][j - 2]), LVFU(medianIn[i][j - 1]),
                               LVFU(medianIn[i][j]), LVFU(medianIn[i][j + 1]),
                               LVFU(medianIn[i][j + 2]), LVFU(medianIn[i + 1][j - 2]),
                               LVFU(medianIn[i + 1][j - 1]), LVFU(medianIn[i + 1][j]),
                               LVFU(medianIn[i + 1][j + 1]), LVFU(medianIn[i + 1][j + 2]),
                               LVFU(medianIn[i + 2][j - 2]), LVFU(medianIn[i + 2][j - 1]),
                               LVFU(medianIn[i + 2][j]), LVFU(medianIn[i + 2][j + 1]),
                ...
                for (; j < width - border; ++j) {
                    if (!useUpperBound || medianIn[i][j] <= upperBound) {
                        medianOut[i][j] =
                            median(medianIn[i - 2][j - 2], medianIn[i - 2][j - 1],
                                   ...
                                   medianIn[i][j - 2], medianIn[i][j - 1], medianIn[i][j],
                                   medianIn[i][j + 1], medianIn[i][j + 2],
                                   medianIn[i + 1][j - 2], medianIn[i + 1][j - 1],
                                   medianIn[i + 1][j], medianIn[i + 1][j + 1],
                                   medianIn[i + 1][j + 2], medianIn[i + 2][j - 2],
                                   medianIn[i + 2][j - 1], medianIn[i + 2][j],
                                   medianIn[i + 2][j + 1], medianIn[i + 2][j + 2]);
                    } else {
                        medianOut[i][j] = medianIn[i][j];
                    }
                }
...
void ImProcFunctions::Median_Denoise(float** src,
                                     float** dst,
                                     const int width,
                                     const int height,
                                     const Median medianType,
                                     const int iterations,
                                     const int numThreads,
                                     float** buffer)
{
    do_median_denoise<false>(src, dst, 0.f, width, height, medianType, iterations,
                             numThreads, buffer);
}
...
void ImProcFunctions::Median_Denoise(float** src,
                                     ...
                                     float** buffer)
{
    do_median_denoise<true>(src, dst, upperBound, width, height, medianType, iterations,
                            numThreads, buffer);
}

void ImProcFunctions::Tile_calc(int tilesize,
                                int overlap,
                                int kall,
                                int imwidth,
                                int imheight,
...
void ImProcFunctions::RGB_denoise(int kall,
                                  Imagefloat* src,
                                  Imagefloat* dst,
                                  Imagefloat* calclum,
                                  float* ch_M,
                                  ...
                                  float* max_b,
                                  bool isRAW,
                                  const procparams::DirPyrDenoiseParams& dnparams,
                                  const double expcomp,
                                  const NoiseCurve& noiseLCurve,
                                  ...
                                  float& highresi)
{
    ...
    if (dnparams.luma == 0 && dnparams.chroma == 0 && !dnparams.median && !noiseLCurve
        && !noiseCCurve)
    {
    ...
    const nrquality nrQuality =
        (dnparams.smethod == "shal") ? QUALITY_STANDARD : QUALITY_HIGH;  // shrink method
    const float qhighFactor =
        (nrQuality == QUALITY_HIGH) ? 1.f / static_cast<float>(settings->nrhigh) : 1.0f;
    const bool useNoiseCCurve = (noiseCCurve && noiseCCurve.getSum() > 5.f);
    const bool useNoiseLCurve = (noiseLCurve && noiseLCurve.getSum() >= 7.f);
    const bool autoch =
        (settings->leveldnautsimpl == 1
         && (dnparams.Cmethod == "AUT" || dnparams.Cmethod == "PRE"))
        || (settings->leveldnautsimpl == 0
            && (dnparams.C2method == "AUTO" || dnparams.C2method == "PREV"));
    ...
    const float noisevarL =
        (useNoiseLCurve && (denoiseMethodRgb || !isRAW))
            ? SQR(((noiseluma + 1.f) / 125.f) * (10.f + (noiseluma + 1.f) / 25.f))
            : SQR((noiseluma / 125.f) * (1.f + noiseluma / 25.f));
...
#pragma omp parallel for schedule(dynamic, 16)
        ...
        for (int ii = 0; ii < hei; ++ii) {
            for (int jj = 0; jj < wid; ++jj) {
                ...
                if (useNoiseCCurve) {
                    ...
                    if (cN > 100) {
                        ccalc[ii][jj] =
                            SQR(1.f + ponderCC * (4.f * noiseCCurve[cN / 60.f]));
                    } else {
    ...
    if (dnparams.luma != 0 || dnparams.chroma != 0 || dnparams.methodmed == "Lab"
        || dnparams.methodmed == "Lonly")
    {
        ...
        const float gain =
            dnparams.autoGain ? static_cast<float>(std::pow(2.0, expcomp)) : 1.f;
        const double params_Ldetail = std::min(
            dnparams.Ldetail,
            99.9);  // max out to avoid div by zero when using noisevar_Ldetail as divisor
        const float noisevar_Ldetail = SQR(
            (SQR(100. - params_Ldetail) + 50.0 * (100.0 - params_Ldetail)) * TS * 0.5);
        ...
        if (denoiseLuminance) {
            ...
            for (int i = 0; i < TS; ++i) {
                ...
                float vmask =
                    (i1 < border ? SQR(sin((rtengine::RT_PI_F * i1) / (2 * border)))
                                 : 1.f);
                float vmask2 =
                    (i1 < 2 * border ? SQR(sin((rtengine::RT_PI_F * i1) / (2 * border)))
                                     : 1.f);
                ...
                for (int j = 0; j < TS; ++j) {
                    ...
                    tilemask_in[i][j] =
                        (vmask
                         * (j1 < border
                                ? SQR(sin((rtengine::RT_PI_F * j1) / (2 * border)))
                                : 1.0f))
                        + epsilon;
                    tilemask_out[i][j] =
                        (vmask2
        ...
        do {
            ...
            Tile_calc(tilesize, overlap,
                      (rgbDenoiseThreadLimit == 0 && !ponder) ? (numTries == 1 ? 0 : 2)
                                                              : 2,
                      imwidth, imheight, numtiles_W, numtiles_H, tilewidth, tileheight,
                      tileWskip, tileHskip);
...
#pragma omp parallel for
            ...
            // According to FFTW-Doc 'it is safe to execute the same plan in parallel by
            // multiple threads', so we now create 4 plans outside the parallel region and
            // use them inside the parallel region.

            // calculate max size of numblox_W.
            int max_numblox_W =
                ceil((static_cast<float>(MIN(imwidth, tilewidth))) / (offset))
                + 2 * blkrad;
            // calculate min size of numblox_W.
            int min_numblox_W =
                ceil((static_cast<float>(
                         (MIN(imwidth, ((numtiles_W - 1) * tileWskip) + tilewidth))
                         - ((numtiles_W - 1) * tileWskip)))
                     / (offset))
                + 2 * blkrad;

            // these are needed only for creation of the plans and will be freed before
            // entering the parallel loop
            ...
            if (denoiseLuminance) {
                ...
                // Creating the plans with FFTW_MEASURE instead of FFTW_ESTIMATE speeds up
                // the execute a bit
                plan_forward_blox[0] = fftwf_plan_many_r2r(
                    2, nfwd, max_numblox_W, Lbloxtmp, nullptr, 1, TS * TS, fLbloxtmp,
                    nullptr, 1, TS * TS, fwdkind, FFTW_MEASURE | FFTW_DESTROY_INPUT);
                plan_backward_blox[0] = fftwf_plan_many_r2r(
                    2, nfwd, max_numblox_W, fLbloxtmp, nullptr, 1, TS * TS, Lbloxtmp,
                    nullptr, 1, TS * TS, bwdkind, FFTW_MEASURE | FFTW_DESTROY_INPUT);
                plan_forward_blox[1] = fftwf_plan_many_r2r(
                    2, nfwd, min_numblox_W, Lbloxtmp, nullptr, 1, TS * TS, fLbloxtmp,
                    nullptr, 1, TS * TS, fwdkind, FFTW_MEASURE | FFTW_DESTROY_INPUT);
                plan_backward_blox[1] = fftwf_plan_many_r2r(
                    2, nfwd, min_numblox_W, fLbloxtmp, nullptr, 1, TS * TS, Lbloxtmp,
                    nullptr, 1, TS * TS, bwdkind, FFTW_MEASURE | FFTW_DESTROY_INPUT);
                fftwf_free(Lbloxtmp);
            ...
            if (settings->verbose) {
                printf(
                    "RGB_denoise uses %d main thread(s) and up to %d nested thread(s) for each main thread\n",
...
#pragma omp parallel num_threads(numthreads) if (numthreads > 1)
#endif
            {
                ...
                if (numtiles == 1 && isRAW && (useNoiseCCurve || useNoiseLCurve)) {
                    ...
                    noisevarlum =
                        new float[((tileheight + 1) / 2) * ((tilewidth + 1) / 2)];
                    noisevarchrom =
                        new float[((tileheight + 1) / 2) * ((tilewidth + 1) / 2)];
                }
...
#pragma omp for schedule(dynamic) collapse(2)
                ...
                for (int tiletop = 0; tiletop < imheight; tiletop += tileHskip) {
                    for (int tileleft = 0; tileleft < imwidth; tileleft += tileWskip) {
                        ...
                        if (isRAW)
                        {  // image is raw; use channel differences for chroma channels
    ...
    num_threads(denoiseNestedLevels) if (denoiseNestedLevels > 1)
                                ...
                                for (int i = tiletop; i < tilebottom; ++i) {
                                    ...
                                    for (int j = tileleft; j < tileright; ++j) {
                                        ...
                                        const float R_ =
                                            Color::denoiseIGammaTab[gain * src->r(i, j)];
                                        const float G_ =
                                            Color::denoiseIGammaTab[gain * src->g(i, j)];
                                        const float B_ =
                                            Color::denoiseIGammaTab[gain * src->b(i, j)];

                                        // apply gamma noise standard (slider)
                                        labdn->L[i1][j1] =
                                            R_ < 65535.f
                                                ? gamcurve[R_]
                                                : Color::gammanf(R_ / 65535.f, gam)
                                                      * 32768.f;
                                        labdn->a[i1][j1] =
                                            G_ < 65535.f
                                                ? gamcurve[G_]
                                                : Color::gammanf(G_ / 65535.f, gam)
                                                      * 32768.f;
                                        labdn->b[i1][j1] =
                                            B_ < 65535.f
                                                ? gamcurve[B_]
                                                : Color::gammanf(B_ / 65535.f, gam)
                                                      * 32768.f;

                                        if (((i1 | j1) & 1) == 0) {
                                            noisevarlum[(i1 >> 1) * width2 + (j1 >> 1)] =
                                                useNoiseLCurve ? lumcalc[i >> 1][j >> 1]
                                                               : noisevarL;
                                            noisevarchrom[(i1 >> 1) * width2
                                                          + (j1 >> 1)] =
                                                useNoiseCCurve
                                                    ? maxNoiseVarab
                                                          * ccalc[i >> 1][j >> 1]
                                    ...
                                    Color::RGB2Lab(labdn->L[i1], labdn->a[i1],
                                                   labdn->b[i1], labdn->L[i1],
                                                   labdn->a[i1], labdn->b[i1], wpfast,
                                                   width);
                                }
                            } else {  // RGB mode
#ifdef _OPENMP
#pragma omp parallel for num_threads(denoiseNestedLevels) if (denoiseNestedLevels > 1)
                                ...
                                for (int i = tiletop; i < tilebottom; ++i) {
                                    ...
                                    for (int j = tileleft; j < tileright; ++j) {
                                        ...
                                        // conversion colorspace to determine luminance
                                        // with no gamma
                                        X = X < 65535.f
                                                ? gamcurve[X]
...
#pragma omp parallel for num_threads(denoiseNestedLevels) if (denoiseNestedLevels > 1)
                            ...
                            for (int i = tiletop; i < tilebottom; ++i) {
                                ...
                                for (int j = tileleft; j < tileright; ++j) {
                                    ...
                                    if (((i1 | j1) & 1) == 0) {
                                        ...
                                        if (useNoiseLCurve || useNoiseCCurve) {
                                            float XL, YL, ZL;
                                            Color::rgbxyz(rLum, gLum, bLum, XL, YL, ZL,
                                                          wp);
                                        ...
                                        if (useNoiseLCurve) {
                                            ...
                                            float kinterm =
                                                epsi + noiseLCurve[xdivf(kN, 15) * 500.f];
                                            float ki = kinterm * 100.f;
                                            ki += noiseluma;
                                            noisevarlum[(i1 >> 1) * width2 + (j1 >> 1)] =
                                                SQR((ki / 125.f) * (1.f + ki / 25.f));
                                        } else {
                                            noisevarlum[(i1 >> 1) * width2 + (j1 >> 1)] =
                                                noisevarL;
                                        }
                                        ...
                                        if (useNoiseCCurve) {
                                            ...
                                            float Cinterm =
                                                1.f
                                                + ponderCC * 4.f * noiseCCurve[cN / 60.f];
                                            noisevarchrom[(i1 >> 1) * width2
                                                          + (j1 >> 1)] =
                                                maxNoiseVarab * SQR(Cinterm);
                                        } else {
                                            noisevarchrom[(i1 >> 1) * width2
                                                          + (j1 >> 1)] = 1.f;
                                        }
                        ...
                        // arguments 4 and 5 of wavelet decomposition are max number of
                        // wavelet decomposition levels; and whether to subsample the
                        // image after wavelet filtering.  Subsampling is coded as binary
                        // 1 or 0 for each level, eg subsampling = 0 means no subsampling,
                        // 1 means subsample the first level only, 7 means subsample the
                        // first three levels, etc. actual implementation only works with
                        // subsampling set to 1
                        float interm_medT = dnparams.chroma / 10.0;
                        bool execwavelet = true;

                        if (!denoiseLuminance && interm_medT < 0.05f && dnparams.median
                            && (dnparams.methodmed == "Lab"
                                || dnparams.methodmed == "Lonly"))
                        {
                            execwavelet =
                                false;  // do not exec wavelet if sliders luminance and
                                        // chroma are very small and median need
                        }
                        ...
                        if (execwavelet) {  // gain time if user choose only median
                                            // sliders L <=1  slider chrom master < 1
                            int levwav = 5;
                            float maxreal = max(realred, realblue);

                            // increase the level of wavelet if user increase much or very
                            // much sliders
                            if (maxreal < 8.f) {
                                levwav = 5;
                                ...
                                levwav = 8;  // maximum ==> I have increase Maxlevel in
                                             // cplx_wavelet_dec.h from 8 to 9
                            }

                            if (nrQuality == QUALITY_HIGH) {
                                levwav +=
                                    settings
                                        ->nrwavlevel;  // increase level for enhanced mode
                            }
                            ...
                            //  if (settings->verbose) printf("levwavelet=%i  noisevarA=%f
                            //  noisevarB=%f \n",levwav, noisevarab_r, noisevarab_b);
                            const std::unique_ptr<wavelet_decomposition> Ldecomp(
                                new wavelet_decomposition(labdn->L[0], labdn->W, labdn->H,
                                                          levwav, 1, 1,
                                                          max(1, denoiseNestedLevels)));
                            ...
                            if (!memoryAllocationFailed) {
                                // precalculate madL, because it's used in adecomp and
                                // bdecomp
    ...
    num_threads(denoiseNestedLevels) if (denoiseNestedLevels > 1)
                                ...
                                for (int lvl = 0; lvl < maxlvl; ++lvl) {
                                    for (int dir = 1; dir < 4; ++dir) {
                                        // compute median absolute deviation (MAD) of
                                        // detail coefficients as robust noise estimator
                                        ...
                                        if (!denoiseMethodRgb) {
                                            madL[lvl][dir - 1] = SQR(
                                                Mad(WavCoeffs_L[dir], Wlvl_L * Hlvl_L));
                                        } else {
                                            madL[lvl][dir - 1] = SQR(MadRgb(
                                                WavCoeffs_L[dir], Wlvl_L * Hlvl_L));
                                        }
                                    }
                                }
                            }
                            ...
                            std::unique_ptr<wavelet_decomposition> adecomp(
                                new wavelet_decomposition(labdn->a[0], labdn->W, labdn->H,
                                                          levwav, 1, 1,
                                                          max(1, denoiseNestedLevels)));
                            ...
                            if (!memoryAllocationFailed) {
                                if (nrQuality == QUALITY_STANDARD) {
                                    if (!WaveletDenoiseAllAB(
                                            *Ldecomp, *adecomp, noisevarchrom, madL,
                                            nullptr, 0, noisevarab_r, useNoiseCCurve,
                                            autoch, denoiseMethodRgb,
                                            denoiseNestedLevels))
                                    {  // enhance mode
                                        memoryAllocationFailed = true;
                                    }
                                } else { /*if (nrQuality==QUALITY_HIGH)*/
                                    if (!WaveletDenoiseAll_BiShrinkAB(
                                            *Ldecomp, *adecomp, noisevarchrom, madL,
                                            nullptr, 0, noisevarab_r, useNoiseCCurve,
                                            autoch, denoiseMethodRgb,
                                            denoiseNestedLevels))
                            ...
                            if (!memoryAllocationFailed) {
                                std::unique_ptr<wavelet_decomposition> bdecomp(
                                    new wavelet_decomposition(
                                        labdn->b[0], labdn->W, labdn->H, levwav, 1, 1,
                                        max(1, denoiseNestedLevels)));
                                    ...
                                    if (nrQuality == QUALITY_STANDARD) {
                                        if (!WaveletDenoiseAllAB(
                                                *Ldecomp, *bdecomp, noisevarchrom, madL,
                                                nullptr, 0, noisevarab_b, useNoiseCCurve,
                                                autoch, denoiseMethodRgb,
                                                denoiseNestedLevels))
                                        {  // enhance mode
                                            memoryAllocationFailed = true;
                                        ...
                                        if (!memoryAllocationFailed) {
                                            if (!WaveletDenoiseAllAB(
                                                    *Ldecomp, *bdecomp, noisevarchrom,
                                                    madL, nullptr, 0, noisevarab_b,
                                                    useNoiseCCurve, autoch,
                                                    denoiseMethodRgb,
                                                    denoiseNestedLevels))
                                            {
                                                memoryAllocationFailed = true;
                                ...
                                if (!memoryAllocationFailed) {
                                    if (denoiseLuminance) {
                                        ...
                                        if (nrQuality == QUALITY_STANDARD) {
                                            if (!WaveletDenoiseAllL(*Ldecomp, noisevarlum,
                                                                    madL, nullptr, edge,
                                                                    denoiseNestedLevels))
                                            {  // enhance mode
                                                memoryAllocationFailed = true;
                                            ...
                                            if (!WaveletDenoiseAll_BiShrinkL(
                                                    *Ldecomp, noisevarlum, madL, nullptr,
                                                    edge, denoiseNestedLevels))
                                            {  // enhance mode
                                                memoryAllocationFailed = true;
                                            }

                                            if (!memoryAllocationFailed) {
                                                if (!WaveletDenoiseAllL(
                                                        *Ldecomp, noisevarlum, madL,
                                                        nullptr, edge,
                                                        denoiseNestedLevels))
                                                {
                                                    memoryAllocationFailed = true;
                                        ...
                                        if (!memoryAllocationFailed) {
                                            // copy labdn->L to Lin before it gets
                                            // modified by reconstruction
...
#pragma omp parallel for num_threads(denoiseNestedLevels) if (denoiseNestedLevels > 1)
                        ...
                        if (!memoryAllocationFailed) {
                            ...
                            const int numblox_H =
                                ceil((static_cast<float>(height)) / (offset))
                                + 2 * blkrad;
                            ...
                            if (denoiseLuminance /*&& execwavelet*/) {
                                ...
                                array2D<float> totwt(
                                    width, height,
                                    ARRAY2D_CLEAR_DATA);  // weight for combining DCT
                                                          // blocks

                                if (numtiles == 1) {
                                    for (int i = 0; i < denoiseNestedLevels * numthreads;
                                         ++i)
                                    {
                                        LbloxArray[i] =
                                            reinterpret_cast<float*>(fftwf_malloc(
                                                max_numblox_W * TS * TS * sizeof(float)));
                                        fLbloxArray[i] =
                                            reinterpret_cast<float*>(fftwf_malloc(
                                                max_numblox_W * TS * TS * sizeof(float)));
                                    }
...
#pragma omp parallel num_threads(denoiseNestedLevels) if (denoiseNestedLevels > 1)
                                ...
                                {
#ifdef _OPENMP
                                    int subThread = masterThread * denoiseNestedLevels
                                                    + omp_get_thread_num();
#else
                                    int subThread = 0;
#endif
                                    ...
                                    float
                                        pBuf[width + TS + 2 * blkrad * offset] ALIGNED16;
...
#pragma omp for
                                    ...
                                    for (int vblk = 0; vblk < numblox_H; ++vblk) {
                                        ...
                                        for (int i = 0; i < TS; ++i) {
                                            ...
                                            for (int j = width;
                                                 j < width + TS + blkrad * offset; ++j)
                                            {
                                                datarow[j] =
                                                    datarow[MAX(0, 2 * width - 2 - j)];
                                            }  // now we have a padded data row

                                            // now fill this row of the blocks with Lab
                                            // high pass data
                                            for (int hblk = 0; hblk < numblox_W; ++hblk) {
                                                int left = (hblk - blkrad) * offset;
                                                int indx = (hblk)*TS;  // index of block
                                                                       // in malloc
                                        ...
                                        if (numblox_W == max_numblox_W) {
                                            fftwf_execute_r2r(
                                                plan_forward_blox[0], Lblox,
                                                fLblox);  // DCT an entire row of tiles
                                        } else {
                                            fftwf_execute_r2r(
                                                plan_forward_blox[1], Lblox,
                                                fLblox);  // DCT an entire row of tiles
                                        }

                                        //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                        // now process the vblk row of blocks for noise
                                        // reduction

                                        for (int hblk = 0; hblk < numblox_W; ++hblk) {
                                            RGBtile_denoise(fLblox, hblk,
                                                            noisevar_Ldetail);
                                            //  RGBtile_denoise(fLblox, hblk,
                                            //  noisevar_Ldetail, nbrwt, blurbuffer);
                                        }  // end of horizontal block loop

                                        //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

                                        // now perform inverse FT of an entire row of
                                        // blocks
                                        if (numblox_W == max_numblox_W) {
                                            fftwf_execute_r2r(plan_backward_blox[0],
                                                              fLblox, Lblox);  // for DCT
                                        } else {
                                            fftwf_execute_r2r(plan_backward_blox[1],
                                                              fLblox, Lblox);  // for DCT
                                        }
                                        ...
                                        RGBoutput_tile_row(Lblox, Ldetail, tilemask_out,
                                                           height, width, topproc);
...
#pragma omp parallel for num_threads(denoiseNestedLevels) if (denoiseNestedLevels > 1)
                                ...
                                for (int i = 0; i < height; ++i) {
                                    for (int j = 0; j < width; ++j) {
                                        // may want to include masking threshold for large
                                        // hipass data to preserve edges/detail
                                        labdn->L[i][j] +=
                                            Ldetail[i][j]
                                            / totwt[i][j];  // note that labdn initially
                                                            // stores the denoised hipass
                                                            // data
                                    }
                                }
                            }

                            if ((metchoice == 1 || metchoice == 2 || metchoice == 3
                                 || metchoice == 4)
                                && dnparams.median)
                            {
                                float** tmL;
                                ...
                                if (dnparams.medmethod == "soft") {
                                    if (metchoice != 4) {
                                        medianTypeL = medianTypeAB =
                                            Median::TYPE_3X3_SOFT;
                                    } else {
                                ...
                                if (metchoice == 1 || metchoice == 2 || metchoice == 4) {
                                    Median_Denoise(labdn->L, labdn->L, wid, hei,
                                                   medianTypeL, dnparams.passes,
                                                   denoiseNestedLevels, tmL);
                                }

                                if (metchoice == 2 || metchoice == 3 || metchoice == 4) {
                                    Median_Denoise(labdn->a, labdn->a, wid, hei,
                                                   medianTypeAB, dnparams.passes,
                                                   denoiseNestedLevels, tmL);
                                    Median_Denoise(labdn->b, labdn->b, wid, hei,
                                                   medianTypeAB, dnparams.passes,
                                                   denoiseNestedLevels, tmL);
                                }
                            ...
                            if (numtiles > 1) {
                                ...
                                for (int i = 0; i < overlap; ++i) {
                                    float mask =
                                        SQR(xsinf((rtengine::RT_PI * i) / (2 * overlap)));
...
#pragma omp parallel for schedule(dynamic, 16) num_threads(denoiseNestedLevels)
                                    ...
                                    for (int i = tiletop; i < tilebottom; ++i) {
                                        ...
                                        Color::Lab2RGBLimit(
                                            labdn->L[i1], labdn->a[i1], labdn->b[i1],
                                            labdn->L[i1], labdn->a[i1], labdn->b[i1], wip,
                                            9000000.f, 1.f + qhighFactor * realred,
                                            1.f + qhighFactor * realblue, width);
                                        for (int j = tileleft; j < tileright; ++j) {
                                            int j1 = j - tileleft;
                                            ...
                                            r_ = r_ < 32768.f
                                                     ? igamcurve[r_]
                                                     : (Color::gammanf(r_ / 32768.f, igam)
                                                        * 65535.f);
                                            g_ = g_ < 32768.f
                                                     ? igamcurve[g_]
                                                     : (Color::gammanf(g_ / 32768.f, igam)
                                                        * 65535.f);
                                            b_ = b_ < 32768.f
                                                     ? igamcurve[b_]
                                                     : (Color::gammanf(b_ / 32768.f, igam)
                                                        * 65535.f);

                                            // readapt arbitrary gamma (inverse from
                                            // beginning)
...
#pragma omp parallel for num_threads(denoiseNestedLevels)
                                    ...
                                    for (int i = tiletop; i < tilebottom; ++i) {
                                        ...
                                        for (int j = tileleft; j < tileright; ++j) {
                                            int j1 = j - tileleft;
                                            float c_h = sqrt(SQR(labdn->a[i1][j1])
                                                             + SQR(labdn->b[i1][j1]));

                                            if (c_h > 3000.f) {
                                                labdn->a[i1][j1] *=
                                                    1.f + qhighFactor * realred / 100.f;
                                                labdn->b[i1][j1] *=
                                                    1.f + qhighFactor * realblue / 100.f;
                                            }
                                            ...
                                            X = X < 32768.f
                                                    ? igamcurve[X]
                                                    : (Color::gammaf(X / 32768.f, igam,
                                                                     igamthresh,
                                                                     igamslope)
                                                       * 65535.f);
                                            Y = Y < 32768.f
                                                    ? igamcurve[Y]
                                                    : (Color::gammaf(Y / 32768.f, igam,
                                                                     igamthresh,
                                                                     igamslope)
                                                       * 65535.f);
                                            Z = Z < 32768.f
                                                    ? igamcurve[Z]
                                                    : (Color::gammaf(Z / 32768.f, igam,
                                                                     igamthresh,
                                                                     igamslope)
                                                       * 65535.f);
...
#pragma omp parallel for
        ...
        } while (memoryAllocationFailed && numTries < 2 && (rgbDenoiseThreadLimit == 0)
                 && !ponder);

        if (memoryAllocationFailed) {
            printf(
                "tiled denoise failed due to isufficient memory. Output is not denoised!\n");
        }
    }
                    ...
                    for (int i = 1; i < hei - 1; ++i) {
                        if (methmed == 0) {
                            for (int j = 1; j < wid - 1; ++j) {
                                tm[i][j] =
                                    median(source->r(i, j), source->r(i - 1, j),
                                           source->r(i + 1, j), source->r(i, j + 1),
                                           source->r(i, j - 1));  // 3x3 soft
                            }
                        } else {
                            for (int j = 1; j < wid - 1; ++j) {
                                tm[i][j] = median(
                                    source->r(i, j), source->r(i - 1, j),
                                    source->r(i + 1, j), source->r(i, j + 1),
                                    source->r(i, j - 1), source->r(i - 1, j - 1),
                                    source->r(i - 1, j + 1), source->r(i + 1, j - 1),
                                    source->r(i + 1, j + 1));  // 3x3
                            }
                        }
                    }
                } else {
                        ...
                        if (methmed == 3) {
                            for (int j = 2; j < wid - 2; ++j) {
                                tm[i][j] = median(
                                    ...
                                    source->r(i, j - 1), source->r(i - 1, j - 1),
                                    source->r(i - 1, j + 1), source->r(i + 1, j - 1),
                                    source->r(i + 1, j + 1), source->r(i - 2, j),
                                    source->r(i + 2, j), source->r(i, j + 2),
                                    source->r(i, j - 2), source->r(i - 2, j - 2),
                                    source->r(i - 2, j + 2), source->r(i + 2, j - 2),
                                    source->r(i + 2, j + 2), source->r(i - 2, j + 1),
                                    source->r(i + 2, j + 1), source->r(i - 1, j + 2),
                                    source->r(i - 1, j - 2), source->r(i - 2, j - 1),
                                    source->r(i + 2, j - 1), source->r(i + 1, j + 2),
                                    source->r(i + 1, j - 2));  // 5x5
                            }
                        } else {
                            for (int j = 2; j < wid - 2; ++j) {
                                tm[i][j] = median(
...
#pragma omp for
                    ...
                    for (int i = 1; i < hei - 1; ++i) {
                        if (methmed == 0) {
                            for (int j = 1; j < wid - 1; ++j) {
                                tm[i][j] =
                                    median(source->b(i, j), source->b(i - 1, j),
                                           source->b(i + 1, j), source->b(i, j + 1),
                                           source->b(i, j - 1));
                            }
                        } else {
                            for (int j = 1; j < wid - 1; ++j) {
                                tm[i][j] = median(
                                    source->b(i, j), source->b(i - 1, j),
                                    source->b(i + 1, j), source->b(i, j + 1),
                                    source->b(i, j - 1), source->b(i - 1, j - 1),
                                    source->b(i - 1, j + 1), source->b(i + 1, j - 1),
                                    source->b(i + 1, j + 1));
                            }
                        }
                    }
                        ...
                        if (methmed == 3) {
                            for (int j = 2; j < wid - 2; ++j) {
                                tm[i][j] = median(
                                    ...
                                    source->b(i, j - 1), source->b(i - 1, j - 1),
                                    source->b(i - 1, j + 1), source->b(i + 1, j - 1),
                                    source->b(i + 1, j + 1), source->b(i - 2, j),
                                    source->b(i + 2, j), source->b(i, j + 2),
                                    source->b(i, j - 2), source->b(i - 2, j - 2),
                                    source->b(i - 2, j + 2), source->b(i + 2, j - 2),
                                    source->b(i + 2, j + 2), source->b(i - 2, j + 1),
                                    source->b(i + 2, j + 1), source->b(i - 1, j + 2),
                                    source->b(i - 1, j - 2), source->b(i - 2, j - 1),
                                    source->b(i + 2, j - 1), source->b(i + 1, j + 2),
                                    source->b(i + 1, j - 2));  // 5x5
                            }
                        } else {
                            for (int j = 2; j < wid - 2; ++j) {
                                tm[i][j] = median(
...
#pragma omp for
                    ...
                    for (int i = 1; i < hei - 1; ++i) {
                        if (methmed == 0) {
                            for (int j = 1; j < wid - 1; ++j) {
                                tm[i][j] =
                                    median(source->g(i, j), source->g(i - 1, j),
                                           source->g(i + 1, j), source->g(i, j + 1),
                                           source->g(i, j - 1));
                            }
                        } else {
                            for (int j = 1; j < wid - 1; ++j) {
                                tm[i][j] = median(
                                    source->g(i, j), source->g(i - 1, j),
                                    source->g(i + 1, j), source->g(i, j + 1),
                                    source->g(i, j - 1), source->g(i - 1, j - 1),
                                    source->g(i - 1, j + 1), source->g(i + 1, j - 1),
                                    source->g(i + 1, j + 1));
                            }
                        }
                    }
                        ...
                        if (methmed == 3) {
                            for (int j = 2; j < wid - 2; ++j) {
                                tm[i][j] = median(
                                    ...
                                    source->g(i, j - 1), source->g(i - 1, j - 1),
                                    source->g(i - 1, j + 1), source->g(i + 1, j - 1),
                                    source->g(i + 1, j + 1), source->g(i - 2, j),
                                    source->g(i + 2, j), source->g(i, j + 2),
                                    source->g(i, j - 2), source->g(i - 2, j - 2),
                                    source->g(i - 2, j + 2), source->g(i + 2, j - 2),
                                    source->g(i + 2, j + 2), source->g(i - 2, j + 1),
                                    source->g(i + 2, j + 1), source->g(i - 1, j + 2),
                                    source->g(i - 1, j - 2), source->g(i - 2, j - 1),
                                    source->g(i + 2, j - 1), source->g(i + 1, j + 2),
                                    source->g(i + 1, j - 2));  // 5x5
                            }
                        } else {
                            for (int j = 2; j < wid - 2; ++j) {
                                tm[i][j] = median(
...
// void ImProcFunctions::RGBtile_denoise(float * fLblox, int hblproc, float
// noisevar_Ldetail, float * nbrwt, float * blurbuffer)  //for DCT
void ImProcFunctions::RGBtile_denoise(float* fLblox,
                                      int hblproc,
                                      float noisevar_Ldetail)  // for DCT
{
    float nbrwt[TS * TS] ALIGNED64;
    const int blkstart = hblproc * TS * TS;

    boxabsblur(fLblox + blkstart, nbrwt, 3, TS, TS,
               false);  // blur neighbor weights for more robust estimation //for DCT
...
void ImProcFunctions::RGBoutput_tile_row(float* bloxrow_L,
                                         float** Ldetail,
                                         float** tilemask_out,
                                         int height,
                                         int width,
...
float ImProcFunctions::Mad(const float* DataList, const int datalen)
{
    ...
    // interpolate
    return ((lmedian - 1) + (datalen / 2 - count_) / (static_cast<float>(count - count_)))
           / 0.6745f;
}
...
void ImProcFunctions::Noise_residualAB(const wavelet_decomposition& WaveletCoeffs_ab,
                                       float& chresid,
                                       float& chmaxresid,
...
// end = maxlev;
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic) collapse(2) reduction(+ : resid) \
    reduction(max : maxresid)                                               \
    num_threads(denoiseNestedLevels) if (denoiseNestedLevels > 1)
#endif
    //   for (int lvl = 0; lvl < WaveletCoeffs_ab.maxlevel(); ++lvl) {
    for (int lvl = beg; lvl < end; ++lvl) {
        // compute median absolute deviation (MAD) of detail coefficients as robust noise
        // estimator
        for (int dir = 1; dir < 4; ++dir) {
            ...
            const float madC =
                SQR(denoiseMethodRgb ? MadRgb(WavCoeffs_ab[dir], Wlvl_ab * Hlvl_ab)
                                     : Mad(WavCoeffs_ab[dir], Wlvl_ab * Hlvl_ab));
...
bool ImProcFunctions::WaveletDenoiseAll_BiShrinkL(wavelet_decomposition& WaveletCoeffs_L,
                                                  float* noisevarlum,
                                                  float madL[8][3],
                                                  float* vari,
                                                  int edge,
...
#pragma omp for schedule(dynamic) collapse(2)
            ...
            {  // for levels less than max, use level diff to make edge mask
                for (int dir = 1; dir < 4; ++dir) {
                    ...
                    if (lvl == maxlvl - 1) {
                        //  int edge = 0;
                        ShrinkAllL(WaveletCoeffs_L, buffer, lvl, dir, noisevarlum,
                                   madL[lvl], vari, edge);
                    } else {
                        ...
                        /*
                           ...
                           && vari) { noisevarlum = blurBuffer;       // we need one
                           buffer, but fortunately we don't have to allocate a new one
                           because we can use blurBuffer

                                                    for (int i = 0; i < Wlvl_L * Hlvl_L;
                           ++i) { noisevarlum[i] = vari[lvl];
                                                    }
                                                }
                                                */
                        float* nvl = nullptr;
                        ...
                        if ((edge == 1 || edge == 2 || edge == 3 || edge == 5
                             || edge == 6)
                            && vari)
                        {
                            //  nvl = blurBuffer;       // we need one buffer, but
                            //  fortunately we don't have to allocate a new one because we
                            //  can use blurBuffer
                            if ((edge == 1 || edge == 3)) {
                                for (int i = 0; i < Hlvl_L * Wlvl_L; ++i) {
                                    nvl[i] = vari[lvl];
                             ...
                             coeffloc_L += 4)
                        {
                            ...
                            STVFU(sfave[coeffloc_L],
                                  mag_Lv
                                      / (mag_Lv
                                         + mad_Lv * xexpf(-mag_Lv / (mad_Lv * ninev))
                                         + epsv));
                        }
                        ...
                        for (; coeffloc_L < Hlvl_L * Wlvl_L; ++coeffloc_L) {
                            ...
                            sfave[coeffloc_L] =
                                mag_L
                                / (mag_L
                                   + levelFactor * nvl[coeffloc_L]
                                         * xexpf(-mag_L
                                                 / (9.f * levelFactor * nvl[coeffloc_L]))
                                   + eps);
                        }
...
#endif
                        boxblur(
                            sfave, sfaved, lvl + 2, Wlvl_L, Hlvl_L,
                            false);  // increase smoothness by locally averaging shrinkage
                             ...
                             coeffloc_L += 4)
                        {
                            ...
                            STVFU(WavCoeffs_L[dir][coeffloc_L],
                        ...
                        for (int i = 0; i < Hlvl_L; ++i) {
                            for (int j = 0; j < Wlvl_L; ++j) {
                                int coeffloc_L = i * Wlvl_L + j;
                                float sf_L = sfave[coeffloc_L];
                                // use smoothed shrinkage unless local shrinkage is much
                                // less
                                WavCoeffs_L[dir][coeffloc_L] *=
                                    (SQR(sfaved[coeffloc_L]) + SQR(sf_L))
                                    / (sfaved[coeffloc_L] + sf_L + eps);
...
bool ImProcFunctions::WaveletDenoiseAll_BiShrinkAB(
...
#pragma omp for schedule(dynamic) collapse(2)
            ...
            for (int lvl = 0; lvl < maxlvl; ++lvl) {
                for (int dir = 1; dir < 4; ++dir) {
                    // compute median absolute deviation (MAD) of detail coefficients as
                    ...
                    if (!denoiseMethodRgb) {
                        madab[lvl][dir - 1] =
                            SQR(Mad(WavCoeffs_ab[dir], Wlvl_ab * Hlvl_ab));
                    } else {
                        madab[lvl][dir - 1] =
...
#pragma omp for schedule(dynamic) collapse(2)
            ...
            {  // for levels less than max, use level diff to make edge mask
                for (int dir = 1; dir < 4; ++dir) {
                    ...
                    if (lvl == maxlvl - 1) {
                        ShrinkAllAB(WaveletCoeffs_L, WaveletCoeffs_ab, buffer, lvl, dir,
                                    noisevarchrom, noisevar_ab, useNoiseCCurve, autoch,
                                    denoiseMethodRgb, madL[lvl], nullptr, 0, madab[lvl],
                                    true);
                    } else {
                        // simple wavelet shrinkage
                        float noisevarfc;
                        ...
                        if ((local == 2 || local == 3) && variC && useNoiseCCurve) {
                            ...
                            for (int p = 0; p < Hlvl_ab * Wlvl_ab; p++) {
                                nvc[p] = 10.f * sqrt(variC[lvl])
                                         * SQR(1.f + 4.f * noisevarchrom[p]);
                            }
                        ...
                        //     float mad_abr = useNoiseCCurve ? noisevar_ab *
                        //     madab[lvl][dir - 1] : SQR(noisevar_ab) * madab[lvl][dir -
                        //     1];
                        float mad_abr = useNoiseCCurve
                                            ? noisevarfc * madab[lvl][dir - 1]
                                            : SQR(noisevarfc) * madab[lvl][dir - 1];
                        ...
                        if (noisevarfc > 0.001f) {
                            ...
                            for (coeffloc_ab = 0; coeffloc_ab < Hlvl_ab * Wlvl_ab - 3;
                                 coeffloc_ab += 4)
                            {
                            ...
                            for (; coeffloc_ab < Hlvl_ab * Wlvl_ab; ++coeffloc_ab) {
                                ...
                                WavCoeffs_ab[dir][coeffloc_ab] *= SQR(
                                    1.f
                                    - xexpf(-(mag_ab / (nvc[coeffloc_ab] * mad_abr))
                                            - (mag_L / (9.f * mad_Lr))) /*satfactor_a*/);
                            ...
                            for (int i = 0; i < Hlvl_ab; ++i) {
                                for (int j = 0; j < Wlvl_ab; ++j) {
                                    ...
                                    WavCoeffs_ab[dir][coeffloc_ab] *= SQR(
                                        1.f
                                        - xexpf(
                                            -(mag_ab / (nvc[coeffloc_ab] * mad_abr))
                                            - (mag_L / (9.f * mad_Lr))) /*satfactor_a*/);
                                }
                            }  // now chrominance coefficients are denoised
...
bool ImProcFunctions::WaveletDenoiseAllL(wavelet_decomposition& WaveletCoeffs_L,
                                         float* noisevarlum,
                                         float madL[8][3],
                                         float* vari,
                                         int edge,
...
bool ImProcFunctions::WaveletDenoiseAllAB(wavelet_decomposition& WaveletCoeffs_L,
                                          wavelet_decomposition& WaveletCoeffs_ab,
                                          float* noisevarchrom,
                                          float madL[8][3],
                                          float* variC,
...
#pragma omp for schedule(dynamic) collapse(2) nowait
            ...
            for (int lvl = 0; lvl < maxlvl; ++lvl) {
                for (int dir = 1; dir < 4; ++dir) {
                    ShrinkAllAB(WaveletCoeffs_L, WaveletCoeffs_ab, buffer, lvl, dir,
                                noisevarchrom, noisevar_ab, useNoiseCCurve, autoch,
                                denoiseMethodRgb, madL[lvl], variC, local, nullptr, 0);
                }
            }
        }
...
void ImProcFunctions::ShrinkAllL(wavelet_decomposition& WaveletCoeffs_L,
                                 float** buffer,
                                 int level,
                                 int dir,
                                 float* noisevarlum,
...

{
    ...
    if ((edge == 1 || edge == 2 || edge == 3 || edge == 5 || edge == 6) && vari) {
        //  nvl = blurBuffer;       // we need one buffer, but fortunately we don't have
        //  to allocate a new one because we can use blurBuffer
        if ((edge == 1 || edge == 3)) {
            for (int i = 0; i < W_L * H_L; ++i) {
    ...
    for (; i < W_L * H_L; ++i) {
        ...
        sfave[i] =
            mag
            / (mag + levelFactor * nvl[i] * xexpf(-mag / (9 * levelFactor * nvl[i]))
               + eps);
    }

    boxblur(sfave, sfaved, level + 2, W_L, H_L,
            false);  // increase smoothness by locally averaging shrinkage
    ...
    for (; i < W_L * H_L - 3; i += 4) {
        ...
        // use smoothed shrinkage unless local shrinkage is much less
        STVFU(WavCoeffs_L[dir][i], LVFU(WavCoeffs_L[dir][i])
...
void ImProcFunctions::ShrinkAllAB(wavelet_decomposition& WaveletCoeffs_L,
                                  wavelet_decomposition& WaveletCoeffs_ab,
                                  float** buffer,
                                  int level,
                                  int dir,
...

{
    ...
    if (noisevarfc > 0.001f) {  // noisevar_ab
        ...
        for (; coeffloc_ab < H_ab * W_ab; ++coeffloc_ab) {
            float mag_L = SQR(WavCoeffs_L[dir][coeffloc_ab]);
            float mag_ab = SQR(WavCoeffs_ab[dir][coeffloc_ab]);
            sfaveab[coeffloc_ab] = (1.f
                                    - xexpf(-(mag_ab / (nvc[coeffloc_ab] * madab))
                                            - (mag_L / (9.f * mad_L))));
        }  // now chrominance coefficients are denoised
...
#endif
        boxblur(sfaveab, sfaveabd, level + 2, W_ab, H_ab,
                false);  // increase smoothness by locally averaging shrinkage

//        boxblur(sfaveab, sfaveabd, blurBuffer, level + 2, level + 2, W_ab, H_ab);
//        //increase smoothness by locally averaging shrinkage
#ifdef __SSE2__
        vfloat epsv = F2V(eps);
        vfloat sfabv;
        vfloat sfaveabv;
        ...
        for (coeffloc_ab = 0; coeffloc_ab < H_ab * W_ab - 3; coeffloc_ab += 4) {
            ...
            // use smoothed shrinkage unless local shrinkage is much less
            STVFU(WavCoeffs_ab[dir][coeffloc_ab], LVFU(WavCoeffs_ab[dir][coeffloc_ab])
        ...
        for (; coeffloc_ab < H_ab * W_ab; ++coeffloc_ab) {
            ...
            // use smoothed shrinkage unless local shrinkage is much less
            WavCoeffs_ab[dir][coeffloc_ab] *= (SQR(sfaveabd[coeffloc_ab]) + SQR(sfab))
...
void ImProcFunctions::ShrinkAll_info(const float* const* WavCoeffs_a,
                                     const float* const* WavCoeffs_b,
                                     int W_ab,
                                     int H_ab,
                                     float** noisevarlum,
                                     float** noisevarchrom,
...
void ImProcFunctions::WaveletDenoiseAll_info(int levwav,
                                             const wavelet_decomposition& WaveletCoeffs_a,
                                             const wavelet_decomposition& WaveletCoeffs_b,
                                             float** noisevarlum,
                                             float** noisevarchrom,
                                             float** noisevarhue,
                                             float& chaut,
                                             ...
                                             bool denoiseMethodRgb)
{
    ...
    for (int lvl = 0; lvl < maxlvl; ++lvl) {
        ...
        ShrinkAll_info(WavCoeffs_a, WavCoeffs_b, Wlvl_ab, Hlvl_ab, noisevarlum,
                       noisevarchrom, noisevarhue, chaut, Nb, redaut, blueaut, maxredaut,
                       maxblueaut, minredaut, minblueaut, schoice, lvl, chromina, sigma,
                       lumema, sigma_L, redyel, skinc, nsknc, maxchred, maxchblue,
                       minchred, minchblue, nb, chau, chred, chblue, denoiseMethodRgb);
    }
}

void ImProcFunctions::RGB_denoise_infoGamCurve(
    const procparams::DirPyrDenoiseParams& dnparams,
    bool isRAW,
    LUTf& gamcurve,
    float& gam,
    ...
    float& gamslope)
{
    ...
    if (denoiseMethodRgb) {
        gamslope = exp(log(static_cast<double>(gamthresh)) / static_cast<double>(gam))
                   / static_cast<double>(gamthresh);
        Color::gammaf2lut(gamcurve, gam, gamthresh, gamslope, 65535.f, 32768.f);
    } else {
        Color::gammanf2lut(gamcurve, gam, 65535.f, 32768.f);
    }
...
void ImProcFunctions::calcautodn_info(float& chaut,
                                      float& delta,
                                      int Nb,
                                      int levaut,
                                      float maxmax,
...
void ImProcFunctions::RGB_denoise_info(Imagefloat* src,
...
{
    if ((settings->leveldnautsimpl == 1 && dnparams.Cmethod == "MAN")
        || (settings->leveldnautsimpl == 0 && dnparams.C2method == "MANU"))
    {
        // nothing to do
        return;
    }
...
#pragma omp parallel for if (multiThread)
                ...
                if (!denoiseMethodRgb)
                {  // lab mode, modification Jacques feb 2013 and july 2014
...
#pragma omp parallel for if (multiThread)
                    ...
                    for (int i = tiletop; i < tilebottom; ++i) {
                        ...
                        for (int j = tileleft; j < tileright; ++j) {
                            ...
                            R_ = R_ < 65535.f
                                     ? gamcurve[R_]
                                     : (Color::gammanf(R_ / 65535.f, gam) * 32768.f);
                            G_ = G_ < 65535.f
                                     ? gamcurve[G_]
                                     : (Color::gammanf(G_ / 65535.f, gam) * 32768.f);
                            B_ = B_ < 65535.f
                                     ? gamcurve[B_]
                                     : (Color::gammanf(B_ / 65535.f, gam) * 32768.f);
                            // true conversion xyz=>Lab
                            float X, Y, Z;
                    ...
                    for (int i = tiletop /*, i1=0*/; i < tilebottom; ++i /*, ++i1*/) {
                        ...
                        for (int j = tileleft /*, j1=0*/; j < tileright; ++j /*, ++j1*/) {
                            ...
                            X = X < 65535.f ? gamcurve[X]
                                            : (Color::gammaf(X / 65535.f, gam, gamthresh,
                                                             gamslope)
                                               * 32768.f);
                            Y = Y < 65535.f ? gamcurve[Y]
                                            : (Color::gammaf(Y / 65535.f, gam, gamthresh,
                                                             gamslope)
                                               * 32768.f);
                            Z = Z < 65535.f ? gamcurve[Z]
                                            : (Color::gammaf(Z / 65535.f, gam, gamthresh,
                                                             gamslope)
                                               * 32768.f);
                ...
                for (int i = tiletop /*, i1=0*/; i < tilebottom; ++i /*, ++i1*/) {
                    ...
                    for (int j = tileleft /*, j1=0*/; j < tileright; ++j /*, ++j1*/) {
                        ...
                        // use gamma sRGB, not good if TIF (JPG) Output profil not with
                        // gamma sRGB  (eg : gamma =1.0, or 1.8...) very difficult to
                        // solve !
                        //  solution ==> save TIF with gamma sRGB and re open
                        float rtmp = Color::igammatab_srgb[src->r(i, j)];
                        float gtmp = Color::igammatab_srgb[src->g(i, j)];
                        ...
                        rtmp = rtmp < 65535.f
                                   ? gamcurve[rtmp]
                                   : (Color::gammanf(rtmp / 65535.f, gam) * 32768.f);
                        gtmp = gtmp < 65535.f
                                   ? gamcurve[gtmp]
                                   : (Color::gammanf(gtmp / 65535.f, gam) * 32768.f);
                        btmp = btmp < 65535.f
                                   ? gamcurve[btmp]
                                   : (Color::gammanf(btmp / 65535.f, gam) * 32768.f);
            ...
            // now perform basic wavelet denoise
            // last two arguments of wavelet decomposition are max number of wavelet
            // decomposition levels; and whether to subsample the image after wavelet
            // filtering.  Subsampling is coded as binary 1 or 0 for each level, eg
            // subsampling = 0 means no subsampling, 1 means subsample the first level
            // only, 7 means subsample the first three levels, etc.
...
#endif
                {
                    adecomp = new wavelet_decomposition(labdn->data + datalen, labdn->W,
                                                        labdn->H, levwav, 1);
                }
#ifdef _OPENMP
#pragma omp section
#endif
                {
                    bdecomp = new wavelet_decomposition(labdn->data + 2 * datalen,
                                                        labdn->W, labdn->H, levwav, 1);
                }
            }

            if (comptlevel == 0) {
                WaveletDenoiseAll_info(levwav, *adecomp, *bdecomp, noisevarlum,
                                       noisevarchrom, noisevarhue, chaut, Nb, redaut,
                                       blueaut, maxredaut, maxblueaut, minredaut,
                                       minblueaut, schoice, chromina, sigma, lumema,
                                       sigma_L, redyel, skinc, nsknc, maxchred, maxchblue,
                                       minchred, minchblue, nb, chau, chred, chblue,
                                       denoiseMethodRgb);  // Enhance mode
            }
