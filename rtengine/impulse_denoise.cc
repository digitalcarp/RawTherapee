...
// Issue 1671:
// often, noise isn't evenly distributed, e.g. only a few noisy pixels in the bright sky,
// but many in the dark foreground, so it's better to schedule dynamic and let every
// thread only process 16 rows, to avoid running big threads out of work Measured it and
// in fact gives better performance than without schedule(dynamic,16). Of course, there
// could be a better choice for the chunk_size than 16 race conditions are avoided by the
// array impish
#ifdef _OPENMP
#pragma omp parallel
#endif
            ...
            for (j = 0; j < 2; j++) {
                ...
                for (i1 = max(0, i - 2); i1 <= min(i + 2, height - 1); i1++)
                    for (j1 = 0; j1 <= j + 2; j1++) {
                        ...
                        dirwt = 1
                                / (SQR(lab->L[i1][j1] - lab->L[i][j])
                                   + eps);  // use more sophisticated rangefn???
...
void ImProcFunctions::impulse_nrcam(CieImage* ncie, double thresh, float** buffers[3])
{
    ...
    // rangeblur<unsigned short, unsigned int> (lab->L, lpf, impish /*used as buffer
    // here*/, width, height, thresh, false);
#ifdef _OPENMP
#pragma omp parallel
#endif
    {
...
#pragma omp for
        ...
        for (int i = 0; i < height; i++) {
            for (j = 0; j < 2; j++) {
                ...
                impish[i][j] =
                    static_cast<float>(hpfabs > ((hfnbrave - hpfabs) * impthrDiv24));
            }
            ...
            for (; j < width - 5; j += 4) {
                ...
                STVFU(impish[i][j],
                      vselfzero(vmaskf_gt(hpfabsv, (hfnbravev - hpfabsv) * impthrDiv24v),
                                onev));
            }
    ...
    float** sraa = buffers[0];  // we can reuse buffers[0] because lpf is not needed
            ...
            for (j = 0; j < 2; j++) {
                ...
                for (i1 = max(0, i - 2); i1 <= min(i + 2, height - 1); i1++)
                    for (j1 = 0; j1 <= j + 2; j1++) {
                        ...
                        dirwt = 1.f
                                / (SQR(ncie->sh_p[i1][j1] - ncie->sh_p[i][j])
                                   + eps);  // use more sophisticated rangefn???
