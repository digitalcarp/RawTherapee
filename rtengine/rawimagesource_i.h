...
inline void RawImageSource::convert_row_to_RGB(float* r,
                                               float* g,
                                               float* b,
                                               const float* const Y,
                                               const float* const I,
...
inline void RawImageSource::convert_to_RGB(float& r,
                                           float& g,
                                           float& b,
                                           const float Y,
                                           const float I,
...
inline void RawImageSource::interpolate_row_rb_mul_pp(const array2D<float>& rawData,
                                                      float* ar,
                                                      float* ab,
                                                      float* pg,
                                                      float* cg,
        ...
        for (int j = x1, jx = 0; jx < width; j += skip, jx++) {
            if (ri->ISRED(i, j)) {
                ...
                if (j == 0) {
                    ...
                    r = g_mul * cg[j]
                        + (r_mul * rawData[i][j - 1] - g_mul * cg[j - 1]
                           + r_mul * rawData[i][j + 1] - g_mul * cg[j + 1])
                              / 2;
                }
                ...
                if (i == 0) {
                    ...
                    b = g_mul * cg[j]
                        + (b_mul * rawData[i - 1][j] - g_mul * pg[j]
                           + b_mul * rawData[i + 1][j] - g_mul * ng[j])
                              / 2;
                }
        ...
        for (int j = x1, jx = 0; jx < width; j += skip, jx++) {
            if (ri->ISBLUE(i, j)) {
                ...
                if (j == 0) {
                    ...
                    b = g_mul * cg[j]
                        + (b_mul * rawData[i][j - 1] - g_mul * cg[j - 1]
                           + b_mul * rawData[i][j + 1] - g_mul * cg[j + 1])
                              / 2;
                }
                ...
                if (i == 0) {
                    ...
                    r = g_mul * cg[j]
                        + (r_mul * rawData[i - 1][j] - g_mul * pg[j]
                           + r_mul * rawData[i + 1][j] - g_mul * ng[j])
                              / 2;
                }
