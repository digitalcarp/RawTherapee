 ...
 *
 *  Copyright (c) 2004-2018 Gabor Horvath <hgabor@rawtherapee.com> and other RawTherapee
 * contributors Split out to own compilation unit and made speedup 2018 Ingo Weyrich
 * (heckflosse67@gmx.de)
 *
...
inline void RawImageSource::interpolate_row_g(float* agh, float* agv, int i)
{
    ...
    for (int j = 0; j < W; j++) {
        if (ri->ISGREEN(i, j)) {
            ...
            if (j > 1 && j < W - 2) {
                gh = (-rawData[i][j - 2] + 2 * rawData[i][j - 1] + 2 * rawData[i][j]
                      + 2 * rawData[i][j + 1] - rawData[i][j + 2])
                     / 4;
            ...
            if (i > 1 && i < H - 2) {
                gv = (-rawData[i - 2][j] + 2 * rawData[i - 1][j] + 2 * rawData[i][j]
                      + 2 * rawData[i + 1][j] - rawData[i + 2][j])
                     / 4;
...
inline void RawImageSource::interpolate_row_rb(float* ar,
                                               float* ab,
                                               float* pg,
                                               float* cg,
                                               float* ng,
                                               int i)
{
    const auto getPg = [pg](int index) { return pg ? pg[index] : 0.f; };

    const auto getNg = [ng](int index) { return ng ? ng[index] : 0.f; };
    ...
    if (j) {
        ...
        if (i == 0) {
            ...
            val2 =
                cg[0] + (rawData[i - 1][0] - getPg(0) + rawData[i + 1][0] - getNg(0)) / 2;
        }
    ...
    for (; j < W - 1; j += 2) {
        ...
        if (j == W - 2) {
            ...
            val1 =
                cg[j + 1] + (rawData[i][j] - cg[j] + rawData[i][j + 2] - cg[j + 2]) / 2;
        }
        ...
        if (i == 0) {
            ...
            val2 = cg[j + 1]
                   + (rawData[i - 1][j + 1] - getPg(j + 1) + rawData[i + 1][j + 1]
                      - getNg(j + 1))
                         / 2;
        }
...
void RawImageSource::eahd_demosaic()
{
    ...
    if (plistener) {
        plistener->setProgressStr(
            Glib::ustring::compose(M("TP_RAW_DMETHOD_PROGRESSBAR"), M("TP_RAW_EAHD")));
    ...
    const float lc00 =
        (0.412453 * imatrices.rgb_cam[0][0] + 0.357580 * imatrices.rgb_cam[0][1]
         + 0.180423 * imatrices.rgb_cam[0][2]);  // / 0.950456;
    const float lc01 =
        (0.412453 * imatrices.rgb_cam[1][0] + 0.357580 * imatrices.rgb_cam[1][1]
         + 0.180423 * imatrices.rgb_cam[1][2]);  // / 0.950456;
    const float lc02 =
        (0.412453 * imatrices.rgb_cam[2][0] + 0.357580 * imatrices.rgb_cam[2][1]
         + 0.180423 * imatrices.rgb_cam[2][2]);  // / 0.950456;

    const float lc10 = 0.212671 * imatrices.rgb_cam[0][0]
                       + 0.715160 * imatrices.rgb_cam[0][1]
                       + 0.072169 * imatrices.rgb_cam[0][2];
    const float lc11 = 0.212671 * imatrices.rgb_cam[1][0]
                       + 0.715160 * imatrices.rgb_cam[1][1]
                       + 0.072169 * imatrices.rgb_cam[1][2];
    const float lc12 = 0.212671 * imatrices.rgb_cam[2][0]
                       + 0.715160 * imatrices.rgb_cam[2][1]
                       + 0.072169 * imatrices.rgb_cam[2][2];

    const float lc20 =
        (0.019334 * imatrices.rgb_cam[0][0] + 0.119193 * imatrices.rgb_cam[0][1]
         + 0.950227 * imatrices.rgb_cam[0][2]);  // / 1.088754;
    const float lc21 =
        (0.019334 * imatrices.rgb_cam[1][0] + 0.119193 * imatrices.rgb_cam[1][1]
         + 0.950227 * imatrices.rgb_cam[1][2]);  // / 1.088754;
    const float lc22 =
        (0.019334 * imatrices.rgb_cam[2][0] + 0.119193 * imatrices.rgb_cam[2][1]
         + 0.950227 * imatrices.rgb_cam[2][2]);  // / 1.088754;

    const float wp[3][3] = { { lc00, lc01, lc02 },
                             { lc10, lc11, lc12 },
                             { lc20, lc21, lc22 } };

    // end of cielab preparation

    JaggedArray<float> rh(W, 3), gh(W, 4), bh(W, 3), rv(W, 3), gv(W, 4), bv(W, 3),
        lLh(W, 3), lah(W, 3), lbh(W, 3), lLv(W, 3), lav(W, 3), lbv(W, 3);
    ...
    for (int i = 1; i < H - 1; i++) {
        ...
        if (i < H - 2) {
            ...
            interpolate_row_rb(rh[(i + 1) % 3], bh[(i + 1) % 3], gh[i % 4],
                               gh[(i + 1) % 4], gh[(i + 2) % 4], i + 1);
            interpolate_row_rb(rv[(i + 1) % 3], bv[(i + 1) % 3], gv[i % 4],
                               gv[(i + 1) % 4], gv[(i + 2) % 4], i + 1);
        } else {
            interpolate_row_rb(rh[(i + 1) % 3], bh[(i + 1) % 3], gh[i % 4],
                               gh[(i + 1) % 4], nullptr, i + 1);
            interpolate_row_rb(rv[(i + 1) % 3], bv[(i + 1) % 3], gv[i % 4],
                               gv[(i + 1) % 4], nullptr, i + 1);
        }

        Color::RGB2Lab(rh[(i + 1) % 3], gh[(i + 1) % 4], bh[(i + 1) % 3],
                       lLh[(i + 1) % 3], lah[(i + 1) % 3], lbh[(i + 1) % 3], wp, W);
        Color::RGB2Lab(rv[(i + 1) % 3], gv[(i + 1) % 4], bv[(i + 1) % 3],
                       lLv[(i + 1) % 3], lav[(i + 1) % 3], lbv[(i + 1) % 3], wp, W);
        ...
        for (int j = 0; j < W; j++) {
            if (ri->ISGREEN(i - 1, j)) {
                ...
                if (hc > vc) {
                    ...
                    green[i - 1][j] =
                        std::max(0.f, (gh[(i - 1) % 4][j] + gv[(i - 1) % 4][j]) / 2);
                }
            }
        }
