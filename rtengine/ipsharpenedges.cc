...
{
    // JD: this algorithm maximize clarity of images; it does not play on accutance. It
    // can remove (partially) the effects of the AA filter) I think we can use this
    // algorithm alone in most cases, or first to clarify image and if you want a very
    // little USM (unsharp mask sharpening) after...
...
#pragma omp parallel for schedule(dynamic, 16)
            ...
            for (int j = 2; j < height - 2; j++) {
                for (int i = 2, offset = j * width + i; i < width - 2; i++, offset++) {
                    ...
                    const float wV =
                        eps2 + std::fabs(L[offset + width] - L[offset - width]);

                    float s = 2.f / (2.f + std::fabs(wH - wV));
                    float wD1 =
                        eps2
                        + std::fabs(L[offset + width + 1] - L[offset - width - 1]) * s;
                    float wD2 =
                        eps2
                        + std::fabs(L[offset + width - 1] - L[offset - width + 1]) * s;
                    s = wD1;
                    ...
                    const float contrast =
                        std::min(std::sqrt(SQR(L[offset + 1] - L[offset - 1])
                                           + SQR(L[offset + width] - L[offset - width]))
                                     * chmax[c],
                                 1.f);
                    ...
                    if (inintervalLoRo(v, L[offset - 1], L[offset + 1])) {
                        ...
                        float f3 = (L[offset - 1] - L[offset - width])
                                   * (L[offset - 1] - L[offset + width]);
                        float f4 =
                            std::sqrt(std::fabs((L[offset - 1] - L[offset - width2])
                                                * (L[offset - 1] - L[offset + width2])));
                        const float difL = f1 * SQR(f2 * f3) * f4;
                        if (difL > 0.f) {
                            f1 = std::fabs(L[offset + 2] - L[offset + 1]);
                            f2 = L[offset + 1] - v;
                            f3 = (L[offset + 1] - L[offset - width])
                                 * (L[offset + 1] - L[offset + width]);
                            f4 = std::sqrt(
                                std::fabs((L[offset + 1] - L[offset - width2])
                                          * (L[offset + 1] - L[offset + width2])));
                            const float difR = f1 * SQR(f2 * f3) * f4;
                            if (difR > 0.f) {
                                lumH = (L[offset - 1] * difR + L[offset + 1] * difL)
                                       / (difL + difR);
                    ...
                    if (inintervalLoRo(v, L[offset - width], L[offset + width])) {
                        ...
                        float f3 = (L[offset - width] - L[offset - 1])
                                   * (L[offset - width] - L[offset + 1]);
                        float f4 =
                            std::sqrt(std::fabs((L[offset - width] - L[offset - 2])
                                                * (L[offset - width] - L[offset + 2])));
                        const float difT = f1 * SQR(f2 * f3) * f4;
                        if (difT > 0.f) {
                            f1 = std::fabs(L[offset + width2] - L[offset + width]);
                            f2 = L[offset + width] - v;
                            f3 = (L[offset + width] - L[offset - 1])
                                 * (L[offset + width] - L[offset + 1]);
                            f4 = std::sqrt(
                                std::fabs((L[offset + width] - L[offset - 2])
                                          * (L[offset + width] - L[offset + 2])));
                            const float difB = f1 * SQR(f2 * f3) * f4;
                            if (difB > 0.f) {
                                lumV =
                                    (L[offset - width] * difB + L[offset + width] * difT)
                                    / (difT + difB);
                    ...
                    if (inintervalLoRo(v, L[offset - 1 - width], L[offset + 1 + width])) {
                        float f1 =
                            std::fabs(L[offset - 2 - width2] - L[offset - 1 - width]);
                        float f2 = L[offset - 1 - width] - v;
                        float f3 = (L[offset - 1 - width] - L[offset - width + 1])
                                   * (L[offset - 1 - width] - L[offset + width - 1]);
                        float f4 = std::sqrt(std::fabs(
                            (L[offset - 1 - width] - L[offset - width2 + 2])
                            * (L[offset - 1 - width] - L[offset + width2 - 2])));
                        ...
                        if (difLT > 0.f) {
                            ...
                            f3 = (L[offset + 1 + width] - L[offset - width + 1])
                                 * (L[offset + 1 + width] - L[offset + width - 1]);
                            f4 = std::sqrt(std::fabs(
                                (L[offset + 1 + width] - L[offset - width2 + 2])
                                * (L[offset + 1 + width] - L[offset + width2 - 2])));
                            const float difRB = f1 * SQR(f2 * f3) * f4;
                            if (difRB > 0.f) {
                                lumD1 = (L[offset - 1 - width] * difRB
                                         + L[offset + 1 + width] * difLT)
                                        / (difLT + difRB);
                    ...
                    if (inintervalLoRo(v, L[offset + 1 - width], L[offset - 1 + width])) {
                        ...
                        if (difLB > 0.f) {
                            ...
                            if (difRT > 0.f) {
                                lumD2 = (L[offset + 1 - width] * difLB
                                         + L[offset - 1 + width] * difRT)
                                        / (difLB + difRT);
                    ...
                    if (c == 0) {
                        if (v < 92.f) {
                            channel[j][i] = std::fabs(
                                327.68f
                                * intp(weight,
                                       (lumH * wH + lumV * wV + lumD1 * wD1 + lumD2 * wD2)
                                           / (wH + wV + wD1 + wD2),
                                       v));  // fabs because lab->L always > 0
                        }
                    } else {
                        channel[j][i] =
                            327.68f
