...
#ifdef __SSE2__
inline void interpolateTransformCubic(rtengine::Imagefloat* src,
                                      int xs,
                                      int ys,
                                      float Dx,
                                      float Dy,
                                      ...
                                      float mul)
{
    ...
    const vfloat weight = _mm_set_ps(t1Hor * Dx, t1Hor * Dx - t1Hor + t2Hor,
...
inline void interpolateTransformCubicLog(rtengine::Imagefloat* src,
                                         int xs,
                                         int ys,
                                         float Dx,
                                         float Dy,
                                         ...
                                         float mul)
{
    ...
    const vfloat weight = _mm_set_ps(t1Hor * Dx, t1Hor * Dx - t1Hor + t2Hor,
                                     1.f - (t1Hor * Dx) - t2Hor, t1Hor - (t1Hor * Dx));
    const vfloat tempv =
        _mm_setr_ps(vhadd(weight * rv), vhadd(weight * gv), vhadd(weight * bv), 0.f);
                                      ...
                                      float mul)
{
    ...
    for (int i = 0; i < 4; ++i) {
        rv[i] = w0Vert * src->r(ys, xs + i) + w1Vert * src->r(ys + 1, xs + i)
                + w2Vert * src->r(ys + 2, xs + i) + w3Vert * src->r(ys + 3, xs + i);
        gv[i] = w0Vert * src->g(ys, xs + i) + w1Vert * src->g(ys + 1, xs + i)
                + w2Vert * src->g(ys + 2, xs + i) + w3Vert * src->g(ys + 3, xs + i);
        bv[i] = w0Vert * src->b(ys, xs + i) + w1Vert * src->b(ys + 1, xs + i)
                + w2Vert * src->b(ys + 2, xs + i) + w3Vert * src->b(ys + 3, xs + i);
    }
...
#endif
#ifdef __SSE2__
inline void interpolateTransformChannelsCubic(const float* const* src,
                                              int xs,
                                              int ys,
                                              float Dx,
                                              float Dy,
                                              ...
                                              float mul)
{
    ...
    const vfloat cv = (w0Vert * LVFU(src[ys][xs]) + w1Vert * LVFU(src[ys + 1][xs]))
...
inline void interpolateTransformChannelsCubicLog(const float* const* src,
                                                 int xs,
                                                 int ys,
                                                 float Dx,
                                                 float Dy,
...
#define CLIPTOC(a, b, c, d) \
    ((a) >= (b) ? ((a) <= (c) ? (a) : (d = true, (c))) : (d = true, (b)))

template <class A, class B, class C> void track_min_max(A& min, B& max, C const val)
{
    if (val < min) min = val;
    if (val > max) max = val;
}
 ...
 */
homogeneous::Matrix<double> perspectiveMatrix(double camera_focal_length,
                                              ...
                                              double projection_scale)
{
    ...
    const homogeneous::Matrix<double> matrix =
        ...
        homogeneous::translationMatrix<double>(-camera_shift_horiz, -camera_shift_vert,
                                               -camera_focal_length)
        *
        // Camera roll.
        ...
        * homogeneous::rotationMatrix<double>(-camera_yaw, homogeneous::Axis::Y) *
        ...
        homogeneous::translationMatrix<double>(
            camera_center_yaw_pitch[0], camera_center_yaw_pitch[1],
            camera_center_yaw_pitch[2] - camera_focal_length)
        *
        ...
        *
        ...
        homogeneous::translationMatrix<double>(
            projection_center_yaw_pitch[0], projection_center_yaw_pitch[1],
            camera_focal_length - projection_center_yaw_pitch[2])
        *
        // Simulate perspective transformation.
        homogeneous::projectionMatrix<double>(projection_center_yaw_pitch[2],
                                              homogeneous::Axis::Z)
        * homogeneous::rotationMatrix<double>(projection_yaw, homogeneous::Axis::Y)
        * homogeneous::rotationMatrix<double>(projection_pitch, homogeneous::Axis::X) *
        // Move to z == 0.
...
bool ImProcFunctions::transCoord(int W,
                                 int H,
                                 ...
                                 const LensCorrection* pLCPMap) const
{
    ...
    const PerspType perspectiveType =
        needsPerspective()
            ? ((params->perspective.method == "camera_based") ? PerspType::CAMERA_BASED
                                                              : PerspType::SIMPLE)
            : PerspType::NONE;
    bool clipped = false;
    ...
    if (!needsCA() && !needsDistortion() && !needsRotation() && !needsPerspective()
        && !needsScaleAny() && (!params->lensProf.useDist || pLCPMap == nullptr))
    {
    ...
    const double ascale =
        ascaleDef > 0 ? ascaleDef
                      : (params->commonTrans.autofill && params->perspective.render
                             ? getTransformAutoFill(oW, oH, pLCPMap)
                             : 1.0 / params->commonTrans.getScale());
    ...
    const double vpteta =
        fabs(vpalpha - rtengine::RT_PI / 2) < 3e-4
            ? 0.0
            : acos((vpdeg > 0 ? 1.0 : -1.0)
                   * sqrt((-oW * oW * tan(vpalpha) * tan(vpalpha)
                           + (vpdeg > 0 ? 1.0 : -1.0) * oW * tan(vpalpha)
                                 * sqrt(16 * maxRadius * maxRadius
                                        + oW * oW * tan(vpalpha) * tan(vpalpha)))
    ...
    const double hpteta =
        fabs(hpalpha - rtengine::RT_PI / 2) < 3e-4
            ? 0.0
            : acos((hpdeg > 0 ? 1.0 : -1.0)
                   * sqrt((-oH * oH * tan(hpalpha) * tan(hpalpha)
                           + (hpdeg > 0 ? 1.0 : -1.0) * oH * tan(hpalpha)
                                 * sqrt(16 * maxRadius * maxRadius
                                        + oH * oH * tan(hpalpha) * tan(hpalpha)))
                          / (maxRadius * maxRadius * 8)));
    const double hpcospt = (hpdeg >= 0 ? 1.0 : -1.0) * cos(hpteta), hptanpt = tan(hpteta);
    // Camera-based.
    const double f = ((params->perspective.camera_focal_length > 0)
                          ? params->perspective.camera_focal_length
                          : PerspectiveParams::DEFAULT_CAMERA_FOCAL_LENGTH)
                     * ((params->perspective.camera_crop_factor > 0)
    ...
    const double f_defish =
        ((params->distortion.focal_length > 0) ? params->distortion.focal_length
                                               : DistortionParams::DEFAULT_FOCAL_LENGTH)
        * (maxRadius / sqrt(18.0 * 18.0 + 12.0 * 12.0));
    const double p_camera_yaw = params->perspective.camera_yaw / 180.0 * rtengine::RT_PI;
    const double p_camera_pitch =
        params->perspective.camera_pitch / 180.0 * rtengine::RT_PI;
    const double p_camera_roll = params->perspective.camera_roll * rtengine::RT_PI_180;
    const double p_camera_shift_horiz =
        oW / 100.0 * params->perspective.camera_shift_horiz;
    const double p_camera_shift_vert =
        oH / -100.0 * params->perspective.camera_shift_vert;
    const double p_projection_shift_horiz =
        oW / 100.0 * params->perspective.projection_shift_horiz;
    const double p_projection_shift_vert =
        oH / -100.0 * params->perspective.projection_shift_vert;
    const double p_projection_rotate =
        params->perspective.projection_rotate * rtengine::RT_PI_180;
    const double p_projection_yaw =
        -params->perspective.projection_yaw * rtengine::RT_PI_180;
    const double p_projection_pitch =
        -params->perspective.projection_pitch * rtengine::RT_PI_180;
    const double p_projection_scale = 1;
    const homogeneous::Matrix<double> p_matrix = perspectiveMatrix(
        f, p_camera_shift_horiz, p_camera_shift_vert, p_camera_roll, p_camera_pitch,
        p_camera_yaw, p_projection_yaw, p_projection_pitch, p_projection_rotate,
        p_projection_shift_horiz, p_projection_shift_vert, p_projection_scale);
    ...
    for (size_t i = 0; i < src.size(); i++) {
        ...
        case PerspType::CAMERA_BASED:
            ...
            const double xw =
                p_matrix[0][0] * x_d + p_matrix[0][1] * y_d + p_matrix[0][3];
            const double yw =
                p_matrix[1][0] * x_d + p_matrix[1][1] * y_d + p_matrix[1][3];
            x_d = xw / w;
            y_d = yw / w;
            break;
        }
...
void ImProcFunctions::transform(Imagefloat* original,
                                ...
                                bool useOriginalBuffer)
{
    ...
    {  // don't check focal length to allow distortion correction for lenses without chip
        const std::shared_ptr<LCPProfile> pLCPProf =
            LCPStore::getInstance()->getProfile(params->lensProf.lcpFile);

        if (pLCPProf) {
            pLCPMap.reset(new LCPMapper(pLCPProf, focalLen, focalLen35mm, focusDist,
                                        fNumber, false, false, oW, oH, params->coarse,
                                        rawRotationDeg));
        }
    }

    if (!(needsCA() || needsDistortion() || needsRotation() || needsPerspective()
          || needsScaleAny() || needsLCP() || needsMetadata() || needsLensfun())
        && (needsVignetting() || needsPCVignetting() || needsGradient()))
    {
        ...
        transformGeneral(highQuality, original, dest, cx, cy, sx, sy, oW, oH, fW, fH,
                         pLCPMap.get(), useOriginalBuffer);
    }
}

// helper function
void ImProcFunctions::calcVignettingParams(int oW,
                                           int oH,
                                           const VignettingParams& vignetting,
                                           double& w2,
                                           double& h2,
                                           double& maxRadius,
                                           double& v,
...
static void
calcGradientParams(int oW, int oH, const GradientParams& gradient, struct grad_params& gp)
{
    int w = oW;
    int h = oH;
    ...
    gp.ys = rtengine::norm2(static_cast<double>(h), static_cast<double>(w))
...
static void calcPCVignetteParams(int fW,
                                 int fH,
                                 int oW,
                                 int oH,
                                 ...
                                 struct pcv_params& pcv)
{
    ...
    pcv.fadeout_mul =
        20.0 / rtengine::norm2(static_cast<double>(oW), static_cast<double>(oW));
    ...
    if (roundness < 0.5f) {
        ...
        float sepf = 2
                     + 4
                           * std::pow(1.f - 2 * roundness,
                                      1.3f);  // gamma 1.3 used to balance the effect in
                                              // the 0.0...0.5 roundness range
        ...
        float rad =
            rtengine::norm2(static_cast<float>(pcv.w), static_cast<float>(pcv.h)) / 2.f;
...
static float calcPCVignetteFactor(const struct pcv_params& pcv, int x, int y)
{
    ...
    if (x < pcv.x1 || x > pcv.x2 || y < pcv.y1 || y > pcv.y2) {
        /*
          The initial plan was to have 1.0 directly outside the crop box (ie no fading),
          but due to rounding/trunction here and there I didn't succeed matching up
          exactly on the pixel with the crop box. To hide that mismatch I made a fade.
         */
    ...
    if (pcv.is_super_ellipse_mode) {
        ...
        float dist_oe2 =
            pcv.oe2_a * pcv.oe2_b
            / normn(pcv.oe2_b * sincosval.y, pcv.oe2_a * sincosval.x, pcv.sep + 2);
                                             ...
                                             int fH)
{
    ...
    if (applyVignetting) {
        calcVignettingParams(oW, oH, params->vignetting, vig_w2, vig_h2, maxRadius, v, b,
                             mul);
    }
    ...
    if (applyPCVignetting) {
        ...
        // transformed->getWidth(), transformed->getHeight(), cx, cy, params->crop.w,
        // params->crop.h);
    ...
    for (int y = 0; y < transformed->getHeight(); y++) {
        ...
        for (int x = 0; x < transformed->getWidth(); x++) {
            ...
            if (applyVignetting) {
                ...
                if (darkening) {
                    factor /=
                        std::max(v + mul * tanh(b * (maxRadius - r) / maxRadius), 0.001);
                } else {
...
void ImProcFunctions::transformGeneral(bool highQuality,
                                       Imagefloat* original,
                                       Imagefloat* transformed,
                                       int cx,
                                       int cy,
                                       ...
                                       bool useOriginalBuffer)
{
    ...
    enum PerspType { NONE, SIMPLE, CAMERA_BASED };
    const bool enableLCPDist =
        pLCPMap && params->lensProf.useDist && pLCPMap->hasDistortionCorrection();
    const bool enableLCPCA =
        pLCPMap && params->lensProf.useCA && pLCPMap->hasCACorrection();
    ...
    const std::array<float* const*, 3> chTrans = { transformed->r.ptrs,
                                                   transformed->g.ptrs,
                                                   transformed->b.ptrs };

    // auxiliary variables for c/a correction
    const std::array<double, 3> chDist = { enableCA ? params->cacorrection.red : 0.0, 0.0,
                                           enableCA ? params->cacorrection.blue : 0.0 };
    ...
    const double vpteta =
        fabs(vpalpha - rtengine::RT_PI / 2) < 3e-4
            ? 0.0
            : acos((vpdeg > 0 ? 1.0 : -1.0)
                   * sqrt((-SQR(oW * tan(vpalpha))
                           + (vpdeg > 0 ? 1.0 : -1.0) * oW * tan(vpalpha)
                                 * sqrt(SQR(4 * maxRadius) + SQR(oW * tan(vpalpha))))
                          / (SQR(maxRadius) * 8)));
    ...
    const double hpteta =
        fabs(hpalpha - rtengine::RT_PI / 2) < 3e-4
            ? 0.0
            : acos((hpdeg > 0 ? 1.0 : -1.0)
                   * sqrt((-SQR(oH * tan(hpalpha))
                           + (hpdeg > 0 ? 1.0 : -1.0) * oH * tan(hpalpha)
                                 * sqrt(SQR(4 * maxRadius) + SQR(oH * tan(hpalpha))))
                          / (SQR(maxRadius) * 8)));
    ...
    const double ascale = params->commonTrans.autofill && params->perspective.render
                              ? getTransformAutoFill(oW, oH, pLCPMap)
                              : 1.0 / params->commonTrans.getScale();
    const double hscale = ascale / params->commonTrans.getScaleHorizontally();
    const double vscale = ascale / params->commonTrans.getScaleVertically();
    ...
    const std::array<const float* const*, 3> chOrig = { original->r.ptrs,
                                                        original->g.ptrs,
                                                        original->b.ptrs };
...
#pragma omp parallel for schedule(dynamic, 16) if (multiThread)
    ...
    for (int y = 0; y < transformed->getHeight(); ++y) {
        for (int x = 0; x < transformed->getWidth(); ++x) {
            ...
            case PerspType::CAMERA_BASED:
                const double w =
                    p_matrix[3][0] * x_d + p_matrix[3][1] * y_d + p_matrix[3][3];
                const double xw =
                    p_matrix[0][0] * x_d + p_matrix[0][1] * y_d + p_matrix[0][3];
                const double yw =
                    p_matrix[1][0] * x_d + p_matrix[1][1] * y_d + p_matrix[1][3];
            ...
            for (int c = 0; c < (doCACorrection ? 3 : 1); ++c) {
                    ...
                    && xc < original->getWidth())
                {
                    ...
                    if (enableVignetting) {
                        const double vig_x_d =
                            hscale * (x + cx - vig_w2);  // centering x coord & scale
                        const double vig_y_d =
                            vscale * (y + cy - vig_h2);  // centering y coord & scale
                        ...
                        if (darkening) {
                            vignmul /= std::max(
                                v + mul * tanh(b * (maxRadius - s * r2) / maxRadius),
                                0.001);
                        } else {
                            vignmul *=
                                (v + mul * tanh(b * (maxRadius - s * r2) / maxRadius));
                    ...
                    if (enableGradient) {
                        vignmul *=
                            static_cast<double>(calcGradientFactor(gp, cx + x, cy + y));
                    }

                    if (enablePCVignetting) {
                        vignmul *= static_cast<double>(
                            calcPCVignetteFactor(pcv, cx + x, cy + y));
                    }
                        ...
                        && xc < original->getWidth() - 2)
                    {
                        ...
                        if (!highQuality) {
                            ...
                            if (doCACorrection) {
                                interpolateTransformChannelsCubic(
                                    chOrig[c], xc - 1, yc - 1, Dx, Dy, chTrans[c][y][x],
                                    vignmul);
                            } else {
                                interpolateTransformCubic(original, xc - 1, yc - 1, Dx,
                                                          Dy, transformed->r(y, x),
                                                          transformed->g(y, x),
                                                          transformed->b(y, x), vignmul);
                            }
                        } else {
                            if (doCACorrection) {
                                interpolateTransformChannelsCubicLog(
                                    chOrig[c], xc - 1, yc - 1, Dx, Dy, chTrans[c][y][x],
                                    vignmul);
                            } else {
                                interpolateTransformCubicLog(
                        ...
                        if (useLog) {
                            if (doCACorrection) {
                                chTrans[c][y][x] =
                                    vignmul
                                    * xexpf(chOrig[c][y1][x1] * (1.0 - Dx) * (1.0 - Dy)
...
double
ImProcFunctions::getTransformAutoFill(int oW, int oH, const LensCorrection* pLCPMap) const
{
    if (!needsCA() && !needsDistortion() && !needsRotation() && !needsPerspective()
...
{
    return fabs(params->cacorrection.red) > 1e-15
           || fabs(params->cacorrection.blue) > 1e-15;
}

bool ImProcFunctions::needsDistortion() const
{
    return params->distortion.defish || fabs(params->distortion.amount) > 1e-15;
}
...
bool ImProcFunctions::needsPerspective() const
{
    return ((params->perspective.method == "simple")
           ...
           || ((params->perspective.method == "camera_based")
               ...
               && (params->perspective.camera_pitch || params->perspective.camera_roll
...
bool ImProcFunctions::needsTransform(int oW,
                                     int oH,
                                     int rawRotationDeg,
                                     const FramesMetaData* metadata) const
{
    ...
    if (needsLf) {
        std::unique_ptr<const LensCorrection> pLCPMap =
            LFDatabase::getInstance()->findModifier(params->lensProf, metadata, oW, oH,
                                                    params->coarse, rawRotationDeg);
        needsLf = pLCPMap.get();
    }
    return needsCA() || needsDistortion() || needsRotation() || needsPerspective()
           || needsScaleAny() || needsGradient() || needsPCVignetting()
           || needsVignetting() || needsLCP() || needsMetadata() || needsLf;
}
