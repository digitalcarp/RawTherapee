...
/* Solves A x = b by the conjugate gradient method, where instead of feeding it the matrix
A you feed it a function which calculates A x where x is some vector. Stops when rms
residual < RMSResidual or when maximum iterates is reached. Stops at n iterates if
MaximumIterates = 0 since that many iterates gives exact solution. Applicable to symmetric
positive definite problems only, which is what unconstrained smooth optimization pretty
much always is. Parameter pass can be passed through, containing whatever info you like it
to contain (matrix info?). Takes less memory with OkToModify_b = true, and Preconditioner
= nullptr. */
float* SparseConjugateGradient(void Ax(float* Product, float* x, void* Pass),
                               float* b,
                               int n,
                               bool OkToModify_b,
                               float* x,
                               ...
                               void* Pass,
                               int MaximumIterates,
                               void Preconditioner(float* Product, float* x, void* Pass))
{
    int iterate;
...
#pragma omp parallel for  // removed schedule(dynamic,10)
    ...
    for (iterate = 0; iterate < MaximumIterates; iterate++) {
        ...
        if (ab == 0.0) {
            break;  // So unlikely. It means perfectly converged or singular, stop either
                    // way.
        }
...
#pragma omp parallel for reduction(+ : rms)
        ...
        for (int ii = 0; ii < n; ii++) {
            x[ii] += abf * d[ii];
            r[ii] -= abf * ax[ii];  //"Fast recursive formula", use explicit r = b - Ax
                                    //occasionally?
...
MultiDiagonalSymmetricMatrix::MultiDiagonalSymmetricMatrix(
    int Dimension,
    int NumberOfDiagonalsInLowerTriangle)
    : buffer(nullptr), DiagBuffer(nullptr)
{
    n = Dimension;
...
bool MultiDiagonalSymmetricMatrix::CreateDiagonal(int index, int StartRow)
{
    ...
    if (index > 0)
        if (StartRow <= StartRows[index - 1]) {
            printf(
                "Error in MultiDiagonalSymmetricMatrix::CreateDiagonal: each StartRow must exceed the previous.\n");
            return false;
        }
    ...
    if (DiagBuffer != nullptr) {
        ...
        if (Diagonals[index] == nullptr) {
            printf(
                "Error in MultiDiagonalSymmetricMatrix::CreateDiagonal: memory allocation failed. Out of memory?\n");
            return false;
        }
...
bool MultiDiagonalSymmetricMatrix::LazySetEntry(float value, int row, int column)
{
    ...
    if (column > row) i = column, column = row, row = i;
...
void MultiDiagonalSymmetricMatrix::VectorProduct(float* RESTRICT Product,
                                                 float* RESTRICT x)
{
...
#pragma omp parallel
#endif
    {
        ...
        // This can be done without intermediate stores to memory and it can be
        // parallelized too
#ifdef _OPENMP
#pragma omp for schedule(dynamic, chunkSize) nowait
        ...
        for (int j = srm; j < lm - 3; j += 4) {
            ...
            for (int i = m - 1; i > 0; i--) {
                int s = StartRows[i];
                prodv += (LVFU(Diagonals[i][j - s]) * LVFU(x[j - s]))
                         + (LVFU(Diagonals[i][j]) * LVFU(x[j + s]));
            }
...
#endif
        {
            ...
            for (int j = lm - ((lm - srm) % 4); j < lm; j++) {
                ...
                for (int i = m - 1; i > 0; i--) {
                    int s = StartRows[i];
                    prod +=
                        (Diagonals[i][j - s] * x[j - s]) + (Diagonals[i][j] * x[j + s]);
                }
            ...
            for (int i = 0; i < m; i++) {
                ...
                if (sr == 0) {
                    for (int j = 0; j < srm; j++) {
                        Product[j] = a[j] * x[j];  // Separate, fairly simple treatment
                    ...
                    // Split the loop in 3 parts, so now the big one in the middle can be
                    // parallelized without race conditions updates 0 to sr - 1. Because
                    // sr is small (in the range of image-width) no benefit by omp
                    for (int j = 0; j < sr; j++) {
                        Product[j] +=
                            a[j] * x[j + sr];  // Contribution from upper triangle
                    }

                    // Updates sr to l - 1. Because sr is small and l is big, this loop is
                    // parallelized
                    for (int j = sr; j < srm; j++) {
                        Product[j] += a[j - sr] * x[j - sr]
                                      + a[j] * x[j + sr];  // Contribution from lower and
                                                           // upper triangle
                    }
                    ...
                    // Updates l to l + sr - 1. Because sr is small (in the range of
                    // image-width) no benefit by omp
                    for (int j = l; j < l + sr; j++) {
                        Product[j] +=
                            a[j - sr] * x[j - sr];  // Contribution from lower triangle
                    }
                }
            }
        }
...
bool MultiDiagonalSymmetricMatrix::CreateIncompleteCholeskyFactorization(int MaxFillAbove)
{
    if (m == 1) {
        printf(
            "Error in MultiDiagonalSymmetricMatrix::CreateIncompleteCholeskyFactorization: just one diagonal? Can you divide?\n");
        return false;
    }

    if (StartRows[0] != 0) {
        printf(
            "Error in MultiDiagonalSymmetricMatrix::CreateIncompleteCholeskyFactorization: main diagonal required to exist for this math.\n");
        return false;
    }

    // How many diagonals in the decomposition?
    MaxFillAbove++;  // Conceptually, now "fill" includes an existing diagonal. Simpler in
                     // the math that follows.
    int mic = 1;

    for (int ii = 1; ii < m; ii++) {
        mic += rtengine::min(StartRows[ii] - StartRows[ii - 1],
                             MaxFillAbove);  // Guaranteed positive since StartRows must
                                             // be created in increasing order.
    }
    ...
    if (!ic->CreateDiagonal(0, 0))
    {  // There's always a main diagonal in this type of decomposition.
        delete ic;
        return false;
    }
    mic = 1;
    ...
    for (int ii = 1; ii < m; ii++) {
        // Set j to the number of diagonals to be created corresponding to a diagonal on
        // this source matrix...
        int j = rtengine::min(StartRows[ii] - StartRows[ii - 1], MaxFillAbove);

        //...and create those diagonals. I want to take a moment to tell you about how
        //much I love minimalistic loops: very much.
        while (j-- != 0)
            if (!ic->CreateDiagonal(mic++, StartRows[ii] - j)) {
                // Beware of out of memory, possible for large, sparse problems if you ask
                // for too much fill.
                printf(
                    "Error in MultiDiagonalSymmetricMatrix::CreateIncompleteCholeskyFactorization: Out of memory. Ask for less fill?\n");
                delete ic;
                return false;
            }
    }
    ...
    for (int j = 0; j < n; j++) {
        ...
        // This is a loop over k from 1 to j, inclusive. We'll cover that by looping over
        // the index of the diagonals (s), and get k from it. The first diagonal is d (k =
        // 0), so skip that and have s start at 1. Cover all available s but stop if k
        // exceeds j.
        int s = 1;
        int k = icStartRows[s];
        ...
        if (UNLIKELY(d[j] == 0.0f)) {
            printf(
                "Error in MultiDiagonalSymmetricMatrix::CreateIncompleteCholeskyFactorization: division by zero. Matrix not decomposable.\n");
            delete ic;
            delete[] DiagMap;
            delete[] MaxIndizes;
            delete[] findmap;
        ...
        for (s = 1; s < icm; s++) {
            ...
            while (mapindex <= MaxIndizes[s] && (k = DiagMap[mapindex].k) <= j) {
                temp -= l[DiagMap[mapindex].sss][j - k] * l[DiagMap[mapindex].ss][j - k]
                        * d[j - k];
                mapindex++;
            }
...
{
    // We want to solve L D Lt x = b where D is a diagonal matrix described by
    // Diagonals[0] and L is a unit lower triagular matrix described by the rest of the
    // diagonals. Let D Lt x = y. Then, first solve L y = b.
    ...
    if (M != DIAGONALSP1) {  // can happen in theory
        for (int j = 0; j < N; j++) {
            float sub =
                b[j];  // using local var to reduce memory writes, gave a big speedup
            int i = 1;
            int c = j - s[i];
        ...
        for (int j = s[M - 1] + 1; j < N; j++) {
            ...
            for (int i = DIAGONALSP1 - 1; i > 0; i--)
            {  // using a constant upperbound allows the compiler to unroll this loop
...
// Took this one out of the while, so it can be parallelized now, which speeds up, because
...
#pragma omp parallel for
    ...
    if (M != DIAGONALSP1) {  // can happen in theory
        ...
        while (j-- > 0) {
            float sub =
                x[j];  // using local var to reduce memory writes, gave a big speedup
            int i = 1;
            int c = j + s[1];
...
EdgePreservingDecomposition::EdgePreservingDecomposition(int width, int height)
    : a0(nullptr), a_1(nullptr), a_w(nullptr), a_w_1(nullptr), a_w1(nullptr)
{
    w = width;
    h = height;
    n = w * h;
    ...
    if (!(A->CreateDiagonal(0, 0) && A->CreateDiagonal(1, 1)
          && A->CreateDiagonal(2, w - 1) && A->CreateDiagonal(3, w)
...
float* EdgePreservingDecomposition::CreateBlur(float* Source,
                                               float Scale,
                                               float EdgeStopping,
                                               int Iterates,
                                               float* Blur,
                                               bool UseBlurForEdgeStop)
{
    ...
    // Create the edge stopping function a, rotationally symmetric and just one instead of
    // (ax, ay). Maybe don't need Blur yet, so use its memory.
    float* RESTRICT a;
    float* RESTRICT g;
...
#pragma omp for
        ...
        for (int y = 0; y < h1; y++) {
            ...
            for (x = 0; x < w1 - 3; x += 4) {
                // Estimate the central difference gradient in the center of a four pixel
                // square. (gx, gy) is actually 2*gradient.
                gxv = (LVFU(rg[x + 1]) - LVFU(rg[x]))
                      + (LVFU(rg[x + w + 1]) - LVFU(rg[x + w]));
                gyv = (LVFU(rg[x + w]) - LVFU(rg[x]))
                      + (LVFU(rg[x + w + 1]) - LVFU(rg[x + 1]));
                // Apply power to the magnitude of the gradient to get the edge stopping
                // function.
                _mm_storeu_ps(
                    &a[x + w * y],
                    Scalev
                        * pow_F((zd5v * vsqrtf(gxv * gxv + gyv * gyv + sqrepsv)),
                                EdgeStoppingv));
            }
            ...
            for (; x < w1; x++) {
                ...
                a[x + w * y] =
                    Scale
                    * pow_F(0.5f * sqrtf(gx * gx + gy * gy + sqreps), -EdgeStopping);
            }
    ...
    /* Now setup the linear problem. I use the Maxima CAS, here's code for making an FEM
    formulation for the smoothness term: p(x, y) := (1 - x)*(1 - y); P(m, n) :=
    A[m][n]*p(x, y) + A[m + 1][n]*p(1 - x, y) + A[m + 1][n + 1]*p(1 - x, 1 - y) + A[m][n +
    1]*p(x, 1 - y); Integrate(f) := integrate(integrate(f, x, 0, 1), y, 0, 1);

        Integrate(diff(P(u, v), x)*diff(p(x, y), x) + diff(P(u, v), y)*diff(p(x, y), y));
        Integrate(diff(P(u - 1, v), x)*diff(p(1 - x, y), x) + diff(P(u - 1, v),
    y)*diff(p(1 - x, y), y)); Integrate(diff(P(u - 1, v - 1), x)*diff(p(1 - x, 1 - y), x)
    + diff(P(u - 1, v - 1), y)*diff(p(1 - x, 1 - y), y)); Integrate(diff(P(u, v - 1),
    x)*diff(p(x, 1 - y), x) + diff(P(u, v - 1), y)*diff(p(x, 1 - y), y)); So yeah. Use the
    numeric results of that to fill the matrix A.*/
...
#pragma omp parallel for
    ...
    for (int y = 0; y < h; y++) {
        ...
        for (int x = 0; x < w; x++, i++) {
            ...
            // Remember, only fill the lower triangle. Memory for upper is never made.
            // It's symmetric. Trust.
            if (x > 0 && y > 0) {
                ac = a[i - w - 1] / 6.0f;
                a_w_1[i - w - 1] -= 2.0f * ac;
                a_w[i - w] -= ac;
    ...
    // Solve & return.
    bool success = A->CreateIncompleteCholeskyFactorization(
        1);  // Fill-in of 1 seems to work really good. More doesn't really help and less
             // hurts (slightly).

    if (!success) {
        fprintf(stderr, "Error: Tonemapping has failed.\n");
        memset(Blur, 0,
               sizeof(float) * n);  // On failure, set the blur to zero.  This is
                                    // subsequently exponentiated in CompressDynamicRange.
        return Blur;
    }
    ...
    SparseConjugateGradient(A->PassThroughVectorProduct, Source, n, false, Blur, 0.0f,
                            (void*)A, Iterates, A->PassThroughCholeskyBackSolve);
    A->KillIncompleteCholeskyFactorization();
    return Blur;
}

float* EdgePreservingDecomposition::CreateIteratedBlur(float* Source,
                                                       float Scale,
                                                       float EdgeStopping,
                                                       int Iterates,
                                                       int Reweightings,
...
void EdgePreservingDecomposition::CompressDynamicRange(float* Source,
                                                       float Scale,
                                                       float EdgeStopping,
...
{
    if (w < 300 && h < 300)
    {  // set number of Reweightings to zero for small images (thumbnails). We could try
       // to find a better solution here.
        Reweightings = 0;
    }

    // Small number intended to prevent division by zero. This is different from the eps
    // in CreateBlur.
...
#pragma omp parallel for
    ...
    // Blur. Also setup memory for Compressed (we can just use u since each element of u
    // is used in one calculation).
    float* u = CreateIteratedBlur(Source, Scale, EdgeStopping, Iterates, Reweightings);

    // Apply compression, detail boost, unlogging. Compression is done on the logged data
    // and detail boost on unlogged.
    float temp;
