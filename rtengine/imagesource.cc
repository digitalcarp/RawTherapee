...
void ImageSource::getAutoWBMultipliersItcGreen(procparams::ProcParams& params,
                                               bool forcewbgrey,
                                               int kcam,
                                               double greenitc,
                                               bool extra,
                                               ...
                                               double& bm)
{
    ...
    if (isRAW()) {  // only with Raw files
        ...
        if (!forcewbgrey && (tempref0bias0 < 3300.f)
            && (greenref < 1.13f && greenref > 0.88f))
        {  // seems good with temp and green...To fixe...limits 1.13 and 0.88
            if (settings->verbose) {
        ...
        if (autowb1) {
            // alternative to camera if camera settings out, using autowb grey to find new
            // ref, then mixed with camera
            //  kcam = 0;
            ...
            if ((greenref > 1.5f || tempref0bias < 3300.f || tempref0bias > 7700.f
                 || forcewbgrey)
                && kcam != 1 && !params.wb.itcwb_sampling)
            {  // probably camera out to adjust...
                getAutoWBMultipliersitc(extra, tempref0bias, greenref, tempitc, greenitc,
                                        temp0, delta, bia, dread, kcam, nocam, studgood,
                                        minchrom, kmin, minhist, maxhist, 0, 0, fh, fw, 0,
                                        0, fh, fw, rm, gm, bm, params.wb, params.icm,
                                        params.raw, params.toneCurve);
                ...
                if (gre > 1.5f && !forcewbgrey) {          // probable wrong value
                    ...
                    gre = 0.5f
                          + 0.5f
                                * LIM(gre, 0.9f,
                                      1.1f);  // empirical formula in case  system out
                } else {
                    if (!forcewbgrey) {
                        ...
                        gre =
                            0.5f * grepro
                            + 0.5f
                                  * LIM(gre, 0.9f, 1.1f);  // empirical green between
                                                           // green camera and autowb grey

                        if (abs(deltemp) < 400.) {  // arbitraries thresholds to refine
                            tem = 0.3 * tem
                                  + 0.7 * tempref0bias;  // find a mixed value between
                                                         // camera and auto grey
                            ...
                            tem = 0.4 * tem
                                  + 0.6 * tempref0bias;  // find a mixed value between
                                                         // camera and auto grey
                            ...
                            if ((pargre && pargref) || (!pargre && !pargref)) {
                                tem =
                                    0.45 * tem
                                    + 0.55 * tempref0bias;  // find a mixed value between
                                                            // camera and auto grey
                            }

                            if (pargre && !pargref) {
                                tem = 0.7 * tem
                                      + 0.3 * tempref0bias;  // find a mixed value between
                                                             // camera and auto grey
                            }
                            ...
                            if (tem >= 4500.f) {
                                if ((pargre && pargref) || (!pargre && !pargref)) {
                                    tem = 0.7 * tem
                                          + 0.3 * tempref0bias;  // find a mixed value
                                                                 // between camera and
                                                                 // auto grey
                                }
                                ...
                                if (pargre && !pargref) {
                                    tem = 0.8 * tem
                                          + 0.2 * tempref0bias;  // find a mixed value
                                                                 // between camera and
                                                                 // auto grey
                                }
                                ...
                                if (!pargre && pargref) {
                                    tem = 0.3 * tem
                                          + 0.7 * tempref0bias;  // find a mixed value
                                                                 // between camera and
                                                                 // auto grey
                                }
                            ...
                            if (tem >= 10000.f) {
                                tem =
                                    0.99 * tem
                                    + 0.01 * tempref0bias;  // find a mixed value between
                                                            // camera and auto grey
                                nocam = 9;
                            } else {
                                if ((pargre && pargref) || (!pargre && !pargref)) {
                                    tem = 0.45 * tem
                                          + 0.55 * tempref0bias;  // find a mixed value
                                                                  // between camera and
                                                                  // auto grey
                                }
                ...
                if (settings->verbose) {
                    printf(
                        "Using new references AWB grey or mixed  Enable Extra - temgrey=%f gregrey=%f tempitc=%f nocam=%i\n",
    ...
    if (isRAW() || !skipRecalculate) {
        greenitc = 1.;
        auto currWBitc = getWB();
        currWBitc = currWBitc.convertObserver(
            params.wb
                .observer);  // change the temp/green couple with the same multipliers
        ...
        if ((greenref > 1.5f || tempref0bias < 3300.f || tempref0bias > 7700.f
             || forcewbgrey)
            && autowb1 && kcam != 1 && !params.wb.itcwb_sampling)
        {  // probably camera out to adjust = greenref ? tempref0bias ?
        ...
        getAutoWBMultipliersitc(extra, tempref, greenref, tempitc, greenitc, temp0, delta,
                                bia, dread, kcam, nocam, studgood, minchrom, kmin,
                                minhist, maxhist, 0, 0, fh, fw, 0, 0, fh, fw, rm, gm, bm,
                                params.wb, params.icm, params.raw, params.toneCurve);
        ...
        autoWB.update(rm, gm, bm, params.wb.equal, params.wb.observer,
                      0.);  // params.wb.tempBias already used before
    }
}
