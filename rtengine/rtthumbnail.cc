 ...
 */
void adjustBlackLevels(float cblack[4],
                       rtengine::eSensorType sensorType,
                       const rtengine::RAWParams* rawParams)
{
    if (!rawParams) {
        return;
    }
 ...
 */
void calculate_scale_mul(float scale_mul[4],
                         ...
                         const rtengine::RawImage* ri)
{
    ...
    if (isMono || ri->get_colors() == 1) {
        ...
        float maxpremul =
            std::max(std::max(std::max(pre_mul[0], pre_mul[1]), pre_mul[2]), pre_mul[3]);
...
void scale_colors(rtengine::RawImage* ri,
                  float scale_mul[4],
                  float cblack[4],
                  bool multiThread)
{
...
#pragma omp parallel for if (multiThread)
        ...
        for (int row = 0; row < height; ++row) {
            ...
            for (; col < width - 1; col += 2) {
                ...
                float val1;
                if (isFloat) {
                    val0 = float_raw_image[(row + top_margin) * raw_width + col
                                           + left_margin];
                    val1 = float_raw_image[(row + top_margin) * raw_width + col
                                           + left_margin + 1];
                } else {
        ...
        for (int row = 0; row < height; ++row) {
            ...
            for (; col < width - 5; col += 6) {
                for (int i = 0; i < 6; ++i) {
                    ...
                    float val;
                    if (isFloat) {
                        val = float_raw_image[(row + top_margin) * raw_width + col + i
                                              + left_margin];
                    } else {
            ...
            for (; col < width; ++col) {  // remaining columns
                ...
                float val;
                if (isFloat) {
                    val = float_raw_image[(row + top_margin) * raw_width + col
                                          + left_margin];
                } else {
        ...
        for (int row = 0; row < height; ++row) {
            for (int col = 0; col < width; ++col) {
                for (int i = 0; i < colors; ++i) {
                    float val = float_raw_image[colors
                                                    * ((row + top_margin) * raw_width
                                                       + col + left_margin)
                                                + i];
                    val -= cblack[i];
                    val *= scale_mul[i];
...
Thumbnail* Thumbnail::loadFromImage(const Glib::ustring& fname,
                                    int& w,
                                    int& h,
                                    int fixwh,
                                    double wbEq,
                                    ...
                                    bool inspectorMode)
{
    ...
    if (inspectorMode) {
        // Special case, meaning that we want a full sized thumbnail image (e.g. for the
        // Inspector feature)
        w = img->getWidth();
        h = img->getHeight();
        tpp->scale = 1.;
    ...
    if (inspectorMode) {
        ...
        if (img->getType() == rtengine::sImage8) {
            ...
            image->computeHistogramAutoWB(avg_r, avg_g, avg_b, n, tpp->aeHistogram,
        ...
        ipf.getAutoExp(tpp->aeHistogram, tpp->aeHistCompression, 0.02,
                       tpp->aeExposureCompensation, tpp->aeLightness, tpp->aeContrast,
                       tpp->aeBlack, tpp->aeHighlightCompression,
                       tpp->aeHighlightCompressionThreshold);
        tpp->aeValid = true;
        ...
        if (n > 0) {
            ...
            cTemp.mul2temp(tpp->redAWBMul, tpp->greenAWBMul, tpp->blueAWBMul,
                           tpp->wbEqual, tpp->wbObserver, tpp->autoWBTemp,
                           tpp->autoWBGreen);
        }
...
Image8*
load_inspector_mode(const Glib::ustring& fname, eSensorType& sensorType, int& w, int& h)
{
    BENCHFUN
    ...
    neutral.raw.xtranssensor.method =
    ...
    float greendeha = 0.f;
    float bluedeha = 0.f;
    src.preprocess(neutral.raw, neutral.lensProf, neutral.coarse, reddeha, greendeha,
                   bluedeha, false);
...
Thumbnail* Thumbnail::loadQuickFromRaw(const Glib::ustring& fname,
                                       ...
                                       bool forHistogramMatching)
{
    ...
    if (inspectorMode && !forHistogramMatching
        && settings->thumbnail_inspector_mode == Settings::ThumbnailInspectorMode::RAW)
    {
    ...
    if (inspectorMode) {
        ...
        if (!forHistogramMatching
            && settings->thumbnail_inspector_mode
                   == Settings::ThumbnailInspectorMode::RAW_IF_NOT_JPEG_FULLSIZE
            && float(std::max(w, h)) / float(std::max(ri->get_width(), ri->get_height()))
                   < 0.9f)
        {
            delete img;
            delete ri;
    ...
    if (rotate && ri->get_rotateDegree() > 0) {
        ...
        // Leaf .mos, Mamiya .mef and Phase One .iiq files have thumbnails already
        // rotated.
...
Thumbnail* Thumbnail::loadFromRaw(const Glib::ustring& fname,
                                  eSensorType& sensorType,
                                  int& w,
                                  int& h,
                                  int fixwh,
                                  ...
                                  bool forHistogramMatching)
{
    ...
    if (ri->get_profileLen())
        tpp->embProfile = cmsOpenProfileFromMem(
            ri->get_profile(),
    ...
    scale_colors(
        ri, scale_mul, cblack,
        forHistogramMatching);  // enable multithreading when forHistogramMatching is true
    ...
    // tpp->defGain = 1.0 / min(ri->get_pre_mul(0), ri->get_pre_mul(1),
    // ri->get_pre_mul(2));
    tpp->defGain = max(scale_mul[0], scale_mul[1], scale_mul[2], scale_mul[3])
                   / min(scale_mul[0], scale_mul[1], scale_mul[2], scale_mul[3]);
    tpp->defGain *= std::pow(2, ri->getBaselineExposure());

    tpp->scaleGain =
        scale_mul[0]
        / pre_mul[0];  // can be used to reconstruct scale_mul later in processing
    ...
    if (ri->get_FujiWidth() != 0) {
        if (fixwh == 1) {  // fix height, scale width
            skip = ((ri->get_height() - ri->get_FujiWidth()) / sqrt(0.5) - firstgreen - 1)
                   / h;
        } else {
    ...
    if (ri->getSensorType() == ST_BAYER) {
        ...
        for (int row = 1, y = 0; row < height - 1 && y < tmph; row += vskip, y++) {
            ...
            for (int col = firstgreen, x = 0; col < width - 1 && x < tmpw;
                 col += hskip, x++)
            {
        ...
        if (ri->getSensorType() == ST_FUJI_XTRANS) {
            ...
            if ((ri->get_maker() == "Sigma" || ri->get_maker() == "Pentax"
                 || ri->get_maker() == "Sony")
                && ri->DNGVERSION())
            {  // Hack to prevent sigma dng files from crashing
            ...
            for (int row = 1 + top_margin; row < iheight + top_margin - 1 && y < hmax;
                 row += vskip, y++)
            {
                rofs = row * iwidth;
                ...
                for (int col = firstgreen; col < iwidth + left_margin - 1 && x < wmax;
                     col += hskip, x++)
                {
                    int ofs = rofs + col;
                    tmpImg->r(y, x) = image[ofs][0];
    ...
    if (ri->get_FujiWidth() != 0) {
        ...
        for (int row = 0; row < high; row++)
            for (int col = 0; col < wide; col++) {
                ...
                double fc = c - uc;
                fImg->r(row, col) =
                    (tmpImg->r(ur, uc) * (1 - fc) + tmpImg->r(ur, uc + 1) * fc) * (1 - fr)
                    + (tmpImg->r(ur + 1, uc) * (1 - fc) + tmpImg->r(ur + 1, uc + 1) * fc)
                          * fr;
                fImg->g(row, col) =
                    (tmpImg->g(ur, uc) * (1 - fc) + tmpImg->g(ur, uc + 1) * fc) * (1 - fr)
                    + (tmpImg->g(ur + 1, uc) * (1 - fc) + tmpImg->g(ur + 1, uc + 1) * fc)
                          * fr;
                fImg->b(row, col) =
                    (tmpImg->b(ur, uc) * (1 - fc) + tmpImg->b(ur, uc + 1) * fc) * (1 - fr)
                    + (tmpImg->b(ur + 1, uc) * (1 - fc) + tmpImg->b(ur + 1, uc + 1) * fc)
                          * fr;
            }
    ...
    const bool rotate_90 =
        rotate && (ri->get_rotateDegree() == 90 || ri->get_rotateDegree() == 270);
    ...
    if (!forHistogramMatching) {  // we don't need this for histogram matching
        ...
        const double camWb[3] = { tpp->camwbRed / compression,
                                  tpp->camwbGreen / compression,
                                  tpp->camwbBlue / compression };
        ...
        tpp->redAWBMul = ri->get_rgb_cam(0, 0) * reds + ri->get_rgb_cam(0, 1) * greens
                         + ri->get_rgb_cam(0, 2) * blues;
        tpp->greenAWBMul = ri->get_rgb_cam(1, 0) * reds + ri->get_rgb_cam(1, 1) * greens
                           + ri->get_rgb_cam(1, 2) * blues;
        tpp->blueAWBMul = ri->get_rgb_cam(2, 0) * reds + ri->get_rgb_cam(2, 1) * greens
                          + ri->get_rgb_cam(2, 2) * blues;
        tpp->wbEqual = wbEq;
        tpp->wbTempBias = 0.0;
        tpp->wbObserver = wbObserver;

        ColorTemp cTemp;
        cTemp.mul2temp(tpp->redAWBMul, tpp->greenAWBMul, tpp->blueAWBMul, tpp->wbEqual,
                       tpp->wbObserver, tpp->autoWBTemp, tpp->autoWBGreen);
    }
...
IImage8* Thumbnail::processImage(const procparams::ProcParams& params,
                                 eSensorType sensorType,
                                 int rheight,
                                 ...
                                 bool forHistogramMatching)
{
    ...
    if (wbEqual < (params.wb.equal - 5e-4) || wbEqual > (params.wb.equal + 5e-4)
        || wbTempBias < (params.wb.tempBias - 5e-4)
        || wbTempBias > (params.wb.tempBias + 5e-4) || wbObserver != params.wb.observer)
    {
        ...
        ColorTemp cTemp;
        cTemp.mul2temp(redAWBMul, greenAWBMul, blueAWBMul, wbEqual, wbObserver,
                       autoWBTemp, autoWBGreen);
        autoWBTemp += autoWBTemp * wbTempBias;
    }

    // compute WB multipliers
    ColorTemp currWB = ColorTemp(params.wb.temperature, params.wb.green, params.wb.equal,
    ...
    if (!params.wb.enabled) {
        ...
        double cam_r = colorMatrix[0][0] * camwbRed + colorMatrix[0][1] * camwbGreen
                       + colorMatrix[0][2] * camwbBlue;
        double cam_g = colorMatrix[1][0] * camwbRed + colorMatrix[1][1] * camwbGreen
                       + colorMatrix[1][2] * camwbBlue;
        double cam_b = colorMatrix[2][0] * camwbRed + colorMatrix[2][1] * camwbGreen
                       + colorMatrix[2][2] * camwbBlue;
        ...
        if (params.wb.compat_version == 1 && !isRaw) {
            ...
            currWB.update(1., 1., 1., params.wb.equal, params.wb.observer,
                          params.wb.tempBias);
        } else {
    ...
    // The RAW exposure is not reflected since it's done in preprocessing. If we only have
    // e.g. the cached thumb, that is already preprocessed. So we simulate the effect here
    // roughly my modifying the exposure accordingly
    if (isRaw) {
    ...
    if (params.coarse.rotate == 90 || params.coarse.rotate == 270) {
        rwidth = rheight;
        rheight = int(size_t(thumbImg->getHeight()) * size_t(rwidth)
                      / size_t(thumbImg->getWidth()));
    } else {
        rwidth = int(size_t(thumbImg->getWidth()) * size_t(rheight)
                     / size_t(thumbImg->getHeight()));
    }
...
#pragma omp simd
        ...
        for (int j = 0; j < rwidth; j++) {
            ...
            if (params.toneCurve.hrenabled && red > MAXVALF && blue > MAXVALF) {
                baseImg->r(i, j) = baseImg->g(i, j) = baseImg->b(i, j) =
                    CLIP((red + green + blue) / 3.f);
            } else {
    ...
    ImProcFunctions ipf(
        &params,
        forHistogramMatching);  // enable multithreading when forHistogramMatching is true
    ipf.setScale(sqrt(double(fw * fw + fh * fh))
                 / sqrt(double(thumbImg->getWidth() * thumbImg->getWidth()
                               + thumbImg->getHeight() * thumbImg->getHeight()))
                 * scale);
    ipf.updateColorProfiles(ICCStore::getInstance()->getDefaultMonitorProfileName(),
                            RenderingIntent(settings->monitorIntent), false, false);

    // Process film negative BEFORE colorspace conversion, if needed
    if (params.filmNegative.enabled
        && params.filmNegative.backCompat == FilmNegativeParams::BackCompat::CURRENT
    ...
    if (isRaw) {
        ...
        RawImageSource::colorSpaceConversion(baseImg, params.icm, currWB, pre_mul,
                                             embProfile, camProfile, cam2xyz, camName,
                                             metadata->getFileName());
    } else {
        StdImageSource::colorSpaceConversion(baseImg, params.icm, embProfile,
                                             thumbImg->getSampleFormat());
    }
    ...
    if (ipf.needsTransform(origFW * tscale + 0.5, origFH * tscale + 0.5, 0, metadata)) {
        ...
        ipf.transform(baseImg, trImg, 0, 0, 0, 0, fw, fh, origFW * tscale + 0.5,
                      origFH * tscale + 0.5, metadata, 0,
                      true);  // Raw rotate degree not detectable here
        delete baseImg;
        baseImg = trImg;
    }
    ...
    if (params.toneCurve.autoexp) {
        if (aeValid) {
            ...
            ipf.getAutoExp(aeHistogram, aeHistCompression, 0.02, expcomp, bright, contr,
                           black, hlcompr, hlcomprthresh);
        }
    }
    ...
    LUTf lumacurve(32770, 0);  // lumacurve[32768] and lumacurve[32769] will be set to
    ...
    CurveFactory::complexCurve(
        ...
        bright, contr, params.toneCurve.curve, params.toneCurve.curve2, hist16, curve1,
        curve2, curve, dummy, customToneCurve1, customToneCurve2, 16);
    ...
    if (params.colorToning.enabled) {
        ...
        clToningcurve(65536);
        CurveFactory::diagonalCurve2Lut(params.colorToning.clcurve, clToningcurve,
                                        scale == 1 ? 1 : 16);
    ...
    if (params.blackwhite.enabled) {
        CurveFactory::curveBW(params.blackwhite.beforeCurve, params.blackwhite.afterCurve,
                              hist16, dummy, customToneCurvebw1, customToneCurvebw2, 16);
    }
    ...
    {  // for colortoning evaluation of saturation settings
        ...
        float satp = ((moyS + 1.5f * eqty) - 0.3f)
                     / 0.7f;  // 1.5 sigma ==> 93% pixels with high saturation -0.3 / 0.7
                              // convert to Hombre scale
        ...
        satLimitOpacity =
            100.f * (moyS - 0.85f * eqty);  //-0.85 sigma==>20% pixels with low saturation
    }

    autor = autog = autob =
        -9000.f;  // This will ask to compute the "auto" values for the B&W tool
    ...
    if (isRaw) {
        cmsHPROFILE dummy;
        RawImageSource::findInputProfile(params.icm.inputProfile, nullptr, camName,
                                         metadata->getFileName(), &dcpProf, dummy);

        if (dcpProf) {
            dcpProf->setStep2ApplyState(params.icm.workingProfile, params.icm.toneCurve,
                                        params.icm.applyLookTable,
                                        params.icm.applyBaselineExposureOffset, as);
        }
    }
    ...
    ipf.rgbProc(baseImg, labView, nullptr, curve1, curve2, curve,
                ...
                satLimitOpacity, ctColorCurve, ctOpacityCurve, opautili, clToningcurve,
                cl2Toningcurve, customToneCurve1, customToneCurve2, customToneCurvebw1,
                customToneCurvebw2, rrm, ggm, bbm, autor, autog, autob, expcomp, hlcompr,
                hlcomprthresh, dcpProf, as, histToneCurve);
    ...
    CurveFactory::complexLCurve(params.labCurve.brightness, params.labCurve.contrast,
                                params.labCurve.lcurve, hist16, lumacurve, dummy, 16,
                                utili);

    const bool clcutili =
        CurveFactory::diagonalCurve2Lut(params.labCurve.clcurve, clcurve, 16);
    ...
    CurveFactory::complexsgnCurve(autili, butili, ccutili, cclutili,
                                  ...
                                  curve1, curve2, satcurve, lhskcurve, 16);
    ...
    ipf.shadowsHighlights(labView, params.sh.enabled, params.sh.lab, params.sh.highlights,
                          params.sh.shadows, params.sh.radius, 16, params.sh.htonalwidth,
        ...
        && params.icm.trcExp)
    {
        ...
        ipf.workingtrc(0, tmpImage1.get(), tmpImage1.get(), GW, GH, -5, prof, 2.4,
                       12.92310, 0, ill, 0, 0, rdx, rdy, grx, gry, blx, bly, meanx, meany,
                       meanxe, meanye, dummy, true, false, false);
        ipf.workingtrc(0, tmpImage1.get(), tmpImage1.get(), GW, GH, 5, prof, gamtone,
                       slotone, 0, illum, prim, locprim, rdx, rdy, grx, gry, blx, bly,
                       meanx, meany, meanxe, meanye, dummy, false, true, true);
...
#pragma omp parallel for
    ...
    if (params.colorappearance.enabled) {
        CurveFactory::curveLightBrightColor(
            ...
            customColCurve2, customColCurve3, 16);
        ...
        ipf.ciecam_02float(cieView, adap, 1, 2, labView, &params, customColCurve1,
                           customColCurve2, customColCurve3, dummy, dummy,
                           CAMBrightCurveJ, CAMBrightCurveQ, CAMMean, 5, sk, execsharp, d,
                           dj, yb, rtt);
        delete cieView;
    }
    ...
    if (params.crop.enabled) {
        ...
        for (int i = 0; i < fh; ++i) {
            for (int j = 0; j < fw; ++j) {
                if (i < params.crop.y * myscale
                    || i > (params.crop.y + params.crop.h) * myscale
                    || j < params.crop.x * myscale
                    || j > (params.crop.x + params.crop.w) * myscale)
                {
                    readyImg->data[ix++] /= 3;
                    readyImg->data[ix++] /= 3;
                    readyImg->data[ix++] /= 3;
...
int Thumbnail::getImageWidth(const procparams::ProcParams& params,
                             int rheight,
                             float& ratio)
{
    if (!thumbImg) {
        return 0;  // Can happen if thumb is just building and GUI comes in with resize
                   // wishes
    }
...
void Thumbnail::getCamWB(double& temp, double& green, StandardObserver observer)
{
    ...
    double cam_b = colorMatrix[2][0] * camwbRed + colorMatrix[2][1] * camwbGreen
                   + colorMatrix[2][2] * camwbBlue;
    ColorTemp currWB = ColorTemp(cam_r, cam_g, cam_b, 1.0,
                                 observer);  // we do not take the equalizer into account
                                             // here, because we want camera's WB
    temp = currWB.getTemp();
    green = currWB.getGreen();
}

void Thumbnail::getAutoWB(double& temp,
                          double& green,
                          double equal,
                          double tempBias,
...
void Thumbnail::applyAutoExp(procparams::ProcParams& params)
{
    ...
    if (params.toneCurve.autoexp && aeHistogram) {
        ...
        ipf.getAutoExp(aeHistogram, aeHistCompression, params.toneCurve.clip,
...
void Thumbnail::getSpotWB(const procparams::ProcParams& params,
                          int xp,
                          int yp,
                          int rect,
                          double& rtemp,
                          double& rgreen)
{
    ...
    double rm =
        colorMatrix[0][0] * reds + colorMatrix[0][1] * greens + colorMatrix[0][2] * blues;
    double gm =
        colorMatrix[1][0] * reds + colorMatrix[1][1] * greens + colorMatrix[1][2] * blues;
    double bm =
        colorMatrix[2][0] * reds + colorMatrix[2][1] * greens + colorMatrix[2][2] * blues;
...
unsigned char* Thumbnail::getGrayscaleHistEQ(int trim_width)
{
    ...
    // to utilize the 8 bit color range of the thumbnail we brighten it and apply gamma
    // correction
    unsigned char* tmpdata = new unsigned char[thumbImg->getHeight() * trim_width];
    int ix = 0;

    if (gammaCorrected) {
        // if it's gamma correct (usually a RAW), we have the problem that there is a lot
        // noise etc. that makes the maximum way too high. Strategy is limit a certain
        // percent of pixels so the overall picture quality when scaling to 8 bit is way
        // better
        ...
        unsigned long cutoff =
            thumbImg->getHeight() * thumbImg->getHeight() * 4 * BurnOffPct;
        ...
        if (thumbImg->getType() == sImage8) {
            ...
            for (int i = 0; i < thumbImg->getHeight(); i++)
                for (int j = (thumbImg->getWidth() - trim_width) / 2;
                     j < trim_width + (thumbImg->getWidth() - trim_width) / 2; j++)
                {
                    ...
                    int r =
                        Color::gammatabThumb[min(r_, static_cast<unsigned short>(max_))
                                                 * scaleForSave
                                             >> 13];
                    int g =
                        Color::gammatabThumb[min(g_, static_cast<unsigned short>(max_))
                                                 * scaleForSave
                                             >> 13];
                    int b =
                        Color::gammatabThumb[min(b_, static_cast<unsigned short>(max_))
                                                 * scaleForSave
                                             >> 13];
        ...
        if (thumbImg->getType() == sImage8) {
            ...
            for (int i = 0; i < image->getHeight(); i++)
                for (int j = (image->getWidth() - trim_width) / 2;
                     j < trim_width + (image->getWidth() - trim_width) / 2; j++)
                {
    ...
    for (int i = 0; i < 256; i++) {
        ...
        if (cdf_min != -1) {
            hist[i] =
                (cdf - cdf_min) * 255 / ((thumbImg->getHeight() * trim_width) - cdf_min);
        }
    }
...
bool Thumbnail::readImage(const Glib::ustring& fname)
{
    ...
    char imgType[31];  // 30 -> arbitrary size, but should be enough for all image type's
                       // name
    fgets(imgType, 30, f);
    imgType[strlen(imgType) - 1] =
    ...
    try {
        ...
        if (keyFile.has_group("LiveThumbData")) {
            ...
            if (keyFile.has_key("LiveThumbData", "AEHistCompression")) {
                aeHistCompression =
                    keyFile.get_integer("LiveThumbData", "AEHistCompression");
            }

            aeValid = true;
            if (keyFile.has_key("LiveThumbData", "AEExposureCompensation")) {
                aeExposureCompensation =
                    keyFile.get_double("LiveThumbData", "AEExposureCompensation");
            } else {
                aeValid = false;
            }
            ...
            if (keyFile.has_key("LiveThumbData", "AEHighlightCompressionThreshold")) {
                aeHighlightCompressionThreshold = keyFile.get_integer(
                    "LiveThumbData", "AEHighlightCompressionThreshold");
            } else {
                aeValid = false;
            }
            ...
            if (keyFile.has_key("LiveThumbData", "ColorMatrix")) {
                std::vector<double> cm =
                    keyFile.get_double_list("LiveThumbData", "ColorMatrix");
                int ix = 0;
        ...
        if (settings->verbose) {
            printf(
                "Thumbnail::readData / Error code %d while reading values from \"%s\":\n%s\n",
                err.code(), fname.c_str(), err.what().c_str());
        }
    } catch (...) {
        if (settings->verbose) {
            printf(
                "Thumbnail::readData / Unknown exception while trying to load \"%s\"!\n",
                fname.c_str());
        }
    }
...
bool Thumbnail::writeData(const Glib::ustring& fname)
{
    ...
    try {
        ...
        keyFile.set_double("LiveThumbData", "AEExposureCompensation",
                           aeExposureCompensation);
        ...
        keyFile.set_integer("LiveThumbData", "AEHighlightCompression",
                            aeHighlightCompression);
        keyFile.set_integer("LiveThumbData", "AEHighlightCompressionThreshold",
                            aeHighlightCompressionThreshold);
        ...
        if (settings->verbose) {
            printf(
                "Thumbnail::writeData / Error code %d while reading values from \"%s\":\n%s\n",
                err.code(), fname.c_str(), err.what().c_str());
        }
    } catch (...) {
        if (settings->verbose) {
            printf(
                "Thumbnail::writeData / Unknown exception while trying to save \"%s\"!\n",
                fname.c_str());
        }
    }
    ...
    if (!f) {
        if (settings->verbose) {
            printf(
                "Thumbnail::writeData / Error: unable to open file \"%s\" with write access!\n",
                fname.c_str());
        }
