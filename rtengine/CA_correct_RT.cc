...
//  copyright (c) for improvements (speedups, iterated correction and avoid colour shift)
//  2018 Ingo Weyrich <heckflosse67@gmx.de>
//
...
float* RawImageSource::CA_correct_RT(bool autoCA,
                                     ...
                                     bool measure)
{
    ...
    if (measure) {
        std::cout << "CA correcting " << W << "x" << H << " image with " << chunkSize
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {
            if (fc(cfa, i, j) == 3) {
                std::cout << "CA correction supports only RGB Colour filter arrays"
                          << std::endl;
                return buffer;
            }
        }
...
#pragma omp parallel for
    ...
    if (!buffer) {
        buffer = static_cast<float*>(
            malloc((height * width + vblsz * hblsz * (2 * 2 + 1)) * sizeof(float)));
    }
    float* Gtmp = buffer;
    ...
    memset(blockwt, 0,
           static_cast<unsigned long>(vblsz) * hblsz * (2 * 2 + 1) * sizeof(float));
    ...
    const size_t iterations = autoCA ? std::max<size_t>(autoIterations, 1) : 1;
    ...
    for (size_t it = 0; it < iterations && processpasstwo; ++it) {
        ...
        constexpr float eps = 1e-5f, eps2 = 1e-10f;  // tolerance to avoid dividing by
                                                     // zero
...
#pragma omp parallel
#endif
        {
            ...
            constexpr int buffersize =
                sizeof(float) * ts * ts + 8 * sizeof(float) * ts * tsh + 8 * 64 + 63;
            constexpr int buffersizePassTwo =
                sizeof(float) * ts * ts + 4 * sizeof(float) * ts * tsh + 4 * 64 + 63;
            char* const bufferThr =
                (char*)malloc((autoCA && !fitParamsSet) ? buffersize : buffersizePassTwo);

            char* const data = (char*)((uintptr_t(bufferThr) + uintptr_t(63)) / 64 * 64);

            // shift the beginning of all arrays but the first by 64 bytes to avoid cache
            // miss conflicts on CPUs which have <= 4-way associative L1-Cache
            ...
            if (autoCA && !fitParamsSet) {
                ...
                float* rbhpfv = (float*)(data + 2 * sizeof(float) * ts * ts
                ...
                float* rblpfv = (float*)(data + 3 * sizeof(float) * ts * ts
                ...
                float* grblpfv = (float*)(data + 4 * sizeof(float) * ts * ts
...
#pragma omp for collapse(2) schedule(dynamic, chunkSize) nowait
                ...
                for (int top = -border; top < height; top += ts - border2) {
                    for (int left = -border; left < width - (W & 1); left += ts - border2)
                    {
                        ...
                        const int ccmax =
                            (right > width - (W & 1)) ? width - (W & 1) - left : cc1;
                        ...
                        for (int rr = rrmin; rr < rrmax; rr++) {
                            ...
                            for (; cc < ccmax - 7; cc += 8, col += 8, indx1 += 8) {
                                ...
                                vfloat nonGreenv =
                                    _mm_shuffle_ps(val1, val2, _MM_SHUFFLE(2, 0, 2, 0));
                        ...
                        if (rrmin > 0) {
                            for (int rr = 0; rr < border; rr++) {
                                for (int cc = ccmin; cc < ccmax; cc++) {
                                    int c = fc(cfa, rr, cc);
                                    rgb[c][(rr * ts + cc) >> ((c & 1) ^ 1)] =
                                        rgb[c]
                                           [((border2 - rr) * ts + cc) >> ((c & 1) ^ 1)];
                                }
                            }
                        }
                        ...
                        if (rrmax < rr1) {
                            for (int rr = 0; rr < border; rr++) {
                                for (int cc = ccmin; cc < ccmax; cc++) {
                                    int c = fc(cfa, rr, cc);
                                    rgb[c][((rrmax + rr) * ts + cc) >> ((c & 1) ^ 1)] =
                                        rawData[(height - rr - 2)][left + cc] / 65535.f;
                                }
                            }
                        }
                        ...
                        if (ccmax < cc1) {
                            for (int rr = rrmin; rr < rrmax; rr++) {
                                for (int cc = 0; cc < border; cc++) {
                                    int c = fc(cfa, rr, cc);
                                    rgb[c][(rr * ts + ccmax + cc) >> ((c & 1) ^ 1)] =
                                        rawData[(top + rr)][(width - cc - 2)] / 65535.f;
                                }
                            }
                        }
                        ...
                        if (rrmin > 0 && ccmin > 0) {
                            for (int rr = 0; rr < border; rr++) {
                                for (int cc = 0; cc < border; cc++) {
                                    int c = fc(cfa, rr, cc);
                                    rgb[c][(rr * ts + cc) >> ((c & 1) ^ 1)] =
                                        rawData[border2 - rr][border2 - cc] / 65535.f;
                                }
                            }
                        }
                            ...
                            for (int rr = 0; rr < border; rr++) {
                                for (int cc = 0; cc < border; cc++) {
                                    ...
                                    rgb[c][((rrmax + rr) * ts + ccmax + cc)
                                           >> ((c & 1) ^ 1)] =
                                        rawData[(height - rr - 2)][(width - cc - 2)]
                                        / 65535.f;
                                }
                            }
                        }
                        ...
                        if (rrmax < rr1 && ccmin > 0) {
                            for (int rr = 0; rr < border; rr++) {
                                for (int cc = 0; cc < border; cc++) {
                                    int c = fc(cfa, rr, cc);
                                    rgb[c][((rrmax + rr) * ts + cc) >> ((c & 1) ^ 1)] =
                                        rawData[(height - rr - 2)][(border2 - cc)]
                                        / 65535.f;
                                }
                            }
                        }
                        ...
                        for (int rr = 3; rr < rr1 - 3; rr++) {
                            ...
                            for (; cc < cc1 - 9; cc += 8, indx += 8) {
                                ...
                                vfloat wtuv =
                                    onev
                                    / SQRV(temp1v
                                           + vabsf(rgbcv - LVFU(rgb[c][(indx - v2) >> 1]))
                                           + vabsf(rgb1mv1v - LC2VFU(rgb[1][indx - v3])));
                                vfloat wtdv =
                                    onev
                                    / SQRV(temp1v
                                           + vabsf(rgbcv - LVFU(rgb[c][(indx + v2) >> 1]))
                                           + vabsf(rgb1pv1v - LC2VFU(rgb[1][indx + v3])));
                                ...
                                vfloat wtlv =
                                    onev
                                    / SQRV(temp2v
                                           + vabsf(rgbcv - LVFU(rgb[c][(indx - 2) >> 1]))
                                           + vabsf(rgb1m1v - LC2VFU(rgb[1][indx - 3])));
                                vfloat wtrv =
                                    onev
                                    / SQRV(temp2v
                                           + vabsf(rgbcv - LVFU(rgb[c][(indx + 2) >> 1]))
                                           + vabsf(rgb1p1v - LC2VFU(rgb[1][indx + 3])));

                                // store in rgb array the interpolated G value at R/B grid
                                // points using directional weighted average
                                vfloat result = (wtuv * rgb1mv1v + wtdv * rgb1pv1v
                                                 + wtlv * rgb1m1v + wtrv * rgb1p1v)
                                                / (wtuv + wtdv + wtlv + wtrv);
                            ...
                            for (; cc < cc1 - 3; cc += 2, indx += 2) {
                                ...
                                float wtu =
                                    1.f
                                    / SQR(eps
                                          + fabsf(rgb[1][indx + v1] - rgb[1][indx - v1])
                                          + fabsf(rgb[c][indx >> 1]
                                                  - rgb[c][(indx - v2) >> 1])
                                ...
                                float wtl =
                                    1.f
                                    / SQR(eps + fabsf(rgb[1][indx + 1] - rgb[1][indx - 1])
                                          + fabsf(rgb[c][indx >> 1]
                                                  - rgb[c][(indx - 2) >> 1])
                                ...
                                // points using directional weighted average
                                rgb[1][indx] =
                                    (wtu * rgb[1][indx - v1] + wtd * rgb[1][indx + v1]
                            ...
                            if (row > -1 && row < height) {
                                ...
                                int indx =
                                    rr * ts + 3 - (left < 0 ? (left + 3) : 0) + offset;
#ifdef __SSE2__
                        ...
                        for (int rr = 4; rr < rr1 - 4; rr++) {
                            ...
                            for (; cc < cc1 - 10; cc += 8, indx += 8) {
                                ...
                                vfloat temp1v = vabsf(
                                    vabsf((rgb1v - rgbcv)
                                          - (rgb1pv4 - LVFU(rgb[c][(indx + v4) >> 1])))
                                    + vabsf(rgb1mv4 - LVFU(rgb[c][(indx - v4) >> 1])
                                            - rgb1v + rgbcv)
                                    - vabsf(rgb1mv4 - LVFU(rgb[c][(indx - v4) >> 1])
                                            - rgb1pv4 + LVFU(rgb[c][(indx + v4) >> 1])));
                                ...
                                vfloat temp2v = vabsf(
                                    vabsf((rgb1v - rgbcv)
                                          - (rgb1p4 - LVFU(rgb[c][(indx + 4) >> 1])))
                                    + vabsf(rgb1m4 - LVFU(rgb[c][(indx - 4) >> 1]) - rgb1v
                                            + rgbcv)
                                    - vabsf(rgb1m4 - LVFU(rgb[c][(indx - 4) >> 1])
                                            - rgb1p4 + LVFU(rgb[c][(indx + 4) >> 1])));
                                STVFU(rbhpfh[indx >> 1], temp2v);

                                // low and high pass 1D filters of G in
                                // vertical/horizontal directions
                                rgb1v = vmul2f(rgb1v);
                                vfloat glpfvv = (rgb1v + LC2VFU(rgb[1][indx + v2])
                                                 + LC2VFU(rgb[1][indx - v2]));
                                vfloat glpfhv = (rgb1v + LC2VFU(rgb[1][indx + 2])
                                                 + LC2VFU(rgb[1][indx - 2]));
                                rgbcv = vmul2f(rgbcv);
                                STVFU(
                                    rblpfv[indx >> 1],
                                    zd25v
                                        * vabsf(glpfvv
                                                - (rgbcv + LVFU(rgb[c][(indx + v2) >> 1])
                                                   + LVFU(rgb[c][(indx - v2) >> 1]))));
                                STVFU(rblpfh[indx >> 1],
                                      zd25v
                                          * vabsf(glpfhv
                                                  - (rgbcv + LVFU(rgb[c][(indx + 2) >> 1])
                                                     + LVFU(rgb[c][(indx - 2) >> 1]))));
                            ...
                            for (; cc < cc1 - 4; cc += 2, indx += 2) {
                                rbhpfv[indx >> 1] = fabsf(
                                    fabsf(
                                        (rgb[1][indx] - rgb[c][indx >> 1])
                                        - (rgb[1][indx + v4] - rgb[c][(indx + v4) >> 1]))
                                    + fabsf((rgb[1][indx - v4] - rgb[c][(indx - v4) >> 1])
                                            - (rgb[1][indx] - rgb[c][indx >> 1]))
                                    - fabsf((rgb[1][indx - v4] - rgb[c][(indx - v4) >> 1])
                                            - (rgb[1][indx + v4]
                                               - rgb[c][(indx + v4) >> 1])));
                                rbhpfh[indx >> 1] = fabsf(
                                    fabsf((rgb[1][indx] - rgb[c][indx >> 1])
                                          - (rgb[1][indx + 4] - rgb[c][(indx + 4) >> 1]))
                                    + fabsf((rgb[1][indx - 4] - rgb[c][(indx - 4) >> 1])
                                            - (rgb[1][indx] - rgb[c][indx >> 1]))
                                    - fabsf(
                                        (rgb[1][indx - 4] - rgb[c][(indx - 4) >> 1])
                                        - (rgb[1][indx + 4] - rgb[c][(indx + 4) >> 1])));
                                ...
                                // low and high pass 1D filters of G in
                                // vertical/horizontal directions
                                float glpfv = (2.f * rgb[1][indx] + rgb[1][indx + v2]
                                               + rgb[1][indx - v2]);
                                float glpfh = (2.f * rgb[1][indx] + rgb[1][indx + 2]
                                               + rgb[1][indx - 2]);
                                rblpfv[indx >> 1] =
                                    0.25f
                                    * fabsf(glpfv
                                ...
                                rblpfh[indx >> 1] =
                                    ...
                                    * fabsf(glpfh
                                            - (2.f * rgb[c][indx >> 1]
                                               + rgb[c][(indx + 2) >> 1]
                                               + rgb[c][(indx - 2) >> 1]));
                                grblpfv[indx >> 1] = 0.25f
                                                     * (glpfv
                                                        + (2.f * rgb[c][indx >> 1]
                                                           + rgb[c][(indx + v2) >> 1]
                                                           + rgb[c][(indx - v2) >> 1]));
                                grblpfh[indx >> 1] = 0.25f
                                                     * (glpfh
                                                        + (2.f * rgb[c][indx >> 1]
                                                           + rgb[c][(indx + 2) >> 1]
                                                           + rgb[c][(indx - 2) >> 1]));
                        ...
                        // along line segments, find the point along each segment that
                        // minimizes the colour variance averaged over the tile; evaluate
                        // for up/down and left/right away from R/B grid point
                        for (int rr = 8; rr < rr1 - 8; rr++) {
                            ...
                            for (; cc < cc1 - 14; cc += 8, indx += 8) {
                                // in linear interpolation, colour differences are a
                                // quadratic function of interpolation position; solve for
                                // the interpolation position that minimizes colour
                                // difference variance over the tile

                                // vertical
                                vfloat temp1 = zd3v
                                               * (LC2VFU(rgb[1][indx + ts + 1])
                                                  - LC2VFU(rgb[1][indx - ts - 1]));
                                vfloat temp2 = zd3v
                                               * (LC2VFU(rgb[1][indx - ts + 1])
                                                  - LC2VFU(rgb[1][indx + ts - 1]));
                                vfloat gdiffvv = (LC2VFU(rgb[1][indx + ts])
                                                  - LC2VFU(rgb[1][indx - ts]))
                                                 + (temp1 - temp2);
                                vfloat deltgrbv =
                                    LVFU(rgb[c][indx >> 1]) - LC2VFU(rgb[1][indx]);

                                vfloat gradwtvv =
                                    (LVFU(rbhpfv[indx >> 1])
                                     + zd5v
                                           * (LVFU(rbhpfv[(indx >> 1) + 1])
                                              + LVFU(rbhpfv[(indx >> 1) - 1])))
                                    * (LVFU(grblpfv[(indx >> 1) - v1])
                                       + LVFU(grblpfv[(indx >> 1) + v1]))
                                    / (epsv
                                       + zd1v
                                             * (LVFU(grblpfv[(indx >> 1) - v1])
                                                + LVFU(grblpfv[(indx >> 1) + v1]))
                                       + LVFU(rblpfv[(indx >> 1) - v1])
                                       + LVFU(rblpfv[(indx >> 1) + v1]));
                                ...
                                // horizontal
                                vfloat gdiffhv =
                                    (LC2VFU(rgb[1][indx + 1]) - LC2VFU(rgb[1][indx - 1]))
                                    + (temp1 + temp2);

                                vfloat gradwthv =
                                    (LVFU(rbhpfh[indx >> 1])
                                     + zd5v
                                           * (LVFU(rbhpfh[(indx >> 1) + v1])
                                              + LVFU(rbhpfh[(indx >> 1) - v1])))
                                    * (LVFU(grblpfh[(indx >> 1) - 1])
                                       + LVFU(grblpfh[(indx >> 1) + 1]))
                                    / (epsv
                                       + zd1v
                                             * (LVFU(grblpfh[(indx >> 1) - 1])
                                                + LVFU(grblpfh[(indx >> 1) + 1]))
                                       + LVFU(rblpfh[(indx >> 1) - 1])
                                       + LVFU(rblpfh[(indx >> 1) + 1]));
                            ...
                            for (; cc < cc1 - 8; cc += 2, indx += 2) {
                                ...
                                // horizontal
                                gdiff =
                                    (rgb[1][indx + 1] - rgb[1][indx - 1])
                                    + 0.3f
                                          * (rgb[1][indx + 1 + ts] - rgb[1][indx - 1 + ts]
                                             + rgb[1][indx + 1 - ts]
                                             - rgb[1][indx - 1 - ts]);

                                gradwt = (rbhpfh[indx >> 1]
                                          + 0.5f
                                                * (rbhpfh[(indx >> 1) + v1]
                                                   + rbhpfh[(indx >> 1) - v1]))
                                         * (grblpfh[(indx >> 1) - 1]
                                ...
                                //  extremum =
                                //  -.5Coefficient[f[x],x]/Coefficient[f[x],x^2]
                            }
                        }
                        ...
                        for (int c = 0; c < 2; c++) {
                            for (int dir = 0; dir < 2; dir++) {  // vert/hor
                                ...
                                // This is the approximate _optical_ location of the R/B
                                // pixels
                                ...
                                // offset gives NW corner of square containing the min;
                                // dir : 0=vert, 1=hor
                                ...
                                // evaluate the shifts to the location that minimizes CA
                                // within the tile
...
#pragma omp single
#endif
                {
                    for (int dir = 0; dir < 2; dir++)
                        for (int c = 0; c < 2; c++) {
                            if (blockdenom[dir][c]) {
                                blockvar[dir][c] =
                                    blocksqave[dir][c] / blockdenom[dir][c]
                    ...
                    if (processpasstwo) {
                        ...
                        {  // left and right sides
                            for (int c = 0; c < 2; c++) {
                                for (int i = 0; i < 2; i++) {
                                    blockshifts[vblock * hblsz][c][i] =
                                        blockshifts[(vblock)*hblsz + 2][c][i];
                                    blockshifts[vblock * hblsz + hblsz - 1][c][i] =
                                        blockshifts[(vblock)*hblsz + hblsz - 3][c][i];
                                }
                            }
                        }
                        ...
                        {  // top and bottom sides
                            for (int c = 0; c < 2; c++) {
                                for (int i = 0; i < 2; i++) {
                                    blockshifts[hblock][c][i] =
                                        blockshifts[2 * hblsz + hblock][c][i];
                        ...
                        for (int i = 0; i < 256; i++) {
                            polymat[0][0][i] = polymat[0][1][i] = polymat[1][0][i] =
                                polymat[1][1][i] = 0;
                        }

                        for (int i = 0; i < 16; i++) {
                            shiftmat[0][0][i] = shiftmat[0][1][i] = shiftmat[1][0][i] =
                                shiftmat[1][1][i] = 0;
                        }
                                ...
                                for (int c = 0; c < 2; c++) {
                                    ...
                                    for (int dir = 0; dir < 2; dir++) {
                                        ...
                                        const std::array<float, 9> p = {
                                            blockshifts[(vblock - 1) * hblsz + hblock - 1]
                                                       [c][dir],
                                            blockshifts[(vblock - 1) * hblsz + hblock][c]
                                                       [dir],
                                            blockshifts[(vblock - 1) * hblsz + hblock + 1]
                                                       [c][dir],
                                            blockshifts[(vblock)*hblsz + hblock - 1][c]
                                                       [dir],
                                            blockshifts[(vblock)*hblsz + hblock][c][dir],
                                            blockshifts[(vblock)*hblsz + hblock + 1][c]
                                                       [dir],
                                    ...
                                    // now prepare coefficient matrix; use only data
                                    // points within caAutostrength/2 std devs of zero
                                    if (SQR(bstemp[0]) > caAutostrength * blockvar[0][c]
                                        || SQR(bstemp[1])
                                               > caAutostrength * blockvar[1][c])
                                    {
                                        continue;
                                    }
                                        ...
                                        for (int i = 0; i < polyord; i++) {
                                            ...
                                            for (int j = 0; j < polyord; j++) {
                                                ...
                                                shiftmat[c][dir][(polyord * i + j)] +=
                                                    powVblockInit * powHblockInit
                                                    * static_cast<double>(bstemp[dir])
                                                    * static_cast<double>(
                                                        blockwt[vblock * hblsz + hblock]);
                        ...
                        if (processpasstwo) {
                            ...
                            for (int c = 0; c < 2; c++) {
                                for (int dir = 0; dir < 2; dir++) {
                                    if (!LinEqSolve(numpar, polymat[c][dir],
                                                    shiftmat[c][dir], fitparams[c][dir]))
                                    {
                                        std::cout
                                            << "CA correction pass failed -- can't solve linear equations for colour %d direction "
                                            << c << std::endl;
                                        processpasstwo = false;
                                    }
                                }
                    ...
                    // fitparams[polyord*i+j] gives the coefficients of (vblock^i
                    // hblock^j) in a polynomial fit for i,j<=4
                }
            ...
            if (processpasstwo) {
                float* grbdiff =
                    (float(*))(data + 2 * sizeof(float) * ts * ts
                               + 3 * 64);  // there is no overlap in buffer usage => share
                // green interpolated to optical sample points for R/B
                float* gshift = (float(*))(
                    data + 2 * sizeof(float) * ts * ts + sizeof(float) * ts * tsh
                    + 4 * 64);  // there is no overlap in buffer usage => share
#ifdef _OPENMP
                ...
                for (int top = -border; top < height; top += ts - border2) {
                    for (int left = -border; left < width - (W & 1); left += ts - border2)
                    {
                        ...
                        for (int rr = rrmin; rr < rrmax; rr++) {
                            ...
                            for (; cc < ccmax - 7;
                                 cc += 8, col += 8, indx += 8, indx1 += 8)
                            {
                                vfloat val1v = LVFU(rawData[row][col]) / c65535v;
                                vfloat val2v = LVFU(rawData[row][col + 4]) / c65535v;
                                STVFU(rgb[c][indx1 >> 1],
                                      _mm_shuffle_ps(val1v, val2v,
                                                     _MM_SHUFFLE(2, 0, 2, 0)));
                                vfloat gtmpv = LVFU(Gtmp[indx >> 1]);
                                STVFU(rgb[1][indx1],
                                      vself(gmask,
                                            PERMUTEPS(gtmpv, _MM_SHUFFLE(1, 1, 0, 0)),
                                            val1v));
                                STVFU(rgb[1][indx1 + 4],
                                      vself(gmask,
                                            PERMUTEPS(gtmpv, _MM_SHUFFLE(3, 3, 2, 2)),
                                            val2v));
                            }
#endif
                        ...
                        if (rrmin > 0 && ccmin > 0) {
                            for (int rr = 0; rr < border; rr++) {
                                for (int cc = 0; cc < border; cc++) {
                                    ...
                                    if ((c & 1) == 0) {
                                        rgb[1][rr * ts + cc] =
                                            Gtmp[((border2 - rr) * width + border2 - cc)
                                                 >> 1];
                                    }
                                }
                            }
...
#endif
                            // manual CA correction; use red/blue slider values to set CA
                            // shift parameters
                            for (int rr = 3; rr < rr1 - 3; rr++) {
                                int cc = 3 + fc(cfa, rr, 1), c = fc(cfa, rr, cc),
                                    indx = rr * ts + cc;
#ifdef __SSE2__
                                for (; cc < cc1 - 10; cc += 8, indx += 8) {
                                    // compute directional weights using image gradients
                                    vfloat val1v =
                                        epsv
                                        + vabsf(LC2VFU(rgb[1][(rr + 1) * ts + cc])
                                                - LC2VFU(rgb[1][(rr - 1) * ts + cc]));
                                    vfloat val2v = epsv
                                                   + vabsf(LC2VFU(rgb[1][indx + 1])
                                                           - LC2VFU(rgb[1][indx - 1]));
                                    vfloat wtuv =
                                        onev
                                        / SQRV(
                                            val1v
                                            + vabsf(
                                                LVFU(rgb[c][(rr * ts + cc) >> 1])
                                                - LVFU(rgb[c][((rr - 2) * ts + cc) >> 1]))
                                            + vabsf(
                                                LC2VFU(rgb[1][(rr - 1) * ts + cc])
                                                - LC2VFU(rgb[1][(rr - 3) * ts + cc])));
                                    vfloat wtdv =
                                        onev
                                        / SQRV(
                                            val1v
                                    ...
                                    // store in rgb array the interpolated G value at R/B
                                    // grid points using directional weighted average
                                    vfloat result = (wtuv * LC2VFU(rgb[1][indx - v1])
                                                     + wtdv * LC2VFU(rgb[1][indx + v1])
                                                     + wtlv * LC2VFU(rgb[1][indx - 1])
                                                     + wtrv * LC2VFU(rgb[1][indx + 1]))
                                                    / (wtuv + wtdv + wtlv + wtrv);
                                    STC2VFU(rgb[1][indx], result);
                                }
#endif
                                for (; cc < cc1 - 3; cc += 2, indx += 2) {
                                    ...
                                    float wtu =
                                        1.f
                                        / SQR(eps
                                              + fabsf(rgb[1][(rr + 1) * ts + cc]
                                                      - rgb[1][(rr - 1) * ts + cc])
                                              + fabsf(rgb[c][(rr * ts + cc) >> 1]
                                                      - rgb[c][((rr - 2) * ts + cc) >> 1])
                                              + fabsf(rgb[1][(rr - 1) * ts + cc]
                                                      - rgb[1][(rr - 3) * ts + cc]));
                                    float wtd =
                        ...
                        if (!autoCA) {
                            ...
                            double vfrac =
                                -((vblock - 0.5) / (vblsz - 2) - 0.5) * height / width;
                            ...
                            // CA auto correction; use CA diagnostic pass to set shift
                            // parameters
                            ...
                            for (int i = 0; i < polyord; i++) {
                                ...
                                for (int j = 0; j < polyord; j++) {
                                    lblockshifts[0][0] +=
                                        powHblock * fitparams[0][0][polyord * i + j];
                                    lblockshifts[0][1] +=
                                        powHblock * fitparams[0][1][polyord * i + j];
                                    lblockshifts[1][0] +=
                                        powHblock * fitparams[1][0][polyord * i + j];
                                    lblockshifts[1][1] +=
                                        powHblock * fitparams[1][1][polyord * i + j];
                                    powHblock *= hblock;
                                }
                                powVblock *= vblock;
                            }
                        ...
                        for (int c = 0; c < 3; c += 2) {
                            ...
                            shiftvfrac[c] =
                                fabs(lblockshifts[c >> 1][0] - shiftvfloor[c]);
                            ...
                            shifthfrac[c] =
                                fabs(lblockshifts[c >> 1][1] - shifthfloor[c]);
                        ...
                        for (int rr = 4; rr < rr1 - 4; rr++) {
                            ...
                            for (; cc < cc1 - 10; cc += 8, indxfc += 8, indxff += 8,
                                                  indxcc += 8, indxcf += 8, indx += 4)
                            {
                                // perform CA correction using colour ratios or colour
                                // differences
                                vfloat Ginthfloorv =
                                    vintpf(shifthfracv, LC2VFU(rgb[1][indxfc]),
                                           LC2VFU(rgb[1][indxff]));
                                vfloat Ginthceilv =
                                    vintpf(shifthfracv, LC2VFU(rgb[1][indxcc]),
                                           LC2VFU(rgb[1][indxcf]));
                                // Gint is bilinear interpolation of G at CA shift point
                                vfloat Gintv =
                                    vintpf(shiftvfracv, Ginthceilv, Ginthfloorv);

                                // determine R/B at grid points using colour differences
                                // at shift point plus interpolated G value at grid point
                                // but first we need to interpolate G-R/G-B to grid
                                // points...
...
#endif
                            for (; cc < cc1 - 4; cc += 2, indxfc += 2, indxff += 2,
                                                 indxcc += 2, indxcf += 2, ++indx)
                            {
                                // perform CA correction using colour ratios or colour
                                // differences
                                float Ginthfloor =
                                    intp(shifthfrac[c], rgb[1][indxfc], rgb[1][indxff]);
                                float Ginthceil =
                                    intp(shifthfrac[c], rgb[1][indxcc], rgb[1][indxcf]);
                                // Gint is bilinear interpolation of G at CA shift point
                        ...
                        for (int rr = 8; rr < rr1 - 8; rr++) {
                            ...
                            for (int indx = rr * ts + cc; cc < cc1 - 14;
                                 cc += 8, indx += 8)
                            {
                                // interpolate colour difference from optical R/B
                                // locations to grid locations
                                vfloat grbdiffinthfloor = vintpf(
                                    shifthfracc, LVFU(grbdiff[(indx - GRBdir1) >> 1]),
                                    LVFU(grbdiff[indx >> 1]));
                                vfloat grbdiffinthceil = vintpf(
                                    shifthfracc,
                                    LVFU(grbdiff[((rr - GRBdir0) * ts + cc - GRBdir1)
                                                 >> 1]),
                                    LVFU(grbdiff[((rr - GRBdir0) * ts + cc) >> 1]));
                                // grbdiffint is bilinear interpolation of G-R/G-B at grid
                                // point
                                vfloat grbdiffint = vintpf(shiftvfracc, grbdiffinthceil,
                                                           grbdiffinthfloor);

                                // now determine R/B at grid points using interpolated
                                // colour differences and interpolated G value at grid
                                // point
                                vfloat cinv = LVFU(rgb[c][indx >> 1]);
                                vfloat rinv = LC2VFU(rgb[1][indx]);
                                vfloat RBint = rinv - grbdiffint;
                                vmask cmask = vmaskf_ge(vabsf(RBint - cinv),
                                                        zd25v * (RBint + cinv));
                                if (_mm_movemask_ps((vfloat)cmask)) {
                                    // if for any of the 4 pixels the condition is true,
                                    // do the math for all 4 pixels and mask the unused
                                    // out at the end
                                    // gradient weights using difference from G at CA
                                    // shift points and G at grid points
                                    vfloat p0 =
                                        onev
                                        / (epsv + vabsf(rinv - LVFU(gshift[indx >> 1])));
                                    vfloat p1 =
                                        onev
                                        / (epsv
                                           + vabsf(
                                               rinv
                                               - LVFU(gshift[(indx - GRBdir1) >> 1])));
                                    vfloat p2 =
                                        onev
                                        / (epsv
                                           + vabsf(
                                               rinv
                                               - LVFU(gshift[((rr - GRBdir0) * ts + cc)
                                                             >> 1])));
                                    vfloat p3 =
                                        onev
                                        / (epsv
                                    ...
                                    grbdiffint = vself(
                                        cmask,
                                        (p0 * LVFU(grbdiff[indx >> 1])
                                         + p1 * LVFU(grbdiff[(indx - GRBdir1) >> 1])
                                         + p2
                                               * LVFU(grbdiff[((rr - GRBdir0) * ts + cc)
                                                              >> 1])
                                         + p3
                                               * LVFU(grbdiff[((rr - GRBdir0) * ts + cc
                                                               - GRBdir1)
                                ...
                                RBint =
                                    vself(vmaskf_gt(vabsf(grbdiffold), vabsf(grbdiffint)),
                                          RBint, cinv);
                                RBint =
                                    vself(vmaskf_lt(grbdiffold * grbdiffint, ZEROV),
                                          rinv - zd5v * (grbdiffold + grbdiffint), RBint);
                                STVFU(rgb[c][indx >> 1], RBint);
                            }
#endif
                            for (int c = fc(cfa, rr, cc), indx = rr * ts + cc;
                                 cc < cc1 - 8; cc += 2, indx += 2)
                            {
                                ...
                                float grbdiffinthfloor =
                                    intp(shifthfrac[c], grbdiff[(indx - GRBdir1) >> 1],
                                         grbdiff[indx >> 1]);
                                float grbdiffinthceil = intp(
                                    shifthfrac[c],
                                    grbdiff[((rr - GRBdir0) * ts + cc - GRBdir1) >> 1],
                                    grbdiff[((rr - GRBdir0) * ts + cc) >> 1]);
                                // grbdiffint is bilinear interpolation of G-R/G-B at grid
                                // point
                                float grbdiffint = intp(shiftvfrac[c], grbdiffinthceil,
                                                        grbdiffinthfloor);
                                ...
                                if (fabsf(RBint - rgb[c][indx >> 1])
                                    < 0.25f * (RBint + rgb[c][indx >> 1]))
                                {
                                    ...
                                    float p0 =
                                        1.f
                                        / (eps + fabsf(rgb[1][indx] - gshift[indx >> 1]));
                                    float p1 =
                                        1.f
                                        / (eps
                                           + fabsf(rgb[1][indx]
                                                   - gshift[(indx - GRBdir1) >> 1]));
                                    float p2 =
                                        1.f
                                        / (eps
                                           + fabsf(rgb[1][indx]
                                                   - gshift[((rr - GRBdir0) * ts + cc)
                                                            >> 1]));
                                    float p3 = 1.f
                                               / (eps
                                                  + fabsf(rgb[1][indx]
                                                          - gshift[((rr - GRBdir0) * ts
                                                                    + cc - GRBdir1)
                                                                   >> 1]));

                                    grbdiffint =
                                        (p0 * grbdiff[indx >> 1]
                                         + p1 * grbdiff[(indx - GRBdir1) >> 1]
                                         + p2 * grbdiff[((rr - GRBdir0) * ts + cc) >> 1]
                                         + p3
                                               * grbdiff[((rr - GRBdir0) * ts + cc
                                                          - GRBdir1)
                                                         >> 1])
                                        / (p0 + p1 + p2 + p3);
                                ...
                                // if colour difference interpolation overshot the
                                // correction, just desaturate
...
#pragma omp for
        ...
        if (avoidColourshift) {
            // to avoid or at least reduce the colour shift caused by raw ca correction we
            // compute the per pixel difference factors of red and blue channel and apply
            // a gaussian blur to them. Then we apply the resulting factors per pixel on
            // the result of raw ca correction
...
#pragma omp for
                ...
                for (int i = 0; i < H - 2 * cb; ++i) {
                    ...
                    for (; j < W - 2 * cb; j += 2) {
                        (*nonGreen)[i / 2][j / 2] =
                            (rawData[i + cb][j + cb] <= 1.f || (*oldraw)[i][j / 2] <= 1.f)
                                ? 1.f
...
#pragma omp single
                ...
                {
                    if (H % 2) {
                        // odd height => factors are not set in last row => use values of
                        // preceding row
                        for (int j = 0; j < (W + 1 - 2 * cb) / 2; ++j) {
                            (*redFactor)[(H - 2 * cb + 1) / 2 - 1][j] =
                                (*redFactor)[(H - 2 * cb + 1) / 2 - 2][j];
                            (*blueFactor)[(H - 2 * cb + 1) / 2 - 1][j] =
                                (*blueFactor)[(H - 2 * cb + 1) / 2 - 2][j];
                        }
                    }

                    if (W % 2) {
                        // odd width => factors for one channel are not set in last column
                        // => use value of preceding column
                        ...
                        for (int i = 0; i < (H + 1 - 2 * cb) / 2; ++i) {
                            (*nonGreen)[i][(W - 2 * cb + 1) / 2 - 1] =
                                (*nonGreen)[i][(W - 2 * cb + 1) / 2 - 2];
                        }
                    }
                }

                // blur correction factors
                gaussianBlur(*redFactor, *redFactor, (W + 1 - 2 * cb) / 2,
                             (H + 1 - 2 * cb) / 2, 30.0);
                gaussianBlur(*blueFactor, *blueFactor, (W + 1 - 2 * cb) / 2,
                             (H + 1 - 2 * cb) / 2, 30.0);
