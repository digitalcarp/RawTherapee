...
void RawImageSource::dual_demosaic_RT(bool isBayer,
...
#pragma omp parallel for schedule(dynamic, 16)
    ...
    // calculate contrast based blend factors to use flat demosaicer in regions with low
    // contrast
...
#endif
            for (int i = 0; i < winh; ++i) {
                // the following is split into 3 loops intentionally to avoid cache
                // conflicts on CPUs with only 4-way cache
