...
public:
    ...
    {
        if (offset >= begin && offset < end) return data[offset - begin];
        if (!input) return 0;
        refill(offset);
        if (offset >= begin && offset < end) return data[offset - begin];
        return 0;
...
struct pana8_param_t
{
    ...
    bool DecodeC8(pana8_bufio_t& bufio,
                  unsigned int width,
                  unsigned int height,
                  ushort* raw_image,
                  ushort raw_width,
...
{
    if (newoffset >= begin && newoffset < end) return;
...
#pragma omp critical
    ...
    if (readwords > 0) invertBits(data.data(), readwords * sizeof(uint64_t));
    begin = newoffset;
...
bool pana8_param_t::DecodeC8(pana8_bufio_t& bufio,
                             unsigned int width,
                             unsigned int height,
                             ushort* raw_image,
                             ushort raw_width,
                             ...
                             uint16_t left_margin)
{
    ...
    if (!halfwidth || !halfheight || bufio.size() < 9) return false;  // invalid input
    ...
    if (_extrahuff) big_huff_table = extrahuff.data();

    uint16_t* gammatable =
        (gammaTable.size() >= 0x10000) && !noGammaFlag ? (uint16_t*)gammaTable.data() : 0;
        ...
        for (uint32_t current_row = 0; current_row < halfheight; current_row++) {
            ...
            for (int32_t col = 0; col < doublewidth; col++) {
                ...
                if (bitportion < 0) {
                    ...
                    if ((int)inqword + 1 >= int(jobsz_in_qwords)) return false;
                    bitportion += 64;
                    uint64_t inputqword = bufio.getQWord(inqword);
                    uint64_t inputqword_next = bufio.getQWord(inqword_next);
                    pixbits = (inputqword_next >> bitportion)
                              | (inputqword << (64 - (uint8_t)(bitportion & 0xffu)));
                    if ((unsigned int)inqword < jobsz_in_qwords) inqword = inqword_next;
                } else {
                    if ((unsigned int)inqword >= jobsz_in_qwords) return false;
                    uint64_t inputqword = bufio.getQWord(inqword);
                    pixbits = (inputqword >> bitportion) | bittail;
                    uint32_t step = (bitportion == 0);
                    if (!bitportion) bitportion = 64;
                    inqword += step;
                }
                int huff_index = 0;
                if (_extrahuff) {
                    huff_index =
                        *(uint8_t*)(big_huff_table + ((pixbits >> 48) & 0xffffu));
                } else {
                ...
                int32_t v39 =
                    (uint16_t)((uint64_t)v38 >> ((uint8_t)v37 - (uint8_t)huff_index))
                    << ((huff_coeff[huff_index] >> 24) & 0xffu);

                if (huff_index - v37 <= 0) v39 &= 0xffff0000u;
...
{
    for (int i = 0; i < 16; i++) {
        if ((a2 & hufftable2[i]) == hufftable1[i]) return i;
    }
...
pana8_param_t::pana8_param_t(const pana8_tags_t& meta) : gammaTable(0)
{
    ...
    for (int i = 0; i < 17; i++)
        huff_coeff[i] = (uint32_t(meta.tag41[i]) << 24) | (uint32_t(meta.tag40a[i]) << 16)
                        | meta.tag40b[i];
    ...
    for (unsigned i = 0; i < 0x10000; i++) {
        ...
        if (i != val) noGammaFlag = 0;
    }

    if (!noGammaFlag) gammaTable = tempGamma;
    ...
    if (v7 < 17) {
        if (extrahuff.size() < 0x10000) extrahuff.resize(0x10000);
        uint64_t v17 = 0LL;
...
int32_t pana8_param_t::gammaCurve(uint32_t idx)
{
    ...
    if ((idx & 0x10000) == 0) v2 = idx & 0x1FFFF;

    int v3 = gamma_base + v2;
    unsigned int v4 = MIN(v3, 0xFFFF);

    int v5 = 0;
    if ((v4 & 0x80000000) != 0) v4 = 0;
...
void invertBits(void* buf, size_t size)
{
    ...
    for (unsigned i = 0; i < sz; i++) {
        ...
        uint64_t r =
            ((uint64_t)_bitRevTable[b[0]] << 56) | ((uint64_t)_bitRevTable[b[1]] << 48)
...
void pana_cs6_page_decoder::read_page()
{
    ...
    pixelbuffer[1] =
        (((wbuffer(1) & 0x3) << 12) | (wbuffer(2) << 4) | (wbuffer(3) >> 4)) & 0x3fff;
...
void pana_cs6_page_decoder::read_page12()
{
    if (!buffer || (maxoffset - lastoffset < 16))
        ;
    pixelbuffer[0] =
        (wbuffer(0) << 4) | (wbuffer(1) >> 4);  // 12 bit: 8/0 + 4 upper bits of /1
    pixelbuffer[1] =
        (((wbuffer(1) & 0xf) << 8) | (wbuffer(2))) & 0xfff;  // 12 bit: 4l/1 + 8/2

    pixelbuffer[2] =
        (wbuffer(3) >> 6) & 0x3;  // 2; 2u/3, 6 low bits remains in wbuffer(3)
    pixelbuffer[3] =
        ((wbuffer(3) & 0x3f) << 2)
        | (wbuffer(4) >> 6);  // 8; 6l/3 + 2u/4; 6 low bits remains in wbuffer(4)
    pixelbuffer[4] =
        ((wbuffer(4) & 0x3f) << 2)
        | (wbuffer(5) >> 6);  // 8: 6l/4 + 2u/5; 6 low bits remains in wbuffer(5)
    pixelbuffer[5] =
        ((wbuffer(5) & 0x3f) << 2)
        | (wbuffer(6) >> 6);  // 8: 6l/5 + 2u/6, 6 low bits remains in wbuffer(6)

    pixelbuffer[6] = (wbuffer(6) >> 4) & 0x3;  // 2, 4 low bits remains in wbuffer(6)
    pixelbuffer[7] =
        ((wbuffer(6) & 0xf) << 4)
        | (wbuffer(7)
           >> 4);  // 8: 4 low bits from wbuffer(6), 4 upper bits from wbuffer(7)
    pixelbuffer[8] =
        ((wbuffer(7) & 0xf) << 4)
        | (wbuffer(8)
           >> 4);  // 8: 4 low bits from wbuffer(7), 4 upper bits from wbuffer(8)
    pixelbuffer[9] =
        ((wbuffer(8) & 0xf) << 4)
        | (wbuffer(9)
           >> 4);  // 8: 4 low bits from wbuffer(8), 4 upper bits from wbuffer(9)

    pixelbuffer[10] =
        (wbuffer(9) >> 2)
        & 0x3;  // 2: bits 2-3 from wbuffer(9), two low bits remain in wbuffer(9)
    pixelbuffer[11] =
        ((wbuffer(9) & 0x3) << 6)
        | (wbuffer(10) >> 2);  // 8: 2 bits from wbuffer(9), 6 bits from wbuffer(10)
    pixelbuffer[12] =
        ((wbuffer(10) & 0x3) << 6)
        | (wbuffer(11) >> 2);  // 8: 2 bits from wbuffer(10), 6 bits from wbuffer(11)
    pixelbuffer[13] =
        ((wbuffer(11) & 0x3) << 6)
        | (wbuffer(12) >> 2);  // 8: 2 bits from wbuffer(11), 6 bits from wbuffer(12)
    ...
    if (RT_pana_info.encoding == 5) {
        ...
        for (int row = 0; row < height; ++row) {
            for (int col = 0; col < raw_width; ++col) {
                ...
                if ((raw_image[(row)*raw_width + (col)] = pred[col & 1]) > 4098
                    && col < width)
                {
                    derror();
                }
        ...
        for (int crow = 0, col = 0; crow < rowstoread; ++crow, col = 0) {
            ...
            for (int rblock = 0; rblock < blocksperrow; rblock++) {
                ...
                for (int pix = 0; pix < pixperblock; ++pix) {
                    ...
                    if (oddeven[pix % 2]) {
                        epixel *= pmul;
                        if (pixel_base < pixelbase_compare
...
void DCraw::panasonicC8_load_raw()
{
    ...
    for (int i = 0; i < RT_pana_info.v8tags.stripe_count && i < 5; i++) {
        if (RT_pana_info.v8tags.stripe_height[i] != raw_height) errs++;
        if (RT_pana_info.v8tags.stripe_offsets[i] < 0
            || (RT_pana_info.v8tags.stripe_offsets[i]
                + INT64((RT_pana_info.v8tags.stripe_compressed_size[i] + 7u) / 8u))
                   > INT64(ifp->size))
            errs++;
...
#pragma omp parallel for
    for (int stream = 0; stream < scount; stream++) {
        if (!pana8_decode_strip(data, stream)) errs++;
    }
    if (errs) derror();
#else
    for (int stream = 0; stream < RT_pana_info.v8tags.stripe_count && stream < 5;
         stream++)
        if (!pana8_decode_strip(data, stream)) derror();
#endif
}
...
bool DCraw::pana8_decode_strip(void* data, int stream)
{
    ...
    if (!data || stream < 0 || stream > 4 || stream > RT_pana_info.v8tags.stripe_count)
        return 1;  // error
    ...
    return pana8_param->DecodeC8(bufio, RT_pana_info.v8tags.stripe_width[stream],
                                 RT_pana_info.v8tags.stripe_height[stream], raw_image,
