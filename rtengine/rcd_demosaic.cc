 ...
 *
 *  Copyright (c) 2017-2020 Luis Sanz Rodriguez (luis.sanz.rodriguez(at)gmail(dot)com) and
 * Ingo Weyrich (heckflosse67@gmx.de)
 *
...
// Luis Sanz Rodriguez significantly optimised the v 2.3 code and simplified the
// directional coefficients in an exact, shorter and more performant formula. In
// cooperation with Hanno Schwalm (hanno@schwalm-bremen.de) and Luis Sanz Rodriguez this
// has been tuned for performance.
...
void RawImageSource::rcd_demosaic(size_t chunkSize, bool measure)
{
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {
            if (FC(i, j) == 3) {
                // avoid crash
                std::cout
                    << "rcd_demosaic supports only RGB Colour filter arrays. Falling back to igv_interpolate"
                    << std::endl;
                igv_interpolate(W, H);
                return;
            }
    ...
    if (plistener) {
        plistener->setProgressStr(
            Glib::ustring::compose(M("TP_RAW_DMETHOD_PROGRESSBAR"), M("TP_RAW_RCD")));
...
#pragma omp for schedule(dynamic, chunkSize) collapse(2) nowait
        ...
        for (int tr = 0; tr < numTh; ++tr) {
            for (int tc = 0; tc < numTw; ++tc) {
                ...
                // Step 1.1: Calculate the square of the vertical and horizontal color
                // difference high pass filter
                for (int row = 3; row < std::min(tileRows - 3, 5); ++row) {
                    for (int col = 4, indx = row * tileSize + col; col < tilecols - 4;
                         ++col, ++indx)
                    {
                        bufferV[row - 3][col - 4] = SQR(
                            (cfa[indx - w3] - cfa[indx - w1] - cfa[indx + w1]
                             + cfa[indx + w3])
                            - 3.f * (cfa[indx - w2] + cfa[indx + w2]) + 6.f * cfa[indx]);
                    }
                }

                // Step 1.2: Obtain the vertical and horizontal directional discrimination
                // strength
                ...
                for (int row = 4; row < tileRows - 4; ++row) {
                    for (int col = 3, indx = row * tileSize + col; col < tilecols - 3;
                         ++col, ++indx)
                    {
                        bufferH[col - 3] = SQR((cfa[indx - 3] - cfa[indx - 1]
                                                - cfa[indx + 1] + cfa[indx + 3])
                    ...
                    {
                        V2[col - 4] = SQR((cfa[indx - w3] - cfa[indx - w1]
                         ...
                         ++col, ++indx)
                    {
                        ...
                        float H_Stat = std::max(epssq, bufferH[col - 4] + bufferH[col - 3]
                                                           + bufferH[col - 2]);
                ...
                // Step 2: Low pass filter incorporating green, red and blue local samples
                // from the raw data
                ...
                for (int row = 4; row < tileRows - 4; ++row) {
                         ...
                         col < tilecols - 4; col += 2, indx += 2, ++lpindx)
                    {
                        ...
                        const float N_Grad =
                            eps
                            + (std::fabs(cfa[indx - w1] - cfa[indx + w1])
                        ...
                        const float W_Grad =
                            eps
                            + (std::fabs(cfa[indx - 1] - cfa[indx + 1])
                        ...
                        const float S_Est = cfa[indx + w1] * (lpfi + lpfi)
                                            / (eps + lpfi + lpf[lpindx + w1]);
                        const float W_Est = cfa[indx - 1] * (lpfi + lpfi)
                                            / (eps + lpfi + lpf[lpindx - 1]);
                        const float E_Est = cfa[indx + 1] * (lpfi + lpfi)
                                            / (eps + lpfi + lpf[lpindx + 1]);
                ...
                // Step 4.0: Calculate the square of the P/Q diagonals color difference
                // high pass filter
                ...
                for (int row = 4; row < tileRows - 4; ++row) {
                    ...
                    {
                        float P_Stat =
                            std::max(epssq, P_CDiff_Hpf[indx3] + P_CDiff_Hpf[indx2]
                                                + P_CDiff_Hpf[indx4 + 1]);
                        float Q_Stat =
                            std::max(epssq, Q_CDiff_Hpf[indx3 + 1] + Q_CDiff_Hpf[indx2]
                                                + Q_CDiff_Hpf[indx4]);
                ...
                for (int row = 4; row < tileRows - 4; ++row) {
                    for (int col = 4 + (fc(cfarray, row, 0) & 1),
                             indx = row * tileSize + col, c = 2 - fc(cfarray, row, col),
                         ...
                         col += 2, indx += 2, ++pqindx, ++pqindx2, ++pqindx3)
                    {
                        ...
                        float PQ_Neighbourhood_Value =
                            0.25f
                            * (PQ_Dir[pqindx2] + PQ_Dir[pqindx2 + 1] + PQ_Dir[pqindx3]
                               + PQ_Dir[pqindx3 + 1]);

                        float PQ_Disc = (std::fabs(0.5f - PQ_Central_Value)
                        ...
                        // Diagonal gradients
                        float NW_Grad =
                            eps + std::fabs(rgb[c][indx - w1 - 1] - rgb[c][indx + w1 + 1])
                ...
                for (int row = 4; row < tileRows - 4; ++row) {
                         ...
                         col < tilecols - 4; col += 2, indx += 2)
                    {
                        ...
                        float VH_Disc = (std::fabs(0.5f - VH_Central_Value)
                                         < std::fabs(0.5f - VH_Neighbourhood_Value))
                                            ? VH_Neighbourhood_Value
                                            : VH_Central_Value;
                ...
                // For the outermost tiles in all directions we can use a smaller border
                // margin
...
#pragma omp critical(rcdprogress)
                        ...
                        {
                            progress +=
                                (double)32 * ((tileSizeN) * (tileSizeN)) / (H * W);
