...
void dirpyr_channel(const float* const* data_fine,
                    float** data_coarse,
                    int width,
                    int height,
                    int level,
                    int scale)
{
    ...
    if (level > 1) {
        ...
        //   multiplied each value of domker by 1000 to avoid multiplication by 1000
        //   inside the loop
#ifdef __SSE2__
        // clang-format off
...
#pragma omp for
            ...
            for (int i = 0; i < height; i++) {
                ...
                for (j = 0; j < scalewin; j++) {
                    ...
                    for (int inbr = std::max(0, i - scalewin);
                         inbr <= std::min(height - 1, i + scalewin); inbr += scale)
                    {
                        for (int jnbr = std::max(0, j - scalewin); jnbr <= j + scalewin;
                             jnbr += scale)
                        {
                            const float dirwt =
                                domker[(inbr - i) / scale + halfwin]
                                      [(jnbr - j) / scale + halfwin]
                                * rangeFn(fabsf(data_fine[inbr][jnbr] - data_fine[i][j]));
                ...
                for (; j < width - scalewin - 3; j += 4) {
                    ...
                    for (int inbr = MAX(0, i - scalewin);
                         inbr <= MIN(height - 1, i + scalewin); inbr += scale)
                    {
                             ...
                             jnbr += scale, ++indexjhlp)
                        {
                            ...
                            const vfloat dirwtv =
                                ...
                                / (vabsf(dftemp1v - dftemp2v) + thousandv);
                ...
                for (; j < width; j++) {
                    ...
                    for (int inbr = std::max(0, i - scalewin);
                         inbr <= std::min(height - 1, i + scalewin); inbr += scale)
                    {
                        for (int jnbr = j - scalewin;
                             jnbr <= std::min(width - 1, j + scalewin); jnbr += scale)
                        {
                            const float dirwt =
                                domker[(inbr - i) / scale + halfwin]
                                      [(jnbr - j) / scale + halfwin]
    ...
    } else {  // level <=1 means that all values of domker would be 1.0f, so no need for
              // multiplication
#ifdef _OPENMP
#pragma omp parallel
#endif
...
#pragma omp for schedule(dynamic, 16)
            ...
            for (int i = 0; i < height; i++) {
                ...
                for (; j < scale; j++) {
                    ...
                    for (int inbr = std::max(0, i - scale);
                         inbr <= std::min(height - 1, i + scale); inbr += scale)
                    {
                        for (int jnbr = std::max(0, j - scale); jnbr <= j + scale;
                             jnbr += scale)
                        {
                            const float dirwt =
                                rangeFn(fabsf(data_fine[inbr][jnbr] - data_fine[i][j]));
                ...
                for (; j < width - scale - 3; j += 4) {
                    ...
                    for (int inbr = MAX(0, i - scale); inbr <= MIN(height - 1, i + scale);
                         inbr += scale)
                    {
                        for (int jnbr = j - scale; jnbr <= j + scale; jnbr += scale) {
                            const vfloat dftemp2v = LVFU(data_fine[inbr][jnbr]);
                            const vfloat dirwtv =
                                thousandv / (vabsf(dftemp2v - dftemp1v) + thousandv);
                            valv += dirwtv * dftemp2v;
                            normv += dirwtv;
                        }
                    }
                ...
                for (; j < width; j++) {
                    ...
                    for (int inbr = std::max(0, i - scale);
                         inbr <= std::min(height - 1, i + scale); inbr += scale)
                    {
                        for (int jnbr = j - scale; jnbr <= std::min(width - 1, j + scale);
                             jnbr += scale)
                        {
                            const float dirwt =
                                rangeFn(fabsf(data_fine[inbr][jnbr] - data_fine[i][j]));
             ...
             float skinprot)
{
    ...
    const float noisehi = 1.33 * noise * dirpyrThreshold / exp(level * log(3.0)),
                noiselo = 0.66 * noise * dirpyrThreshold / exp(level * log(3.0));
    ...
    for (int i = 0; i < 0x20000; i++) {
        if (abs(i - 0x10000) > noisehi || multbis < 1.f) {
            ...
            if (abs(i - 0x10000) < noiselo) {
                ...
                irangefn[i] = 1.f + offs
                              + (multbis - 1.f) * (noisehi - abs(i - 0x10000))
                                    / (noisehi - noiselo + 0.01f);
            }
        }
    }
}

void idirpyr_eq_channel_loc(float** data_coarse,
                            float** data_fine,
                            float** buffer,
                            int width,
                            int height,
...
void idirpyr_eq_channel(const float* const* data_coarse,
                        const float* const* data_fine,
                        float** buffer,
                        int width,
                        int height,
                        int level,
...
#pragma omp parallel for schedule(dynamic, 16)
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                float scale = 1.f;
                const float hipass = data_fine[i][j] - data_coarse[i][j];
                rtengine::Color::SkinSatCbdl(data_fine[i][j] / 327.68f, hue[i][j],
                                             chrom[i][j], skinprot, scale, true, b_l, t_l,
                                             t_r);
...
#pragma omp parallel for schedule(dynamic, 16)
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                ...
                const float hipass = data_fine[i][j] - data_coarse[i][j];
                rtengine::Color::SkinSatCbdl(data_fine[i][j] / 327.68f, hue[i][j],
                                             chrom[i][j], skinprotneg, scale, false, b_l,
                                             t_l, t_r);
...
void idirpyr_eq_channelcam(const float* const* data_coarse,
                           const float* const* data_fine,
                           float** buffer,
                           int width,
                           int height,
...
#pragma omp parallel for schedule(dynamic, 16)
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                const float hipass = data_fine[i][j] - data_coarse[i][j];
                float scale = 1.f;
                rtengine::Color::SkinSatCbdlCam(data_fine[i][j] / 327.68f, h_p[i][j],
                                                C_p[i][j], skinprot, scale, true, b_l,
                                                t_l, t_r);
                buffer[i][j] += (1.f + (irangefn[hipass + 0x10000]) * scale) * hipass;
            }
        }
...
#pragma omp parallel for schedule(dynamic, 16)
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                ...
                const float correct = irangefn[hipass + 0x10000];
                rtengine::Color::SkinSatCbdlCam(data_fine[i][j] / 327.68f, h_p[i][j],
                                                C_p[i][j], skinprotneg, scale, false, b_l,
                                                t_l, t_r);
...
void ImProcFunctions::dirpyr_equalizer(const float* const* src,
                                       float** dst,
                                       int srcwidth,
                                       int srcheight,
                                       ...
                                       int scaleprev)
{
    ...
    for (int lv = 0; lv < maxlevel; ++lv) {
        if (scales[lv] < scaleprev) {
            ...
            multi[lv] = (factor * ((float)mult[lv] - 1.f) / 100.f)
                        + 1.f;  // modulate action if zoom < 100%
        } else {
            multi[lv] = mult[lv];
        }
    }
    ...
    dirpyr_channel(src, dirpyrlo[0], srcwidth, srcheight, 0,
                   std::max(scales[0] / scaleprev, 1));

    for (int level = 1; level < lastlevel; ++level) {
        dirpyr_channel(dirpyrlo[level - 1], dirpyrlo[level], srcwidth, srcheight, level,
                       std::max(scales[level] / scaleprev, 1));
    }
    ...
    if (skinprot) {
        ...
        // by precalculating these values we can greatly reduce the number of calculations
        // in idirpyr_eq_channel() but we need two additional buffers for this
        // preprocessing
...
#pragma omp for
    ...
    // with the current implementation of idirpyr_eq_channel we can safely use the buffer
    // from last level as buffer, saves some memory
    float** buffer = dirpyrlo[lastlevel - 1];

    for (int level = lastlevel - 1; level > 0; --level) {
        idirpyr_eq_channel(dirpyrlo[level], dirpyrlo[level - 1], buffer, srcwidth,
                           srcheight, level, multi[level], dirpyrThreshold, tmpHue,
                           tmpChr, skinprot, b_l, t_l, t_r);
    }
...
void ImProcFunctions::dirpyr_equalizercam(const CieImage* ncie,
                                          float** src,
                                          float** dst,
                                          int srcwidth,
                                          int srcheight,
                                          ...
                                          int scaleprev)
{
    ...
    for (int level = lastlevel - 1; level > 0; --level) {
        idirpyr_eq_channelcam(dirpyrlo[level], dirpyrlo[level - 1], buffer, srcwidth,
                              srcheight, level, multi[level], dirpyrThreshold, h_p, C_p,
                              skinprot, b_l, t_l, t_r);
    }

    idirpyr_eq_channelcam(dirpyrlo[0], dst, buffer, srcwidth, srcheight, 0, multi[0],
                          dirpyrThreshold, h_p, C_p, skinprot, b_l, t_l, t_r);
...
void ImProcFunctions::cbdl_local_temp(float** src,
                                      float** loctemp,
                                      int srcwidth,
                                      int srcheight,
                                      const float* mult,
                                      ...
                                      bool multiThread)
{
    ...
    dirpyr_channel(src, dirpyrlo[0], srcwidth, srcheight, 0,
                   std::max(scalesloc[0] / scaleprev, 1));

    for (int level = 1; level < lastlevel; ++level) {
        dirpyr_channel(dirpyrlo[level - 1], dirpyrlo[level], srcwidth, srcheight, level,
                       std::max(scalesloc[level] / scaleprev, 1));
    }
...
#pragma omp parallel for
    ...
    for (int level = lastlevel - 1; level > 0; level--) {
        idirpyr_eq_channel_loc(dirpyrlo[level], dirpyrlo[level - 1], residbuff, srcwidth,
                               srcheight, level, multi[level], dirpyrThreshold, nullptr,
                               nullptr, skinprot, gamutlab, b_l, t_l, t_r, b_r, choice,
                               scaleprev, multiThread);
    }

    scale = scalesloc[0];

    idirpyr_eq_channel_loc(dirpyrlo[0], src, residbuff, srcwidth, srcheight, 0, multi[0],
                           dirpyrThreshold, nullptr, nullptr, skinprot, gamutlab, b_l,
                           t_l, t_r, b_r, choice, scaleprev, multiThread);
...
#pragma omp parallel for
    ...
    for (int i = 0; i < srcheight; i++) {
        for (int j = 0; j < srcwidth; j++) {
            loct[i][j] =
                LIM(residbuff[i][j], 0.f, 32768.f);  // TODO: Really a clip necessary?
        }
    }
...
#pragma omp parallel for
        ...
        for (int i = 0; i < srcheight; i++) {
            for (int j = 0; j < srcwidth; j++) {
                loctemp[i][j] =
                    LIM((1.f + clar) * loct[i][j] - clar * resid5[i][j], 0.f, 32768.f);
            }
        }
    } else {
#ifdef _OPENMP
