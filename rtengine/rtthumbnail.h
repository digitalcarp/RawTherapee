...
class Thumbnail
{
    ...
    double autoWBTemp, autoWBGreen, wbEqual,
        wbTempBias;  // autoWBTemp and autoWBGreen are updated each time autoWB is
                     // requested and if wbEqual has been modified
...
public:
    ...
    static Thumbnail* loadFromRaw(const Glib::ustring& fname,
                                  ...
                                  bool forHistogramMatching = false);
    static Thumbnail* loadFromImage(const Glib::ustring& fname,
                                    int& w,
                                    int& h,
                                    int fixwh,
                                    double wbEq,
    ...
    void getSpotWB(const procparams::ProcParams& params,
                   int x,
                   int y,
                   int rect,
                   double& temp,
    ...
    unsigned char* getImage8Data();  // accessor to the 8bit image if it is one, which
                                     // should be the case for the "Inspector" mode.

    // Hombre: ... let's hope that proper template can make this cleaner

    static ImageIO*
    resizeToSameType(int nw, int nh, TypeInterpolation interp, ImageIO* srcImg)
    {
