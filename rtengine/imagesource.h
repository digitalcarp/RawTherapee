...
protected:
    double redAWBMul, greenAWBMul,
        blueAWBMul;  // local copy of the multipliers, to avoid recomputing the values
...
public:
    ImageSource()
        : references(1),
          ...
          embProfile(nullptr),
          idata(nullptr),
          dirpyrdenoiseExpComp(RT_INFINITY)
    {
    }

    ~ImageSource() override {}
    virtual int load(const Glib::ustring& fname) = 0;
    virtual void preprocess(const procparams::RAWParams& raw,
                            const procparams::LensProfParams& lensProf,
                            const procparams::CoarseTransformParams& coarse,
                            float& reddeha,
                            float& greendeha,
                            float& bluedeha,
                            bool prepareDenoise = true) {};
    virtual void demosaic(const procparams::RAWParams& raw,
                          bool autoContrast,
                          double& contrastThreshold,
                          bool cache = false) {};
    virtual void retinex(const procparams::ColorManagementParams& cmp,
                         ...
                         LUTu& histLRETI) {};
    virtual void
    retinexPrepareCurves(const procparams::RetinexParams& retinexParams,
                         LUTf& cdcurve,
                         LUTf& mapcurve,
                         ...
                         LUTu& lhist16RETI,
                         LUTu& histLRETI) {};
    virtual void retinexPrepareBuffers(const procparams::ColorManagementParams& cmp,
    ...
    virtual bool isRGBSourceModified()
        const = 0;  // tracks whether cached rgb output of demosaic has been modified
    ...
    // use right after demosaicing image, add coarse transformation and put the result in
    // the provided Imagefloat*
    virtual void getImage(const ColorTemp& ctemp,
                          int tran,
                          Imagefloat* image,
                          const PreviewProps& pp,
                          const procparams::ToneCurveParams& hlp,
                          const procparams::RAWParams& raw) = 0;
    virtual eSensorType getSensorType() const = 0;
    virtual bool isMono() const = 0;
    // true is ready to provide the AutoWB, i.e. when the image has been demosaiced for
    // RawImageSource
    virtual bool isWBProviderReady() = 0;

    virtual void convertColorSpace(
        Imagefloat* image,
        const procparams::ColorManagementParams& cmp,
        const ColorTemp& wb) = 0;  // DIRTY HACK: this method is derived in rawimagesource
                                   // and strimagesource, but (...,RAWParams raw) will be
    ...
    virtual void getAutoWBMultipliersitc(bool extra,
                                         ...
                                         const procparams::ColorManagementParams& cmp,
                                         const procparams::RAWParams& raw,
                                         const procparams::ToneCurveParams& hrp) = 0;
    virtual void getAutoWBMultipliersItcGreen(procparams::ProcParams& params,
                                              bool forcewbgrey,
                                              int kcam,
                                              double greenitc,
                                              bool extra,
    ...
    virtual ColorTemp getSpotWB(std::vector<Coord2D>& red,
                                std::vector<Coord2D>& green,
                                std::vector<Coord2D>& blue,
                                int tran,
                                double equal,
                                StandardObserver observer) = 0;
    virtual void WBauto(bool extra,
                        double& tempref,
                        double& greenref,
                        ...
                        const procparams::RAWParams& raw,
                        const procparams::ToneCurveParams& hrp) = 0;
    virtual void getrgbloc(int begx,
                           int begy,
                           int yEn,
                           int xEn,
                           int cx,
                           ...
                           int bf_h,
                           int bf_w,
                           const procparams::WBParams& wbpar) = 0;
    ...
    virtual DCPProfile* getDCP(const procparams::ColorManagementParams& cmp,
    ...
    void increaseRef() final { references++; }
    ...
    // for RAW files, compute a tone curve using histogram matching on the embedded
    // thumbnail
    virtual void getAutoMatchedToneCurve(const procparams::ColorManagementParams& cp,
                                         const procparams::RAWParams& rawParams,
                                         StandardObserver observer,
                                         std::vector<double>& outCurve)
    {
        outCurve = { 0.0 };
    }

    double getDirPyrDenoiseExpComp() const { return dirpyrdenoiseExpComp; }
    // functions inherited from the InitialImage interface
    Glib::ustring getFileName() const final override { return fileName; }
    cmsHPROFILE getEmbeddedProfile() const final override { return embProfile; }
    const FramesMetaData* getMetaData() const final override { return idata; }
    ImageSource* getImageSource() override { return this; }
    virtual void getRawValues(int x, int y, int rotate, int& R, int& G, int& B) = 0;
    virtual void
    captureSharpening(const procparams::CaptureSharpeningParams& sharpeningParams,
                      bool showMask,
    ...
    virtual bool getDeconvAutoRadius_capturesharpening_SE(float* out = nullptr)
    {
        return false;
    }
};
