...
public:
    Glib::ustring
        iccDirectory;       ///< The directory containing the possible output icc profiles
    ...
    Glib::ustring
        printerProfile;  ///< ICC profile name used for soft-proofing a printer output
    int printerIntent;   ///< Colorimetric intent used with the above profile
    bool printerBPC;     ///< Black Point Compensation for the Labimage->Printer->Monitor
                         ///< transform
    ...
    bool monitorBPC;  ///< Black Point Compensation for the Labimage->Monitor transform
                      ///< (directly, i.e. not soft-proofing and no WCS in between)
    bool autoMonitorProfile;  ///< Try to auto-determine the correct monitor color profile
    bool autocielab;
    bool
        rgbcurveslumamode_gamut;  // controls gamut enforcement for RGB curves in lumamode
    bool verbose;
    ...
    Glib::ustring
        prophoto;  // filename of Prophoto     profile (default to the bundled one)
    Glib::ustring
        widegamut;        // filename of WidegamutRGB profile (default to the bundled one)
    ...
    Glib::ustring
        rec2020;  // filename of Rec2020      profile (default to the bundled one)
    Glib::ustring
        ACESp0;  // filename of ACES P0      profile (default to the bundled one)
    Glib::ustring JDCmax;  // filename of JDCmax      profile (default to the bundled one)
    Glib::ustring
        ACESp1;  // filename of ACES P1      profile (default to the bundled one)
    Glib::ustring
        DCIP3;  // filename of DCIP3         profile (default to the bundled one)

    bool gamutICC;  // no longer used
    bool gamutLch;
    bool HistogramWorking;  // true: histogram is display the value of the image computed
    ...
    Glib::ustring
        lensfunDbDirectory;  // The directory containing the lensfun database. If empty,
                             // the system defaults will be used, as described in
    ...
    enum class ThumbnailInspectorMode { JPEG, RAW, RAW_IF_NOT_JPEG_FULLSIZE };
    ...
    enum class MetadataXmpSync { NONE, READ, READ_WRITE };
