...
{
    // thresh = threshold for performing green equilibration; max percentage difference of
    // G1 vs G2 G1-G2 differences larger than this will be assumed to be Nyquist texture,
    // and left untouched
...
#pragma omp for schedule(dynamic, 16)
        ...
        for (int rr = 4; rr < height - 4; rr++) {
            ...
            for (; cc < width - 12; cc += 8) {
                ...
                vfloat c1 =
                    (vabsf(o1_1 - o1_2) + vabsf(o1_1 - o1_3) + vabsf(o1_1 - o1_4)
                     + vabsf(o1_2 - o1_3) + vabsf(o1_3 - o1_4) + vabsf(o1_2 - o1_4));
                vfloat c2 =
                    (vabsf(o2_1 - o2_2) + vabsf(o2_1 - o2_3) + vabsf(o2_1 - o2_4)
                     + vabsf(o2_2 - o2_3) + vabsf(o2_3 - o2_4) + vabsf(o2_2 - o2_4));
                ...
                if (_mm_movemask_ps((vfloat)mask1))
                {  // if for any of the 4 pixels the condition is true, do the maths for
                   // all 4 pixels and mask the unused out at the end
                    // pixel interpolation
                    ...
                    vfloat ginterp = (gse * wtse + gnw * wtnw + gne * wtne + gsw * wtsw)
                                     / (wtse + wtnw + wtne + wtsw);

                    vfloat val = vself(vmaskf_lt(ginterp - gin, tfv * (ginterp + gin)),
            ...
            for (; cc < width - 6; cc += 2) {
                ...
                float c1 = (fabs(o1_1 - o1_2) + fabs(o1_1 - o1_3) + fabs(o1_1 - o1_4)
                            + fabs(o1_2 - o1_3) + fabs(o1_3 - o1_4) + fabs(o1_2 - o1_4));
                float c2 = (fabs(o2_1 - o2_2) + fabs(o2_1 - o2_3) + fabs(o2_1 - o2_4)
                ...
                if (c1 + c2 < 6 * tf * std::fabs(d1 - d2)) {
                    ...
                    float wtse =
                        1.f
                        / (eps + SQR(gmp2p2) + SQR(cfa[rr + 3][(cc + 3) >> 1] - o1_4));
                    float wtnw =
                        1.f
                        / (eps + SQR(gmm2m2) + SQR(cfa[rr - 3][(cc - 3) >> 1] - o1_1));
                    float wtne =
                        1.f
                        / (eps + SQR(gmm2p2) + SQR(cfa[rr - 3][(cc + 3) >> 1] - o1_2));
                    float wtsw =
                        1.f
                        / (eps + SQR(gmp2m2) + SQR(cfa[rr + 3][(cc - 3) >> 1] - o1_3));

                    float ginterp = (gse * wtse + gnw * wtnw + gne * wtne + gsw * wtsw)
                                    / (wtse + wtnw + wtne + wtsw);
