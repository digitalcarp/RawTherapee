...
std::unique_ptr<LUTf> ImProcFunctions::buildMeaLut(const float inVals[11],
                                                   const float mea[10],
                                                   float& lutFactor)
{
...
void ImProcFunctions::ip_wavelet(LabImage* lab,
                                 LabImage* dst,
                                 int kall,
                                 const procparams::WaveletParams& waparams,
                                 const WavCurve& wavCLVCcurve,
                                 const WavCurve& wavdenoise,
                                 WavCurve& wavdenoiseh,
                                 const Wavblcurve& wavblcurve,
                                 const WavOpacityCurveRG& waOpacityCurveRG,
                                 const WavOpacityCurveSH& waOpacityCurveSH,
                                 const WavOpacityCurveBY& waOpacityCurveBY,
                                 const WavOpacityCurveW& waOpacityCurveW,
                                 const WavOpacityCurveWL& waOpacityCurveWL,
                                 const LUTf& wavclCurve,
                                 int skip)

{
    ...
    FlatCurve* wavguidCurve = new FlatCurve(params->wavelet.wavguidcurve);  // curve
                                                                            // H=f(H)
    ...
    cp.noiseena =
        params->wavelet.expnoise
        && minwinnoise > 128;  // 128 limit for 6 levels wavelet denoise issue 7146
    ...
    cp.maxilev = maxmul;
    static const float scales[10] = { 1.f,  2.f,  4.f,   8.f,   16.f,
                                      32.f, 64.f, 128.f, 256.f, 512.f };
    float scaleskip[10];
    ...
    */
    ...
    cp.strength =
        rtengine::min(1.f, rtengine::max(0.f, ((float)params->wavelet.strength / 100.f)));
    ...
    cp.a_scale = (params->wavelet.labgridAHigh - params->wavelet.labgridALow) / cp.factor
                 / cp.scaling;
    cp.a_base = params->wavelet.labgridALow / cp.scaling;
    cp.b_scale = (params->wavelet.labgridBHigh - params->wavelet.labgridBLow) / cp.factor
                 / cp.scaling;
    ...
    {  // reduce strength if zoom < 100%  for chroma and tuning
        if (sc == 0) {
            if (scaleskip[sc] < 1.f) {
    ...
    */
    ...
    Tile_calc(tilesize, overlap, kall, imwidth, imheight, numtiles_W, numtiles_H,
    ...
    if (kall != 0) {
        ...
        if ((maxnumberofthreadsforwavelet == 6 || maxnumberofthreadsforwavelet == 8)
            && levwav == 10)
        {
            maxnumberofthreadsforwavelet -= 2;
        }
    ...
    // Calculate number of tiles. If less than omp_get_max_threads(), then limit
    // num_threads to number of tiles
    ...
    if (rgbDenoiseThreadLimit > 0) {
        maxnumberofthreadsforwavelet =
            rtengine::LIM(rgbDenoiseThreadLimit / 2, 1, maxnumberofthreadsforwavelet);
    }
    ...
    if (settings->verbose) {
        printf(
            "Ip Wavelet uses %d main thread(s) and up to %d nested thread(s) for each main thread\n",
...
#pragma omp for schedule(dynamic) collapse(2)
        ...
        for (int tiletop = 0; tiletop < imheight; tiletop += tileHskip) {
            for (int tileleft = 0; tileleft < imwidth; tileleft += tileWskip) {
                ...
                if (numtiles == 1)
                {  // untiled processing => we can use output buffer for labco
                    labco = dst;
                    ...
                    if (cp.avoi) {  // we need a buffer to hold a copy of the L channel
                        ...
                        memcpy(LoldBuffer, lab->L[0],
                               tilewidth * tileheight * sizeof(float));
...
#pragma omp parallel for num_threads(wavNestedLevels) if (wavNestedLevels > 1)
                    ...
                    for (int i = 1; i < hei - 1; i++) {
                        for (int j = 1; j < wid - 1; j++) {
                            if ((varhue[i][j] < -1.3f && varhue[i][j] > -2.5f)
                                && (varchro[i][j] > 15.f && varchro[i][j] < 55.f)
                                && labco->L[i][j] > 6000.f)
                            {  // blue sky + med3x3  ==> after for more effect use denoise
                                tmL[i][j] =
                                    median(labco->L[i][j], labco->L[i - 1][j],
                                           labco->L[i + 1][j], labco->L[i][j + 1],
                                           labco->L[i][j - 1], labco->L[i - 1][j - 1],
                                           labco->L[i - 1][j + 1], labco->L[i + 1][j - 1],
                                           labco->L[i + 1][j + 1]);  // 3x3
                            }
                        }
                    }
                ...
                if (numtiles == 1) {
                    // reduce the varhue array to get faster access in following
                    // processing and reduce peak memory usage
                ...
                {  // reduce the varhue array to get faster access in following processing
                    for (int i = 0; i < (tileheight + 1) / 2; i++) {
                        for (int j = 0; j < (tilewidth + 1) / 2; j++) {
                ...
                if ((cp.lev0s > 0.f || cp.lev1s > 0.f || cp.lev2s > 0.f || cp.lev3s > 0.f)
                    && cp.noiseena)
                {
                    ref0 = true;
                }
                ...
                if (exblurL) {
                    if (cp.mul[0] == 0.f) {
                        cp.mul[0] = 0.01f;  // to always enable WaveletcontAllL if no
                                            // contrast is needed
                    }
                }
                ...
                //  (double)cp.mul[5],(double)cp.mul[6],(double)cp.mul[7],(double)cp.mul[8]);
                ...
                if (levwavL < 5 && cp.noiseena) {
                    levwavL = 6;  // to allow edge and denoise  => I always allocate 3 (4)
                                  // levels..because if user select wavelet it is to do
                                  // something !!
                }

                if (!exblurL && cp.contrast == 0.f && cp.blurres == 0.f && !cp.noiseena
                    && !cp.tonemap && !cp.resena && !cp.chromena && !cp.toningena
                    && !cp.finena && !cp.edgeena && cp.conres == 0.f && cp.conresH == 0.f
                    && cp.val == 0 && !ref0 && params->wavelet.CLmethod == "all")
                {  // no processing of residual L or edge=> we probably can reduce the
                   // number of levels
                    while (levwavL > 0 && cp.mul[levwavL - 1] == 0.f)
                    {  // cp.mul[level] == 0.f means no changes to level
                        levwavL--;
                    }
                }
                bool isdenoisL = (cp.lev0n > 0.1f || cp.lev1n > 0.1f || cp.lev2n > 0.1f
                                  || cp.lev3n > 0.1f || cp.lev4n > 0.1f);
                ...
                */
                ...
                if (levwavL > 0) {
                    const std::unique_ptr<wavelet_decomposition> Ldecomp(
                        new wavelet_decomposition(
                            labco->data, labco->W, labco->H, levwavL, 1, skip,
                            rtengine::max(1, wavNestedLevels), DaubLen));
                    //   const std::unique_ptr<wavelet_decomposition> Ldecomp2(new
                    //   wavelet_decomposition(labco->data, labco->W, labco->H, levwavL,
                    //   1, skip, rtengine::max(1, wavNestedLevels), DaubLen));
...
#pragma omp parallel for schedule(dynamic) collapse(2) \
    num_threads(wavNestedLevels) if (wavNestedLevels > 1)
#endif
                        ...
                        for (int lvl = 0; lvl < levwavL; lvl++) {
                            for (int dir = 1; dir < 4; dir++) {
                                ...
                                const float* const* WavCoeffs_L =
                                    Ldecomp->level_coeffs(lvl);

                                madL[lvl][dir - 1] =
                                    SQR(Mad(WavCoeffs_L[dir], Wlvl_L * Hlvl_L));

                                if (settings->verbose) {
                                    //  printf("Luminance noise estimate (sqr) madL=%.0f
                                    //  lvl=%i dir=%i\n", madL[lvl][dir - 1], lvl, dir -
                                    //  1);
                                }
                            }
                        }
                        ...
                        if ((cp.lev0s > 0.f || cp.lev1s > 0.f || cp.lev2s > 0.f
                             || cp.lev3s > 0.f)
                            && cp.noiseena)
                        {
                            ref = true;
                        ...
                        //   if (cp.val > 0 || ref || contr || cp.denoicurv ||
                        //   cp.denoicurvh || cp.noiseena  || cp.levdenlow > 0.f ||
                        //   cp.thrden > 0.f ) { //edge
                        if (cp.val > 0 || ref || contr || cp.denoicurv || cp.denoicurvh
                            || cp.noiseena || cp.thrden > 0.f)
                        {  // edge
                        ...
                        vari[0] =
                            0.8f * SQR((cp.lev0n / 125.f) * (1.f + cp.lev0n / 25.f));
                        vari[1] =
                            0.8f * SQR((cp.lev1n / 125.f) * (1.f + cp.lev1n / 25.f));
                        vari[2] =
                            0.8f * SQR((cp.lev2n / 125.f) * (1.f + cp.lev2n / 25.f));
                        vari[3] =
                            0.8f * SQR((cp.lev3n / 125.f) * (1.f + cp.lev3n / 25.f));
                        vari[4] =
                            0.8f * SQR((cp.lev4n / 125.f) * (1.f + cp.lev4n / 25.f));
                        vari[5] =
                            0.8f * SQR((cp.lev4n / 125.f) * (1.f + cp.lev4n / 25.f));
                        float kr3 = 1.f;
                            ...
                            && cp.noiseena)
                        {
                            ...
                            const std::unique_ptr<wavelet_decomposition> Ldecomp2(
                                new wavelet_decomposition(
                                    labco->data, labco->W, labco->H, levwavL, 1, skip,
                                    rtengine::max(1, wavNestedLevels), DaubLen));
                            if (!Ldecomp2->memory_allocation_failed()) {
                                if (settings->verbose) {
                                    printf(
                                        "LUM var0=%f var1=%f var2=%f var3=%f var4=%f\n",
                                        vari[0], vari[1], vari[2], vari[3], vari[4]);
                                }

                                //     float* noisevarlum = nullptr;  // we need a dummy
                                //     to pass it to WaveletDenoiseAllL
                                int GWL = labco->W;
                                int GHL = labco->H;
                                ...
                                constexpr float nvlh[13] = {
                                    1.f, 1.f, 1.f, 1.f, 1.f,  1.f, 1.f,
                                    1.f, 1.f, 1.f, 1.f, 0.7f, 0.5f
                                };  // high value
                                constexpr float nvll[13] = { 0.1f, 0.15f, 0.2f, 0.25f,
                                ...
                                const float ac =
                                    (nvlh[index] - nvll[index]) / (seuillow - seuilhigh);
...
#pragma omp parallel for
                                    ...
                                    for (int ir = 0; ir < GHL; ir++)
                                        for (int jr = 0; jr < GWL; jr++) {
                                            float hueG = xatan2f(labco->b[ir][jr],
                                                                 labco->a[ir][jr]);
                                            noisevarhue[(ir >> 1) * GW2L + (jr >> 1)] =
                                                1.f
                                                + 2.f
                                                      * (static_cast<float>(
                                                             wavhueCurve->getVal(
                                                                 Color::huelab_to_huehsv2(
                                                                     hueG)))
                                                         - 0.5f);
                                            noisevarlum[(ir >> 1) * GW2L + (jr >> 1)] *=
                                                noisevarhue[(ir >> 1) * GW2L + (jr >> 1)];
                                ...
                                if (cp.quamet == 0) {
                                    ...
                                    WaveletDenoiseAllL(*Ldecomp, noisevarlum, madL, vari,
                                                       edge, 1);
                                } else {
                                    if (settings->verbose) {
                                        printf("denoise bishrink L\n");
                                    }
                                    WaveletDenoiseAll_BiShrinkL(*Ldecomp, noisevarlum,
                                                                madL, vari, edge, 1);
                                ...
                                Evaluate2(*Ldecomp, meand, meanNd, sigmad, sigmaNd, MaxPd,
                                          MaxNd, wavNestedLevels);
                                for (int dir = 1; dir < 4; dir++) {
                                    for (int level = 0; level < levwavL; level++) {
                                        if (mean[level] < 0.1f || meand[level] < 0.1f
                                            || sigma[level] < 0.1f
                                            || sigmad[level] < 0.1f)
                                        {
                                ...
                                if (thrend < 0.01f)
                                    thr = 0.95f;
                                else if (thrend < 0.02f)
                                    thr = 0.9f;
                                else if (thrend < 0.04f)
                                    thr = 0.8f;
                                else if (thrend < 0.06f)
                                    thr = 0.7f;
                                else if (thrend < 0.08f)
                                    thr = 0.6f;
                                else if (thrend < 0.1f)
                                    thr = 0.5f;
                                else if (thrend < 0.2f)
                                ...
                                FlatCurve wavlow({ FCT_MinMaxCPoints, 0, 1, 0.35, 0.35,
                                                   thrend, 1.0, 0.35, 0.35,
                                                   thrend + 0.01f, thr, 0.35, 0.35, 1,
                                                   thr, 0.35, 0.35 });
                                // for level 4
                                float thrhigh = 0.f;
                                ...
                                if (threndhigh < 0.01f)
                                    thrhigh = 0.95f;
                                else if (threndhigh < 0.02f)
                                    thrhigh = 0.9f;
                                else if (threndhigh < 0.04f)
                                    thrhigh = 0.8f;
                                else if (threndhigh < 0.06f)
                                    thrhigh = 0.7f;
                                else if (threndhigh < 0.08f)
                                    thrhigh = 0.6f;
                                else if (threndhigh < 0.1f)
                                    thrhigh = 0.5f;
                                else
                                    thrhigh = 0.f;

                                FlatCurve wavhigh({ FCT_MinMaxCPoints, 0, 1, 0.35, 0.35,
                                                    threndhigh, 1.0, 0.35, 0.35,
                                                    threndhigh + 0.01f, thrhigh, 0.35,
                                                    0.35, 1, thrhigh, 0.35, 0.35 });

                                float thrmed = 0.f;
                                float threndmed = 1.f - cp.limden;
                                if (threndmed < 0.02f)
                                    thrmed = 0.5f;
                                else if (threndmed < 0.05f)
                                    thrmed = 0.2f;
                                else
                                    thrmed = 0.f;

                                FlatCurve wavmed({ FCT_MinMaxCPoints, 0, 1, 0.35, 0.35,
                                                   threndmed, 1.0, 0.35, 0.35,
                                                   threndmed + 0.01f, thrmed, 0.35, 0.35,
                                                   1, thrmed, 0.35, 0.35 });
                                ...
                                //                                    sig5=%f\n",
                                //                                    siglh[0],
                                //                                    siglh[1],siglh[2],siglh[3],siglh[4],siglh[5]);
                                ...
                                if (execut && exitifzero) {
                                    for (int dir = 1; dir < 4; dir++) {
                                        for (int level = 0; level < levref; level++) {
                                            ...
                                            const float* const WavCoeffs_L =
                                                Ldecomp->level_coeffs(
                                                    level)[dir];  // first decomp denoised
                                            const float* const WavCoeffs_L2 =
                                                Ldecomp2->level_coeffs(
                                                    level)[dir];  // second decomp before
                                                                  // denoise
                                            ...
                                            // not denoise
                                            const auto WavL02 =
                                                Ldecomp2->level_coeffs(0)[dir];
                                            const auto WavL12 =
                                                Ldecomp2->level_coeffs(1)[dir];
                                            const auto WavL22 =
                                                Ldecomp2->level_coeffs(2)[dir];
                                            const auto WavL32 =
                                                Ldecomp2->level_coeffs(3)[dir];
                                            const auto WavL42 =
                                                Ldecomp2->level_coeffs(k4)[dir];
                                            const auto WavL52 =
                                                Ldecomp2->level_coeffs(k5)[dir];
                                            if (settings->verbose) {
                                                printf(
                                                    "level=%i mean=%.0f meanden=%.0f sigma=%.0f  sigmaden=%.0f Max=%.0f Maxden=%.0f\n",
                                                    level, mean[level], meand[level],
                                                    sigma[level], sigmad[level],
                                                    MaxP[level], MaxPd[level]);
                                            }

                                            // find local contrast
                                            constexpr float weights[4] = { 0.f, 0.5f,
                                                                           0.7f, 1.f };
                                            const float weightL =
                                                weights[rtengine::LIM(cp.mixmet, 0, 3)];
                                            const float tempmean =
                                                intp(weightL, meand[level], mean[level]);
                                            const float tempsig = intp(
                                                weightL, sigmad[level], sigma[level]);
                                            const float tempmax =
                                                intp(weightL, MaxPd[level], MaxP[level]);
...
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic, Wlvl_L * 16) \
    num_threads(wavNestedLevels) if (wavNestedLevels > 1)
#endif
                                                for (int i = 0; i < Wlvl_L * Hlvl_L; i++)
                                                {
                                                    float absciss;
                                                    const float tempwav = std::fabs(
                                                        intp(weightL, WavCoeffs_L[i],
                                                             WavCoeffs_L2[i]));

                                                    if (tempwav >= maxVal) {  // for max
                                                        absciss = xexpf(
                                                            (xlogf(tempwav) - logmax)
                                                            * rap);
                                                    ...
                                                    if (cp.slimet == 0) {
                                                        ...
                                                        kc = wavdenoise[absciss * 500.f]
                                                             - 1.f;
                                                    }
                                                    if (kc < 0) {
                                                        kc = -SQR(kc);  // approximation
                                                                        // to simulate
                                                                        // sliders denoise
                                                    }
                                                    kc *= kcFactor;

                                                    const float reduceeffect =
                                                        kc <= 0.f
                                                            ? 1.f
                                                            : 1.2f;  // 1.2 allows to
                                                                     // increase denoise
                                                    ...
                                                    if (cp.complex == 1) {
                                                        if (cp.limden > 0.f) {
                                                            const float kcmed = -SQR(
                                                                wavmed.getVal(absciss)
                                                                - 1.f);
                                                            float kintermed =
                                                                1.f
                                                                + reduceeffect * kcmed;
                                                            kintermed = kintermed <= 0.f
                                                                            ? 0.01f
                                                                            : kintermed;
                                                            WavL0[i] =
                                                                WavL02[i]
                                                        ...
                                                        const float kchigh =
                                                            -SQR(wavhigh.getVal(absciss)
                                                                 - 1.f);
                                                        float kintermhigh =
                                                            1.f + reduceeffect * kchigh;
                                                        kintermhigh = kintermhigh <= 0.f
                                                                          ? 0.01f
                                                                          : kintermhigh;
                                                        WavL4[i] =
                                                            WavL42[i]
                                    ...
                                    if (settings->verbose) {
                                        Evaluate2(*Ldecomp, meand, meanNd, sigmad,
                                                  sigmaNd, MaxPd, MaxNd, wavNestedLevels);
                                        for (int dir = 1; dir < 4; dir++) {
                                            for (int level = 0; level < levref; level++) {
                                                printf(
                                                    "AFTER LC level=%i mean=%.0f meanden=%.0f sigma=%.0f  sigmaden=%.0f Max=%.0f Maxden=%.0f\n",
                                                    level, mean[level], meand[level],
                                                    sigma[level], sigmad[level],
                                                    MaxP[level], MaxPd[level]);
                                            }
                        ...
                        FlatCurve* ChCurve =
                            new FlatCurve(params->wavelet.Chcurve);  // curve C=f(H)
                        bool Chutili = false;
                        ...
                        WaveletcontAllL(labco, varhue, varchro, *Ldecomp, wavblcurve, cp,
                                        skip, mean, sigma, MaxP, MaxN, wavCLVCcurve,
                                        waOpacityCurveW, waOpacityCurveSH, ChCurve,
                                        Chutili);
                        ...
                        WaveletcontAllLfinal(*Ldecomp, cp, mean, sigma, MaxP,
                                             waOpacityCurveWL);

                        // Evaluate2(*Ldecomp, cp, ind, mean, meanN, sigma, sigmaN, MaxP,
                        // MaxN, madL);
                        /*
                        ...
                        */
                        ...
                        if (settings->verbose) {
                            printf(
                                "CHRO var0=%f va1=%f va2=%f va3=%f va4=%f val5=%f va6=%f\n",
                                variC[0], variC[1], variC[2], variC[3], variC[4],
                                variC[5], variC[6]);
                        }

                        /*
                                                for (int y = 0; y < 7; y++) {
                                                    printf("y=%i madL=%f varia=%f
                        ...
                        */
                        ...
                        for (int ir = 0; ir < GH; ir++)
                            for (int jr = 0; jr < GW; jr++) {
                                float cN =
                                    sqrt(SQR(labco->a[ir][jr]) + SQR(labco->b[ir][jr]));

                                if (cN < seuil) {
                                    noisevarchrom[(ir >> 1) * GW2 + (jr >> 1)] = nvch;
                                } else if (cN < seuil2) {
                                    noisevarchrom[(ir >> 1) * GW2 + (jr >> 1)] =
                                        ac * cN + bc;
                                } else {
                        ...
                        FlatCurve* hhCurve =
                            new FlatCurve(params->wavelet.hhcurve);  // curve H=f(H)
                        bool hhutili = false;
                        ...
                        if (!hhutili) {  // always a or b
                            ...
                            if (!exblurab && cp.chrores == 0.f && cp.blurcres == 0.f
                                && !cp.noiseena && !cp.tonemap && !cp.resena
                                && !cp.chromena && !cp.toningena && !cp.finena
                                && !cp.edgeena && params->wavelet.CLmethod == "all"
                                && !cp.cbena)
                            {  // no processing of residual ab => we probably can reduce
                               // the number of levels
                                while (
                                    levwava > 0 && !cp.diag
                                    && (((cp.CHmet == 2
                                          && (cp.chro == 0.f
                                              || cp.mul[levwava - 1] == 0.f))
                                         || (cp.CHmet != 2
                                             && (levwava == 10
                                                 || (!cp.curv
                                                     || cp.mulC[levwava - 1] == 0.f)))))
                                    && (!cp.opaRG || levwava == 10
                                        || (cp.opaRG && cp.mulopaRG[levwava - 1] == 0.f))
                                    && ((levwava == 10
                                         || (cp.CHSLmet == 1
                                             && cp.mulC[levwava - 1] == 0.f))))
                                {
                            ...
                            if (levwava > 0) {
                                const std::unique_ptr<wavelet_decomposition> adecomp(
                                    new wavelet_decomposition(
                                        labco->data + datalen, labco->W, labco->H,
                                        levwava, 1, skip,
                                        rtengine::max(1, wavNestedLevels), DaubLen));
                                if (!adecomp->memory_allocation_failed()) {
                                    ...
                                    if (cp.noiseena
                                        && ((cp.chromfi > 0.f || cp.chromco > 0.f)
                                            && cp.quamet == 0 && isdenoisL))
                                    {
                                        if (settings->verbose) {
                                        ...
                                        WaveletDenoiseAllAB(*Ldecomp, *adecomp,
                                                            noisevarchrom, madL, variC,
                                                            edge, noisevarab_r, true,
                                                            false, false, 1);

                                    } else if (cp.noiseena
                                               && ((cp.chromfi > 0.f && cp.chromco >= 0.f)
                                                   && cp.quamet == 1 && isdenoisL))
                                    {
                                        if (settings->verbose) {
                                            printf("denoise bishrink a \n");
                                        }

                                        WaveletDenoiseAll_BiShrinkAB(
                                            *Ldecomp, *adecomp, noisevarchrom, madL,
                                            variC, edge, noisevarab_r, true, false, false,
                                            1);
                                        WaveletDenoiseAllAB(*Ldecomp, *adecomp,
                                                            noisevarchrom, madL, variC,
                                                            edge, noisevarab_r, true,
                                    ...
                                    Evaluate2(*adecomp, meanab, meanNab, sigmaab,
                                              sigmaNab, MaxPab, MaxNab, wavNestedLevels);

                                    WaveletcontAllAB(labco, varhue, varchro, *adecomp,
                                                     wavblcurve, waOpacityCurveW, cp,
                                                     true, skip, meanab, sigmaab);
                            ...
                            {  // no processing of residual ab => we probably can reduce
                               // the number of levels
                                while (
                                    ...
                                    && (((cp.CHmet == 2
                                          && (cp.chro == 0.f
                                              || cp.mul[levwavb - 1] == 0.f))
                                         || (cp.CHmet != 2
                                             && (levwavb == 10
                                                 || (!cp.curv
                                                     || cp.mulC[levwavb - 1] == 0.f)))))
                                    && (!cp.opaBY || levwavb == 10
                                        || (cp.opaBY && cp.mulopaBY[levwavb - 1] == 0.f))
                                    && ((levwavb == 10
                                         || (cp.CHSLmet == 1
                                             && cp.mulC[levwavb - 1] == 0.f))))
                                {
                            ...
                            if (levwavb > 0) {
                                const std::unique_ptr<wavelet_decomposition> bdecomp(
                                    new wavelet_decomposition(
                                        labco->data + 2 * datalen, labco->W, labco->H,
                                        levwavb, 1, skip,
                                        rtengine::max(1, wavNestedLevels), DaubLen));
                                if (levwavb == 6) {
                                    edge = 1;
                                ...
                                if (!bdecomp->memory_allocation_failed()) {
                                    //  if (cp.noiseena && ((cp.chromfi > 0.f ||
                                    //  cp.chromco > 0.f) && cp.chromco < 2.f )) {
                                    if (cp.noiseena
                                        && ((cp.chromfi > 0.f || cp.chromco > 0.f)
                                            && cp.quamet == 0 && isdenoisL))
                                    {
                                        WaveletDenoiseAllAB(*Ldecomp, *bdecomp,
                                                            noisevarchrom, madL, variCb,
                                                            edge, noisevarab_r, true,
                                                            false, false, 1);
                                        if (settings->verbose) {
                                                   ...
                                                   && cp.quamet == 1 && isdenoisL))
                                    {
                                        ...
                                        WaveletDenoiseAll_BiShrinkAB(
                                            *Ldecomp, *bdecomp, noisevarchrom, madL,
                                            variCb, edge, noisevarab_r, true, false,
                                            false, 1);
                                        WaveletDenoiseAllAB(*Ldecomp, *bdecomp,
                                                            noisevarchrom, madL, variCb,
                                                            edge, noisevarab_r, true,
                                    ...
                                    Evaluate2(*bdecomp, meanab, meanNab, sigmaab,
                                              sigmaNab, MaxPab, MaxNab, wavNestedLevels);
                                    WaveletcontAllAB(labco, varhue, varchro, *bdecomp,
                                                     wavblcurve, waOpacityCurveW, cp,
                                                     false, skip, meanab, sigmaab);
                            ...
                            if (levwavab > 0) {
                                ...
                                if (!adecomp->memory_allocation_failed()
                                    && !bdecomp->memory_allocation_failed())
                                {
                                    if (cp.noiseena
                                        && ((cp.chromfi > 0.f || cp.chromco > 0.f)
                                            && cp.quamet == 0 && isdenoisL))
                                    ...
                                    WaveletcontAllAB(labco, varhue, varchro, *bdecomp,
                                                     wavblcurve, waOpacityCurveW, cp,
                                                     false, skip, meanab, sigmaab);
                                    WaveletAandBAllAB(*adecomp, *bdecomp, cp, hhCurve,
                                                      hhutili);
                ...
                {  // in all case since I add contrast curve
                        ...
                        for (int j = tileleft; j < tileright; j++) {
                            ...
                            if (cp.avoi) {  // Gamut and Munsell
                                ...
                                if (wavclCurve && cp.finena) {
                                    labco->L[i1][j1] =
                                        (0.5f * Lin + 1.5f * wavclCurve[Lin])
                                        / 2.f;  // apply contrast curve
                                }
                                ...
                                float R, G, B;
                                Color::gamutLchonly(HH, sincosv, Lprov1, Chprov1, R, G, B,
                                                    wip, highlight, 0.15f, 0.96f);
                                L = Lprov1 * 327.68f;
                                ...
                                Color::AllMunsellLch(true, Lprov1, Lprov2, HH, Chprov,
                                                     memChprov, correctionHue,
                                                     correctlum);

                                if (correctionHue != 0.f || correctlum != 0.f)
                                {  // only calculate sin and cos if HH changed
                                    if (std::fabs(correctionHue) < 0.015f) {
                                        HH += correctlum;  // correct only if correct
...
#pragma omp parallel for
            ...
            for (int y = 0; y < hh; y++) {
                for (int x = 0; x < ww; x++) {
                    ...
                    float valparam = 1.f
                                     * (static_cast<float>(wavguidCurve->getVal(
                                            Color::huelab_to_huehsv2(hueG)))
                                        - 0.5f);
        ...
        if (thrend > 0.f) {
            ...
            const std::unique_ptr<wavelet_decomposition> LdecompLL(
                new wavelet_decomposition(LL[0], ww, hh, levwavL, 1, skip,
                                          rtengine::max(1, wavNestedLevels), DaubLen));
            const std::unique_ptr<wavelet_decomposition> Ldecompdst(
                new wavelet_decomposition(dst->L[0], ww, hh, levwavL, 1, skip,
                                          rtengine::max(1, wavNestedLevels), DaubLen));
            if (!LdecompLL->memory_allocation_failed()
                && !Ldecompdst->memory_allocation_failed())
            {
                Evaluate2(*LdecompLL, meang, meanNg, sigmag, sigmaNg, MaxPg, MaxNg,
                          wavNestedLevels);
                Evaluate2(*Ldecompdst, mean, meanN, sigma, sigmaN, MaxP, MaxN,
                          wavNestedLevels);
                ...
                FlatCurve wavguid({ FCT_MinMaxCPoints, 0, 1, 0.35, 0.35, thrend, 1.0,
                                    0.35, 0.35, thrend + 0.01f, thr, 0.35, 0.35, 1, thr,
                                    0.35, 0.35 });
                ...
                for (int dir = 1; dir < 4; dir++) {
                    for (int level = 0; level < levwavL - 1; level++) {
                        ...
                        float* const* WavCoeffs_L =
                            LdecompLL->level_coeffs(level);  // first decomp denoised
                        float* const* WavCoeffs_L2 = Ldecompdst->level_coeffs(
                            level);  // second decomp before denoise
                        if (settings->verbose) {
                            printf(
                                "level=%i mean=%.0f meanden=%.0f sigma=%.0f  sigmaden=%.0f Max=%.0f Maxden=%.0f\n",
                                level, mean[level], meang[level], sigma[level],
                                sigmag[level], MaxP[level], MaxPg[level]);
                        }
                        ...
                        {                                       // curve
                            ...
                            const float rapX = (tempmean + sig * tempsig)
                                               / tempmax;  // rapport between sD / max
                            const float inx = log(insigma);
                            const float iny = log(rapX);
                            const float rap = inx / iny;  // koef
...
#pragma omp parallel for schedule(dynamic, Wlvl_L * 16) \
    num_threads(wavNestedLevels) if (wavNestedLevels > 1)
                            ...
                            for (int i = 0; i < Wlvl_L * Hlvl_L; i++) {
                                float absciss;
                                const float tempwav =
                                    std::fabs(0.7f * WavCoeffs_L[dir][i]
                                              + 0.3f * WavCoeffs_L2[dir][i]);
                                ...
                                if (tempwav >= tempmean + sig * tempsig) {  // for max
                                    ...
                                    if (sig == 2.f)
                                    {  // for sig = 2.f we can use a faster calculation
                                       // because the exponent in this case is 0.25
                                ...
                                kc =
                                    kc < 0.f ? -SQR(kc) : kc;  // approximation to
                                                               // simulate sliders denoise

                                const float reduceeffect =
                                    kc <= 0.f ? 1.f : 1.2f;  // 1.2 allows to increase
                                                             // denoise (not used)
                                const float kinterm =
                                    rtengine::max(1.f + reduceeffect * kc, 0.f);
                                WavCoeffs_L[dir][i] = intp(
                                    kinterm, WavCoeffs_L[dir][i],
                                    WavCoeffs_L2[dir][i]);  // interpolate using kinterm
                            }
                        }
                    }
                }
...
void ImProcFunctions::Aver(const float* RESTRICT DataList,
                           int datalen,
                           float& averagePlus,
                           float& averageNeg,
                           float& max,
                           ...
                           int numThreads)
{
    ...
    constexpr float thres =
        32.7f;  // different from zero to take into account only data large enough 32.7 =
                // 0.1 in range 0..100 very low value
    max = 0.f;
    min = RT_INFINITY_F;
#ifdef _OPENMP
...
void ImProcFunctions::Sigma(const float* RESTRICT DataList,
                            int datalen,
                            float averagePlus,
                            float averageNeg,
                            float& sigmaPlus,
                            ...
                            int numThreads)
{
    ...
    float thres = 32.7f;  // different from zero to take into account only data large
                          // enough 32.7 = 0.1 in range 0..100

#ifdef _OPENMP
#pragma omp parallel for reduction(+ : variP, variN, countP, countN) \
    num_threads(numThreads) if (numThreads > 1)
#endif
...
void ImProcFunctions::calceffect(int level,
                                 float* mean,
                                 float* sigma,
                                 float* mea,
                                 float effect,
...
void ImProcFunctions::Eval2(const float* const* WavCoeffs_L,
                            int level,
                            int W_L,
                            int H_L,
                            float* mean,
...
void ImProcFunctions::CompressDR(float* Source,
                                 int W_L,
                                 int H_L,
                                 float Compression,
                                 float DetailBoost)
...
void ImProcFunctions::ContrastResid(float* WavCoeffs_L0,
                                    const cont_params& cp,
                                    int W_L,
                                    int H_L,
                                    float max0)
{
...
#pragma omp parallel for
    ...
    const float Compression = std::exp(
...
void ImProcFunctions::EPDToneMapResid(float* WavCoeffs_L0,
...
#pragma omp parallel for
    ...
    epd2.CompressDynamicRange(WavCoeffs_L0, sca / skip, edgest, Compression, DetailBoost,
                              Iterates, rew);
...
void ImProcFunctions::WaveletcontAllLfinal(wavelet_decomposition& WaveletCoeffs_L,
                                           const cont_params& cp,
                                           float* mean,
                                           float* sigma,
                                           float* MaxP,
                                           const WavOpacityCurveWL& waOpacityCurveWL)
{
    ...
    for (int dir = 1; dir < 4; dir++) {
        for (int lvl = 0; lvl < maxlvl; lvl++) {
            ...
            finalContAllL(WavCoeffs_L, WavCoeffs_L0, lvl, dir, cp, Wlvl_L, Hlvl_L, mean,
                          sigma, MaxP, waOpacityCurveWL);
        }
    }
}

void ImProcFunctions::WaveletcontAllL(LabImage* labco,
                                      float** varhue,
                                      float** varchrom,
                                      ...
                                      bool Chutili)
{
    ...
    if (contrast != 0.f || (cp.tonemap && cp.resena))
    {  // contrast = 0.f means that all will be multiplied by 1.f, so we can skip this
       // step
#ifdef _OPENMP
#pragma omp parallel for reduction(+ : avedbl) reduction(max : max0) \
    num_threads(wavNestedLevels) if (wavNestedLevels > 1)
#endif
    ...
    DiagonalCurve resid_contrast({ DCT_NURBS, 0, 0, avg - avg * (0.6 - contreal / 250.0),
                                   avg - avg * (0.6 + contreal / 250.0),
                                   avg + (1. - avg) * (0.6 - contreal / 250.0),
                                   avg + (1. - avg) * (0.6 + contreal / 250.0), 1, 1 });

    if (contrast != 0.f && cp.resena && max0 > 0.f)
    {  // contrast = 0.f means that all will be multiplied by 1.f, so we can skip this
       // step
...
#pragma omp parallel for num_threads(wavNestedLevels) if (wavNestedLevels > 1)
    ...
    //   if ((cp.conres >= 0.f || cp.conresH >= 0.f) && cp.resena && !cp.oldsh) { //
    //   cp.conres = 0.f and cp.comresH = 0.f means that all will be multiplied by 1.f, so
    //   we can skip this step
    if ((cp.conres >= 0.f || cp.conresH >= 0.f) && cp.resena)
    {  // cp.conres = 0.f and cp.comresH = 0.f means that all will be multiplied by 1.f,
...
#pragma omp parallel for num_threads(wavNestedLevels) if (wavNestedLevels > 1)
        ...
        ImProcFunctions::shadowsHighlights(temp.get(), true, 1, cp.conresH, cp.conres,
                                           cp.radius, skip, cp.thH, cp.th);
...
#pragma omp parallel for
        ...
        for (int i = 0; i < W_L * H_L; i++) {
            ...
            if (LL100 < cp.th) {
                constexpr float alp =
                    3.f;  // increase contrast sahdow in lowlights  between 1 and ??
...
#pragma omp parallel num_threads(wavNestedLevels) if (wavNestedLevels > 1)
#endif
    {
        ...
        //  I was inspired by the principle of Canny and Lipschitz (continuity and
        //  derivability) I adapted the principle but have profoundly changed the
        //  algorithm One can 1) change all parameters and found good parameters;
        ...
        if (cp.detectedge && cp.val > 0)
        {  // enabled Lipschitz control...more memory..more time...
...
#pragma omp for schedule(dynamic) collapse(2)
            ...
            for (int lvl = 0; lvl < 4; lvl++) {
                for (int dir = 1; dir < 4; dir++) {
                    ...
                    float tempkoeli = 0.f;
                    calckoe(WavCoeffs_LL[dir], gradw, tloww, koeLi[lvl * 3 + dir - 1],
                            lvl, W_L, H_L, edd, tempkoeli, tmC);
                    maxkoeLi[lvl * 3 + dir - 1] = tempkoeli;
                    // return convolution KoeLi and maxkoeLi of level 0 1 2 3 and Dir
                    // Horiz, Vert, Diag
                }
            }
                    ...
                    for (int j = 1; j < W_L - 1; j++) {
                        ...
                        if (cp.lip3 && cp.lipp) {
                            ...
                            for (int dir = 1; dir < 4; dir++) {  // neighbours proxi
                                koeLi[lvl * 3 + dir - 1][i * W_L + j] =
                                    (kneigh * koeLi[lvl * 3 + dir - 1][i * W_L + j]
                                     + 2.f * koeLi[lvl * 3 + dir - 1][(i - 1) * W_L + j]
                                     + 2.f * koeLi[lvl * 3 + dir - 1][(i + 1) * W_L + j]
                                     + 2.f * koeLi[lvl * 3 + dir - 1][i * W_L + j + 1]
                                     + 2.f * koeLi[lvl * 3 + dir - 1][i * W_L + j - 1]
                                     + koeLi[lvl * 3 + dir - 1][(i - 1) * W_L + j - 1]
                                     + koeLi[lvl * 3 + dir - 1][(i - 1) * W_L + j + 1]
                                     + koeLi[lvl * 3 + dir - 1][(i + 1) * W_L + j - 1]
                                     + koeLi[lvl * 3 + dir - 1][(i + 1) * W_L + j + 1])
                                    / somm;
                        ...
                        for (int dir = 1; dir < 4; dir++) {
                            // here I evaluate combinaison of vert / diag / horiz...we are
                            // with multiplicators of the signal
                            interm += SQR(koeLi[lvl * 3 + dir - 1][i * W_L + j]);
                        }

                        interm = sqrt(interm);

                        //                  interm /= 1.732f;//interm = pseudo variance
                        //                  koeLi
                        interm *= 0.57736721f;
                        float kampli = 1.f;
                        float eps = 0.0001f;
                        ...
                        float alph = koeLi[lvl * 3][i * W_L + j]
                                     / (koeLi[lvl * 3 + 1][i * W_L + j]
                                        + eps);  // ratio between horizontal and vertical
                        float beta = koeLi[lvl * 3 + 2][i * W_L + j]
                                     / (koeLi[lvl * 3 + 1][i * W_L + j]
                                        + eps);  // ratio between diagonal and horizontal

                        float alipinfl = (eddlipampl - 1.f) / (1.f - eddlipinfl);
                        float blipinfl = eddlipampl - alipinfl;

                        // alph evaluate the direction of the gradient regularity
                        // Lipschitz
                        //  if = 1 we are on an edge
                        //  if 0 we are not
                        //  we can change and use log..or Arctg but why ?? we can change
                        //  if need ...
                        ...
                        if (alph > eddlipinfl && beta < 0.85f * eddlipinfl)
                        {  // 0.85 arbitrary value ==> eliminate from edge if H V D too
                           // different
                            bet = beta;
                        }
                        ...
                        if (alph > eddlipinfl) {
                            AmpLip =
                                alipinfl * alph + blipinfl;  // If beta low reduce kampli
                            kampli = SQR(bet) * AmpLip * aamp;
                        } else {
                            AmpLip = (1.f / eddlipinfl)
                                     * SQR(SQR(alph * bet));  // Strong Reduce if beta low
                            kampli = AmpLip / aamp;
                        }
                        ...
                        koeLi[lvl * 3][i * W_L + j] = koeLi[lvl * 3 + 1][i * W_L + j] =
                            koeLi[lvl * 3 + 2][i * W_L + j] = interm;  // new value
                        // here KoeLi contains values where gradient is high and coef
                        // high, and eliminate low values...
                    }
                }
            }
...
#pragma omp for schedule(dynamic) collapse(2)
        ...
        for (int dir = 1; dir < 4; dir++) {
            for (int lvl = 0; lvl < maxlvl; lvl++) {
                ...
                ContAllL(koeLi, maxkoeLi[lvl * 3 + dir - 1], true, maxlvl, labco, varhue,
                         varchrom, WavCoeffs_L, WavCoeffs_L0, lvl, dir, cp, Wlvl_L,
                         Hlvl_L, skip, mean, sigma, MaxP, MaxN, wavCLVCcurve,
                         waOpacityCurveW, waOpacityCurveSH, ChCurve, Chutili);
                ...
                if (std::min(Wlvl_L, Hlvl_L) > 180) {
                    if (wavblcurve && wavcurvecomp && cp.blena) {
                        ...
                        float lutFactor;
                        const float inVals[] = { 0.05f, 0.2f, 0.7f, 1.f,  1.f,  0.8f,
                                                 0.6f,  0.4f, 0.2f, 0.1f, 0.01f };
                        ...
                        for (; co < Hlvl_L * Wlvl_L - 3; co += 4) {
                            ...
                            STVFU(WavL[co], intp((*meaLut)[vabsf(valv) * lutFactorv],
                                                 LVFU(aft[co]), valv));
                        }
#endif
                        for (; co < Hlvl_L * Wlvl_L; co++) {
                            WavL[co] = intp((*meaLut)[std::fabs(WavL[co]) * lutFactor],
                                            aft[co], WavL[co]);
                        }
                    }
                }
...
void ImProcFunctions::WaveletAandBAllAB(wavelet_decomposition& WaveletCoeffs_a,
                                        wavelet_decomposition& WaveletCoeffs_b,
                                        const cont_params& cp,
                                        FlatCurve* hhCurve,
                                        bool hhutili)
{
...
#pragma omp for schedule(dynamic, 16)
            ...
            for (int i = 0; i < H_L; i++) {
                ...
                for (; k < W_L; k++) {
                    huebuffer[k] =
                        xatan2f(WavCoeffs_b0[i * W_L + k], WavCoeffs_a0[i * W_L + k]);
                    chrbuffer[k] = sqrtf(SQR(WavCoeffs_b0[i * W_L + k])
                                         + SQR(WavCoeffs_a0[i * W_L + k]))
                                   / 327.68f;
                }
                ...
                for (int j = 0; j < W_L; j++) {
...
#else
                    float hueR =
                        xatan2f(WavCoeffs_b0[i * W_L + j], WavCoeffs_a0[i * W_L + j]);
                    float chR = sqrtf(SQR(WavCoeffs_b0[i * W_L + j])
                                      + SQR(WavCoeffs_a0[i * W_L + j]));
#endif
                    ...
                    */
                    float valparam = (static_cast<float>(
                                          hhCurve->getVal(Color::huelab_to_huehsv2(hueR)))
                                      - 0.5f)
                                         * 1.7f
                                     + hueR;  // get H=f(H)  1.7 optimisation !
...
void ImProcFunctions::WaveletcontAllAB(LabImage* labco,
                                       float** varhue,
                                       float** varchrom,
...
#endif
    {
        if (cp.chrores != 0.f && cp.resena)
        {  // cp.chrores == 0.f means all will be multiplied by 1.f, so we can skip the
...
#pragma omp for nowait
            ...
            for (int i = 0; i < W_L * H_L; i++) {
                ...
                float LL = (labco->L[ii * 2][jj * 2])
                           / 327.68f;  // I use labco but I can use also WavCoeffs_L0
                                       // (more exact but more memory)

                float sca = 1.f;  // amplifier - reducter...about 1, but perhaps 0.6
                                  // or 1.3
...
#pragma omp for schedule(dynamic) collapse(2)
        ...
        for (int dir = 1; dir < 4; dir++) {
            for (int lvl = 0; lvl < maxlvl; lvl++) {
                ...
                ContAllAB(labco, maxlvl, varhue, varchrom, WavCoeffs_ab, WavCoeffs_ab0,
                          lvl, dir, waOpacityCurveW, cp, Wlvl_ab, Hlvl_ab, useChannelA,
                          meanab, sigmaab);
                ...
                if (std::min(Wlvl_ab, Hlvl_ab) > 180) {
                    if (wavblcurve && wavcurvecomp && cp.blena && cp.chrwav > 0.f) {
                        ...
                        // blur level
                        const float klev = wavblcurve[lvl * 55.5f] * 80.f / skip;
                        boxblur(WavCoeffs_ab[dir], aft.get(), klev, Wlvl_ab, Hlvl_ab,
                                false);
                        ...
                        for (; co < Hlvl_ab * Wlvl_ab - 3; co += 4) {
                            ...
                            STVFU(WavAb[co], intp((*meaLut)[vabsf(valv) * lutFactorv],
                                                  LVFU(aft[co]), valv));
                        }
#endif
                        for (; co < Hlvl_ab * Wlvl_ab; co++) {
                            WavAb[co] = intp((*meaLut)[std::fabs(WavAb[co]) * lutFactor],
                                             aft[co], WavAb[co]);
                        }
                    }
                }
...
void ImProcFunctions::calckoe(const float* WavCoeffs,
                              float gradw,
                              float tloww,
                              float* koeLi,
                              int level,
...
#pragma omp parallel for if (multiThread)
        ...
        for (int i = 1; i < H_L - 1; i++) {
            for (int j = 1; j < W_L - 1; j++) {
                tmC[i][j] =
                    c0 * WavCoeffs[i * W_L + j]
                    + c1
                          * ((WavCoeffs[(i - 1) * W_L + j] + WavCoeffs[(i + 1) * W_L + j])
                             + (WavCoeffs[i * W_L + j + 1] + WavCoeffs[i * W_L + j - 1]))
                    + c2
                          * ((WavCoeffs[(i - 1) * W_L + j - 1]
                              + WavCoeffs[(i - 1) * W_L + j + 1])
                             + (WavCoeffs[(i + 1) * W_L + j - 1]
                                + WavCoeffs[(i + 1) * W_L + j + 1]));
            }
                                ...
                                + c3
                                      * ((WavCoeffs[(i - 2) * W_L + j]
                                         ...
                                         + (WavCoeffs[i * W_L + j - 2]
                                            + WavCoeffs[i * W_L + j + 2]))
                                + c4
                                      * ((WavCoeffs[(i - 2) * W_L + j - 1]
                                          + WavCoeffs[(i - 2) * W_L + j + 1])
                                         + (WavCoeffs[(i + 2) * W_L + j + 1]
                                            + WavCoeffs[(i + 2) * W_L + j - 1])
                                         + (WavCoeffs[(i - 1) * W_L + j - 2]
                                            + WavCoeffs[(i - 1) * W_L + j + 2])
                                         + (WavCoeffs[(i + 1) * W_L + j + 2]
                                            + WavCoeffs[(i + 1) * W_L + j - 2]))
                                + c5
                                      * ((WavCoeffs[(i - 2) * W_L + j - 2]
                                          + WavCoeffs[(i - 2) * W_L + j + 2])
                                         + (WavCoeffs[(i + 2) * W_L + j - 2]
                                            + WavCoeffs[(i + 2) * W_L + j + 2]));
                }
...
#pragma omp parallel for if (multiThread)
    ...
    const float thr2 =
        1.5f * edd + gradw / 30.f;  // edd can be modified in option ed_detect
    ...
    for (int i = borderL; i < H_L - borderL; i++) {
        for (int j = borderL; j < W_L - borderL; j++) {
            ...
            koeLi[i * W_L + j] =
                rtengine::min(thr2, std::fabs(tmC[i][j] / temp));  // limit maxi

            // it will be more complicated to calculate both Wh and Wv, but we have also
            // Wd==> pseudo Lipschitz
...
void ImProcFunctions::complete_local_contrast(
    LabImage* lab,
    LabImage* dst,
    const procparams::WaveletParams& waparams,
    const procparams::ColorManagementParams& cmparams,
    const WavOpacityCurveWL& cmOpacityCurveWL,
    int skip,
    int& level_hr,
    int& maxlevpo,
    bool& wavcurvecont)
{
    ...
    if (wavcurvecont && cmparams.wavExp) {  // enable curve and expander
        ...
        // 6 contrast profiles to change range levels and rolloff for high contrast
        // positive and negative - of course we can add anothers I change only values for
        // LUT for high contrast values and not for low levels, but we can...
        float inva5 = 0.8f;
        float inva6 = 0.7f;
        float inva7 = 0.5f;
        float inva8 = 0.4f;
        ...
        if (pyrwav == 1) {               // low contrast
            ...
            level_hr =
                wavelet_lev
                + 2;  //+1  //be careful the preview must be big enough to see the changes
            if (!cmparams.wsmoothcie) {
                inva5 = 0.8f;    // 0.85
                inva6 = 0.6f;    // 0.75
                inva7 = 0.5f;    // 0.55
        ...
        {  // agresive - maximum - in this case LUT are minimal to avoid artifacts -be
           // careful the preview must be big enough to see the changes
            level_bl = 0;
            level_hl = 0;
        ...
        maxlevpo = maxlvl;
        // decomposition wavelet , we can change Daublen (moment wavelet) in Tab - Wavelet
        // Levels with subsampling = 1
        std::unique_ptr<wavelet_decomposition> wdspot =
            std::unique_ptr<wavelet_decomposition>(new wavelet_decomposition(
                lab->L[0], width, height, maxlvl, 1, skip, numThreads, DaubLen));
...
#pragma omp parallel for if (multiThread)
        ...
        //  declaration with 10 levels to calculate mean , mean negative, sigma, sigma
        //  negative, Max et Max negative for each level
        float mean[10];
        float meanN[10];
        float sigma[10];
        float sigmaN[10];
        ...
        Evaluate2(*wdspot, mean, meanN, sigma, sigmaN, MaxP, MaxN,
                  numThreads);  // calculate mean sigma Max for each levels
        float alow = 1.f;
        float blow = 0.f;
        ...
        for (int dir = 1; dir < 4; dir++) {                        // for each direction
            for (int level = level_bl; level < maxlvl; ++level) {  // for each levels
                ...
                //  I use only positives values to simplify calculations... possible
                //  improvment.
                if (MaxP[level] > 0.f && mean[level] != 0.f && sigma[level] != 0.f) {
                    float insigma = 0.666f;  // SD standard deviation (modelisation)
                    float logmax = log(MaxP[level]);  // log Max
                    float rapX = (offset * mean[level] + sigmafin * sigma[level])
                    ...
                    const float threshold =
                        offset * mean[level]
                        + sigmafin * sigma[level];  // base signal calculation.
                    float lutFactor;  // inva5, inva6, inva7, inva8, inva9, inva10 are
                                      // define in Contrast profiles.
                    float inVals[] = {
                        0.05f, 0.2f,  0.7f,  1.f,   1.f,   inva5,
                        inva6, inva7, inva8, inva9, inva10
                    };  // values to give for calculate LUT along signal : minimal near 0
                        // or MaxP
                    ...
                    for (int y = 0; y < H_L; y++) {
                        for (int x = 0; x < W_L; x++) {  // for each pixel
                            if (cmOpacityCurveWL) {      // if curve enable
                                ...
                                if (WavCL >= threshold)
                                {  // for max take into account attenuation response and
                                   // offset
                                    float valcour = xlogf(fabsf(val));
                                    float valc = valcour - logmax;
                                    float vald = valc * rap;
                                    absciss = xexpf(vald);
                                } else if (WavCL >= offset * mean[level]) {  // offset
                                                                             // only
                                 ...
                                 */
                                float kc =
                                    klev * (cmOpacityCurveWL[absciss * 500.f] - 0.5f);
                                float amplieffect =
                                    kc <= 0.f ? 1.f : 1.7f;  // we can change 1.5 - to 1.7
                                                             // or more or less
...
void ImProcFunctions::finalContAllL(float* const* WavCoeffs_L,
                                    float* WavCoeffs_L0,
                                    int level,
                                    int dir,
                                    const cont_params& cp,
                                    ...
                                    const WavOpacityCurveWL& waOpacityCurveWL)
{
    if (cp.diagcurv && cp.finena && MaxP[level] > 0.f && mean[level] != 0.f
        && sigma[level] != 0.f)
    {                                     // curve
        float insigma = 0.666f;           // SD
        float logmax = log(MaxP[level]);  // log Max
        float rapX = (mean[level] + cp.sigmafin * sigma[level])
                     / (MaxP[level]);  // rapport between sD / max
        float inx = log(insigma);
        float iny = log(rapX);
        float rap = inx / iny;  // koef
...
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic, W_L * 16) \
    num_threads(wavNestedLevels) if (wavNestedLevels > 1)
#endif
        ...
        for (int i = 0; i < W_L * H_L; i++) {
            ...
            if (std::fabs(WavCoeffs_L[dir][i])
                >= (mean[level] + cp.sigmafin * sigma[level]))
            {  // for max
...
void ImProcFunctions::ContAllL(float* koeLi[12],
                               float maxkoeLi,
                               bool lipschitz,
                               int maxlvl,
                               LabImage* labco,
                               const float* const* varhue,
                               const float* const* varchrom,
                               float* const* WavCoeffs_L,
                               float* WavCoeffs_L0,
                               int level,
                               int dir,
                               ...
                               bool Chutili)
{
    ...
    */
    ...
    if (cp.val > 0 && cp.edgeena) {
            ...
            if (cp.detectedge) {
...

                {
                    for (int i = 1; i < H_L - 1; i++) {
                        for (int j = 1; j < W_L - 1; j++) {
                            ...
                            tmC[i][j] = (4.f * WavCoeffs_L[dir][i * W_L + j]
                                         + 2.f * WavCoeffs_L[dir][(i - 1) * W_L + j]
                                         + 2.f * WavCoeffs_L[dir][(i + 1) * W_L + j]
                                         + 2.f * WavCoeffs_L[dir][i * W_L + j + 1]
                                         + 2.f * WavCoeffs_L[dir][i * W_L + j - 1]
                                         + WavCoeffs_L[dir][(i - 1) * W_L + j - 1]
                                         + WavCoeffs_L[dir][(i - 1) * W_L + j + 1]
                                         + WavCoeffs_L[dir][(i + 1) * W_L + j - 1]
                                         + WavCoeffs_L[dir][(i + 1) * W_L + j + 1])
                                        / 16.f;

                            // apply to each direction Wavelet level : horizontal /
                            // vertiacle / diagonal
                        }
                    }
                }
                ...
                for (int i = borderL; i < H_L - borderL; i++) {
                    for (int j = borderL; j < W_L - borderL; j++) {
                        ...
                        koe[i * W_L + j] =
                            rtengine::min(thr2, std::fabs(tmC[i][j] / temp));
        ...
        // end detect edge
        float rad =
            ((float)cp.rad) / 60.f;  // radius ==> not too high value to avoid artifacts
        float value = ((float)cp.val) / 8.f;  // strength

        if (scaleskip[1] < 1.f) {
            float atten01234 = 0.80f;
            value *= (atten01234 * scaleskip[1]);  // for zoom < 100% reduce strength...I
                                                   // choose level 1...but!!
        }
        ...
        float edge = 1.f;
        float lim0 =
            limrad;  // arbitrary limit for low radius and level between 2 or 3 to 30 maxi
        float lev = float(level);
        float repart = (float)cp.til;

        if (cp.reinforce != 2) {
            const float brepart = cp.reinforce == 1 ? edghig : edglow;
        ...
        float koef =
            ak * level
            + bk;  // modulate for levels : more levels high, more koef low ==>
                   // concentrated action on low levels, without or near for high levels
        float expkoef =
            -std::pow(std::fabs(rad - lev), koef);  // reduce effect for high levels
        //  printf("repart=%f\n", repart);

        if (cp.reinforce == 3) {
            if (rad < (lim0 / 60.f) && level == 0) {
                expkoef *= abs(repart);  // reduce effect for low values of rad and
                                         // level=0==> quasi only level 1 is effective
            }
        }

        if (cp.reinforce == 1) {
            if (rad < (lim0 / 60.f) && level == 1) {
                expkoef /= repart;  // increase effect for low values of rad and
                                    // level=1==> quasi only level 0 is effective
            }
        }
        ...
        if (cp.EDmet == 2 && MaxP[level] > 0.f) {  // curve
            ...
            float logmax = log(MaxP[level]);  // log Max
            float rapX =
                (mean[level] + sigma[level]) / (MaxP[level]);  // rapport between sD / max
            float inx = log(insigma);
            float iny = log(rapX);
            float rap = inx / iny;  // koef
            ...
            for (int i = borderL; i < H_L - borderL; i++) {
                for (int j = borderL; j < W_L - borderL; j++) {
                    ...
                    if (cp.detectedge) {
                        if (!lipschitz) {
                            if (cp.eddet > 10.f) {
                                edge = (aedstr * cp.eddet + bedstr)
                                       * (edgePrecalc * (1.f + koe[k]))
                                       / (1.f + 0.9f * maxkoe);
                            } else {
                                edge = (edgePrecalc * (1.f + koe[k]))
                                       / (1.f + 0.9f * maxkoe);
                            }
                        }
                        ...
                        if (lipschitz) {
                            if (level < 4) {
                                edge = 1.f
                                       + (edgePrecalc - 1.f) * (koeLi[level * 3][k])
                                             / (1.f + 0.9f * maxkoeLi);
                            } else {
                                edge = edgePrecalc;
                            }
                    ...
                    if (cp.edgcurv) {
                        ...
                        // result is different not best or bad than threshold slider...but
                        // similar
                        ...
                        if (kc >= 0.f) {
                            ...
                            kinterm =
                                1.f
                                + reduceeffect * kmul
                                      * (wavCLVCcurve[absciss * 500.f]
                                         - 0.5f);  // about 1 to 3 general and big
                                                   // amplification for max (under 0)
                        } else {
                            const float kcd =
                                kmuld * (wavCLVCcurve[absciss * 500.f] - 0.5f);
            ...
            float MaxPCompare =
                MaxP[level]
                * SQR(cp.edg_max / 100.f);  // 100 instead of b_r...case if b_r < 100
            float MaxNCompare =
                MaxN[level]
                * SQR(cp.edg_max
                      / 100.f);  // always reduce a little edge for near max values
            float edgeSdCompare =
                (mean[level] + 1.5f * sigma[level])
                * SQR(
                    cp.edg_sd
                    / t_r);  // 1.5 standard deviation #80% range between mean 50% and 80%
            ...
            float edgMaxFsup = (cp.edg_max / b_r);  // reduce increase of effect for high
                                                    // values contrast..if slider > b_r
            ...
            for (int i = borderL; i < H_L - borderL; i++) {
                for (int j = borderL; j < W_L - borderL; j++) {
                    ...
                    //  Bottom left ==> minimal low value for local contrast (not 0,
                    //  but 5...we can change) 0 10*10 35*35 100*100 substantially
                    //  correspond to the true distribution of low value, mean,
                    //  standard-deviation and max (ed 5, 50, 400, 4000 Top left ==> mean
                    //  reference value (for each level), we can change
                    //  cbrt(cp.edg_mean/10.f) Top Right==> standard deviation (for each
                    //  level) we can change (cp.edg_sd/35.f) bottom right ==> Max for
                    //  positif and negatif contrast we can change cp.edg_max/100.f If we
                    //  move sliders to the left, local contrast is reduced if we move
                    //  sliders to the right local contrast is increased MaxP, MaxN, mean,
                    //  sigma are calculated if necessary (val > 0) by evaluate2(),
                    //  eval2(), aver() , sigma()
                    //                    if (b_r < 100.f  && cp.edg_max / b_r > 1.f) {
                    //                    //in case of b_r < 100 and slider move to right
                    if (cp.edg_max / b_r > 1.f)
                    {  // in case of b_r < 100 and slider move to right
                    ...
                    if (std::fabs(WavCoeffs_L[dir][k]) >= edgeMeanCompare
                        && std::fabs(WavCoeffs_L[dir][k]) < edgeSdCompare)
                    {
    ...
    if (cpMul != 0.f && cp.contena)
    {  // cpMul == 0.f means all will be multiplied by 1.f, so we can skip this
        ...
        for (int i = 0; i < W_L * H_L; i++) {
            ...
            for (int ml = 0; ml < maxlvl; ml++) {
                if (ml < maxlvl / 2) {
                    kH[ml] = (LL100res + ml * delta)
                             / LL100res;  // fixed a priori max to level middle
                } else {
            ...
            if (useChromAndHue) {
                ...
                if (skinprot > 0.f) {
                    Color::SkinSatCbdl2(LL100, modhue, modchro, skinprot, scale, true,
                                        cp.b_l, cp.t_l, cp.t_r, cp.b_r,
                                        0);  // 0 for skin and extand
                } else if (skinprot < 0.f) {
                    Color::SkinSatCbdl2(LL100, modhue, modchro, skinprotneg, scale, false,
                                        cp.b_l, cp.t_l, cp.t_r, cp.b_r, 0);
            ...
            if (Chutili) {
                ...
                float valparam =
                    static_cast<float>(ChCurve->getVal(Color::huelab_to_huehsv2(modhue2)))
                    - 0.5f;  // get valparam=f(H)
            ...
            float diagacc = 1.f;
            float alpha =
                (1024.f + 15.f * (float)cpMul * scale * scale2 * lbeta * diagacc)
                / 1024.f;

            if (cp.HSmet && cp.contena) {
                // if (cp.HSmet  && cp.contena  && waOpacityCurveSH) {//waOpacityCurveSH
                // no long use
                ...
                if (level <= cp.numlevH) {  // in function of levels
                    if ((LL100 > cp.t_lhl * kH[level] && LL100 < cp.t_rhl * kH[level])) {
                        kLlev = alpha;
                    } else if ((LL100 > cp.b_lhl * kH[level]
                                && LL100 <= cp.t_lhl * kH[level]))
                    {
                        kLlev = aaal * LL100 + bbal;
                    } else if ((LL100 > cp.t_rhl * kH[level]
                                && LL100 <= cp.b_rhl * kH[level]))
                    {
                ...
                if (level >= cp.numlevS - 1) {
                    //   if(klevred < 0.f && level >= 3) {//level > 3 to avoid bad use of
                    //   the curve if user put positives values negatives
        ...
        if (cp.opaW && cp.BAmet == 2) {
            ...
            for (int j = 0; j < it; j++) {
                ...
                for (int i = 0; i < W_L * H_L; i++) {
                    ...
                    float k1 = 0.3f
                               * (waOpacityCurveW[6.f * LL100]
                                  - 0.5f);  // k1 between 0 and 0.5    0.5==> 1/6=0.16
                    float k2 = k1 * 2.f;
        ...
        if (cp.BAmet == 1) {
            ...
            for (int j = 0; j < it; j++) {
                ...
                for (int i = 0; i < W_L * H_L; i++) {
                    ...
                    // k1=600.f;
                    // k2=300.f;
                    // k1=0.3f*(waOpacityCurveW[6.f*LL100]-0.5f);//k1 between 0 and 0.5
                    // 0.5==> 1/6=0.16 k2=k1*2.f;
                    if (dir < 3) {
                        kba = 1.f + bal / k1;
                    }
...
void ImProcFunctions::ContAllAB(LabImage* labco,
                                int maxlvl,
                                float** varhue,
                                float** varchrom,
                                float* const* WavCoeffs_ab,
                                float* WavCoeffs_ab0,
                                int level,
                                int dir,
                                const WavOpacityCurveW& waOpacityCurveW,
                                ...
                                float* sigmaab)
{
    ...
    if (cpMul != 0.f && cp.CHmet == 2 && cp.chro != 0.f && cp.chromena)
    {  // cpMul == 0.f or cp.chro = 0.f means all will be multiplied by 1.f, so we can
       // skip this
        const float skinprot = params->wavelet.skinprotect;
        const float skinprotneg = -skinprot;
        const float factorHard = (1.f - skinprotneg / 100.f);
        ...
        for (int i = 0; i < W_ab * H_ab; i++) {
            ...
            const float alphaC =
                (1024.f + 15.f * cpMul * cpChrom * betaab * scale / 50.f) / 1024.f;
    ...
    //  if ( (cp.curv || cp.CHSLmet==1) && cp.CHmet!=2 && level < 9 && cpMulC != 0.f) { //
    //  cpMulC == 0.f means all will be multiplied by 1.f, so we can skip
    if (cp.CHmet != 2 && level < 9 && cpMulC != 0.f && cp.chromena)
    {  // cpMulC == 0.f means all will be multiplied by 1.f, so we can skip
        const float skinprot = params->wavelet.skinprotect;
        const float skinprotneg = -skinprot;
        const float factorHard = (1.f - skinprotneg / 100.f);
    ...
    if ((useOpacity && level < 9 && mulOpacity != 0.f) && cp.toningena) {  // toning
        ...
        for (int co = 0; co < W_ab * H_ab; co++) {
            ...
            if (cp.protab > 0.f) {
                if (useChannelA) {
                    if ((labco->a[ii * 2][jj * 2] > arefmoins1)
                        && (labco->a[ii * 2][jj * 2] < arefplus1))
                    {
                        kreduc1 = 0.5f * protec;

                        if ((labco->a[ii * 2][jj * 2] > 0.8f * arefmoins1)
                            && (labco->a[ii * 2][jj * 2] < 0.8f * arefplus1))
                        {
                            kreduc1 = protec;
                        }
                    }
                    ...
                    if ((labco->b[ii * 2][jj * 2] > brefmoins1)
                        && (labco->b[ii * 2][jj * 2] < brefplus1))
                    {
                        kreduc1 = 0.5f * protec;

                        if ((labco->b[ii * 2][jj * 2] > 0.8f * brefmoins1)
                            && (labco->b[ii * 2][jj * 2] < 0.8f * brefplus1))
                        {
                            kreduc1 = protec;
                        }
                    }
                ...
                if (useChannelA) {
                    if ((labco->a[ii * 2][jj * 2] > arefmoins2)
                        && (labco->a[ii * 2][jj * 2] < arefplus2))
                    {
                        kreduc2 = 0.5f * protec;

                        if ((labco->a[ii * 2][jj * 2] > 0.8f * arefmoins2)
                            && (labco->a[ii * 2][jj * 2] < 0.8f * arefplus2))
                        {
                            kreduc2 = protec;
                        }
                    }
                } else {
                    if ((labco->b[ii * 2][jj * 2] > brefmoins2)
                        && (labco->b[ii * 2][jj * 2] < brefplus2))
                    {
                        kreduc2 = 0.5f * protec;
            ...
            float beta =
                (1024.f + 50.f * mulOpacity * betaab * kreduc1 * kreduc2) / 1024.f;
