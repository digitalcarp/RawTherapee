            ...
            || kind == DCT_CatumullRom)
        {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (std::fabs(x[i] - y[i]) >= 0.000009) {
                    // the smallest possible difference between x and y curve point values
                    // is ~ 0.00001 checking against >= 0.000009 is a bit saver than
                    // checking against >= 0.00001
                    identity = false;
                }
            }
            ...
            if (x[0] == 1.0 && x[1] == 1.0)
            // Avoid crash when first two points are at x = 1 (100 in gui) (git Issue
            // 2923)
            {
                x[0] = 0.99;
            }
            ...
            if ((p.size() == 8 || p.size() == 9)
                && (p.at(4) != 0.0 || p.at(5) != 0.0 || p.at(6) != 0.0 || p.at(7) != 0.0))
            {
                identity = false;
...
void DiagonalCurve::spline_cubic_set()
{
    ...
    delete[] ypp;  // TODO: why do we delete ypp here since it should not be allocated
                   // yet?
    ypp = new double[N];
...
void DiagonalCurve::NURBS_set()
{
    ...
    for (unsigned int i = 0; i < sc_xsize /*sc_x.size()*/; i += 3) {
        // TODO: Speeding-up the interface by caching the polyline, instead of rebuilding
        // it at each action on sliders !!!
        nbr_points = (int)(((double)(ppn + N - 2) * sc_length[i / 3]) / total_length);

        if (nbr_points < 0) {
            for (unsigned int it = 0; it < sc_x.size(); it += 3)
            {  // used unsigned int instead of size_t to avoid %zu in printf
                printf("sc_length[%u/3]=%f \n", it, sc_length[it / 3]);
            }

            printf(
                "NURBS diagonal curve: error detected!\n i=%u nbr_points=%d ppn=%d N=%d sc_length[i/3]=%f total_length=%f",
...
inline double catmull_rom_tj(double ti, double xi, double yi, double xj, double yj)
{
...
inline void
catmull_rom_reflect(double px, double py, double cx, double cy, double& rx, double& ry)
{
#if 0
    double dx = px - cx;
    double dy = py - cy;
...
void catmull_rom_chain(int n_points,
                       int n_cp,
                       double* x,
                       double* y,
                       std::vector<double>& res_x,
                       std::vector<double>& res_y)
{
    ...
    catmull_rom_reflect(x[n_cp - 2], y[n_cp - 2], x[n_cp - 1], y[n_cp - 1], x_last,
                        y_last);
    ...
    for (int i = 0; i < segments; ++i) {
        ...
        catmull_rom_spline(n, i == 0 ? x_first : x[i - 1], i == 0 ? y_first : y[i - 1],
                           x[i], y[i], x[i + 1], y[i + 1],
...
double DiagonalCurve::getVal(double t) const
{
    ...
    case DCT_Parametric:
    {
        ...
        double tv =
            xexp(max(mc * xlog(t), -236.0));  // prevents numerical issues when calling
                                              // pfull, at the cost of minor artifacts
        ...
        if (t < x[2]) {
            ...
            if (htv < 1e-6) {
                return stretched;  // this part of the curve isn't affected by highlight,
    ...
    case DCT_NURBS:
    {
        // get the hash table entry by rounding the value (previously multiplied by
        // "hashSize")
        ...
        if (UNLIKELY(i > (hashSize + 1))) {
            ...
            // corresponding polygon's point #%d (out of %d point) x value: %.8f\n\n", i,
            // t, hash.at(i), poly_x.size(), poly_x[hash.at(i)]);
            printf("\nOVERFLOW: hash #%d is used while seeking for value %.8f\n\n", i, t);
            return t;
        }
