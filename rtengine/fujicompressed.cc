...
void CLASS init_fuji_main_grads(const fuji_compressed_params* params,
...
#pragma omp critical
#endif
        {
            ...
            info->cur_buf_size =
                fread(info->cur_buf, 1, std::min(info->max_read_size, FUJI_BUF_SIZE),
                      info->input);
        }
#endif
...
void CLASS init_fuji_block(struct fuji_compressed_block* info,
...
void CLASS copy_line_to_xtrans(struct fuji_compressed_block* info,
                               int cur_line,
                               int cur_block,
                               int cur_block_width)
{
...
void CLASS copy_line_to_bayer(struct fuji_compressed_block* info,
                              int cur_line,
                              int cur_block,
                              int cur_block_width)
{
...
#define fuji_quant_gradient(max, q, v1, v2) \
    (q->q_grad_mult * q->q_table[(max) + (v1)] + q->q_table[(max) + (v2)])
...
inline void CLASS fuji_read_code(struct fuji_compressed_block* info,
                                 int* data,
                                 int bits_to_read)
{
    *data = 0;
...
int CLASS fuji_decode_sample_even(struct fuji_compressed_block* info,
                                  const struct fuji_compressed_params* params,
                                  ushort* line_buf,
                                  int pos,
                                  struct fuji_grads* grad_params)
{
    int interp_val = 0;
    int errcnt = 0;
...
int CLASS fuji_decode_sample_odd(struct fuji_compressed_block* info,
                                 const struct fuji_compressed_params* params,
                                 ushort* line_buf,
                                 int pos,
                                 struct fuji_grads* grad_params)
...
void CLASS fuji_extend_generic(ushort* linebuf[_ltotal],
                               int line_width,
                               int start,
                               int end)
{
...
void CLASS xtrans_decode_block(struct fuji_compressed_block* info,
                               const struct fuji_compressed_params* params)
{
...
void CLASS fuji_bayer_decode_block(struct fuji_compressed_block* info,
                                   const struct fuji_compressed_params* params)
{
    int r_even_pos = 0, r_odd_pos = 1;
    int g_even_pos = 0, g_odd_pos = 1;
...
void CLASS fuji_decode_strip(fuji_compressed_params* params,
                             int cur_block,
                             INT64 raw_offset,
                             unsigned dsize,
                             uchar* q_bases)
{
    ...
    // release data
    if (!fuji_lossless) free(info_common);
    free(info.linealloc);
#ifndef MYFILE_MMAP
    free(info.cur_buf);
#endif
...
void CLASS fuji_decode_loop(fuji_compressed_params* common_info,
                            int count,
                            INT64* raw_block_offsets,
                            unsigned* block_sizes,
                            uchar* q_bases)
{
...
#pragma omp parallel for schedule( \
        dynamic, 1)  // dynamic scheduling is faster if count > number of cores (e.g.
                     // count for GFX 50S is 12)
#endif
...
void CLASS parse_fuji_compressed_header()
{
    ...
    // general validation
    if (signature != 0x4953 || lossless > 1 || h_raw_height > 0x4002 || h_raw_height < 6
        || h_raw_height % 6 || h_raw_width > 0x4200 || h_raw_width < 0x300
        || h_raw_width % 24 || h_raw_rounded_width > 0x4200
        || h_raw_rounded_width < h_block_size || h_raw_rounded_width % h_block_size
        || h_raw_rounded_width - h_raw_width >= h_block_size || h_block_size != 0x300
        || h_blocks_in_row > 0x10 || h_blocks_in_row == 0
        || h_blocks_in_row != h_raw_rounded_width / h_block_size || h_total_lines > 0xAAB
