...
inline void copyAndClamp(
    const LabImage* src,
    unsigned char* dst,
    const double rgb_xyz[3][3],
    bool multiThread,
    int pro)  // int pro to switch to Working Profile histogram mode without gamma
{
    const int W = src->W;
    const int H = src->H;
...
{
    return vself(vmaskf_le(x, g3), x * s,
                 (F2V(1.f) + g4) * xexpf(xlogf(x) / p) - g4);  // continuous
}
#endif
...
//
// If monitorTransform, divide by 327.68 then apply monitorTransform (which can integrate
// soft-proofing) otherwise divide by 327.68, convert to xyz and apply the sRGB transform,
// before converting with gamma2curve
...
#pragma omp parallel firstprivate(lab, data, W, H)
#endif
        {
            ...
            float* outbuffer =
                gamutWarning ? mBuf.data : pBuf.data;  // make in place transformations
                                                       // when gamutWarning is not needed
...
#pragma omp for schedule(dynamic, 16)
        ...
        copyAndClamp(lab, image->data, sRGB_xyz, multiThread,
                     0);  // int pro = 0 always sent 'normal' to monitor
    }
}
...
// Generate an Image8
//
// If output profile used, divide by 327.68 then apply the "profile" profile (eventually
// with a standard gamma) otherwise divide by 327.68, convert to xyz and apply the RGB
// transform, before converting with gamma2curve
Image8* ImProcFunctions::lab2rgb(LabImage* lab,
                                 int cx,
                                 int cy,
                                 int cw,
                                 int ch,
                                 ...
                                 bool consider_histogram_settings)
{
    ...
    if (oprof) {
        const cmsUInt32Number flags =
            ...
            | (icm.outputBPC ? cmsFLAGS_BLACKPOINTCOMPENSATION
                             : 0);  // NOCACHE is important for thread safety

        lcmsMutex->lock();
        cmsHPROFILE LabIProf = cmsCreateLab4Profile(nullptr);
        cmsHTRANSFORM hTransform = cmsCreateTransform(
            LabIProf, TYPE_Lab_DBL, oprof, TYPE_RGB_FLT, icm.outputIntent, flags);
...
#pragma omp for firstprivate(lab) schedule(dynamic, 16)
        ...
        copyAndClamp(lab, image->data, xyz_rgb, multiThread,
                     pro);  // int pro = 1 to switch to Working Profile for histogram and
                            // navigator panel and Lockable color picker whitout gamma
    }
 ...
 *
 * Provide a pointer to a 7 floats array for "ga" (uninitialized ; this array will be
 * filled with the gamma values) if you want to use the custom gamma scenario. Those gamma
 * values will correspond to the ones of the chosen standard output profile (Prophoto if
 * non standard output profile given)
 *
 * If "ga" is NULL, then we're considering standard gamma with the chosen output profile.
 *
 * Generate an Image16
 *
 * If a custom gamma profile can be created, divide by 327.68, convert to xyz and apply
 * the custom gamma transform otherwise divide by 327.68, convert to xyz and apply the
 * sRGB transform, before converting with gamma2curve
 */
Imagefloat* ImProcFunctions::lab2rgbOut(LabImage* lab,
                                        int cx,
                                        int cy,
                                        int cw,
                                        int ch,
                                        const procparams::ColorManagementParams& icm)
{
    ...
    if (oprof) {
        ...
        cmsHTRANSFORM hTransform = cmsCreateTransform(
            iprof, TYPE_Lab_FLT, oprof, TYPE_RGB_FLT, icm.outputIntent, flags);
        lcmsMutex->unlock();
...
#pragma omp parallel for schedule(dynamic, 16) if (multiThread)
        ...
        for (int i = cy; i < cy + ch; i++) {
            ...
            for (int j = cx; j < cx + cw; j++) {
                ...
                float fy =
                    (Color::c1By116 * rL[j]) / 327.68f + Color::c16By116;  // (L+16)/116
                ...
                float y_ = (LL > (float)Color::epskap)
                               ? 65535.0f * fy * fy * fy
                               : 65535.0f * LL / (float)Color::kappa;
...
void ImProcFunctions::preserv(LabImage* nprevl, LabImage* provis, int cw, int ch)
{
    ...
    float neutral = 2000000000.f;  // if a2 + b2 < 200000000 scale 0..100 a and b about :
                                   // 140 > a & b > -140  decrease effect
...
void ImProcFunctions::gamutcompr(Imagefloat* src,
                                 Imagefloat* dst,
                                 float& mac,
                                 float& mac0,
                                 float& mac1,
                                 float& mac2) const
{
    ...
    dcip3[0][0] = 0.4861607;  // 0.4451698;(original) //0.4861607 with chromatic
                              // adaptation D63 => D50
    dcip3[0][1] = 0.3238514;  // 0.2771344;(original)//0.3238514 with chromatic adaptation
                              // D63 => D50
    dcip3[0][2] =
        0.1541879;  // 0.1722827;(original//0.1541879 with chromatic adaptation D63 => D50
    dcip3[1][0] =
        0.2266839;  // 0.2094917;(original//0.2266839 with chromatic adaptation D63 => D50
    dcip3[1][1] =
        0.7103336;  // 0.7215953;(original//0.7103336 with chromatic adaptation D63 => D50
    dcip3[1][2] = 0.0629826;  // 0.0689131;(original// 0.0629826 with chromatic adaptation
                              // D63 => D50
    dcip3[2][0] =
        -0.0008016;  // 0.0;(original//-0.0008016 with chromatic adaptation D63 => D50
    dcip3[2][1] = 0.0432353;  // 0.0470606;(original// 0.0432353 with chromatic adaptation
                              // D63 => D50
    dcip3[2][2] = 0.7824663;  // 0.9073554;(original// 0.7824663 with chromatic adaptation
                              // D63 => D50
...
#ifdef _OPENMP
#pragma omp parallel for reduction(max : maxac) reduction(max : maxac0) \
    reduction(max : maxac1) reduction(max : maxac2)                     \
    schedule(dynamic, 16) if (multiThread)
#endif
    ...
    for (int i = 0; i < height; ++i)
        for (int j = 0; j < width; ++j) {
            ...
            Color::aces_reference_gamut_compression(rgb_in, th, dl, to_out, from_out, pw,
                                                    roll, rout, gout, bout, ac, ac0, ac1,
                                                    ac2);
            if (ac > maxac) {
                maxac = ac;
            }
...
inline float norm3(float r, float g, float b, TMatrix ws)
{
    ...
    return std::min(hi,
                    power_norm2(r, g, b) / 2.f + Color::rgbLuminance(r, g, b, ws) / 2.f);
}

void ImProcFunctions::apsatur(int sp,
                              Imagefloat* tmpImage,
                              Imagefloat* tmpImage2,
                              int bfw,
                              int bfh,
                              float satu)
// act on saturation RGB after Abstract Profile - main and Selective Editing
{
    ...
    const auto sfcie = [=](float s, float c) -> float {
        if (c > noise) {
    ...
    const auto apply_satcie = [&](float& r, float& g, float& b, float f, float ll,
                                  float satu) -> void {
        float rl = r - ll;
        float gl = g - ll;
        float bl = b - ll;
        float s = intp(max(sfcie(rl, r), sfcie(gl, g), sfcie(bl, b)),
                       pow_F(f, 0.3f * satu) * (0.6f) + (0.4f), 1.f);
        r = ll + s * rl;
        g = ll + s * gl;
        b = ll + s * bl;
    };
...
void ImProcFunctions::workingtrc(int sp,
                                 Imagefloat* src,
                                 Imagefloat* dst,
                                 int cw,
                                 int ch,
...
#pragma omp parallel for
        ...
        // It is probably intellectually more relevant to place this algorithm at the end,
        // but it is complex at the GUI level (at least for me). The errors made are
        // relatively minimal and result seems good enough
        meanx = 0.f;
        meany = 0.f;
...
#pragma omp parallel for reduction(+ : meanx, meany) if (multiThread)
    ...
    const float toxyz[3][3] = {
        {
            static_cast<float>(
                wprof[0][0]
                / ((normalizeIn ? 65535.0 : 1.0))),  // I have suppressed / Color::D50x
        ...
        },
        {
            static_cast<float>(
                wprof[2][0]
                / ((normalizeIn ? 65535.0 : 1.0))),  // I have suppressed / Color::D50z
            static_cast<float>(wprof[2][1] / ((normalizeIn ? 65535.0 : 1.0))),
            static_cast<float>(wprof[2][2] / ((normalizeIn ? 65535.0 : 1.0))),
        },
    };

    if (profile == "sRGB" || profile == "Adobe RGB" || profile == "ProPhoto"
        || profile == "WideGamut" || profile == "BruceRGB" || profile == "Beta RGB"
        || profile == "BestRGB" || profile == "Rec2020" || profile == "ACESp0"
        || profile == "ACESp1" || profile == "JDCmax" || profile == "JDCmax stdA")
    {
        if (settings->verbose) {
            printf("Profile=%s\n", profile.c_str());
        }
    ...
    if (mul == -5 && gampos == 2.4 && slpos == 12.92310)
    {  // must be change if we change settings RT sRGB
        // only in this case we can shortcut..all process..no gamut control..because we
        // reduce...leads to very small differences, but big speedup
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic, 16) if (multiThread)
#endif
    ...
    {  // || (params->icm.wprim == ColorManagementParams::Primaries::DEFAULT &&
       // params->icm.will == ColorManagementParams::Illuminant::DEFAULT)) { //shortcut
       // and speedup when no call primaries and illuminant - no gamut control...in this
       // case be careful
        GammaValues g_a;  // gamma parameters
        double pwr = 1.0 / static_cast<double>(gampos);
        Color::calcGamma(pwr, slpos,
...
#pragma omp parallel for schedule(dynamic, 16) if (multiThread)
        ...
        for (int y = 0; y < ch; ++y) {
            ...
            for (; x < cw - 3; x += 4) {
                STVFU(dst->r(y, x), F2V(65536.f)
                                        * gammalog(LVFU(src->r(y, x)), F2V(gampos),
                                                   F2V(slpos), F2V(g_a[3]), F2V(g_a[4])));
                STVFU(dst->g(y, x), F2V(65536.f)
                                        * gammalog(LVFU(src->g(y, x)), F2V(gampos),
                                                   F2V(slpos), F2V(g_a[3]), F2V(g_a[4])));
                STVFU(dst->b(y, x), F2V(65536.f)
                                        * gammalog(LVFU(src->b(y, x)), F2V(gampos),
                                                   F2V(slpos), F2V(g_a[3]), F2V(g_a[4])));
            }
            ...
            for (; x < cw; ++x) {
                dst->r(y, x) =
                    65536.f * gammalog(src->r(y, x), gampos, slpos, g_a[3], g_a[4]);
                dst->g(y, x) =
                    65536.f * gammalog(src->g(y, x), gampos, slpos, g_a[3], g_a[4]);
                dst->b(y, x) =
                    65536.f * gammalog(src->b(y, x), gampos, slpos, g_a[3], g_a[4]);
            }
        }
    ...
    if (locprim == 1 && mul == 5) {
        ...
        if (params->locallab.spots.at(sp).illMethod == "d50") {
            illum = 2;
            xyD = { 0.3457, 0.3585,
                    1.0 };  // near LCMS values but not perfect... it's a compromise!!
            Wx = 0.964295676;
            Wz = 0.825104603;
    ...
    // fixed crash when there is no space or too small..just a line...Possible if bx, by
    // aligned with Gx,Gy Rx,Ry fix crash if user select 0 for redyy, bluyy, greyy
    if (redyy == 0.f) {
        redyy = epsil;
    }
    ...
    if ((bluyy < yc + 0.0004f) && (bluyy > yc - 0.0004f))
    {  // under 0.0004 in some case crash because space too small
        return;
    }
    ...
    if (transform) {
        ...
        GammaValues g_a;                 // gamma parameters
        Color::calcGamma(pwr, ts, g_a);  // call to calcGamma with selected gamma and
                                         // slope : return parameters for LCMS2
        ...
        if (rtengine::settings->verbose) {
            printf("ga0=%f ga1=%f ga2=%f ga3=%f ga4=%f\n", gammaParams[0], gammaParams[1],
                   gammaParams[2], gammaParams[3], gammaParams[4]);
        }
        ...
        GammaTRC[0] = GammaTRC[1] = GammaTRC[2] = cmsBuildParametricToneCurve(
            NULL, five, gammaParams);  // 5 = more smoother than 4
        ...
        if (rtengine::settings->verbose && mul == 5) {
            cmsCIEXYZ* redT =
                static_cast<cmsCIEXYZ*>(cmsReadTag(oprofdef, cmsSigRedMatrixColumnTag));
            cmsCIEXYZ* greenT =
                static_cast<cmsCIEXYZ*>(cmsReadTag(oprofdef, cmsSigGreenMatrixColumnTag));
            cmsCIEXYZ* blueT =
                static_cast<cmsCIEXYZ*>(cmsReadTag(oprofdef, cmsSigBlueMatrixColumnTag));
        ...
        if (oprofdef) {
            ...
            lcmsMutex->lock();
            hTransform = cmsCreateTransform(iprof, TYPE_RGB_FLT, oprofdef, TYPE_RGB_FLT,
                                            params->icm.aRendIntent, flags);
            lcmsMutex->unlock();
        }
    }
                ...
                if (locprim == 1) {
                    ...
                    for (int y = 0; y < ch ; ++y) {
                        for (int x = 0; x < cw ; ++x) {
                            ...
                            float xcb, ycb, zcb;
                            Color::rgbxyz(RR, GG, BB, xcb, ycb, zcb, wb2);//use sRGB Adobe
        Rec2020 ACESp0
