...
void RawImageSource::fast_demosaic()
{
    ...
    if (plistener) {
        plistener->setProgressStr(
            Glib::ustring::compose(M("TP_RAW_DMETHOD_PROGRESSBAR"), M("TP_RAW_FAST")));
...
#pragma omp single
        ...
        const double progressInc =
            16.0 * (1.0 - progress) / ((H * W) / ((TS - 4) * (TS - 4)));
            ...
            for (int left = bord - 2; left < W - bord + 2; left += TS - (4)) {
                ...
                for (int i = top, rr = 0; i < bottom; i++, rr++) {
                    ...
                    for (j = left, cc = 0; j < right - 3; j += 4, cc += 4) {
                        ...
                        wtuv = INVGRADV(
                            absv + vabsf(tempv - LVFU(rawData[i - 2][j]))
                            + vabsf(LVFU(rawData[i - 1][j]) - LVFU(rawData[i - 3][j])));
                        wtdv = INVGRADV(
                            absv + vabsf(tempv - LVFU(rawData[i + 2][j]))
                            + vabsf(LVFU(rawData[i + 1][j]) - LVFU(rawData[i + 3][j])));
                        abs2v = vabsf(LVFU(rawData[i][j - 1]) - LVFU(rawData[i][j + 1]));
                        wtlv = INVGRADV(
                            abs2v + vabsf(tempv - LVFU(rawData[i][j - 2]))
                            + vabsf(LVFU(rawData[i][j - 1]) - LVFU(rawData[i][j - 3])));
                        wtrv = INVGRADV(
                            abs2v + vabsf(tempv - LVFU(rawData[i][j + 2]))
                            + vabsf(LVFU(rawData[i][j + 1]) - LVFU(rawData[i][j + 3])));
                        greenv = (wtuv * LVFU(rawData[i - 1][j])
                                  + wtdv * LVFU(rawData[i + 1][j])
                                  + wtlv * LVFU(rawData[i][j - 1])
                                  + wtrv * LVFU(rawData[i][j + 1]))
                                 / (wtuv + wtdv + wtlv + wtrv);
                        _mm_store_ps(&greentile[rr * TS + cc],
                                     vself(selmask, greenv, tempv));
                    ...
                    for (; j < right; j++, cc++) {
                        ...
                        if (fc(cfarray, i, j) == 1) {
                            ...
                            wtu = INVGRAD((abs(rawData[i + 1][j] - rawData[i - 1][j])
                                           + abs(rawData[i][j] - rawData[i - 2][j])
                                           + abs(rawData[i - 1][j] - rawData[i - 3][j])));
                            wtd = INVGRAD((abs(rawData[i - 1][j] - rawData[i + 1][j])
                                           + abs(rawData[i][j] - rawData[i + 2][j])
                                           + abs(rawData[i + 1][j] - rawData[i + 3][j])));
                            wtl = INVGRAD((abs(rawData[i][j + 1] - rawData[i][j - 1])
                                           + abs(rawData[i][j] - rawData[i][j - 2])
                                           + abs(rawData[i][j - 1] - rawData[i][j - 3])));
                            wtr = INVGRAD((abs(rawData[i][j - 1] - rawData[i][j + 1])
                                           + abs(rawData[i][j] - rawData[i][j + 2])
                                           + abs(rawData[i][j + 1] - rawData[i][j + 3])));
                            ...
                            greentile[rr * TS + cc] =
                                (wtu * rawData[i - 1][j] + wtd * rawData[i + 1][j]
                                 + wtl * rawData[i][j - 1] + wtr * rawData[i][j + 1])
                                / (wtu + wtd + wtl + wtr);
                        }
                ...
                for (int i = top + 1, rr = 1; i < bottom - 1; i++, rr++) {
                    if (fc(cfarray, i, left + (fc(cfarray, i, 2) & 1) + 1) == 0)
                        ...
                        for (int j = left + 1, cc = 1; j < right - 1; j += 4, cc += 4) {
                            ...
                            _mm_storeu_ps(
                                &bluetile[rr * TS + cc],
                                LVFU(greentile[rr * TS + cc])
                                    - zd25v
                                          * ((LVFU(greentile[(rr - 1) * TS + (cc - 1)])
                                              + LVFU(greentile[(rr - 1) * TS + (cc + 1)])
                                              + LVFU(greentile[(rr + 1) * TS + cc + 1])
                                              + LVFU(greentile[(rr + 1) * TS + cc - 1]))
                                             - vminf(LVFU(rawData[i - 1][j - 1])
                                                         + LVFU(rawData[i - 1][j + 1])
                                                         + LVFU(rawData[i + 1][j + 1])
                                                         + LVFU(rawData[i + 1][j - 1]),
                                                     clip_ptv)));
                        }
                        ...
                        for (int cc = (fc(cfarray, i, 2) & 1) + 1, j = left + cc;
                             j < right - 1; j += 2, cc += 2)
                        {
                            // interpolate B/R colors at R/B sites
                            bluetile[rr * TS + cc] =
                ...
                for (int i = top + 2, rr = 2; i < bottom - 2; i++, rr++) {
                    ...
                    for (int cc = 2 + (fc(cfarray, i, 2) & 1), j = left + cc;
                         j < right - 2; j += 4, cc += 4)
                    {
                        // no need to take care about the borders of the tile. There's
                        // enough free space.
                        // interpolate R and B colors at G sites
                        greenv = LVFU(greentile[rr * TS + cc]);
                        greensumv = LVFU(greentile[(rr - 1) * TS + cc])
                                    + LVFU(greentile[(rr + 1) * TS + cc])
                                    + LVFU(greentile[rr * TS + cc - 1])
                                    + LVFU(greentile[rr * TS + cc + 1]);

                        temp1v = LVFU(redtile[rr * TS + cc]);
                        temp2v = greenv
                                 - zd25v
                                       * (greensumv - LVFU(redtile[(rr - 1) * TS + cc])
                                          - LVFU(redtile[(rr + 1) * TS + cc])
                                          - LVFU(redtile[rr * TS + cc - 1])
                                          - LVFU(redtile[rr * TS + cc + 1]));

                        _mm_storeu_ps(&redtile[rr * TS + cc],
                                      vself(selmask, temp1v, temp2v));

                        temp1v = LVFU(bluetile[rr * TS + cc]);

                        temp2v = greenv
                                 - zd25v
                                       * (greensumv - LVFU(bluetile[(rr - 1) * TS + cc])
                                          - LVFU(bluetile[(rr + 1) * TS + cc])
                                          - LVFU(bluetile[rr * TS + cc - 1])
                                          - LVFU(bluetile[rr * TS + cc + 1]));

                        _mm_storeu_ps(&bluetile[rr * TS + cc],
                                      vself(selmask, temp1v, temp2v));
                    }
                         ...
                         j < right - 2; j += 2, cc += 2)
                    {
                        ...
                        redtile[rr * TS + cc] = greentile[rr * TS + cc]
                                                - 0.25f
                                                      * ((greentile[(rr - 1) * TS + cc]
                                                          - redtile[(rr - 1) * TS + cc])
                                                         + (greentile[(rr + 1) * TS + cc]
                                                            - redtile[(rr + 1) * TS + cc])
                                                         + (greentile[rr * TS + cc - 1]
                                                            - redtile[rr * TS + cc - 1])
                                                         + (greentile[rr * TS + cc + 1]
                                                            - redtile[rr * TS + cc + 1]));
                        bluetile[rr * TS + cc] =
                ...
                for (int i = top + 2, rr = 2; i < bottom - 2; i++, rr++) {
                    ...
                    for (j = left + 2, cc = 2; j < right - 5; j += 4, cc += 4) {
                        _mm_storeu_ps(&red[i][j],
                                      vmaxf(LVFU(redtile[rr * TS + cc]), ZEROV));
                        _mm_storeu_ps(&green[i][j],
                                      vmaxf(LVFU(greentile[rr * TS + cc]), ZEROV));
                        _mm_storeu_ps(&blue[i][j],
                                      vmaxf(LVFU(bluetile[rr * TS + cc]), ZEROV));
                    }
