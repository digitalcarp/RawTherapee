...
// almost all data are taken for BG, YR, G excepted a few extreme values with a slight
// correction No LUTf for BG and Y : low corrections Only between 5B and 5PB for L > 40 :
// under very low corrections for L < 40
...
LUTf Color::_10PB10, Color::_10PB20, Color::_10PB30, Color::_10PB40, Color::_10PB50,
    Color::_10PB60;
LUTf Color::_9PB10, Color::_9PB20, Color::_9PB30, Color::_9PB40, Color::_9PB50,
    Color::_9PB60, Color::_9PB70, Color::_9PB80;
LUTf Color::_75PB10, Color::_75PB20, Color::_75PB30, Color::_75PB40, Color::_75PB50,
    Color::_75PB60, Color::_75PB70, Color::_75PB80;
LUTf Color::_6PB10, Color::_6PB20, Color::_6PB30, Color::_6PB40, Color::_6PB50,
    Color::_6PB60, Color::_6PB70, Color::_6PB80;
LUTf Color::_45PB10, Color::_45PB20, Color::_45PB30, Color::_45PB40, Color::_45PB50,
    Color::_45PB60, Color::_45PB70, Color::_45PB80;
LUTf Color::_3PB10, Color::_3PB20, Color::_3PB30, Color::_3PB40, Color::_3PB50,
    Color::_3PB60, Color::_3PB70, Color::_3PB80;
LUTf Color::_15PB10, Color::_15PB20, Color::_15PB30, Color::_15PB40, Color::_15PB50,
    Color::_15PB60, Color::_15PB70, Color::_15PB80;
...
LUTf Color::_10YR20, Color::_10YR30, Color::_10YR40, Color::_10YR50, Color::_10YR60,
    Color::_10YR70, Color::_10YR80, Color::_10YR90;
LUTf Color::_85YR20, Color::_85YR30, Color::_85YR40, Color::_85YR50, Color::_85YR60,
    Color::_85YR70, Color::_85YR80, Color::_85YR90;
LUTf Color::_7YR30, Color::_7YR40, Color::_7YR50, Color::_7YR60, Color::_7YR70,
    Color::_7YR80;
LUTf Color::_55YR30, Color::_55YR40, Color::_55YR50, Color::_55YR60, Color::_55YR70,
    Color::_55YR80, Color::_55YR90;
...
LUTf Color::_10GY30, Color::_10GY40, Color::_10GY50, Color::_10GY60, Color::_10GY70,
    Color::_10GY80;
LUTf Color::_75GY30, Color::_75GY40, Color::_75GY50, Color::_75GY60, Color::_75GY70,
    Color::_75GY80;
...
#pragma omp section
#endif
        {
            ...
            gamma2curve.share(
                gammatab_srgb,
                LUT_CLIP_BELOW | LUT_CLIP_ABOVE);  // shares the buffer with gammatab_srgb
                                                   // but has different clip flags
        }
#ifdef _OPENMP
#pragma omp section
#endif
        {
            ...
            //  gamma2curve.share(gammatab_srgb, LUT_CLIP_BELOW | LUT_CLIP_ABOVE); //
            //  shares the buffer with gammatab_srgb but has different clip flags
        }
#ifdef _OPENMP
#pragma omp section
#endif
...
#pragma omp section
#endif
        // modify arbitrary data for Lab..I have test : nothing, gamma 2.6 11 - gamma 4 5
        // - gamma 5.5 10 we can put other as gamma g=2.6 slope=11, etc. but noting to do
        // with real gamma !!!: it's only for data Lab # data RGB finally I opted for
        // gamma55 and with options we can change
...
{  // do not use this function in a loop. It really eats processing time caused by
   // Glib::ustring comparisons
    ...
    if (workingSpace) {  // display working profile
        ...
        if (profile == settings->srgb || profile == settings->adobe)
        {  // apply sRGB inverse gamma
            if (r > 0.04045f) {
                r = pow_F(((r + 0.055f) / 1.055f), rtengine::Color::sRGBGammaCurve);
            } else {
                r /= 12.92f;
    ...
    const TMatrix wprof =
        rtengine::ICCStore::getInstance()->workingSpaceMatrix(profileCalc);
    ...
    const float var_X =
        (xyz_rgb[0][0] * r + xyz_rgb[0][1] * g + xyz_rgb[0][2] * b) / Color::D50x;
    const float var_Y = (xyz_rgb[1][0] * r + xyz_rgb[1][1] * g + xyz_rgb[1][2] * b);
    const float var_Z =
        (xyz_rgb[2][0] * r + xyz_rgb[2][1] * g + xyz_rgb[2][2] * b) / Color::D50z;
...
void Color::xyz2srgb(float x, float y, float z, float& r, float& g, float& b)
{
    ...
    // Transform to output color.  Standard sRGB is D65, but internal representation is
    // D50 Note that it is only at this point that we should have need of clipping color
    // data
...
void Color::trcGammaBWRow(float* r,
                          float* g,
                          float* b,
                          int width,
                          float gammabwr,
 ...
 */
void Color::computeBWMixerConstants(const Glib::ustring& setting,
                                    ...
                                    double& bbm)
{
    ...
    if (!autoc) {
        // if     (setting=="RGB-Abs" || setting=="ROYGCBPM-Abs")  {} //Keep the RGB mixer
        // values as is! else if(setting=="RGB-Rel" || setting=="ROYGCBPM-Rel")  {} //Keep
        // the RGB mixer values as is!
    ...
    if (setting == "ROYGCBPM-Abs" || setting == "ROYGCBPM-Rel") {
        ...
        if (mixerOrange != 33.f) {
            ...
            const float obM =
                complement ? (-0.492f * mixerOrange + 16.236f) / 100.f : 0.f;
        ...
        if (mixerYellow != 33.f) {
            ...
            const float ybM =
                complement ? (-0.492f * mixerYellow + 16.236f) / 100.f : 0.f;
        ...
        if (mixerMagenta != 33.f) {
            ...
            const float mgM =
                complement ? (-0.492f * mixerMagenta + 16.236f) / 100.f : 0.f;
        ...
        if (mixerPurple != 33.f) {
            ...
            const float pgM =
                complement ? (-0.492f * mixerPurple + 16.236f) / 100.f : 0.f;
...
void Color::interpolateRGBColor(const float balance,
                                const float r1,
                                const float g1,
                                const float b1,
                                const float r2,
...
void Color::interpolateRGBColor(float realL,
                                float iplow,
                                float iphigh,
                                int algm,
                                const float balance,
                                ...
                                float& bo)
{
    ...
    if (twoc == 0) {  // 2 colours
        ...
        if (realL > 0.f && realL <= iphigh) {
            ...
            calH = realL * chromat;  //*(iphigh-1.f) - chromat*(iphigh-1.f);//it is better
                                     //without transition in highlight
        }
    }
...
void Color::calcGamma(double pwr, double ts, GammaValues& gamma)
{
    ...
    if (g[0]) {
        g[5] = 1.
                   / (g[1] * SQR(g[3]) / 2. - g[4] * (1. - g[3])
                      + (1. - pow(g[3], 1. + g[0])) * (1. + g[4]) / (1. + g[0]))
               - 1.;
    } else {
        g[5] = 1.
                   / (g[1] * SQR(g[3]) / 2. + 1. - g[2] - g[3]
                      - g[2] * g[3] * (log(g[3]) - 1.))
               - 1.;
    }
                       ...
                       float factor)
{
    ...
    for (int i = 0; i < 65536; ++i) {
        gammacurve[i] =
            gammaf(static_cast<float>(i) / divisor, gamma, start, slope) * factor;
    }

#endif
}

void Color::gammanf2lut(LUTf& gammacurve,
                        float gamma,
                        float divisor,
                        float factor)  // standard gamma without slope...
{
    ...
    // inside the loop we can use xlogfNoCheck and xexpfNoCheck because we know about the
    // input values
                    ...
                    int width)
{
    ...
    for (; i < width - 3; i += 4) {
        ...
        if (_mm_movemask_ps((vfloat)vorm(vmaskf_gt(vmaxf(xv, vmaxf(yv, zv)), maxvalfv),
                                         vmaskf_lt(vminf(xv, vminf(yv, zv)), minvalfv))))
        {
            // take slower code path for all 4 pixels if one of the values is > MAXVALF.
            // Still faster than non SSE2 version
            for (int k = 0; k < 4; ++k) {
                float x = xv[k];
                float y = yv[k];
                float z = zv[k];
...
void Color::RGB2L(const float* R,
                  const float* G,
                  const float* B,
                  float* L,
                  const float wp[3][3],
...
void Color::Lab2RGBLimit(float* L,
                         float* a,
                         float* b,
                         float* R,
                         float* G,
...
void Color::agx_trans(const std::array<float, 3>& rgb_in,
                      const Matrix& to_agx,
                      float& R,
                      float& G,
                      float& B)
...
void Color::aces_reference_gamut_compression(
    ...
    float& ac0,
    float& ac1,
    float& ac2)  // ac, ac0, ac1, ac2 achromatic values for Red, Green, Blue
{
    std::array<float, 3> rgb{ rgb_in[0], rgb_in[1], rgb_in[2] };
    ...
    if (!rolloff) {
        ...
        for (unsigned i = 0; i < cd.size(); ++i) {
            if (d[i] >= threshold[i]) {
                ...
                cd[i] =
                    s[i] * (sqrt(d[i] - threshold[i] + c2_4) - fabs(c_2)) + threshold[i];
            }
        }
    } else {
        for (unsigned i = 0; i < cd.size(); ++i) {
            if (d[i] >= threshold[i]) {
                if (threshold[i] == 1.f) {
                    ...
                    const float scale =
                        (limit - thres)
                        / pow(pow((1.0f - thres) / (limit - thres), -pwr) - 1.0f,
                              1.0f / pwr);
...
void Color::primaries_to_xyz(double p[6], double Wx, double Wz, double* pxyz, int cat)
{
    ...
    double Sr =
        Wx * inv_input_prim[0][0] + Wy * inv_input_prim[1][0] + Wz * inv_input_prim[2][0];
    double Sg =
        Wx * inv_input_prim[0][1] + Wy * inv_input_prim[1][1] + Wz * inv_input_prim[2][1];
    double Sb =
        Wx * inv_input_prim[0][2] + Wy * inv_input_prim[1][2] + Wz * inv_input_prim[2][2];
...
void Color::gamutmap(float& X, float Y, float& Z, const double p[3][3])
{
    ...
    for (int c = 0; c < 3; c++)
        for (int m = 0; m < 2; m++) {
            ...
            lam[c][m] =
                (-(p[0][c1] * p[1][c]
                   * ((-12 + 3 * u0 + 20 * v0) * Y + 4 * m * 65535 * v0 * p[2][c2]))
                 + p[0][c] * p[1][c1]
                       * ((-12 + 3 * u0 + 20 * v0) * Y + 4 * m * 65535 * v0 * p[2][c2])
...
void Color::skinredfloat(float J,
                         float h,
                         float sres,
                         float Sp,
                         float dred,
                         ...
                         float& s)
{
    ...
    // rough correspondence between h (JC) and H (lab) that has relatively little
    // importance because transitions that blur the correspondence is not linear
    if (h > 8.6f && h <= 74.f) {
    ...
    if (doskin) {
        ...
        factor = chromapro;
        Color::transitred(HH, s, dred, factorskin, protect_red, factorskinext, deltaHH,
                          chromapro, factor);  // transition
        s *= factor;
    } else {
        s = ko * sres;
    }
                     ...
                     float& scaleext)
{
    if (rstprotection < 99.9999f) {
        ...
        // scaleext=HH*(1.0f-scale)/deltaHH + 1.0f - (1.3f+deltaHH)*(1.0f-scale)/deltaHH;
        // //transition for Hue (red - yellow) optimized formula
        {
            scaleext = (HH * (1.0f - scale) + deltaHH - (1.3f + deltaHH) * (1.0f - scale))
                       / deltaHH;  // transition for Hue (red - yellow)
        } else if ((HH < 0.15f && HH > (0.15f - deltaHH)))
        // scaleext=HH*(scale-1.0f)/deltaHH + 1.0f - (0.15f-deltaHH)*(scale-1.0f)/deltaHH;
        // //transition for hue (red purple) optimized formula
        {
            scaleext =
                (HH * (scale - 1.0f) + deltaHH - (0.15f - deltaHH) * (scale - 1.0f))
                / deltaHH;  // transition for hue (red purple)
        }
    }
}

void Color::transitred(const float HH,
                       float const Chprov1,
                       const float dred,
                       const float factorskin,
                       const float protect_red,
                       const float factorskinext,
                       const float deltaHH,
                       const float factorsat,
                       float& factor)
{
    if (HH >= 0.15f && HH < 1.3f) {
        if (Chprov1 < dred) {
 ...
 * correction ; false : only chroma float Lprov1 , Loldd : luminance after and before
 *    float HH: hue before
 ...
 */
void Color::AllMunsellLch(bool lumaMuns,
                          float Lprov1,
                          float Loldd,
                          float HH,
                          float Chprov1,
                          ...
                          float& correctlum)
{
    ...
    if (CC >= 6.f && CC < 140.f)
    {  // if C > 140 we say C=140 (only in Prophoto ...with very large saturation)
        constexpr float huelimit[8] = {
            -2.48f, -0.55f, 0.44f, 1.52f, 1.87f, 3.09f, -0.27f, 0.44f
        };  // limits hue of blue-purple, red-yellow, green-yellow, red-purple
        ...
        for (int zo = 1; zo <= 4; ++zo) {
            if (HH > huelimit[2 * zo - 2] && HH < huelimit[2 * zo - 1]) {
                ...
                MunsellLch(Lprov1, HH, Chprov1, CC, correctionHue, zo, correctionHueLum,
                ...
                if (lumaMuns) {
                    ...
                    if (std::fabs(Lprov1 - Loldd) > 6.f) {
                        ...
                        MunsellLch(Loldd, HH, Chprov1, Chprov1, correctionHue, zo,
                                   correctionHueLum, correctL);
                        if (correctL) {
                            if (contin1) {
 ...
 *    float coorectionHuechroma : correction Hue for chromaticity (saturation)
 */
void Color::AllMunsellLch(float Lprov1,
                          float HH,
                          float Chprov1,
                          float CC,
                          float& correctionHuechroma)
 ...
 * Copyright (c)2012  Jacques Desmis <jdesmis@gmail.com> and Jean-Christophe Frisch
 * <natureh@free.fr>
 *
 ...
 * float coef : a float number between [0.95 ; 1.0[... the nearest it is from 1.0, the
 * more precise it will be... and the longer too as more iteration will be necessary) bool
 * neg and moreRGB : only in DEBUG mode to calculate iterations for negatives values and >
 * 65535
 */
void Color::gamutLchonly(float HH,
                         float& Lprov1,
                         float& Chprov1,
                         float& R,
                         float& G,
                         ...
                         const bool isHLEnabled,
                         const float lowerCoef,
                         const float higherCoef)
{
    ...
    do {
        ...
        // conversion Lab RGB to limit Lab values - this conversion is useful before
        // Munsell correction
        ...
        if (R < 0.0f || G < 0.0f || B < 0.0f) {
            ...
            // gamut for L with ultra blue : we can improve the algorithm ... thinner, and
            // other color ???
                   ...
                   && rtengine::min(R, G, B) <= ClipLevel)
        {
            ...
            // if "highlight reconstruction" is enabled or the point is completely white
            // (clipped, no color), don't control Gamut
            if (Lprov1 > 99.999f) {
                Lprov1 = 99.98f;
            }
                         ...
                         const float higherCoef)
{
    ...
    do {
        ...
        float z_ = 65535.0f * f2xyz(fz) * D50z;
        float y_ =
            (Lprov1 > epskap) ? 65535.0f * fy * fy * fy : 65535.0f * Lprov1 / kappa;
 ...
 * bool isHLEnabled : if "highlight reconstruction " is enabled
 * float coef : a float number between [0.95 ; 1.0[... the nearest it is from 1.0, the
 * more precise it will be... and the longer too as more iteration will be necessary)
 */
void Color::gamutLchonly(float HH,
                         float2 sincosval,
                         float& Lprov1,
...
void Color::gamutLchonly(float2 sincosval,
                         float& Lprov1,
                         float& Chprov1,
                         const float wip[3][3],
                         const bool isHLEnabled,
 ...
 * Copyright (c) 2012  Jacques Desmis <jdesmis@gmail.com>
 *
 * This function is the overall Munsell's corrections, but only on global statement: I
 * think it's better to use local statement with AllMunsellLch not for use in a "for" or
 * "do while" loop they are named accordingly :  gamutLchonly and AllMunsellLch it can be
 * used before and after treatment (saturation, gamma, luminance, ...)
 *
 * Parameters:
 ...
 *                            true:  apply luma + chroma Munsell correction if delta L >
 * 10; false: only chroma correction only bool gamut            : performs gamutLch const
 * double wip[3][3]: matrix for working profile bool multiThread      : parallelize the
 * loop
 */
void Color::LabGamutMunsell(float* labL,
                            float* laba,
                            float* labb,
                            const int N,
                            bool corMunsell,
                            ...
                            const double wip[3][3])
{
    ...
    for (int j = 0; j < N; j++) {
        ...
        if (gamut) {
            // According to mathematical laws we can get the sin and cos of HH by simple
            // operations
            float R, G, B;
            ...
            // gamut control : Lab values are in gamut
            gamutLchonly(HH, sincosval, Lprov1, Chprov1, R, G, B, wip, isHLEnabled, 0.15f,
                         0.96f);
        }
 ...
 * pay attention to white balance, and do not change hue and saturation, upstream of the
 * modification
 *
 */
void Color::SkinSat(float lum, float hue, float chrom, float& satreduc)
{
    ...
    constexpr float reduction =
        0.3f;  // use "reduction" for  "real" skin color : take into account a slightly
               // usage of contrast and saturation in RT if option "skin" = 1
    constexpr float extendedreduction =
        0.4f;  // use "extendedreduction" for wide area skin color, useful if not accurate
               // colorimetry or if the user has changed hue and saturation
    constexpr float extendedreduction2 =
        0.6f;  // use "extendedreduction2" for wide area for transition

    constexpr float C9 = 8.f, C8 = 15.f, C7 = 12.f, C4 = 7.f, C3 = 5.f, C2 = 5.f,
                    C1 = 5.f;
    constexpr float H9 = 0.05f, H8 = 0.25f, H7 = 0.1f, H4 = 0.02f, H3 = 0.02f, H2 = 0.1f,
                    H1 = 0.1f, H10 = -0.2f,
 ...
 * http://www.cis.rit.edu/research/mcsl2/online/munsell.php each LUT give Hue in function
 * of C, for each color Munsell and Luminance eg: _6PB20 : color Munsell 6PB for L=20 c=5
 * c=45 c=85 c=125..139 when possible: interpolation between values no value for C<5
 * (gray) low memory footprint -- maximum: 195 LUTf * 140 values errors due to small
 * number of samples in LUT and linearization are very low (1 to 2%) errors due to a
 * different illuminant "Daylight" than "C" are low, about 10%. For example, a theoretical
 * correction of 0.1 radian will be made with a real correction of 0.09 or 0.11 depending
 * on the color illuminant D50 errors due to the use of a very different illuminant "C",
 * for example illuminant "A" (tungsten) are higher, about 20%. Theoretical correction of
 * 0.52 radians will be made with a real correction of 0.42
 */
void Color::initMunsell()
{
    ...
    _75PB10(maxInd);  // limits hue -1.23  -0.71  _75PBx   x=Luminance  eg_75PB10 for L >5
                      // and L<=15
    _75PB10.clear();
