////////////////////////////////////////////////////////////////
//
//  Bilinear bayer demosaic, optimized for speed, intended use is for flat regions of
//  dual-demosaic
//
...
#pragma GCC ivdep
#endif
        for (int j = 2 - (FC(i, 1) & 1); j < W - 2; j += 2)
        {  // always begin with a green pixel
            green[i][j] = intp(blend[i][j], green[i][j], rawData[i][j]);
            nonGreen1[i][j] = intp(blend[i][j], nonGreen1[i][j],
                                   (rawData[i][j - 1] + rawData[i][j + 1]) * 0.5f);
            nonGreen2[i][j] = intp(blend[i][j], nonGreen2[i][j],
                                   (rawData[i - 1][j] + rawData[i + 1][j]) * 0.5f);
            green[i][j + 1] = intp(blend[i][j + 1], green[i][j + 1],
                                   ((rawData[i - 1][j + 1] + rawData[i][j])
                                    + (rawData[i][j + 2] + rawData[i + 1][j + 1]))
                                       * 0.25f);
            nonGreen1[i][j + 1] =
                intp(blend[i][j + 1], nonGreen1[i][j + 1], rawData[i][j + 1]);
            nonGreen2[i][j + 1] = intp(blend[i][j + 1], nonGreen2[i][j + 1],
