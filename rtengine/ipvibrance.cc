...
/*
 * Vibrance correction
 * copyright (c)2011  Jacques Desmis <jdesmis@gmail.com> and Jean-Christophe Frisch
 * <natureh@free.fr>
 *
 */
                               ...
                               const Glib::ustring& workingProfile)
{
    ...
    DiagonalCurve* dcurve =
        new DiagonalCurve(vibranceParams.skintonescurve, CURVES_MIN_POLY_POINTS);
    ...
    const float limitpastelsatur =
        (static_cast<float>(vibranceParams.psthreshold.getTopLeft()) / 100.f)
            * (1.f - p00)
        + p00;
    ...
    const float transitionweighting =
        static_cast<float>(vibranceParams.psthreshold.getBottomLeft()) / 100.f;
    float chromamean = 0.f;

    if (chromaPastel != chromaSatur) {
        // if sliders pastels and saturated are different: transition with a double linear
        // interpolation: between p2 and limitpastelsatur, and between limitpastelsatur
        // and s0 modify the "mean" point in function of double threshold  => differential
        // transition
...
#pragma omp for schedule(dynamic, 16)
        ...
        for (int i = 0; i < height; i++) {
            ...
            for (int j = 0; j < width; j++) {
                ...
                float saturation;
                Color::gamutLchonly(HH, sincosval, Lprov, Chprov, saturation, wip,
                                    highlight, 0.15f, 0.98f);
                ...
                if (Chprov > 6.f) {
                    ...
                    if (saturation > 0.f) {
                        if (satredu != 1.f) {
                            ...
                            sathue[0] = sathue[1] = sathue[2] = sathue[3] = sathue[4] =
                                1.f;
                            sathue2[0] = sathue2[1] = sathue2[2] = sathue2[3] = 1.f;
                        } else {
                            // double pyramid: LL and HH
                            // I try to take into account: Munsell response (human vision)
                            // and Gamut..(less response for red): preferably using
                            // Prophoto or WideGamut blue: -1.80 -3.14  green = 2.1 3.14
                            // green-yellow=1.4 2.1  red:0 1.4  blue-purple:-0.7  -1.4
                            // purple: 0 -0.7 these values allow a better and differential
                            // response
                            if (LL < 20.0f)
                            {  // more for blue-purple, blue and red modulate
                                sathue[4] = 0.4f;
                                sathue2[3] = 1.f;
                                ...
                                //          {sathue[0]=1.1f;sathue[1]=1.1f;sathue[2]=1.1f;sathue[3]=1.0f
                                //          ;sathue[4]=0.4f;sathue2[0]=1.0f
                                //          ;sathue2[1]=1.0f ;sathue2[2]=1.0f
                                //          ;sathue2[3]=1.0f;}//red   0.8 0.7
                                else if (/*HH>= 0.0f    &&*/ HH <= 1.4f)
                                {
                            ...
                            {  // more for blue and green, less for red and green-yellow
                                sathue[4] = 0.4f;
                                if (/*HH> -3.1415f &&*/ HH < -1.5f) {
                                    sathue[0] = 1.5f;  // blue
                                    sathue[1] = 1.4f;
                                ...
                                //          else if(  HH>= 0.0f    &&   HH<= 1.4f   )
                                //          {sathue[0]=0.8f;sathue[1]=0.8f;sathue[2]=0.8f;sathue[3]=0.8f
                                //          ;sathue[4]=0.4f;sathue2[0]=0.8f
                                //          ;sathue2[1]=0.8f ;sathue2[2]=0.8f
                                //          ;sathue2[3]=0.8f;}//red   0.8 0.7
                                else if (/*HH>= 0.0f    &&*/ HH <= 1.4f)
                                {
                            ...
                            {  // more for green, less for red and green-yellow
                                sathue[4] = 0.3f;
                                if (/*HH> -3.1415f &&*/ HH < -1.5f) {
                                    sathue[0] = 1.3f;  // blue
                                    sathue[1] = 1.2f;
                                ...
                                //          else if(  HH>= 0.0f    &&   HH<= 1.4f   )
                                //          {sathue[0]=0.8f;sathue[1]=0.8f;sathue[2]=0.8f
                                //          ;sathue[3]=0.8f
                                //          ;sathue[4]=0.3f;sathue2[0]=0.8f
                                //          ;sathue2[1]=0.8f ;sathue2[2]=0.8f
                                //          ;sathue2[3]=0.8f;}//red   0.8 0.7
                                else if (/*HH>= 0.0f    &&*/ HH <= 1.4f)
                                {
                                    ...
                                    sathue[0] = 1.6f;  // green - even with Prophoto green
                                                       // are too "little"  1.5 1.3
                                    sathue[1] = 1.4f;
                                    sathue[2] = 1.3f;
                                    sathue[3] = 1.25f;
                                    sathue2[0] = 1.25f;
                            ...
                            } else { /*if (LL>=80.0f)*/  // more for green-yellow, less
                                                         // for red and purple
                                sathue[4] = 0.2f;
                                if (/*HH> -3.1415f &&*/ HH < -1.5f) {
                                    sathue[0] = 1.0f;  // blue
                                ...
                                //          else if(  HH>= 0.0f    &&   HH<= 1.4f   )
                                //          {sathue[0]=0.8f;sathue[1]=0.8f;sathue[2]=0.8f;sathue[3]=0.8f;sathue[4]=0.2f;sathue2[0]=0.8f;sathue2[1]=0.8f
                                //          ;sathue2[2]=0.8f ;sathue2[3]=0.8f;}//red   0.8
                                //          0.7
                                else if (/*HH>= 0.0f    &&*/ HH <= 1.4f)
                                {
                                    sathue[0] = 1.1f;  // red   0.8 0.7
                        ...
                        if (chromaPastel != chromaSatur) {
                            ...
                            if (saturation > p2 && saturation < limitpastelsatur) {
                                float newchromaPastel =
                                    chromaPastel_a * saturation + chromaPastel_b;
                                chmod = newchromaPastel * satredu * sathue[3];
                            }

                            // Saturated
                            if (saturation < s0 && saturation >= limitpastelsatur) {
                                float newchromaSatur =
                                    chromaSatur_a * saturation + chromaSatur_b;
                ...
                if (skinCurveIsSet) {
                    ...
                    // transition hue
                    else if (HH > (skbeg - dhue) && HH <= skbeg
                             && Chprov < (60.f + dchr * 0.5f))
                    {
                        ...
                        HH = adh * HH + bdh;
                        hhModified = true;
                    } else if (HH >= skend && HH < (skend + dhue)
                               && Chprov < (60.f + dchr * 0.5f))
                    {
                ...
                const float yy_ = (Lprov > static_cast<float>(Color::epskap))
                                      ? fyy * fyy * fyy
                                      : Lprov / Color::kappaf;
                ...
                do {
                    ...
                    // if "highlight reconstruction" enabled don't control Gamut for
                    // highlights
