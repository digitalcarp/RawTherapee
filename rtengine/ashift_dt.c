...
#define ROTATION_RANGE_SOFT \
    20  // allowed min/max range for rotation parameter with manual adjustment
#define LENSSHIFT_RANGE 0.5  // allowed min/max default range for lensshift parameters
#define LENSSHIFT_RANGE_SOFT \
    1  // allowed min/max range for lensshift parameters with manual adjustment
#define SHEAR_RANGE 0.2  // allowed min/max range for shear parameter
#define SHEAR_RANGE_SOFT \
    0.5  // allowed min/max range for shear parameter with manual adjustment
#define CAMERA_ANGLE_RANGE_SOFT 80
#define MIN_LINE_LENGTH \
    5  // the minimum length of a line in pixels to be regarded as relevant
#define MAX_TANGENTIAL_DEVIATION \
    30  // by how many degrees a line may deviate from the +/-180 and +/-90 to be regarded
        // as relevant
...
#define RANSAC_ELIMINATION_RATIO \
    60  // percentage of lines we try to eliminate as outliers
...
#define RANSAC_HURDLE \
    5  // hurdle rate: the number of lines below which we do a complete permutation
...
#define NMS_CROP_ITERATIONS \
    100                // number of iterations for Nelder-Mead simplex on crop fitting
...
enum {
    ...
    ASHIFT_LINE_DIRVERT =
        1 << 1,  // the line is (mostly) vertical, else (mostly) horizontal
...
enum {
    ...
    ASHIFT_FIT_VERTICALLY =
        ASHIFT_FIT_ROTATION | ASHIFT_FIT_LENS_VERT | ASHIFT_FIT_LINES_VERT,
    ASHIFT_FIT_HORIZONTALLY =
        ASHIFT_FIT_ROTATION | ASHIFT_FIT_LENS_HOR | ASHIFT_FIT_LINES_HOR,
    ...
    ASHIFT_FIT_BOTH_NO_ROTATION = ASHIFT_FIT_LENS_VERT | ASHIFT_FIT_LENS_HOR
                                  | ASHIFT_FIT_LINES_VERT | ASHIFT_FIT_LINES_HOR,
    ASHIFT_FIT_BOTH_SHEAR = ASHIFT_FIT_ROTATION | ASHIFT_FIT_LENS_VERT
                            | ASHIFT_FIT_LENS_HOR | ASHIFT_FIT_SHEAR
                            | ASHIFT_FIT_LINES_VERT | ASHIFT_FIT_LINES_HOR,
    ...
    ASHIFT_FIT_FLIP = ASHIFT_FIT_LENS_VERT | ASHIFT_FIT_LENS_HOR | ASHIFT_FIT_LINES_VERT
...
/*
static void homography(float *homograph, const float angle, const float shift_v, const
float shift_h, const float shear, const float f_length_kb, const float orthocorr, const
float aspect, const int width, const int height, dt_iop_ashift_homodir_t dir)
*/
                       ...
                       dt_iop_ashift_homodir_t dir)
{
    ...
    */
    ...
    const rtengine::homogeneous::Vector<float> camera_center_yaw_pitch =
        rtengine::homogeneous::rotationMatrix<float>(pitch,
                                                     rtengine::homogeneous::Axis::X)
        * rtengine::homogeneous::rotationMatrix<float>(yaw,
                                                       rtengine::homogeneous::Axis::Y)
        * center;

    const rtengine::homogeneous::Matrix<float> matrix =
        // Perspective correction.
        rtengine::homogeneous::projectionMatrix<float>(camera_center_yaw_pitch[2],
                                                       rtengine::homogeneous::Axis::Z)
        * rtengine::homogeneous::rotationMatrix<float>(yaw,
                                                       rtengine::homogeneous::Axis::Y)
        * rtengine::homogeneous::rotationMatrix<float>(pitch,
                                                       rtengine::homogeneous::Axis::X)
        ...
        *
        ...
        rtengine::homogeneous::translationMatrix<float>((0.01f * shift_h - 0.5f) * u,
                                                        (-0.01f * shift_v - 0.5f) * v, f);
...
static inline int isneutral(dt_iop_ashift_data_t* data)
{
    ...
    return (fabs(data->rotation) < eps && fabs(data->lensshift_v) < eps
            && fabs(data->lensshift_h) < eps && fabs(data->shear) < eps
            && fabs(data->aspect - 1.0f) < eps && data->cl < eps && 1.0f - data->cr < eps
            && data->ct < eps && 1.0f - data->cb < eps);
}
...
// simple conversion of rgb image into greyscale variant suitable for line segment
// detection the lsd routines expect input as *double, roughly in the range [0.0; 256.0]
                            ...
                            dt_iop_ashift_enhance_t dir)
{
    ...
    // select kernel
    const double* kernel = (dir == ASHIFT_ENHANCE_HORIZONTAL) ? (const double*)hkernel
...
// detail enhancement via bilateral grid (function arguments in and out may represent
// identical buffers)
...
// apply gamma correction to RGB buffer (function arguments in and out may represent
// identical buffers)
                       ...
                       const int is_raw)
{
    ...
    if (lines_count > 0) {
        ...
        ashift_lines = (dt_iop_ashift_line_t*)malloc((size_t)lines_count
                                                     * sizeof(dt_iop_ashift_line_t));
        ...
        for (int n = 0; n < lines_count; n++) {
            ...
            // calculate homogeneous coordinates of connecting line (defined by the two
            // points)
    ...
    printf("%d lines (vertical %d, horizontal %d, not relevant %d)\n", lines_count,
           vertical_count, horizontal_count, lct - vertical_count - horizontal_count);
...
static int get_structure(dt_iop_module_t* module, dt_iop_ashift_enhance_t enhance)
{
    ...
    if (!line_detect(buffer, width, height, x_off, y_off, scale, &lines, &lines_count,
                     &vertical_count, &horizontal_count, &vertical_weight,
                     &horizontal_weight, enhance,
                     module->is_raw))  // dt_image_is_raw(&module->dev->image_storage)))
        goto error;
...
// by a number of dry runs first. The target average percentage value of lines to
// eliminate as outliers (without judging on the quality of the model) is given by
// RANSAC_ELIMINATION_RATIO, note: the actual percentage of outliers removed in the final
// run will be lower because we will finally look for the best quality model with the
// optimized epsilon and that quality value also encloses the number of good lines
                   ...
                   const int ymax)
{
    ...
    for (int r = 0; r < optiruns + riter; r++) {
        ...
        if (vec3isnull(V)
            || (fabs(V[2]) > 0.0f && V[0] / V[2] >= xmin && V[1] / V[2] >= ymin
                && V[0] / V[2] <= xmax && V[1] / V[2] <= ymax))
        {
            ...
            for (int n = 2; n < set_count; n++) {
                ...
                // of the "distance" between point and line. Note that this is not the
                // real euclidean distance but - with the given normalization - just a
                // pragmatically selected number that goes to zero if V lies on L and
                // increases the more V and L are apart
                ...
                if (inout[n] == 1) {
                    // a quality parameter that depends 1/3 on the number of lines within
                    // the model, 1/3 on their weight, and 1/3 on their weighted distance
                    // d to the vantage point
        ...
        if (r < optiruns) {
                ...
                && (valid_runs > 0))
            {
                ...
                float ratio =
                    100.0f * (float)lines_eliminated / ((float)set_count * valid_runs);
...
static int remove_outliers(dt_iop_module_t* module)
{
    ...
    if (vnb > 2)
        ransac(g->lines, lines_set, inout_set, vnb, g->vertical_weight, xmin, xmax, ymin,
               ymax);
    ...
    // it only makes sense to call ransac if we have more than two lines
    if (hnb > 2)
        ransac(g->lines, lines_set, inout_set, hnb, g->horizontal_weight, xmin, xmax,
               ymin, ymax);
...
static double model_fitness(double* params, void* data)
{
    ...
    */
    ...
    float homograph[3][3];
    homography((float*)homograph, rotation, lensshift_v, lensshift_h, shear, camera_pitch,
               camera_yaw, f_length_kb, orthocorr, aspect, width, height,
               ASHIFT_HOMOGRAPH_FORWARD);
    ...
    for (int n = 0; n < lines_count; n++) {
        ...
        // y^2 = 1 / (1 + m^2) and x^2 = m^2 / (1 + m^2) with m defining the slope of the
        // line
        vec3lnorm(L, L);

        // get scalar product of line L with orthogonal axis A -> gives 0 if line is
        // perpendicular
...
#ifdef ASHIFT_DEBUG
    /*
    printf("fitness with rotation %f, lensshift_v %f, lensshift_h %f, shear %f -> lines
    %d, quality %10f\n", rotation, lensshift_v, lensshift_h, shear, count, sum);
    */
    printf(
        "fitness with rotation %f, camera_pitch %f, camera_yaw %f -> lines %d, quality %10f\n",
                                        ...
                                        int min_line_count)
{
    ...
    fit.f_length_kb = (p->mode == ASHIFT_MODE_GENERIC) ? (float)DEFAULT_F_LENGTH
    ...
    if(mdir & ASHIFT_FIT_LENS_VERT)
    {
      ...
      fit.params_count++;
      params[pcount] = logit(fit.lensshift_v, -fit.lensshift_v_range,
    fit.lensshift_v_range); pcount++; fit.lensshift_v = NAN;
    }
    ...
    if(mdir & ASHIFT_FIT_LENS_HOR)
    {
      ...
      fit.params_count++;
      params[pcount] = logit(fit.lensshift_h, -fit.lensshift_h_range,
    fit.lensshift_h_range); pcount++; fit.lensshift_h = NAN;
    }
    ...
    */
    ...
    int iter = simplex(model_fitness, params, fit.params_count, NMS_EPSILON, NMS_SCALE,
    ...
    fit.rotation = isnan(fit.rotation)
                       ...
                       : fit.rotation;
    /*
    fit.lensshift_v = isnan(fit.lensshift_v) ? ilogit(params[pcount++],
    -fit.lensshift_v_range, fit.lensshift_v_range) : fit.lensshift_v; fit.lensshift_h =
    isnan(fit.lensshift_h) ? ilogit(params[pcount++], -fit.lensshift_h_range,
    fit.lensshift_h_range) : fit.lensshift_h; fit.shear = isnan(fit.shear) ?
    ilogit(params[pcount++], -fit.shear_range, fit.shear_range) : fit.shear;
    */
    fit.camera_pitch =
        isnan(fit.camera_pitch)
            ? ilogit(params[pcount++], -fit.camera_pitch_range, fit.camera_pitch_range)
            : fit.camera_pitch;
    fit.camera_yaw =
        isnan(fit.camera_yaw)
            ? ilogit(params[pcount++], -fit.camera_yaw_range, fit.camera_yaw_range)
            : fit.camera_yaw;
#ifdef ASHIFT_DEBUG
    /*
    printf("params after optimization (%d iterations): rotation %f, lensshift_v %f,
    lensshift_h %f, shear %f\n", iter, fit.rotation, fit.lensshift_v, fit.lensshift_h,
    fit.shear);
    */
    printf(
        "params after optimization (%d iterations): rotation %f, camera_pitch %f, camera_yaw %f\n",
        iter, fit.rotation, fit.camera_pitch, fit.camera_yaw);
#endif

    /*
    // sanity check: in case of extreme values the image gets distorted so strongly that
  it spans an insanely huge area. we check that
    // case and assume values that increase the image area by more than a factor of 4 as
  being insane. float homograph[3][3]; homography((float *)homograph, fit.rotation,
  fit.lensshift_v, fit.lensshift_h, fit.shear, fit.f_length_kb, fit.orthocorr, fit.aspect,
  fit.width, fit.height, ASHIFT_HOMOGRAPH_FORWARD);
    ...
    {
  #ifdef ASHIFT_DEBUG
      printf("optimization not successful: degenerate case with area growth factor (%f)
  exceeding limits\n", (xM - xm) * (yM - ym) / (fit.width * fit.height)); #endif return
  NMS_INSANE;
    }
    */
...
static int do_get_structure(dt_iop_module_t* module,
                            dt_iop_ashift_params_t* p,
                            dt_iop_ashift_enhance_t enhance)
{
    dt_iop_ashift_gui_data_t* g = (dt_iop_ashift_gui_data_t*)module->gui_data;
    ...
    if (!get_structure(module, enhance)) {
        ...
        // find out more
        printf(
            "do_get_structure: buf %p, buf_hash %lu, buf_width %d, buf_height %d, lines %p, lines_count %d\n",
    ...
    if (!remove_outliers(module)) {
        ...
        // find out more
        printf(
            "remove_outliers: buf %p, buf_hash %lu, buf_width %d, buf_height %d, lines %p, lines_count %d\n",
            g->buf, g->buf_hash, g->buf_width, g->buf_height, g->lines, g->lines_count);
#endif
        goto error;
    }
...
static int do_fit(dt_iop_module_t* module,
                  dt_iop_ashift_params_t* p,
                  dt_iop_ashift_fitaxis_t dir,
                  int min_line_count = MINIMUM_FITLINES)
{
    dt_iop_ashift_gui_data_t* g = (dt_iop_ashift_gui_data_t*)module->gui_data;
...
static void update_lines_count(const dt_iop_ashift_line_t* lines,
