...
void cfaboxblur(const float* const* riFlatFile,
                float* cfablur,
                int boxH,
                int boxW,
                int H,
                int W)
{
    if (boxW < 0 || boxH < 0 || (boxW == 0 && boxH == 0))
    ...
    // if boxW == 0 we can skip the horizontal blur and process the vertical blur from
    // riFlatFile to cfablur without using a temporary buffer
...
#pragma omp for
            ...
            for (int row = 0; row < H; ++row) {
                ...
                for (int col = 2; col <= boxW; col += 2) {
                    cfatmp[row * W + col] =
                        (cfatmp[row * W + col - 2] * len + riFlatFile[row][boxW + col])
                        / (len + 1);
                    cfatmp[row * W + col + 1] = (cfatmp[row * W + col - 1] * len
                                                 + riFlatFile[row][boxW + col + 1])
                                                / (len + 1);
                    len++;
                }
                ...
                for (int col = boxW + 2; col < W - boxW; col++) {
                    cfatmp[row * W + col] =
                        cfatmp[row * W + col - 2]
                        + (riFlatFile[row][boxW + col] - cfatmp[row * W + col - boxW - 2])
                              * rlen;
                }

                for (int col = W - boxW; col < W; col += 2) {
                    cfatmp[row * W + col] = (cfatmp[row * W + col - 2] * len
                                             - cfatmp[row * W + col - boxW - 2])
                                            / (len - 1);

                    if (col + 1 < W) {
                        cfatmp[row * W + col + 1] = (cfatmp[row * W + col - 1] * len
                                                     - cfatmp[row * W + col - boxW - 1])
                                                    / (len - 1);
                    }
...
#pragma omp for nowait
            ...
            for (int col = 0; col < W - 7; col += 8) {
                ...
                for (row = 2; row < boxH + 2; row += 2) {
                    ...
                    temp1v = (temp1v * lenv + LVFU(srcVertical[(row + boxH) * W + col]))
                             / lenp1v;
                    temp2v =
                        (temp2v * lenv + LVFU(srcVertical[(row + boxH + 1) * W + col]))
                        / lenp1v;
                    temp3v =
                        (temp3v * lenv + LVFU(srcVertical[(row + boxH) * W + col + 4]))
                        / lenp1v;
                    temp4v = (temp4v * lenv
                              + LVFU(srcVertical[(row + boxH + 1) * W + col + 4]))
                             / lenp1v;
                ...
                for (; row < H - boxH - 1; row += 2) {
                    temp1v = temp1v
                             + (LVFU(srcVertical[(row + boxH) * W + col])
                                - LVFU(srcVertical[(row - boxH - 2) * W + col]))
                                   / lenv;
                    temp2v = temp2v
                             + (LVFU(srcVertical[(row + 1 + boxH) * W + col])
                                - LVFU(srcVertical[(row + 1 - boxH - 2) * W + col]))
                                   / lenv;
                    temp3v = temp3v
                             + (LVFU(srcVertical[(row + boxH) * W + col + 4])
                                - LVFU(srcVertical[(row - boxH - 2) * W + col + 4]))
                                   / lenv;
                    temp4v = temp4v
                             + (LVFU(srcVertical[(row + 1 + boxH) * W + col + 4])
                                - LVFU(srcVertical[(row + 1 - boxH - 2) * W + col + 4]))
                                   / lenv;
                    STVFU(cfablur[row * W + col], temp1v);
                    STVFU(cfablur[(row + 1) * W + col], temp2v);
                    STVFU(cfablur[row * W + col + 4], temp3v);
                ...
                for (; row < H - 1; row += 2) {
                    ...
                    temp1v =
                        (temp1v * lenv - LVFU(srcVertical[(row - boxH - 2) * W + col]))
                        / lenm1v;
                    temp2v =
                        (temp2v * lenv - LVFU(srcVertical[(row - boxH - 1) * W + col]))
                        / lenm1v;
            ...
            for (int col = W - (W % 8); col < W; ++col) {
                ...
                for (int row = 2; row < boxH + 2; row += 2) {
                    cfablur[row * W + col] = (cfablur[(row - 2) * W + col] * len
                                              + srcVertical[(row + boxH) * W + col])
                                             / (len + 1);
                    cfablur[(row + 1) * W + col] =
                        (cfablur[(row - 1) * W + col] * len
                         + srcVertical[(row + boxH + 1) * W + col])
                        / (len + 1);
                    ++len;
                }

                for (int row = boxH + 2; row < H - boxH; ++row) {
                    cfablur[row * W + col] = cfablur[(row - 2) * W + col]
                                             + (srcVertical[(row + boxH) * W + col]
                                                - srcVertical[(row - boxH - 2) * W + col])
                                                   / len;
                }
...
void RawImageSource::processFlatField(const procparams::RAWParams& raw,
                                      RawImage* riFlatFile,
                                      ...
                                      const float black[4])
{
    ...
    std::array<float, 4> ffblack;
    {
        ...
        riFlatFile->set_filters(
            riFlatFile->prefilters);  // we need 4 blacks for bayer processing
        riFlatFile->get_colorsCoeff(nullptr, nullptr, ffblack.data(), false);
        riFlatFile->set_filters(tmpfilters);
    }

    if (raw.ff_BlurType
        == procparams::RAWParams::getFlatFieldBlurTypeString(
            procparams::RAWParams::FlatFieldBlurType::V))
    {
        cfaboxblur(riFlatFile->data, cfablur.get(), 2 * BS, 0, H, W);
    } else if (raw.ff_BlurType
               == procparams::RAWParams::getFlatFieldBlurTypeString(
                   procparams::RAWParams::FlatFieldBlurType::H))
    {
        cfaboxblur(riFlatFile->data, cfablur.get(), 0, 2 * BS, H, W);
    } else if (raw.ff_BlurType
               == procparams::RAWParams::getFlatFieldBlurTypeString(
                   procparams::RAWParams::FlatFieldBlurType::VH))
    {
        // slightly more complicated blur if trying to correct both vertical and
        // horizontal anomalies
        cfaboxblur(riFlatFile->data, cfablur.get(), BS, BS, H,
                   W);  // first do area blur to correct vignette
    } else
    {  //(raw.ff_BlurType == RAWParams::getFlatFieldBlurTypeString(RAWParams::area_ff))
    ...
    if (ri->getSensorType() == ST_BAYER || ri->get_colors() == 1) {
        ...
        for (int m = 0; m < 2; ++m)
            for (int n = 0; n < 2; ++n) {
                ...
                const int c4 =
                    ri->get_colors() != 1 ? ((c == 1 && !(row & 1)) ? 3 : c) : 0;
...
#pragma omp parallel for reduction(max : maxval) schedule(dynamic, 16)
                    ...
                    for (int row = 0; row < H - m; row += 2) {
                        for (int col = 0; col < W - n && !clippedBefore; col += 2) {
                            ...
                            const float tempval =
                                (rawVal - black[c4])
                                * (refcolor[m][n]
                                   / std::max(1e-5f, cfablur[(row + m) * W + col + n]
                                                         - ffblack[c4]));
                    ...
                    if (maxval + black[c4] >= ri->get_white(c4)) {
                        if (!clippedBefore) {
                            limitFactor = std::min(
                                limitFactor, ri->get_white(c4) / (maxval + black[c4]));
                        } else {
                            limitFactor = 1.f;
                        }
                    }
            ...
            flatFieldAutoClipValue =
                (1.f - limitFactor)
                * 100.f;  // this value can be used to set the clip control slider in gui
        } else {
        ...
        constexpr float minValue =
            1.f;  // if the pixel value in the flat field is less or equal this value, no
                  // correction will be applied.

#ifdef __SSE2__
        const vfloat refcolorv[2] = {
        ...
        const vfloat blackv[2] = { _mm_set_ps(black[c4[0][1]], black[c4[0][0]],
                                              black[c4[0][1]], black[c4[0][0]]),
                                   _mm_set_ps(black[c4[1][1]], black[c4[1][0]],
                                              black[c4[1][1]], black[c4[1][0]]) };
        const vfloat ffblackv[2] = { _mm_set_ps(ffblack[c4[0][1]], ffblack[c4[0][0]],
                                                ffblack[c4[0][1]], ffblack[c4[0][0]]),
                                     _mm_set_ps(ffblack[c4[1][1]], ffblack[c4[1][0]],
                                                ffblack[c4[1][1]], ffblack[c4[1][0]]) };
...
#pragma omp parallel for schedule(dynamic, 16)
        ...
        for (int row = 0; row < H; ++row) {
            ...
            for (; col < W; ++col) {
                ...
                const float vignettecorr =
                    blur <= minValue ? 1.f : refcolor[row & 1][col & 1] / blur;
                rawData[row][col] =
                    (rawData[row][col] - black[c4[row & 1][col & 1]]) * vignettecorr
                    + black[c4[row & 1][col & 1]];
            }
        }
        ...
        if (raw.ff_AutoClipControl) {
            ...
            float maxval = 0.f;
            // xtrans files have only one black level actually, so we can simplify the
            // code a bit
#ifdef _OPENMP
#pragma omp parallel for reduction(max : maxval) schedule(dynamic, 16)
#endif
            for (int row = 0; row < H; ++row) {
                for (int col = 0; col < W && !clippedBefore; ++col) {
                    ...
                    const float tempval =
                        (rawVal - black[0])
                        * (refcolor[ri->XTRANSFC(row, col)]
                           / std::max(1e-5f, cfablur[(row)*W + col] - black[0]));
                    maxval = std::max(maxval, tempval);
                }
            }
            ...
            if (!clippedBefore && maxval + black[0] > ri->get_white(0)) {
                ...
                flatFieldAutoClipValue =
                    (1.f - limitFactor) * 100.f;  // this value can be used to set the
                                                  // clip control slider in gui
            }
        } else {
...
#pragma omp parallel for
        ...
        for (int row = 0; row < H; ++row) {
            for (int col = 0; col < W; ++col) {
                ...
                rawData[row][col] =
                    (rawData[row][col] - black[c]) * vignettecorr + black[c];
            }
        }
    }
            ...
            procparams::RAWParams::FlatFieldBlurType::VH))
    {
        ...
        // slightly more complicated blur if trying to correct both vertical and
        // horizontal anomalies
        cfaboxblur(riFlatFile->data, cfablur1.get(), 0, 2 * BS, H,
                   W);  // now do horizontal blur
        cfaboxblur(riFlatFile->data, cfablur2.get(), 2 * BS, 0, H,
                   W);  // now do vertical blur
...
#pragma omp parallel for schedule(dynamic, 16)
            ...
            for (int row = 0; row < H; ++row) {
                ...
                for (; col < W - 3; col += 4) {
                    const vfloat linecorrv =
                        SQRV(vmaxf(LVFU(cfablur[row * W + col]) - ffrowBlackv, epsv))
                ...
                for (; col < W; ++col) {
                    const float linecorr =
                        SQR(std::max(1e-5f, cfablur[row * W + col]
                                                - ffblack[c4[row & 1][col & 1]]))
                        / (std::max(1e-5f, cfablur1[row * W + col]
                                               - ffblack[c4[row & 1][col & 1]])
                           * std::max(1e-5f, cfablur2[row * W + col]
                                                 - ffblack[c4[row & 1][col & 1]]));
                    rawData[row][col] =
                        (rawData[row][col] - black[c4[row & 1][col & 1]]) * linecorr
                        + black[c4[row & 1][col & 1]];
                }
            }
        } else if (ri->getSensorType() == ST_FUJI_XTRANS) {
...
#pragma omp parallel for
            ...
            for (int row = 0; row < H; ++row) {
                for (int col = 0; col < W; ++col) {
                    const int c = ri->XTRANSFC(row, col);
                    const float hlinecorr =
                        std::max(1e-5f, cfablur[(row)*W + col] - black[c])
                        / std::max(1e-5f, cfablur1[(row)*W + col] - black[c]);
                    const float vlinecorr =
                        std::max(1e-5f, cfablur[(row)*W + col] - black[c])
                        / std::max(1e-5f, cfablur2[(row)*W + col] - black[c]);
                    rawData[row][col] =
                        (rawData[row][col] - black[c]) * hlinecorr * vlinecorr + black[c];
                }
            }
        }
    }
