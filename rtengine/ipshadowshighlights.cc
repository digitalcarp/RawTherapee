...
namespace rtengine {
// modifications to pass parameters needs by locallab, to avoid 2 functions - no change in
// process - J.Desmis march 2019
                                        ...
                                        int shtonal)
{
    ...
    const auto rgb2lab = [&](float R, float G, float B, float& l, float& a,
                             float& b) -> void {
        float x, y, z;
    ...
    const auto lab2rgb = [&](float l, float a, float b, float& R, float& G,
                             float& B) -> void {
        float x, y, z;
    ...
    const auto apply = [&](int amount, int tonalwidth, bool hl) -> void {
...
#pragma omp parallel for if (multiThread)
        ...
        DiagonalCurve sh_contrast(
            { DCT_NURBS, 0, 0, 0.125, std::pow(0.125 / 0.25, contrast) * 0.25, 0.25, 0.25,
              0.375, std::pow(0.375 / 0.25, contrast) * 0.25, 1, 1 });
            ...
            for (int x = 0; x < width; ++x) {
                ...
                if (l >= 0.f && l < 32768.f) {
                    if (lab_mode) {
                        ...
                        rgb2lab(rgb[0], rgb[1], rgb[2], lab->L[y][x], lab->a[y][x],
                                lab->b[y][x]);
                    }
                }
            }
