...
void RawImageSource::amaze_demosaic_RT(int winx,
                                       int winy,
                                       int winw,
                                       int winh,
                                       ...
                                       bool measure)
{
    ...
    if (measure) {
        std::cout << "Demosaicing " << W << "x" << H << " image using AMaZE with "
    ...
    if (plistener) {
        plistener->setProgressStr(
            Glib::ustring::compose(M("TP_RAW_DMETHOD_PROGRESSBAR"), M("TP_RAW_AMAZE")));
...
// If AMAZETS is undefined it will be set to 160, which is the fastest on modern x86/64
// machines
#ifndef AMAZETS
#define AMAZETS 160
#endif
    // Tile size; the image is processed in square tiles to lower memory requirements and
    // facilitate multi-threading We assure that Tile size is a multiple of 32 in the
    // range [96;992]
    ...
    constexpr float gaussodd[4] = { 0.14659727707323927f, 0.103592713382435f,
                                    0.0732036125103057f, 0.0365543548389495f };
    // nyquist texture test threshold
    constexpr float nyqthresh = 0.5;
    // gaussian on 5x5, sigma=1.2, multiplied with nyqthresh to save some time later in
    // loop
...
#pragma omp parallel
#endif
    {
        ...
        char* buffer = (char*)calloc(14 * sizeof(float) * ts * ts
                                         + sizeof(char) * ts * tsh + 18 * cldf * 64 + 63,
                                     1);
        ...
        float* delhvsqsum =
            (float(*))((char*)rgbgreen + sizeof(float) * ts * ts + cldf * 64);  // 1
        // gradient based directional weights for interpolation
        float* dirwts0 =
            (float(*))((char*)delhvsqsum + sizeof(float) * ts * ts + cldf * 64);  // 1
        float* dirwts1 =
            (float(*))((char*)dirwts0 + sizeof(float) * ts * ts + cldf * 64);  // 1
        // vertically interpolated colour differences G-R, G-B
        float* vcd =
            (float(*))((char*)dirwts1 + sizeof(float) * ts * ts + cldf * 64);  // 1
        ...
        float* vcdalt =
            (float(*))((char*)hcd + sizeof(float) * ts * ts + cldf * 64);  // 1
        // alternative horizontal interpolation
        float* hcdalt =
            (float(*))((char*)vcdalt + sizeof(float) * ts * ts + cldf * 64);  // 1
        // square of average colour difference
        float* cddiffsq =
            (float(*))((char*)hcdalt + sizeof(float) * ts * ts + cldf * 64);  // 1
        // weight to give horizontal vs vertical interpolation
        float* hvwt =
            (float(*))((char*)cddiffsq + sizeof(float) * ts * ts + 2 * cldf * 64);  // 1
        ...
        // horizontal and vertical curvature of interpolated G (used to refine
        // interpolation in Nyquist texture regions)
        ...
        float* dginth =
            (float(*))((char*)dgintv + sizeof(float) * ts * ts + cldf * 64);  // 1
        // square of diagonal colour differences
        float* Dgrbsq1m =
            (float(*))((char*)dginth + sizeof(float) * ts * ts + cldf * 64);  // 1
        float* Dgrbsq1p =
            (float(*))((char*)Dgrbsq1m + sizeof(float) * ts * tsh + cldf * 64);  // 1
        // tile raw data
        float* cfa =
            (float(*))((char*)Dgrbsq1p + sizeof(float) * ts * tsh + cldf * 64);  // 1
        // relative weight for combining plus and minus diagonal interpolations
        float* pmwt =
            (float(*))delhvsqsum;  // there is no overlap in buffer usage => share
        ...
        float* nyqutest = (float(*))((char*)nyquist + sizeof(unsigned char) * ts * tsh
                                     + cldf * 64);  // 1

        // Main algorithm: Tile loop
        // use collapse(2) to collapse the 2 loops to one large loop, so there is better
        // scaling
#ifdef _OPENMP
            ...
            for (int left = winx - 16; left < winx + width; left += ts - 32) {
                ...
                if (rrmax < rr1) {
                    for (int rr = 0; rr < 16; rr++)
                        for (int cc = ccmin; cc < ccmax; cc += 4) {
                            ...
                            vfloat tempv =
                                LVFU(rawData[(winy + height - rr - 2)][left + cc])
                                / c65535v;
                            STVF(cfa[indx1], tempv);
                            STVF(rgbgreen[indx1], tempv);
                        }
                ...
                if (rrmax < rr1) {
                    for (int rr = 0; rr < 16; rr++)
                        for (int cc = ccmin; cc < ccmax; cc++) {
                            cfa[(rrmax + rr) * ts + cc] =
                                (rawData[(winy + height - rr - 2)][left + cc]) / 65535.f;
                            rgbgreen[(rrmax + rr) * ts + cc] =
                                cfa[(rrmax + rr) * ts + cc];
                        }
                }
                ...
                if (ccmax < cc1) {
                    for (int rr = rrmin; rr < rrmax; rr++)
                        for (int cc = 0; cc < 16; cc++) {
                            cfa[rr * ts + ccmax + cc] =
                                (rawData[(top + rr)][(winx + width - cc - 2)]) / 65535.f;
                ...
                if (rrmin > 0 && ccmin > 0) {
                    for (int rr = 0; rr < 16; rr++)
                        for (int cc = 0; cc < 16; cc++) {
                            cfa[(rr)*ts + cc] =
                                (rawData[winy + 32 - rr][winx + 32 - cc]) / 65535.f;
                            rgbgreen[(rr)*ts + cc] = cfa[(rr)*ts + cc];
                        }
                }
                ...
                if (rrmin > 0 && ccmax < cc1) {
                    for (int rr = 0; rr < 16; rr++)
                        for (int cc = 0; cc < 16; cc++) {
                            cfa[(rr)*ts + ccmax + cc] =
                                (rawData[(winy + 32 - rr)][(winx + width - cc - 2)])
                                / 65535.f;
                            rgbgreen[(rr)*ts + ccmax + cc] = cfa[(rr)*ts + ccmax + cc];
                        }
                }
                ...
                if (rrmax < rr1 && ccmin > 0) {
                    for (int rr = 0; rr < 16; rr++)
                        for (int cc = 0; cc < 16; cc++) {
                            cfa[(rrmax + rr) * ts + cc] =
                                (rawData[(winy + height - rr - 2)][(winx + 32 - cc)])
                                / 65535.f;
                            rgbgreen[(rrmax + rr) * ts + cc] =
                                cfa[(rrmax + rr) * ts + cc];
                        }
                ...
                for (int rr = 4; rr < rr1 - 4; rr++) {
                    ...
                    for (int indx = rr * ts + 4; indx < rr * ts + cc1 - 7; indx += 4) {
                        ...
                        vfloat cruv =
                            LVF(cfa[indx - v1])
                            * (LVF(dirwts0[indx - v2]) + LVF(dirwts0[indx]))
                        ...
                        vfloat crlv =
                            LVFU(cfa[indx - 1])
                            * (LVFU(dirwts1[indx - 2]) + LVF(dirwts1[indx]))
                        ...
                        // G interpolated in vert/hor directions using Hamilton-Adams
                        // method
                        vfloat guhav =
                            LVF(cfa[indx - v1]) + zd5v * (cfav - LVF(cfa[indx - v2]));
                        vfloat gdhav =
                            LVF(cfa[indx + v1]) + zd5v * (cfav - LVF(cfa[indx + v2]));
                        vfloat glhav =
                            LVFU(cfa[indx - 1]) + zd5v * (cfav - LVFU(cfa[indx - 2]));
                        vfloat grhav =
                            LVFU(cfa[indx + 1]) + zd5v * (cfav - LVFU(cfa[indx + 2]));

                        // G interpolated in vert/hor directions using adaptive ratios
                        vfloat guarv = vself(vmaskf_lt(vabsf(onev - cruv), arthreshv),
                                             cfav * cruv, guhav);
                        vfloat gdarv = vself(vmaskf_lt(vabsf(onev - crdv), arthreshv),
                                             cfav * crdv, gdhav);
                        vfloat glarv = vself(vmaskf_lt(vabsf(onev - crlv), arthreshv),
                                             cfav * crlv, glhav);
                        vfloat grarv = vself(vmaskf_lt(vabsf(onev - crrv), arthreshv),
                                             cfav * crrv, grhav);

                        // adaptive weights for vertical/horizontal directions
                        vfloat hwtv =
                            LVFU(dirwts1[indx - 1])
                            / (LVFU(dirwts1[indx - 1]) + LVFU(dirwts1[indx + 1]));
                        vfloat vwtv =
                            LVF(dirwts0[indx - v1])
                            / (LVF(dirwts0[indx + v1]) + LVF(dirwts0[indx - v1]));
                        ...
                        vmask clipmask = vorm(vorm(vmaskf_gt(cfav, clip_pt8v),
                                                   vmaskf_gt(Gintvhav, clip_pt8v)),
                        ...
                        STVF(vcd[indx],
                             vself(clipmask, vcdaltv,
                                   sgnv * (vintpf(vwtv, gdarv, guarv) - cfav)));
                        STVF(hcd[indx],
                             vself(clipmask, hcdaltv,
                                   sgnv * (vintpf(hwtv, grarv, glarv) - cfav)));

                        // differences of interpolations in opposite directions
                        STVF(dgintv[indx],
                             vminf(SQRV(guhav - gdhav), SQRV(guarv - gdarv)));
                        STVF(dginth[indx],
                             vminf(SQRV(glhav - grhav), SQRV(glarv - grarv)));
                    }
                }
                ...
                for (int rr = 4; rr < rr1 - 4; rr++) {
                    ...
                    for (int cc = 4, indx = rr * ts + cc; cc < cc1 - 4; cc++, indx++) {
                        ...
                        // adaptive weights for vertical/horizontal directions
                        float hwt =
                            dirwts1[indx - 1] / (dirwts1[indx - 1] + dirwts1[indx + 1]);
                        float vwt = dirwts0[indx - v1]
                        ...
                        if (cfa[indx] > clip_pt8 || Gintvha > clip_pt8
                            || Ginthha > clip_pt8)
                        {
                            // use HA if highlights are (nearly) clipped
                            guar = guha;
                ...
                for (int rr = 4; rr < rr1 - 4; rr++) {
                    ...
                    for (int indx = rr * ts + 4; indx < rr * ts + cc1 - 4; indx += 4) {
                        ...
                        vfloat hcdaltvarv =
                            SQRV(LVFU(hcdalt[indx - 2]) - hcdaltv)
                        ...
                        vfloat vcdaltvarv =
                            SQRV(LVF(vcdalt[indx - v2]) - vcdaltv)
                            + SQRV(LVF(vcdalt[indx - v2]) - LVF(vcdalt[indx + v2]))
                            + SQRV(vcdaltv - LVF(vcdalt[indx + v2]));

                        // choose the smallest variance; this yields a smoother
                        // interpolation
                ...
                for (int rr = 4; rr < rr1 - 4; rr++) {
                    for (int cc = 4, indx = rr * ts + cc, c = fc(cfarray, rr, cc) & 1;
                         cc < cc1 - 4; cc++, indx++)
                    {
                        float hcdvar = 3.f
                                           * (SQR(hcd[indx - 2]) + SQR(hcd[indx])
                                              + SQR(hcd[indx + 2]))
                                       - SQR(hcd[indx - 2] + hcd[indx] + hcd[indx + 2]);
                        float hcdaltvar =
                            3.f
                                * (SQR(hcdalt[indx - 2]) + SQR(hcdalt[indx])
                                   + SQR(hcdalt[indx + 2]))
                            - SQR(hcdalt[indx - 2] + hcdalt[indx] + hcdalt[indx + 2]);
                        float vcdvar = 3.f
                                           * (SQR(vcd[indx - v2]) + SQR(vcd[indx])
                                              + SQR(vcd[indx + v2]))
                                       - SQR(vcd[indx - v2] + vcd[indx] + vcd[indx + v2]);
                        float vcdaltvar =
                            3.f
                                * (SQR(vcdalt[indx - v2]) + SQR(vcdalt[indx])
                                   + SQR(vcdalt[indx + v2]))
                        ...
                        if (c) {                             // G site
                            ...
                            if (hcd[indx] > 0) {
                                if (3.f * hcd[indx] > (Ginth + cfa[indx])) {
                                    hcd[indx] =
                                        -median(Ginth, cfa[indx - 1], cfa[indx + 1])
                                        + cfa[indx];
                                } else {
                                    float hwt =
                                        1.f - 3.f * hcd[indx] / (eps + Ginth + cfa[indx]);
                                    hcd[indx] = hwt * hcd[indx]
                                                + (1.f - hwt)
                                                      * (-median(Ginth, cfa[indx - 1],
                                                                 cfa[indx + 1])
                                                         + cfa[indx]);
                                }
                            }
                            ...
                            if (vcd[indx] > 0) {
                                if (3.f * vcd[indx] > (Gintv + cfa[indx])) {
                                    vcd[indx] =
                                        -median(Gintv, cfa[indx - v1], cfa[indx + v1])
                                        + cfa[indx];
                                } else {
                                    float vwt =
                                        1.f - 3.f * vcd[indx] / (eps + Gintv + cfa[indx]);
                                    vcd[indx] = vwt * vcd[indx]
                                                + (1.f - vwt)
                                                      * (-median(Gintv, cfa[indx - v1],
                                                                 cfa[indx + v1])
                                                         + cfa[indx]);
                                }
                            }
                            ...
                            if (Ginth > clip_pt) {
                                hcd[indx] = -median(Ginth, cfa[indx - 1], cfa[indx + 1])
                                            + cfa[indx];
                            }

                            if (Gintv > clip_pt) {
                                vcd[indx] = -median(Gintv, cfa[indx - v1], cfa[indx + v1])
                                            + cfa[indx];
                            }
                ...
                for (int rr = 6; rr < rr1 - 6; rr++) {
                    for (int indx = rr * ts + 6 + (fc(cfarray, rr, 2) & 1);
                         indx < rr * ts + cc1 - 6; indx += 8)
                    {
                        // compute colour difference variances in cardinal directions
                        vfloat tempv = LC2VFU(vcd[indx]);
                        vfloat uavev = tempv + LC2VFU(vcd[indx - v1])
                                       + LC2VFU(vcd[indx - v2]) + LC2VFU(vcd[indx - v3]);
                        vfloat davev = tempv + LC2VFU(vcd[indx + v1])
                                       + LC2VFU(vcd[indx + v2]) + LC2VFU(vcd[indx + v3]);
                        vfloat Dgrbvvaruv = SQRV(tempv - uavev)
                                            + SQRV(LC2VFU(vcd[indx - v1]) - uavev)
                                            + SQRV(LC2VFU(vcd[indx - v2]) - uavev)
                                            + SQRV(LC2VFU(vcd[indx - v3]) - uavev);
                        vfloat Dgrbvvardv = SQRV(tempv - davev)
                                            + SQRV(LC2VFU(vcd[indx + v1]) - davev)
                                            + SQRV(LC2VFU(vcd[indx + v2]) - davev)
                                            + SQRV(LC2VFU(vcd[indx + v3]) - davev);

                        vfloat hwtv = vadivapb(LC2VFU(dirwts1[indx - 1]),
                                               LC2VFU(dirwts1[indx + 1]));
                        vfloat vwtv = vadivapb(LC2VFU(dirwts0[indx - v1]),
                                               LC2VFU(dirwts0[indx + v1]));

                        tempv = LC2VFU(hcd[indx]);
                        vfloat lavev =
                            tempv + vaddc2vfu(hcd[indx - 3]) + LC2VFU(hcd[indx - 1]);
                        vfloat ravev =
                            tempv + vaddc2vfu(hcd[indx + 1]) + LC2VFU(hcd[indx + 3]);

                        vfloat Dgrbhvarlv = SQRV(tempv - lavev)
                                            + SQRV(LC2VFU(hcd[indx - 1]) - lavev)
                                            + SQRV(LC2VFU(hcd[indx - 2]) - lavev)
                                            + SQRV(LC2VFU(hcd[indx - 3]) - lavev);
                        vfloat Dgrbhvarrv = SQRV(tempv - ravev)
                                            + SQRV(LC2VFU(hcd[indx + 1]) - ravev)
                                            + SQRV(LC2VFU(hcd[indx + 2]) - ravev)
                                            + SQRV(LC2VFU(hcd[indx + 3]) - ravev);

                        vfloat vcdvarv = epssqv + vintpf(vwtv, Dgrbvvardv, Dgrbvvaruv);
                        vfloat hcdvarv = epssqv + vintpf(hwtv, Dgrbhvarrv, Dgrbhvarlv);

                        // compute fluctuations in up/down and left/right interpolations
                        // of colours
                        Dgrbvvaruv =
                            LC2VFU(dgintv[indx - v1]) + LC2VFU(dgintv[indx - v2]);
                        Dgrbvvardv =
                            LC2VFU(dgintv[indx + v1]) + LC2VFU(dgintv[indx + v2]);
                        ...
                        vfloat vcdvar1v = epssqv + LC2VFU(dgintv[indx])
                                          + vintpf(vwtv, Dgrbvvardv, Dgrbvvaruv);
                        vfloat hcdvar1v = epssqv + LC2VFU(dginth[indx])
                                          + vintpf(hwtv, Dgrbhvarrv, Dgrbhvarlv);
                        ...
                        // if both agree on interpolation direction, choose the one with
                        // strongest directional discrimination; otherwise, choose the u/d
                        // and l/r difference fluctuation weights
                        vmask decmask =
                            vandm(vmaskf_gt((zd5v - varwtv) * (zd5v - diffwtv), ZEROV),
                ...
                for (int rr = 6; rr < rr1 - 6; rr++) {
                    for (int cc = 6 + (fc(cfarray, rr, 2) & 1), indx = rr * ts + cc;
                         cc < cc1 - 6; cc += 2, indx += 2)
                    {
                        ...
                        float dave =
                            vcd[indx] + vcd[indx + v1] + vcd[indx + v2] + vcd[indx + v3];
                        float lave =
                            hcd[indx] + hcd[indx - 1] + hcd[indx - 2] + hcd[indx - 3];
                        float rave =
                            hcd[indx] + hcd[indx + 1] + hcd[indx + 2] + hcd[indx + 3];

                        // colour difference (G-R or G-B) variance in up/down/left/right
                        // directions
                        float Dgrbvvaru =
                            SQR(vcd[indx] - uave) + SQR(vcd[indx - v1] - uave)
                            + SQR(vcd[indx - v2] - uave) + SQR(vcd[indx - v3] - uave);
                        float Dgrbvvard =
                            SQR(vcd[indx] - dave) + SQR(vcd[indx + v1] - dave)
                            + SQR(vcd[indx + v2] - dave) + SQR(vcd[indx + v3] - dave);
                        float Dgrbhvarl =
                            SQR(hcd[indx] - lave) + SQR(hcd[indx - 1] - lave)
                            + SQR(hcd[indx - 2] - lave) + SQR(hcd[indx - 3] - lave);
                        float Dgrbhvarr =
                            SQR(hcd[indx] - rave) + SQR(hcd[indx + 1] - rave)
                            + SQR(hcd[indx + 2] - rave) + SQR(hcd[indx + 3] - rave);
                        ...
                        // of colours
                        Dgrbvvaru =
                            (dgintv[indx]) + (dgintv[indx - v1]) + (dgintv[indx - v2]);
                        Dgrbvvard =
                            (dgintv[indx]) + (dgintv[indx + v1]) + (dgintv[indx + v2]);
                        Dgrbhvarl =
                            (dginth[indx]) + (dginth[indx - 1]) + (dginth[indx - 2]);
                        Dgrbhvarr =
                            (dginth[indx]) + (dginth[indx + 1]) + (dginth[indx + 2]);
                ...
                for (int rr = 6; rr < rr1 - 6; rr++) {
                    ...
                    for (; cc < cc1 - 7; cc += 8, indx += 8) {
                        vfloat valv = (gausso0 * LC2VFU(cddiffsq[indx])
                                       + gausso1
                                             * (LC2VFU(cddiffsq[(indx - m1)])
                                                + LC2VFU(cddiffsq[(indx + p1)])
                                                + LC2VFU(cddiffsq[(indx - p1)])
                                                + LC2VFU(cddiffsq[(indx + m1)]))
                                       + gausso2
                                             * (LC2VFU(cddiffsq[(indx - v2)])
                                                + LC2VFU(cddiffsq[(indx - 2)])
                                                + LC2VFU(cddiffsq[(indx + 2)])
                                                + LC2VFU(cddiffsq[(indx + v2)]))
                                       + gausso3
                                             * (LC2VFU(cddiffsq[(indx - m2)])
                                                + LC2VFU(cddiffsq[(indx + p2)])
                                                + LC2VFU(cddiffsq[(indx - p2)])
                                                + LC2VFU(cddiffsq[(indx + m2)])))
                                      - (gaussg0 * LC2VFU(delhvsqsum[indx])
                                         + gaussg1
                                               * (LC2VFU(delhvsqsum[indx - v1])
                                                  + LC2VFU(delhvsqsum[indx - 1])
                                                  + LC2VFU(delhvsqsum[indx + 1])
                                                  + LC2VFU(delhvsqsum[indx + v1]))
                                         + gaussg2
                                               * (LC2VFU(delhvsqsum[indx - m1])
                                                  + LC2VFU(delhvsqsum[indx + p1])
                                                  + LC2VFU(delhvsqsum[indx - p1])
                                                  + LC2VFU(delhvsqsum[indx + m1]))
                                         + gaussg3
                                               * (LC2VFU(delhvsqsum[indx - v2])
                                                  + LC2VFU(delhvsqsum[indx - 2])
                                                  + LC2VFU(delhvsqsum[indx + 2])
                                                  + LC2VFU(delhvsqsum[indx + v2]))
                                         + gaussg4
                                               * (LC2VFU(delhvsqsum[indx - v2 - 1])
                                                  + LC2VFU(delhvsqsum[indx - v2 + 1])
                                                  + LC2VFU(delhvsqsum[indx - ts - 2])
                                                  + LC2VFU(delhvsqsum[indx - ts + 2])
                                                  + LC2VFU(delhvsqsum[indx + ts - 2])
                                                  + LC2VFU(delhvsqsum[indx + ts + 2])
                                                  + LC2VFU(delhvsqsum[indx + v2 - 1])
                                                  + LC2VFU(delhvsqsum[indx + v2 + 1]))
                                         + gaussg5
                                               * (LC2VFU(delhvsqsum[indx - m2])
                                                  + LC2VFU(delhvsqsum[indx + p2])
                                                  + LC2VFU(delhvsqsum[indx - p2])
                                                  + LC2VFU(delhvsqsum[indx + m2])));
                    ...
                    for (; cc < cc1 - 6; cc += 2, indx += 2) {
                        nyqutest[indx >> 1] =
                            (gaussodd[0] * cddiffsq[indx]
                             + gaussodd[1]
                                   * (cddiffsq[(indx - m1)] + cddiffsq[(indx + p1)]
                                      + cddiffsq[(indx - p1)] + cddiffsq[(indx + m1)])
                             + gaussodd[2]
                                   * (cddiffsq[(indx - v2)] + cddiffsq[(indx - 2)]
                                      + cddiffsq[(indx + 2)] + cddiffsq[(indx + v2)])
                             + gaussodd[3]
                                   * (cddiffsq[(indx - m2)] + cddiffsq[(indx + p2)]
                                      + cddiffsq[(indx - p2)] + cddiffsq[(indx + m2)]))
                            - (gaussgrad[0] * delhvsqsum[indx]
                               + gaussgrad[1]
                                     * (delhvsqsum[indx - v1] + delhvsqsum[indx + 1]
                                        + delhvsqsum[indx - 1] + delhvsqsum[indx + v1])
                               + gaussgrad[2]
                                     * (delhvsqsum[indx - m1] + delhvsqsum[indx + p1]
                                        + delhvsqsum[indx - p1] + delhvsqsum[indx + m1])
                               + gaussgrad[3]
                                     * (delhvsqsum[indx - v2] + delhvsqsum[indx - 2]
                               ...
                               + gaussgrad[4]
                                     * (delhvsqsum[indx - v2 - 1]
                                        ...
                                        + delhvsqsum[indx + v2 + 1])
                               + gaussgrad[5]
                                     * (delhvsqsum[indx - m2] + delhvsqsum[indx + p2]
                ...
                for (int rr = 6; rr < rr1 - 6; rr++) {
                         ...
                         cc < cc1 - 6; cc += 2, indx += 2)
                    {
                        ...
                        // nyquist texture test: ask if difference of vcd compared to hcd
                        // is larger or smaller than RGGB gradients
                ...
                if (doNyquist) {
                    ...
                    for (int rr = nystartrow; rr < nyendrow; rr++) {
                        ...
                        for (int indx = rr * ts; indx < rr * ts + cc1; indx += 32) {
                            ...
                            valv =
                                vselc(_mm_cmpgt_epi8(nyquisttemp1v, fourvb), onevb, valv);
                            valv =
                                vselinotzero(_mm_cmplt_epi8(nyquisttemp1v, fourvb), valv);
                            _mm_store_si128((vint*)&nyquist2[indx >> 1], valv);
                        }

#else

                        for (int indx = rr * ts + nystartcol + (fc(cfarray, rr, 2) & 1);
                             indx < rr * ts + nyendcol; indx += 2)
                        {
                            ...
                            // if most of your neighbours are named Nyquist, it's likely
                            // that you're one too, or not
                            nyquist2[indx >> 1] =
                                nyquisttemp > 4
                                    ? 1
                                    : (nyquisttemp < 4 ? 0 : nyquist[indx >> 1]);
                        }
                    ...
                    for (int rr = nystartrow; rr < nyendrow; rr++)
                             ...
                             indx < rr * ts + nyendcol; indx += 2)
                        {
                            ...
                            if (nyquist2[indx >> 1]) {
                                ...
                                float sumcfa = 0.f, sumh = 0.f, sumv = 0.f, sumsqh = 0.f,
                                      sumsqv = 0.f, areawt = 0.f;
                                ...
                                for (int i = -6; i < 7; i += 2) {
                                    ...
                                    for (int j = -6; j < 7; j += 2, indx1 += 2) {
                                        if (nyquist2[indx1 >> 1]) {
                                            ...
                                            sumsqh += SQR(cfatemp - cfa[indx1 - 1])
                                                      + SQR(cfatemp - cfa[indx1 + 1]);
                                            sumsqv += SQR(cfatemp - cfa[indx1 - v1])
                                ...
                                // horizontal and vertical colour differences, and
                                // adaptive weight
                ...
                for (int rr = 8; rr < rr1 - 8; rr++)
                    for (int indx = rr * ts + 8 + (fc(cfarray, rr, 2) & 1);
                         indx < rr * ts + cc1 - 8; indx += 2)
                    {

                        // first ask if one gets more directional discrimination from
                        // nearby B/R sites
                        float hvwtalt =
                            xdivf(hvwt[(indx - m1) >> 1] + hvwt[(indx + p1) >> 1]
                                      + hvwt[(indx - p1) >> 1] + hvwt[(indx + m1) >> 1],
                                  2);

                        hvwt[indx >> 1] =
                            fabsf(0.5f - hvwt[indx >> 1]) < fabsf(0.5f - hvwtalt)
                                ? hvwtalt
                                : hvwt[indx >> 1];
                        // a better result was obtained from the neighbours

                        Dgrb[0][indx >> 1] =
                            intp(hvwt[indx >> 1], vcd[indx],
                                 hcd[indx]);  // evaluate colour differences

                        rgbgreen[indx] =
                            cfa[indx] + Dgrb[0][indx >> 1];  // evaluate G (finally!)
                ...
                if (doNyquist) {
                    for (int rr = nystartrow; rr < nyendrow; rr++)
                             ...
                             indx < rr * ts + nyendcol; indx += 2)
                        {
                            ...
                            if (nyquist2[indx >> 1]) {
                                // local averages (over Nyquist pixels only) of G
                                // curvature squared
                                float gvarh = epssq
                ...
                for (int rr = 6; rr < rr1 - 6; rr++) {
                    if ((fc(cfarray, rr, 2) & 1) == 0) {
                        for (int cc = 6, indx = rr * ts + cc; cc < cc1 - 6;
                             cc += 8, indx += 8)
                        {
                            vfloat tempv = LC2VFU(cfa[indx + 1]);
                            vfloat Dgrbsq1pv =
                                (SQRV(tempv - LC2VFU(cfa[indx + 1 - p1]))
                                 + SQRV(tempv - LC2VFU(cfa[indx + 1 + p1])));
                            STVFU(delp[indx >> 1],
                                  vabsf(LC2VFU(cfa[indx + p1]) - LC2VFU(cfa[indx - p1])));
                            STVFU(delm[indx >> 1],
                                  vabsf(LC2VFU(cfa[indx + m1]) - LC2VFU(cfa[indx - m1])));
                            vfloat Dgrbsq1mv =
                                (SQRV(tempv - LC2VFU(cfa[indx + 1 - m1]))
                                 + SQRV(tempv - LC2VFU(cfa[indx + 1 + m1])));
                             ...
                             cc += 8, indx += 8)
                        {
                            ...
                            vfloat Dgrbsq1pv = (SQRV(tempv - LC2VFU(cfa[indx - p1]))
                                                + SQRV(tempv - LC2VFU(cfa[indx + p1])));
                            STVFU(delp[indx >> 1], vabsf(LC2VFU(cfa[indx + 1 + p1])
                                                         - LC2VFU(cfa[indx + 1 - p1])));
                            STVFU(delm[indx >> 1], vabsf(LC2VFU(cfa[indx + 1 + m1])
                                                         - LC2VFU(cfa[indx + 1 - m1])));
                            vfloat Dgrbsq1mv = (SQRV(tempv - LC2VFU(cfa[indx - m1]))
                                                + SQRV(tempv - LC2VFU(cfa[indx + m1])));
                            STVFU(Dgrbsq1m[indx >> 1], Dgrbsq1mv);
                            STVFU(Dgrbsq1p[indx >> 1], Dgrbsq1pv);
                        }
                    }
                ...
                for (int rr = 6; rr < rr1 - 6; rr++) {
                    if ((fc(cfarray, rr, 2) & 1) == 0) {
                        ...
                        for (int cc = 6, indx = rr * ts + cc; cc < cc1 - 6;
                             cc += 2, indx += 2)
                        {
                            Dgrbsq1p[indx >> 1] = (SQR(cfa[indx] - cfa[indx - p1])
                                                   + SQR(cfa[indx] - cfa[indx + p1]));
                            Dgrbsq1m[indx >> 1] = (SQR(cfa[indx] - cfa[indx - m1])
                                                   + SQR(cfa[indx] - cfa[indx + m1]));
                            delp[indx >> 1] =
                                fabsf(cfa[indx + 1 + p1] - cfa[indx + 1 - p1]);
                            delm[indx >> 1] =
                                fabsf(cfa[indx + 1 + m1] - cfa[indx + 1 - m1]);
                        }
                    }
                }
                ...
                for (int rr = 8; rr < rr1 - 8; rr++) {
                    ...
                    for (int indx = rr * ts + 8 + (fc(cfarray, rr, 2) & 1),
                             indx1 = indx >> 1;
                         indx < rr * ts + cc1 - 8; indx += 8, indx1 += 4)
                    {
                        ...
                        rbsev = vself(vmaskf_lt(vabsf(onev - rbsev), arthreshv),
                                      cfav * rbsev, temp1v + zd5v * (cfav - temp2v));

                        temp1v = LC2VFU(cfa[indx - m1]);
                        temp2v = LC2VFU(cfa[indx - m2]);
                        vfloat rbnwv = vmul2f(temp1v) / (epsv + cfav + temp2v);
                        rbnwv = vself(vmaskf_lt(vabsf(onev - rbnwv), arthreshv),
                                      cfav * rbnwv, temp1v + zd5v * (cfav - temp2v));

                        temp1v = epsv + LVFU(delm[indx1]);
                        vfloat wtsev =
                            temp1v + LVFU(delm[(indx + m1) >> 1])
                            + LVFU(
                                delm[(indx + m2) >> 1]);  // same as for wtu,wtd,wtl,wtr
                        vfloat wtnwv = temp1v + LVFU(delm[(indx - m1) >> 1])
                                       + LVFU(delm[(indx - m2) >> 1]);

                        vfloat rbmv = (wtsev * rbnwv + wtnwv * rbsev) / (wtsev + wtnwv);

                        temp1v =
                            median(rbmv, LC2VFU(cfa[indx - m1]), LC2VFU(cfa[indx + m1]));
                        ...
                        STVFU(rbm[indx1], vself(vmaskf_gt(temp2v, clip_ptv),
                                                median(temp2v, LC2VFU(cfa[indx - m1]),
                                                       LC2VFU(cfa[indx + m1])),
                                                temp2v));
                        ...
                        rbnev = vself(vmaskf_lt(vabsf(onev - rbnev), arthreshv),
                                      cfav * rbnev, temp1v + zd5v * (cfav - temp2v));

                        temp1v = LC2VFU(cfa[indx - p1]);
                        temp2v = LC2VFU(cfa[indx - p2]);
                        vfloat rbswv = vmul2f(temp1v) / (epsv + cfav + temp2v);
                        rbswv = vself(vmaskf_lt(vabsf(onev - rbswv), arthreshv),
                                      cfav * rbswv, temp1v + zd5v * (cfav - temp2v));

                        temp1v = epsv + LVFU(delp[indx1]);
                        vfloat wtnev = temp1v + LVFU(delp[(indx + p1) >> 1])
                                       + LVFU(delp[(indx + p2) >> 1]);
                        vfloat wtswv = temp1v + LVFU(delp[(indx - p1) >> 1])
                                       + LVFU(delp[(indx - p2) >> 1]);

                        vfloat rbpv = (wtnev * rbswv + wtswv * rbnev) / (wtnev + wtswv);

                        temp1v =
                            median(rbpv, LC2VFU(cfa[indx - p1]), LC2VFU(cfa[indx + p1]));
                        ...
                        STVFU(rbp[indx1], vself(vmaskf_gt(temp2v, clip_ptv),
                                                median(temp2v, LC2VFU(cfa[indx - p1]),
                                                       LC2VFU(cfa[indx + p1])),
                                                temp2v));
                        ...
                        vfloat rbvarmv =
                            epssqv
                            + (gausseven0v
                                   * (LVFU(Dgrbsq1m[(indx - v1) >> 1])
                                      + LVFU(Dgrbsq1m[(indx - 1) >> 1])
                                      + LVFU(Dgrbsq1m[(indx + 1) >> 1])
                                      + LVFU(Dgrbsq1m[(indx + v1) >> 1]))
                               + gausseven1v
                                     * (LVFU(Dgrbsq1m[(indx - v2 - 1) >> 1])
                                        + LVFU(Dgrbsq1m[(indx - v2 + 1) >> 1])
                                        + LVFU(Dgrbsq1m[(indx - 2 - v1) >> 1])
                                        + LVFU(Dgrbsq1m[(indx + 2 - v1) >> 1])
                                        + LVFU(Dgrbsq1m[(indx - 2 + v1) >> 1])
                                        + LVFU(Dgrbsq1m[(indx + 2 + v1) >> 1])
                                        + LVFU(Dgrbsq1m[(indx + v2 - 1) >> 1])
                        ...
                        STVFU(
                            ...
                            rbvarmv
                                / ((epssqv
                                    + (gausseven0v
                                           * (LVFU(Dgrbsq1p[(indx - v1) >> 1])
                                              + LVFU(Dgrbsq1p[(indx - 1) >> 1])
                                              + LVFU(Dgrbsq1p[(indx + 1) >> 1])
                                              + LVFU(Dgrbsq1p[(indx + v1) >> 1]))
                                       + gausseven1v
                    ...
                    for (int cc = 8 + (fc(cfarray, rr, 2) & 1), indx = rr * ts + cc,
                             indx1 = indx >> 1;
                         cc < cc1 - 8; cc += 2, indx += 2, indx1++)
                    {
                        ...
                        // diagonal colour ratios
                        float crse =
                            xmul2f(cfa[indx + m1]) / (eps + cfa[indx] + (cfa[indx + m2]));
                        float crnw =
                            xmul2f(cfa[indx - m1]) / (eps + cfa[indx] + (cfa[indx - m2]));
                        float crne =
                            xmul2f(cfa[indx + p1]) / (eps + cfa[indx] + (cfa[indx + p2]));
                        float crsw =
                            xmul2f(cfa[indx - p1]) / (eps + cfa[indx] + (cfa[indx - p2]));
                        ...
                        if (fabsf(1.f - crse) < arthresh) {
                            rbse = cfa[indx] * crse;  // use this if more precise diag
                                                      // interp is necessary
                        } else {
                        ...
                        float wtse =
                            ...
                            + delm[(indx + m2) >> 1];  // same as for wtu,wtd,wtl,wtr
                        float wtnw = eps + delm[indx1] + delm[(indx - m1) >> 1]
                                     + delm[(indx - m2) >> 1];
                        float wtne = eps + delp[indx1] + delp[(indx + p1) >> 1]
                                     + delp[(indx + p2) >> 1];
                        float wtsw = eps + delp[indx1] + delp[(indx - p1) >> 1]
                                     + delp[(indx - p2) >> 1];
                        ...
                        if (rbp[indx1] < cfa[indx]) {
                            if (xmul2f(rbp[indx1]) < cfa[indx]) {
                                rbp[indx1] =
                                    median(rbp[indx1], cfa[indx - p1], cfa[indx + p1]);
                            } else {
                                float pwt = xmul2f(cfa[indx] - rbp[indx1])
                                            / (eps + rbp[indx1] + cfa[indx]);
                                rbp[indx1] = pwt * rbp[indx1]
                                             + (1.f - pwt)
                                                   * median(rbp[indx1], cfa[indx - p1],
                                                            cfa[indx + p1]);
                            }
                        }
                        ...
                        if (rbm[indx1] < cfa[indx]) {
                            if (xmul2f(rbm[indx1]) < cfa[indx]) {
                                rbm[indx1] =
                                    median(rbm[indx1], cfa[indx - m1], cfa[indx + m1]);
                            } else {
                                float mwt = xmul2f(cfa[indx] - rbm[indx1])
                                            / (eps + rbm[indx1] + cfa[indx]);
                                rbm[indx1] = mwt * rbm[indx1]
                                             + (1.f - mwt)
                                                   * median(rbm[indx1], cfa[indx - m1],
                                                            cfa[indx + m1]);
                            }
                        }
                ...
                for (int rr = 10; rr < rr1 - 10; rr++)
#ifdef __SSE2__
                    for (int indx = rr * ts + 10 + (fc(cfarray, rr, 2) & 1),
                             indx1 = indx >> 1;
                         indx < rr * ts + cc1 - 10; indx += 8, indx1 += 4)
                    {
                        ...
                        tempv =
                            vself(vmaskf_lt(vabsf(zd5v - tempv), vabsf(zd5v - pmwtaltv)),
                                  pmwtaltv, tempv);
                        STVFU(pmwt[indx1], tempv);
                        STVFU(rbint[indx1], zd5v
                                                * (LC2VFU(cfa[indx])
                                                   + vintpf(tempv, LVFU(rbp[indx1]),
                                                            LVFU(rbm[indx1]))));
                    }

#else

                    for (int cc = 10 + (fc(cfarray, rr, 2) & 1), indx = rr * ts + cc,
                             indx1 = indx >> 1;
                         cc < cc1 - 10; cc += 2, indx += 2, indx1++)
                    {
                        ...
                        // nearby B/R sites
                        float pmwtalt =
                            xdivf(pmwt[(indx - m1) >> 1] + pmwt[(indx + p1) >> 1]
                                      + pmwt[(indx - p1) >> 1] + pmwt[(indx + m1) >> 1],
                                  2);

                        if (fabsf(0.5f - pmwt[indx1]) < fabsf(0.5f - pmwtalt)) {
                            pmwt[indx1] = pmwtalt;  // a better result was obtained from
                                                    // the neighbours
                        }
                ...
                for (int rr = 12; rr < rr1 - 12; rr++)
#ifdef __SSE2__
                    for (int indx = rr * ts + 12 + (fc(cfarray, rr, 2) & 1),
                             indx1 = indx >> 1;
                         indx < rr * ts + cc1 - 12; indx += 8, indx1 += 4)
                    {
                        vmask copymask = vmaskf_ge(vabsf(zd5v - LVFU(pmwt[indx1])),
                                                   vabsf(zd5v - LVFU(hvwt[indx1])));

                        if (_mm_movemask_ps((vfloat)copymask))
                        {  // if for any of the 4 pixels the condition is true, do the
                           // maths for all 4 pixels and mask the unused out at the end
                            // now interpolate G vertically/horizontally using R+B values
                            // unfortunately, since G interpolation cannot be done
                            // diagonally this may lead to colour shifts colour ratios for
                            // G interpolation
                            vfloat rbintv = LVFU(rbint[indx1]);

                            // interpolated G via adaptive ratios or Hamilton-Adams in
                            // each cardinal direction
                            vfloat cruv = vmul2f(LC2VFU(cfa[indx - v1]))
                                          / (epsv + rbintv + LVFU(rbint[(indx1 - v1)]));
                            vfloat guv = rbintv * cruv;
                            vfloat gu2v = LC2VFU(cfa[indx - v1])
                                          + zd5v * (rbintv - LVFU(rbint[(indx1 - v1)]));
                            guv = vself(vmaskf_lt(vabsf(onev - cruv), arthreshv), guv,
                                        gu2v);

                            vfloat crdv = vmul2f(LC2VFU(cfa[indx + v1]))
                                          / (epsv + rbintv + LVFU(rbint[(indx1 + v1)]));
                            vfloat gdv = rbintv * crdv;
                            vfloat gd2v = LC2VFU(cfa[indx + v1])
                                          + zd5v * (rbintv - LVFU(rbint[(indx1 + v1)]));
                            gdv = vself(vmaskf_lt(vabsf(onev - crdv), arthreshv), gdv,
                                        gd2v);

                            vfloat Gintvv = (LC2VFU(dirwts0[indx - v1]) * gdv
                                             + LC2VFU(dirwts0[indx + v1]) * guv)
                                            / (LC2VFU(dirwts0[indx + v1])
                                               + LC2VFU(dirwts0[indx - v1]));
                            vfloat Gint1v = median(Gintvv, LC2VFU(cfa[indx - v1]),
                                                   LC2VFU(cfa[indx + v1]));
                            vfloat vwtv =
                                vmul2f(rbintv - Gintvv) / (epsv + Gintvv + rbintv);
                            vfloat Gint2v = vintpf(vwtv, Gintvv, Gint1v);
                            Gint1v =
                                vself(vmaskf_lt(vmul2f(Gintvv), rbintv), Gint1v, Gint2v);
                            Gintvv = vself(vmaskf_lt(Gintvv, rbintv), Gint1v, Gintvv);
                            Gintvv = vself(vmaskf_gt(Gintvv, clip_ptv),
                                           median(Gintvv, LC2VFU(cfa[indx - v1]),
                                                  LC2VFU(cfa[indx + v1])),
                                           Gintvv);

                            vfloat crlv = vmul2f(LC2VFU(cfa[indx - 1]))
                                          / (epsv + rbintv + LVFU(rbint[(indx1 - 1)]));
                            vfloat glv = rbintv * crlv;
                            vfloat gl2v = LC2VFU(cfa[indx - 1])
                                          + zd5v * (rbintv - LVFU(rbint[(indx1 - 1)]));
                            glv = vself(vmaskf_lt(vabsf(onev - crlv), arthreshv), glv,
                                        gl2v);

                            vfloat crrv = vmul2f(LC2VFU(cfa[indx + 1]))
                                          / (epsv + rbintv + LVFU(rbint[(indx1 + 1)]));
                            vfloat grv = rbintv * crrv;
                            vfloat gr2v = LC2VFU(cfa[indx + 1])
                                          + zd5v * (rbintv - LVFU(rbint[(indx1 + 1)]));
                            grv = vself(vmaskf_lt(vabsf(onev - crrv), arthreshv), grv,
                                        gr2v);

                            vfloat Ginthv =
                                (LC2VFU(dirwts1[indx - 1]) * grv
                                 + LC2VFU(dirwts1[indx + 1]) * glv)
                                / (LC2VFU(dirwts1[indx - 1]) + LC2VFU(dirwts1[indx + 1]));
                            vfloat Gint1h = median(Ginthv, LC2VFU(cfa[indx - 1]),
                                                   LC2VFU(cfa[indx + 1]));
                            vfloat hwtv =
                                vmul2f(rbintv - Ginthv) / (epsv + Ginthv + rbintv);
                            vfloat Gint2h = vintpf(hwtv, Ginthv, Gint1h);
                            Gint1h =
                                vself(vmaskf_lt(vmul2f(Ginthv), rbintv), Gint1h, Gint2h);
                            Ginthv = vself(vmaskf_lt(Ginthv, rbintv), Gint1h, Ginthv);
                            Ginthv = vself(vmaskf_gt(Ginthv, clip_ptv),
                                           median(Ginthv, LC2VFU(cfa[indx - 1]),
                                                  LC2VFU(cfa[indx + 1])),
                                           Ginthv);

                            vfloat greenv =
                                vself(copymask, vintpf(LVFU(hvwt[indx1]), Gintvv, Ginthv),
                                      LC2VFU(rgbgreen[indx]));
                            STC2VFU(rgbgreen[indx], greenv);

                            STVFU(Dgrb[0][indx1],
                                  vself(copymask, greenv - LC2VFU(cfa[indx]),
                                        LVFU(Dgrb[0][indx1])));
                        }
                    }
                    ...
                    for (int cc = 12 + (fc(cfarray, rr, 2) & 1), indx = rr * ts + cc,
                             indx1 = indx >> 1;
                         cc < cc1 - 12; cc += 2, indx += 2, indx1++)
                    {
                        ...
                        // now interpolate G vertically/horizontally using R+B values
                        // unfortunately, since G interpolation cannot be done diagonally
                        // this may lead to colour shifts

                        // colour ratios for G interpolation
                        float cru = cfa[indx - v1] * 2.0
                                    / (eps + rbint[indx1] + rbint[(indx1 - v1)]);
                        float crd = cfa[indx + v1] * 2.0
                                    / (eps + rbint[indx1] + rbint[(indx1 + v1)]);
                        float crl = cfa[indx - 1] * 2.0
                                    / (eps + rbint[indx1] + rbint[(indx1 - 1)]);
                        float crr = cfa[indx + 1] * 2.0
                                    / (eps + rbint[indx1] + rbint[(indx1 + 1)]);

                        // interpolation of G in four directions
                        float gu, gd, gl, gr;

                        // interpolated G via adaptive ratios or Hamilton-Adams in each
                        // cardinal direction
                        if (fabsf(1.f - cru) < arthresh) {
                            gu = rbint[indx1] * cru;
                        } else {
                        ...
                        if (fabsf(1.f - crl) < arthresh) {
                            ...
                            gl =
                                cfa[indx - 1] + xdiv2f(rbint[indx1] - rbint[(indx1 - 1)]);
                        }
                        ...
                        if (Gintv < rbint[indx1]) {
                            if (2 * Gintv < rbint[indx1]) {
                                ...
                                float vwt = 2.0 * (rbint[indx1] - Gintv)
                        ...
                        if (Ginth < rbint[indx1]) {
                            if (2 * Ginth < rbint[indx1]) {
                                ...
                                float hwt = 2.0 * (rbint[indx1] - Ginth)
                        ...
                        rgbgreen[indx] =
                            Ginth * (1.f - hvwt[indx1]) + Gintv * hvwt[indx1];
                ...
                for (int rr = 13 - ey; rr < rr1 - 12; rr += 2)
                    for (int indx1 = (rr * ts + 13 - ex) >> 1;
                         indx1 < (rr * ts + cc1 - 12) >> 1; indx1++)
                    {                                     // B coset
...
#ifdef __SSE2__
                    for (int cc = 14 + (fc(cfarray, rr, 2) & 1), indx = rr * ts + cc,
                             c = 1 - fc(cfarray, rr, cc) / 2;
                         cc < cc1 - 14; cc += 8, indx += 8)
                    {
                        vfloat tempv = epsv
                                       + vabsf(LVFU(Dgrb[c][(indx - m1) >> 1])
                                               - LVFU(Dgrb[c][(indx + m1) >> 1]));
                        vfloat temp2v = epsv
                                        + vabsf(LVFU(Dgrb[c][(indx + p1) >> 1])
                                                - LVFU(Dgrb[c][(indx - p1) >> 1]));
                        vfloat wtnwv = onev
                                       / (tempv
                                          + vabsf(LVFU(Dgrb[c][(indx - m1) >> 1])
                                                  - LVFU(Dgrb[c][(indx - m3) >> 1]))
                                          + vabsf(LVFU(Dgrb[c][(indx + m1) >> 1])
                                                  - LVFU(Dgrb[c][(indx - m3) >> 1])));
                        vfloat wtnev = onev
                                       / (temp2v
                                          + vabsf(LVFU(Dgrb[c][(indx + p1) >> 1])
                                                  - LVFU(Dgrb[c][(indx + p3) >> 1]))
                                          + vabsf(LVFU(Dgrb[c][(indx - p1) >> 1])
                                                  - LVFU(Dgrb[c][(indx + p3) >> 1])));
                        vfloat wtswv = onev
                                       / (temp2v
                        ...
                        STVFU(Dgrb[c][indx >> 1],
                              (wtnwv
                                   * (oned325v * LVFU(Dgrb[c][(indx - m1) >> 1])
                                      - zd175v * LVFU(Dgrb[c][(indx - m3) >> 1])
                                      - zd075v
                                            * (LVFU(Dgrb[c][(indx - m1 - 2) >> 1])
                                               + LVFU(Dgrb[c][(indx - m1 - v2) >> 1])))
                               + wtnev
                                     * (oned325v * LVFU(Dgrb[c][(indx + p1) >> 1])
                                        - zd175v * LVFU(Dgrb[c][(indx + p3) >> 1])
                                        - zd075v
                                              * (LVFU(Dgrb[c][(indx + p1 + 2) >> 1])
                ...
                for (int rr = 16; rr < rr1 - 16; rr++) {
                    ...
                    for (; indx < rr * ts + cc1 - 18 - (cc1 & 1); indx += 4, col += 4) {
                        ...
                        STVFU(red[row][col],
                              vmaxf(c65535v * vself(selmask, redv1, redv2), ZEROV));
                        STVFU(blue[row][col],
                              vmaxf(c65535v * vself(selmask, bluev1, bluev2), ZEROV));
                    }
                    ...
                    if (offset == 0) {
                        for (; indx < rr * ts + cc1 - 16 - (cc1 & 1); indx++, col++) {
                            float temp =
                                1.f
                                / (hvwt[(indx - v1) >> 1] + 2.f - hvwt[(indx + 1) >> 1]
                            ...
                            col++;
                            red[row][col] = std::max(
                                0.f, 65535.f * (rgbgreen[indx] - Dgrb[0][indx >> 1]));
                            blue[row][col] = std::max(
                                0.f, 65535.f * (rgbgreen[indx] - Dgrb[1][indx >> 1]));
                        }
                ...
                for (int rr = 16; rr < rr1 - 16; rr++) {
                    ...
                    for (; cc < cc1 - 19; cc += 4) {
                        STVFU(green[row][cc + left],
                              vmaxf(LVF(rgbgreen[rr * ts + cc]) * c65535v, ZEROV));
                    }
                    ...
                    for (; cc < cc1 - 16; cc++) {
                        green[row][cc + left] =
                            std::max(0.f, 65535.f * rgbgreen[rr * ts + cc]);
                    }
                }
...
#pragma omp critical(amazeprogress)
                        ...
                        {
                            progress +=
                                (double)32 * ((ts - 32) * (ts - 32)) / (height * width);
