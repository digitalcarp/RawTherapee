...
public:
    ...
    void apply(Imagefloat* img,
               ...
               bool apply_hue_sat_map = true) const;
...
private:
    ...
    std::array<double, 2> neutralToXy(const Triple& neutral,
                                      int preferred_illuminant) const;
    Matrix makeXyzCam(const ColorTemp& white_balance,
                      const Triple& pre_mul,
                      const Matrix& cam_wb_matrix,
                      int preferred_illuminant) const;
...
private:
    ...
    // these contain standard profiles from RT. keys are all in uppercase, file path is
    // value
