...
public:
    ...
    /** @return false: not an HDR file ; true: single or multi-frame HDR file (e.g. Pentax
     * HDR raw file or 32 bit float DNG file or Log compressed) */
    ...
    /** Functions to convert between floating point and string representation of shutter
     * and aperture */
    ...
    /** Functions to convert between floating point and string representation of exposure
     * compensation */
     ...
     * @param rml is a struct containing information about metadata location of the first
     * frame. Use it only for raw files. In caseof jpgs and tiffs pass a NULL pointer.
     * @param firstFrameOnly must be true to get the MetaData of the first frame only,
     * e.g. for a PixelShift file.
     * @return The metadata */
...
public:
    ...
    /** This member function is called when the percentage of the progress has been
     * changed.
     * @param p is a number between 0 and 1 */
    virtual void setProgress(double p) = 0;
    /** This member function is called when a textual information corresponding to the
     ...
     * @param inProcessing =true if the processing has been started, =false if it has been
     * stopped */
...
/**
 * This class represents an image loaded into the memory. It is the basis of further
 * processing. The embedded icc profile and metadata information can be obtained through
 * this class, too.
 */
class InitialImage
{
...
public:
    ...
    /** Returns a class providing access to the exif and iptc metadata tags of all frames
     * of the image.
    ...
    /** This class has manual reference counting. You have to call this function each time
     * to make a new reference to an instance. */
    virtual void increaseRef() {}
    /** This class has manual reference counting. You have to call this function each time
     * to remove a reference (the last one deletes the instance automatically). */
     ...
     * @param errorCode is a pointer to a variable that is set to nonzero if an error
     * happened (output)
     * @param pl is a pointer pointing to an object implementing a progress listener. It
     * can be NULL, in this case progress is not reported.
     * @return an object representing the loaded and pre-processed image */
    static InitialImage* load(const Glib::ustring& fname,
                              bool isRaw,
                              int* errorCode,
                              ProgressListener* pl = nullptr);
};

/** When the preview image is ready for display during staged processing (thus the changes
 * have been updated), the staged processor notifies the listener class implementing a
 * PreviewImageListener. It is important to note that the file passed to the listener can
 * be used in a shared manner (copying it is not needed) as long as the mutex
 * corresponding to the image is used every time the image is accessed. If the scale of
 * the preview image is >1, no sharpening, no denoising and no cropping is applied, and
 * the transform operations (rotate, c/a, vignetting correction) are performed using a
 * faster less quality algorithm. The image you get with this listener is created to
 * display on the monitor (monitor profile has been already applied). */
class PreviewImageListener
{
public:
    virtual ~PreviewImageListener() = default;
    /** With this member function the staged processor notifies the listener that it
     * allocated a new image to store the end result of the processing. It can be used in
     * a shared manner.
     * @param img is a pointer to the image
     * @param scale describes the current scaling applied compared to the 100% size
     * (preview scale)
     * @param cp holds the coordinates of the current crop rectangle */
    virtual void
    setImage(IImage8* img, double scale, const procparams::CropParams& cp) = 0;
    /** With this member function the staged processor notifies the listener that the
     * image passed as parameter will be deleted, and no longer used to store the preview
     * image.
     * @param img the pointer to the image to be destroyed. The listener has to free the
     * image!  */
    virtual void delImage(IImage8* img) = 0;
    /** With this member function the staged processor notifies the listener that the
     * preview image has been updated.
     * @param cp holds the coordinates of the current crop rectangle */
    virtual void imageReady(const procparams::CropParams& cp) = 0;
};

/** When the detailed crop image is ready for display during staged processing (thus the
 * changes have been updated), the staged processor notifies the listener class
 * implementing a DetailedCropListener. It is important to note that the file passed to
 * the listener can <b>not</b> be used in a shared manner, the class implementing this
 * interface has to store a copy of it. */
class DetailedCropListener
{
public:
    virtual ~DetailedCropListener() = default;
    /** With this member function the staged processor notifies the listener that the
     * detailed crop image has been updated.
     * @param img is a pointer to the detailed crop image */
    virtual void setDetailedCrop(IImage8* img,
                                 IImage8* imgtrue,
...
/** This listener is used when the full size of the final image has been changed (e.g.
 * rotated by 90 deg.) */
class SizeListener
{
public:
...
public:
     ...
     * @param histGreen is the array of size 256 containing the histogram of the green
     * channel
     * @param histBlue is the array of size 256 containing the histogram of the blue
     * channel
     * @param histLuma is the array of size 256 containing the histogram of the luminance
     * channel other for curves backgrounds, histRAW is RAW without colors */
    virtual void histogramChanged(const LUTu& histRed,
                                  ...
                                  const array2D<int>& waveformLuma) = 0;
...
/** This listener is used when the auto exposure has been recomputed (e.g. when the
 * clipping ratio changed). */
...
public:
    ...
    virtual void autoExpChanged(double brightness,
                                int bright,
                                int contrast,
                                int black,
                                int hlcompr,
    ...
    virtual void autoMatchedToneCurveChanged(procparams::ToneCurveMode curveMode,
                                             const std::vector<double>& curve) = 0;
};
...
public:
    ...
    virtual void
    noiseTilePrev(int tileX, int tileY, int prevX, int prevY, int sizeT, int sizeP) = 0;
};
...
public:
    ...
    virtual void achromaticChanged(double acmax,
                                   double acmax0,
                                   double acmax1,
                                   double acmax2,
                                   bool auto_dc,
...
public:
    ...
    virtual void minmaxChanged(double cdma,
                               double cdmin,
                               double mini,
                               double maxi,
                               double Tmean,
...
public:
    ...
    struct locallabshGHSbw2
    {  // update sliders black and white point
        double ghsbw_slider[2];
        bool ghs_auto;
    };

    struct locallabshGHSbw
    {  // infos black and white point GHS and update auto SP
        int ghsbw[2];
        double ghsbwvalue[2];
        double ghs_sym;
        double ghs_color[4];
    ...
    virtual void minmaxChanged(const std::vector<locallabRetiMinMax>& minmax,
                               int selspot) = 0;
    virtual void denChanged(const std::vector<locallabDenoiseLC>& denlc, int selspot) = 0;
    virtual void madChanged(const std::vector<locallabDenoiseMAD>& madlc,
                            int selspot) = 0;
    virtual void den2Changed(const std::vector<locallabDenoiseLC2>& den2lc,
                             int selspot) = 0;
    virtual void cieChanged(const std::vector<locallabcieLC>& cielc, int selspot) = 0;
    virtual void ghsbwChanged(const std::vector<locallabshGHSbw>& shghsbw,
                              int selspot) = 0;
    virtual void ghsbw2Changed(const std::vector<locallabshGHSbw2>& shghsbw2,
                               int selspot) = 0;
    ...
    virtual void ciebefChanged(const std::vector<locallabcieBEF>& ciebef,
                               int selspot) = 0;

    virtual void sharbefChanged(const std::vector<locallabsharBEF>& sharbef,
                                int selspot) = 0;
    virtual void sharaftChanged(const std::vector<locallabsharAFT>& sharaft,
                                int selspot) = 0;

    virtual void refChanged2(float* huerefp,
                             float* chromarefp,
                             float* lumarefp,
                             float* fabrefp,
                             int selspot) = 0;
    //    virtual void mainChanged(int spottype, int selspot, bool iscolor, bool issh,
    //    bool isvib, bool isexpos, bool issoft, bool isblur, bool istom, bool isret, bool
    //    issharp, bool iscont, bool iscbdl, bool islog, bool ismas, bool iscie) = 0;
...
public:
    ...
    virtual void
    primChanged(float rx, float ry, float bx, float by, float gx, float gy) = 0;
    virtual void iprimChanged(float r_x,
                              float r_y,
                              float b_x,
                              float b_y,
                              float g_x,
...
public:
    ...
    virtual void WBChanged(int met,
                           double temp,
                           double green,
                           double rw,
                           double gw,
...
public:
    ...
    virtual void imageTypeChanged(bool isRaw,
                                  bool isBayer,
                                  bool isXtrans,
                                  bool is_Mono = false,
                                  bool isGainMapSupported = false) = 0;
};
...
public:
    ...
    virtual void
    filmRefValuesChanged(const procparams::FilmNegativeParams::RGB& refInput,
                         const procparams::FilmNegativeParams::RGB& refOutput) = 0;
};
...
public:
     ...
     * @param tOperator is a pointer to the object that will alter the ProcParams for the
     * rendering */
    virtual void setTweakOperator(TweakOperator* tOperator) = 0;
    /** Unset the TweakOperator
     * @param tOperator is a pointer to the object that were altering the ProcParams for
     * the rendering It will only unset the tweak operator if tOperator is the same than
     * the currently set operator. If it doesn't match, the currently set TweakOperator
     * will remain set. */
    virtual void unsetTweakOperator(TweakOperator* tOperator) = 0;
    /** Returns the current processing parameters.
     * Since the ProcParams can be tweaked by a GUI to operate on the image at a specific
     * stage or with disabled tool, you'll have to specify if you want the tweaked version
     * for the current special mode, or the untweaked one.
     * @param dst is the location where the image processing parameters are copied (it is
     * assumed that the memory is allocated by the caller)
     * @param tweaked is used to choose between the tweaked ProcParams (if there is one)
     * or the untweaked one */
    virtual void getParams(procparams::ProcParams* dst, bool tweaked = false) = 0;
    /** An essential member function. Call this when a setting has been changed. This
     * function returns a pointer to the processing parameters, that you have to update to
     * reflect the changed situation. When ready, call the paramsUpdateReady function to
     * start the image update.
     * @param change is the ID of the changed setting */
    virtual procparams::ProcParams* beginUpdateParams() = 0;
    /** An essential member function. This indicates that you are ready with the update of
     * the processing parameters you got with the beginUpdateParams call, so the image can
     * be updated. This function returns immediately. The image update starts immediately
     * in the background. If it is ready, the result is passed to a PreviewImageListener
    ...
    /** Sets the scale of the preview image. The larger the number is, the faster the
     * image updates are (typical values are 4-5).
    ...
    virtual void setLocallabMaskVisibility(bool previewDeltaE,
                                           int locallColorMask,
     ...
     * @param editDataProvider pointer to the EditDataProvider that communicates with the
     * EditSubscriber
     * @return a pointer to the Crop object that handles the image data trough its own
     * pipeline */
    virtual DetailedCrop* createCrop(::EditDataProvider* editDataProvider,
                                     bool isDetailWindow) = 0;

    virtual bool getAutoWB(double& temp,
                           double& green,
                           double equal,
    ...
    virtual bool getFilmNegativeSpot(int x,
                                     int y,
                                     int spotSize,
                                     procparams::FilmNegativeParams::RGB& refInput,
                                     procparams::FilmNegativeParams::RGB& refOutput) = 0;
    ...
    virtual void setMonitorProfile(const Glib::ustring& monitorProfile,
                                   RenderingIntent intent) = 0;
    virtual void getMonitorProfile(Glib::ustring& monitorProfile,
                                   RenderingIntent& intent) const = 0;
 ...
 * @param loadAll if false, don't load the various dependencies (profiles, HALDClut files,
 * ...), they'll be loaded from disk each time they'll be used (launching time
 * improvement) */
...
/** This class  holds all the necessary information to accomplish the full processing of
 * the image */
class ProcessingJob
{
...
public:
    ...
    /** Creates a processing job from a file name. This function always succeeds. It only
     * stores the data into the ProcessingJob class, it does not load the image thus it
     * returns immediately.
     * @param fname the name of the file
     * @param isRaw shall be true if it is a raw file
     * @param pparams is a struct containing the processing parameters
     * @return an object containing the data above. It can be passed to the functions that
     * do the actual image processing. */
    static ProcessingJob* create(const Glib::ustring& fname,
                                 bool isRaw,
    ...
    /** Creates a processing job from a file name. This function always succeeds. It only
     * stores the data into the ProcessingJob class, it does not load the image thus it
     * returns immediately. This function increases the reference count of the
     * initialImage. If you decide not the process the image you have to cancel it by
     * calling the member function void cancel(). If the image is processed the reference
     * count of initialImage is decreased automatically, thus the ProcessingJob instance
     * gets invalid. You can not use a ProcessingJob instance to process an image twice.
     * @param initialImage is a loaded and pre-processed initial image
     * @param pparams is a struct containing the processing parameters
     * @return an object containing the data above. It can be passed to the functions that
     * do the actual image processing. */
    static ProcessingJob* create(InitialImage* initialImage,
                                 const procparams::ProcParams& pparams,
                                 bool fast = false);

    /** Cancels and destroys a processing job. The reference count of the corresponding
     * initialImage (if any) is decreased. After the call of this function the
     * ProcessingJob instance gets invalid, you must not use it any more. Don't call this
     * function while the job is being processed.
...
/** This function performs all the image processing steps corresponding to the given
 * ProcessingJob. It returns when it is ready, so it can be slow. The ProcessingJob passed
 * becomes invalid, you can not use it any more.
 * @param job the ProcessingJob to cancel.
 * @param errorCode is the error code if an error occurred (e.g. the input image could not
 * be loaded etc.)
 * @param pl is an optional ProgressListener if you want to keep track of the progress
 * @return the resulting image, with the output profile applied, exif and iptc data set.
 * You have to save it or you can access the pixel data directly.  */
IImagefloat* processImage(ProcessingJob* job,
                          int& errorCode,
                          ProgressListener* pl = nullptr,
                          bool flush = false);

/** This class is used to control the batch processing. The class implementing this
 * interface will be called when the full processing of an image is ready and the next job
 * to process is needed. */
class BatchProcessingListener : public ProgressListener
{
public:
    /** This function is called when an image gets ready during the batch processing. It
     * has to return with the next job, or with NULL if there is no jobs left.
...
/** This function performs all the image processing steps corresponding to the given
 *ProcessingJob. It runs in the background, thus it returns immediately, When it finishes,
 *it calls the BatchProcessingListener with the resulting image and asks for the next job.
 *It the listener gives a new job, it goes on with processing. If no new job is given, it
 *finishes. The ProcessingJob passed becomes invalid, you can not use it any more.
 * @param job the ProcessingJob to cancel.
 * @param bpl is the BatchProcessingListener that is called when the image is ready or the
