...
namespace {
void fillCurveArray(const rtengine::DiagonalCurve* diagCurve,
                    LUTf& outCurve,
                    int skip,
                    bool needed)
{
        ...
        if (skip > 1) {
            ...
            for (int i = 0; i <= 0x10000 - skip; i += skip) {
                for (int j = 1; j < skip; j++) {
                    outCurve[i + j] =
                        (outCurve[i] * (skip - j) + outCurve[i + skip] * j) * skipmul;
                }
            }
        }
...
bool sanitizeCurve(std::vector<double>& curve)
{
    ...
    // 3) curve[0] == DCT_Parametric and curve size is >= 8 and curve[1] .. curve[3] are
    // ordered ascending and are distinct
    if (curve.empty()) {
 ...
 * This method return the number of control points of a curve. Not suitable for parametric
 * curves.
 * @return number of control points of the curve. 0 will be sent back for Parametric
 * curves
 */
...
// Wikipedia sRGB: Unlike most other RGB color spaces, the sRGB gamma cannot be expressed
// as a single numerical value. The overall gamma is approximately 2.2, consisting of a
// linear (gamma 1.0) section near black, and a non-linear section elsewhere involving
// a 2.4 exponent and a gamma (slope of log output versus log input) changing from 1.0
// through about 2.3.
const double CurveFactory::sRGBGamma = 2.2;
const double CurveFactory::sRGBGammaCurve = 2.4;

void CurveFactory::curveLightBrightColor(const std::vector<double>& curvePoints1,
                                         const std::vector<double>& curvePoints2,
                                         const std::vector<double>& curvePoints3,
                                         const LUTu& histogram,
                                         LUTu& outBeforeCCurveHistogram,  // for Luminance
                                         const LUTu& histogramC,
                                         LUTu& outBeforeCCurveHistogramC,  // for chroma
                                         ...
                                         int skip)
{
    ...
    if (!curvePoints3.empty() && curvePoints3[0] > DCT_Linear
        && curvePoints3[0] < DCT_Unchanged)
    {
    ...
    if (!curvePoints2.empty() && curvePoints2[0] > DCT_Linear
        && curvePoints2[0] < DCT_Unchanged)
    {
    ...
    if (!curvePoints1.empty() && curvePoints1[0] > DCT_Linear
        && curvePoints1[0] < DCT_Unchanged)
    {
...
void CurveFactory::curveBW(const std::vector<double>& curvePointsbw,
                           const std::vector<double>& curvePointsbw2,
                           const LUTu& histogrambw,
                           LUTu& outBeforeCCurveHistogrambw,  // for Luminance
                           ...
                           int skip)
{
    ...
    if (!curvePointsbw2.empty() && curvePointsbw2[0] > DCT_Linear
        && curvePointsbw2[0] < DCT_Unchanged)
    {
    ...
    if (!curvePointsbw.empty() && curvePointsbw[0] > DCT_Linear
        && curvePointsbw[0] < DCT_Unchanged)
    {
...
bool CurveFactory::diagonalCurve2Lut(const std::vector<double>& curvePoints,
                                     LUTf& curve,
                                     int skip,
                                     const LUTu& histogram,
                                     LUTu& outBeforeCurveHistogram)
{
    bool needed = false;
...
void CurveFactory::complexsgnCurve(bool& autili,
                                   bool& butili,
                                   bool& ccutili,
                                   bool& cclutili,
                                   const std::vector<double>& acurvePoints,
                                   const std::vector<double>& bcurvePoints,
                                   const std::vector<double>& cccurvePoints,
                                   const std::vector<double>& lccurvePoints,
                                   LUTf& aoutCurve,
                                   LUTf& boutCurve,
                                   LUTf& satCurve,
                                   LUTf& lhskCurve,
...
void CurveFactory::complexCurve(double ecomp,
                                double black,
                                double hlcompr,
                                double hlcomprthresh,
                                double shcompr,
                                double br,
                                double contr,
                                ...
                                int skip)
{
    ...
    // the curve shapes are defined in sRGB gamma, but the output curves will operate on
    // linear floating point data, hence we do both forward and inverse gamma conversions
    // here.
    const float gamma_ = Color::sRGBGammaCurve;
    ...
    // tone curve base. a: slope (from exp.comp.), b: black, def_mul: max. x value (can
    // be>1), hr,sr: highlight,shadow recovery
    ...
    if (br > 0.00001 || br < -0.00001) {
        ...
        brightcurve.reset(
            new DiagonalCurve(brightcurvePoints, CURVES_MIN_POLY_POINTS / skip));
    }

    hlCurve.setClip(
        LUT_CLIP_BELOW);  // used LUT_CLIP_BELOW, because we want to have a baseline of
                          // 2^expcomp in this curve. If we don't clip the lut we get
                          // wrong values, see Issue 2621 #14 for details
    float exp_scale = a;
    float scale = 65536.0;
    ...
    if (comp <= 0.0f) {
        ...
        if (i & 1) {  // original formula, slower than optimized formulas below but only
                      // used once or none, so I let it as is for reference
            // change to [0,1] range
            float val = (float)i - shoulder;
            float R = val * comp / (scalemshoulder);
            hlCurve[i] =
                xlog(1.0 + R * exp_scale) / R;  // don't use xlogf or 1.f here. Leads to
                                                // errors caused by too low precision
            i++;
        }
    ...
    // change to [0,1] range
    shCurve.setClip(
        LUT_CLIP_ABOVE);  // used LUT_CLIP_ABOVE, because the curve converges to 1.0 at
                          // the upper end and we don't want to exceed this value.
    if (black == 0.0) {
    ...
    if (brightcurve) {
        val0 = brightcurve->getVal(
            val0);  // TODO: getVal(double) is very slow! Optimize with a LUTf
    }
    ...
    for (int i = 1; i < 0x10000; i++) {
        ...
        // apply brightness curve
        if (brightcurve) {
            val = LIM01<float>(brightcurve->getVal(
                val));  // TODO: getVal(double) is very slow! Optimize with a LUTf
        }
    ...
    if (contr > 0.00001 || contr < -0.00001) {
        ...
        contrastcurvePoints[5] =
            avg + (1 - avg) * (0.6 - contr / 250.0);  // shoulder point
        contrastcurvePoints[6] =
            avg + (1 - avg) * (0.6 + contr / 250.0);  // value at shoulder point

        contrastcurvePoints[7] = 1.;  // white point
        contrastcurvePoints[8] = 1.;  // value at white point

        const DiagonalCurve contrastcurve(contrastcurvePoints,
                                          CURVES_MIN_POLY_POINTS / skip);
    ...
    if (!curvePoints.empty() && curvePoints[0] > DCT_Linear
        && curvePoints[0] < DCT_Unchanged)
    {
...
void CurveFactory::Curvelocalhl(double ecomp,
                                double hlcompr,
                                double hlcomprthresh,
                                LUTf& hlCurve)
{
    ...
    if (comp <= 0.0f) {
        ...
        if (i & 1) {  // original formula, slower than optimized formulas below but only
            ...
            float R = val * comp / (scalemshoulder);
            hlCurve[i] =
                xlog(1.0f + R * exp_scale) / R;  // don't use xlogf or 1.f here. Leads to
                                                 // errors caused by too low precision
            i++;
        }
...
void CurveFactory::complexCurvelocal(double ecomp,
                                     double black,
                                     double hlcompr,
                                     double hlcomprthresh,
                                     double shcompr,
                                     ...
                                     int skip)
{
    ...
    const float start =
        expf(gamma_ * logf(-0.0954f / ((1.0f / gamma_ - 1.0f) * 1.0954f)));
    ...
    if (br > 0.00001 || br < -0.00001) {
        ...
        if (br > 0) {
            ...
            brightcurvePoints.at(6) =
                min(1.0, 0.6 + br / 200.0);  // value at shoulder point
        } else {
        ...
        {  // L values range up to 32767, higher values are for highlight overflow
    ...
    if (cont > 0.00001 || cont < -0.00001) {
        ...
        contrastcurvePoints.at(4) =
            avg - avg * (0.6 + cont / 250.0);  // value at toe point

        contrastcurvePoints.at(5) =
            avg + (1 - avg) * (0.6 - cont / 250.0);  // shoulder point
        contrastcurvePoints.at(6) =
            avg + (1 - avg) * (0.6 + cont / 250.0);  // value at shoulder point
    ...
    {  // set last two elements of lut to 32768 and 32769 to allow linear interpolation
...
void CurveFactory::complexLCurve(double br,
                                 double contr,
                                 const std::vector<double>& curvePoints,
                                 const LUTu& histogram,
                                 LUTf& outCurve,
                                 ...
                                 bool& utili)
{
    ...
    if (br > 0.00001 || br < -0.00001) {
        ...
        if (br > 0) {
            ...
            brightcurvePoints.at(6) =
                min(1.0, 0.7 + br / 300.0);  // value at shoulder point
        } else {
            brightcurvePoints.at(3) = 0.1 - br / 150.0;  // toe point
            brightcurvePoints.at(4) = 0.1;               // value at toe point
    ...
    if (contr > 0.00001 || contr < -0.00001) {
        ...
        if (sum) {
            ...
            contrastcurvePoints.at(4) =
                avg - avg * (0.6 + contr / 250.0);  // value at toe point
            ...
            // sum has an invalid value (next to 0, producing a division by zero, so we
            // create a fake contrast curve, producing a white image
...
void CurveFactory::RGBCurve(const std::vector<double>& curvePoints,
                            LUTf& outCurve,
                            int skip)
{
...
void LocretigainCurverab::Set(const Curve& pCurve)
{
    ...
    lutLocretigainCurverab(
        501);  // raise this value if the quality suffers from this number of samples
    sum = 0.f;
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocretigainCurverab[i] < 0.02f) {
            lutLocretigainCurverab[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f
...
{
    //  if (HHutili && !curvePoints.empty() && curvePoints[0] > FCT_Linear &&
    //  curvePoints[0] < FCT_Unchanged) {
    if (!curvePoints.empty() && curvePoints[0] > FCT_Linear
        && curvePoints[0] < FCT_Unchanged)
    {
...
void LocLLmaskblCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocLLmaskblCurve[i] < 0.02f) {
            lutLocLLmaskblCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi
                                             // no action for each value
        }
...
void LocCCmaskblCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocCCmaskblCurve[i] < 0.02f) {
            lutLocCCmaskblCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi
                                             // no action for each value
        }
...
void LocLLmasktmCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocLLmasktmCurve[i] < 0.02f) {
            lutLocLLmasktmCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi
                                             // no action for each value
        }
...
void LocCCmasktmCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocCCmasktmCurve[i] < 0.02f) {
            lutLocCCmasktmCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi
                                             // no action for each value
        }
...
void LocLLmaskretiCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocLLmaskretiCurve[i] < 0.02f) {
            lutLocLLmaskretiCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f
                                               // quasi no action for each value
        }
...
void LocCCmaskretiCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocCCmaskretiCurve[i] < 0.02f) {
            lutLocCCmaskretiCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f
                                               // quasi no action for each value
        }
...
void LocLLmaskcbCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocLLmaskcbCurve[i] < 0.02f) {
            lutLocLLmaskcbCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi
                                             // no action for each value
        }
...
void LocCCmaskcbCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocCCmaskcbCurve[i] < 0.02f) {
            lutLocCCmaskcbCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi
                                             // no action for each value
        }
...
void LocLLmaskSHCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocLLmaskSHCurve[i] < 0.02f) {
            lutLocLLmaskSHCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi
                                             // no action for each value
        }
...
void LocCCmaskSHCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocCCmaskSHCurve[i] < 0.02f) {
            lutLocCCmaskSHCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi
                                             // no action for each value
        }
...
void LocLLmaskexpCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocLLmaskexpCurve[i] < 0.02f) {
            lutLocLLmaskexpCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi
                                              // no action for each value
        }
...
void LocCCmaskexpCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocCCmaskexpCurve[i] < 0.02f) {
            lutLocCCmaskexpCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi
                                              // no action for each value
        }
...
void LocCCmaskCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocCCmaskCurve[i] < 0.02f) {
            lutLocCCmaskCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi no
...
void LocLLmaskCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocLLmaskCurve[i] < 0.02f) {
            lutLocLLmaskCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi no
                                           // action for each value
        }
...
void LocHHCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocHHCurve[i] < 0.02f) {
            lutLocHHCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi no
                                       // action for each value
        }
...
void LocLHCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocLHCurve[i] < 0.02f) {
            lutLocLHCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi no
                                       // action for each value
        }
...
bool LocLHCurve::Set(const std::vector<double>& curvePoints)
{
    ...
    if (!curvePoints.empty() && curvePoints[0] > FCT_Linear
        && curvePoints[0] < FCT_Unchanged)
    {
        //    if (LHutili && !curvePoints.empty() && curvePoints[0] > FCT_Linear &&
        //    curvePoints[0] < FCT_Unchanged) {
        FlatCurve tcurve(curvePoints, false, CURVES_MIN_POLY_POINTS / 2);
        tcurve.setIdentityValue(0.);
        Set(tcurve);
...
void LocCHCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocCHCurve[i] < 0.02f) {
            lutLocCHCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi no
                                       // action for each value
        }
...
void LocwavCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocwavCurve[i] < 0.02f) {
            lutLocwavCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi no
                                        // action for each value
        }
...
void LocretitransCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocretitransCurve[i] < 0.02f) {
            lutLocretitransCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi
                                              // no action for each value
        }
...
void LocretigainCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutLocretigainCurve[i] < 0.02f) {
            lutLocretigainCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi
                                             // no action for each value
        }
...
{
    if (pCurve.isIdentity()) {
        luttransmission.reset();  // raise this value if the quality suffers from this
                                  // number of samples
        return;
    }
...
{
    if (pCurve.isIdentity()) {
        lutgaintransmission.reset();  // raise this value if the quality suffers from this
                                      // number of samples
        return;
    }
...
void ToneCurve::Set(const Curve& pCurve, float gamma)
{
    ...
    if (gamma <= 0.0 || gamma == 1.) {
        ...
        // apply gamma, that is 'pCurve' is defined with the given gamma and here we
        // convert it to a curve in linear space
        for (int i = 0; i < 65536; i++) {
...
{
    if (pCurve->isIdentity()) {
        lutOpacityCurve.reset();  // raise this value if the quality suffers from this
                                  // number of samples
        return;
    }
...
void WavCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutWavCurve[i] < 0.02f) {
            lutWavCurve[i] = 0.02f;  // avoid 0.f for wavelet : under 0.01f quasi no
                                     // action for each value
        }
...
{
    if (pCurve.isIdentity()) {
        lutOpacityCurveBY.reset();  // raise this value if the quality suffers from this
                                    // number of samples
        return;
    }
...
{
    if (pCurve.isIdentity()) {
        lutOpacityCurveW.reset();  // raise this value if the quality suffers from this
                                   // number of samples
        return;
    }
...
{
    if (pCurve.isIdentity()) {
        lutOpacityCurveWL.reset();  // raise this value if the quality suffers from this
                                    // number of samples
        return;
    }
...
void NoiseCurve::Set(const Curve& pCurve)
{
    ...
    for (int i = 0; i < 501; i++) {
        ...
        if (lutNoiseCurve[i] < 0.01f) {
            lutNoiseCurve[i] = 0.01f;  // avoid 0.f for wavelet : under 0.01f quasi no
                                       // action for each value
        }
...
void ColorGradientCurve::SetXYZ(const Curve* pCurve,
                                ...
                                float lumin)
{
    ...
    double dY = 0.;
    low = nextX;
    lr1 = (0.5f + low) / 2.f;  // optimize use of gamut in low light..one can optimize
                               // more using directly low ?
    // lr1=low;
    ...
    for (int i = 0; i <= upperBound; ++i) {
        ...
        if (x > nextX) {
            ...
            if (ptNum < nPoints) {
                ...
                dY = nextY - prevY;
                high = nextX;
                lr2 = (0.5f + high) / 2.f;  // optimize use of gamut in high light..one
                                            // can optimize more using directly high ?
                // lr2=high;
            }
        }
        ...
        if (!ptNum) {
            ...
            if (dY > 0.000001 || dY < -0.000001) {
                ...
                // I use XYZ to mix color 1 and 2 rather than rgb (gamut) and rather than
                // Lab artifacts
...
void ColorGradientCurve::SetXYZ(const std::vector<double>& curvePoints,
                                const double xyz_rgb[3][3],
                                float satur,
                                float lumin)
{
    ...
    for (int i = 0; i <= upperBound; ++i) {
        ...
        if (x > nextX) {
            ...
            if (ptNum < nPoints) {
                ...
                dY = nextY - prevY;
                dir =
                    Color::getHueInterpolationDirection(prevY, nextY, Color::IP_SHORTEST);
            }
        }
        ...
        if (!ptNum) {
            ...
            if (dY > 0.0000001 || dY < -0.0000001) {
                ...
                Color::interpolateRGBColor(currY / dY, r1, g1, b1, r2, g2, b2,
                                           Color::CHANNEL_LIGHTNESS
                                               | Color::CHANNEL_CHROMATICITY
                                               | Color::CHANNEL_HUE,
                                           xyz_rgb, rgb_xyz, ro, go, bo);
#endif
                lut1[i] = ro;
                lut2[i] = go;
                lut3[i] = bo;
...
// this is a generic cubic spline implementation, to clean up we could probably use
// something already existing elsewhere
void PerceptualToneCurve::cubic_spline(const float x[],
                                       const float y[],
                                       const int len,
                                       const float out_x[],
                                       float out_y[],
    ...
    for (i = 0; i < out_len; i++) {
        ...
        for (j = 0; j < len - 1; j++) {
            if (x[j] <= x_out && x_out <= x[j + 1]) {
                ...
                y_out =
                    y[j]
                    + ((y[j + 1] - y[j]) / d[j] - (2 * d[j] * c[j] + c[j + 1] * d[j]) / 6)
                          * v
...
// generic function for finding minimum of f(x) in the a-b range using the interval
// halving method
float PerceptualToneCurve::find_minimum_interval_halving(float (*func)(float x,
                                                                       void* arg),
                                                         void* arg,
                                                         float a,
                                                         float b,
...
float PerceptualToneCurve::find_tc_slope_fun(float k, void* arg)
{
    ...
    for (int i = 0; i < steps; i++) {
        float x =
            0.1 + ((float)i / (steps - 1)) * 0.5;  // testing (sRGB) range [0.1 - 0.6], ie
                                                   // ignore highligths and dark shadows
        float y = CurveFactory::gamma2(
            a->tc->lutToneCurve[CurveFactory::igamma2(x) * 65535] / 65535.0);
        float y1 = k * x;
        ...
        areasum += (y - y1) * (y - y1);  // square is a rough approx of (twice) the area,
...
float PerceptualToneCurve::get_curve_val(float x,
                                         float range[2],
                                         float lut[],
                                         size_t lut_size)
{
...
float PerceptualToneCurve::calculateToneCurveContrastValue() const
{
    ...
    // Note: the analysis is made on the gamma encoded curve, as the LUT is linear we make
    // backwards gamma to
    struct find_tc_slope_fun_arg arg = { this };
    float k = find_minimum_interval_halving(find_tc_slope_fun, &arg, 0.1, 5.0, 0.01,
    ...
    float maxslope = 0;
    {
        ...
        const float xd = 0.07;
        const float tx0[] = { 0.30, 0.35, 0.40,
                              0.45 };  // we only look in the midtone range

        for (size_t i = 0; i < sizeof(tx0) / sizeof(tx0[0]); i++) {
            float x0 = tx0[i] - xd;
            float y0 = CurveFactory::gamma2(
                           lutToneCurve[CurveFactory::igamma2(x0) * 65535.f] / 65535.f)
                       - k * x0;
            float x1 = tx0[i] + xd;
            float y1 = CurveFactory::gamma2(
                           lutToneCurve[CurveFactory::igamma2(x1) * 65535.f] / 65535.f)
                       - k * x1;
        ...
        float e_maxslope = 0;
        {
            ...
            for (size_t i = 0; i < sizeof(tx) / sizeof(tx[0]); i++) {
                float x0 = tx[i] - xd;
                float y0 =
                    CurveFactory::gamma2(lutToneCurve[CurveFactory::igamma2(x0) * 65535.f]
                                         / 65535.f)
                    - k * x0;
                float x1 = tx[i] + xd;
                float y1 =
                    CurveFactory::gamma2(lutToneCurve[CurveFactory::igamma2(x1) * 65535.f]
                                         / 65535.f)
                    - k * x1;
                float slope = 1.0 + (y1 - y0) / (x1 - x0);
        ...
        //  midtone slope is more important for contrast, but weigh in some slope from
        //  brights and darks too.
        maxslope = maxslope * 0.7 + e_maxslope * 0.3;
    }
    return maxslope;
}

void PerceptualToneCurve::BatchApply(const size_t start,
                                     const size_t end,
                                     float* rc,
                                     float* gc,
                                     float* bc,
                                     const PerceptualToneCurveState& state) const
{
    ...
    for (size_t i = start; i < end; ++i) {
        ...
        if (!state.isProphoto) {
            // convert to prophoto space to make sure the same result is had regardless of
            // working color space
        ...
        // we use the Adobe (RGB-HSV hue-stabilized) curve to decide luminance, which
        // generally leads to a less contrasty result compared to a pure luminance curve.
        // We do this to be more compatible with the most popular curves.
        ...
        // move to JCh so we can modulate chroma based on the global contrast-related
        ...
        float plum = 100.f;
        Ciecam02::xyz2jch_ciecam02float(J, C, h, aw, fl, x * 0.0015259022f,
                                        y * 0.0015259022f, z * 0.0015259022f, xw, yw, zw,
                                        c, nc, pow1, nbb, ncb, cz, d, c16, plum);

        if (!isfinite(J) || !isfinite(C) || !isfinite(h)) {
            // this can happen for dark noise colours or colours outside human gamut. Then
            // we just return the curve's result.
            if (!state.isProphoto) {
...

        {
            ...
            constexpr float lolim = 35.f;  // lower limit, below this chroma all colors
                                           // will keep original chroma scaling factor
            constexpr float hilim =
                60.f;  // high limit, above this chroma the chroma scaling factor is
                       // multiplied with the saturated scale factor value above
            ...
            if (C < lolim) {
                ...
                float cx =
                    (C - lolim) / (hilim - lolim);  // x = [0..1], 0 at lolim, 1 at hilim
...

        {
            ...
            float nL =
                Color::gamma2curve[newLuminance];  // apply gamma so we make comparison
                                                   // and transition with a more
            ...
            if (nL < lolim) {
                ...
                // S-curve transition
                float cx =
                    (nL - lolim) / (hilim - lolim);  // x = [0..1], 0 at lolim, 1 at hilim
        ...
        {
            // to avoid strange CIECAM02 chroma errors on close-to-shadow-clipping colors
            // we reduce chroma scaling towards 1.0 for black colors
        ...
        C *= cmul;
        Ciecam02::jch2xyz_ciecam02float(x, y, z, J, C, h, xw, yw, zw, c, nc, pow1, nbb,
                                        ncb, fl, cz, d, aw, c16, plum);

        if (!isfinite(x) || !isfinite(y) || !isfinite(z)) {
            // can happen for colours on the rim of being outside gamut, that worked
            // without chroma scaling but not with. Then we return only the curve's
            // result.
            if (!state.isProphoto) {
                float newr = state.Prophoto2Working[0][0] * r
                             + state.Prophoto2Working[0][1] * g
        ...
        {
            // limit saturation increase in rgb space to avoid severe clipping and
            // flattening in extreme highlights

            // we use the RGB-HSV hue-stable "Adobe" curve as reference. For S-curve
            // contrast it increases saturation greatly, but desaturates extreme
            // highlights and thus provide a smooth transition to the white point. However
            // the desaturation effect is quite strong so we make a weighting
            const float as = Color::rgb2s(ar, ag, ab);
            const float s = Color::rgb2s(r, g, b);

            const float sat_scale =
                as <= 0.f ? 1.f : s / as;  // saturation scale compared to Adobe curve
            float keep = 0.2f;
            constexpr float lolim = 1.00f;  // only mix in the Adobe curve if we have
                                            // increased saturation compared to it
...
void PerceptualToneCurve::init()
{
    ...
    float plum = 100.f;
    Ciecam02::initcam1float(yb, 1.f, f, la, xw, yw, zw, n, d, nbb, ncb, cz, aw, wh, pfl,
                            fl, c, c16, plum);
...

    {
        ...
        // contrast value in the left column, chroma scaling in the right. Handles for a
        // spline. Put the columns in a file (without commas) and you can plot the spline
        // with gnuplot: "plot 'curve.txt' smooth csplines" A spline can easily get
        // overshoot issues so if you fine-tune the values here make sure that the
        // resulting spline is smooth afterwards, by plotting it for example.
        const float p[] = {
            0.60, 0.70,                                      // lowest contrast
            0.70, 0.80, 0.90, 0.94, 0.99, 1.00, 1.00, 1.00,  // 1.0 (linear curve) to 1.0,
                                                             // no scaling
...
void PerceptualToneCurve::initApplyState(PerceptualToneCurveState& state,
                                         const Glib::ustring& workingSpace) const
{
