...
float tileVariance(const float* const* data,
                   size_t tileY,
                   size_t tileX,
                   size_t tilesize,
                   float avg)
...
float calcContrastThreshold2(float** luminance,
                             int tileY,
                             int tileX,
                             int tilesize,
                             float factor)
{
    ...
    for (int j = tileY + 2; j < tileY + tilesize - 2; ++j) {
        ...
        for (; i < tileX + tilesize - 5; i += 4) {
            vfloat contrastv =
                vsqrtf(SQRV(LVFU(luminance[j][i + 1]) - LVFU(luminance[j][i - 1]))
                       + SQRV(LVFU(luminance[j + 1][i]) - LVFU(luminance[j - 1][i]))
                       + SQRV(LVFU(luminance[j][i + 2]) - LVFU(luminance[j][i - 2]))
                       + SQRV(LVFU(luminance[j + 2][i]) - LVFU(luminance[j - 2][i])))
                * scalev;
        ...
        for (; i < tileX + tilesize - 2; ++i) {
            ...
            float contrast =
                sqrtf(rtengine::SQR(luminance[j][i + 1] - luminance[j][i - 1])
...
float calcContrastThreshold(const float* const* luminance,
                            int tileY,
                            int tileX,
                            int tilesize)
{
...
void findMinMaxPercentile(const float* data,
                          size_t size,
                          float minPrct,
                          float& minOut,
                          float maxPrct,
                          ...
                          bool multithread)
{
    ...
    // We need to find the (minPrct*size) smallest value and the (maxPrct*size) smallest
    // value in data. We use a histogram based search for speed and to reduce memory
    // usage. Memory usage of this method is histoSize * sizeof(uint32_t) * (t + 1) byte,
    ...
    // The current implementation is not guaranteed to work correctly if size > 2^32
    // (4294967296).
...
#pragma omp parallel for reduction(min : minVal) reduction(max : maxVal) \
    ...
    {  // fast exit, also avoids division by zero in calculation of scale factor
        minOut = maxOut = minVal;
        return;
    }

    // Caution: Currently this works correctly only for histoSize in range[1;65536].
    // For small data size (i.e. thumbnails) we reduce the size of the histogram to the
    // size of data.
    ...
    if (numThreads == 1) {
        ...
        for (size_t i = 0; i < size; ++i) {
            // we have to subtract minVal and multiply with scale to get the data in
            // [0;histosize] range
...
void buildBlendMask(const float* const* luminance,
                    float** blend,
                    int W,
                    int H,
                    ...
                    float** clipMask)
{
    ...
    if (autoContrast) {
        ...
        for (int pass = 0; pass < 2; ++pass) {
            ...
            std::vector<std::vector<float>> variances(numTilesH,
                                                      std::vector<float>(numTilesW));
...
#pragma omp parallel for schedule(dynamic)
            ...
            for (int i = 0; i < numTilesH; ++i) {
                ...
                for (int j = 0; j < numTilesW; ++j) {
                    ...
                    if (avg < minLuminance || avg > maxLuminance) {
                        ...
                        variances[i][j] =
                            tileVariance(luminance, tileY, tileX, tilesize, avg);
            ...
            if (minvar <= 1.f || pass == 1) {
                ...
                if (pass == 0) {
                    // a variance <= 1 means we already found a flat region and can skip
                    // second pass
                    contrastThreshold =
                        calcContrastThreshold(luminance, minY, minX, tilesize);
                    break;
                } else {
                    // in second pass we allow a variance of 8
                    // we additionally scan the tiles +-skip pixels around the best tile
                    // from pass 2 Means we scan (2 * skip + 1)^2 tiles in this step to
                    // get a better hit rate fortunately the scan is quite fast, so we use
                    // only one core and don't parallelize
                    ...
                    contrastThreshold =
                        minvar <= 8.f
                            ? calcContrastThreshold(luminance, topLeftYStart + minI,
                                                    topLeftXStart + minJ, tilesize)
                            : 0.f;
                }
            }
        }
...
#pragma omp for schedule(dynamic, 16)
            ...
            for (int j = 2; j < H - 2; ++j) {
                ...
                for (; i < W - 2; ++i) {
                    ...
                    blend[j][i] = (clipMask ? clipMask[j][i] : 1.f)
                                  * calcBlendFactor(contrast, contrastThreshold);
                }
            }
...
#ifdef __SSE2__
            // flush denormals to zero for gaussian blur to avoid performance penalty if
            // there are a lot of zero values in the mask
...
void buildBlendMask2(float** luminance,
                     float** blend,
                     int W,
                     int H,
                     float& contrastThreshold,
...
#pragma omp parallel for schedule(dynamic)
            ...
            if (minvar <= 1.f || pass == 1) {
                ...
                if (pass == 0) {
                    // a variance <= 1 means we already found a flat region and can skip
                    // second pass
                    contrastThreshold = calcContrastThreshold2(
                    ...
                    contrastThreshold =
                        minvar <= 8.f
                            ? calcContrastThreshold2(luminance, topLeftYStart + minI,
                                                     topLeftXStart + minJ, tilesize,
                                                     luminance_factor)
                            : 0.f;
...
#pragma omp for schedule(dynamic, 16)
            ...
            for (int j = 2; j < H - 2; ++j) {
                ...
                for (; i < W - 5; i += 4) {
                    vfloat contrastv =
                        vsqrtf(
                            SQRV(LVFU(luminance[j][i + 1]) - LVFU(luminance[j][i - 1]))
                            + SQRV(LVFU(luminance[j + 1][i]) - LVFU(luminance[j - 1][i]))
                            + SQRV(LVFU(luminance[j][i + 2]) - LVFU(luminance[j][i - 2]))
                            + SQRV(LVFU(luminance[j + 2][i]) - LVFU(luminance[j - 2][i])))
                        * scalev;
                    ...
                    STVFU(blend[j][i],
                          amountv * calcBlendFactor(contrastv, contrastThresholdv));
                }
#endif
                for (; i < W - 2; ++i) {
...
#pragma omp parallel
    ...
    float impthrDiv24 =
        impthr / 24.0f;  // Issue 1671: moved the Division outside the loop, impthr can be
...
#pragma omp for
        ...
        for (int i = 0; i < height; i++) {
            ...
            for (; j < width - 5; j += 4) {
                ...
                int mask =
                    _mm_movemask_ps((hfnbravev - hpfabsv) * impthrDiv24v - hpfabsv);
                impulse[i][j] = (mask & 1);
...
// void get_luminance(const Imagefloat *src, array2D<float> &out, const float ws[3][3],
// bool multithread)
...
void multiply(Imagefloat* img,
              const array2D<float>& num,
              const array2D<float>& den,
              bool multithread)
{
...
double
accumulateProduct(const float* data1, const float* data2, size_t n, bool multiThread)
{
    if (n == 0) {
        return 0.0;
    }
