...
inline void setUnlessOOG(float& r,
                         float& g,
                         float& b,
                         const float& rr,
                         const float& gg,
...
inline void setUnlessOOG(vfloat& r,
                         vfloat& g,
                         vfloat& b,
                         const vfloat rr,
                         const vfloat gg,
...
protected:
    // functions calculating the parameters of the contrast curve based on the desired
    // slope at the center
    ...
    // basic convex function between (0,0) and (1,1). m1 and m2 controls the slope at the
    // start and end point
    ...
    static inline double basel_alt(double x) { return (2.0 - x) * x * x * x; }
    // basic concave function between (0,0) and (1,1). m1 and m2 controls the slope at the
    // start and end point
    static inline double baseu(double x, double m1, double m2)
    {
        return 1.0 - basel(1.0 - x, m1, m2);
    }
    static inline double baseu_alt(double x) { return x * (2.0 + (x - 2.0) * x * x); }
    // convex curve between (0,0) and (1,1) with slope m at (0,0). hr controls the
    // highlight recovery
    ...
    // concave curve between (0,0) and (1,1) with slope m at (1,1). sr controls the shadow
    // recovery
    ...
    {
        // curve for b<0; starts with positive slope and then rolls over toward straight
        // line to x=y=1
    ...
    basecurve(double x, double a, double b, double D, double hr, double sr)
    {
        if (b < 0) {
            ...
            if (x > m) {
                return y
                       + (x - m)
                             * slope;  // value on straight line between (m,y) and (1,1)
            } else {
...
public:
    ...
    {
        return x <= 0.03928 ? x / 12.92310
                            : exp(log((x + 0.055) / 1.055) * sRGBGammaCurve);
    }
    static inline float gamma2(float x)
    {
        return x <= 0.00304f
                   ? x * 12.92310f
                   : 1.055f * expf(logf(x) / static_cast<float>(sRGBGammaCurve)) - 0.055f;
    }
    static inline float igamma2(float x)
    {
        return x <= 0.03928f ? x / 12.92310f
                             : expf(logf((x + 0.055f) / 1.055f)
                                    * static_cast<float>(sRGBGammaCurve));
    }
    ...
    static inline float
    gamma(float x, float gamma, float start, float slope, float mul, float add)
    {
        return (x <= start ? x * slope : xexpf(xlogf(x) / gamma) * mul - add);
    }
    static inline float
    igamma(float x, float gamma, float start, float slope, float mul, float add)
    {
        return (x <= start * slope ? x / slope : xexpf(xlogf((x + add) / mul) * gamma));
    }
#ifdef __SSE2__
    static inline vfloat
    igamma(vfloat x, vfloat gamma, vfloat start, vfloat slope, vfloat mul, vfloat add)
    {
#if !defined(__clang__)
        return (x <= start * slope ? x / slope : xexpf(xlogf((x + add) / mul) * gamma));
#else
        return vself(vmaskf_le(x, start * slope), x / slope,
                     xexpf(xlogf((x + add) / mul) * gamma));
#endif
    }
#endif
    static inline float
    hlcurve(const float exp_scale, const float comp, const float hlrange, float level)
    {
        if (comp > 0.f) {
...
public:
    static void complexCurve(double ecomp,
                             double black,
                             double hlcompr,
                             ...
                             ToneCurve& outToneCurve,
                             ToneCurve& outToneCurve2,
                             int skip = 1);

    static void complexCurvelocal(double ecomp,
                                  double black,
                                  double hlcompr,
                                  double hlcomprthresh,
                                  double shcompr,
    ...
    static void
    Curvelocalhl(double ecomp, double hlcompr, double hlcomprthresh, LUTf& hlCurve);

    static void curveBW(const std::vector<double>& curvePointsbw,
                        const std::vector<double>& curvePointsbw2,
                        const LUTu& histogrambw,
                        LUTu& outBeforeCCurveHistogrambw,
                        ToneCurve& customToneCurvebw1,
                        ToneCurve& customToneCurvebw2,
                        int skip);

    static bool diagonalCurve2Lut(const std::vector<double>& curvePoints,
                                  LUTf& curve,
                                  int skip,
                                  const LUTu& histogram,
                                  LUTu& outBeforeCurveHistogram);
    static bool
    diagonalCurve2Lut(const std::vector<double>& curvePoints, LUTf& curve, int skip);

    static void complexsgnCurve(bool& autili,
                                bool& butili,
                                bool& ccutili,
                                bool& clcutili,
    ...
    static void updatechroma(const std::vector<double>& cccurvePoints,
                             LUTu& histogramC,
                             LUTu& outBeforeCCurveHistogramC,  // for chroma
                             int skip = 1);
    static void complexLCurve(double br,
                              double contr,
                              const std::vector<double>& curvePoints,
                              const LUTu& histogram,
                              LUTf& outCurve,
    ...
    static void curveLightBrightColor(const std::vector<double>& curvePoints,
                                      const std::vector<double>& curvePoints2,
                                      const std::vector<double>& curvePoints3,
                                      const LUTu& histogram,
                                      LUTu& outBeforeCCurveHistogram,
                                      const LUTu& histogramC,
                                      LUTu& outBeforeCCurveHistogramC,
                                      ColorAppearance& outColCurve1,
                                      ColorAppearance& outColCurve2,
                                      ColorAppearance& outColCurve3,
                                      int skip = 1);
    static void
    RGBCurve(const std::vector<double>& curvePoints, LUTf& outCurve, int skip);
};
...
protected:
    ...
    {
        return x <= 0.5 ? CurveFactory::clower(x * 2, 2.0, prot) * 0.5
                        : 0.5 + CurveFactory::cupper((x - 0.5) * 2, 2.0, prot) * 0.5;
    }
    static inline double p10(double x, double prot)
    {
        return x <= 0.5 ? CurveFactory::cupper(x * 2, 2.0, prot) * 0.5
                        : 0.5 + CurveFactory::clower((x - 0.5) * 2, 2.0, prot) * 0.5;
    }
    static inline double pfull(double x, double prot, double sh, double hl)
    {
        return (1 - sh) * (1 - hl) * p00(x, prot) + sh * hl * p11(x, prot)
               + (1 - sh) * hl * p01(x, prot) + sh * (1 - hl) * p10(x, prot);
    }
    static inline double pfull_alt(double x, double sh, double hl)
    {
        double t = (1.0 - sh) * (1.0 - hl) * CurveFactory::basel_alt(x)
                   + sh * hl * CurveFactory::baseu_alt(x);
        return x <= 0.5
                   ? t + (1.0 - sh) * hl * CurveFactory::basel_alt(2.0 * x) * 0.5
                         + sh * (1.0 - hl) * CurveFactory::baseu_alt(2.0 * x) * 0.5
                   : t
                         + (1.0 - sh) * hl
                               * (0.5 + CurveFactory::baseu_alt(2.0 * x - 1.0) * 0.5)
                         + sh * (1.0 - hl)
                               * (0.5 + CurveFactory::basel_alt(2.0 * x - 1.0) * 0.5);
    }
...
public:
    explicit DiagonalCurve(const std::vector<double>& points,
                           int ppn = CURVES_MIN_POLY_POINTS);
...
public:
    explicit FlatCurve(const std::vector<double>& points,
...
public:
    ...
    float operator[](float index) const { return luttransmission[index]; }

    operator bool(void) const { return luttransmission; }
};
...
public:
    ...
    void Set(const Curve* pCurve);
    void Set(const std::vector<double>& curvePoints);
    float operator[](float index) const { return lutgaintransmission[index]; }

    operator bool(void) const { return lutgaintransmission; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocLHCurve[index]; }
    operator bool(void) const { return lutLocLHCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocHHmaskblCurve[index]; }
    operator bool(void) const { return lutLocHHmaskblCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocCCmaskblCurve[index]; }
    operator bool(void) const { return lutLocCCmaskblCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocLLmaskblCurve[index]; }
    operator bool(void) const { return lutLocLLmaskblCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocHHmasktmCurve[index]; }
    operator bool(void) const { return lutLocHHmasktmCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocCCmasktmCurve[index]; }
    operator bool(void) const { return lutLocCCmasktmCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocLLmasktmCurve[index]; }
    operator bool(void) const { return lutLocLLmasktmCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocHHmaskretiCurve[index]; }
    operator bool(void) const { return lutLocHHmaskretiCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocCCmaskretiCurve[index]; }
    operator bool(void) const { return lutLocCCmaskretiCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocLLmaskretiCurve[index]; }
    operator bool(void) const { return lutLocLLmaskretiCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocHHmaskcbCurve[index]; }
    operator bool(void) const { return lutLocHHmaskcbCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocCCmaskcbCurve[index]; }
    operator bool(void) const { return lutLocCCmaskcbCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocLLmaskcbCurve[index]; }
    operator bool(void) const { return lutLocLLmaskcbCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocHHmaskexpCurve[index]; }
    operator bool(void) const { return lutLocHHmaskexpCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocCCmaskexpCurve[index]; }
    operator bool(void) const { return lutLocCCmaskexpCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocLLmaskexpCurve[index]; }
    operator bool(void) const { return lutLocLLmaskexpCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocHHmaskSHCurve[index]; }
    operator bool(void) const { return lutLocHHmaskSHCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocCCmaskSHCurve[index]; }
    operator bool(void) const { return lutLocCCmaskSHCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocLLmaskSHCurve[index]; }
    operator bool(void) const { return lutLocLLmaskSHCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocHHmaskCurve[index]; }
    operator bool(void) const { return lutLocHHmaskCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocCCmaskCurve[index]; }
    operator bool(void) const { return lutLocCCmaskCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocLLmaskCurve[index]; }
    operator bool(void) const { return lutLocLLmaskCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocHHCurve[index]; }
    operator bool(void) const { return lutLocHHCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocCHCurve[index]; }
    operator bool(void) const { return lutLocCHCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocretigainCurve[index]; }
    operator bool(void) const { return lutLocretigainCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocretitransCurve[index]; }
    operator bool(void) const { return lutLocretitransCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocwavCurve[index]; }

#ifdef __SSE2__
    vfloat operator[](vfloat index) const { return lutLocwavCurve[index]; }
#endif

    operator bool(void) const { return lutLocwavCurve; }
};
...
public:
    ...
    float operator[](float index) const { return lutLocretigainCurverab[index]; }
    operator bool(void) const { return lutLocretigainCurverab; }
};
...
public:
    ...
    //  void Set(const std::vector<double> &curvePoints, bool &opautili);
    void Set(const std::vector<double>& curvePoints);
    float operator[](float index) const { return lutblcurve[index]; }

    operator bool(void) const { return lutblcurve; }
};
...
public:
    ...
    //  void Set(const std::vector<double> &curvePoints, bool &opautili);
    void Set(const std::vector<double>& curvePoints);
    float operator[](float index) const { return lutOpacityCurveSH[index]; }

    operator bool(void) const { return lutOpacityCurveSH; }
};
...
public:
    ...
    void Reset();
    void
    SetXYZ(const Curve* pCurve, const double xyz_rgb[3][3], float satur, float lumin);
    void SetXYZ(const std::vector<double>& curvePoints,
...
public:
    ...
    void
    BatchApply(const size_t start, const size_t end, float* r, float* g, float* b) const;
};
...
// Tone curve whose purpose is to keep the color appearance constant, that is the curve
// changes contrast but colors appears to have the same hue and saturation as before. As
// contrast and saturation is tightly coupled in human vision saturation is modulated
// based on the curve's contrast, and that way the appearance can be kept perceptually
...
private:
    ...
    static void cubic_spline(const float x[],
                             const float y[],
                             const int len,
                             const float out_x[],
                             float out_y[],
                             const int out_len);
    static float find_minimum_interval_halving(float (*func)(float x, void* arg),
                                               void* arg,
                                               float a,
                                               float b,
                                               float tol,
...
public:
    static void init();
    void initApplyState(PerceptualToneCurveState& state,
                        const Glib::ustring& workingSpace) const;
    void BatchApply(const size_t start,
                    const size_t end,
                    float* r,
                    float* g,
                    float* b,
...
inline void StandardToneCurve::BatchApply(const size_t start,
                                          const size_t end,
                                          float* r,
                                          float* g,
                                          float* b) const
{
    ...
    while (true) {
        ...
        setUnlessOOG(r[i], g[i], b[i], lutToneCurve[r[i]], lutToneCurve[g[i]],
                     lutToneCurve[b[i]]);
        i++;
    }
    ...
    for (; i + 3 < end; i += 4) {
        ...
        setUnlessOOG(r_val, g_val, b_val, lutToneCurve[r_val], lutToneCurve[g_val],
                     lutToneCurve[b_val]);
        STVF(r[i], r_val);
        STVF(g[i], g_val);
        STVF(b[i], b_val);
...
inline void AdobeToneCurve::BatchApply(const size_t start,
                                       const size_t end,
                                       float* r,
                                       float* g,
                                       float* b) const
{
    ...
    for (; i + 3 < end; i += 4) {
        ...
        const vfloat nr = vself(vmaskf_eq(rc, maxvalold), maxval,
                                vself(vmaskf_eq(rc, minvalold), minval, medval));
        const vfloat ng = vself(vmaskf_eq(gc, maxvalold), maxval,
                                vself(vmaskf_eq(gc, minvalold), minval, medval));
        const vfloat nb = vself(vmaskf_eq(bc, maxvalold), maxval,
                                vself(vmaskf_eq(bc, minvalold), minval, medval));
...
inline void AdobeToneCurve::RGBTone(float& maxval, float& medval, float& minval) const
{
    ...
    medval =
        minval + ((maxval - minval) * (medvalold - minvalold) / (maxvalold - minvalold));
}
#ifdef __SSE2__
inline void AdobeToneCurve::RGBTone(vfloat& maxval, vfloat& medval, vfloat& minval) const
{
    ...
    minval = lutToneCurve[minvalold];
    medval =
        minval + ((maxval - minval) * (medval - minvalold) / (maxvalold - minvalold));
...
inline void WeightedStdToneCurve::BatchApply(const size_t start,
                                             const size_t end,
                                             float* r,
                                             float* g,
                                             float* b) const
