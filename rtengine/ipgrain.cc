...
public:
    GrainEvaluator(int offset_x,
                   ...
                   int fww,
                   int fhh)
        : ox(offset_x), oy(offset_y), fw(full_width), fh(full_height), scale(scale)
    {
        simplex_noise_init();
                    ...
                    int fhh)
    {
        ...
        const double zoom =
            (1.0 + 8 * (double(isogr) / GRAIN_SCALE_FACTOR) / 100.0) / 800.0;
        const double s =
            std::max(scale / 3.0, 1.0) / (double(std::max(scalegr, 1)) / 100.0);
...
#pragma omp parallel for if (multithread)
        ...
        for (int j = 0; j < H; ++j) {
            ...
            for (int i = 0; i < W; ++i) {
                ...
                lab_L[j][i] +=
                    lut_lookup(noise * strength * GRAIN_LIGHTNESS_STRENGTH_SCALE,
                               lab_L[j][i] / 32768.f);
            }
        }
    }
...
private:
    ...
    float FASTFLOOR(float x) { return (x > 0 ? (int)(x) : (int)(x)-1); }
    ...
    float paper_resp(float exposure, float mb, float gp, float divgr)
    {
        ...
        const float delta = dived * GRAIN_LUT_DELTA_MAX
                            * expf((mb / 100.0f) * logf(GRAIN_LUT_DELTA_MIN / dived));
        const float density =
            (1.0f + 2.0f * delta)
        ...
        for (int i = 0; i < GRAIN_LUT_SIZE; i++) {
            for (int j = 0; j < GRAIN_LUT_SIZE; j++) {
                ...
                grain_lut[j * GRAIN_LUT_SIZE + i] =
                    ...
                    * (paper_resp(gu
                                      + paper_resp_inverse(
                                          l, mb, divg * GRAIN_LUT_PAPER_GAMMA, divgr),
                                  mb, divg * GRAIN_LUT_PAPER_GAMMA, divgr)
                       - l);
            }
        }
    }
    ...
    {
        const float _x =
            LIM((x + 0.5f) * (GRAIN_LUT_SIZE - 1), 0.f, float(GRAIN_LUT_SIZE - 1));
