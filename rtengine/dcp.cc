...
// This sRGB gamma is taken from DNG reference code, with the added linear extension
// past 1.0, as we run clipless here
...
DCPProfile::Matrix mix3x3(const DCPProfile::Matrix& a,
                          double mul_a,
...
std::map<std::string, std::string> getAliases(const Glib::ustring& profile_dir)
{
    const std::unique_ptr<std::FILE, std::function<void(std::FILE*)>> file(
        g_fopen(Glib::build_filename(profile_dir, "camera_model_aliases.json").c_str(),
                "rb"),
    ...
    const std::unique_ptr<cJSON, decltype(&cJSON_Delete)> root(cJSON_Parse(buffer.get()),
                                                               cJSON_Delete);
    if (!root || !root->child) {
        if (settings->verbose) {
...
public:
    explicit DCPMetadata(FILE* file) : file_(file), order_(UNKNOWN) {}
    ...
    std::string toString(int id) const
    {
        ...
        if (tag != tags_.end()) {
            if (tag->second.type == ASCII) {
                return std::string(tag->second.value.begin(), tag->second.value.end())
                    .c_str();
            }
        }
        return {};
        ...
        case SSHORT:
        {
            if (offset + 1 < tag->second.value.size()) {
                return static_cast<std::int16_t>(
                    sget2(tag->second.value.data() + offset));
            }
            return 0;
        }
        ...
        case RATIONAL:
        {
            if (offset + 7 < tag->second.value.size()) {
                const std::uint32_t denominator =
                    sget4(tag->second.value.data() + offset + 4);
                return denominator == 0 ? 0
                                        : static_cast<std::int32_t>(
                                              sget4(tag->second.value.data() + offset))
                                              / denominator;
            }
            return 0;
        }
    ...
    int toShort(int id, std::size_t offset = 0) const { return toInt(id, offset, SHORT); }
...
private:
    ...
    bool parseTag(Tag& tag)
    {
        ...
        // Note: The large count is to be able to pass LeafData ASCII tag which can be up
        // to almost 10 megabytes, (only a small part of it will actually be parsed
        // though)
        if (tag.type == INVALID || tag.type > DOUBLE || tag.count > 10 * 1024 * 1024) {
            tag.type = INVALID;
            return false;
        }
        ...
        const std::size_t value_size =
            static_cast<std::size_t>(tag.count) * getTypeSize(tag.type);
...
DCPProfileApplyState::DCPProfileApplyState() : data(new Data{}) {}
...
{
    delta_info.hue_step = delta_info.val_step = look_info.hue_step = look_info.val_step =
        0;
    ...
    enum TagKey {
        ...
        TAG_KEY_PROFILE_LOOK_TABLE_DIMS =
    ...
    const std::unique_ptr<std::FILE, std::function<void(std::FILE*)>> file(
        g_fopen(filename.c_str(), "rb"), [](std::FILE* file) { std::fclose(file); });
    ...
    light_source_1 = md.find(TAG_KEY_CALIBRATION_ILLUMINANT_1)
                         ? md.toShort(TAG_KEY_CALIBRATION_ILLUMINANT_1)
                         : -1;
    light_source_2 = md.find(TAG_KEY_CALIBRATION_ILLUMINANT_2)
    ...
    const bool has_second_hue_sat =
        md.find(TAG_KEY_PROFILE_HUE_SAT_MAP_DATA_2);  // Some profiles have two matrices,
                                                      // but just one huesat
        ...
        for (int row = 0; row < 3; ++row) {
            for (int col = 0; col < 3; ++col) {
                forward_matrix_1[row][col] =
                    md.toDouble(TAG_KEY_FORWARD_MATRIX_1, (col + row * 3) * 8);
            }
        }
    }
        ...
        for (int row = 0; row < 3; ++row) {
            for (int col = 0; col < 3; ++col) {
                forward_matrix_2[row][col] =
                    md.toDouble(TAG_KEY_FORWARD_MATRIX_2, (col + row * 3) * 8);
            }
        }
    }
    ...
    for (int row = 0; row < 3; ++row) {
        for (int col = 0; col < 3; ++col) {
            color_matrix_1[row][col] =
                md.toDouble(TAG_KEY_COLOR_MATRIX_1, (col + row * 3) * 8);
        }
    }
    ...
    if (md.find(TAG_KEY_PROFILE_LOOK_TABLE_DIMS)) {
        ...
        look_info.srgb_gamma = md.find(TAG_KEY_PROFILE_LOOK_TABLE_ENCODING)
                               && md.toInt(TAG_KEY_PROFILE_LOOK_TABLE_ENCODING);
        ...
        for (unsigned int i = 0; i < look_info.array_count; i++) {
            look_table[i].hue_shift =
                md.toDouble(TAG_KEY_PROFILE_LOOK_TABLE_DATA, (i * 3) * tiff_float_size);
            look_table[i].sat_scale = md.toDouble(TAG_KEY_PROFILE_LOOK_TABLE_DATA,
                                                  (i * 3 + 1) * tiff_float_size);
            look_table[i].val_scale = md.toDouble(TAG_KEY_PROFILE_LOOK_TABLE_DATA,
                                                  (i * 3 + 2) * tiff_float_size);
        }

        // Precalculated constants for table application
        look_info.pc.h_scale = look_info.hue_divisions < 2
                                   ? 0.0f
    ...
    if (md.find(TAG_KEY_PROFILE_HUE_SAT_MAP_DIMS)) {
        ...
        delta_info.srgb_gamma = md.find(TAG_KEY_PROFILE_HUE_SAT_MAP_ENCODING)
                                && md.toInt(TAG_KEY_PROFILE_HUE_SAT_MAP_ENCODING);
        ...
        for (unsigned int i = 0; i < delta_info.array_count; ++i) {
            deltas_1[i].hue_shift = md.toDouble(TAG_KEY_PROFILE_HUE_SAT_MAP_DATA_1,
                                                (i * 3) * tiff_float_size);
            deltas_1[i].sat_scale = md.toDouble(TAG_KEY_PROFILE_HUE_SAT_MAP_DATA_1,
                                                (i * 3 + 1) * tiff_float_size);
            deltas_1[i].val_scale = md.toDouble(TAG_KEY_PROFILE_HUE_SAT_MAP_DATA_1,
                                                (i * 3 + 2) * tiff_float_size);
        }

        delta_info.pc.h_scale = delta_info.hue_divisions < 2
                                    ? 0.0f
    ...
    if (light_source_2 != -1) {
        ...
        for (int row = 0; row < 3; ++row) {
            for (int col = 0; col < 3; ++col) {
                color_matrix_2[row][col] =
                    cm2 ? md.toDouble(TAG_KEY_COLOR_MATRIX_2, (col + row * 3) * 8)
                        : color_matrix_1[row][col];
            }
        }
        ...
        if (has_second_hue_sat) {
            ...
            for (unsigned int i = 0; i < delta_info.array_count; ++i) {
                deltas_2[i].hue_shift = md.toDouble(TAG_KEY_PROFILE_HUE_SAT_MAP_DATA_2,
                                                    (i * 3) * tiff_float_size);
                deltas_2[i].sat_scale = md.toDouble(TAG_KEY_PROFILE_HUE_SAT_MAP_DATA_2,
                                                    (i * 3 + 1) * tiff_float_size);
                deltas_2[i].val_scale = md.toDouble(TAG_KEY_PROFILE_HUE_SAT_MAP_DATA_2,
                                                    (i * 3 + 2) * tiff_float_size);
            }
        }
    }
    ...
    if (md.find(TAG_KEY_PROFILE_TONE_CURVE)) {
        ...
        for (unsigned int i = 0, n = md.getCount(TAG_KEY_PROFILE_TONE_CURVE); i < n;
             i += 2)
        {
            const double x =
                md.toDouble(TAG_KEY_PROFILE_TONE_CURVE, (i + 0) * tiff_float_size);
            const double y =
                md.toDouble(TAG_KEY_PROFILE_TONE_CURVE, (i + 1) * tiff_float_size);
        ...
        if (md.find(TAG_KEY_PROFILE_TONE_COPYRIGHT)
            && md.toString(TAG_KEY_PROFILE_TONE_COPYRIGHT).find("Adobe Systems")
                   != std::string::npos)
        {
            // An Adobe profile without tone curve is expected to have the Adobe Default
            // Curve, we add that
            std::vector<double> curve_points = { static_cast<double>(DCT_Spline) };
...
void DCPProfile::apply(Imagefloat* img,
                       ...
                       bool apply_hue_sat_map) const
{

    const TMatrix work_matrix =
        ICCStore::getInstance()->workingSpaceInverseMatrix(working_space);

    const Matrix xyz_cam =
        makeXyzCam(white_balance, pre_mul, cam_wb_matrix,
                   preferred_illuminant);  // Camera RGB to XYZ D50 matrix
    ...
    if (!apply_hue_sat_map) {
        // The fast path: No LUT --> Calculate matrix for direct conversion raw -> working
        // space
        float mat[3][3] = {};
...
#pragma omp parallel for
        ...
        for (int y = 0; y < img->getHeight(); ++y) {
            for (int x = 0; x < img->getWidth(); x++) {
                const float& newr = mat[0][0] * img->r(y, x) + mat[0][1] * img->g(y, x)
                                    + mat[0][2] * img->b(y, x);
                const float& newg = mat[1][0] * img->r(y, x) + mat[1][1] * img->g(y, x)
                                    + mat[1][2] * img->b(y, x);
                const float& newb = mat[2][0] * img->r(y, x) + mat[2][1] * img->g(y, x)
                                    + mat[2][2] * img->b(y, x);
...
#pragma omp parallel for schedule(dynamic, 16)
        ...
        for (int y = 0; y < img->getHeight(); ++y) {
            for (int x = 0; x < img->getWidth(); x++) {
                ...
                // If point is in negative area, just the matrix, but not the LUT. This is
                // checked inside Color::rgb2hsvdcp
                float h;
                float s;
                float v;
...
void DCPProfile::step2ApplyTile(float* rc,
                                float* gc,
                                float* bc,
                                int width,
                                int height,
                                int tile_width,
                                const DCPProfileApplyState& as_in) const
{
...
DCPProfile::Matrix DCPProfile::findXyztoCamera(const std::array<double, 2>& white_xy,
                                               int preferred_illuminant) const
{
    ...
    if (has_col_1 && has_col_2) {
        /*
          Note: We're using DNG SDK reference code for XY to temperature translation to
          get the exact same mix as the reference code does.
        */
                                              ...
                                              int preferred_illuminant) const
{
    ...
    for (unsigned int pass = 0; pass < MAX_PASSES; ++pass) {
        ...
        if (std::fabs(next_xy[0] - last_xy[0]) + std::fabs(next_xy[1] - last_xy[1])
            < 0.0000001)
        {
            return next_xy;
        }
                                          ...
                                          int preferred_illuminant) const
{
    ...
    // Note that we do not support monochrome or colorplanes > 3 (no reductionMatrix
    // support), we do not support cameracalibration either.

    Triple
        neutral;  // Same as the DNG "AsShotNeutral" tag if white balance is Camera's own
    {
        /* A bit messy matrixing and conversions to get the neutral[] array from RT's own
           white balance which is stored in sRGB space, while the DCP code needs
           multipliers in CameraRGB space */
        double r, g, b;
    ...
    /* Calculate what the RGB multipliers corresponds to as a white XY coordinate, based
       on the DCP ColorMatrix or ColorMatrices if dual-illuminant. This is the DNG
       reference code way to do it, which is a bit different from RT's own white balance
       model at the time of writing. When RT's white balance can make use of the DCP color
       matrices we could use that instead. */
    ...
    if ((has_col_1 && has_col_2) || (has_fwd_1 && has_fwd_2)) {
        /* DNG ref way to convert XY to temperature, which affect matrix mixing. A
           different model here typically does not affect the result too much, ie it's
           probably not strictly necessary to use the DNG reference code here, but we do
           it for now. */
        const double wbtemp = xyCoordToTemperature(white_xy);
    ...
    */
    ...
    // Convert cam_xyz (XYZ D50 to CameraRGB, "PCS to Camera" in DNG terminology) to
    // mXYZCAM

    // This block can probably be simplified, seems unnecessary to pass through the sRGB
    // matrix (probably dcraw legacy), it does no harm though as we don't clip anything.
    int i, j, k;
...
DCPProfile::makeHueSatMap(const ColorTemp& white_balance, int preferred_illuminant) const
{
    if (deltas_1.empty()) {
...
inline void DCPProfile::hsdApply(const HsdTableInfo& table_info,
                                 ...
                                 float& v) const
{
    ...
    if (table_info.val_divisions < 2) {
        ...
        const float hue_shift0 = h_fract0 * table_base[e00_index].hue_shift
                                 + h_fract1 * table_base[e01_index].hue_shift;
        ...
        const float hueShift1 = h_fract0 * table_base[e00_index].hue_shift
                                + h_fract1 * table_base[e01_index].hue_shift;
        ...
        std::vector<HsbModify>::size_type e00_index = v_index0 * table_info.pc.val_step
                                                      + h_index0 * table_info.pc.hue_step
                                                      + s_index0;
        std::vector<HsbModify>::size_type e01_index =
            e00_index + (h_index1 - h_index0) * table_info.pc.hue_step;
        ...
        const float hueShift0 = v_fract0
                                    * (h_fract0 * table_base[e00_index].hue_shift
                                       + h_fract1 * table_base[e01_index].hue_shift)
                                + v_fract1
                                      * (h_fract0 * table_base[e10_index].hue_shift
                                         + h_fract1 * table_base[e11_index].hue_shift);
        const float satScale0 = v_fract0
                                    * (h_fract0 * table_base[e00_index].sat_scale
                                       + h_fract1 * table_base[e01_index].sat_scale)
                                + v_fract1
                                      * (h_fract0 * table_base[e10_index].sat_scale
                                         + h_fract1 * table_base[e11_index].sat_scale);
        const float valScale0 = v_fract0
                                    * (h_fract0 * table_base[e00_index].val_scale
                                       + h_fract1 * table_base[e01_index].val_scale)
                                + v_fract1
                                      * (h_fract0 * table_base[e10_index].val_scale
        ...
        const float hueShift1 = v_fract0
                                    * (h_fract0 * table_base[e00_index].hue_shift
                                       + h_fract1 * table_base[e01_index].hue_shift)
                                + v_fract1
                                      * (h_fract0 * table_base[e10_index].hue_shift
                                         + h_fract1 * table_base[e11_index].hue_shift);
        const float satScale1 = v_fract0
                                    * (h_fract0 * table_base[e00_index].sat_scale
                                       + h_fract1 * table_base[e01_index].sat_scale)
                                + v_fract1
                                      * (h_fract0 * table_base[e10_index].sat_scale
                                         + h_fract1 * table_base[e11_index].sat_scale);
        const float valScale1 = v_fract0
                                    * (h_fract0 * table_base[e00_index].val_scale
                                       + h_fract1 * table_base[e01_index].val_scale)
                                + v_fract1
                                      * (h_fract0 * table_base[e10_index].val_scale
...
void DCPStore::init(const Glib::ustring& rt_profile_dir, bool loadAll)
{
    ...
    auto profile_dirpath =
        Glib::build_filename(App::get().options().rtdir, "dcpprofiles");
    ...
    while (!dirs.empty()) {
        ...
        for (const Glib::ustring& sname : *dir) {
            ...
            if (!Glib::file_test(fname, Glib::FILE_TEST_IS_DIR)) {
                ...
                {
                    file_std_profiles[casefold_collate_key(sname.substr(0, lastdot))] =
    ...
    for (const auto& alias : getAliases(rt_profile_dir)) {
        ...
        const std::map<std::string, Glib::ustring>::const_iterator real =
            file_std_profiles.find(casefold_collate_key(alias.second));
...
DCPProfile* DCPStore::getProfile(const Glib::ustring& filename) const
{
    ...
    MyMutex::MyLock lock(mutex);
    const std::map<std::string, DCPProfile*>::const_iterator iter =
        profile_cache.find(key);
...
{
    const std::map<std::string, Glib::ustring>::const_iterator iter =
        file_std_profiles.find(casefold_collate_key(requested_cam_short_name));
