...
void rotateLine(const float* const line,
                rtengine::PlanarPtr<float>& channel,
                const int tran,
                const int i,
                const int w,
                const int h)
...
void transLineStandard(const float* const red,
                       const float* const green,
                       const float* const blue,
                       const int i,
                       rtengine::Imagefloat* const image,
                       const int tran,
                       const int imwidth,
                       const int imheight)
{
...
void transLineFuji(const float* const red,
                   const float* const green,
                   const float* const blue,
                   const int i,
                   rtengine::Imagefloat* const image,
    ...
    case TR_R180:
        for (int j = start; j < end; j++) {
            ...
            if (x >= 0 && y < image->getHeight() && y >= 0 && x < image->getWidth()) {
                ...
                image->g(image->getHeight() - 1 - y, image->getWidth() - 1 - x) =
                    green[j];
...
void transLineD1x(const float* const red,
                  const float* const green,
                  const float* const blue,
                  const int i,
                  rtengine::Imagefloat* const image,
                  ...
                  const bool clip)
{
    ...
    case TR_R180:  // rotate 180 degree
        ...
        if (i == 1 || i == 2) {  // linear interpolation
            ...
            for (int j = 0; j < imwidth; j++) {
                ...
                image->r(row, col) = MAX(
                    0.f,
                    -0.0625f * (red[j] + image->r(row + 3, col))
                        + 0.5625f * (image->r(row - 1, col) + image->r(row + 1, col)));
                image->g(row, col) = MAX(
                    0.f,
                    -0.0625f * (green[j] + image->g(row + 3, col))
                        + 0.5625f * (image->g(row - 1, col) + image->g(row + 1, col)));
                image->b(row, col) = MAX(
                    0.f,
                    -0.0625f * (blue[j] + image->b(row + 3, col))
                        + 0.5625f * (image->b(row - 1, col) + image->b(row + 1, col)));
    ...
    case TR_R90:  // rotate right
        ...
        if (i == 1 || i == 2) {  // linear interpolation
            ...
            for (int j = 0; j < imwidth; j++) {
                ...
                if (oddHeight && i == 2) {
                    image->r(j, 2 * imheight) =
                        (red[j] + image->r(j, 2 * imheight - 2)) / 2;
                    image->g(j, 2 * imheight) =
                        (green[j] + image->g(j, 2 * imheight - 2)) / 2;
                    image->b(j, 2 * imheight) =
                        (blue[j] + image->b(j, 2 * imheight - 2)) / 2;
                }
            }
            ...
            for (int j = 0; j < imwidth; j++) {
                image->r(j, col) = MAX(
                    0.f, -0.0625f * (red[j] + image->r(j, col + 3))
                             + 0.5625f * (image->r(j, col - 1) + image->r(j, col + 1)));
                image->g(j, col) = MAX(
                    0.f, -0.0625f * (green[j] + image->g(j, col + 3))
                             + 0.5625f * (image->g(j, col - 1) + image->g(j, col + 1)));
                image->b(j, col) = MAX(
                    0.f, -0.0625f * (blue[j] + image->b(j, col + 3))
                             + 0.5625f * (image->b(j, col - 1) + image->b(j, col + 1)));
    ...
    case TR_R270:  // rotate left
        if (i == 0) {
            ...
            for (int j = imwidth - 1, row = 0; j >= 0; j--, row++) {
                image->r(row, 2 * i - 3) =
                    MAX(0.f, -0.0625f * (red[j] + image->r(row, 2 * i - 6))
                                 + 0.5625f
                                       * (image->r(row, 2 * i - 2)
                                          + image->r(row, 2 * i - 4)));
                image->g(row, 2 * i - 3) =
                    MAX(0.f, -0.0625f * (green[j] + image->g(row, 2 * i - 6))
                                 + 0.5625f
                                       * (image->g(row, 2 * i - 2)
                                          + image->g(row, 2 * i - 4)));
                image->b(row, 2 * i - 3) =
                    MAX(0.f, -0.0625f * (blue[j] + image->b(row, 2 * i - 6))
                                 + 0.5625f
                                       * (image->b(row, 2 * i - 2)
                                          + image->b(row, 2 * i - 4)));
        ...
        if (i == imheight - 1) {
            for (int j = imwidth - 1, row = 0; j >= 0; j--, row++) {
                image->r(row, 2 * i - 1) = MAX(
                    0.f,
                    -0.0625f * (red[j] + image->r(row, 2 * i - 4))
                        + 0.5625f * (image->r(row, 2 * i) + image->r(row, 2 * i - 2)));
                image->g(row, 2 * i - 1) = MAX(
                    0.f,
                    -0.0625f * (green[j] + image->g(row, 2 * i - 4))
                        + 0.5625f * (image->g(row, 2 * i) + image->g(row, 2 * i - 2)));
                image->b(row, 2 * i - 1) = MAX(
                    0.f,
                    -0.0625f * (blue[j] + image->b(row, 2 * i - 4))
                        + 0.5625f * (image->b(row, 2 * i) + image->b(row, 2 * i - 2)));
    ...
    default:
        ...
        if (i == 1 || i == 2) {  // linear interpolation
            ...
            for (int j = 0; j < imwidth; j++) {
                image->r(2 * i - 3, j) = MAX(
                    0.f,
                    -0.0625f * (red[j] + image->r(2 * i - 6, j))
                        + 0.5625f * (image->r(2 * i - 2, j) + image->r(2 * i - 4, j)));
                image->g(2 * i - 3, j) = MAX(
                    0.f,
                    -0.0625f * (green[j] + image->g(2 * i - 6, j))
                        + 0.5625f * (image->g(2 * i - 2, j) + image->g(2 * i - 4, j)));
                image->b(2 * i - 3, j) = MAX(
                    0.f,
                    -0.0625f * (blue[j] + image->b(2 * i - 6, j))
                        + 0.5625f * (image->b(2 * i - 2, j) + image->b(2 * i - 4, j)));
        ...
        if (i == imheight - 1) {
            for (int j = 0; j < imwidth; j++) {
                image->r(2 * i - 1, j) = MAX(
                    0.f, -0.0625f * (red[j] + image->r(2 * i - 4, j))
                             + 0.5625f * (image->r(2 * i, j) + image->r(2 * i - 2, j)));
                image->g(2 * i - 1, j) = MAX(
                    0.f, -0.0625f * (green[j] + image->g(2 * i - 4, j))
                             + 0.5625f * (image->g(2 * i, j) + image->g(2 * i - 2, j)));
                image->b(2 * i - 1, j) = MAX(
                    0.f, -0.0625f * (blue[j] + image->b(2 * i - 4, j))
                             + 0.5625f * (image->b(2 * i, j) + image->b(2 * i - 2, j)));
...
bool checkRawDataDimensions(const array2D<float>& rawData,
                            const rtengine::RawImage& rawImage,
                            int width,
                            int height)
{
...
void RawImageSource::transformRect(const PreviewProps& pp,
                                   int tran,
                                   int& ssx1,
                                   int& ssy1,
                                   int& width,
...
float calculate_scale_mul(float scale_mul[4],
                          const float pre_mul_[4],
                          const float c_white[4],
                          const float c_black[4],
                          bool isMono,
                          int colors)
{
    if (isMono || colors == 1) {
    ...
    float gain = max(scale_mul[0], scale_mul[1], scale_mul[2], scale_mul[3])
                 / min(scale_mul[0], scale_mul[1], scale_mul[2], scale_mul[3]);
    return gain;
}
...
void RawImageSource::wbMul2Camera(double& rm, double& gm, double& bm)
{
    ...
    if (imatrices) {
        double rr = imatrices->cam_rgb[0][0] * r + imatrices->cam_rgb[0][1] * g
                    + imatrices->cam_rgb[0][2] * b;
        double gg = imatrices->cam_rgb[1][0] * r + imatrices->cam_rgb[1][1] * g
                    + imatrices->cam_rgb[1][2] * b;
        double bb = imatrices->cam_rgb[2][0] * r + imatrices->cam_rgb[2][1] * g
                    + imatrices->cam_rgb[2][2] * b;
        r = rr;
        g = gg;
        b = bb;
    }
...
void RawImageSource::wbCamera2Mul(double& rm, double& gm, double& bm)
{
    ...
    if (imatrices) {
        double rr = imatrices->rgb_cam[0][0] * r + imatrices->rgb_cam[0][1] * g
                    + imatrices->rgb_cam[0][2] * b;
        double gg = imatrices->rgb_cam[1][0] * r + imatrices->rgb_cam[1][1] * g
                    + imatrices->rgb_cam[1][2] * b;
        double bb = imatrices->rgb_cam[2][0] * r + imatrices->rgb_cam[2][1] * g
                    + imatrices->rgb_cam[2][2] * b;
        r = rr;
        g = gg;
        b = bb;
    }
...
void RawImageSource::getWBMults(const ColorTemp& ctemp,
                                const RAWParams& raw,
                                std::array<float, 4>& out_scale_mul,
                                float& autoGainComp,
                                float& rm,
                                float& gm,
                                float& bm) const
{
    ...
    if (ctemp.getTemp() < 0) {
        // no white balance, ie revert the pre-process white balance to restore original
        // unbalanced raw camera color
        rm = ri->get_pre_mul(0);
        gm = ri->get_pre_mul(1);
        bm = ri->get_pre_mul(2);
    } else {
        ctemp.getMultipliers(r, g, b);
        rm = imatrices.cam_rgb[0][0] * r + imatrices.cam_rgb[0][1] * g
             + imatrices.cam_rgb[0][2] * b;
        gm = imatrices.cam_rgb[1][0] * r + imatrices.cam_rgb[1][1] * g
             + imatrices.cam_rgb[1][2] * b;
        bm = imatrices.cam_rgb[2][0] * r + imatrices.cam_rgb[2][1] * g
             + imatrices.cam_rgb[2][2] * b;
    }

    // adjust gain so the maximum raw value of the least scaled channel just hits max
    const float new_pre_mul[4] = { ri->get_pre_mul(0) / rm, ri->get_pre_mul(1) / gm,
                                   ri->get_pre_mul(2) / bm, ri->get_pre_mul(3) / gm };
    float new_scale_mul[4];

    bool isMono = (ri->getSensorType() == ST_FUJI_XTRANS
                  ...
                  || (ri->getSensorType() == ST_BAYER
                      && raw.bayersensor.method
                             == RAWParams::BayerSensor::getMethodString(
                                 RAWParams::BayerSensor::Method::MONO));

    float c_white[4];

    for (int i = 0; i < 4; ++i) {
        c_white[i] = (ri->get_white(i) - cblacksom[i]) / static_cast<float>(raw.expos)
                     + cblacksom[i];
    }

    float gain = calculate_scale_mul(new_scale_mul, new_pre_mul, c_white, cblacksom,
                                     isMono, ri->get_colors());
                              ...
                              const RAWParams& raw)
{
    ...
    if (true) {
        ...
        //        // old scaling: used a fixed reference gain based on camera (as-shot)
        //        white balance
        //
    ...
    if (d1x) {
        ...
        // if the height is odd we also have to add an additional row to avoid a black
        // line
    ...
    if (fuji) {  // zero image to avoid access to uninitialized values in further
                 // processing because fuji super-ccd processing is not clean...
    ...
    const bool doClip =
        (chmax[0] >= clmax[0] || chmax[1] >= clmax[1] || chmax[2] >= clmax[2])
        && !hrp.hrenabled && hrp.clampOOG;
...
#ifdef _OPENMP
#pragma omp \
    parallel if (!d1x)  // omp disabled for D1x to avoid race conditions (see Issue 1088
                        // http://code.google.com/p/rawtherapee/issues/detail?id=1088)
    {
#endif
...
#pragma omp for schedule(dynamic, 16)
        ...
        for (int ix = 0; ix < imheight; ix++) {
            ...
            if (ri->getSensorType() == ST_BAYER || ri->getSensorType() == ST_FUJI_XTRANS
                || ri->get_colors() == 1 || ri->get_colors() == 3)
            {
                for (int j = 0, jx = sx1; j < imwidth; j++, jx += skip) {
                    ...
                    if (doClip) {
                        // note: as hlmax[] can be larger than CLIP and we can later apply
                        // negative exposure this means that we can clip away local
                        // highlights which actually are not clipped. We have to do that
                        // though as we only check pixel by pixel and don't know if this
                        // will transition into a clipped area, if so we need to clip also
                        // surrounding to make a good colour transition
                        rtot = CLIP(rtot);
                        gtot = CLIP(gtot);
                        btot = CLIP(btot);
                    }
            ...
            if (d1x) {
                transLineD1x(line_red, line_grn, line_blue, ix, image, tran, imwidth,
                             imheight, d1xHeightOdd, doClip);
            } else if (fuji) {
                transLineFuji(line_red, line_grn, line_blue, ix, image, tran, imheight,
                              fw);
            } else {
                transLineStandard(line_red, line_grn, line_blue, ix, image, tran, imwidth,
                                  imheight);
            }
        }
    ...
    if (fuji) {
        int a = ((tran & TR_ROT) == TR_R90 && image->getWidth() % 2 == 0)
                || ((tran & TR_ROT) == TR_R180
                    && image->getHeight() % 2 + image->getWidth() % 2 == 1)
                || ((tran & TR_ROT) == TR_R270 && image->getHeight() % 2 == 0);

        // first row
        for (int j = 1 + a; j < image->getWidth() - 1; j += 2) {
            image->r(0, j) =
                (image->r(1, j) + image->r(0, j + 1) + image->r(0, j - 1)) / 3;
            image->g(0, j) =
                (image->g(1, j) + image->g(0, j + 1) + image->g(0, j - 1)) / 3;
            image->b(0, j) =
                (image->b(1, j) + image->b(0, j + 1) + image->b(0, j - 1)) / 3;
        }
        ...
        for (int i = 1; i < image->getHeight() - 1; i++) {
            for (int j = 2 - (a + i + 1) % 2; j < image->getWidth() - 1; j += 2) {
                ...
                float dh = (std::fabs(image->r(i, j + 1) - image->r(i, j - 1))
                            + std::fabs(image->g(i, j + 1) - image->g(i, j - 1))
                            + std::fabs(image->b(i, j + 1) - image->b(i, j - 1)));
                float dv = (std::fabs(image->r(i + 1, j) - image->r(i - 1, j))
                            + std::fabs(image->g(i + 1, j) - image->g(i - 1, j))
                            + std::fabs(image->b(i + 1, j) - image->b(i - 1, j)));
                float eh = 1.f / (1.f + dh);
                float ev = 1.f / (1.f + dv);
                image->r(i, j) = (eh * (image->r(i, j + 1) + image->r(i, j - 1))
                                  + ev * (image->r(i + 1, j) + image->r(i - 1, j)))
                                 / (2.f * (eh + ev));
                image->g(i, j) = (eh * (image->g(i, j + 1) + image->g(i, j - 1))
                                  + ev * (image->g(i + 1, j) + image->g(i - 1, j)))
                                 / (2.f * (eh + ev));
                image->b(i, j) = (eh * (image->b(i, j + 1) + image->b(i, j - 1))
                                  + ev * (image->b(i + 1, j) + image->b(i - 1, j)))
                                 / (2.f * (eh + ev));
            }
            ...
            if (2 - (a + i + 1) % 2 == 2) {
                image->r(i, 0) =
                    (image->r(i + 1, 0) + image->r(i - 1, 0) + image->r(i, 1)) / 3;
                image->g(i, 0) =
                    (image->g(i + 1, 0) + image->g(i - 1, 0) + image->g(i, 1)) / 3;
                image->b(i, 0) =
                    (image->b(i + 1, 0) + image->b(i - 1, 0) + image->b(i, 1)) / 3;
            }

            // last pixel
            if (2 - (a + i + image->getWidth()) % 2 == 2) {
                image->r(i, image->getWidth() - 1) =
                    (image->r(i + 1, image->getWidth() - 1)
                     + image->r(i - 1, image->getWidth() - 1)
                     + image->r(i, image->getWidth() - 2))
                    / 3;
                image->g(i, image->getWidth() - 1) =
                    (image->g(i + 1, image->getWidth() - 1)
                     + image->g(i - 1, image->getWidth() - 1)
                     + image->g(i, image->getWidth() - 2))
                    / 3;
                image->b(i, image->getWidth() - 1) =
                    (image->b(i + 1, image->getWidth() - 1)
                     + image->b(i - 1, image->getWidth() - 1)
                     + image->b(i, image->getWidth() - 2))
                    / 3;
            }
        }
        ...
        // last row
        int offset =
            (a == 1 && image->getHeight() % 2) || (a == 0 && image->getHeight() % 2 == 0);

        for (int j = 1 + offset; j < image->getWidth() - 1; j += 2) {
            image->r(image->getHeight() - 1, j) =
                (image->r(image->getHeight() - 2, j)
                 + image->r(image->getHeight() - 1, j + 1)
                 + image->r(image->getHeight() - 1, j - 1))
                / 3;
            image->g(image->getHeight() - 1, j) =
                (image->g(image->getHeight() - 2, j)
                 + image->g(image->getHeight() - 1, j + 1)
                 + image->g(image->getHeight() - 1, j - 1))
                / 3;
            image->b(image->getHeight() - 1, j) =
                (image->b(image->getHeight() - 2, j)
                 + image->b(image->getHeight() - 1, j + 1)
                 + image->b(image->getHeight() - 1, j - 1))
                / 3;
        }
    }
...
DCPProfile* RawImageSource::getDCP(const ColorManagementParams& cmp,
                                   DCPProfileApplyState& as)
{
    ...
    cmsHPROFILE dummy;
    findInputProfile(cmp.inputProfile, nullptr,
                     (static_cast<const FramesData*>(getMetaData()))->getCamera(),
                     fileName, &dcpProf, dummy);
    ...
    dcpProf->setStep2ApplyState(cmp.workingProfile, cmp.toneCurve, cmp.applyLookTable,
                                cmp.applyBaselineExposureOffset, as);
    return dcpProf;
}

void RawImageSource::convertColorSpace(Imagefloat* image,
                                       ...
                                       const ColorTemp& wb)
{
    ...
    if (!findInputProfile(cmp.inputProfile, embProfile,
                          (static_cast<const FramesData*>(getMetaData()))->getCamera(),
                          fileName, &dcpProf, in))
    {
        return;
    ...
    colorSpaceConversion_(image, cmp, wb, pre_mul, camProfile, imatrices.xyz_cam, in,
                          dcpProf);
}
...
void RawImageSource::computeFullSize(const RawImage* ri,
                                     int tr,
                                     int& w,
                                     int& h,
                                     int border)
...
int RawImageSource::load(const Glib::ustring& fname, bool firstFrameOnly)
{
    ...
    numFrames =
        firstFrameOnly && (numFrames < 7 || !isHasselblad) ? 1 : ri->getFrameCount();
...
#pragma omp critical
    ...
    if (numFrames > 1) {  // this disables multi frame support for Fuji S5 until I found a
                          // solution to handle different dimensions
        if (riFrames[0]->get_width() != riFrames[1]->get_width()
            || riFrames[0]->get_height() != riFrames[1]->get_height())
        {
            numFrames = 1;
        }
    }
    ...
    for (int i = 0; i < 3; i++)
        for (int j = 0; j < 3; j++) {
            imatrices.rgb_cam[i][j] =
                ri->get_colors() == 1 ? (i == j) : ri->get_rgb_cam(i, j);
        }
    ...
    camProfile =
        ICCStore::getInstance()->createFromMatrix(imatrices.xyz_cam, false, "Camera");
    inverse33(imatrices.xyz_cam, imatrices.cam_xyz);

    // First we get the "as shot" ("Camera") white balance and store it
    float pre_mul[4];
    // FIXME: get_colorsCoeff not so much used nowadays, when we have
    // calculate_scale_mul() function here
    ri->get_colorsCoeff(pre_mul, scale_mul, c_black, false);  // modify  for black level
    camInitialGain = max(scale_mul[0], scale_mul[1], scale_mul[2], scale_mul[3])
                     / min(scale_mul[0], scale_mul[1], scale_mul[2], scale_mul[3]);
    ...
    double cam_r = imatrices.rgb_cam[0][0] * camwb_red
                   + imatrices.rgb_cam[0][1] * camwb_green
                   + imatrices.rgb_cam[0][2] * camwb_blue;
    double cam_g = imatrices.rgb_cam[1][0] * camwb_red
                   + imatrices.rgb_cam[1][1] * camwb_green
                   + imatrices.rgb_cam[1][2] * camwb_blue;
    double cam_b = imatrices.rgb_cam[2][0] * camwb_red
                   + imatrices.rgb_cam[2][1] * camwb_green
                   + imatrices.rgb_cam[2][2] * camwb_blue;
    camera_wb =
        ColorTemp(cam_r, cam_g, cam_b, 1., ColorTemp::DEFAULT_OBSERVER);  // as shot WB
    ...
    /*{
        ...
        camwb_red = imatrices.cam_rgb[0][0]*r + imatrices.cam_rgb[0][1]*g +
    imatrices.cam_rgb[0][2]*b; camwb_green = imatrices.cam_rgb[1][0]*r +
    imatrices.cam_rgb[1][1]*g + imatrices.cam_rgb[1][2]*b; camwb_blue =
    imatrices.cam_rgb[2][0]*r + imatrices.cam_rgb[2][1]*g + imatrices.cam_rgb[2][2]*b;
        ...
        for (int c = 0; c < 3; c++) {
            ...
            scale_mul[c] = pre_mul[c] * 65535.0 / sat;
                ...
                scale_mul[3] = pre_mul[1] * 65535.0 / (ri->get_white(3) -
    ri->get_cblack(3)); initialGain = 1.0 / min(pre_mul[0], pre_mul[1], pre_mul[2]);
    }*/
    ...
    plistener = nullptr;  // This must be reset, because only load() is called through
                          // progressConnector
    t2.set();
                                ...
                                bool prepareDenoise)
{
    ...
    {
        // Recalculate the scaling coefficients, using auto WB if selected in the
        // Preprocess WB param. Auto WB gives us better demosaicing and CA auto-correct
        ...
        ri->get_colorsCoeff(ref_pre_mul, scale_mul, dummy_cblk,
                            raw.preprocessWB.mode == RAWParams::PreprocessWB::Mode::AUTO);
        ...
        initialGain = max(scale_mul[0], scale_mul[1], scale_mul[2], scale_mul[3])
                      / min(scale_mul[0], scale_mul[1], scale_mul[2], scale_mul[3]);

        const double ref_r = imatrices.rgb_cam[0][0] * refwb_red
                             + imatrices.rgb_cam[0][1] * refwb_green
                             + imatrices.rgb_cam[0][2] * refwb_blue;
        const double ref_g = imatrices.rgb_cam[1][0] * refwb_red
                             + imatrices.rgb_cam[1][1] * refwb_green
                             + imatrices.rgb_cam[1][2] * refwb_blue;
        const double ref_b = imatrices.rgb_cam[2][0] * refwb_red
                             + imatrices.rgb_cam[2][1] * refwb_green
                             + imatrices.rgb_cam[2][2] * refwb_blue;
        const ColorTemp ReferenceWB =
            ColorTemp(ref_r, ref_g, ref_b, 1., ColorTemp::DEFAULT_OBSERVER);

        if (settings->verbose) {
            printf(
                "Raw Reference white balance: temp %f, tint %f, multipliers [%f %f %f | %f %f %f]\n",
                ReferenceWB.getTemp(), ReferenceWB.getGreen(), ref_r, ref_g, ref_b,
                refwb_red, refwb_blue, refwb_green);
        }
    }
    ...
    if (!raw.df_autoselect) {
        ...
        rid = DFManager::getInstance().searchDarkFrame(
            idata->getMake(), idata->getModel(), idata->getISOSpeed(),
            idata->getShutterSpeed(), idata->getDateTimeAsTS());
    }
    ...
    if (ri->zeroIsBad()
        || (getMetaData()->hasFixBadPixelsConstant()
            && getMetaData()->getFixBadPixelsConstant() == 0))
    {  // mark all pixels with value zero as bad, has to be called before FF and DF. dcraw
       // sets this flag only for some cameras (mainly Panasonic and Leica)
    ...
    if (!raw.ff_AutoSelect) {
        ...
        rif = ffm.searchFlatField(idata->getMake(), idata->getModel(), idata->getLens(),
    ...
    if (numFrames == 4) {
        ...
        for (unsigned int i = 0; i < 4; ++i) {
            if (i == currFrame) {
                copyOriginalPixels(raw, ri, rid, rif, rawData, reddeha, greendeha,
                                   bluedeha);
                ...
                ++bufferNumber;
                copyOriginalPixels(raw, riFrames[i].get(), rid, rif, *rawDataFrames[i],
                                   reddeha, greendeha, bluedeha);
            }
        }
        ...
        copyOriginalPixels(raw, riFrames[1].get(), rid, rif, *rawDataFrames[1], reddeha,
                           greendeha, bluedeha);
    ...
    const std::vector<badPix>* bp = DFManager::getInstance().getBadPixels(
        ri->get_maker(), ri->get_model(), idata->getSerialNumber());
    ...
    if (bp) {
        ...
        if (settings->verbose) {
            std::cout << "Correcting " << bp->size() << " pixels from .badpixels"
                      << std::endl;
        }
    }
    ...
    if (raw.df_autoselect) {
        bp = DFManager::getInstance().getHotPixels(
            idata->getMake(), idata->getModel(), idata->getISOSpeed(),
            idata->getShutterSpeed(), idata->getDateTimeAsTS());
    ...
    if (bp) {
        ...
        if (settings->verbose && !bp->empty()) {
            std::cout << "Correcting " << bp->size() << " hotpixels from darkframe"
                      << std::endl;
        }
    }
    ...
    if (numFrames == 4) {
        ...
        scaleColors(0, 0, W, H, raw,
                    rawData);  //+ + raw parameters for black level(raw.blackxx)
    }

    // Correct vignetting of lens profile
    if (!hasFlatField && lensProf.useVign
        && lensProf.lcMode != LensProfParams::LcMode::NONE)
    {
        ...
        if (lensProf.useMetadata()) {
            ...
            pmap = LFDatabase::getInstance()->findModifier(lensProf, idata, W, H, coarse,
                                                           -1);
        } else {
            const std::shared_ptr<LCPProfile> pLCPProf =
                LCPStore::getInstance()->getProfile(lensProf.lcpFile);

            if (pLCPProf)
            {  // don't check focal length to allow distortion correction for lenses
               // without chip, also pass dummy focal length 1 in case of 0
                pmap.reset(new LCPMapper(pLCPProf, max(idata->getFocalLen(), 1.0),
                                         idata->getFocalLen35mm(), idata->getFocusDist(),
                                         idata->getFNumber(), true, false, W, H, coarse,
                                         -1));
            }
        }
    ...
    if (ri->getSensorType() == ST_BAYER
        && (raw.hotPixelFilter > 0 || raw.deadPixelFilter > 0))
    {
        if (plistener) {
        ...
        int nFound = findHotDeadPixels(*bitmapBads, raw.hotdeadpix_thresh,
                                       raw.hotPixelFilter, raw.deadPixelFilter);
        totBP += nFound;
    ...
    // check if green equilibration is needed. If yes, compute G channel pre-compensation
    // factors
    const auto globalGreenEq = [&]() -> bool {
        CameraConstantsStore* ccs = CameraConstantsStore::getInstance();
        const CameraConst* cc =
            ccs->get(ri->get_maker().c_str(), ri->get_model().c_str());
    ...
    if (ri->getSensorType() == ST_BAYER && raw.bayersensor.linenoise > 0) {
        ...
        if (raw.bayersensor.linenoiseDirection
            == RAWParams::BayerSensor::LineNoiseDirection::PDAF_LINES)
        {
            PDAFLinesFilter f(ri);
        ...
        cfa_linedn(0.00002 * (raw.bayersensor.linenoise),
                   int(raw.bayersensor.linenoiseDirection)
                       & int(RAWParams::BayerSensor::LineNoiseDirection::VERTICAL),
                   int(raw.bayersensor.linenoiseDirection)
                       & int(RAWParams::BayerSensor::LineNoiseDirection::HORIZONTAL),
                   *line_denoise_rowblender);
    }

    if ((raw.ca_autocorrect || std::fabs(raw.cared) > 0.001
         || std::fabs(raw.cablue) > 0.001)
        && ri->getSensorType() == ST_BAYER)
    {  // Auto CA correction disabled for X-Trans, for now...
        if (plistener) {
        ...
        if (numFrames == 4) {
            double fitParams[64];
            float* buffer =
                CA_correct_RT(raw.ca_autocorrect, raw.caautoiterations, raw.cared,
                              raw.cablue, raw.ca_avoidcolourshift, raw.bayersensor.border,
                              *rawDataFrames[0], fitParams, false, true, nullptr, false,
                              options.chunkSizeCA, options.measure);
            for (int i = 1; i < 3; ++i) {
                CA_correct_RT(raw.ca_autocorrect, raw.caautoiterations, raw.cared,
                              raw.cablue, raw.ca_avoidcolourshift, raw.bayersensor.border,
                              *rawDataFrames[i], fitParams, true, false, buffer, false,
                              options.chunkSizeCA, options.measure);
            }

            CA_correct_RT(raw.ca_autocorrect, raw.caautoiterations, raw.cared, raw.cablue,
                          raw.ca_avoidcolourshift, raw.bayersensor.border,
                          *rawDataFrames[3], fitParams, true, false, buffer, true,
                          options.chunkSizeCA, options.measure);
        } else {
            CA_correct_RT(raw.ca_autocorrect, raw.caautoiterations, raw.cared, raw.cablue,
                          raw.ca_avoidcolourshift, raw.bayersensor.border, rawData,
                          nullptr, false, false, nullptr, true, options.chunkSizeCA,
                          options.measure);
        }
    }
    ...
    if (prepareDenoise) {
        ...
        ImProcFunctions::getAutoExp(aehist, aehistcompr, clip, dirpyrdenoiseExpComp,
                                    brightness, contrast, black, hlcompr, hlcomprthresh);
    }
                              ...
                              bool cache)
{
    ...
    if (ri->getSensorType() == ST_BAYER) {
        ...
        {
            hphd_demosaic();
        } else if (raw.bayersensor.method
                   == RAWParams::BayerSensor::getMethodString(
                       RAWParams::BayerSensor::Method::VNG4))
        {
                       ...
                       RAWParams::BayerSensor::Method::AMAZE))
        {
            amaze_demosaic_RT(0, 0, W, H, rawData, red, green, blue,
                              options.chunkSizeAMAZE, options.measure);
        } else if (raw.bayersensor.method
                       == RAWParams::BayerSensor::getMethodString(
                           RAWParams::BayerSensor::Method::AMAZEBILINEAR)
                   || raw.bayersensor.method
                          == RAWParams::BayerSensor::getMethodString(
                              RAWParams::BayerSensor::Method::AMAZEVNG4)
                   || raw.bayersensor.method
                          == RAWParams::BayerSensor::getMethodString(
                              RAWParams::BayerSensor::Method::DCBBILINEAR)
                   || raw.bayersensor.method
                          == RAWParams::BayerSensor::getMethodString(
                              RAWParams::BayerSensor::Method::DCBVNG4)
                   || raw.bayersensor.method
                          == RAWParams::BayerSensor::getMethodString(
                              RAWParams::BayerSensor::Method::RCDBILINEAR)
                   || raw.bayersensor.method
                          == RAWParams::BayerSensor::getMethodString(
                              RAWParams::BayerSensor::Method::RCDVNG4))
        {
            if (!autoContrast) {
                double threshold = raw.bayersensor.dualDemosaicContrast;
                dual_demosaic_RT(true, raw, W, H, rawData, red, green, blue, threshold,
                                 false);
            } else {
                dual_demosaic_RT(true, raw, W, H, rawData, red, green, blue,
                                 contrastThreshold, true);
            }
        } else if (raw.bayersensor.method
                   == RAWParams::BayerSensor::getMethodString(
                       RAWParams::BayerSensor::Method::PIXELSHIFT))
        {
            pixelshift(0, 0, W, H, raw, currFrame, ri->get_maker(), ri->get_model(),
                       raw.expos);
        } else if (raw.bayersensor.method
                   == RAWParams::BayerSensor::getMethodString(
                       RAWParams::BayerSensor::Method::DCB))
        {
            dcb_demosaic(raw.bayersensor.dcb_iterations, raw.bayersensor.dcb_enhance);
                   ...
                   == RAWParams::BayerSensor::getMethodString(
                       RAWParams::BayerSensor::Method::EAHD))
        {
            eahd_demosaic();
        } else if (raw.bayersensor.method
                   == RAWParams::BayerSensor::getMethodString(
                       RAWParams::BayerSensor::Method::IGV))
        {
            igv_interpolate(W, H);
                   ...
                   == RAWParams::BayerSensor::getMethodString(
                       RAWParams::BayerSensor::Method::LMMSE))
        {
            lmmse_interpolate_omp(W, H, rawData, red, green, blue,
                                  raw.bayersensor.lmmse_iterations);
        } else if (raw.bayersensor.method
                   == RAWParams::BayerSensor::getMethodString(
                       RAWParams::BayerSensor::Method::FAST))
        {
            fast_demosaic();
        } else if (raw.bayersensor.method
                   == RAWParams::BayerSensor::getMethodString(
                       RAWParams::BayerSensor::Method::MONO))
        {
            nodemosaic(true);
                   ...
                   == RAWParams::BayerSensor::getMethodString(
                       RAWParams::BayerSensor::Method::RCD))
        {
                ...
                RAWParams::XTransSensor::Method::FAST))
        {
            fast_xtrans_interpolate(rawData, red, green, blue);
                   ...
                   == RAWParams::XTransSensor::getMethodString(
                       RAWParams::XTransSensor::Method::ONE_PASS))
        {
                       ...
                       RAWParams::XTransSensor::Method::THREE_PASS))
        {
            xtrans_interpolate(3, true, options.chunkSizeXT, options.measure);
                       ...
                       == RAWParams::XTransSensor::getMethodString(
                           RAWParams::XTransSensor::Method::FOUR_PASS)
                   || raw.xtranssensor.method
                          == RAWParams::XTransSensor::getMethodString(
                              RAWParams::XTransSensor::Method::TWO_PASS))
        {
            if (!autoContrast) {
                double threshold = raw.xtranssensor.dualDemosaicContrast;
                dual_demosaic_RT(false, raw, W, H, rawData, red, green, blue, threshold,
                                 false);
            } else {
                dual_demosaic_RT(false, raw, W, H, rawData, red, green, blue,
                                 contrastThreshold, true);
            }
        } else if (raw.xtranssensor.method
                   == RAWParams::XTransSensor::getMethodString(
...
// void RawImageSource::retinexPrepareBuffers(ColorManagementParams cmp, RetinexParams
// retinexParams, multi_array2D<float, 3> &conversionBuffer, LUTu &lhist16RETI)
void RawImageSource::retinexPrepareBuffers(const ColorManagementParams& cmp,
                                           const RetinexParams& retinexParams,
                                           multi_array2D<float, 4>& conversionBuffer,
                                           LUTu& lhist16RETI)
{
    bool useHsl = (retinexParams.retinexcolorspace == "HSLLOG"
                   || retinexParams.retinexcolorspace == "HSLLIN");
    ...
    */
    /*
    if (retinexParams.highlig < 100 && retinexParams.retinexMethod == "highliplus") {//try
    to recover magenta...very difficult ! float hig =
    ((float)retinexParams.highlig)/100.f; float higgb = ((float)retinexParams.grbl)/100.f;
            ...
            for (int i = border; i < H - border; i++) {
                for (int j = border; j < W - border; j++) {
                    ...
                    //empirical method to find highlight magenta with no conversion RGB
    and no white balance
                    //red = master   Gr and Bl default higgb=0.5
         //           if (R_>65535.f*hig  && G_ > 65535.f*higgb && B_ > 65535.f*higgb)
    conversionBuffer[3][i - border][j - border] = R_;
          //          else conversionBuffer[3][i - border][j - border] = 0.f;
                }
            }
    }
    */
    if (retinexParams.gammaretinex != "none" && retinexParams.str != 0 && retinexgamtab)
    {  // gamma
...
#pragma omp for
            ...
            for (int i = border; i < H - border; i++) {
                ...
                for (; j < W - border - 3; j += 4) {
                    vfloat H, S, L;
                    Color::rgb2hsl(LVFU(red[i][j]), LVFU(green[i][j]), LVFU(blue[i][j]),
                                   H, S, L);
                    ...
                    if (lhist16RETI) {
                        for (int p = 0; p < 4; p++) {
                            int pos = (conversionBuffer[2][i - border]
                                                       [j - border
                                                        + p]);  // histogram in curve HSL
                            lhist16RETIThr[pos]++;
                        }
                    }
                }
                ...
                for (; j < W - border; j++) {
                    float L;
                    // rgb=>lab
                    Color::rgb2hslfloat(red[i][j], green[i][j], blue[i][j],
                                        conversionBuffer[0][i - border][j - border],
                                        conversionBuffer[1][i - border][j - border], L);
                    L *= 32768.f;
...
#pragma omp critical
        ...
        // Conversion rgb -> lab is hard to vectorize because it uses a lut (that's not
        // the main problem) and it uses a condition inside XYZ2Lab which is almost
        // impossible to vectorize without making it slower...
#ifdef _OPENMP
#pragma omp parallel
#endif
        {
...
#pragma omp for schedule(dynamic, 16)
            ...
            for (int i = border; i < H - border; i++)
                for (int j = border; j < W - border; j++) {
                    ...
                    //                   if (R_>40000.f  && G_ > 30000.f && B_ > 30000.f)
                    //                   conversionBuffer[3][i - border][j - border] = R_;
                    //                   else conversionBuffer[3][i - border][j - border]
                    //                   = 0.f;
                    if (lhist16RETI) {
                        int pos = L;
...
void RawImageSource::retinexPrepareCurves(
    const RetinexParams& retinexParams,
    LUTf& cdcurve,
    LUTf& mapcurve,
    ...
    LUTu& histLRETI)
{
    useHsl = (retinexParams.retinexcolorspace == "HSLLOG"
              || retinexParams.retinexcolorspace == "HSLLIN");

    if (useHsl) {
        retinexcontlutili = CurveFactory::diagonalCurve2Lut(
            retinexParams.cdHcurve, cdcurve, 1, lhist16RETI, histLRETI);
    } else {
        retinexcontlutili = CurveFactory::diagonalCurve2Lut(
            retinexParams.cdcurve, cdcurve, 1, lhist16RETI, histLRETI);
    }

    mapcontlutili = CurveFactory::diagonalCurve2Lut(retinexParams.mapcurve, mapcurve, 1,
                                                    lhist16RETI, histLRETI);
    mapcurve *= 0.5f;
    retinexParams.getCurves(retinextransmissionCurve, retinexgaintransmissionCurve);
}

void RawImageSource::retinex(
    const ColorManagementParams& cmp,
    const RetinexParams& deh,
    const ToneCurveParams& Tc,
    LUTf& cdcurve,
    LUTf& mapcurve,
...
#pragma omp critical
    ...
    if (hist16RET) {  // update histogram
        // TODO : When rgbcurvesspeedup branch is merged into master, replace this by the
        // following 1-liner hist16RET.compressTo(histLRETI); also remove declaration and
        // init of dLcurve some lines above then and finally remove this comment :)
    ...
    MSR(LBuffer, conversionBuffer[2], conversionBuffer[3], mapcurve, mapcontlutili, WNew,
        HNew, deh, dehatransmissionCurve, dehagaintransmissionCurve, minCD, maxCD, mini,
        maxi, Tmean, Tsigma, Tmin, Tmax);
...
#pragma omp parallel for
            ...
            for (int i = border; i < H - border; i++) {
                ...
                for (; j < W - border; j++) {
                    ...
                    float valp =
                        (chcurve->getVal(conversionBuffer[3][i - border][j - border])
                         - 0.5f);
...
#pragma omp parallel for
        ...
        for (int i = border; i < H - border; i++) {
            ...
            for (; j < W - border - 3; j += 4) {
                vfloat R, G, B;
                Color::hsl2rgb(LVFU(conversionBuffer[0][i - border][j - border]),
                               LVFU(conversionBuffer[1][i - border][j - border]),
                               LVFU(LBuffer[i - border][j - border]) / c32768, R, G, B);
            ...
            for (; j < W - border; j++) {
                Color::hsl2rgbfloat(conversionBuffer[0][i - border][j - border],
                                    conversionBuffer[1][i - border][j - border],
                                    LBuffer[i - border][j - border] / 32768.f, red[i][j],
                                    green[i][j], blue[i][j]);
            }
        }
        ...
        TMatrix wiprof =
            ICCStore::getInstance()->workingSpaceInverseMatrix(cmp.workingProfile);
...
#pragma omp for
            ...
            for (int i = border; i < H - border; i++) {
                ...
                for (int j = border; j < W - border; j++) {
                    ...
                    if (chutili) {  // c=f(H)
                        float valp =
                            float((chcurve->getVal(Color::huelab_to_huehsv2(HH)) - 0.5f));
                        Chprov1 *= (1.f + 2.f * valp);
                    }

                    float R, G, B;
                    // gamut control : Lab values are in gamut
                    Color::gamutLchonly(HH, sincosval, Lprov1, Chprov1, R, G, B, wip,
                                        highlight, 0.15f, 0.96f);

                    conversionBuffer[0][i - border][j - border] =
                        327.68f * Chprov1 * sincosval.y;
                    conversionBuffer[1][i - border][j - border] =
                        327.68f * Chprov1 * sincosval.x;
...
#pragma omp parallel for
        ...
        for (int i = border; i < H - border; i++) {
            ...
            for (; j < W - border - 3; j += 4) {
                vfloat x_, y_, z_;
                vfloat R, G, B;
                Color::Lab2XYZ(LVFU(LBuffer[i - border][j - border]),
                               LVFU(conversionBuffer[0][i - border][j - border]),
                               LVFU(conversionBuffer[1][i - border][j - border]), x_, y_,
                               z_);
            ...
            for (; j < W - border; j++) {
                float x_, y_, z_;
                float R, G, B;
                Color::Lab2XYZ(LBuffer[i - border][j - border],
                               conversionBuffer[0][i - border][j - border],
                               conversionBuffer[1][i - border][j - border], x_, y_, z_);
 ...
 * subtract dark frame (if present) from current image and apply flat field correction (if
 * present)
 */
void RawImageSource::copyOriginalPixels(const RAWParams& raw,
                                        RawImage* src,
                                        const RawImage* riDark,
                                        RawImage* riFlatFile,
                                        ...
                                        float& bluedeha)
{
    ...
    if (ri->getSensorType() == ST_BAYER || ri->getSensorType() == ST_FUJI_XTRANS) {
        ...
        {  // This works also for xtrans-sensors, because black[0] to black[4] are equal
           // for these
...
#pragma omp parallel for
            ...
            for (int row = 0; row < H; row++) {
                ...
                for (col = 0; col < W - 1; col += 2) {
                    rawData[row][col] =
                        max(src->data[row][col] + black0 - riDark->data[row][col], 0.0f);
                    rawData[row][col + 1] =
                        max(src->data[row][col + 1] + black1 - riDark->data[row][col + 1],
                            0.0f);
                }
        ...
        */
        ...
        if (riDark && W == riDark->get_width() && H == riDark->get_height()) {
            for (int row = 0; row < H; row++) {
                for (int col = 0; col < W; col++) {
                    rawData[row][col] = max(
                        src->data[row][col] + black[0] - riDark->data[row][col], 0.0f);
                }
            }
        } else {
            for (int row = 0; row < H; row++) {
        ...
        if (riDark && W == riDark->get_width() && H == riDark->get_height()) {
            for (int row = 0; row < H; row++) {
                for (int col = 0; col < W; col++) {
                    ...
                    rawData[row][3 * col + 0] =
                        max(src->data[row][3 * col + 0] + black[c4]
                                - riDark->data[row][3 * col + 0],
                            0.0f);
                    rawData[row][3 * col + 1] =
                        max(src->data[row][3 * col + 1] + black[c4]
                                - riDark->data[row][3 * col + 1],
                            0.0f);
                    rawData[row][3 * col + 2] =
                        max(src->data[row][3 * col + 2] + black[c4]
                                - riDark->data[row][3 * col + 2],
                            0.0f);
                }
            }
        } else {
...
void RawImageSource::scaleColors(int winx,
                                 int winy,
                                 int winw,
                                 int winh,
                                 const RAWParams& raw,
                                 array2D<float>& rawData)
{
    ...
    if (getSensorType() == ST_BAYER || getSensorType() == ST_FOVEON) {
        ...
        isMono =
            RAWParams::BayerSensor::getMethodString(RAWParams::BayerSensor::Method::MONO)
            == raw.bayersensor.method;
    } else if (getSensorType() == ST_FUJI_XTRANS) {
        ...
        if (raw.xtranssensor.Dehablackx) {
            ...
            black_lev[3] =
                raw.xtranssensor.blackgreen;  // G2  (set, only used with a Bayer filter)
        }

        isMono = RAWParams::XTransSensor::getMethodString(
                     RAWParams::XTransSensor::Method::MONO)
                 == raw.xtranssensor.method;
    }
    ...
    initialGain = calculate_scale_mul(
        scale_mul, ref_pre_mul, c_white, cblacksom, isMono,
        ri->get_colors());  // recalculate scale colors with adjusted levels

    // fprintf(stderr, "recalc: %f [%f %f %f %f]\n", initialGain, scale_mul[0],
    // scale_mul[1], scale_mul[2], scale_mul[3]);
    for (int i = 0; i < 4; i++) {
...
#pragma omp for nowait
            ...
            for (int row = winy; row < winy + winh; row++) {
                for (int col = winx; col < winx + winw; col++) {
                    ...
                    const int c4 = (c == 1 && !(row & 1))
                                       ? 3
                                       : c;  // four  colors,  0=R, 1=G1, 2=B, 3=G2
                    const float val =
                        max(0.f, rawData[row][col] - cblacksom[c4]) * scale_mul[c4];
                    rawData[row][col] = val;
...
#pragma omp for nowait
            ...
            for (int row = winy; row < winy + winh; row++) {
                for (int col = winx; col < winx + winw; col++) {
                    const float val =
                        max(0.f, rawData[row][col] - cblacksom[0]) * scale_mul[0];
                    rawData[row][col] = val;
...
#pragma omp for nowait
            ...
            for (int row = winy; row < winy + winh; row++) {
                for (int col = winx; col < winx + winw; col++) {
                    ...
                    const float val =
                        max(0.f, rawData[row][col] - cblacksom[c]) * scale_mul[c];
                    rawData[row][col] = val;
                    tmpchmax[c] = max(tmpchmax[c], val);
                }
            }
            ...
            for (int row = winy; row < winy + winh; row++) {
                for (int col = winx; col < winx + winw; col++) {
                    for (int c = 0; c < 3; c++) {  // three colors,  0=R, 1=G,  2=B
                        const float val =
                            max(0.f, rawData[row][3 * col + c] - cblacksom[c])
                            * scale_mul[c];
...
// Thread called part
void RawImageSource::processFalseColorCorrectionThread(Imagefloat* im,
                                                       array2D<float>& rbconv_Y,
                                                       array2D<float>& rbconv_I,
                                                       array2D<float>& rbconv_Q,
                                                       array2D<float>& rbout_I,
                                                       array2D<float>& rbout_Q,
                                                       const int row_from,
                                                       const int row_to)
{
    ...
    convert_row_to_YIQ(im->r(row_from - 1), im->g(row_from - 1), im->b(row_from - 1),
                       rbconv_Y[px], rbconv_I[px], rbconv_Q[px], W);
    convert_row_to_YIQ(im->r(row_from), im->g(row_from), im->b(row_from), rbconv_Y[cx],
                       rbconv_I[cx], rbconv_Q[cx], W);
    ...
    for (int i = row_from; i < row_to; i++) {
        ...
        convert_row_to_YIQ(im->r(i + 1), im->g(i + 1), im->b(i + 1), rbconv_Y[nx],
                           rbconv_I[nx], rbconv_Q[nx], W);
        ...
        pre2[0] = _mm_setr_ps(rbconv_I[px][1], rbconv_Q[px][1], 0, 0),
        pre2[1] = _mm_setr_ps(rbconv_I[cx][1], rbconv_Q[cx][1], 0, 0),
        pre2[2] = _mm_setr_ps(rbconv_I[nx][1], rbconv_Q[nx][1], 0, 0);
        ...
        for (int j = 1; j < W - 2; j += 2) {
            post1[0] = _mm_setr_ps(rbconv_I[px][j + 1], rbconv_Q[px][j + 1], 0, 0),
            post1[1] = _mm_setr_ps(rbconv_I[cx][j + 1], rbconv_Q[cx][j + 1], 0, 0),
            post1[2] = _mm_setr_ps(rbconv_I[nx][j + 1], rbconv_Q[nx][j + 1], 0, 0);
            const auto middle =
                middle4of6(pre2[0], pre2[1], pre2[2], post1[0], post1[1], post1[2]);
            vfloat medianval = median(pre1[0], pre1[1], pre1[2], middle[0], middle[1],
                                      middle[2], middle[3]);
            rbout_I[cx][j] = medianval[0];
            rbout_Q[cx][j] = medianval[1];
            post2[0] = _mm_setr_ps(rbconv_I[px][j + 2], rbconv_Q[px][j + 2], 0, 0),
            post2[1] = _mm_setr_ps(rbconv_I[cx][j + 2], rbconv_Q[cx][j + 2], 0, 0),
            post2[2] = _mm_setr_ps(rbconv_I[nx][j + 2], rbconv_Q[nx][j + 2], 0, 0);
            medianval = median(post2[0], post2[1], post2[2], middle[0], middle[1],
                               middle[2], middle[3]);
        ...
        // median I channel
        for (int j = 1; j < W - 2; j += 2) {
            post1[0] = rbconv_I[px][j + 1], post1[1] = rbconv_I[cx][j + 1],
            post1[2] = rbconv_I[nx][j + 1];
            const auto middle =
                middle4of6(pre2[0], pre2[1], pre2[2], post1[0], post1[1], post1[2]);
            rbout_I[cx][j] = median(pre1[0], pre1[1], pre1[2], middle[0], middle[1],
                                    middle[2], middle[3]);
            post2[0] = rbconv_I[px][j + 2], post2[1] = rbconv_I[cx][j + 2],
            post2[2] = rbconv_I[nx][j + 2];
            rbout_I[cx][j + 1] = median(post2[0], post2[1], post2[2], middle[0],
        ...
        // median Q channel
        for (int j = 1; j < W - 2; j += 2) {
            post1[0] = rbconv_Q[px][j + 1], post1[1] = rbconv_Q[cx][j + 1],
            post1[2] = rbconv_Q[nx][j + 1];
            const auto middle =
                middle4of6(pre2[0], pre2[1], pre2[2], post1[0], post1[1], post1[2]);
            rbout_Q[cx][j] = median(pre1[0], pre1[1], pre1[2], middle[0], middle[1],
                                    middle[2], middle[3]);
            post2[0] = rbconv_Q[px][j + 2], post2[1] = rbconv_Q[cx][j + 2],
            post2[2] = rbconv_Q[nx][j + 2];
            rbout_Q[cx][j + 1] = median(post2[0], post2[1], post2[2], middle[0],
                                        middle[1], middle[2], middle[3]);
            std::swap(pre1, post1);
            std::swap(pre2, post2);
        }
        ...
        // blur i-1th row
        if (i > row_from) {
            convert_to_RGB(im->r(i - 1, 0), im->g(i - 1, 0), im->b(i - 1, 0),
                           rbconv_Y[px][0], rbout_I[px][0], rbout_Q[px][0]);
...
#pragma omp simd
            ...
            for (int j = 1; j < W - 1; j++) {
                float I = (rbout_I[px][j - 1] + rbout_I[px][j] + rbout_I[px][j + 1]
                           + rbout_I[cx][j - 1] + rbout_I[cx][j] + rbout_I[cx][j + 1]
                           + rbout_I[nx][j - 1] + rbout_I[nx][j] + rbout_I[nx][j + 1])
                          * onebynine;
                float Q = (rbout_Q[px][j - 1] + rbout_Q[px][j] + rbout_Q[px][j + 1]
                           + rbout_Q[cx][j - 1] + rbout_Q[cx][j] + rbout_Q[cx][j + 1]
                           + rbout_Q[nx][j - 1] + rbout_Q[nx][j] + rbout_Q[nx][j + 1])
                          * onebynine;
                convert_to_RGB(im->r(i - 1, j), im->g(i - 1, j), im->b(i - 1, j),
                               rbconv_Y[px][j], I, Q);
            }

            convert_to_RGB(im->r(i - 1, W - 1), im->g(i - 1, W - 1), im->b(i - 1, W - 1),
                           rbconv_Y[px][W - 1], rbout_I[px][W - 1], rbout_Q[px][W - 1]);
        }
    }

    // blur last 3 row and finalize H-1th row
    convert_to_RGB(im->r(row_to - 1, 0), im->g(row_to - 1, 0), im->b(row_to - 1, 0),
                   rbconv_Y[cx][0], rbout_I[cx][0], rbout_Q[cx][0]);
#ifdef _OPENMP
#pragma omp simd
#endif
    ...
    for (int j = 1; j < W - 1; j++) {
        ...
        float Q = (rbout_Q[px][j - 1] + rbout_Q[px][j] + rbout_Q[px][j + 1]
                  ...
                  * onebynine;
        convert_to_RGB(im->r(row_to - 1, j), im->g(row_to - 1, j), im->b(row_to - 1, j),
                       rbconv_Y[cx][j], I, Q);
    }

    convert_to_RGB(im->r(row_to - 1, W - 1), im->g(row_to - 1, W - 1),
                   im->b(row_to - 1, W - 1), rbconv_Y[cx][W - 1], rbout_I[cx][W - 1],
                   rbout_Q[cx][W - 1]);
}
...
#pragma omp parallel
    {
        ...
        for (int t = 0; t < steps; t++) {
            ...
            if (tid < nthreads - 1) {
                processFalseColorCorrectionThread(im, buffer[0], buffer[1], buffer[2],
                                                  buffer[3], buffer[4], 1 + tid * blk,
                                                  1 + (tid + 1) * blk);
            } else {
                processFalseColorCorrectionThread(im, buffer[0], buffer[1], buffer[2],
                                                  buffer[3], buffer[4], 1 + tid * blk,
...
#pragma omp barrier
    ...
    for (int t = 0; t < steps; t++) {
        processFalseColorCorrectionThread(im, buffer[0], buffer[1], buffer[2], buffer[3],
                                          buffer[4], 1, im->getHeight() - 1);
    }
...
// gamma is applied before the CMS, correct line fac=lineFac*rawPixel+LineSum after the
// CMS
void RawImageSource::getProfilePreprocParams(cmsHPROFILE in,
                                             float& gammaFac,
                                             float& lineFac,
                                             float& lineSum)
{
    ...
    if (cmsGetProfileInfoASCII(in, cmsInfoCopyright, cmsNoLanguage, cmsNoCountry,
                               copyright, 256)
        > 0)
    {
        if (strstr(copyright, "Phase One") != nullptr) {
            ...
            gammaFac = 0.5;
            lineFac = -0.4;
            lineSum = 1.35;  // determined in reverse by measuring NX an RT developed
                             // colorchecker PNGs
        }
    }
}
...
static void lab2ProphotoRgbD50(float L, float A, float B, float& r, float& g, float& b)
{
    float X;
    float Y;
    float Z;
...
// Converts raw image including ICC input profile to working space - floating point
// version
void RawImageSource::colorSpaceConversion_(Imagefloat* im,
                                           const ColorManagementParams& cmp,
                                           const ColorTemp& wb,
                                           double pre_mul[3],
                                           cmsHPROFILE camprofile,
                                           ...
                                           DCPProfile* dcpProf)
{
    ...
    if (dcpProf != nullptr) {
        // DCP processing
        const DCPProfile::Triple pre_mul_row = { pre_mul[0], pre_mul[1], pre_mul[2] };
        ...
        dcpProf->apply(im, cmp.dcpIlluminant, cmp.workingProfile, wb, pre_mul_row,
                       cam_matrix, cmp.applyHueSatMap);
        return;
    }
    ...
    if (in == nullptr) {
        ...
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++)
                for (int k = 0; k < 3; k++) {
                    mat[i][j] +=
                        work[i][k] * camMatrix[k][j];  // rgb_xyz * imatrices.xyz_cam
                }
...
#pragma omp parallel for
        ...
        /*
          The goal here is to in addition to user-made custom ICC profiles also support
          profiles supplied with other popular raw converters. As curves affect color
          rendering and different raw converters deal with them differently (and few if
          any is as flexible as RawTherapee) we cannot really expect to get the *exact*
          same color rendering here. However we try hard to make the best out of it.

          Third-party input profiles that contain a LUT (usually A2B0 tag) often needs
          some preprocessing, as ICC LUTs are not really designed for dealing with linear
          camera data. Generally one must apply some sort of curve to get efficient use of
          the LUTs. Unfortunately how you should preprocess is not standardized so there
          are almost as many ways as there are software makers, and for each one we have
          to reverse engineer to find out how it has been done. (The ICC files made for RT
          has linear LUTs)

          ICC profiles which only contain the <r,g,b>XYZ tags (ie only a color matrix)
          should (hopefully) not require any pre-processing.

          Some LUT ICC profiles apply a contrast curve and desaturate highlights (to give
          a "film-like" behavior. These will generally work with RawTherapee, but will not
          produce good results when you enable highlight recovery/reconstruction, as that
          data is added linearly on top of the original range. RawTherapee works best with
          linear ICC profiles.
        */

        enum camera_icc_type {
            CAMERA_ICC_TYPE_GENERIC,  // Generic, no special pre-processing required, RTs
                                      // own is this way
            CAMERA_ICC_TYPE_PHASE_ONE,  // Capture One profiles
            CAMERA_ICC_TYPE_LEAF,  // Leaf profiles, former Leaf Capture now in Capture
                                   // One, made for Leaf digital backs
        ...
        float leaf_prophoto_mat[3][3];
        {
            ...
            cmsGetProfileInfoASCII(in, cmsInfoCopyright, cmsNoLanguage, cmsNoCountry,
                                   copyright, 256);
            cmsGetProfileInfoASCII(in, cmsInfoDescription, cmsNoLanguage, cmsNoCountry,
                                   description, 256);
            camera_icc_type = CAMERA_ICC_TYPE_GENERIC;

            // Note: order the identification with the most detailed matching first since
            // the more general ones may also match the more detailed
        ...
        cmsHPROFILE prophoto = ICCStore::getInstance()->workingSpace(
            "ProPhoto");  // We always use Prophoto to apply the ICC profile to minimize
                          // problems with clipping in LUT conversion.
        ...
        if (!working_space_is_prophoto && camera_icc_type == CAMERA_ICC_TYPE_GENERIC) {
            TMatrix toxyz =
                ICCStore::getInstance()->workingSpaceMatrix(cmp.workingProfile);
            TMatrix torgb =
                ICCStore::getInstance()->workingSpaceInverseMatrix("ProPhoto");
            ...
            for (int i = 0; i < 2 && !working_space_is_prophoto; ++i) {
                ...
                for (int j = 0; j < 2; ++j) {
                    if (rgb[j] < 0.f || rgb[j] > 1.f) {
                        ...
                        prophoto =
                            ICCStore::getInstance()->workingSpace(cmp.workingProfile);

                        if (settings->verbose) {
                            std::cout << "colorSpaceConversion_: converting directly to "
                                      << cmp.workingProfile
                                      << " instead of passing through ProPhoto"
                                      << std::endl;
                        }
        ...
        {
            // These profiles have a RGB to Lab cLUT, gives gamma 1.8 output, and expects
            // a "film-like" curve on input
            transform_via_pcs_lab = true;
            separate_pcs_lab_highlights = true;
            // We transform to Lab because we can and that we avoid getting an unnecessary
            // unmatched gamma conversion which we would need to revert.
            hTransform = cmsCreateTransform(in, TYPE_RGB_FLT, nullptr, TYPE_Lab_FLT,
        ...
        default:
            hTransform = cmsCreateTransform(
                in, TYPE_RGB_FLT, prophoto, TYPE_RGB_FLT, INTENT_RELATIVE_COLORIMETRIC,
                cmsFLAGS_NOOPTIMIZE
        ...
        if (hTransform == nullptr) {
            ...
            lcmsMutex->lock();
            hTransform = cmsCreateTransform(camprofile, TYPE_RGB_FLT, prophoto,
        ...
        if (!working_space_is_prophoto) {
            ...
            torgb = ICCStore::getInstance()->workingSpaceInverseMatrix(
                cmp.workingProfile);  // sRGB .. Adobe...Wide...
        }
            ...
            for (int h = 0; h < im->getHeight(); ++h) {
                ...
                for (int w = 0; w < im->getWidth(); ++w) {
                    ...
                    if (maxc <= 1.0) {
                        ...
                        // highlight recovery extend the range past the clip point, which
                        // means we can get values larger than 1.0 here. LUT ICC profiles
                        // only work in the 0-1 range so we scale down to fit and restore
                        // after conversion.
                    ...
                    switch (camera_icc_type) {
                    case CAMERA_ICC_TYPE_PHASE_ONE:
                        // Here we apply a curve similar to Capture One's "Film Standard"
                        // + gamma, the reason is that the LUTs embedded in the ICCs are
                        // designed to work on such input, and if you provide it with a
                        // different curve you don't get as good result. We will revert
                        // this curve after we've made the color transform. However when
                        // we revert the curve, we'll notice that highlight rendering
                        // suffers due to that the LUT transform don't expand well,
                        // therefore we do a less compressed conversion too and mix them,
                        // this gives us the highest quality and most flexible result.
                    ...
                    case CAMERA_ICC_TYPE_LEAF:
                    {
                        // Leaf profiles expect that the camera native RGB has been
                        // converted to Prophoto RGB
                        float newr = leaf_prophoto_mat[0][0] * r
                                     + leaf_prophoto_mat[0][1] * g
                                     + leaf_prophoto_mat[0][2] * b;
                        float newg = leaf_prophoto_mat[1][0] * r
                                     + leaf_prophoto_mat[1][1] * g
                                     + leaf_prophoto_mat[1][2] * b;
                        float newb = leaf_prophoto_mat[2][0] * r
                                     + leaf_prophoto_mat[2][1] * g
                                     + leaf_prophoto_mat[2][2] * b;
                ...
                if (separate_pcs_lab_highlights) {
                    cmsDoTransform(hTransform, hl_buffer.data, hl_buffer.data,
                                   im->getWidth());
                }
                ...
                for (int w = 0; w < im->getWidth(); ++w) {
                    ...
                    if (transform_via_pcs_lab) {
                        ...
                        if (separate_pcs_lab_highlights) {
                            lab2ProphotoRgbD50(hl_buffer.data[3 * w + 0],
                                               hl_buffer.data[3 * w + 1],
                                               hl_buffer.data[3 * w + 2], hr, hg, hb);
                        }
                    } else {
                        r = *(p++);
                        g = *(p++);
                    ...
                    // restore pre-processing and/or add post-processing for the various
                    // ICC types
                    switch (camera_icc_type) {
                    default:
                        break;
                    ...
                    case CAMERA_ICC_TYPE_LEAF:
                    {
                        // note the 1/1.8 gamma, it's the gamma that the profile has
                        // applied, which we must revert before we can revert the curve
                        ...
                        const float mix =
                            0.25;  // may seem a low number, but remember this is linear
                                   // space, mixing starts 2 stops from clipping
                        const float maxc = max(r, g, b);

                        if (maxc > mix) {
                            float fac = (maxc - mix) / (1.0 - mix);
                            fac = sqrtf(sqrtf(fac));  // gamma 0.25 to mix in highlight
                                                      // render relatively quick
                            r = (1.0 - fac) * r + fac * hr;
                            g = (1.0 - fac) * g + fac * hg;
                            b = (1.0 - fac) * b + fac * hb;
                        }
                    ...
                    // If we don't have ProPhoto as chosen working profile, convert. This
                    // conversion is clipless, ie if we convert to a small space such as
                    // sRGB we may end up with negative values and values larger than max.
                    if (!working_space_is_prophoto) {
                    ...
                    // return to the 0.0 - 65535.0 range (with possible negative and > max
                    // values present)
                    r *= 65535.0;
                    g *= 65535.0;
                    b *= 65535.0;
...
bool RawImageSource::findInputProfile(Glib::ustring inProfile,
                                      cmsHPROFILE embedded,
                                      std::string camName,
                                      const Glib::ustring& fileName,
                                      DCPProfile** dcpProf,
...
void RawImageSource::HLRecovery_blend(float* rin,
                                      float* gin,
                                      float* bin,
                                      int width,
                                      float maxval,
                                      float* hlmax)
{
    ...
    constexpr float trans[ColorCount][ColorCount] = { { 1, 1, 1 },
                                                      { 1.7320508, -1.7320508, 0 },
                                                      { -1, -1, 2 } };
    constexpr float itrans[ColorCount][ColorCount] = { { 1, 0.8660254, -0.5 },
                                                       { 1, -0.8660254, -0.5 },
                                                       { 1, 0, 1 } };

    float minpt =
        rtengine::min(hlmax[0], hlmax[1], hlmax[2]);  // min of the raw clip points
...
void RawImageSource::HLRecovery_Luminance(float* rin,
                                          float* gin,
                                          float* bin,
                                          float* rout,
                                          float* gout,
...
void RawImageSource::HLRecovery_CIELab(float* rin,
                                       float* gin,
                                       float* bin,
                                       float* rout,
                                       float* gout,
                                       ...
                                       double cam_xyz[3][3])
{
    ...
    for (int i = 0; i < width; i++) {
        ...
        if (r > maxval || g > maxval || b > maxval) {
            ...
            float fy =
                (yy < 65535.0 ? Color::cachef[yy] / 327.68 : std::cbrt(yy / MAXVALD));
            // compute LCH decomposition of the clipped pixel (only color information,
            // thus C and H will be used)
...
void RawImageSource::hlRecovery(const std::string& method,
                                float* red,
                                float* green,
                                float* blue,
                                int width,
                                float* hlmax)
{
    ...
    if (method == "Luminance") {
        ...
        HLRecovery_CIELab(red, green, blue, red, green, blue, width, 65535.0,
                          imatrices.xyz_cam, imatrices.cam_xyz);
...
void RawImageSource::getAutoExpHistogram(LUTu& histogram, int& histcompr)
{
    ...
    histogram.clear();
    const float refwb[3] = { static_cast<float>(refwb_red / (1 << histcompr)),
                             static_cast<float>(refwb_green / (1 << histcompr)),
                             static_cast<float>(refwb_blue / (1 << histcompr)) };
...
void RawImageSource::getRAWHistogram(LUTu& histRedRaw,
                                     LUTu& histGreenRaw,
                                     LUTu& histBlueRaw)
{
    //    BENCHFUN
    ...
    const float scale =
        maxWhite <= 1.f ? 65535.f
                        : 1.f;  // special case for float raw images in [0.0;1.0] range
    ...
    const bool fourColours =
        ri->getSensorType() == ST_BAYER
        && ((mult[1] != mult[3] || cblacksom[1] != cblacksom[3]) || FC(0, 0) == 3
            || FC(0, 1) == 3 || FC(1, 0) == 3 || FC(1, 1) == 3);
...
#ifdef _OPENMP
    int numThreads;
    // reduce the number of threads under certain conditions to avoid overhead of too many
    // critical regions
...
#pragma omp critical
    ...
    const auto getidx = [&](int c, int i) -> int {
    ...
    if (ri->getSensorType()
        == ST_BAYER)  // since there are twice as many greens, correct for it
        for (int i = 0; i < 256; i++) {
            histGreenRaw[i] >>= 1;
        }
    else if (ri->getSensorType()
             == ST_FUJI_XTRANS)  // since Xtrans has 2.5 as many greens, correct for it
        for (int i = 0; i < 256; i++) {
...
static void histoxyY_low(int bfhitc,
                         int bfwitc,
                         const array2D<float>& xc,
                         const array2D<float>& yc,
                         const array2D<float>& Yc,
                         LUTf& xxx,
                         LUTf& yyy,
                         LUTf& YYY,
                         LUTu& histxy)
{
    ...
    // this "choice" are guided by generally colors who are in nature skin, sky, etc. in
    // those cases "steps" are small
...
#pragma omp for schedule(dynamic, 4) nowait
        ...
        for (int y = 0; y < bfhitc; y++) {
            for (int x = 0; x < bfwitc; x++) {
                ...
                if (xc[y][x] < 0.12f && xc[y][x] > 0.03f && yc[y][x] > 0.1f)
                {  // near Prophoto
                    if (yc[y][x] < 0.2f) {
                        nh = 0;
                        // blue hard
...
static void histoxyY(int bfhitc,
                     int bfwitc,
                     const array2D<float>& xc,
                     const array2D<float>& yc,
                     const array2D<float>& Yc,
                     ...
                     bool purpe)
{
    ...
    // this "choice" are guided by generally colors who are in nature skin, sky, etc. in
    // those cases "steps" are small of course we can change to be more precise purp
    // enable or not purple color in xyY - approximation...
    // enable display cells
    //     int totalpixels = 0;
        ...
        for (int y = 0; y < bfhitc; y++) {
            for (int x = 0; x < bfwitc; x++) {
                ...
                {  // near Prophoto
                    ...
                    if (yc[y][x] < 0.2f && purp) {  // no take into account if purp =
                                                    // false
                        nh = 177;
...
float static studentXY(const array2D<float>& YYcurr,
                       const array2D<float>& reffYY,
                       int sizcurr,
                       int Nc,
                       int tt)
{
    ...
    const float diviY =
        std::sqrt(((somsqueccurrY + somsquecreffY) * (1.f / sizcurr + 1.f / Nc))
                  / (sizcurr + Nc - 2));
    // divisor student
...
void RawImageSource::ItcWB(bool extra,
                           double& tempref,
                           double& greenref,
                           double& tempitc,
                           double& greenitc,
                           ...
                           const ToneCurveParams& hrp)
{
    ...
    This algorithm try to find temperature correlation between 20 to 80 colors between 201
    spectral color and about 20 to 55 color found in the image between 236, I just found
    the idea in the web "correlate with chroma" instead of RGB grey point,but I don't use
    any algo found on the web.

    I have test many many algorithms to find the first one that work :)
    Probably (sure) there are improvement to do...

    I have create a table temperature with temp and white point with 191 values between
    2000K and 15000K we can obviously  change these values, more...with different steps I
    have create a table for tint (green)with 134 values between 0.4 to 4. I have create or
    recuparate and transformed 406 spectral colors from Colorchecker24, others color and
    my 468 colors target, or from web flowers, etc. with a step of 5nm, I think it is
    large enough. I think this value of 265 is now complete: I tested correlation with 60,
    90, 100, 120, 155...better student increase with number of color, but now it seems
    stabilized Of course we can increase this number :)
    ...
    4) then I create for the image an "histogram", but for xyY (CIE 1931 color space or
    CIE 1964) 5) for each pixel (in fact to accelerate only 1/3 for and 1/3 for y), I
    determine for each couple xy, the number of occurrences 6) I sort this result in
    ascending order 7) in option we can sort in another manner to take into account chroma
    : chromax = x - white point x, chromay = y - white point y 8) then I compare this
    result, with spectral data found above in 3) with deltaE (limited to chroma) 9) at
    this point we have xyY values that match Camera temp, and spectral data associated 10)
    then I recalculate RGB values from xyY histogram 11) after, I vary temp, between 2000K
    to 12000K 12) RGB values are recalculated from 10) with RGB multipliers, and then xyY
    are calculated for each temp 13) spectral data choose are recalculated with temp
    between 2000K to 12000K with matrix spectral calculation, that leads to xyY values 14)
    I calculated for each couple xy, Student correlation (without Snedecor test) 15) the
    good result, is the best correlation 16) we have found the best temperature where
    color image and color references are correlate 17) after we pass this value to
    improccoordinator

    18) in a second part if camera green is out, I used an "extra" algorithm
    19) we make vary green between 2 limits
    20) between these green limits, we make slightly vary temp and recalculated RGB
    multipliers 21) with this multipliers for the RGB color find in histogram we
    recalculate xyY 22) we re-adjust references color for these xyY from 20) 23) then find
    all Student correlation for each couple green / temp 24) sort these Student values,
    and choose the minimum 25) then for the 3 better couple "temp / green" choose the one
    where green is nearest from 1.

    Some variables or function are not used, keep in case of
    I have test with cat02 but result are not stable enough ! why ??, therefore cat02
    neutralized This operation is done (actually) 100 times and compare Student
    coefficient, and keep the absolute  minimum, We can probably optimize.... But actually
    the goal is to find the good algorithm !

    I think, this algo is very good in most cases :) ...to verify of course.
    You can used it in images :flowers, landscape, portrait, skin, where illuminants are
    "normal" (daylight, blackbody) You must avoid when illuminant is non standard
    (fluorescent, LED...) and also, when the subject is lost in the image (some target to
    generate profiles).
    ...
    Itcwb_prim : sRGB, Beta rgb (default), XYZcam, JDCmax = Use near Ciexy diagram instead
    of sRGB itcwb_delta : 4 by default can be set between 0 to 5 ==> delta temp to build
    histogram xy - if camera temp is not probably good itcwb_nopurple : false default -
    allow to bypass highlight recovery and inpait opposed when need flowers and not purple
    due to highlights... itcwb_green - adjust green refinement
    */
    // BENCHFUN
    ...
    while (itciterate) {  // loop to find best mix minchrom and studgood and deltaE patch
        ...
        {  // when no input profile found or if user select Camera standard
            if (settings->verbose) {
                printf("Use Camera Dcraw-Matrix and modify rgbloc\n");
            }

            // improvment with new values for redloc, greenloc, blueloc when Camera Dcraw
            // is used
            ...
            for (int i = 0; i < 3; i++)
                for (int j = 0; j < 3; j++)
                    for (int k = 0; k < 3; k++) {
                        mat[i][j] +=
                            iwork[i][k]
                            * imatrices.xyz_cam[k][j];  // rgb_xyz * imatrices.xyz_cam
                    }
...
#pragma omp parallel for
            ...
            for (int y = 0; y < bfh; y++)
                for (int x = 0; x < bfw; x++) {
                    ...
                    float newred = mat[0][0] * redloc[y][x] + mat[0][1] * greenloc[y][x]
                                   + mat[0][2] * blueloc[y][x];
                    float newgreen = mat[1][0] * redloc[y][x] + mat[1][1] * greenloc[y][x]
                                     + mat[1][2] * blueloc[y][x];
                    float newblue = mat[2][0] * redloc[y][x] + mat[2][1] * greenloc[y][x]
                                    + mat[2][2] * blueloc[y][x];
        ...
        constexpr WbGreen gree[134] = {
            // symmetric coefficient between 0.717 and 1.40
            { 0.400, 1.f }, { 0.420, 1.f }, { 0.440, 1.f },
            { 0.460, 1.f }, { 0.480, 1.f }, { 0.500, 1.f },
            { 0.520, 1.f }, { 0.540, 1.f }, { 0.550, 1.f },
            { 0.560, 1.f }, { 0.570, 1.f }, { 0.580, 1.f },
            { 0.590, 1.f }, { 0.600, 1.f }, { 0.610, 1.f },
            { 0.620, 1.f },  // extended range
            { 0.630, 1.f }, { 0.640, 1.f }, { 0.650, 1.f },
            { 0.660, 1.f }, { 0.670, 1.f }, { 0.680, 1.f },
            { 0.690, 1.f }, { 0.700, 1.f }, { 0.714, 1.f },  // usual 2 range
            { 0.727, 1.f }, { 0.741, 1.f }, { 0.755, 1.f },
            { 0.769, 1.f }, { 0.784, 1.f }, { 0.800, 1.f },
            { 0.806, 1.f }, { 0.813, 1.f }, { 0.820, 1.f },  // usual range
            { 0.826, 1.f }, { 0.833, 1.f }, { 0.840, 1.f },
            { 0.847, 1.f }, { 0.855, 1.f }, { 0.862, 1.f },
            { 0.870, 1.f }, { 0.877, 1.f }, { 0.885, 1.f },
            { 0.893, 1.f }, { 0.901, 1.f }, { 0.909, 1.f },
            { 0.917, 1.f }, { 0.926, 1.f }, { 0.935, 1.f },
            { 0.943, 1.f },  // 49 limit low normal
            { 0.952, 1.f }, { 0.962, 1.f }, { 0.971, 1.f },
            { 0.980, 1.f }, { 0.990, 1.f }, { 1.000, 1.f },  // 55 reference
            { 1.010, 1.f }, { 1.020, 1.f }, { 1.030, 1.f },
            { 1.040, 1.f }, { 1.050, 1.f }, { 1.060, 1.f },
            { 1.070, 1.f }, { 1.080, 1.f }, { 1.090, 1.f },
            { 1.100, 1.f }, { 1.110, 1.f }, { 1.120, 1.f },
            { 1.130, 1.f }, { 1.140, 1.f }, { 1.150, 1.f },
            { 1.160, 1.f }, { 1.170, 1.f }, { 1.180, 1.f },
            { 1.190, 1.f }, { 1.200, 1.f }, { 1.210, 1.f },
            { 1.220, 1.f }, { 1.230, 1.f }, { 1.240, 1.f },
            { 1.250, 1.f },  // usual range
            { 1.275, 1.f }, { 1.300, 1.f }, { 1.325, 1.f },
            { 1.350, 1.f }, { 1.375, 1.f }, { 1.400, 1.f },  // usual 2 range
            { 1.425, 1.f }, { 1.450, 1.f }, { 1.475, 1.f },
            { 1.500, 1.f }, { 1.525, 1.f }, { 1.550, 1.f },
            { 1.575, 1.f },  // extended range
            { 1.600, 1.f }, { 1.633, 1.f }, { 1.666, 1.f },
            { 1.700, 1.f }, { 1.733, 1.f }, { 1.766, 1.f },
            { 1.800, 1.f }, { 1.833, 1.f }, { 1.866, 1.f },
            { 1.900, 1.f }, { 1.933, 1.f }, { 1.966, 1.f },
            { 2.000, 1.f }, { 2.033, 1.f }, { 2.066, 1.f },
            { 2.100, 1.f }, { 2.133, 1.f }, { 2.166, 1.f },
            { 2.200, 1.f }, { 2.250, 1.f }, { 2.300, 1.f },
            { 2.350, 1.f }, { 2.400, 1.f }, { 2.450, 1.f },
            { 2.500, 1.f }, { 2.550, 1.f }, { 2.600, 1.f },
            { 2.650, 1.f }, { 2.700, 1.f }, { 2.750, 1.f },
            { 2.800, 1.f }, { 2.850, 1.f }, { 2.900, 1.f },
            { 2.950, 1.f }, { 3.000, 1.f }, { 3.200, 1.f },
            { 3.400, 1.f }, { 3.600, 1.f }, { 3.800, 1.f },
            { 4.000, 1.f },
        };
        ...
        // we can change step to increase precision if need  - also in Colortemp.cc with
        // same changes I don't know how to pass this structure to Colortemp !
        //  X and Z values calculate for each temp between 2000K to  15000K, so no result
        //  after 15000K !
        // of course we can change the step between each temp if need

        constexpr WbTxyz Txyz[191] = {
            // temperature Xwb Zwb 191 values  x wb and y wb are calculated after,  Xwb
            // and Ywb calculated with a spreadsheet
            ...
            { 9401., 0.953081, 1.423213 },  // since 5 2023 I increased the number of temp
                                            // references above 12000K
            ...
            { 12001., 0.960440, 1.601019 },  // since 5 2023 I increased the number of
                                             // temp refrences above 12000K
        ...
        // compatibility 5.9
        constexpr WbTxyz Txyzs[118] = {
            // temperature Xwb Zwb 118 values - same table as in Rawimagesource.cc  x wb
            // and y wb are calculated after
            { 2001., 1.273842, 0.145295 },  { 2101., 1.244008, 0.167533 },
            { 2201., 1.217338, 0.190697 },  { 2301., 1.193444, 0.214632 },
            { 2401., 1.171996, 0.239195 },  { 2501., 1.152883, 0.264539 },
            { 2605., 1.134667, 0.290722 },  { 2655., 1.126659, 0.303556 },
            { 2705., 1.119049, 0.316446 },  { 2755., 1.111814, 0.329381 },
            { 2803., 1.105381, 0.342193 },  { 2856., 1.098258, 0.355599 },
            { 2910., 1.091550, 0.369645 },  { 2960., 1.085649, 0.382655 },
            { 3003., 1.080982, 0.394258 },  { 3050., 1.075727, 0.406057 },
            { 3103., 1.070277, 0.419815 },  { 3153., 1.065384, 0.432769 },
            { 3203., 1.060906, 0.446161 },  { 3250., 1.056535, 0.457806 },
            { 3303., 1.052034, 0.471422 },  { 3353., 1.047990, 0.484218 },
            { 3400., 1.044547, 0.496719 },  { 3450., 1.040667, 0.508891 },
            { 3500., 1.037145, 0.521523 },  { 3550., 1.033783, 0.534090 },
            { 3600., 1.030574, 0.546590 },  { 3650., 1.027510, 0.559020 },
            { 3699., 1.024834, 0.571722 },  { 3801., 1.019072, 0.596102 },
            { 3851., 1.016527, 0.608221 },  { 3902., 1.014244, 0.621136 },
            { 3952., 1.011729, 0.632447 },  { 4002., 0.996153, 0.609518 },
            { 4052., 0.993720, 0.620805 },  { 4102., 0.993908, 0.631520 },
            { 4152., 0.989179, 0.643262 },  { 4202., 0.989283, 0.653999 },
            { 4252., 0.985039, 0.665536 },  { 4302., 0.985067, 0.676288 },
            { 4352., 0.981271, 0.687599 },  { 4402., 0.981228, 0.698349 },
            { 4452., 0.977843, 0.709425 },  { 4502., 0.977736, 0.720159 },
            { 4552., 0.974728, 0.730993 },  { 4602., 0.974562, 0.741698 },
            { 4652., 0.971899, 0.752284 },  { 4702., 0.971681, 0.762949 },
            { 4752., 0.969335, 0.773285 },  { 4802., 0.969069, 0.783899 },
            { 4827., 0.967570, 0.788836 },  { 4852., 0.967011, 0.793982 },
            { 4877., 0.966465, 0.799108 },  { 4902., 0.965933, 0.804214 },
            { 4927., 0.965414, 0.809229 },  { 4952., 0.964908, 0.814366 },
            { 4977., 0.964415, 0.819412 },  { 5002., 0.963934, 0.824438 },
            { 5027., 0.963465, 0.829444 },  { 5052., 0.963008, 0.834429 },
            { 5077., 0.962563, 0.839395 },  { 5102., 0.962129, 0.844339 },
            { 5127., 0.961706, 0.849263 },  { 5152., 0.961294, 0.854166 },
            { 5177., 0.960893, 0.859049 },  { 5202., 0.960501, 0.863911 },
            { 5252., 0.959749, 0.873572 },  { 5302., 0.959313, 0.883815 },
            { 5352., 0.958361, 0.892644 },  { 5402., 0.957903, 0.902793 },
            { 5452., 0.957116, 0.911379 },  { 5502., 0.956639, 0.921431 },
            { 5552., 0.956002, 0.929779 },  { 5602., 0.955509, 0.939728 },
            { 5652., 0.955008, 0.947842 },  { 5702., 0.954502, 0.957685 },
            { 5752., 0.954124, 0.965569 },  { 5802., 0.953608, 0.975303 },
            { 5852., 0.953342, 0.982963 },  { 5902., 0.952818, 0.992584 },
            { 5952., 0.952652, 1.000025 },  { 6002., 0.952122, 1.009532 },
            { 6052., 0.952047, 1.016759 },  { 6102., 0.951514, 1.026149 },
            { 6152., 0.951520, 1.033168 },  { 6202., 0.950985, 1.042439 },
            { 6252., 0.951064, 1.049256 },  { 6302., 0.950530, 1.058406 },
            { 6352., 0.950674, 1.065027 },  { 6402., 0.950143, 1.074055 },
            { 6452., 0.950345, 1.080484 },  { 6502., 0.950201, 1.088097 },
            { 6552., 0.950070, 1.095633 },  { 6602., 0.949952, 1.103094 },
            { 6652., 0.949846, 1.110479 },  { 6702., 0.949752, 1.119138 },
            { 6752., 0.949668, 1.125027 },  { 6802., 0.949596, 1.132190 },
            { 6902., 0.949033, 1.147691 },  { 7002., 0.949402, 1.160129 },
            { 7152., 0.949348, 1.180429 },  { 7301., 0.948896, 1.201432 },
            { 7451., 0.949434, 1.219076 },  { 7601., 0.949099, 1.239061 },
            { 7751., 0.949729, 1.255559 },  { 7901., 0.949498, 1.274460 },
            { 8151., 0.950361, 1.300912 },  { 8301., 0.950253, 1.318464 },
            { 8451., 0.950966, 1.332651 },  { 8601., 0.950941, 1.349261 },
            { 8801., 0.951772, 1.367421 },  { 9001., 0.951969, 1.387639 },
            { 9201., 0.952784, 1.404422 },  { 9401., 0.953081, 1.423213 },
            { 9901., 0.954537, 1.464134 },  { 10501., 0.956321, 1.508623 },
            { 11001., 0.957747, 1.541281 }, { 12001., 0.960440, 1.601019 },
        };
        ...
        // I used them by default to select good spectral values !! but they are changed
        // after
        ...
        const bool isMono = (ri->getSensorType() == ST_FUJI_XTRANS
                             && raw.xtranssensor.method
                                    == RAWParams::XTransSensor::getMethodString(
                                        RAWParams::XTransSensor::Method::MONO))
                            || (ri->getSensorType() == ST_BAYER
        ...
        for (int tt = 0; tt < N_t; ++tt) {
            ...
            if (!oldsampling) {
                ColorTemp(Txyz[tt].Tem, greenitc, 1., "Custom", wbpar.observer)
                    .getMultipliers(r, g, b);
            } else {
                ColorTemp(Txyzs[tt].Tem, greenitc, 1., "Custom", wbpar.observer)
                    .getMultipliers(r, g,
                                    b);  // brings differences with old version 5.9, maybe
                                         // Observer in 5.9, I did not find a solution
            }
            ...
            const float new_pre_mul[4] = { ri->get_pre_mul(0) / rm,
                                           ri->get_pre_mul(1) / gm,
                                           ri->get_pre_mul(2) / bm,
                                           ri->get_pre_mul(3) / gm };
            float new_scale_mul[4];
            const float gain = calculate_scale_mul(new_scale_mul, new_pre_mul, c_white,
                                                   cblacksom, isMono, ri->get_colors());
            rm = new_scale_mul[0] / scale_mul[0] * gain;
            gm = new_scale_mul[1] / scale_mul[1] * gain;
            bm = new_scale_mul[2] / scale_mul[2] * gain;
            rmm[tt] = rm / gm;
        ...
        LUTf xxx(
            siza);  // for color references calculated ==> max in images "like histogram"
        ...
        // call tempxy to calculate for 406 or 201 color references Temp and XYZ with
        // cat02
        double wpx = 0.;

        double wpz = 0.;

        ColorTemp::tempxy(separated, repref, Tx, Ty, Tz, Ta, Tb, TL, TX, TY, TZ, wbpar,
                          ttbeg, ttend, wpx, wpz, WPX,
                          WPZ);  // calculate chroma xy (xyY) for Z known colors on under
                                 // 200 illuminants
        ...
        for (int j = 0; j < Ncr; j++) {
            ...
            /*
                       //display spectral datas
                                           ...
                                           float Z_r = (65535.f * (1.f - xr - yr) * Yr) /
               yr; float Y_r = 65535.f * Yr; float Lr, ar, br; Color::XYZ2Lab(X_r, Y_r,
               Z_r, Lr, ar, br);//it make sense, because known spectral color


                                   printf("Nc=%i repref=%i xxx=%f yyy=%f YY=%f Lr=%f a=%f
               b=%f\n", j, repref, (double) xxx, (double) yyy, (double) YY, (double)
               Lr/327.68f, (double) ar/327.68f, (double) br/327.68f);
            */
        }
        ...
        if (oldsampling == false) {
            ...
            int pas = 2;
            ImProcFunctions::Median_Denoise(redloc, redloc, bfw, bfh, medianTypeL, pas,
                                            false, tmL);
            ImProcFunctions::Median_Denoise(greenloc, greenloc, bfw, bfh, medianTypeL,
                                            pas, false, tmL);
            ImProcFunctions::Median_Denoise(blueloc, blueloc, bfw, bfh, medianTypeL, pas,
                                            false, tmL);
...
#pragma omp parallel for
            ...
            for (int y = 0; y < bfh; ++y) {
                for (int x = 0; x < bfw; ++x) {
                    ...
                    Color::rgbxyz(RR, GG, BB, xc[y][x], yc[y][x], zc[y][x],
                                  wb2);  // use sRGB Adobe Rec2020 ACESp0
                    float X_r = xc[y][x];
                    float Y_r = yc[y][x];
                    float Z_r = zc[y][x];
            ...
            // calculate for this image the mean values for each family of color, near
            // histogram x y (number) xy vary from x 0..0.77  y 0..0.82 neutral values are
            // near x=0.34 0.33 0.315 0.37 y =0.35 0.36 0.34 skin are about x 0.45  0.49 y
            // 0.4 0.47 blue sky x=0.25 y=0.28  and x=0.29 y=0.32
            //  step about 0.02   x 0.32 0.34  y= 0.34 0.36 skin    --  sky x 0.24 0.30 y
            //  0.28 0.32

            if (wbpar.itcwb_nopurple == true)
            {  // since 21 april - change to filter magenta
                purp = false;
            }

            histoxyY(bfhitc, bfwitc, xc, yc, Yc, xxx, yyy, YYY, histxy,
                     purp);  // purp enable,  enable purple color in WB
...
#pragma omp parallel for
                ...
                // return histogram x and y for each temp and in a range of 158 colors
                // (siza)
            }
        }
        ...
        */
        ...
        array2D<float> chronum_curref_reduc(N_t,
                                            sizcurrref);    // new array to improve patch
        ...
        if (oldsampling == true) {
            for (int i = 0; i < sizcu4; ++i) {  // take the max values
                ...
                xx_curref[i][repref] =
                    xxx[Wbhis[siza - (i + 1)].index] / histcurrref[i][repref];
                yy_curref[i][repref] =
                    yyy[Wbhis[siza - (i + 1)].index] / histcurrref[i][repref];
                YY_curref[i][repref] =
                    YYY[Wbhis[siza - (i + 1)].index] / histcurrref[i][repref];
            }
            if (settings->verbose) {
            ...
            for (int nh = 0; nh < sizcu4; ++nh) {
                const float chxy = std::sqrt(SQR(xx_curref[nh][repref] - xwpr)
                                             + SQR(yy_curref[nh][repref] - ywpr));
            ...
            sizcurr2ref =
                rtengine::min(sizcurr2ref, maxval);  // keep about the biggest values,

            for (int i = 0; i < sizcurr2ref; ++i) {
                // is condition chroxy necessary ?
                if (wbchro[sizcu4 - (i + 1)].chrox > 0.1f
                    && wbchro[sizcu4 - (i + 1)].chroy > 0.1f
                    && wbchro[sizcu4 - (i + 1)].chroxy > 0.0f)
                {  // suppress value too far from reference spectral
                    w++;
            ...
            // calculate deltaE xx to find best values of spectrals data - limited to
            // chroma values
            int maxnb = 3;
            ...
            for (int nb = 1; nb <= maxnb; ++nb) {
                for (int i = 0; i < w; ++i) {
                    ...
                    for (int j = 0; j < Ncr; j++) {
                        if (!good_spectral[j]) {
                            const float deltaE = SQR(xx_curref_reduc[i][repref]
                                                     - reff_spect_xx_camera[j][repref])
                                                 + SQR(yy_curref_reduc[i][repref]
                                                       - reff_spect_yy_camera[j][repref]);
                    ...
                    {  // display in console for 5.9
                        ...
                        float dE = sqrt(SQR(xx_curref_reduc[i][repref]
                                            - reff_spect_xx_camera[kN][repref])
                                        + SQR(yy_curref_reduc[i][repref]
                                              - reff_spect_yy_camera[kN][repref]));
                        dEmean += dE;
                        ndEmean++;
                        ...
                        if (settings->verbose) {
                            ...
                            Color::XYZ2Lab(
                                X_r, Y_r, Z_r, Lr, ar,
                                br);  // it make sense, because known spectral color

                            if (dE > spectlimit) {
                                printf(
                                    "i=%i kn=%i REFLAB for info not used - not relevant Lr=%3.2f ar=%3.2f br=%3.2f \n",
                                    i, kN, (double)(Lr / 327.68f), (double)(ar / 327.68f),
                                    (double)(br / 327.68f));
                                printf(
                                    "IMAGE: kn=%i hist=%7.0f chro_num=%5.1f hue=%2.2f chro=%2.3f xx=%f yy=%f YY=%f\n",
                                    kN, (double)nn_curref_reduc[i][repref],
                                    (double)chronum_curref_reduc[i][repref],
                                    (double)hue_curref_reduc[i][repref],
                                    (double)chro_curref_reduc[i][repref],
                                    (double)xx_curref_reduc[i][repref],
                                    (double)yy_curref_reduc[i][repref],
                                    (double)YY_curref_reduc[i][repref]);
                                printf("kn=%i REfxy xxr=%f yyr=%f YYr=%f\n", kN,
                                       (double)reff_spect_xx_camera[kN][repref],
                                       (double)reff_spect_yy_camera[kN][repref],
                                       (double)reff_spect_Y_camera[kN][repref]);
                    ...
                    good_spectral[kN] = true;  // good spectral are spectral color that
                                               // match color histogram xy
                }
            }
        } else {
            ...
            {  // 20 empirical minimal value default to ensure a correlation
                if (!good_size[j]) {
                    ...
                    for (int nh = 0; nh < j; ++nh) {
                        ...
                        if (ind1 < j && !isponderate) {
                            chxy1 = std::sqrt(SQR(xx_curref[ind1][repref] - xwpr)
                                              + SQR(yy_curref[ind1][repref] - ywpr));
                        }

                        if (ind2 <= 0 && !isponderate) {
                            chxy2 = std::sqrt(SQR(xx_curref[ind2][repref] - xwpr)
                                              + SQR(yy_curref[ind2][repref] - ywpr));
                        }
                        ...
                        wbchro[nh].hue = fmodf(xatan2f(yy_curref[nh][repref] - ywpr,
                                                       xx_curref[nh][repref] - xwpr),
                                               2.f * RT_PI_F);
                        const float chxynum = wbchro[nh].chroxy_number =
                            chxy
                            * pow((double)histcurrref[nh][repref],
                                  0.05);  // sqrt was too big no convergence

                        // We can replace 0.05 by powponder
                        if (ind1 < j && isponderate) {  // with issorted ponderate chroma
                            chxynum1 =
                                chxy1
                                * pow((double)histcurrref[ind1][repref],
                                      0.05);  // 0.05 to 0.1 allows convergence, near 1.5
                                              // betwween max and min value
                        }  // We can replace 0.05 by powponder

                        if (ind2 < 0 && isponderate) {
                            chxynum2 = chxy2
                                       * pow((double)histcurrref[ind2][repref],
                                             0.05);  // We can replace 0.05 by powponder
                        }
                        ...
                        if (isponderate) {
                            ...
                            countchxynum +=
                                pow((double)histcurrref[nh][repref],
                                    0.05);  // no error, to take into account mean value
                                            // //We can replace 0.05 by powponder
                        }
                    }
            ...
            for (int i = index1; i < index2; ++i) {
                if (wbchro[sizcu4 - (i + 1)].number < 400.f)
                {  // remove too low numbers datas about an area 60*60 pixels or reparted
                    indn++;
                }
            }

            Tppat[repref].minhi = (float)rtengine::max(
                (int)wbchro[sizcu4 - (indn + 1)].number, (int)Tppat[repref].minhi);
            ...
            if (settings->verbose) {
                printf(
                    "Info2 - patch estimation of wp displacement (before):j=%i repref=%i real=%i Tppat=%f chrom=%f hue=%f\n",
                    kmin, repref, index2 - indn, (double)Tppat[repref].minchroma,
                    (double)minchrom, (double)estim_hue[kmin][repref]);
            };

            float limexclu = 0.96f;  // to avoid highlight in some rare cases (sky...)

            for (int i = indn; i < index2; ++i) {
                // improvment to limit high Y values wbchro[sizcu4 - (i + 1)].Y < 0.96
                // 0.96 arbitrary high value, maybe 0.9 or 0.98...or 1.0
                if (wbchro[sizcu4 - (i + 1)].chrox > limx
                    && wbchro[sizcu4 - (i + 1)].chroy > limy
                    && wbchro[sizcu4 - (i + 1)].Y < limexclu)
                {         // remove value too far from reference spectral
                    w++;  // w number of real tests
                    xx_curref_reduc[w][repref] = wbchro[sizcu4 - (i + 1)].chrox;
                    yy_curref_reduc[w][repref] = wbchro[sizcu4 - (i + 1)].chroy;
                    YY_curref_reduc[w][repref] = wbchro[sizcu4 - (i + 1)].Y;
                    chronum_curref_reduc[w][repref] =
                        wbchro[sizcu4 - (i + 1)].chroxy_number;
                    nn_curref_reduc[w][repref] = wbchro[sizcu4 - (i + 1)].number;
            ...
            for (int nb = 1; nb <= maxnb; ++nb)
            {  // 1 is good, but 2 3 or 4 help to find more spectral values
                for (int i = 0; i < w; ++i) {
                    float mindeltaE = 100000.f;
                    int kN = 0;
                    ...
                    float spectlimit = settings->itcwb_deltaspec;
                    float dE = sqrt(
                        SQR(xx_curref_reduc[i][repref] - reff_spect_xx_camera[kN][repref])
                        + SQR(yy_curref_reduc[i][repref]
                              - reff_spect_yy_camera[kN][repref]));
                    dEmean += dE;
                    ndEmean++;
                ...
                if (settings->verbose && !oldsampling) {
                    printf(
                        "Patch Mean - Repref=%i deltaE=%f minhisto=%6.0f maxhisto=%7.0f \n",
                        repref, (double)dEmean / ndEmean, (double)minhist,
                        (double)maxhist);
                }
            }
        }
        ...
        for (int i = 0; i < w; i++) {
            const float X = 65535.f * xx_curref_reduc[i][repref]
                            * YY_curref_reduc[i][repref] / yy_curref_reduc[i][repref];
            const float Y = 65535.f * YY_curref_reduc[i][repref];
            const float Z =
                65535.f * (1.f - xx_curref_reduc[i][repref] - yy_curref_reduc[i][repref])
                * YY_curref_reduc[i][repref] / yy_curref_reduc[i][repref];
            float r, g, b;
        ...
        ColorTemp::tempxy(
            separated, repref, Tx, Ty, Tz, Ta, Tb, TL, TX, TY, TZ, wbpar, ttbeg, ttend,
            wpx1, wpz1, WPX,
            WPZ);  // calculate chroma xy (xyY) for Z known colors on under 90 illuminants
        // calculate x y Y
        ...
        for (int tt = ttbeg; tt < ttend; ++tt) {  // N_t
            for (int i = 0; i < w; ++i) {
                ...
                Color::rgbxyY(RR, GG, BB, xxyycurr_reduc[2 * i][tt],
                              xxyycurr_reduc[2 * i + 1][tt], unused, wb);
            }

            for (int j = 0; j < Ncr; ++j) {
                reff_spect_xxyy_prov[2 * j][tt] =
                    std::max(Tx[j][tt] / (Tx[j][tt] + Ty[j][tt] + Tz[j][tt]),
                             0.01f);  // x from xyY
                reff_spect_xxyy_prov[2 * j + 1][tt] =
                    std::max(Ty[j][tt] / (Tx[j][tt] + Ty[j][tt] + Tz[j][tt]),
                             0.01f);  // y from xyY
            }
            ...
            const float abstud = std::fabs(
                studentXY(xxyycurr_reduc, reff_spect_xxyy, 2 * w, 2 * (kk + 1), tt));
        ...
        {
            // always used if extra = true because I made this choice, brings better
            // results
            struct Tempgreen
            {
                float student;
            ...
            int dgoodref =
                rtengine::LIM(newdelta, 1, 10);  // 1.8 increase delta temp scan
            ...
            for (int gr = Rangegreenused.begin; gr < Rangegreenused.end; ++gr) {
                ...
                for (int tt = scantempbeg; tt < scantempend; ++tt) {
                    ...
                    if (!oldsampling) {
                        ColorTemp(Txyz[tt].Tem, gree[gr].green, 1., "Custom",
                                  wbpar.observer)
                            .getMultipliers(r, g, b);
                    } else {
                        ColorTemp(Txyzs[tt].Tem, gree[gr].green, 1., "Custom",
                                  wbpar.observer)
                            .getMultipliers(r, g, b);
                    }

                    float rm = imatrices.cam_rgb[0][0] * r + imatrices.cam_rgb[0][1] * g
                               + imatrices.cam_rgb[0][2] * b;
                    float gm = imatrices.cam_rgb[1][0] * r + imatrices.cam_rgb[1][1] * g
                               + imatrices.cam_rgb[1][2] * b;
                    float bm = imatrices.cam_rgb[2][0] * r + imatrices.cam_rgb[2][1] * g
                               + imatrices.cam_rgb[2][2] * b;
                    ...
                    float new_scale_mul[4];
                    const float gain =
                        calculate_scale_mul(new_scale_mul, new_pre_mul, c_white,
                                            cblacksom, isMono, ri->get_colors());
                ...
                for (int tt = scantempbeg; tt < scantempend; ++tt) {  // N_t
                    ...
                    for (int i = 0; i < Ncr; ++i) {
                        if (good_spectral[i]) {
                            kkg++;
                            reff_spect_xxyy[2 * kkg][tt] =
                                reff_spect_xxyy_prov[2 * i][tt];
                            reff_spect_xxyy[2 * kkg + 1][tt] =
                                reff_spect_xxyy_prov[2 * i + 1][tt];
                        }
                    }

                    // now we have good spectral data
                    // calculate student correlation
                    const float abstudgr = std::fabs(studentXY(
                        xxyycurr_reduc, reff_spect_xxyy, 2 * w, 2 * (kkg + 1), tt));
            ...
            const float xwprf =
                WPX[goodrefgr] / swpr;  // white point for tt in xy coordinates

            const float ywprf = 1.f / swprf;

            for (int nh = 0; nh < w; ++nh) {
                const float chxy =
                    std::sqrt(SQR(xxyycurr_reduc[2 * nh][goodrefgr] - xwprf)
                              + SQR(xxyycurr_reduc[2 * nh + 1][goodrefgr] - ywprf));
            ...
            if (settings->verbose) {
                printf(
                    "New white point calculated patch for information : xwprf=%f ywprf=%f\n",
                    (double)xwprf, (double)ywprf);
                printf(
                    "Info - patch estimation of white-point displacement: chrom=%f hue=%f\n",
            ...
            if (!oldsampling) {
                ...
                // now search the value of green the nearest of 1 with a good student
                // value, I think it is a good choice, perhaps no... I take the first
                // values I admit a symetrie in green coefiicient for rgb
                // multiplier...probably not exactly true perhaps we can used a Snedecor
                // test ? but why...at least we have confidence interval > 90%
                int greengood = 55;

                int maxkgood =
                    3;  // default 3 - we can change ...to test 2, 4, 5, 6. High values
                        // perhaps less good student, but it is a compromise...
                ...
                int mingood = std::min(std::fabs(Tgstud[0].greenref - 55),
                                       std::fabs(Tgstud[1].greenref - 55));

                for (int k = 0; k < maxkgood; ++k) {
                    mingood =
                        std::min(std::fabs(mingood), std::fabs(Tgstud[k].greenref - 55));
                }
                ...
                if (settings->verbose) {
                    ...
                    printf("Student_0=%f Student_k= %f\n", Tgstud[0].student,
                           Tgstud[maxkgood - 1].student);
                    printf("mingood=%i greeng=%i goodref=%i stud=%f\n", mingood,
                           greengood, goodref, (double)studgood);
                }
                ...
                if (((keepgreen >= 0.952 && keepgreen < 1.25) && greengood > 55)
                    && !greenex)
                {
                    double ag = 0.;
                    double gcal =
                        gree[greengood]
                            .green;  // empirical  correction when green suspicious
                ...
                if (((greengood > 41 && keepgreen < 0.7)
                     || (greengood > 46 && keepgreen < 0.952))
                    && !greenex)
                {
                    double ag = 0.;
                    double gcal = gree[greengood].green;
                    ag = 0.95 * (gcal - keepgreen);  // empirical  correction when green
                                                     // low - to improve
                ...
                if (estimchrom < 0.025f) {
                    float ac =
                        -2.40f * estimchrom
                        + 0.06f;  // small empirical  correction, maximum 0.06 if chroma=0
                                  // for all image, currently for very low chroma +0.02
                    greenitc += ac;
                }
        ...
        if (!oldsampling) {
            if (lastitc && nocam == 0 && wbpar.itcwb_alg == false)
            {  // try to find if another tempref
                if ((tempitc < 4000.f || tempitc > 6000.f) || extra == true) {
                    ...
                    if (tempitc < 4000.f)
                    {  // change the second temp to be near of the first one
        ...
        if (!oldsampling) {
            ...
            if (settings->verbose) {
                for (int d = 0; d < 2; d++) {
                    printf(
                        "n=%i nbitc=%i stu=%f minc=%f tempitc=%f greenitc=%f deltaE=%f choiceitc=%i\n",
                        d, nbitc, (double)optitc[d].stud, (double)optitc[d].minc,
                        (double)optitc[d].titc, (double)optitc[d].gritc,
                        (double)optitc[d].delt, choiceitc);
                }
            }
                ...
                && oldsampling == false)
            {
                ...
                if ((std::max(optitc[1].stud, 0.0004f) * optitc[1].delt
                     < std::max(optitc[0].stud, 0.0004f) * optitc[0].delt)
                    && wbpar.itcwb_alg == false)
                {
                    bia = 3;
                } else {
                    bia = 2;
...
void RawImageSource::WBauto(bool extra,
                            double& tempref,
                            double& greenref,
                            array2D<float>& redloc,
                            array2D<float>& greenloc,
                            ...
                            const ToneCurveParams& hrp)
{
    ...
    if (wbpar.method == "autitcgreen") {
        ...
        tempitc = 5000.;
        ItcWB(extra, tempref, greenref, tempitc, greenitc, temp0, delta, bia, dread, kcam,
              nocam, studgood, minchrom, kmin, minhist, maxhist, redloc, greenloc,
              blueloc, bfw, bfh, avg_rm, avg_gm, avg_bm, cmp, raw, wbpar, hrp);
    }
}

void RawImageSource::getrgbloc(int begx,
                               int begy,
                               int yEn,
                               int xEn,
                               int cx,
                               ...
                               const WBParams& wbpar)
{
    ...
    const int bfw =
        W / precision
        + ((W % precision) > 0 ? 1 : 0);  // 5 arbitrary value can be change to 3 or 9 ;
...
#pragma omp parallel for reduction(+ : avgL, nn)
    ...
    for (int i = 0; i < H; i++) {
        for (int j = 0; j < W; j++) {
            const float LL =
                0.299f * red[i][j] + 0.587f * green[i][j] + 0.114f * blue[i][j];
...
#pragma omp parallel for
    ...
    for (int i = 0; i < bfh; ++i) {
        ...
        if (ii < H) {
            ...
            {  // isnan and <0 and > 65535 in case of
...
void RawImageSource::getAutoWBMultipliersitc(bool extra,
                                             double& tempref,
                                             double& greenref,
                                             double& tempitc,
                                             double& greenitc,
...
#pragma omp parallel for reduction(+ : avg_r, avg_g, avg_b, rn, gn, bn) \
    schedule(dynamic, 8)
                ...
                for (int i = 32; i < H - 32; i += 2)
                    for (int j = 32; j < W - 32; j += 2) {
                        // average each Bayer quartet component individually if
                        // non-clipped
                        double d[2][2];
                        d[0][0] = rawData[i][j];
                        d[0][1] = rawData[i][j + 1];
    ...
    if (wbpar.method == "autitcgreen") {
        ...
        const int bfw =
            W / precision
            + ((W % precision) > 0 ? 1
                                   : 0);  // 5 arbitrary value can be change to 3 or 9 ;
        const int bfh = H / precision + ((H % precision) > 0 ? 1 : 0);
        WBauto(extra, tempref, greenref, redloc, greenloc, blueloc, bfw, bfh, avg_rm,
               avg_gm, avg_bm, tempitc, greenitc, temp0, delta, bia, dread, kcam, nocam,
               studgood, minchrom, kmin, minhist, maxhist, twotimes, wbpar, begx, begy,
               yEn, xEn, cx, cy, cmp, raw, hrp);
    }

    if (settings->verbose && wbpar.method != "autitcgreen") {
        printf("RGB grey AVG: %g %g %g\n", avg_r / std::max(1, rn),
               avg_g / std::max(1, gn), avg_b / std::max(1, bn));
    }
    ...
    if (wbpar.method != "autitcgreen") {
        ...
        redAWBMul = rm = imatrices.rgb_cam[0][0] * reds + imatrices.rgb_cam[0][1] * greens
                         + imatrices.rgb_cam[0][2] * blues;
        greenAWBMul = gm = imatrices.rgb_cam[1][0] * reds
                           + imatrices.rgb_cam[1][1] * greens
                           + imatrices.rgb_cam[1][2] * blues;
        blueAWBMul = bm = imatrices.rgb_cam[2][0] * reds
                          + imatrices.rgb_cam[2][1] * greens
                          + imatrices.rgb_cam[2][2] * blues;
    }
}
...
#pragma omp parallel for reduction(+ : avg_r, avg_g, avg_b, rn, gn, bn) \
    ...
    if (settings->verbose) {
        printf("AVG: %g %g %g\n", avg_r / std::max(1, rn), avg_g / std::max(1, gn),
               avg_b / std::max(1, bn));
    }

    //    return ColorTemp (pow(avg_r/rn, 1.0/6.0)*img_r, pow(avg_g/gn, 1.0/6.0)*img_g,
    //    pow(avg_b/bn, 1.0/6.0)*img_b);
    ...
    redAWBMul = rm = imatrices.rgb_cam[0][0] * reds + imatrices.rgb_cam[0][1] * greens
                     + imatrices.rgb_cam[0][2] * blues;
    greenAWBMul = gm = imatrices.rgb_cam[1][0] * reds + imatrices.rgb_cam[1][1] * greens
                       + imatrices.rgb_cam[1][2] * blues;
    blueAWBMul = bm = imatrices.rgb_cam[2][0] * reds + imatrices.rgb_cam[2][1] * greens
                      + imatrices.rgb_cam[2][2] * blues;
}

ColorTemp RawImageSource::getSpotWB(std::vector<Coord2D>& red,
                                    ...
                                    StandardObserver observer)
{
    ...
    if (ri->getSensorType() != ST_BAYER) {
        ...
        for (size_t i = 0; i < red.size(); i++) {
            ...
            transformPosition(green[i].x, green[i].y, tran, x,
                              y);  // these are redundant now ??? if not, repeat for these
                                   // blocks same as for red[]
            rloc = gloc = bloc = rnbrs = gnbrs = bnbrs = 0;
    ...
    if (2u * rn < red.size()) {
        ...
        double rm = imatrices.rgb_cam[0][0] * reds + imatrices.rgb_cam[0][1] * greens
                    + imatrices.rgb_cam[0][2] * blues;
        double gm = imatrices.rgb_cam[1][0] * reds + imatrices.rgb_cam[1][1] * greens
                    + imatrices.rgb_cam[1][2] * blues;
        double bm = imatrices.rgb_cam[2][0] * reds + imatrices.rgb_cam[2][1] * greens
                    + imatrices.rgb_cam[2][2] * blues;
...
{
    double nom = (rgb_cam[0][2] * rgb_cam[1][1] * rgb_cam[2][0]
                  - rgb_cam[0][1] * rgb_cam[1][2] * rgb_cam[2][0]
                  - rgb_cam[0][2] * rgb_cam[1][0] * rgb_cam[2][1]
                  + rgb_cam[0][0] * rgb_cam[1][2] * rgb_cam[2][1]
                  + rgb_cam[0][1] * rgb_cam[1][0] * rgb_cam[2][2]
                  - rgb_cam[0][0] * rgb_cam[1][1] * rgb_cam[2][2]);
    cam_rgb[0][0] = (rgb_cam[1][2] * rgb_cam[2][1] - rgb_cam[1][1] * rgb_cam[2][2]) / nom;
    cam_rgb[0][1] =
        -(rgb_cam[0][2] * rgb_cam[2][1] - rgb_cam[0][1] * rgb_cam[2][2]) / nom;
    cam_rgb[0][2] = (rgb_cam[0][2] * rgb_cam[1][1] - rgb_cam[0][1] * rgb_cam[1][2]) / nom;
    cam_rgb[1][0] =
        -(rgb_cam[1][2] * rgb_cam[2][0] - rgb_cam[1][0] * rgb_cam[2][2]) / nom;
    cam_rgb[1][1] = (rgb_cam[0][2] * rgb_cam[2][0] - rgb_cam[0][0] * rgb_cam[2][2]) / nom;
    cam_rgb[1][2] =
        -(rgb_cam[0][2] * rgb_cam[1][0] - rgb_cam[0][0] * rgb_cam[1][2]) / nom;
    cam_rgb[2][0] = (rgb_cam[1][1] * rgb_cam[2][0] - rgb_cam[1][0] * rgb_cam[2][1]) / nom;
    cam_rgb[2][1] =
        -(rgb_cam[0][1] * rgb_cam[2][0] - rgb_cam[0][0] * rgb_cam[2][1]) / nom;
...
void RawImageSource::init()
{

    {
        ...
        /* This curve is derived from TIFFTAG_TRANSFERFUNCTION of a Capture One P25+ image
           with applied film curve, exported to TIFF with embedded camera ICC. It's
           assumed to be similar to most standard curves in Capture One. It's not
           necessary to be exactly the same, it's just to be close to a typical curve to
           give the Phase One ICC files a good working space. */
        const double phase_one_forward[] = {
            0.0000000000, 0.0000000000, 0.0152590219, 0.0029602502, 0.0305180438,
            0.0058899825, 0.0457770657, 0.0087739376, 0.0610360876, 0.0115968566,
            0.0762951095, 0.0143587396, 0.0915541314, 0.0171969177, 0.1068131533,
            0.0201876860, 0.1220721752, 0.0232852674, 0.1373311971, 0.0264744030,
            0.1525902190, 0.0297245747, 0.1678492409, 0.0330205234, 0.1831082628,
            0.0363775082, 0.1983672847, 0.0397802701, 0.2136263066, 0.0432593271,
            0.2288853285, 0.0467841611, 0.2441443503, 0.0503700313, 0.2594033722,
            0.0540474556, 0.2746623941, 0.0577859159, 0.2899214160, 0.0616159304,
            0.3051804379, 0.0655222400, 0.3204394598, 0.0695353628, 0.3356984817,
            0.0736552987, 0.3509575036, 0.0778973068, 0.3662165255, 0.0822461280,
            0.3814755474, 0.0867170214, 0.3967345693, 0.0913252461, 0.4119935912,
            0.0960860609, 0.4272526131, 0.1009994659, 0.4425116350, 0.1060654612,
            0.4577706569, 0.1113298238, 0.4730296788, 0.1167925536, 0.4882887007,
            0.1224841688, 0.5035477226, 0.1284046693, 0.5188067445, 0.1345540551,
            0.5340657664, 0.1409781033, 0.5493247883, 0.1476615549, 0.5645838102,
            0.1546501869, 0.5798428321, 0.1619287404, 0.5951018540, 0.1695277333,
            0.6103608759, 0.1774776837, 0.6256198978, 0.1858091096, 0.6408789197,
            0.1945525292, 0.6561379416, 0.2037384604, 0.6713969635, 0.2134279393,
            0.6866559854, 0.2236667430, 0.7019150072, 0.2345159075, 0.7171740291,
            0.2460517281, 0.7324330510, 0.2583047227, 0.7476920729, 0.2714122225,
            0.7629510948, 0.2854352636, 0.7782101167, 0.3004959182, 0.7934691386,
            0.3167620356, 0.8087281605, 0.3343862058, 0.8239871824, 0.3535820554,
            0.8392462043, 0.3745937285, 0.8545052262, 0.3977111467, 0.8697642481,
            0.4232547494, 0.8850232700, 0.4515754940, 0.9002822919, 0.4830701152,
            0.9155413138, 0.5190966659, 0.9308003357, 0.5615320058, 0.9460593576,
            0.6136263066, 0.9613183795, 0.6807965209, 0.9765774014, 0.7717402914,
        ...
        for (unsigned int i = 0;
             i < sizeof(phase_one_forward) / sizeof(phase_one_forward[0]); i += 2)
        {
...
{
    if (!checkRawDataDimensions(rawData, *ri, W, H) || d1x)
    {  // Nikon D1x has special sensor. We just skip it
        R = G = B = 0;
        return;
    }
    ...
    xnew = LIM(xnew, 0, W - 1);
    ynew = LIM(ynew, 0, H - 1);
    int c = ri->getSensorType() == ST_FUJI_XTRANS ? ri->XTRANSFC(ynew, xnew)
                                                  : ri->FC(ynew, xnew);
...
bool RawImageSource::isGainMapSupported() const
{
    ...
    for (const auto& m : gainMaps) {
        if (m.MapGain.size() < 1) {
            if (rtengine::settings->verbose) {
                std::cout << "GainMap has invalid size of " << m.MapGain.size()
                          << std::endl;
            }
            return false;
        }
        if (m.MapGain.size()
            != static_cast<std::size_t>(m.MapPointsV)
                   * static_cast<std::size_t>(m.MapPointsH)
                   * static_cast<std::size_t>(m.MapPlanes))
        {
            if (rtengine::settings->verbose) {
                std::cout << "GainMap has size of " << m.MapGain.size() << ", but needs "
                          << m.MapPointsV * m.MapPointsH * m.MapPlanes << std::endl;
            }
            return false;
        }
        if (m.RowPitch != 2 || m.ColPitch != 2) {
            if (rtengine::settings->verbose) {
                std::cout << "GainMap needs Row/ColPitch of 2/2, but has " << m.RowPitch
                          << "/" << m.ColPitch << std::endl;
            }
            return false;
        }
        ...
        for (size_t i = 0; noOp && i < m.MapGain.size(); ++i) {
            if (m.MapGain[i] != 1.f)
            {  // we have at least one value != 1.f => map is not a nop
                noOp = false;
            }
        }
    }
    if (noOp || check != 15)
    {  // all maps are nops or the structure of the combination of 4 maps is not correct
        if (rtengine::settings->verbose) {
            if (noOp) {
...
void RawImageSource::applyDngGainMap(const float black[4],
