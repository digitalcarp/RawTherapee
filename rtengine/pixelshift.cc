...
float nonGreenDiffCross(float right,
                        float left,
                        float top,
                        float bottom,
                        float centre,
...
void paintMotionMask(int index,
                     bool showMotion,
                     float* maskDest,
                     float* nonMaskDest0,
                     float* nonMaskDest1)
...
void invertMask(int xStart,
                int xEnd,
                int yStart,
                int yEnd,
                const array2D<uint8_t>& maskIn,
                array2D<uint8_t>& maskOut)
{
#ifdef _OPENMP
...
void floodFill4Impl(int yin,
                    ...
                    std::stack<std::pair<uint16_t, uint16_t>,
                               std::vector<std::pair<uint16_t, uint16_t>>>& coordStack)
{
...
void calcFrameBrightnessFactor(unsigned int frame,
                               uint32_t datalen,
                               LUTu* histo[4],
                               float brightnessFactor[4])
{
    float medians[4];
...
void RawImageSource::pixelshift(int winx,
                                int winy,
                                int winw,
                                int winh,
                                const procparams::RAWParams& rawParamsIn,
                                unsigned int frame,
                                const std::string& make,
                                const std::string& model,
                                float rawWpCorrection)
{
    ...
    if (bayerParams.pixelShiftMotionCorrectionMethod
        == procparams::RAWParams::BayerSensor::PSMotionCorrectionMethod::AUTO)
    {
    ...
    } else if (bayerParams.pixelShiftMotionCorrectionMethod
               == procparams::RAWParams::BayerSensor::PSMotionCorrectionMethod::OFF)
    {
    ...
    if (motionDetection) {
        if (!showOnlyMask) {
            if (bayerParams.pixelShiftMedian || bayerParams.pixelShiftAverage)
            {  // We need the demosaiced frames for motion correction
                if (bayerParams.pixelShiftDemosaicMethod
                    == bayerParams.getPSDemosaicMethodString(
                        procparams::RAWParams::BayerSensor::PSDemosaicMethod::LMMSE))
                {
                    lmmse_interpolate_omp(winw, winh, *(rawDataFrames[0]), red, green,
                                          blue, bayerParams.lmmse_iterations);
                } else if (bayerParams.pixelShiftDemosaicMethod
                           == bayerParams.getPSDemosaicMethodString(
                               procparams::RAWParams::BayerSensor::PSDemosaicMethod::
                                   AMAZEVNG4))
                {
                    dual_demosaic_RT(true, rawParamsIn, winw, winh, *(rawDataFrames[0]),
                                     red, green, blue, bayerParams.dualDemosaicContrast,
                                     true);
                } else if (bayerParams.pixelShiftDemosaicMethod
                           == bayerParams.getPSDemosaicMethodString(
                               procparams::RAWParams::BayerSensor::PSDemosaicMethod::
                                   RCDVNG4))
                {
                    ...
                    amaze_demosaic_RT(winx, winy, winw, winh, *(rawDataFrames[0]), red,
                                      green, blue, options.chunkSizeAMAZE,
                                      options.measure);
                }
                ...
                for (int i = 0; i < 3; i++) {
                    if (bayerParams.pixelShiftDemosaicMethod
                        == bayerParams.getPSDemosaicMethodString(
                            procparams::RAWParams::BayerSensor::PSDemosaicMethod::LMMSE))
                    {
                        lmmse_interpolate_omp(winw, winh, *(rawDataFrames[i + 1]),
                                              redTmp[i], greenTmp[i], blueTmp[i],
                                              bayerParams.lmmse_iterations);
                    } else if (bayerParams.pixelShiftDemosaicMethod
                               == bayerParams.getPSDemosaicMethodString(
                                   procparams::RAWParams::BayerSensor::PSDemosaicMethod::
                                       AMAZEVNG4))
                    {
                        dual_demosaic_RT(true, rawParamsIn, winw, winh,
                                         *(rawDataFrames[i + 1]), redTmp[i], greenTmp[i],
                                         blueTmp[i], bayerParams.dualDemosaicContrast,
                                         true);
                    } else if (bayerParams.pixelShiftDemosaicMethod
                               == bayerParams.getPSDemosaicMethodString(
                                   procparams::RAWParams::BayerSensor::PSDemosaicMethod::
                                       RCDVNG4))
                    {
                        ...
                        amaze_demosaic_RT(winx, winy, winw, winh, *(rawDataFrames[i + 1]),
                                          redTmp[i], greenTmp[i], blueTmp[i],
                                          options.chunkSizeAMAZE, options.measure);
                    }
                }
...
#pragma omp parallel for schedule(dynamic, 16)
                    ...
                    for (int i = winy + border; i < winh - border; i++) {
                        for (int j = winx + border; j < winw - border; j++) {
                            red[i][j] =
                                median(red[i][j], redTmp[0][i + 1][j],
                                       redTmp[1][i + 1][j + 1], redTmp[2][i][j + 1]);
                        }

                        for (int j = winx + border; j < winw - border; j++) {
                            green[i][j] =
                                median(green[i][j], greenTmp[0][i + 1][j],
                                       greenTmp[1][i + 1][j + 1], greenTmp[2][i][j + 1]);
                        }

                        for (int j = winx + border; j < winw - border; j++) {
                            blue[i][j] =
                                median(blue[i][j], blueTmp[0][i + 1][j],
                                       blueTmp[1][i + 1][j + 1], blueTmp[2][i][j + 1]);
                        }
                    }
                } else {
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic, 16)
                ...
                if (bayerParams.pixelShiftDemosaicMethod
                    == bayerParams.getPSDemosaicMethodString(
                        procparams::RAWParams::BayerSensor::PSDemosaicMethod::LMMSE))
                {
                    lmmse_interpolate_omp(winw, winh, rawData, red, green, blue,
                                          bayerParams.lmmse_iterations);
                } else if (bayerParams.pixelShiftDemosaicMethod
                           == bayerParams.getPSDemosaicMethodString(
                               procparams::RAWParams::BayerSensor::PSDemosaicMethod::
                                   AMAZEVNG4))
                {
                    ...
                    rawParamsTmp.bayersensor.method =
                        procparams::RAWParams::BayerSensor::getMethodString(
                            procparams::RAWParams::BayerSensor::Method::AMAZEVNG4);
                    dual_demosaic_RT(true, rawParamsTmp, winw, winh, rawData, red, green,
                                     blue, bayerParams.dualDemosaicContrast, true);
                } else if (bayerParams.pixelShiftDemosaicMethod
                           == bayerParams.getPSDemosaicMethodString(
                               procparams::RAWParams::BayerSensor::PSDemosaicMethod::
                                   RCDVNG4))
    ...
    // currently we use the same values for K-1 and K-1 Mark II, though for the K-1 Mark
    // II the values seem a bit aggressive
    static const float nReadK1[] = {
        3.45f,  // ISO 100
        3.15f,  // ISO 125
        3.45f,  // ISO 160
        3.0f,   // ISO 200
        ...
        2.4f,   // ISO 160000
        2.4f,   // ISO 204800
        2.4f,   // ISO 256000  // these are for K-1 Mark II to avoid crashes when using
                // high-ISO files
        2.4f,   // ISO 320000
        2.4f,   // ISO 409600
        2.4f,   // ISO 512000
    ...
    // due to integer scaling quantization, B) much different noise behaviour of PDAF
    // pixels
    static const float nReadILCE7RM3[] = {
        4.2f,   // ISO 100
        3.9f,   // ISO 125
        3.6f,   // ISO 160
        3.55f,  // ISO 200
    ...
    if (make.find("Sony") != string::npos) {
        ...
        {  // as long as we don't have values for Pentax KP, we use the values from K-70
    ...
    const float eperIsoRed =
        (eperIso / scale_mul[0]) * (65535.f / (c_white[0] - c_black[0]));
    const float eperIsoGreen =
        (eperIso / scale_mul[1]) * (65535.f / (c_white[1] - c_black[1]));
    const float eperIsoBlue =
        (eperIso / scale_mul[2]) * (65535.f / (c_white[2] - c_black[2]));
...
#pragma omp parallel
#endif
            {
                ...
                for (int i = 0; i < 4; ++i) {
                    histogreenThr[i] =
                        new LUTu(65536, LUT_CLIP_BELOW | LUT_CLIP_ABOVE, true);
                    historedThr[i] =
                        new LUTu(65536, LUT_CLIP_BELOW | LUT_CLIP_ABOVE, true);
                    histoblueThr[i] =
                        new LUTu(65536, LUT_CLIP_BELOW | LUT_CLIP_ABOVE, true);
...
#pragma omp for schedule(dynamic, 16) nowait
                ...
                for (int i = winy + 1; i < winh - 1; ++i) {
                    ...
                    for (int j = winx + 1, offset = fc(cfarray, i, j) & 1; j < winw - 1;
                         ++j, offset ^= 1)
                    {
                        (*histogreenThr[1 - offset])[(
                            *rawDataFrames[1 - offset])[i - offset + 1][j]]++;
                        (*histogreenThr[3 - offset])[(
                            *rawDataFrames[3 - offset])[i + offset][j + 1]]++;

                        if (bluerow) {
                            (*historedThr[2 - offset])[(
                                *rawDataFrames[2 - offset])[i + 1][j - offset + 1]]++;
                            (*histoblueThr[(offset << 1) + offset])[(
                                *rawDataFrames[(offset << 1) + offset])[i][j + offset]]++;
                        } else {
                            (*historedThr[(offset << 1) + offset])[(
                                *rawDataFrames[(offset << 1) + offset])[i][j + offset]]++;
                            (*histoblueThr[2 - offset])[(
                                *rawDataFrames[2 - offset])[i + 1][j - offset + 1]]++;
                        }
                    }
                }
...
#pragma omp critical
            ...
            calcFrameBrightnessFactor(frame, (winh - 2) * (winw - 2) / 4, histored,
                                      redBrightness);
            calcFrameBrightnessFactor(frame, (winh - 2) * (winw - 2) / 4, histoblue,
                                      blueBrightness);
            calcFrameBrightnessFactor(frame, (winh - 2) * (winw - 2) / 2, histogreen,
                                      greenBrightness);
...
#pragma omp parallel for schedule(dynamic, 16)
        ...
        for (int i = winy + 1; i < winh - 1; ++i) {
            ...
            // offset to keep the code short. It changes its value between 0 and 1 for
        ...
        // now that the temporary planes are filled for easy access we do the motion
        // detection
        ...
        if (!bayerParams.pixelShiftMedian) {
            // We have to adjust the offsets for the selected subframe we use for areas
            // with motion
            switch (frame) {
            case 0:
                offsX = offsY = 0;
...
#pragma omp parallel for schedule(dynamic, 16)
        ...
        for (int i = winy + border - offsY; i < winh - (border + offsY); ++i) {
            ...
            for (int j = winx + border - offsX; j < winw - (border + offsX);
                 ++j, offset ^= 1)
            {
                psMask[i][j] = noMotion;

                if (checkGreen) {
                    if (greenDiff((*rawDataFrames[1 - offset])[i - offset + 1][j]
                                      * greenBrightness[1 - offset],
                                  (*rawDataFrames[3 - offset])[i + offset][j + 1]
                                      * greenBrightness[3 - offset],
                                  stddevFactorGreen, eperIsoGreen, nRead, prnu)
                        > 0.f)
                    {
                        psMask[i][j] = greenWeight;
                        // do not set the motion pixel values. They have already been set
                        // by demosaicer
                        continue;
                    }
                }
                ...
                if (checkNonGreenCross) {
                    ...
                    float redDiff = nonGreenDiffCross(
                        redRight, redLeft, redTop, redBottom, redCentre, clippedRed,
                        stddevFactorRed, eperIsoRed, nRead, prnu);
                    ...
                    float blueDiff = nonGreenDiffCross(
                        blueRight, blueLeft, blueTop, blueBottom, blueCentre, clippedBlue,
                        stddevFactorBlue, eperIsoBlue, nRead, prnu);
...
#pragma omp parallel for schedule(dynamic, 16)
        ...
        for (int i = winy + border - offsY; i < winh - (border + offsY); ++i) {
            ...
            for (int voffset = 2; j < winw - (border + offsX); ++j, ++voffset) {
                float colSum =
                    psMask[i - 1][j + 1] + psMask[i][j + 1] + psMask[i + 1][j + 1];
        ...
        if (holeFill) {
            ...
            invertMask(winx + border - offsX, winw - (border + offsX),
                       winy + border - offsY, winh - (border + offsY), mask, maskInv);
            floodFill4(winx + border - offsX, winw - (border + offsX),
                       winy + border - offsY, winh - (border + offsY), maskInv);
            xorMasks(winx + border - offsX, winw - (border + offsX),
                     winy + border - offsY, winh - (border + offsY), maskInv, mask);
        }
...
#pragma omp parallel for schedule(dynamic, 16)
        ...
        for (int i = winy + border - offsY; i < winh - (border + offsY); ++i) {
            ...
            if (smoothTransitions) {  //
                ...
                for (; j < winw - (border + offsX); ++j) {
                    psMask[i][j] =
                        smoothFactor == 0.f
                            ? 1.f
                            : pow_F(std::max(psMask[i][j] - 1.f, 0.f), smoothFactor);
                }
            }
                 ...
                 ++j, offset ^= 1)
            {
                if (showOnlyMask) {
                    if (smoothTransitions)
                    {  // we want only motion mask => paint areas according to their
                       // motion (dark = no motion, bright = motion)
#ifdef __SSE2__
                        ...
                        const float blend =
                            smoothFactor == 0.f
                                ...
                                : pow_F(std::max(psMask[i][j] - 1.f, 0.f), smoothFactor);
#endif
                        redDest[j + offsX] = greenDest[j + offsX] = blueDest[j + offsX] =
                            blend * 32768.f;
                    } else {
                        redDest[j + offsX] = greenDest[j + offsX] = blueDest[j + offsX] =
                            mask[i][j] == 255 ? 65535.f : 0.f;
                    }
                    ...
                    if (smoothTransitions) {
                        ...
                        const float blend =
                            smoothFactor == 0.f
                                ...
                                : pow_F(std::max(psMask[i][j] - 1.f, 0.f), smoothFactor);
#endif
                        redDest[j + offsX] = intp(
                            blend, showMotion ? 0.f : redDest[j + offsX], psRed[i][j]);
                        greenDest[j + offsX] =
                            intp(blend, showMotion ? 13500.f : greenDest[j + offsX],
                                 ((*rawDataFrames[1 - offset])[i - offset + 1][j]
                                      * greenBrightness[1 - offset]
                                  + (*rawDataFrames[3 - offset])[i + offset][j + 1]
                                        * greenBrightness[3 - offset])
                                     * 0.5f);
                        blueDest[j + offsX] = intp(
                            blend, showMotion ? 0.f : blueDest[j + offsX], psBlue[i][j]);
                    } else {
...
#pragma omp parallel for schedule(dynamic, 16)
        ...
        for (int i = winy + 1; i < winh - 1; ++i) {
            ...
            for (; j < winw - 1; ++j) {
                ...
                green[i][j] = ((*rawDataFrames[1 - offset])[i - offset + 1][j]
                                   * greenBrightness[1 - offset]
                               + (*rawDataFrames[3 - offset])[i + offset][j + 1]
                                     * greenBrightness[3 - offset])
                              * 0.5f;
