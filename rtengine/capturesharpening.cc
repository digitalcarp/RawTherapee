...
void buildClipMaskBayer(const float* const* rawData,
                        int W,
                        int H,
                        float** clipMask,
    ...
    for (int row = 2; row < H - 2; ++row) {
        ...
        for (int col = 2; col < W - 2; ++col) {
            if (rawData[row][col] >= clip0) {
                clipMask[row - 2][col - 1] = clipMask[row - 2][col] =
                    clipMask[row - 2][col + 1] = 0.f;
                clipMask[row - 1][col - 2] = clipMask[row - 1][col - 1] =
                    clipMask[row - 1][col] = clipMask[row - 1][col + 1] =
                        clipMask[row - 1][col + 2] = 0.f;
                clipMask[row][col - 2] = clipMask[row][col - 1] = clipMask[row][col] =
                    clipMask[row][col + 1] = clipMask[row][col + 2] = 0.f;
                clipMask[row + 1][col - 2] = clipMask[row + 1][col - 1] =
                    clipMask[row + 1][col] = clipMask[row + 1][col + 1] =
                        clipMask[row + 1][col + 2] = 0.f;
                clipMask[row + 2][col - 1] = clipMask[row + 2][col] =
...
void buildClipMaskXtrans(const float* const* rawData,
                         int W,
                         int H,
                         float** clipMask,
                         const float whites[6][6])
{
...
void buildClipMaskMono(const float* const* rawData,
                       int W,
                       int H,
                       float** clipMask,
                       float white)
...
float calcRadiusBayer(const float* const* rawData,
                      int W,
                      int H,
                      float lowerLimit,
                      float upperLimit,
                      const unsigned int fc[2])
{
...
float calcRadiusXtrans(const float* const* rawData,
                       int W,
                       int H,
                       float lowerLimit,
                       float upperLimit,
        ...
        for (int col = startx + 2; col < W - 4; col += 3) {
            ...
            const bool squareClipped =
                rtengine::max(valp1p1, rawData[row + 1][col + 2],
                              rawData[row + 2][col + 1], rawData[row + 2][col + 2])
                >= upperLimit;
            const float greenSolitary = rawData[row][col];
            if (greenSolitary > 1.f
                && std::max(rawData[row - 1][col - 1], rawData[row - 1][col + 1])
                       < upperLimit)
            {
            ...
            if (!squareClipped) {
                ...
                if (valp2p2 > 1.f) {
                    ...
                    if (rtengine::max(greenSolitaryRight, rawData[row + 4][col + 2],
                                      rawData[row + 4][col + 4])
                        < upperLimit)
                    {
                        if (greenSolitaryRight > 1.f) {
                            ...
                            if (maxVal > lowerLimit) {
                                const float minVal =
                                    std::min(greenSolitaryRight, valp2p2);
                                if (UNLIKELY(maxVal > maxRatio * minVal)) {
                                    maxRatio = maxVal / minVal;
                                }
                            }
                ...
                if (valp2p1 > 1.f) {
                    ...
                    if (rtengine::max(greenSolitaryLeft, rawData[row + 4][col - 1],
                                      rawData[row + 4][col + 1])
                        < upperLimit)
                    {
...
void CaptureDeconvSharpening(float** luminance,
        ...
        for (int i = border; i < H - border; i += tileSize) {
            for (int j = border; j < W - border; j += tileSize) {
                ...
                if (endOfRow || endOfCol) {
                    ...
                    float maxVal = 0.f;
                    if (checkIterStop) {
                        for (int k = 0, ii = endOfCol ? H - fullTileSize + border : i;
                             k < tileSize; ++k, ++ii)
                        {
                            for (int l = 0, jj = endOfRow ? W - fullTileSize + border : j;
                                 l < tileSize; ++l, ++jj)
                            {
                                iterCheck[k][l] =
                    ...
                    for (int k = 0, ii = endOfCol ? H - fullTileSize : i - border;
                         k < fullTileSize; ++k, ++ii)
                    {
                        for (int l = 0, jj = endOfRow ? W - fullTileSize : j - border;
                             l < fullTileSize; ++l, ++jj)
                        {
                ...
                if (is3x3) {
                    ...
                    if (sigmaCornerOffset != 0.f) {
                        const float distance =
                            sqrt(rtengine::SQR(i + tileSize / 2 - H / 2)
                                 + rtengine::SQR(j + tileSize / 2 - W / 2));
                ...
                if (endOfRow || endOfCol) {
                    // special handling for small tiles at end of row or column
                    for (int k = border, ii = endOfCol ? H - fullTileSize : i - border;
                         k < fullTileSize - border; ++k)
                    {
                        for (int l = border,
                                 jj = endOfRow ? W - fullTileSize : j - border;
...
bool WaveletDenoiseAllL2(wavelet_decomposition& WaveletCoeffs_L,
                         float* noisevarlum,
                         float madL[8][3],
                         float* vari,
                         int edge,
...
#pragma omp for schedule(dynamic) collapse(2)
            ...
            for (int lvl = 0; lvl < maxlvl; ++lvl) {
                for (int dir = 1; dir < 4; ++dir) {
                    ImProcFunctions::ShrinkAllL(WaveletCoeffs_L, buffer, lvl, dir,
...
void RawImageSource::captureSharpening(
    const procparams::CaptureSharpeningParams& sharpeningParams,
    bool showMask,
    double& conrastThreshold,
    double& radius)
{
    ...
    // predoise : small median to denoise before capture sharpening : allow CS to work
    // correctly and reduce a little the noise
    //  high median acts also on chroma noise
    // J.Desmis October 2024 - May 2025 - be carefull not to strong...
    if (sharpeningParams.noisecap > 0.f && sharpeningParams.noisecaptype == false)
    {  // median
...
#pragma omp parallel for schedule(dynamic, 16)
    ...
    if (sharpeningParams.noisecap > 0.f && sharpeningParams.noisecaptype == true)
    {  // wavelets, slower
...
#endif
        int levwav = 6;  // 64x64 must be enough for this usage...and no test memory
                         // allocation, we work on all image in Raw mode, probably too
                         // strong... but no problem with vari[]
...
#pragma omp parallel for schedule(dynamic, 16)
        ...
        // contrary to usual practice, I do not denoise the 'a' and 'b' with a specific
        // manner (or R and B) channels, but duplicate 3 times as if each channel was of
        // the same type, as if R,G,B are "luminance"
        wavelet_decomposition Ldecompgpre(labdngpre.L[0], labdngpre.W, labdngpre.H,
                                          levwav, 1, 1, numThreads,
                                          8);  // daublen = 8 - better moment wavelet
        if (Ldecompgpre.memory_allocation_failed()) {
            memoryAllocationFailed = true;
        }
        wavelet_decomposition Ldecomprpre(labdngpre.a[0], labdngpre.W, labdngpre.H,
                                          levwav, 1, 1, numThreads,
                                          8);  // daublen = 8 - better moment wavelet
        if (Ldecomprpre.memory_allocation_failed()) {
            memoryAllocationFailed = true;
        }
        wavelet_decomposition Ldecompbpre(labdngpre.b[0], labdngpre.W, labdngpre.H,
                                          levwav, 1, 1, numThreads,
                                          8);  // daublen = 8 - better moment wavelet
        ...
        if (!Ldecompgpre.memory_allocation_failed()) {
            ...
            for (int lvl = 0; lvl < levwav; lvl++) {
                for (int dir = 1; dir < 4; dir++) {
                    ...
                    madL[lvl][dir - 1] =
                        SQR(ImProcFunctions::MadRgb(WavCoeffs_L[dir], Wlvl_L * Hlvl_L));
                }
            }
        }
        if (!memoryAllocationFailed) {
            ...
            vari[0] =
                0.15f * noisevarL;  // empirical 'reduction' of action for level 0 2x2
            vari[1] =
                0.15f * noisevarL;  // empirical 'reduction' of action for level 1 4x4

            vari[2] =
                0.05f * noisevarL;  // empirical low 'reduction' of action for level 2
            vari[3] =
                0.05f * noisevarL;  // empirical low 'reduction' of action for level 3
            if (noiseluma > 15.f) {
                vari[2] =
                    0.15f
                    * noisevarL;  // empirical normal 'reduction' of action for level 2
                vari[3] =
                    0.15f
                    * noisevarL;  // empirical normal 'reduction' of action for level 3
            }
            vari[4] = 0.005f * noisevarL;  // empirical 'reduction' of action for level 4
                                           // very low action
            vari[5] = 0.005f * noisevarL;  // empirical 'reduction' of action for level 5
                                           // very low action
            ...
            float nvlh[13] = { 1.f, 1.f, 1.f, 1.f, 1.f,  1.f, 1.f,
                               1.f, 1.f, 1.f, 1.f, 0.7f, 0.5f };  // high value
            float nvll[13] = { 0.1f,  0.15f, 0.2f, 0.25f, 0.3f, 0.35f, 0.4f,
                               0.45f, 0.7f,  0.8f, 1.f,   1.f,  1.f };  // low value

            float seuillow = 4000.f;    // low empirical RGB values
            float seuilhigh = 35000.f;  // high empirical RGB values
            int noiselequal =
                5;  // equalizer black - white - same value for white and black
...
#pragma omp parallel for schedule(dynamic, 16)
            ...
            WaveletDenoiseAllL2(Ldecompgpre, noisevarlum, madL, vari, edge,
                                numThreads);  // simplified version of WaveletDenoiseAllL
            WaveletDenoiseAllL2(Ldecomprpre, noisevarlum, madL, vari, edge,
                                numThreads);  // simplified version of WaveletDenoiseAllL
            WaveletDenoiseAllL2(Ldecompbpre, noisevarlum, madL, vari, edge,
                                numThreads);  // simplified version of WaveletDenoiseAllL

            delete[] noisevarlum;

            Ldecompgpre.reconstruct(
                labdngpre.L[0]);  // reconstruct channel G after wavelets
            Ldecomprpre.reconstruct(
                labdngpre.a[0]);  // reconstruct channel R after wavelets
            Ldecompbpre.reconstruct(
                labdngpre.b[0]);  // reconstruct channel B after wavelets

#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic, 16)
#endif
            for (int i = 0; i < H; ++i) {  // re active redVals blueVals greenVals with
                                           // denoise and taking account mask
                for (int j = 0; j < W; ++j) {
    ...
    if (getSensorType() == ST_BAYER) {
        const float whites[2][2] = {
            { (ri->get_white(FC(0, 0)) - c_black[FC(0, 0)]) * scale_mul[FC(0, 0)]
                  * clipLimit,
              (ri->get_white(FC(0, 1)) - c_black[FC(0, 1)]) * scale_mul[FC(0, 1)]
                  * clipLimit },
            { (ri->get_white(FC(1, 0)) - c_black[FC(1, 0)]) * scale_mul[FC(1, 0)]
                  * clipLimit,
              (ri->get_white(FC(1, 1)) - c_black[FC(1, 1)]) * scale_mul[FC(1, 1)]
                  * clipLimit },
        };
        ...
        if (sharpeningParams.autoRadius) {
            radius =
                std::min(calcRadiusBayer(rawData, W, H, 1000.f, clipVal, fc), maxSigma);
        }
        ...
        for (int i = 0; i < 6; ++i) {
            for (int j = 0; j < 6; ++j) {
                ...
                whites[i][j] = (ri->get_white(color) - c_black[color]) * scale_mul[color]
                               * clipLimit;
            }
        }
        ...
        if (sharpeningParams.autoRadius) {
            radius = std::min(calcRadiusXtrans(rawData, W, H, 1000.f, clipVal, i, j),
                              maxSigma);
        }

    } else if (ri->get_colors() == 1) {
        buildClipMaskMono(rawData, W, H, clipMask,
                          (ri->get_white(0) - c_black[0]) * scale_mul[0] * clipLimit);
        if (sharpeningParams.autoRadius) {
    ...
    if (showMask || showMaskperso) {
        array2D<float>& L =
            blue;  // blue will be overridden anyway => we can use its buffer to store L
#ifdef _OPENMP
#pragma omp parallel for
        ...
        buildBlendMask(L, clipMask, W, H, contrast, sharpeningParams.autoContrast,
                       clipMask);
        if (plistener) {
...
#pragma omp parallel for schedule(dynamic, 16)
    ...
    conrastThreshold = contrast * 100.f;
    CaptureDeconvSharpening(YNew, YOld, clipMask, W, H, radius,
                            sharpeningParams.deconvradiusOffset,
                            sharpeningParams.deconviter, sharpeningParams.deconvitercheck,
                            plistener, 0.2, 0.9);
    if (plistener) {
...
#endif
        int levwav = 6;  // 64 x 64 must be enough for this usage...and no test memory
                         // allocation, we work on all image in Raw mode!
...
#pragma omp parallel for schedule(dynamic, 16)
        ...
        for (int i = 0; i < H; ++i) {  // save values of red green blue
            for (int j = 0; j < W; ++j) {
                ...
                labdng.L[i][j] = prov1->g(
                    i, j);  // initialize labdng.L - channel green "near" Luminance
        ...
        // manner (or R and B) channels, but duplicate 3 times as if each channel was of
        // the same type, as if R,G,B are "luminance"
        wavelet_decomposition Ldecompg(
            labdng.L[0], labdng.W, labdng.H, levwav, 1, 1, numThreads,
            22);  // daublen = 22 - maximum better moment wavelet
        if (Ldecompg.memory_allocation_failed()) {
            memoryAllocationFailed = true;
        }
        wavelet_decomposition Ldecompr(
            labdng.a[0], labdng.W, labdng.H, levwav, 1, 1, numThreads,
            22);  // daublen = 22 - maximum better moment wavelet
        if (Ldecompr.memory_allocation_failed()) {
            memoryAllocationFailed = true;
        }
        wavelet_decomposition Ldecompb(
            labdng.b[0], labdng.W, labdng.H, levwav, 1, 1, numThreads,
            22);  // daublen = 22 - maximum better moment wavelet
        ...
        if (!memoryAllocationFailed) {
            ...
            vari[5] =
                0.6f
                * noisevarL;  // empirical 'reduction' of action for level 5 - 128x128
            int edge = 6;     // as maxlevels
...
#pragma omp parallel for schedule(dynamic, 16)
            ...
            // but only one noisevarlum for the 3 channels
            // 3 times the same wavelet for G, R and B
            WaveletDenoiseAllL2(Ldecompg, noisevarlum, madL, vari, edge,
                                numThreads);  // simplified version of WaveletDenoiseAllL
            WaveletDenoiseAllL2(Ldecompr, noisevarlum, madL, vari, edge,
                                numThreads);  // simplified version of WaveletDenoiseAllL
            WaveletDenoiseAllL2(Ldecompb, noisevarlum, madL, vari, edge,
                                numThreads);  // simplified version of WaveletDenoiseAllL
...
#pragma omp parallel for schedule(dynamic, 16)
            ...
            for (int ir = 0; ir < H; ir++) {
                for (int jr = 0; jr < W; jr++) {
                    labdng.L[ir][jr] = intp(clipMask[ir][jr], prov1->g(ir, jr),
                                            labdng.L[ir][jr]);  // green
                    labdng.a[ir][jr] = intp(clipMask[ir][jr], prov1->r(ir, jr),
                                            labdng.a[ir][jr]);  // red
                    labdng.b[ir][jr] = intp(clipMask[ir][jr], prov1->b(ir, jr),
                                            labdng.b[ir][jr]);  // blue
                }
            }
...
#endif
            for (int i = 0; i < H; ++i)
            {  // re active red blue green with denoise and taking account mask
                for (int j = 0; j < W; ++j) {
