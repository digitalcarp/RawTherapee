...
public:
    ...
    int loadRaw(bool loadData,
                unsigned int imageNum = 0,
                bool closeFile = true,
                ProgressListener* plistener = nullptr,
                double progressRange = 1.0);
    void
    get_colorsCoeff(float* pre_mul_, float* scale_mul_, float* cblack_, bool forceAutoWB);
    void set_prefilters()
    {
    ...
    float** compress_image(unsigned int frameNum,
                           bool freeImage = true);  // revert to compressed pixels format
                                                    // and release image data
    float**
        data;  // holds pixel values, data[i][j] corresponds to the ith row and jth column
...
public:
    ...
    float get_pre_mul(int c) const
    {
        if (std::isfinite(pre_mul[c])) {
            ...
            std::cout
                << "Failure decoding '" << filename
                << "', please file a bug report including the raw file and the line below:"
                << std::endl;
            std::cout << "rawimage.h get_pre_mul() : pre_mul[" << c << "] value "
                      << pre_mul[c] << " automatically replaced by value 1.0"
                      << std::endl;
            return 1.f;
        }
    }
