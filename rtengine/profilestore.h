...
public:
    ...
    /** @brief Called whenever the file list has been updated and the content of the
     * listener has to be updated. */
    virtual void updateProfileList() = 0;
    /** @brief Called whenever the profile list has changed and the old value have to be
     * restored (if possible). */
    virtual void restoreValue() = 0;
};

/// @brief ProfileStoreEntry type (folder or file)
typedef enum PSE_TYPE { PSET_FOLDER, PSET_FILE } PSEType;
...
public:
    ...
    unsigned short parentFolderId;  /// index of the element's path in the folder list; id
                                    /// == 0 is reserved
    unsigned short folderId;  /// index of the folder's own path in the folder list; will
                              /// be null for file entries

    /** @brief Create a new ProfileStoreLabel with null values, that will have to be set
     * later with setValues or the copy operator
     */
    ProfileStoreEntry();

    /** @brief Create a new ProfileStoreLabel with values
     * @param label         Label to be used in menu or combobox; also used as profile's
     * filename
     ...
     */
    ProfileStoreEntry(Glib::ustring label,
                      PSEType type,
...
private:
    struct SortProfiles
    {
        bool operator()(const ProfileStoreEntry* const a1,
                        const ProfileStoreEntry* const a2)
        {
            return a1->parentFolderId == a2->parentFolderId
                       ? a1->label < a2->label
                       : a1->parentFolderId < a2->parentFolderId;
        }
    };
     ...
     * The first element (#0) will be a fake path so ProfileStoreEntry can be attached to
     * the root container */
    ...
    std::map<const ProfileStoreEntry*, rtengine::procparams::AutoPartialProfile*>
        partProfiles;
    ...
    /** @brief Method to recursively parse a profile folder with a level depth arbitrarily
     * limited to 3
     *
     * @param realPath       current full path of the scanned directory ; e.g.:
     * ~/MyProfiles/
     * @param virtualPath    current full path that will be saved in "options" ; must
     * start with either ${U} or ${G}, standing for User's and Global's (RT) profile
     * folder, respectively
     * @param currDir        name of the directory to scan; it's the last element of the
     * virtualPath
     * @param parentId       path entry of the parent folder
     * @param level          current level of the directory tree
     * @param displayLevel0  if true, level 0 is created in order to have a User's and
     * Bundled profiles separation (i.e. 2 root directories are expected) if false, only
     * one root directory is expected
     */
    bool parseDir(Glib::ustring& realPath,
                  Glib::ustring& virtualPath,
                  Glib::ustring& currDir,
                  unsigned int parentId,
                  unsigned char level,
                  bool displayLevel0);
    /** @brief Will parse the profiles's dir only once. Subsequent call to this function
     * will be ignored unless the profile list has been cleared
     */
...
public:
    ...
    rtengine::procparams::PartialProfile*
    loadDynamicProfile(const rtengine::FramesMetaData* im, const Glib::ustring& filename);
