...
template <class T>
void calculateYvVFactors(const T sigma, T& b1, T& b2, T& b3, T& B, T M[3][3])
{
    ...
    // From: Bill Triggs, Michael Sdika: Boundary Conditions for Young-van Vliet Recursive
    // Filtering
...
#pragma omp single
#endif
    {
        ...
        for (int j = 1; j < W - 1; j++) {
            dst[H - 1][j] =
                b1 * (src[H - 1][j - 1] + src[H - 1][j + 1]) + b0 * src[H - 1][j];
        }
...
#pragma omp single nowait
    ...
    {
        dst[0][0] =
            rtengine::max(divBuffer[0][0] / (src[0][0] > 0.f ? src[0][0] : 1.f), 0.f);
        ...
        dst[0][W - 1] = rtengine::max(
            divBuffer[0][W - 1] / (src[0][W - 1] > 0.f ? src[0][W - 1] : 1.f), 0.f);
    }
...
#pragma omp single
#endif
    {
        dst[H - 1][0] = rtengine::max(
            divBuffer[H - 1][0] / (src[H - 1][0] > 0.f ? src[H - 1][0] : 1.f), 0.f);

        for (int j = 1; j < W - 1; j++) {
            float tmp =
                (b1 * (src[H - 1][j - 1] + src[H - 1][j + 1]) + b0 * src[H - 1][j]);
            dst[H - 1][j] =
                rtengine::max(divBuffer[H - 1][j] / (tmp > 0.f ? tmp : 1.f), 0.f);
        }

        dst[H - 1][W - 1] = rtengine::max(
            divBuffer[H - 1][W - 1] / (src[H - 1][W - 1] > 0.f ? src[H - 1][W - 1] : 1.f),
            0.f);
    }
}

template <class T>
void gauss7x7div(T** RESTRICT src,
                 T** RESTRICT dst,
                 T** RESTRICT divBuffer,
                 const int W,
                 const int H,
...
#pragma omp for schedule(dynamic, 16) nowait
    ...
    for (int i = 3; i < H - 3; ++i) {
        ...
        for (int j = 3; j < W - 3; ++j) {
            const float val =
                c31
                    * (src[i - 3][j - 1] + src[i - 3][j + 1] + src[i - 1][j - 3]
                       + src[i - 1][j + 3] + src[i + 1][j - 3] + src[i + 1][j + 3]
                       + src[i + 3][j - 1] + src[i + 3][j + 1])
                + c30 * (src[i - 3][j] + src[i][j - 3] + src[i][j + 3] + src[i + 3][j])
                + c22
                      * (src[i - 2][j - 2] + src[i - 2][j + 2] + src[i + 2][j - 2]
                         + src[i + 2][j + 2])
                + c21
                      * (src[i - 2][j - 1] + src[i - 2][j + 1] * c21 + src[i - 1][j - 2]
                         + src[i - 1][j + 2] + src[i + 1][j - 2] + src[i + 1][j + 2]
                         + src[i + 2][j - 1] + src[i + 2][j + 1])
...
template <class T>
void gauss5x5div(T** RESTRICT src,
                 T** RESTRICT dst,
                 T** RESTRICT divBuffer,
                 const int W,
                 const int H,
...
#pragma omp for schedule(dynamic, 16) nowait
    ...
    for (int i = 2; i < H - 2; ++i) {
        ...
        for (int j = 2; j < W - 2; ++j) {
            const float val =
                c21
                    * (src[i - 2][j - 1] + src[i - 2][j + 1] + src[i - 1][j - 2]
                       + src[i - 1][j + 2] + src[i + 1][j - 2] + src[i + 1][j + 2]
                       + src[i + 2][j - 1] + src[i + 2][j + 1])
                + c20 * (src[i - 2][j] + src[i][j - 2] + src[i][j + 2] + src[i + 2][j])
                + c11
...
template <class T>
void gauss7x7mult(T** RESTRICT src,
                  T** RESTRICT dst,
                  const int W,
                  const int H,
                  float sigma)
...
template <class T>
void gauss5x5mult(T** RESTRICT src,
                  T** RESTRICT dst,
                  const int W,
                  const int H,
                  float sigma)
...
template <class T>
void gaussHorizontal3(T** src, T** dst, int W, int H, const float c0, const float c1)
{
    T temp[W] ALIGNED16;
#ifdef _OPENMP
#pragma omp for
...
#ifdef __SSE2__
template <class T>
void gaussVertical3(T** src, T** dst, int W, int H, const float c0, const float c1)
{
...
template <class T>
void gaussHorizontalSse(T** src, T** dst, const int W, const int H, const float sigma)
{
...
#pragma omp for nowait
    ...
    for (int i = 0; i < H - 3; i += 4) {
        ...
        Tm2v = _mm_set_ps(src[i][1], src[i + 1][1], src[i + 2][1], src[i + 3][1]) * Bv
               + Tm3v * b1v + Tv * (b2v + b3v);
        STVF(tmp[1][0], Tm2v);

        Rv = _mm_set_ps(src[i][2], src[i + 1][2], src[i + 2][2], src[i + 3][2]) * Bv
             + Tm2v * b1v + Tm3v * b2v + Tv * b3v;
        STVF(tmp[2][0], Rv);

        for (int j = 3; j < W; j++) {
            Tv = Rv;
            Rv = _mm_set_ps(src[i][j], src[i + 1][j], src[i + 2][j], src[i + 3][j]) * Bv
                 + Tv * b1v + Tm2v * b2v + Tm3v * b3v;
            STVF(tmp[j][0], Rv);
            Tm3v = Tm2v;
            Tm2v = Tv;
        }

        Tv = _mm_set_ps(src[i][W - 1], src[i + 1][W - 1], src[i + 2][W - 1],
                        src[i + 3][W - 1]);

        temp2Wp1 = Tv + F2V(M[2][0]) * (Rv - Tv) + F2V(M[2][1]) * (Tm2v - Tv)
                   + F2V(M[2][2]) * (Tm3v - Tv);
        temp2W = Tv + F2V(M[1][0]) * (Rv - Tv) + F2V(M[1][1]) * (Tm2v - Tv)
                 + F2V(M[1][2]) * (Tm3v - Tv);

        Rv = Tv + F2V(M[0][0]) * (Rv - Tv) + F2V(M[0][1]) * (Tm2v - Tv)
             + F2V(M[0][2]) * (Tm3v - Tv);
        STVF(tmp[W - 1][0], Rv);
...
#pragma omp single
    ...
    for (int i = H - (H % 4); i < H; i++) {
        ...
        for (int j = 3; j < W; j++) {
            tmp[j][0] = B * src[i][j] + b1 * tmp[j - 1][0] + b2 * tmp[j - 2][0]
                        + b3 * tmp[j - 3][0];
        }

        float temp2Wm1 = src[i][W - 1] + M[0][0] * (tmp[W - 1][0] - src[i][W - 1])
                         + M[0][1] * (tmp[W - 2][0] - src[i][W - 1])
                         + M[0][2] * (tmp[W - 3][0] - src[i][W - 1]);
        float temp2W = src[i][W - 1] + M[1][0] * (tmp[W - 1][0] - src[i][W - 1])
                       + M[1][1] * (tmp[W - 2][0] - src[i][W - 1])
                       + M[1][2] * (tmp[W - 3][0] - src[i][W - 1]);
        float temp2Wp1 = src[i][W - 1] + M[2][0] * (tmp[W - 1][0] - src[i][W - 1])
                         + M[2][1] * (tmp[W - 2][0] - src[i][W - 1])
                         + M[2][2] * (tmp[W - 3][0] - src[i][W - 1]);

        tmp[W - 1][0] = temp2Wm1;
        tmp[W - 2][0] =
            B * tmp[W - 2][0] + b1 * tmp[W - 1][0] + b2 * temp2W + b3 * temp2Wp1;
        tmp[W - 3][0] =
            B * tmp[W - 3][0] + b1 * tmp[W - 2][0] + b2 * tmp[W - 1][0] + b3 * temp2W;

        for (int j = W - 4; j >= 0; j--) {
            tmp[j][0] = B * tmp[j][0] + b1 * tmp[j + 1][0] + b2 * tmp[j + 2][0]
                        + b3 * tmp[j + 3][0];
        }
...
// fast gaussian approximation if the support window is large
template <class T>
void gaussHorizontal(T** src, T** dst, const int W, const int H, const double sigma)
{
    double b1, b2, b3, B, M[3][3];
    calculateYvVFactors<double>(sigma, b1, b2, b3, B, M);
...
#pragma omp for
    ...
    for (int i = 0; i < H; i++) {
        ...
        for (int j = 3; j < W; j++) {
            temp2[j] =
                B * src[i][j] + b1 * temp2[j - 1] + b2 * temp2[j - 2] + b3 * temp2[j - 3];
        }

        double temp2Wm1 = src[i][W - 1] + M[0][0] * (temp2[W - 1] - src[i][W - 1])
                          + M[0][1] * (temp2[W - 2] - src[i][W - 1])
                          + M[0][2] * (temp2[W - 3] - src[i][W - 1]);
        double temp2W = src[i][W - 1] + M[1][0] * (temp2[W - 1] - src[i][W - 1])
                        + M[1][1] * (temp2[W - 2] - src[i][W - 1])
                        + M[1][2] * (temp2[W - 3] - src[i][W - 1]);
        double temp2Wp1 = src[i][W - 1] + M[2][0] * (temp2[W - 1] - src[i][W - 1])
                          + M[2][1] * (temp2[W - 2] - src[i][W - 1])
                          + M[2][2] * (temp2[W - 3] - src[i][W - 1]);

        temp2[W - 1] = temp2Wm1;
        temp2[W - 2] = B * temp2[W - 2] + b1 * temp2[W - 1] + b2 * temp2W + b3 * temp2Wp1;
        temp2[W - 3] =
            B * temp2[W - 3] + b1 * temp2[W - 2] + b2 * temp2[W - 1] + b3 * temp2W;

        for (int j = W - 4; j >= 0; j--) {
            temp2[j] =
                B * temp2[j] + b1 * temp2[j + 1] + b2 * temp2[j + 2] + b3 * temp2[j + 3];
        }
...
#ifdef __SSE2__
template <class T>
void gaussVerticalSse(T** src, T** dst, const int W, const int H, const float sigma)
{
    double b1, b2, b3, B, M[3][3];
    calculateYvVFactors<double>(sigma, b1, b2, b3, B, M);
...
#pragma omp for nowait
    ...
    for (int i = 0; i < W - 7; i += 8) {
        ...
        temp2Wp1 = Tv + F2V(M[2][0]) * (Rv - Tv) + F2V(M[2][1]) * (Tm2v - Tv)
                   + F2V(M[2][2]) * (Tm3v - Tv);
        temp2Wp11 = Tv1 + F2V(M[2][0]) * (Rv1 - Tv1) + F2V(M[2][1]) * (Tm2v1 - Tv1)
                    + F2V(M[2][2]) * (Tm3v1 - Tv1);
        temp2W = Tv + F2V(M[1][0]) * (Rv - Tv) + F2V(M[1][1]) * (Tm2v - Tv)
                 + F2V(M[1][2]) * (Tm3v - Tv);
        temp2W1 = Tv1 + F2V(M[1][0]) * (Rv1 - Tv1) + F2V(M[1][1]) * (Tm2v1 - Tv1)
                  + F2V(M[1][2]) * (Tm3v1 - Tv1);

        Rv = Tv + F2V(M[0][0]) * (Rv - Tv) + F2V(M[0][1]) * (Tm2v - Tv)
             + F2V(M[0][2]) * (Tm3v - Tv);
        Rv1 = Tv1 + F2V(M[0][0]) * (Rv1 - Tv1) + F2V(M[0][1]) * (Tm2v1 - Tv1)
              + F2V(M[0][2]) * (Tm3v1 - Tv1);
        STVFU(dst[H - 1][i], Rv);
...
#pragma omp single
    ...
    for (int i = W - (W % 8); i < W; i++) {
        ...
        for (int j = 3; j < H; j++) {
            tmp[j][0] = B * src[j][i] + b1 * tmp[j - 1][0] + b2 * tmp[j - 2][0]
                        + b3 * tmp[j - 3][0];
        }

        float temp2Hm1 = src[H - 1][i] + M[0][0] * (tmp[H - 1][0] - src[H - 1][i])
                         + M[0][1] * (tmp[H - 2][0] - src[H - 1][i])
                         + M[0][2] * (tmp[H - 3][0] - src[H - 1][i]);
        float temp2H = src[H - 1][i] + M[1][0] * (tmp[H - 1][0] - src[H - 1][i])
                       + M[1][1] * (tmp[H - 2][0] - src[H - 1][i])
                       + M[1][2] * (tmp[H - 3][0] - src[H - 1][i]);
        float temp2Hp1 = src[H - 1][i] + M[2][0] * (tmp[H - 1][0] - src[H - 1][i])
                         + M[2][1] * (tmp[H - 2][0] - src[H - 1][i])
                         + M[2][2] * (tmp[H - 3][0] - src[H - 1][i]);

        tmp[H - 1][0] = temp2Hm1;
        tmp[H - 2][0] =
            B * tmp[H - 2][0] + b1 * tmp[H - 1][0] + b2 * temp2H + b3 * temp2Hp1;
        tmp[H - 3][0] =
            B * tmp[H - 3][0] + b1 * tmp[H - 2][0] + b2 * tmp[H - 1][0] + b3 * temp2H;
...
#ifdef __SSE2__
template <class T>
void gaussVerticalSsemult(T** RESTRICT src,
                          T** RESTRICT dst,
                          const int W,
                          const int H,
                          const float sigma)
...
template <class T>
void gaussVerticalSsediv(T** src,
                         T** dst,
                         T** divBuffer,
                         const int W,
                         const int H,
...
#pragma omp for nowait
    ...
    for (int i = 0; i < W - 7; i += 8) {
        ...
        STVFU(dst[H - 1][i],
              LVFU(divBuffer[H - 1][i]) / vself(vmaskf_gt(Rv, ZEROV), Rv, onev));
        STVFU(dst[H - 1][i + 4],
              LVFU(divBuffer[H - 1][i + 4]) / vself(vmaskf_gt(Rv1, ZEROV), Rv1, onev));

        Tm2v = Bv * Tm2v + b1v * Rv + b2v * temp2W + b3v * temp2Wp1;
        Tm2v1 = Bv * Tm2v1 + b1v * Rv1 + b2v * temp2W1 + b3v * temp2Wp11;
        STVFU(dst[H - 2][i],
              LVFU(divBuffer[H - 2][i]) / vself(vmaskf_gt(Tm2v, ZEROV), Tm2v, onev));
        STVFU(dst[H - 2][i + 4], LVFU(divBuffer[H - 2][i + 4])
                                     / vself(vmaskf_gt(Tm2v1, ZEROV), Tm2v1, onev));

        Tm3v = Bv * Tm3v + b1v * Tm2v + b2v * Rv + b3v * temp2W;
        Tm3v1 = Bv * Tm3v1 + b1v * Tm2v1 + b2v * Rv1 + b3v * temp2W1;
        STVFU(dst[H - 3][i],
              LVFU(divBuffer[H - 3][i]) / vself(vmaskf_gt(Tm3v, ZEROV), Tm3v, onev));
        STVFU(dst[H - 3][i + 4], LVFU(divBuffer[H - 3][i + 4])
                                     / vself(vmaskf_gt(Tm3v1, ZEROV), Tm3v1, onev));
        ...
        for (int j = H - 4; j >= 0; j--) {
            ...
            Rv1 = LVF(tmp[j][4]) * Bv + Tv1 * b1v + Tm2v1 * b2v + Tm3v1 * b3v;
            STVFU(dst[j][i],
                  vmaxf(LVFU(divBuffer[j][i]) / vself(vmaskf_gt(Rv, ZEROV), Rv, onev),
                        ZEROV));
            STVFU(dst[j][i + 4], vmaxf(LVFU(divBuffer[j][i + 4])
                                           / vself(vmaskf_gt(Rv1, ZEROV), Rv1, onev),
                                       ZEROV));
            Tm3v = Tm2v;
            Tm3v1 = Tm2v1;
            Tm2v = Tv;
...
#pragma omp single
    ...
    for (int i = W - (W % 8); i < W; i++) {
        ...
        for (int j = 0; j < H; j++) {
            dst[j][i] =
                rtengine::max(divBuffer[j][i] / (tmp[j][0] > 0.f ? tmp[j][0] : 1.f), 0.f);
        }
    }
}
...
template <class T>
void gaussVertical(T** src, T** dst, const int W, const int H, const double sigma)
{
    double b1, b2, b3, B, M[3][3];
    calculateYvVFactors<double>(sigma, b1, b2, b3, B, M);
    ...
    auto coord = [](const int& x, const int& y) -> int { return x * numcols + y; };
#ifdef _OPENMP
#pragma omp for nowait
#endif

    for (unsigned int i = 0; i < static_cast<unsigned>(std::max(0, W - numcols + 1));
         i += numcols)
    {
        for (int k = 0; k < numcols; k++) {
            temp2[coord(0, k)] = B * src[0][i + k] + b1 * src[0][i + k]
                                 + b2 * src[0][i + k] + b3 * src[0][i + k];
            temp2[coord(1, k)] = B * src[1][i + k] + b1 * temp2[coord(0, k)]
                                 + b2 * src[0][i + k] + b3 * src[0][i + k];
            temp2[coord(2, k)] = B * src[2][i + k] + b1 * temp2[coord(1, k)]
                                 + b2 * temp2[coord(0, k)] + b3 * src[0][i + k];
        }

        for (int j = 3; j < H; j++) {
            for (int k = 0; k < numcols; k++) {
                temp2[coord(j, k)] = B * src[j][i + k] + b1 * temp2[coord(j - 1, k)]
                                     + b2 * temp2[coord(j - 2, k)]
                                     + b3 * temp2[coord(j - 3, k)];
            }
        }

        for (int k = 0; k < numcols; k++) {
            temp2Hm1[k] = src[H - 1][i + k]
                          + M[0][0] * (temp2[coord(H - 1, k)] - src[H - 1][i + k])
                          + M[0][1] * (temp2[coord(H - 2, k)] - src[H - 1][i + k])
                          + M[0][2] * (temp2[coord(H - 3, k)] - src[H - 1][i + k]);
            temp2H[k] = src[H - 1][i + k]
                        + M[1][0] * (temp2[coord(H - 1, k)] - src[H - 1][i + k])
                        + M[1][1] * (temp2[coord(H - 2, k)] - src[H - 1][i + k])
                        + M[1][2] * (temp2[coord(H - 3, k)] - src[H - 1][i + k]);
            temp2Hp1[k] = src[H - 1][i + k]
                          + M[2][0] * (temp2[coord(H - 1, k)] - src[H - 1][i + k])
                          + M[2][1] * (temp2[coord(H - 2, k)] - src[H - 1][i + k])
                          + M[2][2] * (temp2[coord(H - 3, k)] - src[H - 1][i + k]);
        }

        for (int k = 0; k < numcols; k++) {
            dst[H - 1][i + k] = temp2[coord(H - 1, k)] = temp2Hm1[k];
            dst[H - 2][i + k] = temp2[coord(H - 2, k)] =
                B * temp2[coord(H - 2, k)] + b1 * temp2[coord(H - 1, k)] + b2 * temp2H[k]
                + b3 * temp2Hp1[k];
            dst[H - 3][i + k] = temp2[coord(H - 3, k)] =
                B * temp2[coord(H - 3, k)] + b1 * temp2[coord(H - 2, k)]
                + b2 * temp2[coord(H - 1, k)] + b3 * temp2H[k];
        }

        for (int j = H - 4; j >= 0; j--) {
            for (int k = 0; k < numcols; k++) {
                dst[j][i + k] = temp2[coord(j, k)] =
                    B * temp2[coord(j, k)] + b1 * temp2[coord(j + 1, k)]
                    + b2 * temp2[coord(j + 2, k)] + b3 * temp2[coord(j + 3, k)];
            }
        }
    }
...
#pragma omp single
    ...
    for (int i = W - (W % numcols); i < W; i++) {
        temp2[coord(0, 0)] =
            B * src[0][i] + b1 * src[0][i] + b2 * src[0][i] + b3 * src[0][i];
        temp2[coord(1, 0)] =
            B * src[1][i] + b1 * temp2[coord(0, 0)] + b2 * src[0][i] + b3 * src[0][i];
        temp2[coord(2, 0)] = B * src[2][i] + b1 * temp2[coord(1, 0)]
                             + b2 * temp2[coord(0, 0)] + b3 * src[0][i];

        for (int j = 3; j < H; j++) {
            temp2[coord(j, 0)] = B * src[j][i] + b1 * temp2[coord(j - 1, 0)]
                                 + b2 * temp2[coord(j - 2, 0)]
                                 + b3 * temp2[coord(j - 3, 0)];
        }

        double temp2Hm1 = src[H - 1][i]
                          + M[0][0] * (temp2[coord(H - 1, 0)] - src[H - 1][i])
                          + M[0][1] * (temp2[coord(H - 2, 0)] - src[H - 1][i])
                          + M[0][2] * (temp2[coord(H - 3, 0)] - src[H - 1][i]);
        double temp2H = src[H - 1][i] + M[1][0] * (temp2[coord(H - 1, 0)] - src[H - 1][i])
                        + M[1][1] * (temp2[coord(H - 2, 0)] - src[H - 1][i])
                        + M[1][2] * (temp2[coord(H - 3, 0)] - src[H - 1][i]);
        double temp2Hp1 = src[H - 1][i]
                          + M[2][0] * (temp2[coord(H - 1, 0)] - src[H - 1][i])
                          + M[2][1] * (temp2[coord(H - 2, 0)] - src[H - 1][i])
                          + M[2][2] * (temp2[coord(H - 3, 0)] - src[H - 1][i]);

        dst[H - 1][i] = temp2[coord(H - 1, 0)] = temp2Hm1;
        dst[H - 2][i] = temp2[coord(H - 2, 0)] = B * temp2[coord(H - 2, 0)]
                                                 + b1 * temp2[coord(H - 1, 0)]
                                                 + b2 * temp2H + b3 * temp2Hp1;
        dst[H - 3][i] = temp2[coord(H - 3, 0)] =
            B * temp2[coord(H - 3, 0)] + b1 * temp2[coord(H - 2, 0)]
            + b2 * temp2[coord(H - 1, 0)] + b3 * temp2H;

        for (int j = H - 4; j >= 0; j--) {
            dst[j][i] = temp2[coord(j, 0)] =
                B * temp2[coord(j, 0)] + b1 * temp2[coord(j + 1, 0)]
                + b2 * temp2[coord(j + 2, 0)] + b3 * temp2[coord(j + 3, 0)];
        }
    }
}
    ...
    for (int i = 0; i < W - numcols + 1; i += numcols) {
        ...
        for (int k = 0; k < numcols; k++) {
            dst[H - 1][i + k] = rtengine::max(
                divBuffer[H - 1][i + k] / (temp2[coord(H - 1, k)] = temp2Hm1[k]), 0.0);
            dst[H - 2][i + k] = rtengine::max(
                divBuffer[H - 2][i + k]
                    / (temp2[coord(H - 2, k)] = B * temp2[coord(H - 2, k)]
                                                + b1 * temp2[coord(H - 1, k)]
                                                + b2 * temp2H[k] + b3 * temp2Hp1[k]),
                0.0);
        ...
        for (int j = H - 4; j >= 0; j--) {
            for (int k = 0; k < numcols; k++) {
                dst[j][i + k] = rtengine::max(
                    divBuffer[j][i + k]
                        / (temp2[coord(j, k)] = B * temp2[coord(j, k)]
                                                + b1 * temp2[coord(j + 1, k)]
                                                + b2 * temp2[coord(j + 2, k)]
                                                + b3 * temp2[coord(j + 3, k)]),
                    0.0);
            }
...
#pragma omp single
    ...
    for (int i = W - (W % numcols); i < W; i++) {
        ...
        dst[H - 1][i] =
            rtengine::max(divBuffer[H - 1][i] / (temp2[coord(H - 1, 0)] = temp2Hm1), 0.0);
        dst[H - 2][i] =
            rtengine::max(divBuffer[H - 2][i]
                              / (temp2[coord(H - 2, 0)] = B * temp2[coord(H - 2, 0)]
                                                          + b1 * temp2[coord(H - 1, 0)]
                                                          + b2 * temp2H + b3 * temp2Hp1),
                          0.0);
        dst[H - 3][i] = rtengine::max(
            divBuffer[H - 3][i]
                / (temp2[coord(H - 3, 0)] = B * temp2[coord(H - 3, 0)]
                                            + b1 * temp2[coord(H - 2, 0)]
                                            + b2 * temp2[coord(H - 1, 0)] + b3 * temp2H),
            0.0);
...
template <class T>
void gaussVerticalmult(T** src, T** dst, const int W, const int H, const double sigma)
{
    double b1, b2, b3, B, M[3][3];
    calculateYvVFactors<double>(sigma, b1, b2, b3, B, M);
...
#pragma omp single
    ...
    for (int i = W - (W % numcols); i < W; i++) {
        ...
        for (int j = H - 4; j >= 0; j--) {
            dst[j][i] *=
                (temp2[coord(j, 0)] = B * temp2[coord(j, 0)] + b1 * temp2[coord(j + 1, 0)]
                                      + b2 * temp2[coord(j + 2, 0)]
                                      + b3 * temp2[coord(j + 3, 0)]);
        }
    }
