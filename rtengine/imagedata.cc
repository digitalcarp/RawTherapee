...
const std::string& validateUft8(const std::string& str,
...
bool tag_values_equal(const Iterator& iter,
...
class TagValueReader
{
     ...
     * @return The value.
     */
    template <typename T, T (&getter)(const Exiv2::byte*, Exiv2::ByteOrder)> T readValue()
    {
...
public:
     ...
     */
    TagValueReader(const Exiv2::Value& value,
                   Exiv2::ByteOrder defaultByteOrder = Exiv2::bigEndian)
        : data(value.size()), defaultByteOrder(defaultByteOrder)
    {
     ...
     */
    double readDouble() { return readValue<double, Exiv2::getDouble>(); }

    /**
     * Reads a float from the current offset and advances the offset.
     */
    float readFloat() { return readValue<float, Exiv2::getFloat>(); }

    /**
     * Reads an unsigned integer from the current offset and advances the
     * offset.
     */
    std::uint32_t readUInt() { return readValue<std::uint32_t, Exiv2::getULong>(); }

    /**
     * Sets the offset.
     */
    void seekAbsolute(DataOffset newOffset) { offset = newOffset; }

    /**
     * Advances the offset by the given amount.
     */
    void seekRelative(DataOffset offsetDifference) { offset += offsetDifference; }
};
...
GainMap readGainMap(TagValueReader& reader)
{
    ...
    const std::size_t n = static_cast<std::size_t>(gainMap.MapPointsV)
                          * static_cast<std::size_t>(gainMap.MapPointsH)
                          * static_cast<std::size_t>(gainMap.MapPlanes);
...
void readOpcodesList(const Exiv2::Value& value,
                     ...
                     std::vector<GainMap>* gainMaps)
{
    ...
    if (ntags >= reader.size() / 12)
    {  // rough check for wrong value (happens for example with DNG files from DJI FC6310)
        return;
    }
      ...
      h_(-1)
{
    ...
    try {
        ...
        const auto find_exif_tag = [&exif, &pos](const std::string& name) -> bool {
            try {
        ...
        const auto find_tag = [&exif, &pos](decltype(Exiv2::make) func) -> bool {
            pos = func(exif);
            return pos != exif.end() && pos->size();
        };
        ...
        if (make.size() > 0) {
            for (const auto& corp :
                 { "Canon", "NIKON", "EPSON", "KODAK", "Kodak", "OLYMPUS", "PENTAX",
                   "RICOH", "MINOLTA", "Minolta", "Konica", "CASIO", "Sinar", "Phase One",
                   "SAMSUNG", "Mamiya", "MOTOROLA", "Leaf", "Panasonic" })
            {
        ...
        if (find_tag(Exiv2::isoSpeed)) {
            // If standard exif iso tag, use the old way of interpreting the return value
            // to be more regression-save
        ...
        // Some newer cameras support iso settings that exceed the 16 bit of exif's
        // ISOSpeedRatings
        if (iso_speed == 65535 || iso_speed == 0) {
            if (find_exif_tag("Exif.PentaxDng.ISO") || find_exif_tag("Exif.Pentax.ISO")) {
                iso_speed = std::atof(pos->print().c_str());
            } else if ((make == "SONY" || make == "Canon")
        ...
        if (find_tag(Exiv2::serialNumber)) {
            ...
            if (serial_number_tags.cend()
                != std::find_if(serial_number_tags.cbegin(), serial_number_tags.cend(),
                                find_exif_tag))
            {
                serial = validateUft8(pos->toString());
            } else if (find_exif_tag("Exif.Minolta.WBInfoA100")
                       || find_exif_tag("Exif.SonyMinolta.WBInfoA100"))
            {
            ...
            } else if (find_exif_tag("Exif.Pentax.CameraInfo")
                       || find_exif_tag("Exif.PentaxDng.CameraInfo"))
            {
                const long index = 4;
                if (pos->count() >= index) {
        ...
        if (sscanf(datetime_taken.c_str(), "%d:%d:%d %d:%d:%d", &time.tm_year,
                ...
                } else if (std::regex_match(line, m, std::regex("Shutter: +([0-9.]+) *")))
                {
                    ...
                    if (shutter) {
                        shutter = 1.0 / shutter;
                                            ...
                                            std::regex("Lens \\(mm\\): +([0-9.]+) *")))
                {
                    focal_len = d();
                } else if (std::regex_match(line, m,
                                            std::regex("Exp Comp: +([0-9.]+) *")))
                {
                    expcomp = d();
                }
            }
        ...
        const auto pi =
            exif.findKey(Exiv2::ExifKey("Exif.Image.PhotometricInterpretation"));
        const auto c = exif.findKey(Exiv2::ExifKey("Exif.Image.Compression"));

        if (!make.compare(0, 6, "PENTAX")
            || (!make.compare(0, 5, "RICOH") && !model.compare(0, 6, "PENTAX")))
        {
            ...
            if (find_exif_tag("Exif.SubImage1.BitsPerSample") && to_long(pos) == 14) {
                    ...
                    && find_exif_tag("Exif.SubImage1.Compression") && to_long(pos) == 1)
                {
                    isPixelShift = true;
                       ...
                       && spp != exif.end() && to_long(spp) == 4 && c != exif.end()
                       && to_long(c) == 1 && find_exif_tag("Exif.Image.Software")
                       && pos->toString() == "make_arq")
            {
                isPixelShift = true;
            }
        } else if (make == "FUJIFILM") {
            if (bps != exif.end() && to_long(bps) == 16 && spp != exif.end()
                && to_long(spp) == 4 && c != exif.end() && to_long(c) == 1
                && find_exif_tag("Exif.Image.Software") && pos->toString() == "make_arq")
            {
                isPixelShift = true;
            }
        }
...
#if PRINT_HDR_PS_DETECTION
                        printf(
                            "PixelShift detected ! -> \"Make\" = SONY, bitsPerPixel > 8, samplesPerPixel == 4\n");
#endif
                    }
                }
            }
        ...
        if (find_exif_tag("Exif.Image.DNGVersion") && pos->count() == 4) {
            for (int i = 0; i < 4; i++) {
                dngVersion =
                    (dngVersion << 8) + static_cast<std::uint32_t>(to_long(pos, i));
            }
        }
        ...
        if (dngVersion
            && (find_exif_tag("Exif.SubImage1.OpcodeList1")
                || find_exif_tag("Exif.Image.OpcodeList1")))
        {
            readOpcodesList(pos->value(), &fixBadPixelsConstant,
                            &hasFixBadPixelsConstant_, nullptr);
        }
        // Read DNG OpcodeList2.
        if (dngVersion
            && (find_exif_tag("Exif.SubImage1.OpcodeList2")
                || find_exif_tag("Exif.Image.OpcodeList2")))
        {
...
double FramesMetaData::shutterFromString(std::string s)
{
    ...
    if (i == std::string::npos) {
        ...
        return denominator ? std::atof(s.substr(0, i).c_str()) / denominator : 0.0;
    }
}
...
template <class T> void set_exif(Exiv2::ExifData& exif, const std::string& key, T val)
{
    try {
        exif[key] = val;
...
void FramesData::fillBasicTags(Exiv2::ExifData& exif) const
{
    ...
    set_exif(exif, "Exif.Photo.FNumber",
    ...
    set_exif(exif, "Exif.Photo.ExposureBiasValue",
