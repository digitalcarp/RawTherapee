...
protected:
    ...
    Image8* cropImg;  // "one chunk" allocation ; displayed image in monitor color space,
                      // showing the output profile as well (soft-proofing enabled, which
                      // then correspond to workimg) or not
    float* shbuf_real;  // "one chunk" allocation

    // --- automatically allocated and deleted when necessary, and only renewed on size
    // changes
    ...
    bool newUpdatePending;  /// Flag telling the updater thread that a new update is
                            /// pending
    int skip;
    int cropx, cropy, cropw,
        croph;  /// size of the detail crop image ('skip' taken into account), with border
    int trafx, trafy, trafw, trafh;  /// the size and position to get from the imagesource
                                     /// that is transformed to the requested crop area
    int rqcropx, rqcropy, rqcropw,
        rqcroph;  /// size of the requested detail crop image (the image might be smaller)
                  /// (without border)
    ...
    bool setCropSizes(const int cropX,
                      const int cropY,
                      const int cropW,
                      const int cropH,
                      const int skip,
