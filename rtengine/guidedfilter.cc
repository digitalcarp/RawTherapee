...
void guidedFilter(const array2D<float>& guide,
...
#pragma omp parallel for if (multithread)
    ...
    const auto f_subsample = [=](array2D<float>& d, const array2D<float>& s) -> void {
...
#pragma omp parallel for if (multithread)
    ...
    const auto f_mean = [multithread](array2D<float>& d, array2D<float>& s,
        ...
        boxblur(static_cast<float**>(s), static_cast<float**>(d), rad, s.getWidth(),
                s.getHeight(), multithread);
    };
...
void guidedFilterLog(const array2D<float>& guide,
                     float base,
                     array2D<float>& chan,
                     int r,
                     float eps,
...
void guidedFilterLog(float base,
                     array2D<float>& chan,
                     int r,
                     float eps,
                     bool multithread,
